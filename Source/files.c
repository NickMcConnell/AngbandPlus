...
static void display_player_equippy(int y, int x)
{
	...
	for (i = INVEN_WIELD1; i < INVEN_TOTAL; ++i)
	{
		...
		/* Dump */
		Term_putch(x+i-INVEN_WIELD1, y, a, c);
	}
}
...
static cptr display_player_flag_names[4][8] =
{
...

	{
		...
		/*
		 *The TR4 flags aren't described here because
		 *doing so would require redesign of the play 
		 *with object screen
		 */
};
...
static void display_player_flag_info(void)
{
	...
	for (x = 0; x < 4; x++)
	{
		...
		for (y = 0; y < 8; y++)
		{
			...
			/* Check equipment */
			for (n = 6, i = INVEN_WIELD1; i < INVEN_TOTAL; ++i, ++n)
			{
...
/*
 * This is the XML-esque help system from ToME (2.0.0) 
 * This probably slows down the help system, but I find
 * it far more convenient to use.  
 */
