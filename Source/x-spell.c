...
/*#define SPELL_BEDLAM                    40	I've heard this spell is totally worthless once you get it*/
#define SPELL_LAW_STRIKE				40	/* here's the replacement */
#define SPELL_REND_SOUL                 41	/* I think this spell is out of charachter, but I won't replace it yet */
...
#define SPELL_FLOOD						64
#define SPELL_GLOBE_OF_INVULNERABILITY	65
#define SPELL_BLASTING					66
...
static bool cast_mage_spell(int spell)
{
	...
	switch (spell)
	{
		...
		{
			if (!get_aim_dir(&dir)) return (FALSE);
			fire_bounce(GF_ELEC, dir,
			          damroll(3+((plev-5)/6), 6), (plev/10 + 1), (plev/10 + 2));
			break;
		}
		...
		{
			if (!get_aim_dir(&dir)) return (FALSE);
			fire_bolt_or_beam(beam, GF_LAW, dir, damroll(13, plev));
			break;
		}
