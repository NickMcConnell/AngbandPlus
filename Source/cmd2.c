...
void do_cmd_load()
{
	...
	/* Buffers for the item descriptions */
	...
	object_desc(o_name2, &(inventory[INVEN_SHOTS]), FALSE, 0);

	if (inventory[INVEN_SHOTS].tval == p_ptr->ammo_tval && !object_similar(i_ptr, &inventory[INVEN_SHOTS])) 
	{
		msg_format("Your %s does not match the %s already in your weapon", o_name1, o_name2);
		return;
	}
	else
	{
		if (inventory[INVEN_SHOTS].k_idx) object_absorb(&inventory[INVEN_SHOTS], i_ptr);
		else object_copy(&inventory[INVEN_SHOTS], i_ptr);
		j_ptr->loaded += 1;
	}
...
void do_cmd_fire(void)
{
	...
	object_desc(j_name, j_ptr, FALSE, 3);
	...
	if (inventory[INVEN_SHOTS].tval == p_ptr->ammo_tval) item = INVEN_SHOTS;
	else
	{
		/* Get an item */
		q = "Fire which item? ";
...
void do_cmd_throw(void)
{
	...
	{
		if ((hit_body) || (randint(100) < j))
		{
			/* Message */
			msg_format("The %s shatters!", o_name);
			(void)potion_smash_effect(-1,y,x,k_info[o_ptr->k_idx].sval);
			return;
		}
		else
		{
