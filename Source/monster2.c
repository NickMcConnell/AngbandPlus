...
static bool summon_specific_okay(int r_idx)
{
	...
	switch (summon_specific_type)
	{
		...
		case SUMMON_HOUND:
		{
			okay = (strchr(MON_HOUND, r_ptr->d_char) &&
			        !(r_ptr->flags1 & (RF1_UNIQUE)));
			break;
		}
		...
		case SUMMON_HYDRA:
		{
			okay = (strchr(MON_HYDRA, r_ptr->d_char) &&
			        !(r_ptr->flags1 & (RF1_UNIQUE)));
			break;
		}
		...
		case SUMMON_NONLIFE:
		{
			okay = (strchr(MON_NONLIFE, r_ptr->d_char) &&
			        !(r_ptr->flags1 & (RF1_UNIQUE)));
			break;
		}
		...
		{
			okay = (strchr(MON_HIGH_UNDEAD, r_ptr->d_char));
			break;
		}

		case SUMMON_HI_DRAGON:
		{
			okay = (strchr(MON_HIGH_DRAGON, r_ptr->d_char));
			break;
		}

		case SUMMON_HI_DEMON:
		{
			okay = (strchr(MON_HIGH_DEMON, r_ptr->d_char));
			break;
		}

		case SUMMON_WRAITH:
		{
			okay = ((strchr(MON_RINGWRAITH, r_ptr->d_char)) &&
			        (r_ptr->flags1 & (RF1_UNIQUE)));
			break;
		}
...
bool summon_specific(int y1, int x1, int lev, int type)
{
	...
	for (i = 0; i < 20; ++i)
	{
		...
		/* now 1/4 of time WILL summon onto wards, but not break them */
		if (cave_feat[y][x] == FEAT_GLYPH && randint(4) != 1) continue;
...
void message_pain(int m_idx, int dam)
{
	...
	if (strchr(MON_MUCKY_NOISE, r_ptr->d_char))
	{
		if (percentage > 95)
	...
	/* Dogs and Hounds */
	else if (strchr(MON_HOUND, r_ptr->d_char))
	{
		if (percentage > 95)
	...
	/* Voiced monsters (shrug,cry,scream) */
	else if (strchr(MON_VOICE, r_ptr->d_char))
	{
		if (percentage > 95)
			msg_format("%^s shrugs off the attack.", m_name);
		else if (percentage > 75)
		...
		/* Other monsters (ignore,squeal,shriek) */
	else
	{
		if (percentage > 95)
...
void update_smart_learn(int m_idx, int what)
{
	...
	switch (what)
	{
		...
		case DRS_RES_LAW:
		{
			if (p_ptr->resist_law) m_ptr->smart |= (SM_RES_LAW);
			break;
		}
	}
