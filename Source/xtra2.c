...
bool set_invuln(int v)
{
	...
	/* Hack -- Force good values */
/*	v = (v > 10000) ? 10000 : (v < 0) ? 0 : v; no longer necessary */
	...
	/* Use the value * /
	p_ptr->invuln = v;  not doing this anymore*/
...
void check_experience(void)
{
	                       ...
	                       p_ptr->expfact / 100L)))
	{
		...
		if ((p_ptr->lev)>30 && (p_ptr->lev)%5 == 0)get_new_realm();
...
static void look_mon_desc(char *buf, int m_idx)
{
	...
	if (strchr(MON_NONLIFE, r_ptr->d_char)) living = FALSE;
