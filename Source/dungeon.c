...
static void sense_inventory(void)
{
	...
	for (i = 0; i < INVEN_TOTAL; i++)
	{
		...
		if ((i < INVEN_WIELD1) && (0 != rand_int(5))) continue;
...
static void process_world(void)
{
	...
	if (p_ptr->invuln)
	{
		(void)set_invuln(0);/* no longer time limited, checking in function, the 0 just tells it not to set if not already on */
	}
	...
	/* Process equipment */
	for (j = 0, i = INVEN_WIELD1; i < INVEN_TOTAL; i++)
	{
		/* Get the object */
		o_ptr = &inventory[i];
...
static void process_command(void)
{
	...
	switch (p_ptr->command_cmd)
	{
		...
		case 'm':
		{
			if(cp_ptr->spell_book == TV_MAGIC_BOOK) do_cmd_cast();
			else if(cp_ptr->spell_book == TV_PRAYER_BOOK) do_cmd_pray();
			else do_cmd_cast();
			break;
		}
