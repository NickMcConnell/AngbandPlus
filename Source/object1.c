	...
	if (mode != OBJECT_FLAGS_RANDOM)
	{
		...
		if (mode == OBJECT_FLAGS_KNOWN)
		{
			...
			if (o_ptr->name1)
			{
				...
				(*f4) = (a_ptr->flags4 & (TR4_MUNDANE_MASK));
			}
		}
	}
...
void object_desc(char *buf, const object_type *o_ptr, int pref, int mode)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_SHIELD: /* Shields will now count as weapons, but still be classified as armor for enchanting purposes. */
		{
			show_weapon = TRUE;
			show_armour = TRUE;
			break;
	...
	switch (o_ptr->tval)
	{
		...
		case TV_BOW:
		{
			...
			if ((&k_info[o_ptr->k_idx])->flags4 & TR4_XBOW)
			{
				if (o_ptr == &inventory[INVEN_BOW])
				{
					object_desc_str_macro(t, " (");
					object_desc_num_macro(t, p_ptr->num_fire);
					object_desc_str_macro(t, " loaded)");
				}
				else object_desc_str_macro(t, " (unloaded)");
			}
...
static bool identify_fully_aux2(const object_type *o_ptr, int mode)
{
	...
	/* Mega-Hack -- describe mage weapons */
	if (f4 & TR4_MAGE)
	{
		int realm;
		int f4b;
		realm = 0;
		f4b = f4;
		text_out("   It allows you to access the realm of ");
			f4b = f4b / (TR4_MAGE * 2);
			f4b = f4b % (2<<HREALM_MAX);
			while(f4b > 1) /* base 2 log */
			{
				f4b = f4b >> 1;
				realm++;
			} /* note that realm will be only the 'highest' realm flag on the item. */
/* also note that items with the TR4_MAGE flag but no realm flags will default to TR4_MAGE_LIGHTDARK */
/* Since the same behavior occurs in the use of these items this is fine.   */
			switch (realm)
			{
			case TR4_MAGE_LIGHTDARK:
				text_out("light/dark.");
				break;
			case TR4_MAGE_FIRECOLD:
				text_out("fire/cold.");
				break;
			case TR4_MAGE_WATER:
				text_out("water.");
	...
	if (f4 & (TR4_PRIMARY))
	{
		text_out("   It is too cumbersome to use left handed");
		known = TRUE;
	}
	if (f4 & (TR4_SECONDARY))
	{
		text_out("   It is of limited utility for attacking");
		known = TRUE;
	}
	if (f4 & (TR4_2H))
	{
		text_out("   It takes both hands to weild");
		known = TRUE;
	}
	if (f4 & (TR4_XBOW))
	{
		text_out("   It is cumbersome to reload");
		known = TRUE;
	}
...
char index_to_label(int i)
{
	...
	if (i < INVEN_WIELD1) return (I2A(i));

	/* Indexes for "equip" are offset */
	return (I2A(i - INVEN_WIELD1));
}
...
s16b label_to_equip(int c)
{
	...
	/* Convert */
	i = (islower(c) ? A2I(c) : -1) + INVEN_WIELD1;

	/* Verify the index */
	if ((i < INVEN_WIELD1) || (i >= INVEN_TOTAL)) return (-1);
...
s16b wield_slot(const object_type *o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_POLEARM:
		case TV_SWORD:
		case TV_SHIELD:/* Shields now count as weapons */
		{
			if ((&k_info[o_ptr->k_idx])->flags4 & TR4_PRIMARY) return (INVEN_WIELD1);
			if ((&k_info[inventory[INVEN_WIELD1].k_idx])->flags4 & TR4_2H)
			{
				msg_print("Your current weapon takes both hands");
				return  -1;
			}
			if ((&k_info[o_ptr->k_idx])->flags4 & TR4_SECONDARY) return (INVEN_WIELD2);
			if ((&k_info[o_ptr->k_idx])->flags4 & TR4_2H);
			{
				if (!inventory[INVEN_WIELD2].k_idx) return (INVEN_WIELD1);
				else 
				{
					msg_print("put down your shield/second weapon first");
					return (-1);
				}
			}
			if (inventory[INVEN_WIELD1].k_idx && !((&k_info[o_ptr->k_idx])->flags4 & TR4_PRIMARY)  && !((&k_info[o_ptr->k_idx])->flags4 & TR4_2H) )
				return (INVEN_WIELD2);
			/* Use the primary wield first */
			if (!inventory[INVEN_RIGHT].k_idx) return (INVEN_WIELD1);

			/* Use the secondary wield for swapping (by default) */
			return (INVEN_WIELD2);
		}
...
cptr mention_use(int i)
{
	...
	switch (i)
	{
		case INVEN_WIELD1:p = "Wielding"; break;
		...
		case INVEN_WIELD2:p = "Wielding"; break;
		case INVEN_HEAD:  p = "On head"; break;
		case INVEN_HANDS: p = "On hands"; break;
		case INVEN_FEET:  p = "On feet"; break;
		case INVEN_SHOTS: p = "Loaded"; break;
		default:          p = "In pack"; break;
	}

	/* Hack -- Heavy weapon */
	if (i == INVEN_WIELD1 || INVEN_WIELD2)
	{
		object_type *o_ptr;
		o_ptr = &inventory[i];
...
cptr describe_use(int i)
{
	...
	switch (i)
	{
		case INVEN_WIELD1:p = "attacking monsters with"; break;
		...
		case INVEN_WIELD2:p = "defending yourself with"; break;
		case INVEN_HEAD:  p = "wearing on your head"; break;
		case INVEN_HANDS: p = "wearing on your hands"; break;
		case INVEN_FEET:  p = "wearing on your feet"; break;
		case INVEN_SHOTS: p = "carrying in your crossbow"; break;
...
void display_equip(void)
{
	...
	for (i = INVEN_WIELD1; i < INVEN_TOTAL; i++)
	{
		/* Examine the item */
		o_ptr = &inventory[i];
		...
		/* Display the index (or blank space) */
		Term_putstr(0, i - INVEN_WIELD1, 3, TERM_WHITE, tmp_val);
		...
		/* Display the entry itself */
		Term_putstr(3, i - INVEN_WIELD1, n, attr, o_name);

		/* Erase the rest of the line */
		Term_erase(3+n, i - INVEN_WIELD1, 255);

		/* Display the slot description (if needed) */
		if (show_labels)
		{
			Term_putstr(61, i - INVEN_WIELD1, -1, TERM_WHITE, "<--");
			Term_putstr(65, i - INVEN_WIELD1, -1, TERM_WHITE, mention_use(i));
		}
		...
		if (show_weights && o_ptr->weight)
		{
			...
			sprintf(tmp_val, "%3d.%1d lb", wgt / 10, wgt % 10);
			Term_putstr(col, i - INVEN_WIELD1, -1, TERM_WHITE, tmp_val);
		}
	}

	/* Erase the rest of the window */
	for (i = INVEN_TOTAL - INVEN_WIELD1; i < Term->hgt; i++)
	{
		/* Clear that line */
		Term_erase(0, i, 255);
	}
...
void show_equip(void)
{
	...
	for (k = 0, i = INVEN_WIELD1; i < INVEN_TOTAL; i++)
	{
		o_ptr = &inventory[i];
	...
	while (!done)
	{
		...
		switch (which)
		{
			...
			case '7': case '8': case '9':
			{
				...
				if ((k < INVEN_WIELD1) ? !allow_inven : !allow_equip)
				{
					bell("Illegal object choice (tag)!");
					break;
				}
