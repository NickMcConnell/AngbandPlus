...
static void wr_randarts(void)
{
	...
	for (i = 0; i < z_info->a_max; i++)
	{
		...
		/* because adding a byte for flags4 would break savefile
		 * compatability, and all that is currently in flags4 are
		 * the handedness flags, all randarts will get their 
		 * handedness solely from the base objects.    */
...
static bool wr_savefile_new(void)
{
	...
	wr_byte(p_ptr->realm3);
	wr_u16b(PY_MAX_SPELLS);

	for (i = 0; i < PY_MAX_SPELLS; i++)
	{
		wr_byte(p_ptr->h_spell_flags[i]);
	}

	/* Dump the ordered hspells */
	for (i = 0; i < PY_MAX_SPELLS; i++)
	{
		wr_byte(p_ptr->h_spell_order[i]);
	}
