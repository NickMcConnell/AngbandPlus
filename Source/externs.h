...
extern bool fire_bounce(int typ, int dir, int dam, int rad, int turns);
...
extern void ring_of_power(int dir);
	/* NB */
extern bool fire_flood(int num, int typ, int dir, int dam);
extern bool fire_spray(int typ, int dir, int dam, int rad);
...
/* h-spell stuff */
extern bool cast_high(int spell, int arg);/* h-spell.c */
extern void do_cmd_recall_hs(void);/* cmd6.c */
extern void do_cmd_high(void);/* cmd6.c */
extern int beam_chance();/* in x-spell.c, but needed in h-spell.c as well. */
extern const char *realm_names[HREALM_MAX];
extern const magic_type high_info[PY_MAX_SPELLS];
extern const u32b h_spell_group[10][2];
extern void get_new_realm();
extern void hspell_aux(int realm);
extern cptr get_h_spell_name(int spell);
extern void print_realms(int y, int x);
extern cptr get_h_spel_info(int spell);


/*
 * externs for stuff modified from TOME
 */

void potion_smash_effect(int who,int y,int x,int sval);
