...
static byte spell_color(int type)
{
	...
	switch (type)
	{
		...
		case GF_ACID:		return (randint(5)<3?TERM_SLATE:TERM_YELLOW);
		...
		case GF_HOLY_ORB:	return (randint(5)==1?TERM_GREEN:TERM_WHITE);
		case GF_MANA:		return (randint(5)!=1?TERM_VIOLET:TERM_L_DARK);
		...
		case GF_CHAOS:		return (randint(4)==1?TERM_VIOLET:TERM_L_DARK);
		case GF_DISENCHANT:	return (TERM_VIOLET);
		case GF_NEXUS:		return (randint(5)<3?TERM_L_RED:TERM_VIOLET);
		case GF_CONFUSION:	return (randint(3)==1?TERM_L_UMBER:TERM_UMBER);
		case GF_SOUND:		return (TERM_YELLOW);
		case GF_SHARD:		return (randint(5)<3?TERM_UMBER:TERM_SLATE);
		case GF_FORCE:		return (randint(5)<3?TERM_UMBER:TERM_L_WHITE);
		case GF_INERTIA:	return (randint(5)<3?TERM_SLATE:TERM_L_WHITE);
		case GF_GRAVITY:	return (randint(3)==1?TERM_L_UMBER:TERM_UMBER);
		case GF_TIME:		return (randint(3)==1?TERM_RED:TERM_L_BLUE);
		case GF_LITE_WEAK:	return (TERM_ORANGE);
		case GF_LITE:		return (randint(3)==1?TERM_ORANGE:TERM_YELLOW);
		...
		case GF_LAW:		return (randint(4)==1?TERM_L_BLUE:TERM_L_DARK);
		case GF_ELEM:		return (randint(3)==1?TERM_RED:(randint(2)==1?TERM_BLUE:TERM_GREEN));
	}
...
static int minus_ac(void)
{
	...
	switch (randint(6))
	{
		...
		case 2: o_ptr = &inventory[INVEN_WIELD2]; break;
 ...
 *
 * 0 normal disenchant
 * 1 disenchant that can effect pval
 * 2 random enchant/boost
 * 3 random enchant/boost that can effect pval
 */
bool apply_disenchant(int mode)
{
	...
	/* Disenchant tohit */
	if ((mode > 1) && (rand_int(100) < 20)) o_ptr->to_h++;
	if ((mode > 1) && (o_ptr->to_h < 5) && (rand_int(100) < 20)) o_ptr->to_h++;
	if (o_ptr->to_h > 0) o_ptr->to_h--;
	if ((o_ptr->to_h > 5) && (rand_int(100) < 20)) o_ptr->to_h--;

	/* Disenchant todam */
	if ((mode > 1) && (rand_int(100) < 20)) o_ptr->to_d++;
	if ((mode > 1) && (o_ptr->to_d < 5) && (rand_int(100) < 20)) o_ptr->to_d++;
	if (o_ptr->to_d > 0) o_ptr->to_d--;
	if ((o_ptr->to_d > 5) && (rand_int(100) < 20)) o_ptr->to_d--;

	/* Disenchant toac */
	if ((mode > 1) && (rand_int(100) < 20)) o_ptr->to_a++;
	if ((mode > 1) && (o_ptr->to_a < 5) && (rand_int(100) < 20)) o_ptr->to_a++;
	if (o_ptr->to_a > 0) o_ptr->to_a--;
	if ((o_ptr->to_a > 5) && (rand_int(100) < 20)) o_ptr->to_a--;

	/* Disenchant pval */
	if((mode == 1) || (mode == 3) && (rand_int(100) < 5))
	{
		if ((mode > 1) && (rand_int(100) < 20)) o_ptr->pval++;
		if ((mode > 1) && (o_ptr->pval < 5) && (rand_int(100) < 20)) o_ptr->pval++;
		if (o_ptr->to_h > 0) o_ptr->pval--;
		if ((o_ptr->to_h > 5) && (rand_int(100) < 20)) o_ptr->pval--;
	}
...
/*		case GF_FORCE: */
		...
		/* these now can damage the dungeon, creating rubble piles 
		 * this can only effect totaly vacant grids because to do 
		 * otherwise would require too much work */
		case GF_METEOR:
		case GF_FORCE:
		case GF_SHARD:
		{
			...
			{
				if (randint(100) < 20) cave_feat[y][x] = FEAT_RUBBLE;
			}
		}
		...
		case GF_SHATTER:
		{
			switch(cave_feat[x][y])
			case FEAT_OPEN:
			case FEAT_SECRET:
			{
				cave_feat[x][y] = FEAT_BROKEN;
				break;
			}
			case FEAT_MAGMA:
			case FEAT_MAGMA_H:
			...
			{
				cave_feat[x][y] = FEAT_RUBBLE;
				break;
			}
			default:
			{
				if (cave_feat[x][y] > FEAT_TRAP_HEAD && cave_feat[x][y] < FEAT_DOOR_TAIL)
				{
					cave_feat[x][y] = FEAT_BROKEN;
				}
				break;
	...
	for (this_o_idx = cave_o_idx[y][x]; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		if (do_kill)
		{
			...
			else
			{
				...
				{
					(void)potion_smash_effect(who,y,x,k_info[o_ptr->k_idx].sval);
				}
...
/* helper function for project_m and project_p.  may be also called from melee code eventualy. */
void knockback(int dist, int who,int whom)
{ 
	int a, b, x1, y1, x2, y2, i, x0, y0;
	monster_type *m_ptr;
	bool legal;
	char killer[80];

	if (who) monster_desc(killer, &m_list[who], 0x88);

	i = dist;
	m_ptr = &m_list[whom];
	x0 = (who>0?m_list[who].fx:p_ptr->px);
	y0 = (who>0?m_list[who].fy:p_ptr->py);
	while (i)
	{
		a = 0;
		b = 0;
		/* Get vector from firer to target */
		x1 = (whom>0?(m_ptr->fx - x0):(p_ptr->px-x0) * 10);
		y1 = (whom>0?(m_ptr->fy - y0):(p_ptr->py-y0) * 10);
...
/* can we move the target? */
		if (1)/* no control structure, braces needed for break statements.  */
		{
			legal = FALSE;
			x2 = (whom>0?(m_ptr->fx):(p_ptr->px)) + b;
			y2 = (whom>0?(m_ptr->fy):(p_ptr->py)) + a;

			if (!in_bounds(y2,x2) || !cave_empty_bold(y2, x2)) continue; 
			if (whom && cave_feat[y2][x2] == FEAT_GLYPH) break;
		...
		if (legal)
		{
			monster_swap((whom>0?(m_ptr->fy):(p_ptr->py)), (whom>0?(m_ptr->fx):(p_ptr->px)), y2, x2);
		}
		else
		{
			project_f(who, 1, y2, x2, 0, GF_SHATTER);
			if (who>0) take_hit(dist, killer);
			else if (whom>0) mon_take_hit(whom, dist, &(m_ptr->monfear),"is crushed");
			else m_list[whom].hp -= dist;
		}
		/* since this should only be called from monster hurting functions, death should be checked for when the rest of the damage is done.  */
	...
	switch (typ)
	{
		...
		case GF_ACID:
		{
			...
			if (r_ptr->flags3 & (RF3_RES_ACID))
			{
				...
				dam /= 3;
				if (seen) l_ptr->flags3 |= (RF3_RES_ACID);
			}
			break;
		}
		...
		case GF_WATER:
		{
			...
			else knockback( dam/(m_ptr->maxhp)+1, who, cave_m_idx[y][x]);
			break;
		}
		...
		case GF_OLD_DRAIN:
		{
			...
			if ((r_ptr->flags3 & (RF3_UNDEAD)) ||
			    ...
			    (strchr(MON_NONLIFE, r_ptr->d_char)))
			{
				if (r_ptr->flags3 & (RF3_UNDEAD))
				{
					if (seen) l_ptr->flags3 |= (RF3_UNDEAD);
		...
		case GF_LAW:
		{
			...
			else if (strchr(MON_NONLIFE, r_ptr->d_char))
			{
				dam = - dam;
				note = " seems stronger.";
				obvious = FALSE;
...
static bool project_p(int who, int r, int y, int x, int dam, int typ)
{
	...
	switch (typ)
	{
		...
		case GF_FIRE:
		{
			if (fuzzy) msg_print("You are hit by something burning!");
			fire_dam(dam, killer);
			break;
		}
		...
		case GF_CHAOS:
		{
			...
			if (p_ptr->resist_disen || p_ptr->resist_chaos)
			{
				dam *= 6; dam /= (randint(6) + 6);
			}
			else
			{
				(void)apply_disenchant(1);
			}
			take_hit(dam, killer);
			break;
		}
		...
		case GF_LAW:
		{
			...
			if (!p_ptr->resist_law)
			{
				(void)set_confused(p_ptr->confused - rand_int(20));
			}
			if (!p_ptr->resist_law)
			{
				(void)set_image(0);
			}
			if (!p_ptr->resist_nethr && !p_ptr->resist_law)
			{
				if (p_ptr->hold_life && (rand_int(100) < 75))
				{
					msg_print("You keep hold of your life force!");
			...
			if (p_ptr->resist_disen || p_ptr->resist_law)
			{
				dam *= 6; dam /= (randint(6) + 6);
			}
			else
...
void potion_smash_effect(int who, int y, int x, int o_sval)
 {
	int     radius = 2;
	int     dt = 0;
	int     dam = 0;
