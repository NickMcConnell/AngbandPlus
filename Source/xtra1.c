...
static void prt_equippy(void)
{
	...
	for (i = INVEN_WIELD1; i < INVEN_TOTAL; i++)
	{
		...
		/* Dump */
		Term_putch(COL_EQUIPPY + i - INVEN_WIELD1, ROW_EQUIPPY, a, c);
	}
}
...
static void calc_mana(void)
{
	...
	/* Weigh the armor   NB shields are no longer armor*/
	cur_wgt = 0;
...
static void calc_bonuses(void)
{
	...
	bool old_heavy_wield1;
	bool old_heavy_wield2;
	bool old_icky_wield;
	...
	old_heavy_wield1 = p_ptr->heavy_wield1;
	old_heavy_wield2 = p_ptr->heavy_wield2;
	...
	if (f2 & (TR2_RES_LAW)) p_ptr->resist_law = TRUE;
...
/*
 *   IMPORTANT!!!
 *   Any changes in the handling of BLOWS bonuses, especially with regards
 *   to the use of p_ptr->heavy_wield1 or p_ptr->heavy_wield2 may
 *   necessitate alteration of cmd1.c
 *   the area that needs to be checked is where NB_FLAG0 is defined.  
 */
	...
	/* Normal weapons */
	if (o_ptr->k_idx && !p_ptr->heavy_wield1)
	{
		...
		/* Add in the "bonus blows" */ /* now re-done in attack code, just for display and non-weapon boosters here */
	...
	/* Normal weapons */
	if (o_ptr->k_idx && !p_ptr->heavy_wield2)
	{
		int str_index, dex_index;
		...
		/* Require at least one blow */
		if (p_ptr->num_blow < 1) p_ptr->num_blow = 1;
		/* if we don't have zero blows before limit application, make sure there are at least two. */
		else if (p_ptr->num_blow <2) p_ptr->num_blow = 2;
	}
	...
	if (old_heavy_wield1 != p_ptr->heavy_wield1 || old_heavy_wield2 != p_ptr->heavy_wield2)
	{
		/* Message */
		...
		else if (p_ptr->heavy_wield2)
		{
			if (inventory[INVEN_WIELD2].tval == TV_SHIELD)
					msg_print("You have trouble wielding such a heavy shield.");
			else	msg_print("You have trouble wielding such a heavy weapon.");
		}
		else if (inventory[INVEN_WIELD1].k_idx || inventory[INVEN_WIELD2].k_idx)
		{
	...
	if (old_icky_wield != p_ptr->icky_wield)
	{
		...
		else if (inventory[INVEN_WIELD1].k_idx)
		{
