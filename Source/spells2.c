...
void self_knowledge(void)
{
	...
	for (k = INVEN_WIELD1; k < INVEN_TOTAL; k++)
	{
		u32b t1, t2, t3, t4;
	...
	if (p_ptr->resist_law)
	{
		info[i++] = "You are resistant to law.";
	}
	...
	if (f4 & (TR4_XBOW))
	{
		if (((&inventory[INVEN_BOW])->loaded))
			info[i++] = "Your weapon is loaded.";
		else
			info[i++] = "Your weapon is unloaded.";
	}
...
#else
			o_ptr->pval=0;
			if (o_ptr->tval == TV_WAND)o_ptr->sval = SV_WAND_FRIED;
			if (o_ptr->tval == TV_STAFF)o_ptr->sval = SV_STAFF_FRIED;
#endif
...
/*
 * Cast multiple non-jumping ball spells of increasing size at the same 
 * target.
 *
 * Targets absolute coordinates instead of a specific monster, so that
 * the death of the monster doesn't change the target's location.
 */
bool fire_flood(int num, int typ, int dir, int dam)
{
	bool noticed = FALSE;
	...
	{
		/* Analyze the "dir" and the "target".  Hurt items on floor. */
		if (project(-1, r++, ty, tx, dam, typ, flg)) noticed = TRUE;
	}
...
/*
 * sends a beam at a grid, the beam then splits and hits every monster within rad of the target.  
 * this repeats for turns splittings.  each ray does one damage less than the previous one, and 
 * when dam == 0 the attack stops regardless of turns.  
 */
bool fire_bounce(int typ, int dir, int dam, int rad, int turns)
{
	bool noticed = FALSE;
		...
		for (dist = 0; dist <= rad; dist++)
		{
			...
			for (y = ty - dist; y <= ty + dist; y++)
			{
				for (x = tx - dist; x <= tx + dist; x++)
				{
					...
					if (distance(ty, tx, y, x) != dist) continue;	
	
					/* Ball explosions are stopped by walls */
					if (!los(ty, tx, y, x)) continue;	
		...
		for (t = 0; t <= rad; t++)
		{
			...
			for (i = gm[t]; i < gm[t+1]; i++)
			{
				...
				if (cave_m_idx[y][x] > 0)
				{
					ty = y;
					tx = x;
					if (project(cave_m_idx[y][x], 0, y, x, dam, typ, flg)) noticed = TRUE;
					dam--;
					if (dam == 0) return noticed;
				}
			}
		}
		turns--;
...
/*
 * sends a beam at each grid in the explosion area.  Destroys floor items.  
 */
bool fire_spray(int typ, int dir, int dam, int rad)
{
	bool noticed = FALSE;
	...
	for (t = 0; t <= rad; t++)
	{
		...
		for (i = gm[t]; i < gm[t+1]; i++)
		{
			...
			/* hack, some types now randomized */
			if (typ == GF_CHAOS)
			{
				switch(randint(10))
				{
			...
			if (project(-1, 0, y, x, dam, type, flg)) noticed = TRUE;
		}
	}
	return noticed;
}
