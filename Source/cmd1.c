 ...
 * 
 * Now also uses class flags CF_ASSASINATE and CF_HEAVY_CRIT.  
 */
sint critical_norm(int weight, int plus, int dam)
{
	...
	if (cp_ptr->flags & CF_HEAVY_CRIT) i = 3 * i;
		
	/* Chance */
	if (randint(5000) <= i)
	{
		if (cp_ptr->flags & CF_HEAVY_CRIT){ k = (p_ptr->lev * randint(weight * weight))/10;}
		else if (cp_ptr->flags & CF_ASSASINATE){ k = (p_ptr->lev * 5) + 100 + damroll(((weight * 4) - (weight * weight)/10 + 1), 6);}
		else{ k = weight + randint(650);}
...
void py_attack(int y, int x)
{
	...
	object_type *o_ptr, *w_ptr1, *w_ptr2;

	char m_name[80];

	bool fear = FALSE;
	bool assasinate = FALSE; /* NB */
	bool do_quake = FALSE;

	int blows_primary = 0;
	int blows_secondary = 0;
	...
	/*  Disturb the monster */
	assasinate = (m_ptr->csleep || m_ptr->confused);
	m_ptr->csleep = 0;
	...
	/* Get the weapon */
	w_ptr1 = &inventory[INVEN_WIELD1];
	w_ptr2 = &inventory[INVEN_WIELD2];

	/* Calculate the "attack quality" */
	bonus = p_ptr->to_h + w_ptr1->to_h;
	chance = (p_ptr->skill_thn + (bonus * BTH_PLUS_ADJ));
#define NB_FLAG0
	/* distribute blows between primary and secondary weapons */
	if (inventory[INVEN_WIELD2].k_idx)
	{
		int temp_blows = 0;
		int w1f1, w1f2, w1f3, w1f4, w2f1, w2f2, w2f3, w2f4;
		object_flags(w_ptr1, &w1f1, &w1f2, &w1f3, &w1f4);
		object_flags(w_ptr1, &w2f1, &w2f2, &w2f3, &w2f4);
		temp_blows = p_ptr->num_blow;

		/* remove weapon blows bonuses from total */
		if (w1f1 & TR1_BLOWS&& !p_ptr->heavy_wield1) temp_blows -= w_ptr1->pval;
		if (w2f1 & TR1_BLOWS&& !p_ptr->heavy_wield2) temp_blows -= w_ptr2->pval;

		/* distribute according to weapon weight */
		blows_primary = temp_blows - (w_ptr1->weight / w_ptr2->weight) * blows_secondary;
		blows_secondary = temp_blows - blows_primary;

		if (w1f1 & TR1_BLOWS && !p_ptr->heavy_wield1) blows_primary += w_ptr1->pval;
		if (w2f1 & TR1_BLOWS && !p_ptr->heavy_wield2) blows_primary += w_ptr2->pval;

		/* make sure it adds up */
		if (blows_primary + blows_secondary > p_ptr->num_blow)
		{
			if (blows_secondary >= ((blows_primary + blows_secondary)-p_ptr->num_blow))
			{
				blows_secondary -= 
					((blows_primary + blows_secondary)-p_ptr->num_blow);
			}
			else
			{
				blows_secondary = 0;
				blows_primary -= ((blows_primary + blows_secondary)-p_ptr->num_blow);
			}
		}
...
#undef NB_FLAG0
	...
	if (assasinate)
	{
		bonus += (cp_ptr->flags & CF_ASSASINATE ? (bonus * (min(w_ptr1->weight,w_ptr2->weight) >= 100 ? 1 : 100 - min(w_ptr1->weight,w_ptr2->weight)))/2 : bonus/2);
		chance += p_ptr->lev * 8;
		msg_format("You sneak up on the sleeping %s.", m_name);
	}
	...
	while (num++ < p_ptr->num_blow)
	{
		...
		{
			/* Message */
			if (o_ptr->tval==TV_SHIELD) message_format(MSG_HIT, m_ptr->r_idx, "You shield bash %s.", m_name);
			else message_format(MSG_HIT, m_ptr->r_idx, "You hit %s.", m_name);
