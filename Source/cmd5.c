...
void print_spells(const byte *spells, int num, int y, int x)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		/* Get the spell info */
		if (!high)s_ptr = &mp_ptr->info[spell];
		else s_ptr = &high_info[spell];
		...
		/* Get extra info */
		comment = high ? get_h_spel_info(spell) : get_spell_info(cp_ptr->spell_book, spell);
		...
		else if ((!(p_ptr->spell_flags[spell] & PY_SPELL_LEARNED)) && (!high))
		{
		...
		sprintf(out_val, "  %c) %-30s%2d %4d %3d%%%s",
			I2A(i), high ? get_h_spell_name(spell) : get_spell_name(cp_ptr->spell_book, spell),
...
int get_h_spell_index(int realm, int index)
{
	int spell;
	int num = 0;
	...
	for (spell = 0; spell < PY_MAX_SPELLS; spell++)
	{
		...
		if ((spell < 32) ?
		    (h_spell_group[realm][0] & (1L << spell)) :
		    (h_spell_group[realm][1] & (1L << (spell - 32))))
		{
			/* Found it */
...
static int get_h_spell(int realm)
{
	int i;
...
/*	const magic_type *s_ptr;*/
	...
	for (i = 0; i < 9; i++)
	{
		spell = get_h_spell_index(realm, i);
	...
	{
		/* Look for "okay" spells */
		if (h_spell_okay(spells[i])) okay = TRUE;
	}
	...
	while (!flag && get_com(out_val, &choice))
	{
		...
		/* Require "okay" spells */
		if (!h_spell_okay(spell))
		{
			bell("Illegal spell choice!");
			msg_format("You may not cast that spell.");
			continue;
		}
...
void hspell_aux(int realm)
{
	int arg, chance, spell;
	
	magic_type *s_ptr;
	
	spell = get_h_spell(realm);

	s_ptr = &high_info[spell];

	arg = get_quantity("at what level?", p_ptr->lev);
	chance = (100 - (p_ptr->lev - s_ptr->slevel + 
	...
	/* Sufficient mana */
	if ((s_ptr->smana * arg) <= p_ptr->csp)
	{
		/* Use some mana */
		p_ptr->csp -= (s_ptr->smana * arg);
	}
	...
	else
	{
		...
		/* Damage CON (possibly permanently) */
		if (rand_int(100) < 2 * arg)
		{
			bool perm = (rand_int(100) < arg);

			/* Message */
			msg_print("You have damaged your health!");

			/* Reduce constitution */
			(void)dec_stat(A_CON, 15 + randint(arg), perm);
		}
		if (rand_int(100) < 2 * arg)
		{
			bool perm = (rand_int(100) < arg);
			...
			/* Message */
			msg_print("You have a pounding headache!");

			/* Reduce constitution */
			(void)dec_stat(A_INT, 15 + randint(arg), perm);
		}
		if (rand_int(100) < 2 * arg)
		{
			bool perm = (rand_int(100) < arg);
			...
			/* Message */
			msg_print("You have trouble standing!");
...
/*	char buf[160];*/
	...
	if (!p_ptr->realm1)
	{
		msg_print("You cannot use high magic.");
		return;
	}
	...
	realms = 1;
	if (p_ptr->realm2) realms++;
	if (p_ptr->realm3) realms++;
