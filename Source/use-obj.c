...
static bool eat_food(object_type *o_ptr, bool *ident)
{
	...
	int sval = o_ptr->sval;
	/* XXX Hack -- Magic Mushroom can do anything before it */
	if (sval == SV_FOOD_MAGIC) sval = rand_int(SV_FOOD_MAGIC);
	/* Analyze the food */
	switch (sval)
	{
		case SV_FOOD_POISON:
...
{
	int k, dir, i, chance;
	bool mageart = FALSE; /* this allows artifacts to be mage weapons without being in the SV ranges for normal mage weapons. */
	...
	if (o_ptr->name1)
	{
		...
		switch (a_ptr->activation)
		{
			...
			case ACT_MANA:
			{
				msg_format("Your %s goes black and energy flows into you", o_name);
				if (p_ptr->csp < p_ptr->msp)
				{
					p_ptr->csp = p_ptr->msp;
					p_ptr->csp_frac = 0;
			...
			default:
			{
				mageart = TRUE; /* this tells us to look at it as a mage weapon activation */
			}
		}
		if (!mageart)
		{
	...
	if (o_ptr->tval == TV_DRAG_ARMOR)
	{
		...
		switch (o_ptr->sval)
		{
			...
			case SV_DRAGON_BRONZE:
			{
				...
				fire_spray(GF_CONFUSION, dir, 120, 2);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_CHAOS:
			{
				...
				fire_spray((chance == 1 ? GF_CHAOS : GF_DISENCHANT),
				          dir, 220, 2);
			...
			case SV_DRAGON_LAW:
			{
				...
				fire_spray((chance == 1 ? GF_SOUND : GF_SHARD),
				          dir, 230, 2);
				o_ptr->timeout = rand_int(300) + 300;
				break;
			}
			...
			case SV_DRAGON_SHINING:
			{
				...
				fire_spray((chance == 0 ? GF_LITE : GF_DARK), dir, 200, 2);
				o_ptr->timeout = rand_int(300) + 300;
				break;
			}
			...
			case SV_DRAGON_POWER:
			{
				...
				fire_spray(GF_MISSILE, dir, 300, 2);
				o_ptr->timeout = rand_int(300) + 300;
				break;
			}
		}
	...
	/* mage weapons use the activation command to cast spells */
	if ( ((o_ptr->tval == TV_HAFTED) && (o_ptr->sval >= SV_MAGESTAFF_HEAD)) ||
		((o_ptr->tval == TV_POLEARM) && (o_ptr->sval >= SV_MAGEROD_HEAD)) ||
		((o_ptr->tval == TV_SWORD) && (o_ptr->sval >= SV_MAGEBLADE_HEAD)) || mageart)
	{
		u32b f1, f2, f3, f4;
		int realm = 0;
		char o_name[80];
		...
		/* Extract the flags | this is necesary because the flags determine what can be cast */
		object_flags(o_ptr, &f1, &f2, &f3, & f4);
		if (f4 & TR4_MAGE)
		{
			f4 = f4 / (TR4_MAGE * 2);
			f4 = f4 % (2<<HREALM_MAX);
			while(f4 > 1) /* base 2 log */
			{
				f4 = f4 >> 1;
				realm++;
...
/* also note that items with the TR4_MAGE flag but no realm flags will default to TR4_MAGE_LIGHTDARK */
			hspell_aux(realm);
/* learn new realms after using them from items, currently no confirmation asked.  Should fix this later */
			if ((p_ptr->realm1 != realm) && (p_ptr->realm2 != realm) && (p_ptr->realm3 != realm))
			{
				if (p_ptr->realm1 == 0)
				{
					p_ptr->realm1 = realm;
					msg_print("You have learned a new realm of High Magic");
				}
				else if (p_ptr->realm2 == 0)
				{
					p_ptr->realm2 = realm;
					msg_print("You have learned a new realm of High Magic");
				}
				else if (p_ptr->realm3 == 0)
				{
					p_ptr->realm3 = realm;
					msg_print("You have learned a new realm of High Magic");
				}
			...
			return (FALSE); /* mage weapons don't need to recharge */
		}
	}
...
void describe_item_activation(const object_type *o_ptr)
{
	...
	if (o_ptr->tval == TV_RING)
	{
		text_out("It activates for resist every 50+d50 turns");
	}

	if (k_info[o_ptr->k_idx].flags4 & TR4_MAGE)
	{
		int realm = 0;
		u32b f4 = k_info[o_ptr->k_idx].flags4;
		f4 = f4 / (TR4_MAGE * 2);
		f4 = f4 % (HREALM_MAX);
		while(f4 > 1) /* base 2 log */
		{
...
/* also note that items with the TR4_MAGE flag but no realm flags will default to TR4_MAGE_LIGHTDARK */
		text_out("use high magic");
		text_out(realm_names[realm]);
	}
