...
static void describe_monster_spells(int r_idx, const monster_lore *l_ptr)
{
	...
	if (l_ptr->flags4 & RF4_BR_LAW)		vp[vn++] = "law";
	...
	if (l_ptr->flags6 & RF6_S_NONLIFE)   vp[vn++] = "summon an unliving monster";
...
static void describe_monster_abilities(int r_idx, const monster_lore *l_ptr)
{
	...
	/* Collect low resistances */
	vn = 0;
	if (l_ptr->flags3 & RF3_RES_ACID) vp[vn++] = "acid";
	...
	if (vn)
	{
		...
		{
			/* Intro */
			if (n == 0) text_out(" strongly resists ");
			else if (n < vn-1) text_out(", ");
			else text_out(" and ");
