...
static char *desc_spell[] = {
  ...
  "cast acid balls",
  "create traps",
  "give mortal wounds to its prey",
  "cast psychic blasts",
...
if (!no_graphics_flag) {
  if (cp->cdefense & UNIQUE)
    (void) sprintf(temp, "%c - (%c) %s:\n",
    (cp->cdefense & SHAPECHANGER?
    c_list[randint(MAX_CREATURES-1)].gchar
    :cp->gchar), cp->cchar, cp->name);
  else
    (void) sprintf(temp, "%c - (%c) The %s:\n",
        (cp->cdefense & SHAPECHANGER?
    c_list[randint(MAX_CREATURES-1)].gchar
    :cp->gchar), cp->cchar, cp->name);
  roff(temp);}
...
/* This is the original description code.  It uses sequential search! Yuck!
  ...
  if (rcdefense & CHARM_SLEEP)
	...
	roff(" It");
      roff(" is immune to sleep spells");
    }
