...
static void vault_chapel(ARG_INT ARG_COMMA ARG_INT);
static void vault_object(ARG_INT ARG_COMMA ARG_INT);
static void vault_mixed_dragon(ARG_INT ARG_COMMA ARG_INT); /* ARG_COMMA ARG_INT ARG_COMMA ARG_INT); */
static void vault_alpha(ARG_INT ARG_COMMA ARG_INT);
static void vault_hound(ARG_INT ARG_COMMA ARG_INT);
static void vault_zoo(ARG_INT ARG_COMMA ARG_INT);
static void vault_evil(ARG_INT ARG_COMMA ARG_INT);
static void vault_mix_orc(ARG_INT ARG_COMMA ARG_INT);
static void vault_mix_demon(ARG_INT ARG_COMMA ARG_INT);
#if 0
/* These pits hang the computer. GIANT and TROLL may be outside the normal
int range, as cdefense is a int32u field...  -TY */
static void vault_mix_troll(ARG_INT ARG_COMMA ARG_INT);
static void vault_mix_giant(ARG_INT ARG_COMMA ARG_INT);
#endif
static void vault_generic(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
...
static void vault_giant(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
...
static void place_destroyed(ARG_VOID);
...
static void vault_jelly(y, x) /* Have to change this! - TY*/
int y, x;
{
  int l, m, summon;
  ...
  do {
     ...
     ||  (c_list[m].cchar == 'm'))
     && !(c_list[m].cdefense & EVIL) && !(c_list[m].cdefense & UNIQUE)) {
	summon = TRUE;
...
static void vault_alpha(y, x)
int y, x;
{int l, m, summon;
    summon = FALSE;
    l = m_level[MAX_MONS_LEVEL];
    do {  m = randint(l) - 1;
        if (!(c_list[m].cchar == '|') && !(c_list[m].cdefense & UNIQUE) &&
            !(c_list[m].cchar == '$') && !(c_list[m].cchar == '/') &&
            !(c_list[m].cchar == '?') && !(c_list[m].cchar == '!') &&
            !(c_list[m].cchar == ',') && !(c_list[m].cchar == '.') &&
            !(c_list[m].cchar == ' ') && !(c_list[m].cmove & MV_INVIS) &&
            !(c_list[m].cchar == '&') && !(c_list[m].cdefense & BREAK_WALL) &&
            !(c_list[m].cmove & THRO_CREAT) && !(c_list[m].cmove & THRO_WALL)
            && !(c_list[m].cmove & MULTIPLY))
            { summon = TRUE;
              place_monster (y, x, m, TRUE);
            }
        }
...
static void vault_generic(int y, int x, int z)
{
    int l, m, summon;
    ...
    do {
        m = randint(l) - 1;
        if ((c_list[m].cdefense & z) && !(c_list[m].cdefense & UNIQUE)) {
            summon = TRUE;
            place_monster (y, x, m, TRUE);
        }
        }while (!summon);
}
static void vault_mixed_dragon(int y, int x)
{
   vault_generic(y,x,DRAGON);
}

static void vault_evil(int y, int x)
{
    vault_generic(y,x,EVIL);
}

static void vault_mix_orc(int y, int x)
{
    vault_generic(y,x,ORC);
}

#if 0
static void vault_mix_troll(int y, int x)
{
    vault_generic(y,x,TROLL);
}
static void vault_mix_giant(int y, int x)
{
    vault_generic(y,x,GIANT);
}
#endif
static void vault_mix_demon(int y, int x)
{
    vault_generic(y,x,DEMON);
}

static void vault_zoo(int y, int x)
{
   vault_generic(y,x,ANIMAL);
}
...
int y, x, rank;
{
  ...
  case 1:
    while (i<MAX_CREATURES) {
      if (!stricmp(c_list[i].name, "Clubber demon")) break;
      i++;
    }
    break;
  case 2:
    while (i<MAX_CREATURES) {
      if (!stricmp(c_list[i].name, "Byakhee")) break;
      i++;
    }
    break;
  case 3:
    while (i<MAX_CREATURES) {
      if (!stricmp(c_list[i].name, "Formless spawn of Tsathogghua")) break;
      i++;
    }
    break;
  case 4:
    while (i<MAX_CREATURES) {
      if (!stricmp(c_list[i].name, "Hunting horror")) break;
      i++;
    }
    break;
  case 5:
    while (i<MAX_CREATURES) {
      if (!stricmp(c_list[i].name, "Dark young of Shub-Niggurath")) break;
      i++;
    }
    break;
  case 6:
    while (i<MAX_CREATURES) {
      if (!stricmp(c_list[i].name, "Balrog")) break;
      i++;
    }
    break;
  }
    ...
    int y, x;
        {
          ...
          do {
            m = randint(l) - 1;
            if ((strstr(c_list[m].name, "riest")) ||
            (c_list[m].cchar == 'A') && !(c_list[m].cdefense & UNIQUE))
            { summon = TRUE;
                place_monster (y, x, m, TRUE);
            }
        } while (!summon);
    ...
    int y, x;
        {
            ...
            do {
                m = randint(l) - 1;
                if (((c_list[m].cchar == '$') || (c_list[m].cchar == '/')
                || (c_list[m].cchar == '|') || (c_list[m].cchar == '±')
                || (c_list[m].cchar == '!') || (c_list[m].cchar == '?'))
...
int yval, xval, type;
{
  ...
  if (wizard) {
    ...
    case 8:
      ...
      break;
    case 9:
      msg_print("A Treasure Pit");
      break;
    case 10:
       msg_print("A Hound Pit");
       break;
    case 11:
        msg_print("An Alphabet Mix Pit");
        break;
    case 12:
        msg_print("A Zoo");
        break;
    case 13:
        msg_print("A House of Evil");
        break;
    case 14:
        msg_print("Mixed Orc Pit");
        break;
#if 0
    case 15:
        msg_print("Mixed Troll Pit");
        break;

    case 16:
        msg_print("Mixed Giant Pit");
        break;
#endif
    case 17:
        msg_print("Mixed Demon Pit");
        break;
    default:
        msg_print("Unknown Pit Type");
            }
  }
  ...
  int yval, xval;
{
  ...
  if (randint((dun_level*dun_level+1))<650) good_item_flag = TRUE;
#endif
  tmp = randint(dun_level>82?82:dun_level);
  rating += 10;
  if (tmp < 10)      special_pit(yval, xval,1); /* jelly */
  else if (tmp < 16) special_pit(yval, xval,9); /*treasure pit*/
  else if (tmp < 22)
              {  if (randint(3)!=1) special_pit(yval, xval,2); /* orc */
                else special_pit(yval, xval, 14); /* Mixed orc - TY */
            }
  else if (tmp < 33) special_pit(yval, xval,12); /*zoo*/
  else if (tmp < 43)
    if (randint(3)==1)
        {
#if 0
        if ((dun_level > 35) && randint(3)==1) special_pit (yval, xval, 16);
                        /* Mixed Giant  _TY*/
#endif
        special_pit(yval, xval,7); /* giant -CFT */
    }
    else {
#if 0
        if (randint(3)==1) special_pit(yval, xval, 15); /* mixed troll - TY */
         else
#endif
         special_pit(yval, xval, 3); /* troll */
        }
  else if (tmp < 52) {
        switch(randint(2))
            {
                case 1: special_pit(yval, xval,10); /*hound pit*/
                        break;
                default:  special_pit(yval, xval,13); /*house of evil */
            }
        }

  else if (tmp < 65)
    switch (randint(no_graphics_flag?7:5)) {
        case 1: case 2: special_pit(yval, xval,8); /*chapel*/
                break;
        case 6: case 7: special_pit(yval, xval,11); /*alphabet mix*/
                break;
        default: special_pit(yval, xval,4); /*graveyard*/
    }

  else if (tmp < 75)
        {
            if (randint(3)!=1) special_pit(yval, xval,6);  /* demn */
            else special_pit(yval, xval, 17); /* Mixed demon -TY*/
        }
  else               special_pit(yval, xval,5); /* dragon */
}
  ...
  for (i = 0; i < row_rooms; i++)
      ...
      if (room_map[i][j] == TRUE)
	{
	  ...
	  if (dun_level > randint(DUN_UNUSUAL))
	    {
	      tmp = randint(5);
          if ((tmp == 1) || (spec_level != 0))  build_type1(yloc[k], xloc[k]);
...
/* Begin new code for different sized levels... ES, TY */

  mon_to_alloc = randint(8) + MIN_MALLOC_LEVEL + alloc_level;
  if (40 - dun_level >= 40 * num_screen_wide * num_screen_high / MAX_SCREENS)
   mon_to_alloc = (mon_to_alloc * dun_level/40) +
     (mon_to_alloc * num_screen_high * num_screen_wide / MAX_SCREENS);
  alloc_monster(mon_to_alloc, 0, TRUE);
 /*  This code caused gruesome software errors - until I realized that
 NORMAL size levels were trying to use num_screen_wide & _high but had
 no values for them! Fixed it by giving default values in generate_cave() -TY*/
/*   Old code: alloc_monster((randint(8)+MIN_MALLOC_LEVEL+alloc_level), 0, TRUE); */
...
void generate_cave()
{
      ...
      num_screen_wide = (MAX_HEIGHT/SCREEN_HEIGHT);
      num_screen_high = (MAX_WIDTH/SCREEN_WIDTH);
    /* Let's give some default values! Otherwise: Ka-Boom...  -TY */
  ...
  if (dun_level == 0)
    ...
    {    /* ES's code for varying level sizes, changed so that 1 level
            in 3 (was 1 in 5) has unusual size - TY */
     if (randint(3)==1) {
      num_screen_wide = randint(MAX_HEIGHT/SCREEN_HEIGHT);
      num_screen_high = randint(MAX_WIDTH/SCREEN_WIDTH);
      cur_height = num_screen_wide * SCREEN_HEIGHT;
      cur_width = num_screen_high * SCREEN_WIDTH;
      if (!(cur_height == MAX_HEIGHT && cur_width == MAX_WIDTH)) {
        /* Don't give a bogus message for normal-sized levels! - TY */
      if (wizard) msg_print ("A level of an unusual size.");
      else rating += 5;
    }
     }
     else {
  ...
  if (good_item_flag)
    ...
    strcpy(feeling,"You faint as horrible visions of sudden death fill your mind!");
    paral_flag = TRUE;}
  else if (rating>80)
    strcpy(feeling,"Beware!  This level looks dangerous...");
  else if (rating>60)
    strcpy(feeling,"You have a very bad feeling.");
  else if (rating>40)
    strcpy(feeling,"You have a bad feeling.");
  else if (rating>30)
    strcpy(feeling,"You feel very worried.");
  else if (rating>20)
    strcpy(feeling,"You feel your luck is turning...");
  else if (rating>10)
    strcpy(feeling,"You don't like the look of this place.");
  else if (rating>0)
    strcpy(feeling,"This level looks reasonably safe.");
