...
/*colorizer from spells.c
void color_dam(int32,int32); Nice idea, but it doesn't work
                             perfectly and it also can cause delayed (colorized)
                             feelings to get lost more easily */
...
static void facts(ARG_INV_PTR ARG_COMMA ARG_INT_PTR ARG_COMMA ARG_INT_PTR ARG_COMMA ARG_INT_PTR ARG_COMMA ARG_INT_PTR ARG_COMMA ARG_INT_PTR);
  ...
  switch(t_ptr->subval)
	...
	msg_print(NULL); /* make sure can see the message before new level */
	  ...
	  if (randint(3) == 1) {
        ...
        else msg_print("You seem unaffected by the poison.");
	    }
	} /* no ffall */
      }
      break;
      ...
      if (test_hit(125, 0, 0, p_ptr->pac+p_ptr->ptoac, CLA_MISC_HIT))
	{
	  if (!py.flags.sustain_str)
        ...
        msg_print("A small dart hits you, but you feel strong.");
	}
      else
      ...
      if (test_hit(125, 0, 0, p_ptr->pac+p_ptr->ptoac, CLA_MISC_HIT))
	{
	  if (!py.flags.sustain_con)
        ...
        msg_print("A small dart hits you, but you feel healthy.");
	}
      else
	msg_print("A small dart barely misses you.");
      break;
  ...
  if (i <= TV_MAX_PICK_UP)
      ...
      if (i == TV_GOLD)
	{
	  ...
	  (void) sprintf(out_val,
             "You collect %ld gold pieces worth of %s",
			 i_ptr->cost, tmp_str);
	  prt_gold();
	  (void) delete_object(y, x);
	  msg_print(out_val);
...
int monptr, dam;
{
  ...
  if (m_ptr->hp < 0)
	...
	unlink(tmp);
      ...
      if (c_list[m_ptr->mptr].cdefense & CAN_SPEAK) {
    //     monster_name(m_name, m_ptr, c_ptr);
        if (randint(7)==1) {if(getLine("rumors.txt",0,destination)) {
                sprintf(out_val,"Before dying, %s whispers something to you:", m_name);
                msg_print(out_val);
                msg_print(destination);}}

            else {
            if(getLine("mondeath.txt",0,destination)) {
                sprintf(out_val,"%s says:", m_name);
                msg_print(out_val);
                msg_print(destination);}}
                
                }
      if (c_list[m_ptr->mptr].cdefense & UNIQUE && randint(10)==1)    {
        sprintf(out_val,"There was a price on %s's head!", m_name);
            msg_print(out_val);
        if (getLine("crime.txt",0,destination))
                {sprintf(out_val,"%s was wanted for %s", m_name, destination);
                 msg_print(out_val);}
        kill_money = (250 * randint(c_list[m_ptr->mptr].level));
            if (kill_money < 250) kill_money = 250;
            sprintf(out_val, "You collect a reward of %d gold coins.", kill_money);
            msg_print(out_val);
            py.misc.au += kill_money;
            prt_gold();
                                          }
	...
	if (found) {
	  if (dun_level != c_list[m_ptr->mptr].level) { /* just mesg */
        msg_print("Well done!!  Now continue onward towards the Serpent of Chaos.");
	    }
  ...
  else
   {
    ...
    if (!(m_ptr->monfear) && !(c_list[m_ptr->mptr].cmove & CM_ATTACK_ONLY) &&
     ...
     { if (!(c_list[m_ptr->mptr].cdefense & FEARLESS))
        /* Some monsters don't run away -TY*/
...
int y, x;
{
  ...
  int crptr, monptr, tot_tohit, base_tohit, vorpal = 0;
...
/*#ifdef TC_COLOR
      if (!no_color_flag)
      color_dam(m_list[crptr].maxhp,m_list[crptr].hp);
#endif*/
	    msg_print(out_val);
	    }
      ...
      if ((i_ptr->flags2 & TR_VORPAL) && randint(7)==1) /* Sharpness, Vorpal
                                                         Blade etc. - TY */
           {

            msg_print("Your sword cuts deep into your foe!");
            k *= (i_ptr->name2 == SN_ORCRIST?3:2);
            while (randint(3)==1) vorpal += (k/2);
            if (vorpal>0) k+=vorpal;
        }
      if (i_ptr->name2 == SN_ELVAGIL && randint(7)==1)
	  ...
	  if (mon_take_hit(crptr, k) >= 0)
	    {
              if ((c_list[monptr].cdefense & DEMON) ||
                  ...
                  || (c_list[monptr].cdefense & NONLIVING)
                  )
...
/*#ifdef TC_COLOR
          if (!no_color_flag)
              color_dam(m_list[crptr].maxhp,0);
#endif*/
	      msg_print(out_val);
/*#ifdef TC_COLOR
          textcolor(LIGHTGRAY);
	  ...
	  if (c_ptr->fval <= MAX_OPEN_SPACE) /* Open floor spot	*/
	      ...
	      if (c_ptr->tptr != 0)
		{
		  ...
		  if (i == TV_INVIS_TRAP || i == TV_VIS_TRAP || i == TV_GOLD
            ...
            if (!py.flags.blind)  sprintf(t2, "You can see here %s", t1);
            else sprintf(t2, "You can feel here %s", t1); /* Makes more sense,
                            I hope... -TY*/
		    msg_print(t2);
		    }
  ...
  if (get_dir(NULL, &dir)) {
	...
	if (t_ptr->p1 > 0) {
	  ...
	  if (py.flags.confused > 0)
        ...
        count_msg_print("The lock seems too complicated.");
	}
...
void closeobject()
{
  ...
  if (get_dir(NULL, &dir))
	...
	no_object = TRUE;
      ...
      if (no_object)
	{
      msg_print("You really want to close THAT?");
	  free_turn_flag = TRUE;
	}
    }
  target_mode = temp;
...
int dir;
{
  ...
  if (c_ptr->fval < MIN_CAVE_WALL
	...
	{
      msg_print("You don't know how to tunnel through that!");
	  free_turn_flag = TRUE;
	}
      return;
    }
...
void throw_object()
{
  ...
  if (inven_ctr == 0)
	...
	ok_throw = TRUE; /* if it's a mushroom or potion that does
        ...
        ok_throw = TRUE;  /* Not IDed, but user knows it's cursed... */
          ...
          (t->flags2 & TR_ARTIFACT && known1_p(t)))
        ok_throw = TRUE; /* if user wants to throw cursed, let him
                            But not if it is the One Ring - TY*/
        ...
        ok_throw = TRUE; /* it's junk, let him throw it */
           ...
           (t->tval <= TV_DIGGING) &&!(known2_p(t)))
                                /* No prompt for ANY object we don't know!
                                Otherwise this code can be (mis)used for
                                id'ing things... -TY*/
        ok_throw = TRUE;
      else if ((t->tval >= TV_HAFTED) &&
  ...
  if (!ok_throw) free_turn_flag = TRUE; /* TY */
  if (ok_throw) /* can only be true if selected item, and it either looked
      ...
      if (get_dir(NULL, &dir))
	{
	  ...
	  inven_throw(item_val, &throw_obj);
	    ...
	    if (thits > max_shots)
		thits = max_shots;
      tchar = (no_graphics_flag?throw_obj.tchar:throw_obj.gchar);
...
/*#ifdef TC_COLOR
      ...
      if (mon_take_hit(monster, k) >= 0)
	{
          if ((c_list[m_ptr->mptr].cdefense & UNDEAD) ||
              ...
              (c_list[m_ptr->mptr].cdefense & NONLIVING))  /*Definitely a bug
                        here. It was checking twice for demon... fixed. TY*/
	    (void) sprintf(out_val, "You have destroyed %s.", m_name);
          else
	    (void) sprintf(out_val, "You have slain %s.", m_name);
	  msg_print(out_val);
...
static char *look_mon_desc(int mnum){
  ...
  living = (!(c_list[m->mptr].cdefense & (UNDEAD | DEMON | NONLIVING)) );
...
/*#ifdef TC_COLOR
 if (!no_color_flag)
     color_dam((int32)m->maxhp,(int32)m->hp);
#endif*/
