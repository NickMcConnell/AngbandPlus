  ...
  if ((m_ptr->cdis <= MAX_SIGHT) &&
	...
	flag = TRUE;
      if (py.flags.telepathy) {
        if ((c_list[m_ptr->mptr].cdefense & MINDLESS) ||
            ((c_list[m_ptr->mptr].cdefense & SEMI) && (randint(5) != 1)))
            /* With these two flags this code became SO nice... TY */
            flag = FALSE;
        else
...
register int *mm;
{
  ...
  if (m_list[monptr].monfear)  /* Run away!  Run away! -DGK */
   {
    y=-y;
    x=-x;
   }
...
if ((r_ptr->cdefense & CAN_SPEAK) && (randint(7)==1))
    {
        if(getLine("bravado.txt",0,destination))
              {sprintf(destii, "%s%s",cdesc,destination);
              msg_print(destii);                        }
              }
      ...
      if (flag)
	{
	  ...
	  switch(adesc)
	    ...
	    case 3:
	      ...
	      CUT=TRUE;
	      break;
        case 4: msg_print(strcat(tmp_str, "stings you."));
            CUT=TRUE;
            break;
	    case 5: msg_print(strcat(tmp_str, "touches you.")); break;
        case 6: msg_print(strcat(tmp_str, "kicks you."));
            STUN=TRUE;
            break;
        ...
        case 10: msg_print(strcat(tmp_str, "screams:  'FUCK YOU, ASSHOLE!'"));
            break;
		}
	      break;
	    case 20:
	    ...
	    case 23:
          switch(randint(5))
		  {
          case 1: msg_print(strcat(tmp_str, "sings to you.")); break;
          case 2: msg_print(strcat(tmp_str, "tells you to get off his show.")); break;
          /*case 3: msg_print(strcat(tmp_str, "looks for nice little children")); break;*/
          case 3: msg_print(strcat(tmp_str, "sings:  'I love you, you love me.'")); break;
          case 4: msg_print(strcat(tmp_str, "sings:  'We are a happy family.'")); break;
          case 5: msg_print(strcat(tmp_str, "sings:  'I love you, you love me.'")); break;
		  }
		break;
	  ...
	  switch(attype)
	    ...
	    case 4:    /*Fear attack	*/
	      ...
	      take_hit(damage, ddesc);
          if (player_saves() || (
          (f_ptr->shero+f_ptr->hero+py.flags.fear_resist) > 0) ||
          (py.misc.pclass==1 && randint(3)==1))
            msg_print("You stand your ground!");
	      else if (f_ptr->afraid < 1)
		{
          msg_print("Eek!  You scream in terror!");
	    ...
	    case 10:	/*Blindness attack*/
	        ...
	        if (f_ptr->blind < 1)
		  {
            ...
            msg_print("Somebody seems to have turned off the light.");
		  }
	        else
		  {
		    f_ptr->blind += 5;
	    ...
	    case 11:	/*Paralysis attack*/
	      ...
	      take_hit(damage, ddesc);
	      if (player_saves())
        msg_print("You refuse to be paralyzed!");
	      else if (f_ptr->paralysis < 1)
		{
		  if (f_ptr->free_act)
            msg_print("Paralysis? What's that?");
		  else
		    {
	      ...
	      else
		{
	      ...
	      if (randint(2) == 1)
		{
          msg_print("The thief flees, laughing at you!");
		  blinked = 1; /* added -CFT */
	    ...
	    case 14:	/*Poison	 */
	      ...
	      take_hit(damage, ddesc);
		    ...
		    f_ptr->resist_poison)) {
        msg_print("The attack was poisoned!");
        ...
        msg_print("You feel about as dull as normally.");
	      else
		{
		...
		msg_print("You feel weaker.");
              ...
              else
	      {
              ...
              if (f_ptr->sustain_int)
                ...
                (void) dec_stat (A_INT);
              if (f_ptr->sustain_wis)
                msg_print("You feel as dull as normally.");
              else
	      {
                msg_print("Your wisdom is drained.");
                (void) dec_stat (A_WIS);
	  ...
	  if (py.flags.confuse_monster && py.flags.protevil<=0)
        ...
        (void) sprintf(tmp_str, "%sis not confused.", cdesc);
	      else
		{
...
int32u *rcmove;
{
      ...
      if (c_ptr->fval != BOUNDARY_WALL)
	{
	  ...
	  /* Creature moves through walls? */
		...
		(t_ptr->tval == TV_SECRET_DOOR)) { /* break the door -CFT */
          ...
          msg_print("You hear something smash into small pieces!");
	      disturb(1,0);
	      }
        else {/* otherwise, break those walls! -CFT */
	      (void) twall(newy, newx, 1, 0);
          if (randint(8)==1) msg_print ("You hear a grinding sound.");
        }
	  }
  ...
  while ((!do_turn) && (i < 5));
    ...
    /* Cyberdemons walk around noisily! - TY */
 if (
    (c_list[m_ptr->mptr].spells3 & ROCKET) &&
 ...
 if (!do_turn && m_ptr->monfear) /* Cornered! */
  {
...
if ((r_ptr->cdefense & CAN_SPEAK) && (randint(7)==1))
    {
      ...
      ((m_ptr->hp < ((r_ptr->hd[0]*r_ptr->hd[1])/10))
       || (m_ptr->monfear>0)) && /* If it is afraid, it is desperate, right? -TY */
	...
	case 9:	 /*Serious Wound */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles horribly.");
	  msg_print(cdesc);
	  if (player_saves())
	    msg_print("You resist the effects of the spell.");
	  else
	...
	case 11:  /*Cause Blindness*/
	  if (!blind)
        (void) strcat(cdesc, "casts a spell, turning off the light!");
	  else
        (void) strcat(cdesc, "mumbles, and everything seems even darker.");
	  msg_print(cdesc);
	  if ((player_saves()) || (py.flags.blindness_resist))
	    if (!blind)
	      msg_print("You blink and your vision clears.");
	    else
          msg_print("Or maybe not...");
	...
	case 12:  /*Cause Confuse */
	  if (!blind)
        (void) strcat(cdesc, "creates a dancing, dazzling lights.");
	  else
	...
	case 13:  /*Cause Fear	  */
	  ...
	  msg_print(cdesc);
      if (player_saves() || (
      (py.flags.shero+py.flags.hero+py.flags.fear_resist) > 0))
	...
	case 18:  /*Summon Demon */
	  if (!blind)
        (void) strcat(cdesc, "summons a a demon from the Courts of Chaos!");
	  else
	...
	case 25:  /* Fire Bolt */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles and you smell something burn.");
	  msg_print(cdesc);
	...
	case 26:  /* Frost Bolt */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles you feel an icy wind.");
	  msg_print(cdesc);
	...
	case 27:  /* Acid Bolt */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles and you smell something pungent.");
	  msg_print(cdesc);
	...
	case 28:  /* Magic Missiles */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles and something whizzes through the air.");
	  msg_print(cdesc);
	...
	case 29:	 /*Critical Wound	 */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles hysterically!");
	  msg_print(cdesc);
	  if (player_saves())
        msg_print("You resist the effects of the spell.");
	  else
	...
	case 32:  /* Mana Bolt */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles and you there's magic in the air.");
	  msg_print(cdesc);
	...
	case 33:
	  if (!blind)
        ...
        (void) strcat(cdesc, "breathes, and a wave of anarchy hits you.");
	  msg_print(cdesc);
	...
	case 37:
	  if (!blind)
        ...
        (void) strcat(cdesc, "breathes, and there's a static feeling in the air.");
	  msg_print(cdesc);
	...
	case 39:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you hear the thunder.");
	  msg_print(cdesc);
	...
	case 46:
	  ...
	  if (m_ptr->hp>=m_ptr->maxhp) { /* need >= because, if we recalc-ed
	      ...
	      msg_print(cdesc);
        if (m_ptr->monfear > 0)  m_ptr->monfear = 0;
	      ...
	      if (m_ptr->hp==m_ptr->maxhp)
          { (void) strcat(cdesc, "completely healed!");
                  if (m_ptr->monfear > 0)  m_ptr->monfear = 0; }
	      else
	...
	case 47:
	  if (!blind)
	    ...
	    (void) sprintf(outval, "%smumbles to itself.",cdesc);
	  msg_print(outval);
      if ((m_ptr->cspeed)<=((int)(c_list[m_ptr->mptr].speed))) /* -10 removed
                                                                - TY*/
          {
	...
	case 48:
	  if (!blind)
        ...
        (void) strcat(cdesc, "sounds as if it threw something.");
	  msg_print(cdesc);
	...
	case 49:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles and it seems warmer here.");
	  msg_print(cdesc);
	...
	case 50:
	  ...
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles and something wicked comes your way.");
	  msg_print(cdesc);
	...
	case 52:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and it seems colder here.");
	  msg_print(cdesc);
	...
	case 53:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles. You shrug.");
	  msg_print(cdesc);
	...
	case 54:
	  ...
	  if (player_saves() || randint(2)==1)
        msg_print("But you keep hold of your memories.");
	...
	case 57:
	  if (!blind)
        (void) strcat(cdesc, "gestures at you, and you feel trippy.");
	  else
        (void) strcat(cdesc, "mumbles, and you feel trippy.");
	  msg_print(cdesc);
	...
	case 58:
	  ...
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you get wet.");
	  msg_print(cdesc);
	...
	case 59:
	  ...
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you almost drown.");
	  msg_print(cdesc);
	...
	case 61:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you hear a heavenly choir sing.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called,it needs monptr */
	...
	case 62:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles. Bad smelling things appear nearby.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called,it needs monptr */
	...
	case 63:
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you hear loud barking nearby.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called,it needs monptr */
	...
	case 66:
	  if (!blind)
        ...
        (void) strcat(cdesc, "breathes, and you feel passive.");
	  msg_print(cdesc);
	...
	case 67:
	  if (!blind)
        ...
        (void) strcat(cdesc, "breathes. You shrug.");
	  msg_print(cdesc);
	...
	case 68:
	  if (!blind)
        ...
        (void) strcat(cdesc, "breathes. Your Rolex goes crazy.");
	  msg_print(cdesc);
	...
	case 71: /* plasma */
	  if (!blind)
        ...
        (void) strcat(cdesc, "breathes. Something *HOT* hits you.");
	  msg_print(cdesc);
	...
	case 73:
	  if (!blind)
        (void) strcat(cdesc, "magically summons his knightly retainers and henchmen.");
  	  else
        (void) strcat(cdesc, "mumbles, and you suits of armor clanking nearby.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called, it needs monptr */
	...
	case 74: /* Big darkness storm */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles powerfully. You shrug.");
	  msg_print(cdesc);
	  breath(GF_DARK, char_row, char_col,
                  ((m_ptr->hp/6)>500?500:(m_ptr->hp/6)), ddesc, monptr);
          break;
	case 75: /* Mana storm */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you are hit by a storm of arcane power.");
	  msg_print(cdesc);
	...
	case 76: /* Summon reptiles */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you hear loud hissing nearby.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called, it needs monptr */
	...
	case 77: /* Summon ants */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles, and you hear insectoid legs clicking.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called, it needs monptr */
	...
	case 78: /* Summon unique monsters */
	  if (!blind)
        ...
        (void) strcat(cdesc, "mumbles. You've got a bad feeling about this.");
	  msg_print(cdesc);
	  y = char_row;
	  x = char_col;
	  /* in case compact_monster() is called, it needs monptr */
	...
	case 80: /* Summon ancient dragons */
	  ...
	  break;
case 81: /* Cyberdemon Rockets! - TY*/
      if (!blind)
        (void) strcat(cdesc, "shoots a rocket at you!");
	  else
        (void) strcat(cdesc, "shoots something. There is an explosion!");
	  msg_print(cdesc);
      breath(GF_ROCKET, char_row, char_col,
         (c_list[m_ptr->mptr].level * 3)+randint(150), ddesc, monptr);
	  break;

case 82:
      if (!blind)
        (void) strcat(cdesc, "casts a Ball of Radiation.");
	  else
        (void) strcat(cdesc, "mumbles, and you sense strange radiation.");
	  msg_print(cdesc);
      breath(GF_NUKE, char_row, char_col,
      randint(c_list[m_ptr->mptr].level*3)+(c_list[m_ptr->mptr].level),
       ddesc, monptr);
      break;

case 83:
      if (!blind)
        (void) strcat(cdesc, "breathes toxic waste.");
	  else
        (void) strcat(cdesc, "breathes something disgusting!");
	  msg_print(cdesc);
      breath(GF_NUKE, char_row, char_col,
        ((m_ptr->hp/6)>777?777:(m_ptr->hp/6)), ddesc, monptr);
      break;

case 84:
      if (!blind)
        (void) strcat(cdesc, "invokes raw Logrus!");
	  else
        (void) strcat(cdesc, "mumbles, and you feel distorted.");
	  msg_print(cdesc);
      breath(GF_CHAOS, char_row, char_col,
        ((m_ptr->hp/6)>777?777:(m_ptr->hp/6)), ddesc, monptr);
      break;
