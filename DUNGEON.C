...
int y, x;
{
  ...
  if ((cave_ptr->cptr == 1) && (!find_flag || find_prself))
    return (no_graphics_flag?'@':'à');
  else if (f_ptr->status & PY_BLIND)
    return ' ';
  else if ((f_ptr->image > 0) && (randint (12) == 1))
#ifdef TC_COLOR
    { if (!no_color_flag) textcolor( randint(15) );
      return (no_graphics_flag?randint (95) + 31:randint(127)+128); } /*Even more random images
                                    if you are hallucinating! -TY */
#else
    return (no_graphics_flag?randint (95) + 31:randint(127)+128);
#endif
...
/*#ifdef TC_COLOR
...
#else
    return c_list[m_list[cave_ptr->cptr].mptr].cchar;
#endif  Old code... I'm experimenting with my "graphic" code -TY*/
#ifdef TC_COLOR
    {
      if (!no_color_flag){
	textcolor(mon_color(m_list[cave_ptr->cptr].color));
      }
    return (no_graphics_flag?c_list[m_list[cave_ptr->cptr].mptr].cchar
    :
    ((c_list[m_list[cave_ptr->cptr].mptr].cdefense & SHAPECHANGER)?
    c_list[randint(MAX_CREATURES-1)].gchar
    :c_list[m_list[cave_ptr->cptr].mptr].gchar));
    }
#else
    return (no_graphics_flag?c_list[m_list[cave_ptr->cptr].mptr].cchar:
            c_list[m_list[cave_ptr->cptr].mptr].gchar);
    ...
    return (no_graphics_flag?t_list[cave_ptr->tptr].tchar
        :t_list[cave_ptr->tptr].gchar);
    } /* else-if clause */	 
#else
    return (no_graphics_flag?t_list[cave_ptr->tptr].gchar:
    t_list[cave_ptr->tptr].gchar);
#endif
...
#if 0  /* my temporary fix... (see below) -CFT */
    ...
    if (good_item_flag)
      msg_print("You feel there is something special about this level.");
        ...
        {  msg_print("You faint as horrible visions of sudden death fill your mind!");
           py.flags.paralysis = 1 + randint(5);}
    else if (rating>80)
  ...
  do
    {
	 ...
	 values of feel_chance give worse chance.   -SAC */
...
#endif
	  msg_print(feeling);
      if (paral_flag) {py.flags.paralysis += 2 + randint(4);
      /* Neither strstr or stricmp for the feeling string worked :-< so I had
      to define this flag... TY */
                       paral_flag = FALSE; }
                       /* Just to be sure you don't faint on every following
                          level :-> */
#ifdef TC_COLOR
    if (!no_color_flag) textcolor(LIGHTGRAY); /* make easy to see */
#endif
	}
  if (unfelt) feel_chance = 1000; /* 1 in 1000 chance */
	...
	regen_monsters();
        ...
        if (i_ptr->p1 > 0)
	  {
		...
		/* unlight creatures */
		creatures(FALSE);
        msg_print("It suddenly gets darker!");
	      }
      ...
      if (f_ptr->food < PLAYER_FOOD_ALERT)
	{
     ...
     /* Since speed's power has been reduced by a factor of 10, compensate in
      * quantity of extra food consumed. -ESS */
        if (f_ptr->speed < -9 )
          f_ptr->food -= (f_ptr->speed*f_ptr->speed / 10 - f_ptr->speed) / 20;
	  ...
	  if ((PY_INVULN & f_ptr->status) == 0)
	    {
          ...
          { invulner_flag = TRUE;
            ...
            msg_print(NULL);
            draw_cave();
        } else invulner_flag = FALSE;
        if (!(invulner_flag) || randint(3)!=1)
	  ...
	  if (f_ptr->invuln == 0)
	    {
          ...
          if (invulner_flag) {no_color_flag = FALSE;
            textcolor(LIGHTGRAY);
            invulner_flag = FALSE;
            draw_cave();
            }
    ...
    if (i_ptr->tval != TV_NOTHING && (i_ptr->flags2 & TR_ACTIVATE)) {
	  ...
	  if ((i_ptr->tval==TV_RING) &&
          (!stricmp(object_list[i_ptr->index].name,"Power (The One Ring)")) &&
	      (randint(20)==1) && (py.misc.exp > 0))
	    py.misc.exp--, py.misc.max_exp--, prt_experience();
	}
    if (i_ptr->tval != TV_NOTHING      //An abominable if clause by TY
    && (((i_ptr->flags2 & TR_TY_CURSE) ||
        (py.misc.pclass && i_ptr->name2 == SN_GORLIM)) /* Was ..pclass != 0 */
        && (randint(100)==1))) generic_curse();
      }

  if (inventory[INVEN_WIELD].tval != TV_NOTHING &&
      inventory[INVEN_WIELD].name2 == SN_ELVAGIL &&
      randint(100)==1)
      /*The Chainsword is noisy! - TY*/
    {   char destination[80];
      ...
      if (f_ptr->detect_inv > 0)
	{
	  ...
	  if (f_ptr->detect_inv == 0)
		...
		for (i = INVEN_WIELD; i <= INVEN_LIGHT; i++)
	      ...
	      creatures (FALSE);
          msg_print("You cannot see the air anymore!");
	    }
	}
      /* Timed infra-vision    */
	  ...
	  if (f_ptr->tim_infra == 0)
	    {
	      ...
	      /* unlight but don't move creatures */
	      creatures (FALSE);
          msg_print("Your eyes feel normal now.");
	    }
	}
	  ...
	  )
	{
      ...
      /* Warriors, Rogues and paladins inbuilt ident
        TY: also rangers, although not  as good as any of the above */

      if (((py.misc.pclass==0) && (f_ptr->confused == 0) &&
	  (randint((int)(9000/(py.misc.lev*py.misc.lev+40)) + 1) == 1))
	  ||
      ((py.misc.pclass==4) && (f_ptr->confused == 0) &&
      (randint((int)(120000L/(py.misc.lev*py.misc.lev+40)) + 1) == 1))
      ||
...
register inven_type *t_ptr;
{
  ...
  if ((t_ptr->name2==SN_R) || (t_ptr->name2==SN_RA) ||
      ...
      (t_ptr->name2==SN_ATTACKS) || (t_ptr->name2==SN_EXECUTE_DR) ||
      (t_ptr->name2==SN_LIGHTNING) || (t_ptr->name2==SN_SHARPNESS) ||
...
char com_val;
{
  ...
  int y, x, i, j;
  vtype out_val, tmp_str, destination;
  register struct flags *f_ptr;
  ...
  switch(com_val)
	...
	creatures(FALSE);  /* draw monsters */
      ...
      free_turn_flag = TRUE;
      break;
    case 'v':     /* v)iew feeling */
      free_turn_flag = TRUE;
    if (dun_level == 0)     /* snicker.... -CWS */
    {
#ifdef TC_COLOR
    if (!no_color_flag) textcolor(LIGHTRED); /* make easy to see, heh hee
                                                - TY*/
#endif
...
#endif
      break;
     case 'I':    /* The racial special ability - TY*/
        race_special();
        break;
     case CTRL('Z'): /*The Amberite Pattern mind-walking - TY */
        pattern_walk();
        break;
    ...
    case '=':		/* (=) set options */
      ...
      set_options();
/*      restore_screen(); */
      draw_cave(); /* To update graphics / color display */
      free_turn_flag = TRUE;
      break;
    ...
    case 'G':		/* (G)ain magic spells */
      ...
      gain_spells();
      break;
    case 'g': case ',':       /* (g)et an object... */
      feel_chance = 50;
      if ((getkey_flag) &&
      (cave[char_row][char_col].tptr != 0)) /* minor change -CFT */
 /* TY: So we don't lose a turn accidentally hitting the key */
      ...
      if (wizard)
	{
	  ...
	  switch(com_val)
        ...
        prt("Type '?' or ^H for help.", 0, 0);}
            ...
            if (getLine("error.txt",0,destination))
                    msg_print(destination);}
        }
	}
      else
	{
      if (randint(7)==1) {      prt("Type '?' for help.", 0, 0);
                            }
      else {
        if (getLine("error.txt",0,destination))
                msg_print(destination);}
  ...
  while (!flag){
	...
	continue;
      ...
      test = FALSE;
       ...
       /* Yet another implementation of Aaron Mandelbaum's suggestion. */
       if (choice >= '0' && choice <= '9') {
         int equip_ct = 0;

         for (i = INVEN_WIELD; i <= INVEN_AUX; ++i)
           if ((inventory[i].flags2 & TR_ACTIVATE) && known2_p(&inventory[i])) {
             if (inventory[i].inscrip[0] == choice &&
                 inventory[i].inscrip[1] == '\0') {
                     choice = equip_ct + 'a';
                     break;
              }
              ++equip_ct;
    ...
    if (inventory[i].name2 == SN_RINGIL) {
      ...
      msg_print("The fragment of the Pattern on your sword shines...");
      switch (randint(5)) {
/*        case 1: case 2: case 3: case 4: case 5: case 6:
       if (get_dir(NULL, &dir)) {
        ...
        fire_ball(GF_FIRE, dir, char_row, char_col, 75,3);}*/
              case 1: tele_level(); break;
              case 2: earthquake(); break;
              case 3: teleport(100); break;
              case 4: destroy_area(char_row, char_col);break;
              default: stair_creation();
      }
        inventory[i].timeout=(300+randint(200));
    }
    break;
      case (52):
	...
	if (inventory[i].name2 == SN_ARUNRUTH) {
      msg_print("Your diamond sword glows a pale blue...");
	  if (get_dir(NULL, &dir)) {
	    if (py.flags.confused > 0) {
	      msg_print("You are confused.");
	      do {dir = randint(9);} while (dir == 5);
	...
	if (inventory[i].name2 == SN_AEGLOS) {
      ...
      switch (randint(7)) {
              case 1: destroy_area(char_row, char_col);break;
              default: earthquake();
      }
       remove_curse();
        inventory[i].timeout=randint(777);
/*      }*/
	}
    break;
     case (118):
	...
	break;
      case (422): /* Bronze  - changed the index to match treasure.c
                    Aaaggh... whose idea was to put that damn "Add new
                    items here!" before some DSMs in treasure.c??!?!
                    - TY*/
	...
	if (get_dir(NULL, &dir)) {
      ...
      case (432): /* Shining */
        ...
        break;
      case (433): /* Power Dragon Scale Mail */
    if (inventory[i].name2 == SN_BLADETURNER) {
      ...
      msg_print("You breathe the Elements...");
          if (get_dir(NULL, &dir)) {
            ...
            fire_ball(GF_MAGIC_MISSILE, dir, char_row, char_col, 400,4);
      ...
      py.flags.resist_acid += randint(50)+50;
        ...
        break;
      case (SPECIAL_OBJ+3):
    msg_print("The phial wells with inner light...");
    ...
    wizard_light(TRUE);
    inventory[i].timeout=250+randint(250);
        break;
   case (SPECIAL_OBJ+7):{
        char c; int f = TRUE;
