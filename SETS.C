...
int set_disen_destroy(inven_type *e) /* TY - destroys
                                        items that have magical plusses*/
{
  int x = e->tval;
  if ((x>=TV_MIN_ENCHANT) && (x<=TV_MAX_WEAR) &&
      ((e->p1>0) || (e->tohit>0) || (e->todam>0) || (e->toac>0))
      && (!(e->flags2 & TR_RES_DISENCHANT)) && (!(e->flags2 & TR_ARTIFACT)))
      return (TRUE);
  return (FALSE);
}
