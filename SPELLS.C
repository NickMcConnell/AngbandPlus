...
static char bolt_char(int y,int x,int ny,int nx){ /* this assumes only 1
                            move apart -CFT*/
  if (ny == y) return (no_graphics_flag?'-':'Û');
  if (nx == x) return (no_graphics_flag?'|':'§');
  if ((ny-y) == (nx-x)) return  (no_graphics_flag?'\\':'Ü');
  return (no_graphics_flag?'/':'¿');
}

/*void color_dam(int32 maxhp,int32 thp){
  int32 perc;
...
static int8u bolt_color(int typ){
  ...
  switch(typ){      /* DGK */
    case GF_MAGIC_MISSILE: return (choice<=3?LIGHTCYAN:LIGHTBLUE);
    case GF_MANA:          return (choice<=3?LIGHTCYAN:(choice<=5?MAGENTA:CYAN));
    case GF_LIGHTNING:     return (choice<=3?LIGHTBLUE:(choice<=5?WHITE:BLUE));
    ...
    case GF_FROST:         return (choice<=2?LIGHTBLUE:WHITE);
    case GF_WATER:         return (choice<=2?LIGHTBLUE:BLUE);
    case GF_ICE:           return (choice<=3?LIGHTCYAN:WHITE);
    case GF_FIRE:          return (choice<=3?RED:(choice<=5?YELLOW:WHITE));
    case GF_HOLY_ORB:      return (choice<=5?DARKGRAY:RED);
    case GF_NETHER:        return (choice<=5?DARKGRAY:LIGHTGRAY);
    case GF_CHAOS:         switch(choice){case 1:return RED;
      case 2:return CYAN; case 3:return GREEN; case 4:return BROWN;
      case 5:return MAGENTA; case 6:return DARKGRAY;}
    case GF_TIME:    return (choice<=2?DARKGRAY:(choice<=4?LIGHTGRAY:LIGHTGREEN));
    case GF_DARK:          return (choice<=3?DARKGRAY:0);
    case GF_ARROW:         return BROWN;
    case GF_PLASMA:        return (choice<=4?LIGHTRED:RED);
    case GF_METEOR:        return (choice<=3?LIGHTRED:(choice<=5?BROWN:RED));
    case GF_SHARDS:        return (choice<=3?LIGHTGRAY:(choice<=5?BROWN:DARKGRAY));
    case GF_SOUND:         return (choice<=4?WHITE:LIGHTMAGENTA);
    ...
    case GF_FORCE:         return (choice<=3?LIGHTGRAY:(choice<=5?WHITE:DARKGRAY));
    case GF_INERTIA:       return (choice<=3?LIGHTRED:LIGHTMAGENTA);
    case GF_ROCKET:        return (choice<=3?LIGHTRED:(choice<=5?RED:DARKGRAY));
    case GF_NUKE:          return (choice<=4?YELLOW:(choice<=5?WHITE:LIGHTGREEN));
  }  
  ...
  switch (typ) {
    case GF_FIRE:  
      *destroy = set_fire_destroy;
      break;
    case GF_ACID: case GF_NUKE:
      *destroy = set_acid_destroy;
      break;
    case GF_FROST: case GF_SOUND: case GF_SHARDS: case GF_FORCE: case GF_ICE:
    case GF_GRAVITY: case GF_WATER:
    ...
    case GF_PLASMA:          /* DGK */
      *destroy = set_plasma_destroy;  /*fire+lightning -DGK */
      break;
    case GF_METEOR:          /* DGK */    case GF_ROCKET: /*TY*/
      *destroy = set_meteor_destroy;  /*fire+shards -DGK */
      break;
   case GF_DISENCHANT:
      *destroy = set_disen_destroy; /* TY: items with magical plusses */
      break;
    case GF_MANA:            /* DGK */  case GF_CHAOS: /* TY - raw Logrus
                                                            destroys EVERYTHING!!
                                                            (except maybe the
                                                            real Pattern that
    ...
    case GF_MAGIC_MISSILE: case GF_POISON_GAS:
    case GF_ARROW: case GF_NETHER: case GF_CONFUSION:
    case GF_NEXUS: case GF_INERTIA:
    case GF_LIGHT: case GF_DARK: case GF_TIME:
    *destroy = set_null;
      break;
    default:
	...
	if (c_ptr->cptr > 1)
	  {
        ...
        (void) sprintf(out_val, "%s is wide awake!", m_name);
		msg_print(out_val);
	      }
	    else
	      {
...
int detection()
{
  ...
  for (i = mfptr - 1; i >= MIN_MONIX; i--)
      ...
      if (panel_contains((int)m_ptr->fy, (int)m_ptr->fx))
	{
      ...
      print((char) (no_graphics_flag?c_list[m_ptr->mptr].cchar
      :c_list[m_ptr->mptr].gchar),
       (int)m_ptr->fy,
		(int)m_ptr->fx);
	  if (!no_color_flag) textcolor(LIGHTGRAY);
#else
      print((char)(no_graphics_flag?c_list[m_ptr->mptr].cchar
      :c_list[m_ptr->mptr].gchar), (int)m_ptr->fy,
		(int)m_ptr->fx);
#endif
	  detect = TRUE;
	}
...
int detect_invisible()
{
  ...
  for (i = mfptr - 1; i >= MIN_MONIX; i--)
	  ...
	  (CM_INVISIBLE & c_list[m_ptr->mptr].cmove))
	{
      ...
      print((char)(no_graphics_flag?
      c_list[m_ptr->mptr].cchar:
      c_list[m_ptr->mptr].gchar), (int)m_ptr->fy,
		(int)m_ptr->fx);
	  if (!no_color_flag) textcolor(LIGHTGRAY);
#else
...
/* Identify an object        From CWS's unix angband  */
  ...
  for (i = mfptr - 1; i >= MIN_MONIX; i--)
    {
      m_ptr = &m_list[i];
      m_ptr->csleep = 0;
      if ((m_ptr->cdis <= dis_affect) && (m_ptr->cspeed < 30))
	{
      m_ptr->cspeed += 7+randint(5);
	  aggravate = TRUE;
	}
    }
  if (aggravate)
...
/*        inven_damage(set_frost_destroy, 5);  cold_dam does this already - TY */
	    break;
	  case GF_FIRE:
	    if (blind) 
	  ...
	  case GF_HOLY_ORB:
	    if (blind) 
          msg_print("You are hit by something that fills your heart with awe!");
        dam_hp /= 2; /* player should take less damage -CFT

                        Hmm... maybe he's not a goody goody?
                        Good rogues? (Left it
                            unchanged, though) TY*/
	    take_hit(dam_hp, ddesc);
	    break;
	  case GF_ARROW: /* maybe can miss? */
	    if (blind) 
          msg_print("You are hit by an arrow!");
	    take_hit(dam_hp, ddesc);
	    break;
	  case GF_PLASMA:  /* no resist to plasma? */
        ...
        && (py.flags.fire_resist || py.flags.fire_im)))
                inven_damage(set_flammable, 7);
	    break;
	  case GF_NETHER:
	    if (blind) 
	  ...
	  case GF_WATER:
        ...
        if (!((py.flags.resist_cold)
            && (py.flags.cold_resist || py.flags.cold_im)))
	  ...
	  case GF_CHAOS:
	    ...
	    take_hit(dam_hp, ddesc);
        inven_damage(set_lightning_destroy, 4);
	    break;
	  case GF_SHARDS:
	    if (blind) 
	  ...
	  case GF_INERTIA:
	    if (blind) 
          msg_print("You are hit by something passive!");
	  ...
	  case GF_LIGHT:
	    if (blind) 
          msg_print("You are hit by something! You shrug.");
	  ...
	  case GF_TIME: /* only some effects from time bolt -CFT */
	    if (blind) 
          msg_print("You are hit by something! Your Rolex stops.");
	    if (randint(2) == 1) {
      ...
      case GF_GRAVITY:
	    ...
	    if (py.flags.ffall) {    /* DGK */
	      dam_hp *= 3; /* these 2 lines give avg dam of .25, ranging from */
   ...
   /*        if (blind)
          ...
          This turned out to be redundant with spells.c  - TY */
	    if (py.flags.shards_resist) {
	      dam_hp *= 6; /* these 2 lines give avg dam of .655, ranging from */
	      dam_hp /= (randint(6)+6); /* .858 to .5 -CFT */
	      }
...
/*         inven_damage(set_frost_destroy, 5); Doesn't make sense... -TY */
        take_hit(dam, ddesc);
		    break;
		  case GF_DISENCHANT:
		    if (py.flags.disenchant_resist) {
		...
		if (i_ptr->flags2 & TR_ARTIFACT)
          ...
          case GF_GRAVITY:
		    if ((!py.flags.sound_resist) && (!py.flags.ffall)) /*DGK*/
		      stun_player( randint( (dam > 90) ? 35 : (dam/3+5) ) );
	    if (py.flags.ffall) {    /* DGK */
	      dam_hp *= 3; /* these 2 lines give avg dam of .33, ranging from */
	      dam_hp /= (randint(6)+6); /* .427 to .25 -CFT */
	      }
	    else {  /* DGK */
	     if ((py.flags.slow > 0) && (py.flags.slow < 32000))
		  ...
		  case GF_MANA:
		    take_hit(dam, ddesc);
            inven_damage(set_lightning_destroy, 5);
		    break;
		  case GF_METEOR:
		    take_hit(dam, ddesc);
            inven_damage(set_flammable, 5);
      ...
      case GF_ROCKET:
/*        if (blind)
          msg_print("There is an explosion!"); Redundant with creature.c! -TY*/
	    if (py.flags.shards_resist) {
	      dam_hp *= 6; /* these 2 lines give avg dam of .655, ranging from */
	      dam_hp /= (randint(6)+6); /* .858 to .5 -CFT */
	      }
      ...
      case GF_NUKE:
	    if (blind) 
          ...
          if ((!py.flags.poison_resist) && (!py.flags.poison_im)
            && (randint(3)==1))
          { int  i, j;
            switch (randint(2)) {
                case 1:
                msg_print("You undergo a mutation!");
                mutate();
                break;
            default:
                  msg_print("You can feel the radiation weakening you.");
		          for (i=0;i<6;i++){
		            py.stats.cur_stat[i]=(py.stats.cur_stat[i]*3)/4;
		            if (py.stats.cur_stat[i] < 3) py.stats.cur_stat[i] = 3;
			    set_use_stat(i);
...
/*#ifdef TC_COLOR
        if (!no_color_flag)
		    color_dam((int32)m_ptr->maxhp,0L);
#endif*/
	      msg_print(out_val);
	      prt_experience();
	    }
	  ...
	  if (spd > 0)
	    {
	      ...
	      speed = TRUE;
		    ...
		    randint((py.misc.lev-10)<1?1:(py.misc.lev-10))+10) ||
           (r_ptr->cdefense & UNIQUE) || (m_ptr->cspeed+spd < 4) )
	    {
          (void) sprintf(out_val, "%s is flexes its muscles and is unaffected.", m_name);
	      msg_print(out_val);
	      m_ptr->csleep = 0;
	    }
	  else
...
int dir, y, x, lvl;
{
  ...
  do
      ...
      else if (c_ptr->cptr > 1)
	{
	  ...
	  if ((r_ptr->level >
	       randint((py.misc.lev-10)<1?1:(py.misc.lev-10))+10) ||
          (r_ptr->cdefense & UNIQUE) || (r_ptr->cmove & MV_ONLY_ATT)
          || ((r_ptr->cdefense & FEARLESS) && (randint(5)>2)))
	    {
	      if (m_ptr->ml && (r_ptr->cdefense & CHARM_SLEEP))
		c_recall[m_ptr->mptr].r_cdefense |= CHARM_SLEEP;
          (void) sprintf(out_val, "%s laughs at your feeble attempt.", m_name);
	      msg_print(out_val);
	      m_ptr->csleep = 0;
	    }
	  else
      ...
      else if (c_ptr->cptr > 1)
	{
	  ...
	  if ((r_ptr->level >
	      ...
	      (r_ptr->cdefense & UNIQUE))
	    {
	      if (m_ptr->ml && (r_ptr->cdefense & CHARM_SLEEP))
		c_recall[m_ptr->mptr].r_cdefense |= CHARM_SLEEP;
          (void) sprintf(out_val, "%s is not confused.", m_name);
	      msg_print(out_val);
	      m_ptr->csleep = 0;
	    }
	  else
...
int death_ray(dir, y, x)
int dir, y, x;
{
  int flag, dist, deathray, i;
  register cave_type *c_ptr;
  register monster_type *m_ptr;
  register creature_type *r_ptr;
  vtype out_val, m_name;
  ...
  do
      ...
      else if (c_ptr->cptr > 1)
	{
	  ...
	  if ((r_ptr->level >
	       randint((py.misc.lev-10)<1?1:(py.misc.lev-10))+10) ||
          (r_ptr->cdefense & UNIQUE) ||(r_ptr->cdefense & UNDEAD)
          || (r_ptr->cdefense & DEMON) || (r_ptr->cdefense & NONLIVING)
          || (r_ptr->cchar == 'A'))
	    {
          (void) sprintf(out_val, "%s resists the death ray!", m_name);
	      msg_print(out_val);
          m_ptr->csleep = 0;
	    }
	  else
	    {
	      monster_name (m_name, m_ptr, r_ptr);
          i = mon_take_hit((int)c_ptr->cptr, 10000);
	      if (i >= 0)
		{
          (void) sprintf(out_val, "%s is killed by the death ray!",m_name);
		  msg_print(out_val);
		  prt_experience();
		}
	      else
		{
		  (void) sprintf (out_val,
            pain_message((int)c_ptr->cptr,10000),m_name);
		  msg_print(out_val);
          m_ptr->csleep = 0;
		}
	    }
...
/*#ifdef TC_COLOR
        if (!no_color_flag)
		  color_dam((int32)m_ptr->maxhp,(int32)m_ptr->hp);
#endif*/
		  msg_print(out_val);
		}
	      flag = TRUE;
  ...
  do
      ...
      else if (c_ptr->cptr > 1)
	{
	  ...
	  if ((r_ptr->level < randint((py.misc.lev-10)<1?1:(py.misc.lev-10))+10)
	    ...
	    {
	      monster_name (m_name, m_ptr, r_ptr);
          (void) sprintf(out_val, "%s feels normal.", m_name);
	      msg_print(out_val);
	    }
	}
    }
...
{   vtype cdesc;
	...
	if ((dist > OBJ_BOLT_RANGE) || c_ptr->fval >= MIN_CLOSED_SPACE)
	    ...
	    m_list[c_ptr->cptr].csleep = 0;
            ...
            if (randint(3) == 1)
             {
                m_list[c_ptr->cptr].cspeed += randint(5);
                if (wizard) msg_print("The monster is hasted.");
            }
      ...
      usleep(50000); /* u-secs */
	  ...
	  else
	    {
		 ...
		 this message makes no sense otherwise */
	      if (r_ptr->cdefense & UNIQUE)
        (void) sprintf(out_val, "%s is cannot be genocided!", r_ptr->name);
	      else
        (void) sprintf(out_val, "The %s cannot be genocided!", r_ptr->name);
	      msg_print(out_val);
...
int spd;
{
  ...
  for (i = mfptr - 1; i >= MIN_MONIX; i--)
	...
	{
      (void) sprintf(out_val, "%s flexes its muscles and is unaffected.", m_name);
	  msg_print(out_val);
	}
    }
  return(speed);
...
void lose_int()
{
  if (!py.flags.sustain_int)
    {
      (void) dec_stat(A_INT);
      msg_print("You feel stupid!");
    }
  else
    msg_print("You don't become any more stupid than you are.");
}
...
void lose_wis()
{
  if (!py.flags.sustain_wis)
    ...
    msg_print("You feel about as dumb as normally.");
}
...
int16 limit;
    /* maximum bonus allowed; usually 10, but weapon's maximum damage (DKG) */
int plusses = i_ptr->todam;
  ...
  if (limit <= 0) /* avoid randint(0) call */
    return(FALSE);
  if (limit > 25) /* Glaives of Pain (+10, +60) are -BAD-! -DGK */
    limit = 25;
  for(i=0; i<n; i++){
    chance = 0;
    if (eflag & ENCH_TOHIT) {
    ...
    if (eflag & ENCH_TODAM) {
      ...
      if ((randint(limit)>chance) && (!a || randint(7)>3)) {
      	i_ptr->todam++;
      	res = TRUE;
  ...
  for (i = INVEN_WIELD; i <= INVEN_OUTER; i++)
    {
     ...
     !(py.misc.pclass != 0 && i_ptr->name2 == SN_GORLIM) /* Only
                                           warriors can remove the Terror Mask
                                           with the vanilla Remove Curse */
     &&  (i_ptr->name2 != SN_MORMEGIL))
	{
      if (!(!stricmp(object_list[i_ptr->index].name, "Power (The One Ring)") &&
	    (i_ptr->tval == TV_RING))) {
	      i_ptr->flags &= ~TR_CURSED;
...
void self_knowledge(void){
  ...
  if (py.flags.aggravate)
    prt("Monsters are madly attacking you.", i++, j);
  ...
  if (f & TR_STEALTH)
    prt("You can sneak around stealthily.", i++, j);
  if (f & TR_SEARCH){
    prt("You are very perceptive.", i++, j);
  ...
  if (py.flags.telepathy){
    prt("You can read other creatures' minds telepathically.", i++, j);
    pause_if_screen_full(&i, j);
  }
  ...
  if (py.flags.blindness_resist){
    prt("You cannot become blind.", i++, j);
    pause_if_screen_full(&i, j);
  }
  if (py.flags.fire_im){
...
/* Are these needed?  The player can see this...  For now, in here for
  ...
  if (py.flags.sustain_int){
    prt("You will not become more stupid.", i++, j);
    pause_if_screen_full(&i, j);
  }
  if (py.flags.sustain_wis){
  ...
  if (py.flags.sustain_chr){
    prt("You will not become uglier.", i++, j);
    pause_if_screen_full(&i, j);
  }
 ...
 if (f2 & TR_TY_CURSE) {
    prt("A foul ancient spell is upon you.", i++, j);
    pause_if_screen_full(&i, j);
    }
  ...
  if (f2 & TR_VORPAL){
    prt("You sword is extremely sharp.", i++, j);
    pause_if_screen_full(&i, j);
  }
  if (f2 & TR_SLAY_ORC){
...
static void spell_hit_monster(monster_type *m_ptr, int typ, int *dam, int rad,
  ...
  *x = m_ptr->fx; 
  ...
  switch ( typ ){ /* check for resists... */
	...
	*dam /= (randint(6)+6); /* from .427 to .25 -CFT */
	...
	res = IMMUNE;
      ...
      break;
    case GF_CHAOS:
      if ((r_ptr->spells2 & BREATH_CH) || (r_ptr->spells3 & LOGRUS_BALL)
        || ((r_ptr->cdefense & DEMON) && (randint(3)==1))) /* Demons resist
                            sometimes. -TY */
        { /* assume anything that breathes
	...
	*dam /= (randint(6)+6); /* from .427 to .25 -CFT */
      ...
      !(r_ptr->spells2 & BREATH_CH) && !(r_ptr->spells & LOGRUS_BALL) &&
	  !(r_ptr->cdefense & UNIQUE) &&
	...
	*dam /= (randint(6)+6);
      ...
      else if ((r_ptr->cdefense & HURT_LIGHT) || (r_ptr->cdefense & ORC))
                /* Since PC half-orcs resist darkness - no matter what "orc"
                    race - I believe all NPC orcs should, too... -TY */
        {
	res = SOME_RES;
  ...
  if (res == CHANGED)
    sprintf(outval, "%sis changed!",cdesc);
  else if ((*dam > m_ptr->hp) &&
	   (by_player || !(c_list[m_ptr->mptr].cdefense & UNIQUE))) {
    res = DEAD;
    if ((c_list[m_ptr->mptr].cdefense & (DEMON|UNDEAD|NONLIVING)))
...
/*#ifdef TC_COLOR
     if (!no_color_flag)
      color_dam(m_ptr->maxhp,((int32)m_ptr->hp)-(int32)*dam);
#endif*/
      msg_print(outval);
  }	
}

void line_spell(typ, dir, y, x, dam)
int typ, dir, y, x, dam;
{
