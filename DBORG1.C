...
bool my_inviso;         /* timed inviso spell */
...
void borg_update_frame(void)
{
    ...
    auto_level = borg_skill[BI_CLEVEL] = p_ptr->lev;

    /* Note "Winner" */
    borg_king = p_ptr->total_winner || (p_ptr->lev > PY_MAX+LEVEL);  /* ??? */
    ...
    /* Check for "Poisoned" */
    if (p_ptr->nasty_poisoned) do_nasty_pois = TRUE;
