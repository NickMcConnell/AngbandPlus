...
static cptr ghost_race_names[] =
{
    ...
    "pixie","yeek","dragon","dragon","dragon",
...
{
    TERM_UMBER, TERM_RED, TERM_L_BLUE,
...
static void set_ghost_aux_1(void)
{
    ...
    cptr gr_name = ghost_race_names[grace];
    cptr gc_name = ghost_class_names[gclass];


    /* A wanderer from the town */
    sprintf(r_name + r_ptr->name, "%s, the undead %s %s",
    ...
    /* Use a "player" color */
    r_ptr->d_attr = ghost_class_colors[gclass];
    ...
    if (grace >= RACE_MIN_DRAGON) r_ptr->flags3 |= (RF3_DRAGON);
    ...
    /* Pick suitable attack types */
    if (grace >= RACE_MIN_DRAGON)
    {
        attack1 = RBM_BITE;
        attack2 = RBM_CLAW;
    }
...
static void set_ghost_aux_2(void)
{
    ...
    if (ghost_race < RACE_MIN_DRAGON)
    {
        switch ((lev / 4) + randint(3))
        {
            case 1: case 2: case 3:
            {
                sprintf(r_name + r_ptr->name, "%s, the Skeletal %s", gb_name, gr_name);
                r_ptr->d_char = 's';
    ...
    else
    {
        switch ((lev / 4) + randint(3))
        {
            case 1: case 2: case 3:
            {
                ...
                ghost_blow(0, RBM_BITE, RBE_HURT, 2, 6);
                ghost_blow(1, RBM_CLAW, RBE_HURT, 2, 6);
            ...
            case 6: case 7:
            {
                ...
                ghost_blow(0, RBM_BITE, RBE_HURT, 3, 8);
                ghost_blow(1, RBM_CLAW, RBE_HURT, 3, 8);
                ghost_blow(2, RBM_BITE, RBE_HURT, 3, 8);
            ...
            case 9: case 10:
            {
                ...
                ghost_blow(0, RBM_TOUCH, RBE_LOSE_WIS, 2, 6);
                ghost_blow(1, RBM_TOUCH, RBE_LOSE_DEX, 2, 6);
                ghost_blow(2, RBM_BITE, RBE_HURT, 4, 6);
                ghost_blow(3, RBM_WAIL, RBE_TERRIFY, 0, 0);
            ...
            case 11:
            {
                sprintf(r_name + r_ptr->name, "%s, the Death Drake", gb_name);
                r_ptr->d_char = 'D';
                r_ptr->d_attr = TERM_L_GREEN;
                r_ptr->flags1 |= (RF1_DROP_1D2);
                r_ptr->flags2 |= (RF2_INVISIBLE | RF2_PASS_WALL);
                r_ptr->flags3 |= (RF3_IM_COLD);
                r_ptr->flags5 |= (RF5_BLIND | RF5_HOLD | RF5_DRAIN_MANA);
            ...
            case 12:
            {
                ...
                ghost_blow(0, RBM_BITE, RBE_HURT, 5, 8);
                ghost_blow(1, RBM_CLAW, RBE_HURT, 5, 8);
                ghost_blow(2, RBM_BITE, RBE_EXP_40, 0, 0);
            ...
            case 13:
            {
                sprintf(r_name + r_ptr->name, "%s, the Dracolich", gb_name);
                r_ptr->d_char = 'D';
                r_ptr->d_attr = TERM_L_GREEN;
                r_ptr->flags1 |= (RF1_DROP_2D2 | RF1_DROP_4D2);
                r_ptr->flags2 |= (RF2_OPEN_DOOR | RF2_BASH_DOOR);
                ...
                ghost_blow(0, RBM_BITE, RBE_HURT, 6, 8);
                ghost_blow(1, RBM_CLAW, RBE_HURT, 6, 8);
                ghost_blow(2, RBM_TOUCH, RBE_EXP_20, 0, 0);
            ...
            case 14:
            {
                ...
                ghost_blow(0, RBM_BITE, RBE_HURT, 6, 8);
                ghost_blow(1, RBM_CLAW, RBE_HURT, 6, 8);
                ghost_blow(2, RBM_BITE, RBE_HURT, 6, 8);
                ghost_blow(3, RBM_CLAW, RBE_EXP_80, 0, 0);
...
static void set_ghost(cptr pname, int hp, int grace, int gclass, int lev, bool town)
{
    ...
    /* All ghosts see invisible */
    r_ptr->flags2 |= RF2_SEE_INVIS;
...
s16b place_ghost(void)
{
    ...
    else
    {
        ...
        if (14 > randint((p_ptr->depth / 2) + 11)) return (FALSE);
    ...
    while (1)
    {
        ...
        /* Pick a location */
        y = randint(DUNGEON_HGT - 2);
        x = randint(DUNGEON_WID - 2);
    ...
    if (!place_monster_one(y, x, MAX_R_IDX-1, FALSE))
    {
        return FALSE;
    }
 ...
 * XXX XXX XXX
 *
 * Reads the bone file for the current depth. Create the monster and
 * return an index to it in r_info.
 */
...
void update_smart_learn(int m_idx, int what)
{
    ...
    switch (what)
    {
        ...
        case DRS_RES_ACID:
        {
            if (p_ptr->resist_acid < 100) m_ptr->smart |= (SM_RES_ACID);
        ...
        case DRS_RES_ELEC:
        {
            if (p_ptr->resist_elec < 100) m_ptr->smart |= (SM_RES_ELEC);
        ...
        case DRS_RES_FIRE:
        {
            if (p_ptr->resist_fire < 100) m_ptr->smart |= (SM_RES_FIRE);
        ...
        case DRS_RES_COLD:
        {
            if (p_ptr->resist_cold < 100) m_ptr->smart |= (SM_RES_COLD);
        ...
        case DRS_RES_POIS:
        {
            if (p_ptr->resist_pois < 100) m_ptr->smart |= (SM_RES_POIS);
        ...
        case DRS_RES_FEAR:
        {
            if (p_ptr->resist_fear < 100) m_ptr->smart |= (SM_RES_FEAR);
            break;
        }

        case DRS_RES_LITE:
        {
            if (p_ptr->resist_lite < 100) m_ptr->smart |= (SM_RES_LITE);
            break;
        }

        case DRS_RES_DARK:
        {
            if (p_ptr->resist_dark < 100) m_ptr->smart |= (SM_RES_DARK);
            break;
        }

        case DRS_RES_BLIND:
        {
            if (p_ptr->resist_blind < 100) m_ptr->smart |= (SM_RES_BLIND);
            break;
        }

        case DRS_RES_CONFU:
        {
            if (p_ptr->resist_confu < 100) m_ptr->smart |= (SM_RES_CONFU);
            break;
        }

        case DRS_RES_SOUND:
        {
            if (p_ptr->resist_sound < 100) m_ptr->smart |= (SM_RES_SOUND);
            break;
        }

        case DRS_RES_SHARD:
        {
            if (p_ptr->resist_shard < 100) m_ptr->smart |= (SM_RES_SHARD);
            break;
        }

        case DRS_RES_NEXUS:
        {
            if (p_ptr->resist_nexus < 100) m_ptr->smart |= (SM_RES_NEXUS);
            break;
        }

        case DRS_RES_NETHR:
        {
            if (p_ptr->resist_nethr < 100) m_ptr->smart |= (SM_RES_NETHR);
            break;
        }

        case DRS_RES_CHAOS:
        {
            if (p_ptr->resist_chaos < 100) m_ptr->smart |= (SM_RES_CHAOS);
            break;
        }

        case DRS_RES_DISEN:
        {
            if (p_ptr->resist_disen < 100) m_ptr->smart |= (SM_RES_DISEN);
            break;
        }
    }
