	      ...
	      for (i = 0; i < nobj; i++)
		{
		  ...
		  again:
		    ...
		    case (SPECIAL_OBJ-1):
		      ...
		      break;
		    case (SPECIAL_OBJ):
              if (object_list[tmp].level <= level) msg_print("[Pattern Sorcery]");
		      break;
		    case (SPECIAL_OBJ+1):
              if (object_list[tmp].level <= level) msg_print("[Merlin's Ways]");
		      break;
		    case (SPECIAL_OBJ+2):
              if (object_list[tmp].level <= level) msg_print("[B'harne Fragments]");
		      break;
		    case (SPECIAL_OBJ+3):
		      if (randint(2)==1) goto again;
              if (object_list[tmp].level <= level) msg_print("[Pnakotic Manuscripts]");
		      break;
		    case (SPECIAL_OBJ+4):
		      if (randint(2)==1) goto again;
              if (object_list[tmp].level <= level) msg_print("[Necronomicon]");
		      break;
		    case (SPECIAL_OBJ+5):
              if (object_list[tmp].level <= level) msg_print("[Pilgrim's Diary]");
		      break;
		    case (SPECIAL_OBJ+6):
              if (object_list[tmp].level <= level) msg_print("[Visions of the Grail]");
		      break;
		    case (SPECIAL_OBJ+7):
              if (object_list[tmp].level <= level) msg_print("[De Vermiis Mysteriis]");
		      break;
		    case (SPECIAL_OBJ+8):
              if (object_list[tmp].level <= level) msg_print("[The Book of the Dead]");
		      break;
		    case (SPECIAL_OBJ+9):
		      if (randint(2)>1) goto again;
              if (object_list[tmp].level <= level) msg_print("[The Book of the Unicorn]");
		      break;               /* End of Books */
