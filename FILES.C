...
static void display_player_xtra_info(void)
{
    ...
    Term_putstr(col+9, 2, -1, TERM_L_BLUE, format("%4d", (int)p_ptr->age));

    /* Height */
    Term_putstr(col, 3, -1, TERM_WHITE, "Height");
    Term_putstr(col+9, 3, -1, TERM_L_BLUE, format("%4d", (int)p_ptr->ht));

    /* Weight */
    Term_putstr(col, 4, -1, TERM_WHITE, "Weight");
    Term_putstr(col+9, 4, -1, TERM_L_BLUE, format("%4d", (int)p_ptr->wt));
    ...
    /* Maximise */
    Term_putstr(col, 6, -1, TERM_WHITE, "Maximise");
    Term_putstr(col+12, 6, -1, TERM_L_BLUE, p_ptr->maximise ? "Y" : "N");

    /* Preserve */
    Term_putstr(col, 7, -1, TERM_WHITE, "Preserve");
    Term_putstr(col+12, 7, -1, TERM_L_BLUE, p_ptr->preserve ? "Y" : "N");
    ...
    /* Level */
    Term_putstr(col, 9, -1, TERM_WHITE, "Level");
    ...
    {
        Term_putstr(col+8, 9, -1, TERM_YELLOW,
                    format("%10d", p_ptr->lev));
    }


    /* Current Experience */
    Term_putstr(col, 10, -1, TERM_WHITE, "Cur Exp");
    ...
    /* Maximum Experience */
    Term_putstr(col, 11, -1, TERM_WHITE, "Max Exp");
    Term_putstr(col+8, 11, -1, TERM_L_GREEN,
                format("%10ld", p_ptr->max_exp));


    /* Advance Experience */
    Term_putstr(col, 12, -1, TERM_WHITE, "Adv Exp");
    ...
    Term_putstr(col, 9, -1, TERM_WHITE, "Armor");
    ...
    /* Melee attacks */
    sprintf(buf, "(%+d,%+d%%)", hit, dam);
    Term_putstr(col, 11, -1, TERM_WHITE, "Melee");
    ...
    desc = likert(xsav, 6);
    c_put_str(likert_color, format("%9s", desc), 9, col+14);

    put_str("Stealth", 10, col);
    desc = likert(xstl, 1);
    ...
    put_str("Perception", 15, col);
    desc = likert(xfos, 6);
    c_put_str(likert_color, format("%9s", desc), 15, col+14);

    put_str("Searching", 16, col);
    desc = likert(xsrh, 6);
...
extern void player_flags(u32b *f1, u32b *f2, u32b *f3)
{
    ...
    /* Pixie */
    if (p_ptr->prace == RACE_PIXIE) (*f1) |= (TR1_SPEED);
    if (p_ptr->prace == RACE_PIXIE) (*f3) |= (TR3_FEATHER);
    if (p_ptr->prace == RACE_PIXIE && p_ptr->lev > 40) (*f3) |= (TR3_TELEPATHY);

    /* Yeek */

    /* Crystal Dragon */
    if (p_ptr->prace == RACE_CRYSTALDRAG) (*f3) |= (TR3_FREE_ACT);
    if (p_ptr->prace == RACE_CRYSTALDRAG) (*f3) |= (TR3_FEATHER);
    if (p_ptr->prace == RACE_CRYSTALDRAG) (*f2) |= (TR2_RES_SHARD);
    if (p_ptr->prace == RACE_CRYSTALDRAG) (*f2) |= (TR2_RES_FEAR);

    /* Copper Dragon */
    if (p_ptr->prace == RACE_COPPERDRAG) (*f3) |= (TR3_FREE_ACT);
    if (p_ptr->prace == RACE_COPPERDRAG) (*f3) |= (TR3_FEATHER);
    if (p_ptr->prace == RACE_COPPERDRAG) (*f2) |= (TR2_RES_DISEN);
    if (p_ptr->prace == RACE_COPPERDRAG) (*f2) |= (TR2_RES_FEAR);
    if (p_ptr->prace == RACE_COPPERDRAG) (*f3) |= (TR3_REGEN);

    /* Bronze Dragon */
    if (p_ptr->prace == RACE_BRONZEDRAG) (*f3) |= (TR3_FEATHER);
    if (p_ptr->prace == RACE_BRONZEDRAG) (*f2) |= (TR2_RES_CONFU);
    if (p_ptr->prace == RACE_BRONZEDRAG) (*f2) |= (TR2_RES_FEAR);
    if (p_ptr->prace == RACE_BRONZEDRAG) (*f3) |= (TR3_REGEN);

    /* Gold [Brass] Dragon */
    if (p_ptr->prace == RACE_GOLDDRAG) (*f3) |= (TR3_FEATHER);
    if (p_ptr->prace == RACE_GOLDDRAG) (*f2) |= (TR2_RES_SOUND);
    if (p_ptr->prace == RACE_GOLDDRAG) (*f2) |= (TR2_RES_FEAR);
    if (p_ptr->prace == RACE_GOLDDRAG) (*f3) |= (TR3_REGEN);

    /* Pseudo Dragon */
    if (p_ptr->prace == RACE_PSEUDODRAG) (*f3) |= (TR3_FEATHER);
    if (p_ptr->prace == RACE_PSEUDODRAG) (*f2) |= (TR2_RES_LITE);
    ...
    if (p_ptr->prace == RACE_MULTIHUEDDRAG && p_ptr->lev >= 45) (*f2) |= (TR2_RES_POIS);
 ...
 */
static void display_player_flag_info(int y_offset)
{
    int x, y, i, n;
    ...
    for (x = 0; x < 4; x++)
    {
        ...
        for (y = 0; y < 8; y++)
        {
            ...
            for (n=6, i=INVEN_WIELD; i<INVEN_TOTAL; i++,n++)
            {
                ...
                if ((x==0) && (y==1) && (f[set] & TR2_IM_ELEC))
                    c_put_str(TERM_WHITE, "*", row, col+n);
                if ((x==0) && (y==2) && (f[set] & TR2_IM_FIRE))
                    c_put_str(TERM_WHITE, "*", row, col+n);
                if ((x==0) && (y==3) && (f[set] & TR2_IM_COLD))
                    c_put_str(TERM_WHITE, "*", row, col+n);
            ...
            /* Display temp resists -TM- */
                  if ((x==0) && (y==0) && (p_ptr->oppose_acid))
                    c_put_str(TERM_WHITE, "#", row, col+n);
                  if ((x==0) && (y==1) && (p_ptr->oppose_elec))
                        c_put_str(TERM_WHITE, "#", row, col+n);
                  if ((x==0) && (y==2) && (p_ptr->oppose_fire))
                        c_put_str(TERM_WHITE, "#", row, col+n);
                  if ((x==0) && (y==3) && (p_ptr->oppose_cold))
                        c_put_str(TERM_WHITE, "#", row, col+n);
                  if ((x==0) && (y==4) && (p_ptr->oppose_pois))
                        c_put_str(TERM_WHITE, "#", row, col+n);
                  if ((x==1) && (y==5) && (p_ptr->protevil))
                        c_put_str(TERM_WHITE, "#", row, col+n);
...
static void display_player_misc_info(void)
{
    ...
    /* Class */
    put_str("Class", 4, 1);
    c_put_str(TERM_L_BLUE, cp_ptr->title, 4, 8);
 ...
 */
static void display_player_stat_info(int y_offset)
{
    int i, row, col;
 ...
 */
static void display_player_sust_info(int y_offset)
{
    int i, row, col, stat;
 ...
 * Mode 2 = character dump to a file
 */
void display_player(int mode)
{
    ...
    /* Large text mode -- print both */
    if (screen_y >= 43 && character_generated)
    {
        /* Extra info */
...
errr file_character(cptr name, bool full)
{
    ...
    if (fd >= 0)
    {
        ...
        /* Build query */
        sprintf(out_val, "Append to existing file %s? ", buf);
    ...
    /* Begin dump */
    fprintf(fff, "\n  [DrAngband %d.%d.%d Character Dump]\n\n",
...
bool show_file(cptr name, cptr what, int line, int mode)
{
    ...
    while (TRUE)
    {
        ...
        {
            /* Wait for it */
            prt("[Press a Number, or ESC to exit.]", screen_y-2, 0);
        }
        ...
        {
            /* Wait for it */
            prt("[Press ESC to exit.]", screen_y-2, 0);
        }
        ...
        {
            /* Wait for it */
            prt("[Press Return, Space, -, =, /, or ESC to exit.]", screen_y-2, 0);
        }
...
static void show_info(void)
{
    ...
    prt("Hit any key to see more information (ESC to abort): ", screen_y-2, 0);
    ...
    if (p_ptr->inven_cnt)
    {
        ...
        while (inven_start < INVEN_PACK)
        {
            ...
            Term_clear();
            item_tester_full = TRUE;
            show_inven(inven_start);
            sprintf(temp, "[Page %d/%d] You are carrying: -more-", j, i);
            prt(temp, 0, 0);
            if (inkey() == ESCAPE) return;
            inven_start += screen_y-3;
        }
...
static void display_scores_aux(int from, int to, int note, high_score *score)
{
    ...
    for (k = from, place = k+1; k < i; k += 5)
    {
        ...
        /* Wait for response */
        prt("[Press ESC to quit, any other key to continue.]", screen_y-2, 17);
        j = inkey();
        prt("", screen_y-2, 0);
...
void display_scores(int from, int to)
{
    ...
    /* Wait for response */
    prt("[Press any key to quit.]", screen_y-2, 17);
    (void)inkey();
    prt("", screen_y-2, 0);
