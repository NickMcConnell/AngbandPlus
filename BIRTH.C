...
static hist_type bg[] =
{
    ...
    {"You are a red dragon from the equatorial Wild.  ",             25,68,44, 40},
    {"You are a red dragon from the equatorial Mountains.  ",        50,68,44, 50},
    {"You are a red dragon from the fortress of Angband.  ",        100,68,44, 30},
    {"You are a white dragon from the Northern Wild.  ",             25,69,44, 40},
    {"You are a white dragon from the Northern Misty Mountains.  ",  50,69,44, 50},
    {"You are a white dragon from the fortress of Angband.  ",      100,69,44, 30},
    {"You are a blue dragon from the Wild.  ",                       25,70,44, 40},
    {"You are a blue dragon from deep within the Misty Mountains.  ",50,70,44, 50},
    {"You are a blue dragon from the fortress of Angband.  ",       100,70,44, 30},
    {"You are a green dragon from the Mirkwood.  ",                  25,71,44, 40},
    {"You are a green dragon from the Mirkwood Mountains.  ",        50,71,44, 50},
    {"You are a green dragon from the fortress of Angband.  ",      100,71,44, 30}
};
 ...
 * auto-roller initialization code.  Otherwise, if "maximise" mode
...
static bool player_birth_aux()
{
    ...
    while (1)
    {
        put_str("Use 'maximise' mode? (y/n) ", 20, 2);
        c = inkey();
        if (c == 'Q') quit(NULL);
        if (c == 'S') return (FALSE);
        if (c == ESCAPE) break;
        if ((c == 'y') || (c == 'n')) break;
        if (c == '?') do_cmd_help();
        else bell("Illegal maximise flag!");
    }
