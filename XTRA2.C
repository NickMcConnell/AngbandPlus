...
/*
 * Set "p_ptr->nasty_pois", notice observable changes
 */
bool set_nasty_poisoned(bool set)
{
	bool notice = FALSE;
	...
	if (set)
	{
		if (!p_ptr->nasty_pois)
		{
			msg_print("You feel deathly cold...");
			notice = TRUE;
		}
	}
	...
	else
	{
		if (p_ptr->poisoned)
		{
			msg_print("You feel revived!");
			notice = TRUE;
		}
	}

	/* Use the value */
	p_ptr->nasty_pois = set;
	...
	/* Redraw the "poisoned"
	 * Note: This now doubles for nasty poison
	 */
...
/*
 * Set "p_ptr->ethereal_lock", notice observable changes
 */
bool set_ethereal_lock(int v)
{
	bool notice = FALSE;
	...
	if (v)
	{
		...
		{
                msg_print("An ethereal lock forms on your world!");
			notice = TRUE;
		}
	}
	...
	else
	{
        if (p_ptr->ethereal_lock)
		{
            msg_print("The heavens are no longer sealed.");
			notice = TRUE;
		}
	}

	/* Use the value */
    p_ptr->ethereal_lock = v;
...
bool mon_take_hit(int m_idx, int dam, bool *fear, cptr note)
{
    ...
    /* note the damage */
    if (p_ptr->wizard) msg_format("You do %d (out of %d) damage.", dam, m_ptr->hp);
...
void verify_panel(void)
{
	...
	/* Hack -- handle town */
	if (!p_ptr->depth && !p_ptr->new_town) i = (DUNGEON_WID-SCREEN_WID)/2;
