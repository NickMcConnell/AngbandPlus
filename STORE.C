...
static void store_create(void)
{
    ...
    for (tries = 0; tries < 4; tries++)
    {
        ...
        if (store_num == 6)
        {
            ...
            level = p_ptr->lev + 25 + rand_int(25);
        ...
        else
        {
            ...
            level = rand_range(1, STORE_OBJ_LEVEL + p_ptr->lev);
        }
...
static void store_purchase(void)
{
    ...
    /* Hack -- If a rod, allocate total maximum timeouts
    ...
    {
        /* Hack -- If a rod , allocate total maximum
         * timeouts between those picked up and
...
static void store_sell(void)
{
    ...
    /* Hack -- If a rod, allocate total maximum
     * timeouts to those being sold. -LM-
     */
    if (o_ptr->tval == TV_ROD)
    {
        i_ptr->pval = o_ptr->pval * amt / o_ptr->number;
    ...
    if (store_num != 7)
    {
        ...
        if (choice == 0)
        {
            ...
            /* Hack -- If a rod, let the shopkeeper know just
            ...
            /* Hack -- Allocate charges between those rods sold
    ...
    {
        /* Hack -- If a rod, allocate total maximum timeouts
         * between those dropped and kept in the backpack, unless
...
void do_cmd_store(void)
{
    ...
    while (!leave_store)
    {
        ...
        /* Clear */
        clear_from(screen_y-4);

        /* Basic commands */
        prt(" ESC) Exit from Building.", screen_y-3, 0);

        /* Browse if necessary */
        if (st_ptr->stock_num > fit)
        {
            prt(" SPACE) Next page of stock", screen_y-2, 0);
        }

        /* Commands */
        prt(" g) Get/Purchase an item.", screen_y-3, 40);
        prt(" d) Drop/Sell an item.", screen_y-2, 40);

        /* Prompt */
        prt("You may: ", screen_y-4, 0);
