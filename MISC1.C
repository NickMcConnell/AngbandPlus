...
≠Á6ÎáàÏÌÓÔ“‘’ÙÎıˆPÿ◊.õú
    ...
    ÂÊËÈÍÎÏÌÓÔ6ÒÚÛ∂ıˆ˜¯˘˙˚¸˝˛Üáàâçèêëíùû6ü°•®©™≥¥∏ä66ŸÉÖæ›‡ﬁÑ·‚„‰
    ∂∑∏π∫ªºΩæø¿¡¬√ƒ≈√«»… ÀÃÕŒœ–—»… ÀÃÕŒœ–—±≤Ωªæø¿¡¬√ƒ≈∆«»… À
    Üáàâäçëíëîïóòôòôöúùûü†°ò¢ß£§•¶ß®©™´¨≠Æ6Ø∞±≤≤≤≥¥6µ∂É
    öôõ‘Ÿ
'‡'
...
int slp;
{
  ...
  mon_ptr->cspeed = c_list[z].speed - 13 + randint(6); /* A little random
							element! - TY */
  mon_ptr->stunned = 0;
  ...
  if (c_list[z].cdefense & UNIQUE){
    j=c_list[z].cchar;
    if((j=='k')||(j=='L')||(j=='o')||(j=='O')||(j=='C')||(j=='I')||
       (j=='&')||(j=='S')||(j=='A')||(j=='x')||(j=='M')||(j=='e')
       ||(j=='j')||(j=='Y')||(j=='n')||(j=='$')||(j=='i')){
...
int place_win_monster()
{
  ...
  if (!total_winner) {
    ...
    mon_ptr->cspeed = c_list[mon_ptr->mptr].speed - 13 + randint(6);
    mon_ptr->stunned = 0;
    mon_ptr->cdis = distance(char_row, char_col,y,x);
    cave[y][x].cptr = cur_pos;
    mon_ptr->csleep = 0;
  ...
  int r, c, l;
{
  ...
  if (!dun_level) {
    ...
    g->cdefense |= CAN_SPEAK; /* We cans speak... duh! TY */
  ...
  case 3:
    ...
    g->spells |= (NONE8);
    g->cdefense |= (CHARM_SLEEP|UNDEAD|EVIL|IM_FROST|NO_INFRA); /*Skeletons
                                don't speak */
    if (r==6)
      g->cdefense |= ORC;
    else if (r==7)
  ...
  case 5:
    ...
    if (r==6)           /* Zombies don't speak either */
      g->cdefense |= ORC;
    else if (r==7)
      g->cdefense |= TROLL;
    g->ac = 30;
  ...
  case 6:
    ...
    g->spells |= (NONE8);
    g->cdefense |= (CHARM_SLEEP|UNDEAD|EVIL|IM_FROST|NO_INFRA|CAN_SPEAK);
    g->ac = 20;
    g->speed = 42;
    g->gchar = 'Ò';
    g->cchar = 'G';
  ...
  case 12:
    ...
    g->cdefense |= (CHARM_SLEEP|UNDEAD|EVIL|IM_FROST|NO_INFRA|HURT_LIGHT|CAN_SPEAK);
    g->ac = 40;
    g->speed = 24;
    g->gchar = 'Ÿ';
    g->cchar = 'V';
  ...
  case 14:
    ...
    break;
  case 15:
     sprintf(g->name, "%s%s Logrus ghost", name, (name[strlen(name)-1]=='s')?
	    "'":"'s");
  ...
  case 18:
  default:
    sprintf(g->name, "%s%s Pattern ghost", name, (name[strlen(name)-1]=='s')?
	    "'":"'s");
    g->cmove |= (MV_INVIS|THRO_WALL|MV_ATT_NORM|CARRY_OBJ|HAS_4D2|HAS_2D2);
    g->spells |= (0x2L|HOLD_PERSON|MANA_DRAIN|BLINDNESS|CONFUSION|TELE_TO);
    g->spells2 |= (NETHER_BOLT|NETHER_BALL|BRAIN_SMASH|TELE_LEV);
    g->cdefense |= (CHARM_SLEEP|UNDEAD|EVIL|IM_FROST|NO_INFRA|INTELLIGENT|CAN_SPEAK);
    g->ac = 130;
    g->speed = 41;
    g->gchar = 'Ò';
    g->cchar = 'G';
  ...
  int y,x,mon,slp;
{
  ...
  if (c_list[mon].level > dun_level)
    ...
    if (dun_level && (num_screen_wide * num_screen_high < MAX_SCREENS))
	extra -= (MAX_SCREENS / num_screen_wide / num_screen_high);
 /* No big packs on tiny levels, please... TY, ES*/
...
/* TY: Summon knights - was Summon ringwraiths */
  ...
  do
    {
     ...
     do
       {
     if (c_list[m].cchar == 'k' && !(c_list[m].cdefense & UNIQUE))
	   {
	      ctr = 20;
	      l = 0;
	   }
  ...
  do
    {
     ...
     do
       {
     if (c_list[m].cchar == 'i' && !(c_list[m].cdefense & UNIQUE))
	   {
	      ctr = 20;
	      l = 0;
	   }
...
int give_1_hi_resist(t)   /* JLS: gives one of the "new" resistances to */
...
register inven_type *t;
{
 ...
 switch (randint(13))
  {
   case 1:  t->flags2 |= TR_RES_CONF; tilap = BROWN; break;
   case 2:  t->flags2 |= TR_RES_SOUND; tilap = YELLOW; break;
   case 3:  t->flags2 |= TR_RES_LT; tilap = YELLOW; break;
   case 4:  t->flags2 |= TR_RES_DARK; tilap = DARKGRAY;  break;
   case 5:  t->flags2 |= TR_RES_CHAOS; tilap = DARKGRAY; break;
   case 6:  t->flags2 |= TR_RES_DISENCHANT; tilap = CYAN; break;
   case 7:  t->flags2 |= TR_RES_SHARDS; tilap = LIGHTGRAY; break;
   case 8:  t->flags2 |= TR_RES_NEXUS; tilap = LIGHTMAGENTA; break;
   case 9:  t->flags2 |= TR_RES_BLIND; tilap = DARKGRAY; break;
   case 10: t->flags2 |= TR_RES_NETHER; tilap = DARKGRAY; break;
   case 11: t->flags2 |= TR_RES_FEAR; tilap = LIGHTRED; break;
   case 12: t->flags  |= TR_POISON; tilap = LIGHTGREEN; break;
   case 13: t->flags  |= TR_FFALL; tilap = WHITE; break; /* Also Resist Gravity */
   }
  return tilap;
}

int dragon_resist(register inven_type *dri)
    { int colorx;
      do {
        switch (randint(11))
        { case 1: case 2: case 3: dri->flags |= TR_RES_LIGHT;
                  colorx = LIGHTBLUE;
                  dri->cost += 1000;
                  break;
          case 6: case 4: case 5: dri->flags |= TR_RES_FIRE;
                  colorx = RED;
                  dri->cost += 1000;
                  break;
          case 7: case 8: case 9: dri->flags |= TR_RES_COLD;
                  colorx = WHITE;
                  dri->cost += 1000;
                  break;
          default: if (randint(2)==1)
                   {colorx = give_1_hi_resist (dri);  /* JLS */
                    dri->cost += 1500;
                }
                else {
                    dri->flags |= TR_POISON;
...
inven_type *t_ptr;
{
  ...
  if (!stricmp("& Longsword", name)) {
    ...
    case 1:
      ...
      |TR_ARTIFACT|TR_RES_LT|TR_BLESS_BLADE);
      if (randint(2)==1) t_ptr->flags |= TR_AGGRAVATE;
      else t_ptr->flags2 |= TR_TY_CURSE; /*Give one of them, but not both - TY*/
      t_ptr->p1    = 5+randint(8); /* A little variety won't hurt! */
      t_ptr->cost  = 310000L;
      RINGIL = 1;
      return 1;
    case 2:
    ...
    case 4:
      if (ANDURIL) return 0;
      if (wizard || peek) msg_print("Werewindle");
      else good_item_flag = TRUE;
      t_ptr->name2 = SN_ANDURIL;
      t_ptr->tohit = 10;
      t_ptr->todam = 17;
      t_ptr->flags = (TR_SEE_INVIS|TR_SLAY_EVIL|TR_FREE_ACT|TR_REGEN|
          TR_SLOW_DIGEST|TR_SPEED|
	      TR_SUST_STAT|TR_INT|TR_WIS|TR_RES_FIRE|TR_FLAME_TONGUE);
      t_ptr->flags2|= (TR_ACTIVATE|TR_ARTIFACT|TR_RES_LT|TR_RES_DARK
	       |TR_RES_DISENCHANT|TR_SLAY_DEMON|TR_RES_FEAR|TR_ATTACK_SPD);
    ...
    case 8:
      ...
      if (wizard || peek) msg_print("Bleys");
      else good_item_flag = TRUE;
    ...
    default:
      if (ORCRIST) return 0;
      if (wizard || peek) msg_print("Vorpal Blade");
      else good_item_flag = TRUE;
      ...
      t_ptr->weight = 667;
      t_ptr->p1 = 2;
      t_ptr->flags = (TR_STR|TR_SUST_STAT|TR_SPEED);
      t_ptr->flags2 |= (TR_ARTIFACT|TR_ATTACK_SPD|TR_VORPAL|TR_BLESS_BLADE);
      t_ptr->p1    = 1;
      t_ptr->cost  = 95000L;
      ORCRIST = 1;
      return 1;
    ...
    case 2:
      ...
      if (wizard || peek) msg_print("Gerard");
      else good_item_flag = TRUE;
    ...
    case 3:
      ...
      if (wizard || peek) msg_print("Inquisitor");
      else good_item_flag = TRUE;
    ...
    |TR_SLAY_DEMON|TR_RES_CHAOS|TR_RES_FEAR|TR_BLESS_BLADE);
      t_ptr->p1 = 4;
      t_ptr->damage[0] = 6;
      t_ptr->damage[1] = 4;
      t_ptr->cost = 200000L;
    ...
    default:
      ...
      if (wizard || peek) msg_print("Twilight");
      else good_item_flag = TRUE;
      ...
      t_ptr->flags = (TR_SPEED|TR_AGGRAVATE|TR_CURSED
			|TR_FLAME_TONGUE);
      t_ptr->flags2 |= (TR_ARTIFACT|TR_IM_FIRE|TR_RES_DISENCHANT|TR_RES_FEAR
			    |TR_TY_CURSE);
      give_1_hi_resist (t_ptr);  /* JLS */
      t_ptr->p1    = 5+randint(8);
      t_ptr->weight = 400;
      t_ptr->damage[0] = 5;
      t_ptr->damage[1] = 5;
      t_ptr->toac  = -50;
    ...
    case 2:
      ...
      if (wizard || peek) msg_print("Diamond Blade");
      else good_item_flag = TRUE;
    ...
    case 5:
      ...
      if (wizard || peek) msg_print("Grayswandir");
      else good_item_flag = TRUE;
      t_ptr->name2 = SN_GLAMDRING;
      t_ptr->tohit = 10;
      t_ptr->todam = 18;
      t_ptr->flags = (TR_SLAY_EVIL|TR_CON|TR_SLOW_DIGEST|TR_REGEN|TR_SLAY_DRAGON
	|TR_SLAY_UNDEAD|TR_FROST_BRAND|TR_RES_COLD|TR_SEE_INVIS|TR_SUST_STAT);
      t_ptr->flags2 |= (TR_ARTIFACT|TR_SLAY_DEMON|TR_RES_CHAOS|TR_RES_NETHER
            |TR_RES_DARK|TR_HOLD_LIFE|TR_RES_FEAR|TR_BLESS_BLADE);
      give_1_hi_resist (t_ptr);  /* JLS */
      t_ptr->p1    = 4;
      t_ptr->toac  = 9;
      t_ptr->cost  = 100000L;
    ...
    case 7:
      if (AEGLIN) return 0;
      if (wizard || peek) msg_print("Nothung");
      else good_item_flag = TRUE;
      t_ptr->name2 = SN_AEGLIN;
      t_ptr->tohit = 12;
      t_ptr->todam = 16;
      t_ptr->flags = (TR_SLOW_DIGEST|TR_SEARCH|TR_RES_LIGHT|TR_RES_FIRE
		    |TR_RES_ACID|TR_RES_COLD|TR_RES_LIGHT|TR_POISON
		    |TR_SLAY_X_DRAGON);
      t_ptr->flags2 |= (TR_ARTIFACT|TR_LIGHT|TR_LIGHTNING|TR_RES_FEAR);
      t_ptr->p1    = 4;
      t_ptr->cost  = 95000L;
      AEGLIN = 1;
      return 1;
    default:
      if (ELVAGIL) return 0;
      if (wizard || peek) msg_print("Chainsword");
      else good_item_flag = TRUE;
      ...
      t_ptr->damage[0] = 6;
      t_ptr->damage[1] = 13;
      t_ptr->flags |= (TR_AGGRAVATE|TR_CHR|TR_STR|TR_SLAY_ANIMAL|TR_SPEED);
      t_ptr->flags2|= (TR_ARTIFACT|TR_RES_FEAR|TR_ATTACK_SPD|TR_VORPAL);
      t_ptr->p1    = 1;
      t_ptr->cost  = 50000L;
      ELVAGIL = 1;
      return 1;
  ...
  else if (!stricmp("& Falchion", name)) {
    if (randint(4)>1) return 0;
    if (MAEDHROS) return 0;
    if (wizard || peek) msg_print("Hildeleoma");
    else good_item_flag = TRUE;
    ...
    t_ptr->damage[0] = 2;
    t_ptr->damage[1] = 7;
    t_ptr->flags = (TR_DEX|TR_INT|TR_FREE_ACT|TR_SEE_INVIS|TR_FLAME_TONGUE
		    |TR_RES_FIRE);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_SLAY_GIANT|TR_SLAY_TROLL|TR_SLAY_ORC
            |TR_BLESS_BLADE);
    t_ptr->p1 = 3;
    t_ptr->cost = 30000L;
    MAEDHROS = 1;
    ...
    t_ptr->flags = (TR_FLAME_TONGUE|TR_SLAY_UNDEAD|TR_RES_FIRE|TR_SEE_INVIS|
				TR_FFALL|TR_CHR);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_SOUND|TR_SLAY_GIANT|TR_BLESS_BLADE);
    t_ptr->p1 = 3;
    t_ptr->cost = 22500L;
    OSONDIR = 1;
    return 1;
    ...
    if (randint(2)>1) return 0;
    if (TARATOL) return 0;
    if (wizard || peek) msg_print("Osric");
    else good_item_flag = TRUE;
    ...
    t_ptr->weight = 200;
    t_ptr->damage[1] = 7;
    t_ptr->flags = (TR_SLAY_X_DRAGON|TR_RES_LIGHT|TR_POISON);
    ...
    if (wizard || peek) msg_print("Benedict");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_EORLINGAS;
    t_ptr->tohit = 7;
    t_ptr->todam = 25;
    t_ptr->weight = 360;
    t_ptr->flags |= (TR_SEE_INVIS|TR_SLAY_EVIL|TR_DEX|TR_REGEN|TR_STEALTH
		    |TR_SLAY_ANIMAL|TR_SLAY_DRAGON);
    t_ptr->flags2 |= (TR_SLAY_TROLL|TR_SLAY_ORC|TR_ARTIFACT|TR_SLAY_GIANT
                    |TR_BLESS_BLADE);
    t_ptr->p1 = 3;
    t_ptr->damage[1] = 12;
    t_ptr->cost  = 79000L;
    ...
    if (wizard || peek) msg_print("Executioner");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_BARUKKHELED;
    t_ptr->tohit = 13;
    t_ptr->todam = 19;
    t_ptr->weight = 450;
    t_ptr->flags |= (TR_SEE_INVIS|TR_SLAY_EVIL|TR_CON|TR_SLAY_ANIMAL
		    |TR_SLAY_UNDEAD|TR_SLAY_DRAGON);
    ...
    if (wizard || peek) msg_print("Soulsword");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_HARADEKKET;
    t_ptr->tohit = 12;
    t_ptr->todam = 15;
    t_ptr->flags |= (TR_SEE_INVIS|TR_SLAY_EVIL|TR_INT|TR_WIS|TR_SLAY_UNDEAD
		    |TR_SLAY_DRAGON);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_SLAY_DEMON|TR_RES_CHAOS|TR_RES_DISENCHANT
            |TR_HOLD_LIFE|TR_RES_NEXUS|TR_RES_NETHER|TR_BLESS_BLADE);
    t_ptr->p1 = 2;
    t_ptr->cost  = 106000L;
    HARADEKKET = 1;
    return 1;
    ...
    if (wizard || peek) msg_print("Axe of the Dwarves");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_MUNDWINE;
    t_ptr->tohit = 12;
    t_ptr->todam = 17;
    t_ptr->flags |= (TR_SLAY_EVIL|TR_RES_FIRE|TR_RES_COLD|TR_SEARCH
	     |TR_RES_LIGHT|TR_RES_ACID|TR_TUNNEL|TR_INFRA);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_BLIND|TR_SLAY_GIANT);
    t_ptr->cost  = 64000L;
    t_ptr->p1 = 10;
    MUNDWINE= 1;
    return 1;
    ...
    if (wizard || peek) msg_print("Delta");
    else good_item_flag = TRUE;
    ...
    t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_LT|TR_SLAY_TROLL);
    t_ptr->p1 = 3;
    ...
    if (wizard || peek) msg_print("Finndo");
    else good_item_flag = TRUE;
    ...
    if (wizard || peek) msg_print("Merlin");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_GILETTAR;
    t_ptr->tohit = 5;
    t_ptr->todam = 9;
    t_ptr->p1  = 2;
    t_ptr->flags = (TR_INT|TR_FREE_ACT|TR_FROST_BRAND
	    |TR_REGEN|TR_SLOW_DIGEST|TR_SLAY_ANIMAL|TR_SUST_STAT);
    give_1_hi_resist (t_ptr);  /* JLS */
    t_ptr->flags2 = (TR_ARTIFACT|TR_RES_LT|TR_RES_DARK|TR_RES_DISENCHANT);
    t_ptr->cost  = 67000L;
    GILETTAR = 1;
    return 1;
  }
    ...
    t_ptr->flags2 |= (TR_ARTIFACT|TR_VORPAL); /* Why not? -TY */
    t_ptr->p1    = 5;
    t_ptr->cost  = 44444L;
    AGLARANG = 1;
    return 1;
    ...
    case 1:
      if (AEGLOS) return 0;
      if (wizard || peek) msg_print("Runespear");
      else good_item_flag = TRUE;
      ...
      t_ptr->damage[0] = 1;
      t_ptr->damage[1] = 20;
      t_ptr->flags = (TR_WIS|TR_FLAME_TONGUE|TR_RES_LIGHT|
	      TR_RES_FIRE|TR_FREE_ACT|TR_SLOW_DIGEST);
      t_ptr->flags2 |= (TR_SLAY_TROLL|TR_SLAY_ORC|TR_RES_LT
		|TR_SLAY_GIANT|TR_LIGHTNING|TR_ACTIVATE|TR_ARTIFACT);
      t_ptr->toac  = 5;
      t_ptr->p1    = 4;
      t_ptr->cost  = 180000L;
      AEGLOS = 1;
    ...
    case 5:
      if (NIMLOTH) return 0;
      if (wizard || peek) msg_print("Hagen");
      else good_item_flag = TRUE;
    ...
    case 6:
      if (OROME) return 0;
      if (wizard || peek) msg_print("Destiny");
      else good_item_flag = TRUE;
      t_ptr->name2 = SN_OROME;
      t_ptr->tohit = 15;
      t_ptr->todam = 15;
      t_ptr->flags = (TR_FLAME_TONGUE|TR_SEE_INVIS|TR_SEARCH|TR_INT|TR_WIS|
	      TR_RES_FIRE|TR_FFALL|TR_INFRA|TR_SLAY_EVIL|TR_SLAY_DRAGON);
      give_1_hi_resist (t_ptr);  /* JLS */
      t_ptr->flags2 |= (TR_ACTIVATE|TR_LIGHT|TR_SLAY_GIANT|TR_RES_LT
    |TR_ARTIFACT|TR_SLAY_DEMON|TR_HOLD_LIFE|TR_RES_FEAR|TR_BLESS_BLADE);
      t_ptr->p1 = 4;
      t_ptr->cost = 77700L;
      OROME = 1;
      return 1;
    ...
    case 1:
      if (ANGRIST) return 0;
      if (wizard || peek) msg_print("Caine");
      else good_item_flag = TRUE;
      ...
      t_ptr->damage[0] = 2;
      t_ptr->damage[1] = 5;
      t_ptr->flags = (TR_DEX|TR_SLAY_EVIL|TR_SUST_STAT|TR_SEE_INVIS|
    ...
    case 11:
      ...
      if (wizard || peek) msg_print("Fiona");
      else good_item_flag = TRUE;
      ...
      t_ptr->flags = (TR_FROST_BRAND|TR_RES_COLD|TR_REGEN|TR_SLOW_DIGEST|
              TR_DEX|TR_SEE_INVIS|TR_SPEED);
      give_1_hi_resist (t_ptr);  /* JLS */
      t_ptr->flags2 |= (TR_ACTIVATE|TR_ARTIFACT|TR_ATTACK_SPD);
      t_ptr->p1    = 2;
      t_ptr->cost  = 49000L;
      BELANGIL = 1;
      return 1;
    ...
    t_ptr->flags2 |= (TR_IM_COLD|TR_SLAY_ORC|TR_ACTIVATE|TR_ARTIFACT|TR_BLESS_BLADE);
	t_ptr->p1    = 2;
	t_ptr->cost  = 200000L;
	EONWE = 1;
	return 1;
    ...
    if (AULE) return 0;
    if (wizard || peek) msg_print("Mjollnir");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_AULE;
    t_ptr->damage[0] = 5;
    t_ptr->damage[1] = 6;
    ...
    if (randint(2)>1) return 0;
    if (THEODEN) return 0;
    if (wizard || peek) msg_print("Deirdre(B)");
    else good_item_flag = TRUE;
    t_ptr->name2 = SN_THEODEN;
    t_ptr->tohit = 8;
    t_ptr->todam = 10;
    t_ptr->flags = (TR_WIS|TR_CON|TR_SEARCH|TR_SLOW_DIGEST|TR_SLAY_DRAGON);
    t_ptr->flags2 |= (TR_TELEPATHY|TR_ACTIVATE|TR_ARTIFACT|TR_BLESS_BLADE);
    give_1_hi_resist (t_ptr);  /* JLS */
    t_ptr->p1 = 3;
    t_ptr->cost = 42000L;
    THEODEN = 1;
...
inven_type *t_ptr;
{
  ...
  if (!strncmp("Adamantite", name, 10)) {
    ...
    t_ptr->flags |= (TR_RES_FIRE|TR_RES_COLD|TR_RES_ACID|TR_POISON|TR_RES_LIGHT
	     |TR_DEX|TR_SEARCH|TR_REGEN|TR_FFALL);
    t_ptr->flags2 |= (TR_HOLD_LIFE|TR_RES_CONF|TR_RES_SOUND|TR_RES_LT
	     ...
	     |TR_ARTIFACT|TR_ACTIVATE|TR_RES_FEAR);
    t_ptr->toac += 17;
    t_ptr->p1 = -3;
    t_ptr->ac = 50;
    t_ptr->tohit = -4;
    ...
    if (DAL) return 0;
    if (wizard || peek) msg_print("Flora");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_FREE_ACT|TR_SUST_STAT|TR_RES_ACID|TR_CHR);
    t_ptr->flags2 |= (TR_ACTIVATE|TR_ARTIFACT);
    give_1_hi_resist (t_ptr);  /* JLS */
    t_ptr->name2 = SN_DAL;
    t_ptr->p1 = 6;
    ...
    if (ISILDUR) return 0;
    if (wizard || peek) msg_print("Deirdre(A)");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_SEARCH|TR_RES_ACID|TR_RES_FIRE|TR_RES_COLD
	    |TR_RES_LIGHT|TR_REGEN);
    t_ptr->flags2 |= (TR_RES_SOUND|TR_RES_CHAOS|TR_RES_NETHER|TR_ARTIFACT);
    ...
    if (ROHAN) return 0;
    if (wizard || peek) msg_print("Tristan");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_RES_ACID|TR_RES_FIRE|TR_RES_COLD|TR_RES_LIGHT|
	    TR_STR|TR_DEX|TR_POISON);
    t_ptr->flags2 |= (TR_RES_SOUND|TR_RES_CONF|TR_ARTIFACT|TR_RES_DARK);
    ...
    if (wizard || peek) msg_print("Bruennhilde");
    t_ptr->flags |= (TR_RES_ACID|TR_RES_FIRE|TR_RES_COLD|TR_RES_LIGHT|
		     TR_SUST_STAT);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_SOUND);
    ...
    if (randint(3)>1) return 0;
    if (wizard || peek) msg_print("Frakir");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_RES_ACID|TR_DEX|TR_FREE_ACT|TR_SEE_INVIS|TR_SEARCH);
    t_ptr->flags2 |= (TR_ACTIVATE|TR_ARTIFACT);
    give_1_hi_resist (t_ptr);  /* JLS */
    ...
    if (randint(3)==1) {
      ...
      if (wizard || peek) msg_print("Random");
      else good_item_flag = TRUE;
      t_ptr->flags |= (TR_STR|TR_CON|TR_FREE_ACT|TR_SPEED);
      t_ptr->flags2 |= (TR_ARTIFACT);
      give_1_hi_resist (t_ptr);  /* JLS */
      ...
      if (wizard || peek) msg_print("Llewella");
      else good_item_flag = TRUE;
      t_ptr->flags |= (TR_SUST_STAT|TR_RES_ACID);
    ...
    case 2:
      ...
      if (wizard || peek) msg_print("Corwin");
      else good_item_flag = TRUE;
      t_ptr->flags |= (TR_RES_COLD|TR_RES_ACID|TR_CON|TR_SUST_STAT);
      t_ptr->flags2 |= (TR_ACTIVATE|TR_ARTIFACT);
    ...
    default:
      if (CAMLOST) return 0;
      if (wizard || peek) msg_print("Thanos");
      else good_item_flag = TRUE;
      t_ptr->flags |= (TR_STR|TR_CON|TR_AGGRAVATE|TR_CURSED|TR_TELEPORT);
      t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_NETHER|TR_RES_CHAOS
		|TR_HOLD_LIFE|TR_RES_DISENCHANT|TR_RES_NEXUS
		|TR_IM_POISON|TR_TY_CURSE);
    ...
    if (wizard || peek) msg_print("Lohengrin");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_RES_ACID|TR_RES_FIRE|TR_RES_COLD|TR_RES_LIGHT|
	     TR_STEALTH|TR_WIS|TR_SEE_INVIS);
    t_ptr->flags2 |= (TR_ACTIVATE|TR_ARTIFACT|TR_RES_NETHER|TR_HOLD_LIFE
		|TR_RES_DARK|TR_RES_FEAR);
    ...
    if (wizard || peek) msg_print("Martin");
    else good_item_flag = TRUE;
    ...
    if (wizard || peek) msg_print("Tannhaeuser");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_RES_ACID|TR_RES_FIRE|TR_RES_COLD|TR_RES_LIGHT|
		     TR_STEALTH);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_LT|TR_RES_SOUND);
    give_1_hi_resist (t_ptr);  /* JLS */
    ...
    if (THENGEL) return 0;
    if (wizard || peek) msg_print("Coral");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_RES_ACID|TR_WIS|TR_INT|TR_CHR|TR_SEE_INVIS|TR_INFRA);
    t_ptr->flags2 |= (TR_ARTIFACT|TR_RES_CHAOS);
    ...
    t_ptr->flags |= (TR_CON|TR_STR|TR_RES_ACID|TR_SPEED);
    t_ptr->flags2 |= (TR_ARTIFACT);
    ...
    if (randint(6)==1) {
      ...
      if (wizard || peek) msg_print("Gurnemanz");
      else good_item_flag = TRUE;
      t_ptr->flags |= (TR_INT|TR_WIS|TR_SEE_INVIS|TR_SEARCH|TR_RES_ACID
			|TR_SUST_STAT);
      t_ptr->flags2 |= (TR_ACTIVATE|TR_RES_BLIND|TR_ARTIFACT|TR_RES_CONF);
      ...
      if (GORLIM) return 0;
      if (wizard || peek) msg_print("Terror Mask");
      else good_item_flag = TRUE;
      t_ptr->flags |= (TR_INT|TR_WIS|TR_SEE_INVIS|TR_CURSED|TR_RES_ACID|
			TR_TELEPORT|TR_FREE_ACT);
      t_ptr->flags2 |= (TR_ARTIFACT|TR_IM_COLD
			    |TR_IM_POISON|TR_RES_DISENCHANT|TR_RES_FEAR);
      t_ptr->name2 = SN_GORLIM;
      t_ptr->ident |= ID_SHOW_P1;
      t_ptr->ident |= ID_SHOW_HITDAM;
      t_ptr->todam += 25+randint(25); // Whoops! Should NEVER be better
      t_ptr->tohit += 25+randint(25); // than 50, though! - TY
      t_ptr->cost += (t_ptr->tohit+t_ptr->todam)*1;
      give_1_hi_resist (t_ptr);  /* JLS */
      t_ptr->p1 = -125;
      t_ptr->toac += 25+randint(25);
      t_ptr->cost = 50000L;
      GORLIM = 1;
      return 1;
    }
    ...
    if (randint(3)>1) return 0;
    if (GONDOR) return 0;
    if (wizard || peek) msg_print("Amber");
    else good_item_flag = TRUE;
    ...
    if (wizard || peek) msg_print("Macbeth");
    else good_item_flag = TRUE;
    t_ptr->flags |= (TR_DEX|TR_CON|TR_INT|TR_WIS|TR_TELEPORT|
		TR_RES_ACID|TR_SEE_INVIS|TR_FREE_ACT|TR_CURSED);
    t_ptr->flags2 |= (TR_TELEPATHY|TR_ARTIFACT|TR_TY_CURSE);
...
int x, level, good, not_unique;
{
  ...
  switch (t_ptr->tval)
    ...
    case TV_SHIELD: case TV_HARD_ARMOR: case TV_SOFT_ARMOR:
      ...
      || (t_ptr->index>=422 && t_ptr->index<=423)
      || (t_ptr->index>=429 && t_ptr->index<=433)) {
	...
	if (!artifact) { /* assume cost&mesg done if it was an artifact */
      ...
      else if ((t_ptr->tval == TV_SHIELD) && (t_ptr->subval == 18))
        {
            /* Dragon Shields - TY */
            if (wizard || peek) msg_print ("Dragon Shield");
            t_ptr->color = dragon_resist(t_ptr);
	...
	give_1_hi_resist (t_ptr);  /* JLS */
	    ...
	    t_ptr->name2 = SN_MAGI;
	t_ptr->cost = 12000L + (t_ptr->toac * 100);
	    }
	  ...
	  if (peek) msg_print("Resist");
		  rating += 20;
	  t_ptr->name2 = SN_R;
	  if (randint(10)==1) {t_ptr->flags |= TR_POISON;
				if (peek) msg_print("(Poison)");}
	 give_1_hi_resist (t_ptr);  /* JLS */
		  t_ptr->toac += 8;
	  t_ptr->cost += 15500L;
		}
...
default:
        ...
        if ((randint(3)==1 || good==666) && !not_unique &&
		    unique_armour(t_ptr)) break;
        if (wizard||peek) msg_print("Protection");
			      t_ptr->name2 = SN_PROTECTION;
                  t_ptr->toac += randint(5);
			      t_ptr->cost += 250L;
                  do
                  { t_ptr->cost += 50 * (t_ptr->ac);
                    t_ptr->ac += 1; /* Increase the BASE ac - TY */
                    apu = (t_ptr->ac<10?1:2);
                    }
                 while ((randint(6)<2+apu)
                        && t_ptr->ac<75); /* A sanity check - TY */
                 rating += 9;
#endif
break;
	      }
	...
	if (wizard||peek) msg_print("Blade of Sharpness");
	rating += 20;
	t_ptr->name2 = SN_SHARPNESS;
      ...
      if (magik(chance)||good)
	{
	  ...
	  if (magik(3*special/2)||good==666) { /* was 2 */
       ...
       switch(randint(31)) /* was 16 */
       {
    case 28: /* Whoops! There was no case 28 - had to make something
		up (of course I _could_ have changed the switch to randint
		(27)... :-) TY */
	    if (((randint(2) == 1) || (good == 666))
			    && !not_unique && unique_weapon(t_ptr))
			    break;
	    if (wizard || peek)
		msg_print("Weapon of Slaying");
	    rating += 18;
	    t_ptr->tohit += randint(7);
	t_ptr->todam += randint(7);
	if (randint(6)==1) {t_ptr->cost += 2500;
			    t_ptr->damage[0] *= 2;
			    t_ptr->cost *= 2;
			    if (wizard||peek) msg_print("(Double damage)");
			}
	 else
	  {   do
	{ty_tmp = randint(3);
	 if (ty_tmp != 2) {t_ptr->damage[0]++;
			   t_ptr->cost += 2500;}
	 if (ty_tmp != 1) {t_ptr->damage[1]++;
			   t_ptr->cost += 2500;}
	} while (randint(6)==1);}
	    t_ptr->name2 = SN_SLAYING;
			t_ptr->cost *= 2;
			break;
    case 29: /* Lightning brand */
		  if (((randint(2) == 1) || (good == 666)) && !not_unique &&
		      unique_weapon(t_ptr)) break;
	  if (peek||wizard) msg_print("Lightning");
	  rating += 25;
    t_ptr->flags = TR_RES_LIGHT;
    t_ptr->flags2 |= TR_LIGHTNING;
		  t_ptr->tohit+=2;
		  t_ptr->todam+=2;
	  t_ptr->name2 = SN_LIGHTNING;
	  t_ptr->cost += 5000L;
		  break;
	case 26: /* of Westernesse */
		  if (((randint(2) == 1) || (good == 666)) && !not_unique &&
		      unique_weapon(t_ptr)) break;
	  if (peek) msg_print("Pattern weapon");
		  rating += 30;
		  t_ptr->flags |= (TR_SEE_INVIS|TR_SLAY_EVIL|
				   TR_SLAY_UNDEAD|TR_DEX|TR_CON|TR_STR|
		   TR_FREE_ACT);
	  t_ptr->flags2 |= (TR_SLAY_ORC|TR_SLAY_DEMON);
	  if (randint(3)==1) {
		  give_1_hi_resist (t_ptr);  /* JLS */}
	  if (randint(10)==1) {
		  t_ptr->flags2 |= TR_HOLD_LIFE;}
	  if (randint(7)==1) t_ptr->flags2 |= TR_RES_FEAR;
	  t_ptr->tohit += randint(7)+3;
	  t_ptr->todam += randint(9)+3;
		  t_ptr->p1 = 1;
	  t_ptr->cost += 22500L;
		  t_ptr->cost *= 2;
		  t_ptr->name2 = SN_WEST;
	  ...
	  if (((randint(2) == 1) || (good == 666)) && !not_unique &&
		  ...
		  t_ptr->flags |= (TR_SEE_INVIS|TR_SUST_STAT|TR_SLAY_UNDEAD|
				   TR_SLAY_EVIL|TR_STR);
          t_ptr->flags2 |= (TR_RES_FEAR|TR_BLESS_BLADE);
          if (randint(2)==1) {
		...
		case 2: /* Defender      */
		  ...
		  t_ptr->flags |= (TR_FFALL|TR_RES_LIGHT|TR_SEE_INVIS
				   |TR_FREE_ACT|TR_RES_COLD|TR_RES_ACID
		   |TR_RES_FIRE|TR_REGEN|TR_STEALTH|TR_SUST_STAT);
	  if (randint(2)==1) {
		  t_ptr->flags2 |= TR_HOLD_LIFE;}
	  give_1_hi_resist (t_ptr);  /* JLS */
	  if (randint(10)==1) {t_ptr->flags |= TR_POISON;
		  ...
		  t_ptr->tohit += 3;
		  t_ptr->todam += 3;
	  t_ptr->toac  += 5 + randint(10);
		  t_ptr->name2 = SN_DF;
		  /* the value in p1 is used for stealth */
	  t_ptr->p1    = randint(5); /*Now sust too*/
	  ...
	  if (randint(7) == 1) {t_ptr->flags |=  TR_SLAY_X_DRAGON;
		    if (peek||wizard) msg_print("Execute Dragon");
		    t_ptr->name2 = SN_EXECUTE_DR;}
		else {t_ptr->flags |= TR_SLAY_DRAGON;
      if (peek) msg_print("Slay Dragon");
      t_ptr->name2 = SN_SD;}
	  if (randint(10) == 1) t_ptr->flags |= TR_RES_LIGHT;
	  if (randint(10) == 1) t_ptr->flags |= TR_RES_COLD;
	  if (randint(10) == 1) t_ptr->flags |= TR_RES_ACID;
	  if (randint(10) == 1) t_ptr->flags |= TR_RES_FIRE;
	  if (randint(20) == 1) t_ptr->flags |= TR_POISON;
		  rating += 18;
		  t_ptr->tohit += 3;
		  t_ptr->todam += 3;
	  t_ptr->cost += 5500L;
	  ...
	  t_ptr->flags2 |= TR_HOLD_LIFE;
	  if (randint(20) == 1) {t_ptr->flags2 |= TR_RES_NETHER;
				    t_ptr->cost += 500;
				    if (peek) msg_print("(Nether)");
					}
		...
		case 21: case 22: case 23:
		  ...
		  break;
	case 24: case 25:
	  t_ptr->flags |= (TR_FREE_ACT|TR_SLAY_EVIL|TR_SEARCH|
		TR_REGEN|TR_SLOW_DIGEST|TR_TELEPORT);
	  t_ptr->flags2 |= (TR_RES_NEXUS);
	 give_1_hi_resist (t_ptr);  /* JLS */
	 if (randint(5)==1) {t_ptr->flags2 |= TR_SLAY_DEMON;
				if (peek) msg_print("(SD)");}
	  if (peek) msg_print("Trump Weapon");
	  rating += 22;
	  t_ptr->p1    = randint(5);
	  t_ptr->tohit+=3;
	  t_ptr->todam+=3;
	  ...
	  t_ptr->cost += 7500L;
		  break;
    default:
     /* Blessed Blade -DGK (case 30 and case 31)*/
	  ...
	  if (randint(6)==1)    t_ptr->flags2 |= TR_TY_CURSE;  /*TY *grin* */
	      t_ptr->tohit -= 15;
	      t_ptr->todam -= 15;
	      t_ptr->toac  = -10;
	      t_ptr->weight += 100;
    ...
    case TV_BOW:
	  ...
	  if (magik(chance)||good) {
	    ...
	    case 1: case 2: case 3:
	      ...
	      if (wizard || peek) msg_print("Parsifal");
		      else good_item_flag = TRUE;
		...
		{
		  if (CUBRAGOL) break;
	  if (wizard || peek) msg_print("Brand");
		  else good_item_flag = TRUE;
	...
	do {
	t_ptr->tohit += 1 + randint(5);
	t_ptr->todam += 1 + randint(5);
	} while (randint(5)==1); /* 20% chance of higher plusses - TY */
		t_ptr->name2 = SN_SLAYING;
	...
	t_ptr->tohit += 1+randint(randint(7)==1?10:4);
	t_ptr->todam += 1+randint(randint(7)==1?10:4);
		t_ptr->flags |= TR_STR;
		t_ptr->ident |= ID_SHOW_HITDAM;
		t_ptr->ident |= ID_SHOW_P1;
	  ...
	  if (magik(chance)||good)
	      ...
	      if (magik(special)||(good==666))
		{
		  ...
		  if (tmp == 1)
            ...
            t_ptr->p1 = 2;
            do { t_ptr->p1++; }
                while (magik((86*chance)/100));
            t_ptr->cost += t_ptr->p1*30000L;
		      }
		    }
	...
	case TV_HELM:  /* Helms */
     ...
     if (t_ptr->subval == 9) {
            /* Dragon Helmets - TY */
            if (wizard || peek) msg_print ("Dragon Helmet");
            t_ptr->color = dragon_resist(t_ptr);
            t_ptr->toac += m_bonus(1, 30, level);
            rating += 8;
	...
	give_1_hi_resist (t_ptr);  /* JLS */
	    t_ptr->toac += m_bonus(1, 20, level);
	t_ptr->toac += 7 + randint(12);
	t_ptr->cost += t_ptr->toac*150;
	 break;
	  case 30:     /* Slaying             */
	    t_ptr->ident |= ID_SHOW_HITDAM;
	    t_ptr->todam += m_bonus(1, 25, level)*2;
	t_ptr->todam += randint(8);
	    t_ptr->tohit += m_bonus(1, 25, level)*2;
	t_ptr->tohit += randint(8);
	...
	t_ptr->p1 = 0;
	      ...
	      rating += 20;
	t_ptr->p1 = 1;
      while ((randint(11-(chance/10))==1) && t_ptr->p1<4)
	{t_ptr->p1++;
		 rating+=5;
		 t_ptr->cost *= 2;
		}
    ...
    case 12:   t_ptr->p1 = randint(5)  +     3; break;
    ...
    case TV_CLOAK:
	  ...
	  if (magik(special)||(good==666))
	      ...
	      if (wizard || peek) msg_print("Colluin");
		      ...
		      case 4:
			  if (HOLCOLLETH) break;
	      if (wizard || peek) msg_print("Dworkin");
			  else good_item_flag = TRUE;
			  ...
			  break;
			   ...
			   case 1:
			     if (LUTHIEN) break;
		 if (wizard || peek) msg_print("Tarncloak");
			     else good_item_flag = TRUE;
...
static struct opt_desc { char *o_prompt; int8u *o_var; } options[] = {
  ...
  { "Equippy characters",                       &equippy_chars },
  { "Quick messages",                           &quick_msgs },
  { "Turn off graphics",                        &no_graphics_flag},
#ifdef TC_COLOR
...
static void magic_ammo(inven_type *t_ptr, int good, int chance,
  ...
  if (magik(chance)||good) {
	...
	case 10:
	  ...
	  break;
	case 11:
      t_ptr->tohit += 12; /* reduced because of dice bonus -CFT */
      t_ptr->todam += 12;
