 ...
 */
static int check_hit(int power, int level, monster_type *m_ptr)
{
	int i, k, ac;
    ...
    if (k < 0) msg_format ("You nimbly dodge the feeble blow! ");
    if (k < 10) return (k > 5);
    ...
    if (p_ptr->invis)
    {
        i =(((power + (level *3)) * 8)/10);
        i -=(randint(p_ptr->invis) +1);
    }

    /* Stunned Monster has weaker power. */
    if (m_ptr->stunned)
    {
        i =(( (power + (level *3)) * 8)/10);
    }
...
bool make_attack_normal(int m_idx)
{
	...
	for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
	{
		...
		switch (effect)
		{
			...
			case RBE_NASTY_POIS:	power =  0; break;
		}


		/* Monster hits player */
        if (!effect || check_hit(power, rlev, m_ptr))
		{
			/* Always disturbing */
			disturb(1, 0);
			...
			switch (effect)
			{
				...
				case RBE_UN_POWER:
				{
					...
					for (k = 0; k < 10; k++)
					{
						...
						/* Must pass [hard] saving throw -TM- */
						tmp = p_ptr->skill_sav / 2;
						if (randint(100) < tmp) continue;
				...
				case RBE_NASTY_POIS:
				{
					/* Take "black touch" effect */

					if (set_nasty_poisoned(TRUE))
					{
						obvious = TRUE;
					}
