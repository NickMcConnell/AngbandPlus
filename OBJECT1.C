 ...
 * Currently, the "SHIMMER_OBJECTS" code is disabled anyway.
 */
 ...
 * efficient, portable, versions of some common "sprintf()" (or "strfmt()")
 * commands.
 *
...
void object_desc(char *buf, object_type *o_ptr, int pref, int mode)
{
    ...
    /* hack for randart rings */
    if (o_ptr->name3 && o_ptr->tval == TV_RING)
    {
        basenm = "Ring";
    }
	...
	switch (o_ptr->tval)
	{
		...
		case TV_RING:
		{
            ...
            if (!aware && (o_ptr->sval == SV_RING_POWER_RANDART))
			{
                modstr = "Ornate Gold";
			}
	...
	/* Dagger inscribed #of Smell will be named
	 * Dagger of Smell */
	if (o_ptr->note)
	{
		/* Find a '#' */
...
cptr item_activation(object_type *o_ptr)
{
	...
	switch (o_ptr->name1)
	{
		...
		case ART_EONWE:
		{
			return "scattering every 1000 turns";
		}
		case ART_LOTHARANG:
		{
		...
		case ART_CELEBORN:
		{
			return "great storm every 500 turns";
		}
		case ART_LUTHIEN:
		{
	...
	if (o_ptr->tval == TV_RING)
	{
		switch(o_ptr->sval)
		{
			case SV_RING_FLAMES:
			{
				return "resistance to fire (20+d20 turns) every 10+d10 turns";
			}
			case SV_RING_ICE:
			{
				return "resistance to cold (20+d20 turns) every 10+d10 turns";
			}
			case SV_RING_LIGHTNING:
			{
				return "resistance to electricity (20+d20 turns) every 10+d10 turns";
			}
		}
	}
...
bool identify_fully_aux(object_type *o_ptr)
{
	...
	if (f3 & (TR3_TM_CURSE))
	{
		info[i++] = "It bears an ancient and foul curse.";
	}
	...
	else
	{
		switch (o_ptr->tval)
		{
			...
			case TV_RING:
			{
				...
				/* Hack -- just replace first ring if none free !!! */
				return INVEN_D_RING1;
...
cptr mention_use(int i)
{
	...
	else
	{
		switch (i)
		{
			case INVEN_D_WIELD: p = "Biting with"; break;
			case INVEN_D_BOW:   p = "Clawing with"; break;
			case INVEN_D_OUTER: p = "About body"; break;
			case INVEN_D_HEAD:  p = "On head"; break;
			case INVEN_D_NECK:  p = "Around neck"; break;
			case INVEN_LITE:    p = "Light source"; break;
			case INVEN_D_RING1: p = "On left claw"; break;
			case INVEN_D_RING2: p = "On left claw"; break;
			case INVEN_D_RING3: p = "On left claw"; break;
			case INVEN_D_RING4: p = "On right claw"; break;
			case INVEN_D_RING5: p = "On right claw"; break;
			case INVEN_D_RING6: p = "On right claw"; break;
	...
	/* Hack -- Heavy weapon */
	if (i == INVEN_WIELD && p_ptr->prace < RACE_MIN_DRAGON)
	{
		object_type *o_ptr;
		o_ptr = &inventory[i];
	...
	/* Hack -- Heavy bow */
	if (i == INVEN_BOW && p_ptr->prace < RACE_MIN_DRAGON)
	{
		object_type *o_ptr;
		o_ptr = &inventory[i];
		if (adj_str_hold[p_ptr->stat_ind[A_STR]] < o_ptr->weight / 10)
...
cptr describe_use(int i)
{
	...
	else
	{
		switch (i)
		{
			case INVEN_D_OUTER: p = "wearing on your back"; break;
			case INVEN_D_HEAD:  p = "wearing on your head"; break;
			case INVEN_D_NECK:  p = "wearing around your neck"; break;
			case INVEN_D_LITE:  p = "using to light the way"; break;
			case INVEN_D_RING1: p = "wearing on your left claw"; break;
			case INVEN_D_RING2: p = "wearing on your left claw"; break;
			case INVEN_D_RING3: p = "wearing on your left claw"; break;
			case INVEN_D_RING4: p = "wearing on your right claw"; break;
			case INVEN_D_RING5: p = "wearing on your right claw"; break;
			case INVEN_D_RING6: p = "wearing on your right claw"; break;
			default:            p = "carrying in your pack"; break;
		}
	}
...
void show_inven(int start)
{
    ...
    int out_index[25-1];
    byte out_color[25-1];
    char out_desc[25-1][80];
#else
    int out_index[screen_y-1];
	byte out_color[screen_y-1];
	char out_desc[screen_y-1][80];
#endif
	...
	/* Display the inventory */
	for (k=0, x=0, i = start; (i < z) && (x < screen_y-3); i++, x++)
	{
		o_ptr = &inventory[i];
...
bool get_item(int *cp, cptr pmt, cptr str, int mode)
{
	...
	while (!done)
	{
		...
		if (!p_ptr->command_wrk)
		{
			...
			/* Redraw if needed */
			if (p_ptr->command_see) show_inven(inven_start);
		}
		...
		/* Message about how to scroll inventory */
		strcat(out_val, " + for more,");
		...
		switch (which)
		{
			...
			case '+':
			{
				...
				if (inven_start > INVEN_PACK) inven_start = 0;
