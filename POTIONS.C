      ...
      else while (i != 0)
	{
	  ...
	  switch(j)
	    ...
	    case 12:
	      if (res_stat (A_CHR))
		{
          msg_print("You feel your good looks returning.");
		  ident = TRUE;
		}
	      break;
	    case 13:
	    ...
	    case 21:
          ...
          if ((!py.flags.confusion_resist) && (!py.flags.chaos_resist)) {
               /* The check for chaos resistance was missing here... TY */
            if (f_ptr->confused == 0)
		    {
             ...
             if (randint(10)==1) {  /*Extra nastiness by TY*/
             lose_all_info();
             teleport(100);
             lose_all_info(); /* Have to call it twice :-<
                                 The first time because otherwise you can
                                 briefly see the map where you teleport to,
                                 and the second time or you will leave a lit
                                 spot behind.  */
             msg_print("You wake up somewhere with a sore head.");
             msg_print("You can't remember what happened, or how you got here!");
             }
             else if (randint(10)==1) f_ptr->image += randint(100); /*TY*/
                }
              break;
	    case 22:
	      f_ptr = &py.flags;
	    ...
	    case 34:
		  ...
		  lose_exp(m);
          if (!player_saves()) lose_all_info(); /*TY*/
	      }
	      else msg_print
