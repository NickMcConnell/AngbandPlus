...
#ifdef MSDOS /* fix this gluttonous stack usage to avoid stack overflow -CFT */
{
  ...
  while (not_eof && (rank <= to)){
    ...
    erase_line (1, 0);
    if ((from>1) || (rank > 1)) {
      sprintf(tmp_str, "               Zangband Hall of Fame (from position %d)",
	      (rank > 1 ? rank : from));
#ifdef TC_COLOR
...
static void print_tomb()
{
 ...
 retry:
  flush();
  if (!total_winner && !wizard && !to_be_wizard)
    { sprintf(t_2,"Goodbye, %s!",py.misc.name);
      msg_print(t_2);}
