  ...
  int y, x;
{
 ...
 again:
  ...
  case (SPECIAL_OBJ+7):
    ...
    if ((object_list[tmp].level>dun_level) && (randint(60)>1)) goto again;
    if ((wizard||peek))
      sprintf(str,"Jewel of Judgement");
    else good_item_flag=TRUE;
    THRAIN=TRUE;
    break;
  case (SPECIAL_OBJ+8):
...
int32u good;
{
  ...
  do {
	...
	(object_list[sorted_objects[tmp]].subval > ((good & SPECIAL)?71:67)))
      is_good = TRUE;
    if ((tv == TV_SCROLL2) &&
	(object_list[sorted_objects[tmp]].subval==101))
      {is_good = TRUE;}
...
void
 ...
 /* This function has become ugly since "normal" speed is now -9. */

 /* The mapping goes something like this:
   *  -9 = normal(0)
   * -10 = normal(1) ...
   * -18 = normal(9)
   * -19 = somewhat fast(0) ...
   ...
   *   1 = somewhat slow(9). */
     ...
     if (py.flags.speed > -9) /* The slow speeds. */
       {
       /* We must add 18, because we want the range -8 to 1 to map to the
	* range 10 through 19, etc. */
       i =  (py.flags.speed + 18) / 10;
       j =  (py.flags.speed + 18) % 10;
       }
     else
       {
       i = (py.flags.speed + 9 )/ 10 ;
       j = 0 - (py.flags.speed + 9) % 10;
       }
     ...
     if (i > 3)
       sprintf(buffer, "Painfully Slow   ");
     else if (i == 3)
       sprintf(buffer, "Very Slow(%d)    ", j);
     else if (i == 2)
       sprintf(buffer, "Slow(%d)         ", j);
      else if (i == 1) 
       sprintf(buffer, "Somewhat Slow(%d)", j);
     else if (i == 0)
       sprintf(buffer, "Normal Speed(%d) ", j);
     else if (i == -1)
       sprintf(buffer, "Somewhat Fast(%d)", j);
     else if (i == -2)
       sprintf(buffer, "Fast(%d)         ", j);
     else if (i == -3)
       sprintf(buffer, "Very Fast(%d)    ", j);
     else if (i == -4)
       sprintf(buffer, "Deadly Speed(%d) ", j);
     else
       sprintf(buffer, "Light Speed      ");

     put_buffer(buffer, 23, 49);
  ...
  int c;
{
  ...
  if (randint(7500)<c || randint(12)==1)
    { switch(randint(6))
     {
     case 1:
         if(!py.flags.sustain_str)
             { msg_print("A nasty flesh wound has damaged your strength!");
                dec_stat(A_STR);}
         else msg_print("You feel as strong as before.");
     break;
     case 2:
         if(!py.flags.sustain_con)
                   { msg_print("An ugly cut into your body reduces your health!");
                       dec_stat(A_CON);}
         else msg_print("You feel as healthy as before.");
     break;
     case 3:
     if(!py.flags.sustain_dex)
           { msg_print("Some nerves were cut! Your muscles don't obey you!");
            dec_stat(A_DEX);
            }
         else msg_print("Your nervous system is unaffected.");
     break;
      default:
        if(!py.flags.sustain_chr)
           { msg_print("You have been terribly scarred!");
            dec_stat(A_CHR);}
         else msg_print("Somehow, you avoid being scarred!");
        }
    }
}
  ...
  int s;
{
  ...
  if (!py.flags.sound_resist) {
    ...
    py.flags.stun+=s;
    s=py.flags.stun;
    if (randint(200)<s || randint(12)==1)
    { switch (randint(2))
    {
    case 2:
     if(!py.flags.sustain_wis)
           { msg_print("A nasty blow to your head makes you feel dull!");
            dec_stat(A_WIS);
            }
         else msg_print("Your mind is protected.");
     break;
    default:
     if(!py.flags.sustain_int)
           { msg_print("A vicious blow to your head has caused brain damage!");
            dec_stat(A_INT);
            }
         else msg_print("Your wits are protected.");
        }
    }
    if (s>100) {
 ...
 for (i=INVEN_WIELD; i < INVEN_ARRAY_SIZE; i++)
  {
   ...
   j=i-INVEN_WIELD;
#ifdef TC_COLOR
   if (!no_color_flag) colr(i_ptr);
#endif
   if (i_ptr->tval != TV_NOTHING)
     {
      out_val[0]=(int)(no_graphics_flag?(i_ptr->tchar):(i_ptr->gchar));
...
inven_type *i_ptr;
{
 if (i_ptr->color != 0)
   {
    if (!no_color_flag)
     textcolor(i_ptr->color);
   }
  else switch(i_ptr->tval)
	{
	 case TV_POTION1:
	 case TV_POTION2:
	   if (!no_color_flag) textcolor(tccolors[i_ptr->subval
					- ITEM_SINGLE_STACK_MIN]);
	   break;
	 case TV_WAND:
	 case TV_ROD:
	   if (!no_color_flag) textcolor(tcmetals[i_ptr->subval]);
	   break;
	 case TV_STAFF:
	   if (!no_color_flag) textcolor(tcwoods[i_ptr->subval]);
	   break;
	 case TV_RING:
	   if (!no_color_flag) textcolor(tcrocks[i_ptr->subval]);
	   break;
	 case TV_AMULET:
	   if (!no_color_flag) textcolor(tcamulets[i_ptr->subval]);
	   break;
	 case TV_FOOD:
	   if (!no_color_flag) textcolor(tcmushrooms[i_ptr->subval
					- ITEM_SINGLE_STACK_MIN]);
	   break;
  ...
  switch((x/y))
    {
      ...
      case 9: case 10: case 11: case 12:         return("Superb");
      case 13: case 14: case 15: case 16:        return("Chaos Rank");
      default:                  return("Amber Rank");
      }
}
  ...
  switch((x/y))
    {
      ...
      case 6:
      case 7: case 8:
      case 9: case 10: case 11: case 12:          return(LIGHTGREEN);
      case 13: case 14: case 15: case 16: return(MAGENTA);
...
int weight_limit()
{
  ...
  weight_cap = (long)py.stats.use_stat[A_STR] * (long)PLAYER_WEIGHT_CAP
	+ (long)py.misc.wt;
  weight_cap = weight_cap * 85 / 100; /* TY */
...
register inven_type *i_ptr;
{
  ...
  if (i < new_inven_weight)
    {
    i = 10 * (new_inven_weight-i)  / (i + 1);
        /* My new - more progressive - formula for Ekrem's new speed code
            - TY */
    if (i<1) i=1;}
  else
    i = 0;
...
void check_strength()
{
  ...
  i = weight_limit();
  if (i < inven_weight)
    {i = 10 * (inven_weight-i) / (i+1); /* I'm trying a new formula for the new speed code
				 - TY */
      if (i<1) i=1; }
  else
    i = 0;
...
/* 9 Dec 94: added remaining spell count to display -DRS <sward+@cmu.edu> */
void gain_spells()
{
  char query;
 ...
 /* "Learn which spell?" 18
    "Learn which spell (xx left)?" */
         sprintf(learn_prompt, "Learn which spell (%d left)?", new_spells);
   while (new_spells && get_com(learn_prompt, &query)) {
      j = query - 'a';
	...
	erase_line (j+1, 31);
	print_spells (spells, i, FALSE, -1);
       sprintf(learn_prompt, "Learn which spell (%d left)?",
           new_spells);    /* -DRS- */
      } else
	bell();
    }
  ...
  if (spell_learned != 0 || spell_learned2 != 0)
	...
	new_mana++;
	  ...
	  !((inventory[INVEN_HANDS].flags & TR_FREE_ACT)||
	    ...
	    (inventory[INVEN_HANDS].p1 > 0))) /* gauntlets of dex (or free
						action - DGK) can hardly
...
#if 0
    if (s<0)
	{   msg_print("HEY, there's something funny with attack_blows()!");
	    s=0;}  /* A paranoia check - TY */
#endif /* Removed - so we can have cursed rings of extra attacks - TY */
	d = ((int)blows_table[str_index][dex_index]+s);
      return ((d<1)?1:d);
    }
}
...
int monster;
{
  ...
  if ((((i_ptr->tval >= TV_SLING_AMMO) && (i_ptr->tval <= TV_ARROW)) ||
    ...
    else if ((m_ptr->cchar == 'A') && (i_ptr->flags2 & TR_SLAY_DEMON))
    {
      tdam = tdam * 2;
     /* r_ptr->r_defense |= DEMON;*/
    }
      /* Frost         */
...
char *str;
{
     ...
     /* Confirm duplicate single digit inscriptions. If the player
        has an "accident" (such as firing the wrong item) because of
        confusion over duplicate inscriptions, then it's his fault!
      */
   ...
   for (i = 0; i <= INVEN_AUX; ++i) {
       iptr2 = &inventory[i];
       if (iptr2->tval != TV_NOTHING && iptr2->inscrip[0] == str[0] &&
           iptr2->inscrip[1] == '\0') {
       sprintf(buf, "Duplicate inscription {%c}?", str[0]);
       if (get_check(buf))
           strcpy(i_ptr->inscrip, str);
       return;
       }
   }

   /* Now check for objects in the dungeon. */
   for (i = MIN_TRIX; i < tcptr; ++i) {
       iptr2 = &t_list[i];
       if (iptr2->inscrip[0] == str[0] && iptr2->inscrip[1] == '\0') {
       sprintf(buf, "Duplicate inscription {%c}?", str[0]);
       if (get_check(buf))
           strcpy(i_ptr->inscrip, str);
       return;
   ...
   /* Now check the player's home. */
   for (i = 0; i < store[MAX_STORES-1].store_ctr; ++i) {
       iptr2 = &store[MAX_STORES-1].store_inven[i].sitem;
       if (iptr2->inscrip[0] == str[0] && iptr2->inscrip[1] == '\0') {
       sprintf(buf, "Duplicate inscription {%c}?", str[0]);
       if (get_check(buf))
           strcpy(i_ptr->inscrip, str);
...
{  char c; int f = TRUE;
   if (py.flags.confused > 0)
    ...
    switch(py.misc.prace)
    {
	case 0:        if  (lost_mana()) {
			 msg_print("You try to psychoanalyze yourself.");
			  if (race_test(43-py.flags.stun))
			{ msg_print("You know yourself a little bit better...");
			    self_knowledge();}
		      else msg_print("But you don't have the guts for the job!");
		    }
		   break;
	...
	case 1: case 2:
	    {
	       if (lost_mana()) {
	    msg_print("You attempt to speed up your moves...");
	      if (race_test((py.misc.prace == 2?43:48)-py.flags.stun))
		 {
		    if (py.flags.fast <= 0)
		      {
			py.flags.fast += randint(py.misc.lev) + 15;
		      }
		    else
		    py.flags.fast += randint(5);
		    }
	      else {msg_print("You fail to move faster!");
		    }
				}
	   }
	...
	case 3: 
		{ 
		     if (lost_mana()){
		msg_print("You attempt to create food with your sheer will power!");
		 if (race_test(25-py.flags.stun)) create_food();
		 else msg_print("You can almost taste it, but it vanishes!");}
		}
		 break;
       ...
       case 4:
		 ...
		 if (race_test(24-py.flags.stun)) teleport(py.misc.lev*3);
		 else msg_print("Teleport failed!");
		    }
		}
		 break;
       case 5: {
		 ...
		 if (race_test(15-py.flags.stun))
			      {   detect_sdoor();
				  detect_trap();
				  if (race_test(45-py.flags.stun)) detection();}
		 else msg_print("You do not find anything!");
		    }
		 }
		 break;
       ...
       case 6: {
		     if (lost_mana()) {
			msg_print("You decide to play rough.");
		 if (race_test(17-py.flags.stun))
			 {  remove_fear();
		       if (race_test(30-py.flags.stun))
			   {msg_print("You look around to see how tough your opponents are.");
			    probing();}
			}
		 else msg_print("But it doesn't work!");
		 }
	}
		 break;
	...
	case 7:  {
	 if (lost_mana()) {
	    msg_print("You concentrate hard and try to enter a battle frenzy...");
	 if (race_test((py.misc.pclass==0?10:30)-py.flags.stun))
	    {py.flags.shero = randint(25) + py.misc.lev;
	     break;}
	 else msg_print ("You fail to go berserk!");
	}
	}
	break;

	case 8: { do { /* loop, so RET or other key doesn't accidently exit */
  f = get_com("Start Shadow Shifting (y/n)?", &c);
		} while (f && (c != 'y') && (c != 'Y') && (c != 'n') &&
				(c != 'N'));
	  if (f && (c != 'n') && (c != 'N'))
		{      
		    ...
		    if (race_test(47-py.flags.stun))
				    {msg_print("Your surroundings change.");
				     new_level_flag = TRUE;}
		     else {msg_print("You don't get anywhere!");
		}
		}
	    break;}
	free_turn_flag = TRUE;
	break;
       ...
       case 9: 
		 { 
		     if(lost_mana()) {
			msg_print("You feel sensitive to good and evil...");
		 if (race_test(15-py.flags.stun))
			      {   detect_evil();
				  if (race_test(39-py.flags.stun))
					{msg_print("You can feel your purity protecting you.");
					 protect_evil();}
				  if (race_test(48-py.flags.stun))
					{msg_print("An aura of purity radiates from your body!");
					  dispel_creature(EVIL, (int)(4*py.misc.lev));}
				    }
		   else msg_print("You fail to purify your thoughts!");
		   }
		}
		 break;

      default: msg_print("This race has no special ability.");
      free_turn_flag = TRUE; }
    }


short race_test(int chance)
    {   int std_chance = 75;
	if (py.misc.lev < chance) std_chance -= 5*(chance - py.misc.lev);
	if (std_chance < 5) std_chance = 5;
	if (randint(100)<=std_chance) return TRUE;
	else return FALSE;
    }

int lost_mana()
    {   short divide_factor = 1;
    int ff = TRUE, reduce = 1; char cc;
	switch (py.misc.prace)
	    {case 2: case 3: case 8:
		divide_factor = 5;
	     case 9:
		divide_factor = (py.misc.lev<36?10:(py.misc.lev<45?7:5));
	     default:
		divide_factor = 10;
	    }
      reduce = (py.misc.mana/divide_factor > 0?py.misc.mana/divide_factor
                  :1);
        /* No freebies - TY */
    if (py.misc.cmana >= reduce && py.misc.cmana > 0)
		    /* Changed the check above so people won't just wait for
		       1 point of mana to regenerate! - TY */
       {py.misc.cmana = py.misc.cmana-reduce;
	    if (py.misc.cmana < 0) py.misc.cmana = 0;
	    prt_cmana();}

       else {
	 if ((2*py.misc.chp) < (2*py.misc.mhp)/divide_factor) {
	    /* Why the x 2? Because otherwise people with few hp don't
		get any warning at all (if they are stupid enough to try...)
		- TY */
	  do { /* loop, so RET or other key doesn't accidently exit */
  ff = get_com("Use the intrinsic in your weakened state?", &cc);
	    } while (ff && (cc != 'y') && (cc != 'Y') && (cc != 'n') &&
		    (cc != 'N'));
	  if (ff && (cc != 'y') && (cc != 'Y'))
		    { free_turn_flag = TRUE;
		      return FALSE;}
    ...
    take_hit(randint(py.misc.mhp/divide_factor), "concentrating too hard");
    }
    return TRUE;
    }
...
{   char c; int f = TRUE;
    switch(py.misc.prace)
    {
	case 8: if (py.misc.lev < 35) {
      msg_print("You have not learned the skill of Pattern Mind-walking yet.");
	free_turn_flag = TRUE; break;}
	if (py.misc.chp < py.misc.mhp) {
	  do { /* loop, so RET or other key doesn't accidently exit */
  f = get_com("REALLY perform Pattern Mind-walking in your weakened state?", &c);
		} while (f && (c != 'y') && (c != 'Y') && (c != 'n') &&
				(c != 'N'));
	  if (f && (c != 'n') && (c != 'N'))   pattern_w();
		       break;         }
	  pattern_w();
	  break;

	default: msg_print("You don't even know what the Pattern is!");
		 free_turn_flag = TRUE;
		}
	    }
...
int pattern_w()
    { register struct flags *f_ptr;
      ...
      msg_print("The strain is almost unbearable!  You sweat in agony!");
      take_hit(randint(py.misc.mhp), "Pattern Mind-walking");
	  (void) remove_curse();
	      (void) cure_blindness();
	      (void) cure_confusion();
	      (void) cure_poison();
...
int generic_curse () {   /*This is for new nasty artifacts, by TY*/
int x,y, sm_type, k;
	    disturb(1, 0);
	    switch(randint(19))
{    case 1: case 2:
	  msg_print("Suddenly, you feel nervous!");
	  trap_creation();
	    break;
    case 3:
	  msg_print("Huh?  Who am I?");
	  lose_all_info();
	  break;
    case 4: case 5:
      if (randint(25)==1)
        { msg_print("Suddenly, your surroundings change!");
          new_level_flag = TRUE;
          /* Morale of the story: Don't wear cursed items on special levels -TY*/
        }
      else teleport(100);
	  break;
    case 6: case 7:
	 aggravate_monster(25);
	 msg_print("You feel threatened.");
	 break;
    case 8: case 9:
	  unlight_area(char_row, char_col);
	  break;
    case 10: case 11:
	msg_print("Suddenly, you feel less powerful!");
	switch(randint(10)) {
	  case 1: lose_str(); break;
	  case 2: lose_int(); break;
	  case 3: lose_wis(); break;
	  case 4: lose_dex(); break;
	  case 5: lose_con(); break;
	  case 6: lose_chr(); break;
	  default: lose_str(); lose_int(); lose_wis(); lose_dex(); lose_con();
		    lose_chr();}
	  break;
    case  12:  case  13:
	  msg_print("You feel like a statue!");
	  py.flags.paralysis += randint(23) + 4;
	  break;
    case  14:  case  15:
	  msg_print("You feel your life draining away!");
	  sm_type=randint((int)py.misc.exp)/5;
	  lose_exp(sm_type);
	  break;
default:
	sm_type = randint(88);
	for (k = 0; k < randint(7); k++)
	{ 
	      y = char_row;
	      x = char_col;
	  switch(sm_type) {
	 case 1: summon_ancientd(&y, &x); break;
	 case 2: summon_gundead(&y, &x); break;
	 case 3: summon_wraith(&y, &x); break;
	 case 4: summon_jabberwock(&y, &x); break;
	 case 5: summon_ant(&y, &x); break;
	 case 6: summon_unique(&y, &x);break;
	 case 7: summon_angel(&y, &x); break;
	 case 8: summon_undead(&y, &x); break;
	 case 9: summon_dragon(&y, &x); break;
	 case 10: summon_spider(&y, &x); break;
	 case 11: summon_hound(&y, &x); break;
	 case 12: summon_reptile(&y, &x); break;
	 default:
	    summon_monster(&y, &x, FALSE);}
	} 
    }
  while (randint(7)==1) generic_curse(); /*The marvels of a recursive routine
					--TY*/
