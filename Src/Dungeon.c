...
static void process_command(void)
{
	...
	switch (command_cmd)
	{
		...
		case 'G':
		{
			...
			msg_format("Why not try the library?");
			break;
		}
...
static void dungeon(void)
{
	...
	while (TRUE)
	{
		...
		/* Count game turns */
		turn++;
		if ((turn-old_turn)==2500) do_cmd_feeling();
	}
}
...
void play_game(bool new_game)
{
	...
	if (new_game)
	{
		...
		while(cur_town == TOWN_KADATH)
		{
			cur_town = rand_range(0,MAX_TOWNS-1);
		}
		...
		/* Megahack - If a Vampire, start at the stairs */
