	...
	for (i = 0; i < SURROUND_MAX; i++)
	{
		...
		if (in_bounds(y, x))
		{
			if (cave[y][x].m_idx)
			{
				...
				if (((enemy_ptr->smart & SM_FRIEND) != (m_ptr->smart & SM_FRIEND)) &&
					(!enemy_ptr->csleep))
				{
...
static bool clean_shot(int y1, int x1, int y2, int x2)
{
   ...
   for (dist = 0; dist <= MAX_RANGE; dist++)
   {
       ...
       {
        if (!(m_list[cave[y][x].m_idx].smart & SM_FRIEND))
           break;
        }
...
static void process_monster(int m_idx, bool is_friend)
{
		...
		(m_ptr->generation < 10))
	{
		...
		{
		/* Chance of dying of old age - "Breed Fast, Die Young" */

			if(randint(5)==3) 
			{
				delete_monster(oy,ox);
				return;
			};

			/* Else try to multiply */
