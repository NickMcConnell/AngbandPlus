...
void player_birth_quests(void)
{
	...
	for (i = (MAX_CAVES*2); i<MAX_Q_IDX; i++)
	{
		...
		if (r_info[q_list[i].r_idx].flags1 & RF1_ALWAYS_GUARD) r_info[q_list[i].r_idx].flags1 |= RF1_ALWAYS_GUARD;
...
static bool player_birth_aux()
{
	...
	if ((c == 'Y') || (c == 'y'))
	{
		...
		hack_mutation = FALSE;
		if (k==RACE_BROO) hack_mutation = TRUE;
		p_ptr->prace = k;
        ...
        if (p_ptr->realm1) c_put_str(TERM_L_BLUE, realm_names[p_ptr->realm1],6,15);
        if (p_ptr->realm2) c_put_str(TERM_L_BLUE, realm_names[p_ptr->realm2],7,15);
	...
	else
	{
		...
		/* Prompt for "Long Stairs" option */
		Term_putstr(5, 15, -1, TERM_WHITE,
		"'Long Stairs' means that the mazes of staircases between");
		Term_putstr(5, 16, -1, TERM_WHITE,
		"levels are longer, and do not necessarily come out on an");
		Term_putstr(5, 17, -1, TERM_WHITE,
		"adjacent level.");
		/* Ask the number of additional quests */
		while (1)
		{
			put_str("Use 'long stairs' mode? (y/n) ", 20, 2);
			c = inkey();
			if (c == 'Q') quit(NULL);
			if (c == 'S') return (FALSE);
			if (c == ESCAPE) break;
