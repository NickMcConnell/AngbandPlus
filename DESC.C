...
int pref;
{
  ...
  switch(i_ptr->tval)
    ...
    case  TV_LIGHT:
      ...
      if (!stricmp("The Jewel of Judgement", basenm) && !known2_p(i_ptr))
        basenm = "a Ruby Pendant";
      break;
    case  TV_SPIKE:
      break;
    case  TV_BOW:
    ...
    case  TV_SOFT_ARMOR:
      break;
    case  TV_AMULET:
	if (modify || !(store_bought_p(i_ptr)))
	{
	  basenm = "& %s Amulet";
  ...
  if (!pref)
    ...
    if (i_ptr->name2 == SN_CALLED && known2_p(i_ptr))
    { (void) strcat(tmp_val, " ");
      (void) strcat(tmp_val, i_ptr->artname);}
