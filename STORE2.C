...
static void prt_comment1()
{
  ...
  if (randint(40)==1) {
        ...
        if (getLine("rumors.txt",0,destination))
		...
		if (py.misc.au >= price)
		  {
		    ...
		    store_destroy(store_num, item_val, TRUE);
                     /* If the player just bought another stackable, don't
                        erase the (possible) current inscription.
                     */
                     if ((inventory[item_new].subval > ITEM_SINGLE_STACK_MAX)
               || inventory[item_new].number <= 1)
	  ...
	  /* call known2 for store item, so charges/pluses are known */
		  ...
		  if (real==0) {
		    ...
		    case 3:
		      ...
		      break;
            case 4:
              msg_print("The shopkeeper makes an obscene gesture.");
              break;
		    }
		  } else if (real> (4*cost)) {
            switch (randint(5)) {
		    ...
		    case 4:
		      ...
		      break;
            case 5:
              msg_print("The shopkeeper dances around in joy!");
              break;
		    }
		  } else if (real>cost) {
		    switch (randint(4)) {
