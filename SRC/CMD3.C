...
void do_cmd_tunnel()
{
	...
	/* You cannot dig without a weapon unless you're a dragon*/
	else if (!j_ptr->tval && p_ptr->prace<MIN_DRAGON) {
	...
	/* Okay, try digging */
	    ...
	    } else tabil += 10; /*claws aren't as good as a shovel */
...
void do_cmd_bash()
{
	...
	/* Request to bash something */
		...
		else if (randint(168) > p_ptr->use_stat[A_DEX]) {
...
void inscribe(inven_type *i_ptr, cptr str)
{
    ...
    /* Redraw the choice window just in case */
...
void do_cmd_check_artifacts(void)
{
	...
	if (v_list[k].cur_num) {
		...
		objdes_store(base_name, &forge, 0);
	    }
