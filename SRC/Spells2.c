...
bool hp_player(int num)
{
	...
	if (p_ptr->chp < p_ptr->mhp)
	{
		...
		/* Alex */
		update_act_fail_dam(ACT_CURE_WOUNDS, num);
		update_act_fail_dam(ACT_HEAL1, num);
		update_act_fail_dam(ACT_HEAL2, num);
...
static void uncurse_object(object_type *o_ptr)
{
	...
	/* Alex: now we have to remove curse from artifact entry.
	 * In other case the next Enchant Artifact will restore the curse. */
	if (o_ptr->name1)
	{
...
bool restore_level(void)
{
	...
	if (p_ptr->exp < p_ptr->max_exp)
	{
		...
		/* Alex */
		update_act_fail(ACT_RESTORE_LIFE, (p_ptr->max_exp - p_ptr->exp)*1000/p_ptr->max_exp);
...
bool lose_all_info(void)
{
	...
	/* Alex: also forget some abilities/activation experience */
	forget_abil_act();
...
void set_recall(void)
{
	...
	if (!p_ptr->word_recall)
	{
		...
		/* Alex */
		update_act_fail(ACT_WOR, p_ptr->max_depth/3);
	}
...
bool detect_traps(void)
{
	...
	for (y = p_ptr->wy; y < p_ptr->wy+SCREEN_HGT; y++)
	{
		for (x = p_ptr->wx; x < p_ptr->wx+SCREEN_WID; x++)
		{
			...
			{
				/* Alex */
				if (!(cave_info[y][x] & (CAVE_MARK))) update_act_fail(ACT_DETECT, 5);
...
bool detect_doors(void)
{
	...
	for (y = p_ptr->wy; y < p_ptr->wy+SCREEN_HGT; y++)
	{
		for (x = p_ptr->wx; x < p_ptr->wx+SCREEN_WID; x++)
		{
			...
			{
				/* Alex */
				if (!(cave_info[y][x] & (CAVE_MARK))) n_det++;
	...
	if (detect)
	{
		...
		/* Alex */
		update_act_fail(ACT_DETECT, n_det/5);
	}
...
bool detect_stairs(void)
{
	...
	for (y = p_ptr->wy; y < p_ptr->wy+SCREEN_HGT; y++)
	{
		for (x = p_ptr->wx; x < p_ptr->wx+SCREEN_WID; x++)
		{
			...
			{
				/* Alex */
				if (!(cave_info[y][x] & (CAVE_MARK))) update_act_fail(ACT_DETECT, 3);
...
bool detect_treasure(void)
{
	...
	if (detect)
	{
		...
		/* Alex */
		update_act_fail(ACT_DETECT, 1 + n_det/5);
	}
...
bool detect_objects_normal(void)
{
	...
	if (detect)
	{
		...
		/* Alex */
		update_act_fail(ACT_DETECT, 1 + n_det);
	}
...
bool detect_monsters_normal(void)
{
	...
	if (flag)
	{
		...
		/* Alex */
		update_act_fail(ACT_DETECT, 1 + n_det);
		update_abil_fail(a_telepathy, n_det/500);
	}
...
bool detect_monsters_invis(void)
{
	...
	if (flag)
	{
		...
		/* Alex */
		update_act_fail(ACT_DETECT, 1 + n_det);
		update_abil_fail(a_see_invis, n_det);
		update_abil_fail(a_telepathy, 1 + n_det/100);
	}
...
bool enchant(object_type *o_ptr, int n, int eflag)
{
	...
	for (i=0; i<n; i++)
	{
		/* Alex: now we have to "enchant" artifact entry.
		* In other case the next Enchant Artifact will restore the object to previous state. */
		artifact_type * a_ptr;
		if (a) a_ptr = &a_info[o_ptr->name1];
...
bool ident_spell(void)
{
	...
	/* Alex */
	update_act_fail(ACT_IDENTIFY, k_info[o_ptr->k_idx].level/5);
...
/*
 * Recharge a wand/staff from the pack or on the floor.
 *
...
bool recharge(int num)
{
	...
	int fail_type, recharge_amount;
 
	cptr q, s;


	/* Only accept legal items (wands and staffs) */
	...
	if ((i <= 1) || (rand_int(i) == 0))
	{
		/* 25% chance of the entire stack, 5% more for each wand in the stack
		 * else destroy one wand. */
	...
	/* Destroy all memebers of a stack of objects. */
		if (fail_type == 2)
		{
			/* Dangerous Hack -- Destroy the item, or items */
			if (o_ptr->number > 1)
	...
	else
	{
		...
		{
			recharge_amount +=
				(randint(recharge_amount * (o_ptr->number - 1))) / 5;;
			if (recharge_amount < 1) recharge_amount = 1;
	...
	/* Alex */
	update_act_fail(ACT_RECHARGE1, o_ptr->number * lev/10);
...
bool banishment(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		/* Take some damage */
		take_hit(randint(4), "the strain of casting Banishment", a_last);

		/* Alex */
		update_act_fail(ACT_BANISHMENT, r_ptr->level/20);
...
bool mass_banishment(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		/* Take some damage */
		take_hit(randint(3), "the strain of casting Mass Banishment", a_last);

		/* Alex */
		update_act_fail(ACT_MASS_BANISHMENT, r_ptr->level/30);
...
bool probing(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		if (m_ptr->ml)
		{
			...
			/* Alex */
			update_act_fail(ACT_PROBE, r_ptr->level/20);
...
void earthquake(int cy, int cx, int r)
{
	...
	if (hurt)
	{
		...
		/* Take some damage */
		if (damage) take_hit(damage, "an earthquake", a_last);
	}
		...
		for (dx = -r; dx <= r; dx++)
		{
			...
			if (cave_m_idx[yy][xx] > 0)
			{
				    ...
				    !(r_ptr->flags2 & (RF2_PASS_WALL)))
				{
					...
					/* Alex */
					update_abil_fail_dam(a_impact, damage);
...
bool lite_area(int dam, int rad)
{
	...
	if (!p_ptr->blind)
	{
		...
		/* Alex */
		update_act_fail(ACT_ILLUMINATION, 10);
	}
...
bool brand_bolts(void)
{
	...
	/* Alex */
	update_act_fail(ACT_FIREBRAND, 1);
	/* Done */
	return (TRUE);
}
...
/* Alex */
static bool item_tester_hook_for_create_ego(const object_type *o_ptr)
{
        return !o_ptr->name1 && !o_ptr->name2 &&
		((item_tester_hook_armour(o_ptr) && o_ptr->tval != TV_DRAG_ARMOR) ||
		item_tester_hook_weapon(o_ptr));
}

/* Alex */
bool create_ego(int power, int sacrifice_levels){
	int item;
	bool okay = FALSE;
	s16b old_object_lev = object_level;
	...
	q = "Enchant which item? ";
	s = "You have nothing to enchant.";
	item_tester_hook = item_tester_hook_for_create_ego;
	...
	if (power > 0)
		apply_magic(o_ptr, object_level, FALSE, TRUE, TRUE);
	else
		/* Create cursed item - see apply_magic() */
		apply_magic(o_ptr, -object_level, FALSE, FALSE, TRUE);

	object_level = old_object_lev;
	okay = o_ptr->name2;
	...
	else
	{
		...
		/* Sacrifice levels */
		if (okay && sacrifice_levels > 0)
		{
                        	sacrifice_exp(sacrifice_levels);
		}
	}
...
/* Alex */
bool item_tester_hook_for_create_artifact(const object_type *o_ptr)
{
	return	is_special(o_ptr->tval) ||
		item_tester_hook_armour(o_ptr) ||
		item_tester_hook_weapon(o_ptr);
}

/* Alex */
bool create_artifact(int power, int sacrifice_levels, bool player_choose, int choices, int tries){
	int item;
	bool okay = FALSE;
	...
	q = "Enchant which item? ";
	s = "You have nothing to enchant.";
	item_tester_hook = item_tester_hook_for_create_artifact;
	if (!get_item(&item, q, s, (USE_EQUIP))) return (FALSE);
	/* Get the item (in the pack) */
	o_ptr = &inventory[item];
	...
	if (o_ptr->name1)
	{
		okay = enchant_artifact(o_ptr->name1, power, player_choose, choices, tries);
		if (okay)
	...
	else
	{
		...
		if (o_ptr->tval == TV_LITE && 
			...
			o_ptr->pval = 0;
		okay = add_new_art(power, o_ptr, player_choose, choices, tries);
		if (okay)
		{
			o_ptr->name1 = z_info->a_max - 1;
			apply_magic(o_ptr, -1, TRUE, TRUE, TRUE);
		}
	}
	...
	if (!okay)
	{
		...
		msg_print("The enchantment failed.");
		if (player_choose)
			msg_format("Need at least power of %d", last_power + 1);

		/* Sacrifice levels */
		if (sacrifice_levels > 0) sacrifice_exp(1);
	}
	else
	{
		o_ptr->discount = 0;
		...
		/* Sacrifice levels */
		if (sacrifice_levels > 0) sacrifice_exp(sacrifice_levels);
	}
