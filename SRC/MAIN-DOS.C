...
/*
 * The currently playing midi-song
 */
static MIDI *song = NULL;
...
void play_midi_song(void)
{
    ...
    if (mute_songs) return;

	/* Music section */
	strcpy(section, "Angband-Music");

	/* Get the number of songs */
	n = get_config_int(section, "Songs", 0);
	...
	if (n)
	{
        ...
        if (shuffle_songs)
        {
            ...
            do {
                song_number = randint(n);
                }
                while (!song_number || (song_number > n));

        }


		sprintf(buf, "Song%d", song_number);

		/* Get font filename */
		strcpy(filename, get_config_string(section, buf, ""));

		/* Access the new song */
		path_build(buf, 1024, xtra_music_dir, filename);

		/* Load the new song */
		song = load_midi(buf);

		/* And play it */
		if (song) play_midi(song, 0);
...
void Term_xtra_dos_clear(void)
{
	...
	{
		/* Draw the background */
		stretch_blit(td->background, screen,
			0, 0, td->background->w, td->background->h,
			x1, y1, w1, h1);
	}
	else
...
static errr Term_xtra_dos_sound(int v)
{
	...
	/* Remove the currently playing sound */
	if (sample) destroy_sample(sample);

	/* Load a new one */
	strcpy(filename, sound_file[v]);

	if ((sample = load_sample(filename)))
	{
		/* Play the sound, catch errors */
		return (play_sample(sample, 255, 128, 1000, 0) == 0);
	}
...
#ifdef USE_SOUND
			/*
			 * Check for end of Midi-song
			 * and start a new one
			 */
			if (midi_pos == -1) play_midi_song();
#endif /* USE_SOUND */
...
static errr Term_wipe_dos(int x, int y, int n)
{
	...
	if (td->background)
	{
		int source_x = x * td->background->w / td->cols;
		int source_y = y * td->background->h / td->rows;
		int source_w = n * td->background->w / td->cols;
		int source_h = td->background->h / td->rows;
...
static void Term_nuke_dos(term *t)
{
	...
	if (td->background) destroy_bitmap(td->background);
...
static bool init_sound()
{
	...
	if (!install_sound(DIGI_AUTODETECT, MIDI_AUTODETECT, NULL))
	{
		...
		for (i = 1; i < SOUND_MAX; i++)
		{
			...
			/* Access the sound */
			path_build(buf, 1024, xtra_sound_dir, wav);

			/* Save the sound filename, if it exists */
			if (exists(buf)) sound_file[i] = string_make(buf);
		}
...
errr init_dos(void)
{
	...
	for (i = 0; i < num_windows; i++)
	{
		...
		strcpy(name_background[i], get_config_string(section, "background_file", ""));
	...
	for (i = 0; i < num_windows; i++)
	{
		...
		path_build(filename, 1024, xtra_graf_dir, name_background[i]);

		/* Try to open the bitmap file */
		td->background = load_bitmap(filename, background_pallete);

		/* Try to set the palette for the background */
		if (td->background)
		{
			/* We only use the 16 colors from 223 to 239 */
			set_palette_range(background_pallete, 223, 239, 0);
		}
	...
	/* Is sound-mode requested ? */
	if (arg_sound)
	{
		/* Initialize the sound */
		use_sound = init_sound();
	}
