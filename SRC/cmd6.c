...
static void corpse_effect(object_type *o_ptr, bool cutting)
{
	...
	monster_xtra *x_ptr = &x_info[o_ptr->r_idx];
	...
	int i, dam, idam, mdam, brpow, brdam;

	/* How much of the monster's breath attack remains */
	brpow = (o_ptr->pval > r_ptr->wt ? (o_ptr->pval - r_ptr->wt) / 5 : 0);
	brpow = (brpow > r_ptr->wt / 5 ? r_ptr->wt / 5 : brpow);

	/*
	 * The breath is only discharged by accident or by slicing off pieces
	 * of meat, and only by corpses.
	 */
	if (o_ptr->sval != SV_CORPSE || (rand_int(o_ptr->wt / 5) && !cutting)) brpow = 0;
	...
	if (!cutting)
	{
		for (i = 0; i < 4; i++)
		{
			method = x_ptr->blow[i].method;
			effect = x_ptr->blow[i].effect;
			d_dice = x_ptr->blow[i].d_dice;
			d_side = x_ptr->blow[i].d_side;
			dam = damroll(d_dice, d_side) * o_ptr->pval / o_ptr->wt / 2;
			idam = damroll(d_dice, d_side) * ((o_ptr->wt / o_ptr->pval > 2) ?
					 o_ptr->wt / o_ptr->pval : 2);
	...
	if (r_ptr->flags4 & RF4_BR_DISE && brpow > 0)
   {
	 ...
	 */
	if (!harmful && !cutting && (o_ptr->sval != SV_RAW_MEAT))
	{
...
void do_cmd_eat_food(void)
{
   ...
   /* Assume the food should be destroyed afterward */
   bool destroy = TRUE;
   ...
   if (o_ptr->tval == TV_SKELETON)
   {
		switch (o_ptr->sval)
      {
      	case SV_CORPSE:
      	{
			...
			{
				/* Not all is edible */
				if (((r_ptr->flags3 & RF3_HAS_SKULL) && (o_ptr->wt <= (r_ptr->wt * 7) / 300)) ||
					(!(r_ptr->flags3 & RF3_HAS_SKULL) && (o_ptr->wt <= r_ptr->wt / 150)))
				{
					msg_print("There is not enough meat.");
					return;
				}
			...
			case SV_RAW_MEAT:
			{
				...
				if (!o_ptr->timeout && (o_ptr->wt > o_ptr->pval) &&
					 !(p_ptr->resist_pois || p_ptr->oppose_pois))
				{
					set_poisoned(p_ptr->poisoned + rand_int(o_ptr->wt - o_ptr->pval) +
					(o_ptr->wt - o_ptr->pval));
				}
				break;
			}
		...
		/* Those darn microorganisms */
		if (!o_ptr->timeout && (o_ptr->wt - o_ptr->pval > 10) &&
			 !(p_ptr->resist_pois || p_ptr->oppose_pois))
		{
			set_poisoned(p_ptr->poisoned + rand_int(o_ptr->wt - o_ptr->pval) +
			(o_ptr->wt - o_ptr->pval));
		...
		/* Partially cured */
		if (o_ptr->wt > o_ptr->timeout)
		{
			/* Adjust the "timeout" without overflowing */
			o_ptr->timeout = (o_ptr->timeout * ((100 * o_ptr->timeout) / o_ptr->wt)) / 100;
		}
	}
	else
	{
		...
		switch (o_ptr->sval)
		{
	   	...
	   	case SV_FOOD_SLIME_MOLD:
		   {
			...
			{
				msg_print("The mushrooms are quite tasty.");
				ident = TRUE;
				break;
			}
	...
	/* Set the food value if not already set */
...
void do_cmd_cut_corpse(void)
{
   ...
   if (o_ptr->sval != SV_CORPSE && o_ptr->sval != SV_HEAD)
   {
	...
	switch (o_ptr->sval)
   {
		case SV_CORPSE:
      {
         if (r_ptr->flags3 & RF3_HAS_BONES)
			{
            not_meat = (r_ptr->wt * 3) / 5;
			}
         else
			{
            not_meat = (r_ptr->wt * 7) / 20;
			}
         meat = r_ptr->wt + r_ptr->wt / 10 - not_meat;
         break;
      }
      case SV_HEAD:
      {
         if (r_ptr->flags3 & RF3_HAS_SKULL)
         {
				not_meat = (r_ptr->wt * 7) / 300;
			}
         else
         {
				not_meat = r_ptr->wt / 150;
         }
         meat = r_ptr->wt / 30 + r_ptr->wt / 300 - not_meat;
         break;
      }
   }

   if (o_ptr->wt <= not_meat || meat < 10)
   {
		msg_print("There is not enough meat.");
		return;
	}
   ...
   /* Make some meat */
  	object_prep(i_ptr, lookup_kind(TV_SKELETON, SV_RAW_MEAT));

	i_ptr->number = meat / 10;
	i_ptr->r_idx = o_ptr->r_idx;
...
void do_cmd_cure_meat(void)
{
	...
	switch (i_ptr->sval)
	{
		...
		case SV_POTION_POISON:
		{
			...
			cure = 0;
			o_ptr->pval /= 2;
			if (o_ptr->pval > o_ptr->wt) o_ptr->pval = o_ptr->wt;
		...
		case SV_POTION_DEATH:
		{
			...
			cure = 0;
			o_ptr->pval /= 10;
			if (o_ptr->pval > o_ptr->wt) o_ptr->pval = o_ptr->wt / 2;
	...
	if (((o_ptr->sval == SV_RAW_MEAT) && (o_ptr->wt > o_ptr->pval)) ||
		 o_ptr->wt - o_ptr->pval > 10)
	{
		cure = (cure * o_ptr->pval) / (o_ptr->wt * 20);
	}
...
void do_cmd_quaff_potion(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_POTION_SALT_WATER:
		{
         /* Vomiting is bad if you're internally bleeding */
			if (p_ptr->food < PY_FOOD_DEATH)
         {
			...
			{
         	msg_print("The potion makes you convulse sharply!");
            take_hit(p_ptr->mhp / 2, "drinking salt water");

				if (!(p_ptr->resist_pois || p_ptr->oppose_pois))
            	(void)set_poisoned(p_ptr->poisoned + rand_int(10) + 10);
         }
			ident = TRUE;
			break;
		}
		...
		case SV_POTION_HEALING:
		{
			...
			if (set_stun(0)) ident = TRUE;
			if (set_cut(0)) ident = TRUE;
         if (p_ptr->food >= PY_FOOD_DEATH && set_food(PY_FOOD_FULL)) ident = TRUE;
			break;
		}
		...
		case SV_POTION_STAR_HEALING:
		{
			...
			if (set_stun(0)) ident = TRUE;
			if (set_cut(0)) ident = TRUE;
         if (p_ptr->food >= PY_FOOD_PAIN && set_food(PY_FOOD_FULL)) ident = TRUE;
			break;
		}
		...
		case SV_POTION_LIFE:
		{
			...
			(void)set_stun(0);
			(void)set_cut(0);
         if (p_ptr->food >= PY_FOOD_DEATH) set_food(PY_FOOD_FULL);
...
bool curse_armor(int i)
{
	...
	/* Artifact is *already* badly cursed */
	if (artifact_p(o_ptr) && (a_ptr->flags3 & (TR3_PERMA_CURSE | TR3_HEAVY_CURSE)))
	{
		/* Oops! */
		msg_format("A %s %s! Your %s absorbs the dark energy!",
					  "terrible black aura", "surrounds your armor", o_name);

		if (a_ptr->to_a > 0 - 100) a_ptr->to_a = o_ptr->to_a -= randint(5) + randint(5);

		if (a_ptr->pval > 0 - 100) a_ptr->pval = o_ptr->pval -= randint(3);
	...
	else if (artifact_p(o_ptr) && rand_int(2))
	{
		/* Cool */
	...
	/* Minor curse */
	else if (!(o_ptr->ident & IDENT_CURSED) && rand_int(3))
	{
		/* Oops */
		msg_format("A terrible black aura engulfs your %s!", o_name);
...
bool curse_weapon(int i)
{
	...
	if (artifact_p(o_ptr) && (a_ptr->flags3 & (TR3_PERMA_CURSE | TR3_HEAVY_CURSE)))
	{
		/* Oops! */
		msg_format("A %s %s! Your %s absorbs the dark energy!",
					  "terrible black aura", "surrounds your weapon", o_name);

		if (a_ptr->to_h > -100) a_ptr->to_h = o_ptr->to_h -= damroll(2, 5);
		if (a_ptr->to_d > -100) a_ptr->to_d = o_ptr->to_d -= damroll(2, 5);
		a_ptr->ds = o_ptr->ds += (o_ptr->dd > 1) ? o_ptr->dd : 0;
		a_ptr->dd = o_ptr->dd -= (o_ptr->dd > 1) ? 1 : 0;

		if (a_ptr->pval && (a_ptr->pval > -100))
		{
			a_ptr->pval = o_ptr->pval -= randint(3);
		}
	...
	/* Already evil */
	else if (o_ptr->name2 == EGO_MORGUL)
	{
		/* Oops! */
		msg_format("A %s %s! Your %s absorbs the dark energy!",
					  "terrible black aura", "surrounds your weapon", o_name);

		if (o_ptr->to_h > -100) o_ptr->to_h -= damroll(2, 5);
		if (o_ptr->to_d > -100) o_ptr->to_d -= damroll(2, 5);

		/* Do not adjust dice of bows */
		if (i == INVEN_WIELD)
		{
			o_ptr->dd = rand_spread(o_ptr->dd, 1);
			o_ptr->ds = rand_spread(o_ptr->ds, 1);

			if (o_ptr->dd < 1) o_ptr->dd = 1;
			if (o_ptr->ds < 1) o_ptr->ds = 1;
		}

		if (o_ptr->pval && (o_ptr->pval > -100))
		{
	...
	{
		/* Oops */
		msg_format("A terrible black aura sears your %s!", o_name);
		...
		/* Make worse */
		if (o_ptr->pval) o_ptr->pval -= rand_int(5);
		o_ptr->to_h -= rand_int(10) + 5;
		o_ptr->to_d -= rand_int(10) + 5;
 ...
 * cancelled before use.  Reading them still takes a turn, though.
 */
void do_cmd_read_scroll(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		{
			if (curse_armor(INVEN_BODY + rand_int(6))) ident = TRUE;
			break;
		}

		case SV_SCROLL_CURSE_WEAPON:
		{
			if (curse_weapon(INVEN_WIELD + rand_int(2))) ident = TRUE;
			break;
		}
...
void do_cmd_use_staff(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_STAFF_CURING:
		{
			...
			if (set_stun(0)) ident = TRUE;
			if (set_cut(0)) ident = TRUE;
         if (p_ptr->food >= PY_FOOD_PAIN && p_ptr->food < PY_FOOD_DEATH && set_food(PY_FOOD_FULL)) ident = TRUE;
			break;
		}
...
void do_cmd_aim_wand(void)
{
	...
	switch (sval)
	{
		...
		{
			fire_ball(GF_ACID, dir, 60, 2, FALSE);
			ident = TRUE;
			break;
		}
		...
		{
			fire_ball(GF_ELEC, dir, 32, 2, FALSE);
			ident = TRUE;
			break;
		}
		...
		{
			fire_ball(GF_FIRE, dir, 100, 3, TRUE);
			ident = TRUE;
			break;
		}
		...
		{
			fire_ball(GF_COLD, dir, 80, 3, TRUE);
			ident = TRUE;
			break;
		}
		...
		case SV_WAND_DRAGON_BREATH:
		{
			switch (randint(5))
			{
				case 1:
				{
					fire_ball(GF_ACID, dir, 100, 3, TRUE);
					break;
				}

				case 2:
				{
					fire_ball(GF_ELEC, dir, 80, 3, TRUE);
					break;
				}
				...
				default:
				{
					fire_ball(GF_POIS, dir, 60, 3, TRUE);
					break;
				}
			}
...
static void ring_of_power(int dir)
{
	...
	switch (randint(10))
	{
		...
		{
			/* Mana Breath */
			fire_ball(GF_MANA, dir, 300, 3, TRUE);
	...
	if (o_ptr->name1)
	{
		...
		switch (o_ptr->name1)
		{
			...
			case ART_RAZORBACK:
			{
				...
				for (i = 0; i < 8; i++) fire_ball(GF_ELEC, ddd[i], 150, 3, TRUE);
				o_ptr->timeout = 1000;
				break;
			}
	...
	if (o_ptr->tval == TV_DRAG_ARMOR)
	{
		...
		switch (o_ptr->sval)
		{
			case SV_DRAGON_BLUE:
			{
				...
				fire_ball(GF_ELEC, dir, 100, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_WHITE:
			{
				...
				fire_ball(GF_COLD, dir, 110, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_BLACK:
			{
				...
				fire_ball(GF_ACID, dir, 130, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_GREEN:
			{
				...
				fire_ball(GF_POIS, dir, 150, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_RED:
			{
				...
				fire_ball(GF_FIRE, dir, 200, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			{
				msg_print("You breathe chemicals.");
				fire_ball(GF_CONFUSION, dir, 120, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_GOLD:
			{
				msg_print("You sneeze violently!");
				fire_ball(GF_SOUND, dir, 130, 2, TRUE);
				o_ptr->timeout = rand_int(450) + 450;
				break;
			}
			...
			case SV_DRAGON_SHINING:
			{
				...
				fire_ball((chance == 0 ? GF_LITE : GF_DARK), dir, 200, 2, TRUE);
				o_ptr->timeout = rand_int(300) + 300;
				break;
			}
			...
			case SV_DRAGON_POWER:
			{
				...
				fire_ball(GF_MISSILE, dir, 300, 2, TRUE);
				o_ptr->timeout = rand_int(300) + 300;
				break;
			}
		}
