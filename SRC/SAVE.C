...
static void wr_options(void)
{
    ...
    if (choice_spells)		l |= 0x04000000L;
...
static bool save_player_aux(char *fnam)
{
    ...
    if (!ok) {
        ...
        /* Oops */
        strcpy(temp, "Can't create new savefile");
        if (fd < 0) strcpy(temp, "Error writing to savefile");
        msg_print(temp);
...
bool save_player()
{
    ...
    /* New savefile */
    strcpy(temp1, savefile);
    strcat(temp1, ".new");

    /* Attempt to save the player */
    if (save_player_aux(temp1)) {

        /* Old savefile */
        strcpy(temp2, savefile);
        strcat(temp2, ".old");
