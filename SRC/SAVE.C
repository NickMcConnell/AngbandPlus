...
static void wr_extra(void)
{
	...
	/* D11-Hack: Write D11-Angand additional Data */
	wr_s16b(p_ptr->recall_depth);
	wr_s16b(p_ptr->training);
	wr_s16b(p_ptr->abilities);
	for (i = 0; i < 7; i++) wr_s16b(p_ptr->ab_turn_count[i]);
	for (i = 0; i < 10; i++) wr_byte(p_ptr->quick_spells[i]);
...
static bool wr_savefile_new(void)
{
	...
	/* D11-Hack: Write the backpack */
	for (i = 0; i < BACKPACK_TOTAL; i++)
	{
		object_type *o_ptr = &backpack[i];
