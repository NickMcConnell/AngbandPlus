 ...
 * and other messages, autoscummer behaviour.  Creation of the arena and
 * quests.
 *
...
#define DUN_UNUSUAL 100 /* Level/chance of unusual room (was 200) */
...
#define DUN_WILD_VAULT  0     /* Chance of finding a wilderness vault. */
	...
	for (i = 0; i < num; i++)
	{
		...
		for (flag = FALSE; !flag; )
		{
			...
			for (j = 0; !flag && j <= 3000; j++)
			{
				...
				/* Quest -- must go up */
				else if ((is_quest(dun_level) && (dun_level > 1)) || (dun_level >= MAX_DEPTH-1))
				{
...
static void build_type2x(int qtype)
{
	...
	/* Hack -- Build the vault */
	build_quest_vault((q_ptr->hgt/2) + 2, (q_ptr->wid/2) + 2,
	    q_ptr->hgt, q_ptr->wid, q_text + q_ptr->text,qtype);
}
#endif
...
static bool cave_gen(void)
{
	...
	if ((dun_level <= 19) && (randint(20) > 15))
	{
		...
		{
			 for (i=0; i < randint(DUN_STR_QUA); i++)
				build_streamer2(FEAT_CHAOS_FOG, FALSE, FALSE);
		}
	}
