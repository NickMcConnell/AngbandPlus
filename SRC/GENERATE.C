	...
	for (t = data, dy = 0; dy < ymax; dy++)
	{
		for (dx = 0; dx < xmax; dx++, t++)
		{
			...
			switch (*t)
			{
				...
				case '8':
				{
					place_monster_one(y, x, r_idx, TRUE);
				}
			}
		}
	}
...
static bool room_build(int y0, int x0, int typ)
{
	...
	/* Restrict level */
	if ((p_ptr->depth < room[typ].level) && ((!is_quest(p_ptr->depth)) && (typ != 9))) return (FALSE);
	...
	switch (typ)
	{
		...
		case 9: build_type9(y, x); break; /* Heino Vander Sanden */
...
static void cave_gen(void)
{
	 ...
	 * Heino Vander Sanden
	 */
	if (is_quest(p_ptr->depth) && (get_max_monster() >= 10))
	{
		int r_idx;
		...
		while ((cur_mon >= 10) && (rand_range(p_ptr->depth,100) >= 25))
		{
