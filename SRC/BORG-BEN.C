...
static s32b item_power(auto_item *item)
{
    ...
    if (borg_item_is_armour(item)) {
        ...
        /* Assume full enchantment */
        if (item->to_a < 10) item->to_a = 10;
    ...
    /* Weapons */
        ...
        /* Assume full enchantment */
        if (item->to_h < 10) item->to_h = 10;
        if (item->to_d < 10) item->to_d = 10;
...
static bool borg_notice_aux(auto_item *item)
{
    ...
    /* Hack -- Wait for feelings (unless average) */
...
static void borg_notice(void)
{
    ...
    for (i = INVEN_WIELD; i <= INVEN_FEET; i++) {
        ...
        /* Enchant all armour */
        if (borg_item_is_armour(item) && (item->to_a < 9)) {
            kind_need[kind_enchant_to_ac] += (9 - item->to_a);
        }

        /* Enchant all weapons (to damage) */
        if (borg_item_is_weapon(item) && (item->to_d < 9)) {
            kind_need[kind_enchant_to_dam] += (9 - item->to_d);
        }

        /* Enchant all weapons (to hit) */
        if (borg_item_is_weapon(item) && (item->to_h < 9)) {
            kind_need[kind_enchant_to_hit] += (9 - item->to_h);
        }
    }
...
static bool borg_use_others(void)
{
    ...
    if (b && (a < 9) && (borg_action('r', kind_enchant_to_ac))) {
    ...
    /* Enchant that item if "possible" */
    if (b && (a < 9) && (borg_action('r', kind_enchant_to_hit))) {
    ...
    /* Enchant that item if "possible" */
    if (b && (a < 9) && (borg_action('r', kind_enchant_to_dam))) {
 ...
 * word of recall, teleport, identify, healing potions, etc).
 */
...
static bool borg_test_stuff(void)
{
    ...
    for (i = INVEN_WIELD; i <= INVEN_FEET; i++) {
        ...
        if (borg_action_pval('z', kind_rod_identify) ||
            ...
            borg_info(format("Identifying %s.", item->desc));
