...
static void prt_sp(void)
{
	...
	/* Now repeat for chi */
	put_str("Max CH ", ROW_MAXCHI, COL_MAXCHI);

	sprintf(tmp, "%5d", p_ptr->mchi);
	color = TERM_L_GREEN;

	c_put_str(color, tmp, ROW_MAXCHI, COL_MAXCHI + 7);


	put_str("Cur CH ", ROW_CURCHI, COL_CURCHI);

	sprintf(tmp, "%5d", p_ptr->cchi);
	...
	/* Show mana */
	c_put_str(color, tmp, ROW_CURCHI, COL_CURCHI + 7);
