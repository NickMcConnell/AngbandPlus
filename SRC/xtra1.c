...
static void prt_hunger(void)
{
	...
	{
		c_put_str(TERM_ORANGE, "Bloat! ", ROW_HUNGRY, COL_HUNGRY);
	}

	/* Dying */
	else
	{
		c_put_str(TERM_RED, "DYING ", ROW_HUNGRY, COL_HUNGRY);
	}
}
...
static void calc_mana(void)
{
	...
	/* Weigh the armor */
	cur_wgt = 0;
	cur_wgt += inventory[INVEN_BODY].wt;
	cur_wgt += inventory[INVEN_HEAD].wt;
	cur_wgt += inventory[INVEN_ARM].wt;
	cur_wgt += inventory[INVEN_OUTER].wt;
	cur_wgt += inventory[INVEN_HANDS].wt;
	cur_wgt += inventory[INVEN_FEET].wt;
...
static void calc_bonuses(void)
{
   ...
   if (p_ptr->food >= PY_FOOD_DEATH) p_ptr->pspeed -= 10;
	...
	if (hold < o_ptr->wt / 10)
	{
		...
		p_ptr->to_h += 2 * (hold - o_ptr->wt / 10);
		p_ptr->dis_to_h += 2 * (hold - o_ptr->wt / 10);
