...
/* D11-Add/Hack */
/* Here we print the Status "Trng" (Training) */
static void prt_train(void)
{
	if (p_ptr->training)
	{
		put_str("Tr", ROW_TRAIN, COL_TRAIN);
	}
	else
	{
		put_str("  ", ROW_TRAIN, COL_TRAIN);
	}
}

static void prt_study(void) /* D11-Hack - "St" instead of "Study" */
{
	if (p_ptr->new_spells)
	{
		put_str("St", ROW_STUDY, COL_STUDY);
	}
	else
	{
...
/* D11-Add/Hack
 * Get the available Training Sessions and give a message
 */
void update_train(void)
{
	if (p_ptr->training)
		msg_format("You can train you attributes %d times.", p_ptr->training);
	else
		msg_format("You can train no more attributes know.");
}
...
void update_stuff(void)
{
	...
	/* D11-Hack - Update Training Sessions */
	if (p_ptr->update & (PU_TRAIN))
	{
		p_ptr->update &= ~(PU_TRAIN);
		update_train();
	}
	/* D11-Hack - Update Special Abilities */
	if (p_ptr->update & (PU_ABILITIES))
	{
		p_ptr->update &= ~(PU_ABILITIES);
		update_abilities();
	}
...
void redraw_stuff(void)
{
	...
	/* D11-Hack: Update Training-Status */
	if (p_ptr->redraw & (PR_TRAIN))
	{
		p_ptr->redraw &= ~(PR_TRAIN);
		prt_train();
	}
}
