...
static cptr comment_2a[MAX_COMMENT_2A] =
{
   ...
   "Enough!  I will take no less than %s!"
};
...
static cptr comment_3a[MAX_COMMENT_3A] =
{
	"You try my patience.  %s is final.",
	"My patience grows thin.  %s is final.",
   "Not more than %s!  Take it, or keep your junk!"
};
...
/*
 * Let the customer know we're here
 */

#define MAX_COMMENT_8		4

static cptr comment_8[MAX_COMMENT_8] =
{
	"The shopkeeper hums.",
	"The shopkeeper whistles a tune.",
	"The shopkeeper turns to look at a book.",
	"The shopkeeper takes something from a shelf."
};
 ...
 */
static int store_num = STORE_HOME;
...
/*
 * Determine the space needed for an object
 */
s16b store_bulk(object_type *o_ptr)
{
	s16b b;

	/* The shopkeeper keeps items spaced for display */
	b = o_ptr->bulk + o_ptr->bulk / 10;
	b = (b < 10 ? 10 : b);
...
static bool store_object_similar(object_type *o_ptr, object_type *j_ptr)
{
	...
	/* Require matching discounts or overpriced items */
...
static void store_object_absorb(object_type *o_ptr, object_type *j_ptr)
{
	...
	st_ptr->total_bulk += store_bulk(j_ptr) * j_ptr->number;
...
static bool store_will_buy(object_type *o_ptr)
{
	...
	if (o_ptr->discount == 95) return (FALSE);
	...
	switch (store_num)
	{
		...
		case 0:
		{
			...
			switch (o_ptr->tval)
			{
				case TV_POTION:
				{
					/* Buy booze and salt water */
					if ((o_ptr->sval != SV_POTION_BOOZE) &&
						 (o_ptr->sval != SV_POTION_SALT_WATER)) return (FALSE);
				}
				case TV_BOTTLE:
				case TV_FOOD:
				case TV_LITE:
		...
		case 3:
		{
			...
			switch (o_ptr->tval)
			{
				case TV_POTION:
				{
					/* Don't buy known booze or salt water */
					if (object_known_p(o_ptr) && ((o_ptr->sval == SV_POTION_BOOZE) ||
						 (o_ptr->sval == SV_POTION_SALT_WATER))) return (FALSE);
				}
				case TV_PRAYER_BOOK:
				case TV_SCROLL:
		...
		case 5:
		{
			...
			switch (o_ptr->tval)
			{
				...
				{
					/* Don't buy booze */
					if (o_ptr->sval == SV_POTION_BOOZE) return (FALSE);
				}
				case TV_MAGIC_BOOK:
				case TV_AMULET:
				case TV_RING:
...
static int home_carry(object_type *o_ptr)
{
	...
	for (slot = 0; slot < st_ptr->stock_num; slot++)
	{
		...
		if (object_similar(j_ptr, o_ptr))
		{
			...
			st_ptr->total_bulk += o_ptr->bulk * o_ptr->number;
...
/*
 * Messages for alchemical failure
 */

#define MAX_COMMENT_9A		4

static cptr comment_9a[MAX_COMMENT_9A] =
{
	"The shopkeeper swears loudly.",
	"You hear a hacking cough.",
	"The shopkeeper gags.",
	"You hear a hissing sound."
};

#define MAX_COMMENT_9B		3

static cptr comment_9b[MAX_COMMENT_9B] =
{
	"You hear a muffled curse.",
	"The alchemist empties the potion suddenly.",
	"The alchemist glares at the potion."
};


/*
 * Attempt to make a good object from trash.
 */
static int store_special(object_type *o_ptr)
{
	int i, j, dam, type, slot, num;
	...
	switch(o_ptr->tval)
	{
		...
		case TV_BOTTLE:
		{
			switch(store_num)
			{
				/* General store -- most bottles can hold oil */
				case 0:
				{
					if (rand_int(100) < 90)
					{
						/* The shopkeeper knows what it is */
						object_aware(o_ptr);
						...
						for (slot = 0; slot < st_ptr->stock_num; slot++)
						{
							...
							if (j_ptr->number != 99) continue;

							if ((j_ptr->tval == TV_FLASK) && (j_ptr->sval == 0))
							{
								msg_format("The shopkeeper smiles and puts the bottle%s in a chest.",
											  (plural) ? "s" : "");
								return (-1);
							}
						}
					...
					{
						msg_format("The shopkeeper looks at %s cracked bottle in disgust.",
									  (plural) ? "a" : "the");

						if (plural) o_ptr->number--;
						else return (-1);
					...
					object_prep(o_ptr, lookup_kind(TV_FLASK, 0));
					msg_format("The shopkeeper fills the bottle%s with oil.",
								  (plural) ? "s" : "");
				...
				/* The alchemist can make some potions */
				case 4:
				{
					/* Too many */
					if (o_ptr->number > 1)
					{
						msg_print("The alchemist smiles and puts the bottles in a chest.");
						return (-1);
					}

					i = 0;

					/* Occasional cracked bottle */
					if (rand_int(100) >= 90) i = rand_int(3) + 3;

					msg_print("The alchemist retreats to the back of the store.");

					/* Try to make a potion */
					for (; i < 5; i++)
					{
						type = 0;
						...
						if (p_ptr->poisoned)
						{
							/* Make a cure poison potion */
							type = SV_POTION_SLOW_POISON + rand_int(2);
						}

						if (!type)
						{
							/* Make a restore stat potion */
							for (j = 0; j < 6; j++)
							{
								if (p_ptr->stat_use[j] < p_ptr->stat_top[j])
								{
									/* Ignore some stats occasionally */
									if (type && rand_int(3)) continue;

									type = SV_POTION_RES_STR + j;
								}
							}
						}

						if (!type && (p_ptr->exp < p_ptr->max_exp))
						{
							/* Make a restore life potion */
							type = SV_POTION_RESTORE_EXP;
						}

						if (!type && (p_ptr->chp < p_ptr->mhp))
						{
							/* Make a healing potion */
							type = SV_POTION_CURE_LIGHT + rand_int(5);
						}

						if (!type && (p_ptr->csp < p_ptr->msp))
						{
							/* Make a restore mana potion */
							type = SV_POTION_RESTORE_MANA;
						}

						if (!type && (p_ptr->afraid))
						{
							/* Make a boldness potion */
							type = SV_POTION_BOLDNESS;
						}
						...
						for (slot = 0; slot < st_ptr->stock_num; slot++)
						{
							...
							if ((j_ptr->tval == TV_POTION) && (j_ptr->sval == type))
							{
								/* Choose another type */
								type = 0;
							}
						...
						if (!type || !rand_int(5))
						{
							type = rand_int(MAX_POTION_SV);

							/* Let the general store sell the alcohol */
							while (type == SV_POTION_BOOZE)
							{
								type = rand_int(MAX_POTION_SV);
							}
						}
						...
						if (p_ptr->confused)
						{
							/* Sell the drunk some beer */
							type = SV_POTION_BOOZE;

							/* Message */
							msg_print("The alchemist mutters in annoyance.");
						}

						object_prep(o_ptr, lookup_kind(TV_POTION, type));

						/* Must have a legal object */
						if (!o_ptr->k_idx)
						{
							/* Message */
							msg_print(comment_8[rand_int(MAX_COMMENT_8)]);
							continue;
						}
						...
						/* Make a rarity check */
						if (k_ptr->level > randint(MAX_POTION_DEPTH))
						{
							dam = damroll(3, 4);
							msg_print("Noxious fumes fill the air.");
							msg_print(comment_9a[rand_int(MAX_COMMENT_9A)]);

							/* Poison resistance */
							if (p_ptr->resist_pois || p_ptr->oppose_pois) continue;

							/* The alchemist has killed the player! */
							if (dam > p_ptr->chp)
							{
								/* Oops! */
								msg_print("Everything fades to black...");
								msg_print("You hit something hard with a thump.");
								msg_print("Somebody gasps.");
								msg_print("You hear apologetic mumbling.");

								leave_store = TRUE;
							}

							/* Injury */
							take_hit(dam, "poisonous fumes");

							/* Poisonous fumes */
							set_poisoned(p_ptr->poisoned + rand_int(10) + 10);

							/* Dead player */
							if (leave_store) return (-1);

							continue;
						}
						/* Reject useless potions */
						else if (object_value(o_ptr) <= 0)
						{
							/* Occasional message */
							j = rand_int(MAX_COMMENT_9B * 3);

							if (j < MAX_COMMENT_9B) msg_print(comment_9b[j]);
							else i++;
					...
					if (i >= 5)
					{
						msg_print("There is a shattering sound. The alchemist returns, cursing.");

						return (-1);
					}
					else
					{
						msg_print("The alchemist returns and puts a new potion on the shelf.");

						/* Charge extra */
						o_ptr->discount = ot_ptr->max_inflate;
					}
...
static int store_carry(object_type *o_ptr)
{
	...
	n = store_special(o_ptr);

	/* Failure */
	if (n < 0) return (-1);

	/* No more space available */
	if (st_ptr->total_bulk + store_bulk(o_ptr) * o_ptr->number > STORE_SIZE) return (-1);
   ...
   st_ptr->total_bulk += store_bulk(o_ptr) * o_ptr->number;
...
static void store_item_increase(int item, int num)
{
	...
	/* Increase the bulk */
	st_ptr->total_bulk += store_bulk(o_ptr) * num;
...
static bool store_create(void)
{
	...
	if (st_ptr->stock_num >= st_ptr->stock_size) return FALSE;
	...
	for (tries = 0; tries < 4; tries++)
	{
		...
		if (store_carry(i_ptr) != -1) res = TRUE;
 ...
 */
bool noneedtobargain(s32b minprice)
{
 ...
 */
static bool get_stock(int *cp, cptr pmt)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
	...
	while (!done)
	{
   	strcpy(tmp_val, pmt);
		...
		strcat(tmp_val, format("Your gold:%9ld", (long)p_ptr->au));

		which = show_store(st_ptr, ot_ptr, (store_num == STORE_HOME), tmp_val, &store_top);
		...
		switch (which)
		{
			...
			default:
			{
				...
				for (j = -1, k = i1; k <= i2; k++)
				{
					...
					if (j == A2I(which) + store_top) break;
				}

				if (j != A2I(which) + store_top)
				{
					bell();
					break;
				}
	...
	/* Forget the item_tester_char restriction */
	item_tester_char = '\0';
	...
	if (store_num != STORE_HOME)
	{
		...
		if (choice == 0)
		{
			...
			if (p_ptr->au >= price)
			{
				...
				/* Restore normal price if overpriced */
				if (i_ptr->discount > 100) i_ptr->discount = 0;
				...
				if (st_ptr->stock_num == 0)
				{
					...
					{
						/* Message */
						msg_print("The shopkeeper closes the store.");
...
static void store_sell(void)
{
	...
	if (store_num != STORE_HOME) i_ptr->note = 0;
	else if (st_ptr->total_bulk + i_ptr->bulk * i_ptr->number > STORE_SIZE / 2)
	{
	...
	{
		if (store_num == STORE_HOME) msg_print("Your home is full.");
	...
	while (!leave_store)
	{
		...
		if (!tmp_val[0])
		{
			...
			else
			{
				...
				/* Put the owner name and race */
				sprintf(tmp_val, "%s (%s)", owner_name, race_name);

				/* Show the max price in the store (above prices) */
				strcat(tmp_val, format("     %s (%ld)", store_name, (long)(ot_ptr->max_cost)));
			}
		}
		...
		/* Display the store */
		p_ptr->command_cmd = show_store(st_ptr, ot_ptr, (store_num == STORE_HOME), tmp_val, &store_top);
