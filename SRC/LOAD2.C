...
static bool older_all(byte x,byte y,byte z,byte extra)
{
  if (sf_major < x) return TRUE;
  if (sf_major > x) return FALSE;
  if (sf_minor < y) return TRUE;
...
static bool z_older_all(byte x,byte y,byte z,byte extra)
{
  if (z_major < x) return TRUE;
  if (z_major > x) return FALSE;
  if (z_minor < y) return TRUE;
  ...
  if (z_patch < z) return TRUE;
  if (z_patch > z) return FALSE;
  return (sf_extra < extra); /* so far no z_extra... */
}
	...
	if (z_older_all(2,1,0,1))
	  {
	    ...
	    p_ptr->prob_travel = 0;
	    p_ptr->precognition = 0;
	    p_ptr->mental_barrier = 0;
	    p_ptr->ts_anchor = 0;
	    p_ptr->intensify = 0;
	  }
	else
