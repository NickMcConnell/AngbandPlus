...
static bool do_cmd_tunnel_aux(int y, int x)
{
	...
	/* Titanium */
	if (is_hard_wall(cave_feat[y][x]))
	{
		msg_print("This seems to be permanent rock.");
	}

	/* Granite */
	else if (is_granite(cave_feat[y][x]))
	{
		/* Tunnel */
	...
	/* Quartz / Magma */
	else if (is_vein(cave_feat[y][x]) || is_treasure(cave_feat[y][x]))
	{
		bool okay = FALSE;
		bool gold = FALSE;
		bool hard = FALSE;
		...
		/* Found gold */
		if (is_treasure(cave_feat[y][x])) gold = TRUE;
...
void do_cmd_alter(void)
{
	...
	/* Bash jammed doors */
	else if (is_door(feat) && (feat >= FEAT_DOOR_HEAD + 0x08))
	{
		/* Tunnel */
		more = do_cmd_bash_aux(y, x);
	}
...
void do_cmd_fire(void)
{
	...
	/* Wind crossbows */
	if ((j_ptr->sval >= SV_LIGHT_XBOW) && (j_ptr->timeout == 0))
	{
		j_ptr->timeout = 1;
		msg_print("You wind your crossbow.");
	...
	for (cur_dis = 0; cur_dis <= tdis; )
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			if (test_hit_fire(chance - cur_dis, r_ptr->ac, m_ptr->ml))
			{
				...
				tdam = critical_shot(i_ptr->wt, i_ptr->to_h, tdam);
