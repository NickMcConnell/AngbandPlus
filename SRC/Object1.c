...
/* Alex: moved some object_desc_XXX macros to z-utils.h */
...
void object_desc(char *buf, size_t max, const object_type *o_ptr, int pref, int mode)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_AMULET:
		{
			...
			/* Alex: add the word "Amulet" for new artifacts */
			if (artifact_p(o_ptr) && (o_ptr->name1 < file_a_max) && aware) break;
		...
		{
			/* Hack -- Known artifacts */
			/* Alex: add the word "Ring" for new artifacts */
			if (artifact_p(o_ptr) && (o_ptr->name1 < file_a_max) && aware) break;
	...
	if (known)
	{
		...
		if (o_ptr->name1)
		{
			...
			object_desc_chr_macro(t, ' ');
			/* Alex: new artifacts use separate names string */
			if (o_ptr->name1 < file_a_max)
