...
static errr rd_extra(void)
{
	...
	/* D11-Hack: Load D11-Angand additional Data */
	rd_s16b(&p_ptr->recall_depth);
	rd_s16b(&p_ptr->training);
	rd_s16b(&p_ptr->abilities);
	for (i = 0; i < 7; i++) rd_s16b(&p_ptr->ab_turn_count[i]);
	for (i = 0; i < 10; i++) rd_byte(&p_ptr->quick_spells[i]);
...
/*
 * D11-Hack: Read the backpack
 */
static errr rd_backpack(void)
{
	int slot = 0;
	...
	while (1)
	{
		...
		/* Verify slot */
		if (n >= BACKPACK) return (-1);


		/* Warning -- backpack is full */
		else if (p_ptr->backpack_count == BACKPACK_TOTAL)
		{
			/* Oops */
			note("Too many items in the backpack!");

			/* Fail */
			return (-1);
		}

		/* Put Items to backpack*/
		else
		{
			/* Get a slot */
			n = slot++;
