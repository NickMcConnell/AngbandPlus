...
#ifdef PLUG_IN_PLAYER_CLASS

/*
 * Read a player-class record
 */
static void rd_pclass(void)
{
	int i;
	char title[80];
	player_class *p_class = &(p_ptr->pl_class);

	/* Type of class */
	rd_string(title, 79);
	p_class->title = string_make(title);

	/* Class stat modifier */
	for (i = 0; i < 6; i++)
	{
		rd_s16b(&p_class->c_adj[i]);
	}

	rd_s16b(&p_class->c_dis); /* class disarming */
	rd_s16b(&p_class->c_dev); /* class magic devices */
	rd_s16b(&p_class->c_sav); /* class saving throws */
	rd_s16b(&p_class->c_stl); /* class stealth */
	rd_s16b(&p_class->c_srh); /* class searching ability */
	rd_s16b(&p_class->c_fos); /* class searching frequency */
	rd_s16b(&p_class->c_thn); /* class to hit (normal) */
	rd_s16b(&p_class->c_thb); /* class to hit (bows) */

	rd_s16b(&p_class->x_dis); /* extra disarming */
	rd_s16b(&p_class->x_dev); /* extra magic devices */
	rd_s16b(&p_class->x_sav); /* extra saving throws */
	rd_s16b(&p_class->x_stl); /* extra stealth */
	rd_s16b(&p_class->x_srh); /* extra searching ability */
	rd_s16b(&p_class->x_fos); /* extra searching frequency */
	rd_s16b(&p_class->x_thn); /* extra to hit (normal) */
	rd_s16b(&p_class->x_thb); /* extra to hit (bows) */

	rd_s16b(&p_class->c_mhp); /* Class hit-dice adjustment */
	rd_s16b(&p_class->c_exp); /* Class experience factor */
}

#endif /* PLUG_IN_PLAYER_CLASS */
