...
bool make_attack_normal(int m_idx)
{
	...
	for (ap_cnt = 0; ap_cnt < MONSTER_BLOW_MAX; ap_cnt++)
	{
		...
		{
			/* Alex */
			update_abil_fail(a_dex, (power + (rlev * 3))/60);
			update_abil_fail(a_to_a, (power + (rlev * 3))/30);
			update_abil_fail(a_ac, (power + (rlev * 3))/100);
			if (!visible)
			{
				update_abil_fail(a_see_invis, 1 + rlev/5);
				if (!(r_ptr->flags2 & RF2_COLD_BLOOD)) update_abil_fail(a_infra, rlev/2);
				if (!(r_ptr->flags2 & RF2_EMPTY_MIND)) update_abil_fail(a_telepathy, rlev/20);
			}
			    ...
			    ((rand_int(100) + p_ptr->lev) > 50))
			{
				...
				update_act_fail(ACT_PROT_EVIL, rlev/5);
