...
static void spoil_mon_info(cptr fname)
{
	...
	for (n = 1; n < max_r_idx - 1; n++)
	{
		...
		else if (flags3 & (RF3_DUNADAN)) spoil_out (" Dunadan");
		...
		{
			spoil_out(wd_che[msex]);
			spoil_out(" is of friendly disposition.  ");
		}

		if (flags7 & (RF7_UNFAIR))
		{
			spoil_out(wd_che[msex]);
			spoil_out(" is rather unfair.  ");
		}
