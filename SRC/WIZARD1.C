...
/*
 * Describe the kind.  Utility function used only by spoil_obj_desc().
 */
static void kind_info(char *buf, char *dam, char *wgt, int *lev, s32b *val, int k)
{
        ...
        /* Weight */
        if (metric)
        sprintf(wgt, "%3d.%d", q_ptr->weight / 20, q_ptr->weight % 10);
        else
...
/*
 * Create a spoiler file for items.  Utility function used only
 * by do_cmd_spoilers().
 */
...
static grouper group_artifact[] =
{
        ...
        { TV_DAGGER,            "Daggers" },
        { TV_AXE,               "Axes"},
        { TV_TWO_HANDED,        "Two-handed Weapons"},
        { TV_BOW,               "Bows" },
...
static const flag_desc misc_flags3_desc[] =
{
        ...
        { TR3_TELEPORT,           "Teleports" },
        { TR3_DRAIN_EXP,          "Drains experience" },
        { TR3_AGGRAVATE,          "Aggravates" },
        { TR3_STAT_CURSE,         "Halts stats" },
        { TR3_BLESSED,            "Blessed Blade" },
        { TR3_GOLD_CURSE,         "Eats gold" },
        { TR3_DROP_SELF_CURSE,    "Drops self" },
        { TR3_VANISH_CURSE,       "Unstable" },
        { TR3_TL_CURSE,           "Teleports level" },
        { TR3_YELL_CURSE,         "Brags aloud" },
        { TR3_CLONE_CURSE,        "Clones monsters" },
};
...
{
        const u64b all_stats = (TR1_STR | TR1_INT | TR1_WIS |
        ...
        /* And now the "rest" */
        affects_list = spoiler_flag_aux(f1, pval_flags1_desc, affects_list,
...
{
        const u64b all_sustains = (TR2_SUST_STR | TR2_SUST_INT | TR2_SUST_WIS |
...
static void print_header(void)
{
        ...
        /* Print buffer and a line of dashes */
        fprintf(fff, "%s\n", buf);
        buf_len = strlen(buf);
        while (buf_len-- > 0)
                fputc('-', fff);
...
static bool make_fake_artifact(object_type *o_ptr, int name1)
{
	...
	for (i = 0; i < 7; i++)
	{
	    o_ptr->to_stat[i] = a_ptr->to_stat[i];
	}
...
static void spoil_artifact(cptr fname)
{
        ...
        for (i = 0; group_artifact[i].tval; i++)
        {
                ...
                if (group_artifact[i].name)
                {
                        ...
                        str_len = strlen(group_artifact[i].name);
                        while (str_len-- > 0)
                                fputc('-', fff); /* line of dashes */
                        fprintf(fff, "\n\n");
                }
 ...
 */
static void spoiler_outstr(cptr str)
{
        cptr r;
...
{
        char buf[1024];
        int msex, vn, i, j, k, n, str_len;
        bool breath, magic, sin;
        cptr p, q;
        cptr vp[64];
        u32b flags1, flags2, flags3;
        u64b flags4, flags5, flags6; 
        u32b flags7, flags8, flags9;
        ...
        for (n = 1; n < max_r_idx - 1; n++)
        {
                ...
                /* invalid record if name == null */
                if (*(r_name + r_ptr->name) == 0)
                        continue;
                ...
                /* Color */
                spoiler_outstr(attr_to_text(r_ptr->d_attr));
                ...
                /* Describe */
                spoiler_outstr(r_text + r_ptr->text);
                spoiler_outstr("\nThis");


                if (flags2 & RF2_ELDRITCH_HORROR) spoiler_outstr (" sanity-blasting");
                if (flags3 & RF3_ANIMAL) spoiler_outstr(" natural");

                if (!(flags2 & (RF2_CHAOTIC) || flags2 & (RF2_LAWFUL) ||
                      flags3 & (RF3_EVIL) || flags3 & (RF3_GOOD)))
                spoiler_outstr(" true neutral");
                else
                {
		if (flags2 & (RF2_CHAOTIC))      spoiler_outstr(" chaotic");
		else if (flags2 & (RF2_LAWFUL))  spoiler_outstr(" lawful");
                else spoiler_outstr(" neutral");
		if (flags3 & (RF3_EVIL))         spoiler_outstr(" evil");
		else if (flags3 & (RF3_GOOD))    spoiler_outstr(" good");
                else spoiler_outstr(" neutral");
                }

                if (flags3 & RF3_UNDEAD) spoiler_outstr(" undead");

                if (flags3 & RF3_DRAGON) spoiler_outstr(" dragon");
                else if (flags3 & RF3_DEMON) spoiler_outstr(" demon");
                else if (flags3 & RF3_GIANT) spoiler_outstr(" giant");
                else if (flags3 & RF3_TROLL) spoiler_outstr(" troll");
                else if (flags3 & RF3_ORC) spoiler_outstr(" orc");
                else if (flags3 & RF3_DUNADAN) spoiler_outstr (" Dunadan");
                else spoiler_outstr(" creature");

                if (flags2 & RF2_DOOM) spoiler_outstr(" of Id");

                spoiler_outstr(" moves");

                if ((flags1 & RF1_RAND_50) && (flags1 & RF1_RAND_25))
                        spoiler_outstr(" extremely erratically");
                else
                        if (flags1 & (RF1_RAND_50))
                                spoiler_outstr(" somewhat erratically");
                        else
                                if (flags1 & (RF1_RAND_25))
                                        spoiler_outstr(" a bit erratically");
                                else
                                        spoiler_outstr(" normally");

                if (flags1 & RF1_NEVER_MOVE)
                        spoiler_outstr(", but does not deign to chase intruders");
                ...
                {
                        sprintf(buf, "%s is always created sluggish.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }

                if (flags2 & RF2_AURA_FIRE)
                {
                        sprintf(buf, "%s is surrounded in flames.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }

                if (flags2 & RF2_AURA_ELEC)
                {
                        sprintf(buf, "%s is surrounded in electricity.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }

                if (flags3 & RF3_AURA_COLD)
                {
                        sprintf(buf, "%s is surrounded by ice.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }

                if (flags3 & RF2_AURA_FIRE)
                {
                        sprintf(buf, "%s is cloaked in fear.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }

                if (flags2 & RF2_REFLECTING)
                {
                        sprintf(buf, "%s reflects bolt spells.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }
                ...
                if (flags1 & RF1_ESCORT)
                {
                        ...
                        if (flags1 & (RF1_ESCORTS))
                                spoiler_outstr("escorts.\n");
                        else
                                spoiler_outstr("an escort.\n");
                }

                if ((flags1 & RF1_FRIEND) || (flags1 & RF1_FRIENDS))
                {
                        sprintf(buf, "%s usually appears in groups.\n", wd_che[msex]);
                        spoiler_outstr(buf);
                }
		...
		if (flags4 & RF4_CH_SHAR) vp[vn++] = "fire chainshards";
                ...
                if (vn)
                {
                        ...
                        if (flags2 & RF2_POWERFUL) spoiler_outstr(" powerfully");
                }
                ...
                if (vn)
                {
                        ...
                        spoiler_outstr(" magical, casting spells");
                        if (flags2 & RF2_SMART) spoiler_outstr(" intelligently");

                        for (i = 0; i < vn; i++)
                        {
                                if (!i)
                                        spoiler_outstr(" which ");
                                else
                                        if (i < vn-1)
                                                spoiler_outstr(", ");
                                        else
                ...
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" lives in water.\n");
                }

                if (flags7 & RF7_CAN_SWIM)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" can swim.\n");
                }

                if (flags7 & RF7_CAN_FLY)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" can fly.\n");
                }

                if (flags7 & RF7_FRIENDLY)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is of friendly disposition.\n");
                }

                if (flags7 & RF7_UNFAIR)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is rather unfair.\n");
                }

                if (flags2 & RF2_PLAYER_GHOST)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is a player ghost template.\n");
                }

                if (flags2 & RF2_INVISIBLE)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is invisible.\n");
                }
                if (flags2 & RF2_COLD_BLOOD)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is cold blooded.\n");
                }
                if (flags2 & RF2_EMPTY_MIND)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is not detected by telepathy.\n");
                }
                if (flags2 & RF2_WEIRD_MIND)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" is rarely detected by telepathy.\n");
                }
                if (flags2 & RF2_MULTIPLY)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" breeds explosively.\n");
                }
                if (flags2 & RF2_REGENERATE)
                {
                        spoiler_outstr(wd_che[msex]);
                        spoiler_outstr(" regenerates quickly.\n");
                }
                ...
                if (flags9 & RF9_HURT_ELEC) vp[vn++] = "lightning";
                if (flags9 & RF9_HURT_ACID) vp[vn++] = "acid";
                if (flags9 & RF9_HURT_POIS) vp[vn++] = "poison";
                ...
                if (flags7 & RF7_RES_BASH) vp[vn++] = "blunt weapons";
                if (flags7 & RF7_RES_SLASH) vp[vn++] = "edged weapons";
                ...
                if (flags9 & RF9_RES_STON) vp[vn++] = "stoning";
                ...
                spoiler_outstr(wd_che[msex]);
                if (r_ptr->sleep > 200)     spoiler_outstr(" prefers to ignore");
                else if (r_ptr->sleep > 95) spoiler_outstr(" pays very little attention to");
                else if (r_ptr->sleep > 75) spoiler_outstr(" pays little attention to");
                else if (r_ptr->sleep > 45) spoiler_outstr(" tends to overlook");
                else if (r_ptr->sleep > 25) spoiler_outstr(" takes quite a while to see");
                else if (r_ptr->sleep > 10) spoiler_outstr(" takes a while to see");
                else if (r_ptr->sleep > 5)  spoiler_outstr(" is fairly observant of");
                else if (r_ptr->sleep > 3)  spoiler_outstr(" is observant of");
                else if (r_ptr->sleep > 1)  spoiler_outstr(" is very observant of");
                else if (r_ptr->sleep > 0)  spoiler_outstr(" is vigilant for");
                else spoiler_outstr(" is ever vigilant for");
                ...
                if (i)
                {
                        ...
                        {
                                if (sin) spoiler_outstr("n");
                                spoiler_outstr(" great object");
                        }
                        else if (flags1 & RF1_DROP_GOOD)
                                spoiler_outstr(" good object");
                        else if (flags1 & RF1_DROP_USEFUL)
                                spoiler_outstr(" useful object");
                        else if (flags1 & RF1_ONLY_ITEM)
                                spoiler_outstr(" object");
                        else if (flags1 & RF1_ONLY_GOLD)
                                spoiler_outstr(" treasure");
                        else
                        {
                                if (sin)
                                        spoiler_outstr("n");
                                ...
                                if (i > 1)
                                        spoiler_outstr("s");
                                spoiler_outstr(" or treasure");
                        }
                        if (i > 1)
                                spoiler_outstr("s");

                        if (flags1 & RF1_DROP_CHOSEN)
                                spoiler_outstr(", in addition to chosen objects");
                ...
                if (k)
                        ...
                        {
                                sprintf(buf, "%s has no physical attacks.\n", wd_che[msex]);
                                spoiler_outstr(buf);
                        }
                /*
                 *
                 * Effects of eating a corpse
                 *
                 */

                if (r_ptr->flags9 & RF9_DROP_CORPSE)
                {
                spoiler_outstr("Eating ");
                ...
                if (flags1 & RF1_FEMALE)
                        ...
                        if (flags1 & RF1_MALE)
                                ...
                                spoiler_outstr("its");
                spoiler_outstr(" corpse ");

                vn = 0; /* We're reusing the vp[vn] vars from above */


                if (r_ptr->flags9 & RF9_EAT_BLIND)
                        vp[vn++] =  "cause blindness";
                if (r_ptr->flags9 & RF9_EAT_CONF)
                        vp[vn++] =  "cause confusion";
                if (r_ptr->flags9 & RF9_EAT_VOMIT)
                        vp[vn++] =  "cause severe sickness";
                if (r_ptr->flags9 & RF9_EAT_TELEPORT)
                        vp[vn++] =  "cause long-range teleportation";
                if (r_ptr->flags9 & RF9_EAT_BERSERKER)
                        vp[vn++] =  "cause berserk rage";
                if (r_ptr->flags9 & RF9_EAT_MANA)
                        vp[vn++] =  "restore your mana";
                if (r_ptr->flags9 & RF9_EAT_INFLATE)
                        vp[vn++] =  "inflate your entrails";
                if (r_ptr->flags9 & RF9_EAT_FORGET)
                        vp[vn++] =  "cause severe forgetfulness";
                if (r_ptr->flags9 & RF9_EAT_BLINK)
                        vp[vn++] =  "cause short-range teleportation";
                if (r_ptr->flags9 & RF9_EAT_SLEEP)
                        vp[vn++] =  "paralyze you";
                if (r_ptr->flags9 & RF9_EAT_NEXUS)
                        vp[vn++] =  "unleash the chaos of nexus";
                if (r_ptr->flags9 & RF9_EAT_SPEED)
                        vp[vn++] =  "increase your speed";
                if (r_ptr->flags9 & RF9_EAT_FIERY)
                        vp[vn++] =  "burn your throat";
                if (r_ptr->flags9 & RF9_EAT_ACIDIC)
                        vp[vn++] =  "dissolve your stomach";
                if (r_ptr->flags9 & RF9_EAT_HEAL_FEAR)
                        vp[vn++] =  "negate the effects of fear";
                if (r_ptr->flags9 & RF9_EAT_HEAL_STUN)
                        vp[vn++] =  "negate the effects of concussion";
                if (r_ptr->flags9 & RF9_EAT_HEAL_CONF)
                        vp[vn++] =  "restore clear mind";
                if (r_ptr->flags9 & RF9_EAT_GIVE_ESP)
                        vp[vn++] =  "increase mental awareness";
                if (r_ptr->flags9 & RF9_EAT_ACID_RES)
                        vp[vn++] =  "protect against acid";
                if (r_ptr->flags9 & RF9_EAT_ELEC_RES)
                        vp[vn++] =  "protect against lightning";
                if (r_ptr->flags9 & RF9_EAT_FIRE_RES)
                        vp[vn++] =  "protect against fire";
                if (r_ptr->flags9 & RF9_EAT_COLD_RES)
                        vp[vn++] =  "protect against cold";
                if (r_ptr->flags9 & RF9_EAT_POIS_RES)
                        vp[vn++] =  "protect against poison";
                if (r_ptr->flags9 & RF9_EAT_DRAIN_EXP)
                        vp[vn++] =  "debilitate you";
                if (r_ptr->flags9 & RF9_EAT_INSANITY)
                        vp[vn++] =  "endanger your mental stability";
                if (r_ptr->flags9 & RF9_EAT_POISONOUS)
                        vp[vn++] =  "cause poisoning";
                if (r_ptr->flags9 & RF9_EAT_SUPER_POISONOUS)
                        vp[vn++] =  "cause severe poisoning";
                if (r_ptr->flags9 & RF9_EAT_DRAIN_MANA)
                        vp[vn++] =  "drain your magical reserves";
                if (r_ptr->flags9 & RF9_EAT_GIVE_STR)
                        vp[vn++] =  "increase your strength";
                if (r_ptr->flags9 & RF9_EAT_GIVE_DEX)
                        vp[vn++] =  "increase your dexterity";
                if (r_ptr->flags9 & RF9_EAT_GIVE_WIS)
                        vp[vn++] =  "increase your wisdom";
                if (r_ptr->flags9 & RF9_EAT_GIVE_INT)
                        vp[vn++] =  "increase your intelligence";
                if (r_ptr->flags9 & RF9_EAT_GIVE_CON)
                        vp[vn++] =  "increase your constitution";
                if (r_ptr->flags9 & RF9_EAT_GIVE_CHR)
                        vp[vn++] =  "increase your charisma";
                if (r_ptr->flags9 & RF9_EAT_LOSE_STR)
                        vp[vn++] =  "decrease your strength";
                if (r_ptr->flags9 & RF9_EAT_LOSE_DEX)
                        vp[vn++] =  "decrease your dexterity";
                if (r_ptr->flags9 & RF9_EAT_LOSE_WIS)
                        vp[vn++] =  "decrease your wisdom";
                if (r_ptr->flags9 & RF9_EAT_LOSE_INT)
                        vp[vn++] =  "decrease your intelligence";
                if (r_ptr->flags9 & RF9_EAT_LOSE_CON)
                        vp[vn++] =  "decrease your constitution";
                if (r_ptr->flags9 & RF9_EAT_LOSE_CHR)
                        vp[vn++] =  "decrease your charisma";

                if (vn == 0)
                        spoiler_outstr("is safe; no good or bad effects.");
                else
                {
                        spoiler_outstr("can ");
                        /* cycle thru all but the last one */
                        for (i = 0; i < vn - 1; i++)
                        {
                                spoiler_outstr(vp[i]);
                                if (vn > 2)
                        ...
                        if (vn == 2)
                                ...
                                if (vn > 2)
                                        spoiler_outstr("and ");
                        spoiler_outstr(vp[vn - 1]); /* the last one */
                        spoiler_outstr(".");
                }
                }
