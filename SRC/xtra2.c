...
bool set_fast(int v)
{
	...
	if (v)
	{
		...
		else if (p_ptr->fast + v > 100) notice = TRUE;
	}
...
bool set_slow(int v)
{
	...
	if (v)
	{
		...
		else if (p_ptr->slow + v > 50) notice = TRUE;
	}
...
bool set_food(int v)
{
	...
	/* Hack -- Force good values */
	v = (v >= PY_FOOD_DEATH) ? (PY_FOOD_DEATH + 1000) :
		 (v < 0) ? (PY_FOOD_DEATH + 1000) : v;
	...
	/* Gorged */
	else if (p_ptr->food < PY_FOOD_PAIN)
	{
		old_aux = 5;
	}

	/* Bloated */
	else if (p_ptr->food < PY_FOOD_DEATH)
	{
		old_aux = 6;
	}
	...
	/* Gorged */
	else if (v < PY_FOOD_PAIN)
	{
		new_aux = 5;
	}

	/* Bloated */
	else if (v < PY_FOOD_DEATH)
	{
		new_aux = 6;
	}
	...
	if (new_aux > old_aux)
	{
		...
		switch (new_aux)
		{
         ...
         case 6:
         {
				msg_print("You are very bloated!");
				break;
			}
         ...
         case 7:
         {
				msg_print("Your stomach ruptures!!");
				break;
			}
		}
...
/*
 * Determine what will be hit be a flying object.
 */
static void lof(int y1, int x1, int *y2, int *x2)
{
	int dist, y, x;
 ...
 */
static void spatter(int *yp, int *xp, int y, int x, int d)
{
	int nx, ny;
 ...
 * Handle injuries.
 */
void bleed(int m_idx, int dam)
{
	int x, y, tx, ty, num, dist, feat;
	...
	if (m_idx < 0)
	{
		...
		/* Correct overly large values */
		if (num > p_ptr->mhp / 10)
		{
			num = (p_ptr->mhp < 10) ? 1 : p_ptr->mhp / 10;
		}

		/* Trolls have black blood */
		if (p_ptr->prace == RACE_HALF_TROLL) black = TRUE;
	}
	...
	else if (m_idx > 0)
	{
		...
		/* Correct overly large values */
		if (num > m_ptr->maxhp / 10)
		{
			num = (m_ptr->maxhp < 10) ? 1 : m_ptr->maxhp / 10;
		}

		/* Some monsters don't bleed */
		if (strchr("$,acglmsvwzEIKLW", r_ptr->d_char)) return;

		/* Some monsters bleed green */
		if (strchr("ijkGQ", r_ptr->d_char)) green = TRUE;

		/* Some monsters bleed black */
		if (strchr("dnuDOSTU", r_ptr->d_char)) black = TRUE;
	}
	...
	for (; num > 0; num--)
	{
		...
		{
			spatter(&ty, &tx, y, x, dist);

			feat = cave_feat[ty][tx];

			/* Enlarge the pool */
			if (is_bloody(feat) && rand_int(3)) continue;

			/* Ignore blood in water */
			if (feat == FEAT_WATER) break;

			/* Normal walls */
			if (is_granite(feat))
			{
				if (green) cave_set_feat(ty, tx, FEAT_WALL_G);
				else if (black) cave_set_feat(ty, tx, FEAT_WALL_D);
				else cave_set_feat(ty, tx, FEAT_WALL_R);

				break;
			}

			/* Floor */
			else if (is_floor(feat) && (feat != FEAT_INVIS))
			{
				if (green) cave_set_feat(ty, tx, FEAT_FLOOR_G);
				else if (black) cave_set_feat(ty, tx, FEAT_FLOOR_D);
				else cave_set_feat(ty, tx, FEAT_FLOOR_R);
...
/*
 * Check for completion of a quest
 * Make a staircase and drop a reward if possible
 */
void check_quest(monster_race *r_ptr, monster_type *m_ptr, bool drop)
{
	int i, total = 0, nx, ny;

	/* Advance in the quest */
	if (p_ptr->quest_max) p_ptr->quest_cur++;

	if (p_ptr->quest_cur < p_ptr->quest_max) return;

	/* Hack -- Mark quests as complete */
	for (i = 0; !p_ptr->quest_max && (i < MAX_Q_IDX); i++)
	{
	...
	/* Need some stairs */
	if (total || p_ptr->quest_max)
	{
		ny = m_ptr->fy;
		nx = m_ptr->fx;

		/* Stagger around */
		while (cave_perma_bold(ny, nx))
		{
			/* Pick a location */
			scatter(&ny, &nx, m_ptr->fy, m_ptr->fx, 2, 0);
		}

		if (p_ptr->depth < MAX_DEPTH - 1)
		{
			/* Create stairs down */
			cave_set_feat(ny, nx, FEAT_MORE);
		}
		else
		{
			/* Create stairs down */
			cave_set_feat(ny, nx, FEAT_LESS);
		}

		/* Explain the staircase */
		msg_print("A magical staircase appears...");

		/* Drop a reward if possible */
		if (drop && p_ptr->quest_max)
		{
			object_type object_type_body;
			object_type *i_ptr;

			int num = randint(p_ptr->depth / 20);

			object_level = (r_ptr->level + p_ptr->depth) / 2;

			/* Create some good items */
			for (i = 0; i < num; i++)
			{
				/* Get local object */
	...
	else
	{
		...
		/* Congratulations */
		msg_print("Morgoth has been defeated.");
		msg_print("You may retire (commit suicide) when you are ready.");
	}
}
 ...
 *
 * Note that monsters now have basic inventories, and when a monster dies,
 * it drops all of its objects, which may disappear in very crowded rooms.
 * Although they shouldn't now.
 */
void monster_death(int m_idx)
{
	...
	for (this_o_idx = m_ptr->hold_o_idx; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		if (o_ptr->tval == TV_GOLD) dump_gold++;
		else dump_item++;
		...
		/* Take note if the item was dropped by a townsperson */
		if (r_ptr->level == 0 && !(r_ptr->flags3 & RF3_EVIL))
		{
			i_ptr->discount = 95;
		}
	...
	{
		/* Morgoth has been defeated */
		if (strstr(m_name, "Morgoth"))
		{
			/* Get local object */
			i_ptr = &object_type_body;
		...
		if (strstr(m_name, "Dark Lord"))
		{
			/* Get local object */
			i_ptr = &object_type_body;

			/* Mega-Hack -- Prepare to make the One Ring */
			object_prep(i_ptr, lookup_kind(TV_RING, SV_RING_POWER));

			/* Mega-Hack -- Mark this item as the One Ring */
			i_ptr->name1 = ART_POWER;
	...
	/* Drop monster corpse */
	if (!force_coin) place_corpse(m_ptr);
	...
	if (r_ptr->flags1 & (RF1_QUESTOR)) check_quest(r_ptr, m_ptr, TRUE);
}
 ...
 *
 * Monster experience decreases over time by using
 * "(m_exp * m_lev * (m_lev)) / (p_lev * (m_lev + n_killed))" instead
 * of simply "(m_exp * m_lev) / (p_lev)", to make the first monster
 * worth more than subsequent monsters.  This also induces changes
 * in the monster recall code.
 */
bool mon_take_hit(int m_idx, int dam, bool *fear, cptr note)
{
	...
	if (m_ptr->hp < 0)
	{
		...
		/* Hack - Phoenices may be fully healed at death */
		if ((rand_int(100) < 50) && (strstr(m_name, "phoenix") || strstr(m_name, "Phoenix")))
		{
			m_ptr->hp = m_ptr->maxhp;
			msg_format("%^s returns to life!", m_name);
			return(FALSE);
		}
		...
		/* Hack - Phoenices do not die */
		if (strstr(m_name, "phoenix") || strstr(m_name, "Phoenix"));
		/* When the player kills any other Unique, it stays dead */
		else if (r_ptr->flags1 & (RF1_UNIQUE)) r_ptr->max_num--;
		...
		/* Permanently destroy ghosts */
		if (r_idx >= RACE_MAX_NORMAL)
		{
			kill_ghost(r_idx - RACE_MAX_NORMAL);
		}
	...
	if (!m_ptr->monfear && (!(r_ptr->flags3 & (RF3_NO_FEAR)) ||
		 ((r_ptr->flags2 & (RF2_HI_REGEN)) && !p_ptr->aggravate)))
	{
		int percentage;
			 ...
			 ((dam >= m_ptr->hp) && (rand_int(100) < 80)))
		{
			...
			/* *Regenerators* run until safe */
			if (r_ptr->flags2 & (RF2_HI_REGEN)) m_ptr->monfear = 255;
		}
	}
...
static bool target_set_accept(int y, int x)
{
	...
	if (cave_info[y][x] & (CAVE_MARK))
	{
		...
		/* Notice shops */
		if (is_shop(feat)) return (TRUE);

		/* Notice traps */
		if (is_trap(feat)) return (TRUE);

		/* Notice doors */
		if (is_door(feat)) return (TRUE);
	...
	while (1)
	{
		...
		for (this_o_idx = cave_o_idx[y][x]; this_o_idx; this_o_idx = next_o_idx)
		{
			...
			if (o_ptr->marked)
			{
				...
				s32b this_size = (o_ptr->bulk + o_ptr->wt) * o_ptr->number / 2;

				/* Obscured by other objects */
				if (this_size * 3 / 4 < max_size) continue;
				/* Visible (at least partially) - may obscure others */
				else max_size = this_size;
		...
		if (boring || (!is_floor(feat)))
		{
			...
			/* Hack -- special information for water */
			if (is_liquid(feat))
			{
				if (cave_m_idx[y][x])
				{
			...
			if (d)
			{
				...
				while (flag && (i < 0))
				{
					...
					else
					{
						...
						{
							if (change_panel(d)) target_set_prepare(mode);
						}
