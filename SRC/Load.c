...
/* Alex */
static u32b new_art_version;
/* Alex */
static u32b patches_flags;
...
static errr rd_item(object_type *o_ptr)
{
	...
	/* Alex */
	if (patches_flags & PATCH_NEW_ART)
		rd_s32b(&o_ptr->name1);
	else
	{
		byte tmp;
...
static errr rd_extra(void)
{
	...
	/* Future use */
	/* Alex: it was 40 bytes */
	strip_bytes(32);

	/* Alex: all patches flags */
	rd_u32b(&patches_flags);
	/* Alex: new artifact version */
	rd_u32b(&new_art_version);
...
/* Alex: read one artifact.
 * If ignore, read common art to local variable (i.e. ignore data).
 * Note that new artifacts always actually read.
 */
static void rd_art(int a_idx, bool ignore){
	artifact_type *a_ptr;
	artifact_type tmp_art;

	if (!ignore || a_idx >= file_a_max)
		a_ptr = &a_info[a_idx];
	else
		a_ptr = &tmp_art;
	...
	/* Alex: read name and text for new artifacts */
	if (a_idx >= file_a_max)
	{
		rd_u32b(&a_ptr->name);
		rd_u32b(&a_ptr->text);
...
/*
 * Alex: Read the new artifacts
 */
static errr rd_new_arts(void)
{
	int i;
	u16b artifact_count;

	/* Alex: if new artifacts then always read all artifacts */
	if (patches_flags & PATCH_NEW_ART)
	{
		...
		{
			note(format("Too many (%u) new artifacts!", artifact_count));
			return (-1);
		}
...
/*
 * Alex: Read abilities/activation fail probabilities
 */
static errr rd_fails(void)
{
	if (patches_flags & PATCH_NEW_ART && new_art_version >= 2)
	{
		u16b a;
		u16b max;
		...
		if (max != a_last)
		{
			note(format("Invalid number (%u) of artifact abilities!", max));
			return (-1);
		}
		for (a = 0; a < a_last; a++)
			rd_u16b(&abil_fail[a]);
		...
		if (max != ACT_MAX)
		{
			note(format("Invalid number (%u) of artifact activations!", max));
			return (-1);
		}
		for (a = 0; a < ACT_MAX; a++)
			rd_u16b(&act_fail[a]);
...
static errr rd_savefile_new_aux(void)
{
	...
	/* Alex: New Art patch: check for possible extra artifacts */
	if (tmp16u > z_info->a_max)
	{
                if (!realloc_art(tmp16u - z_info->a_max))
		{
			note("Not enough memory for new artifacts!");
                        return (-1);
		}
	}
	...
	/* Alex: New Art patch writes/reads full artifact info */
	if (patches_flags & PATCH_NEW_ART)
	{
		if (rd_new_arts()) return (-1);
		if (arg_fiddle) note("Loaded New Artifacts");
	}
	/* Alex: read random artifacts only if no New Art patch*/
	else if (adult_rand_artifacts)
	{
	...
	/* Alex */
	rd_fails();
	if (arg_fiddle) note("Loaded Fails");
