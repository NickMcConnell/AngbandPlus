...
#define MAX_SHROOM     24       /* Used with mushrooms (min 20) */
...
static cptr ring_adj[MAX_ROCKS] =
{
	...
	"Engagement", "Adamantite", "Slate", "Shale"
};
...
static byte ring_col[MAX_ROCKS] =
{
	...
	TERM_YELLOW, TERM_L_GREEN, TERM_SLATE, TERM_L_DARK
};
...
static byte amulet_col[MAX_AMULETS] =
{
	...
	TERM_L_UMBER, TERM_L_GREEN, TERM_RED
};
...
static cptr staff_adj[MAX_WOODS] =
{
	...
	"Golden", "Ashen", "Gnarled", "Ivory", "Willow", "Coal"
};
...
static byte staff_col[MAX_WOODS] =
{
	...
	TERM_YELLOW, TERM_SLATE, TERM_UMBER, TERM_WHITE, TERM_SLATE, TERM_L_DARK
};
...
static byte food_col[MAX_SHROOM] =
{
	...
	TERM_L_DARK, TERM_YELLOW, TERM_L_WHITE, TERM_RED
};
 ...
 * (water, apple juice, slime mold juice, booze)
 */

static cptr potion_adj[MAX_COLORS] =
{
	"Clear", "Light Brown", "Icky Green", "Amber",
	...
	"Green Speckled", "Grey", "Grey Speckled", "Frothy", "Hazy", "Indigo",
...
{
	TERM_WHITE, TERM_L_UMBER, TERM_GREEN, TERM_L_UMBER,
	...
	TERM_GREEN, TERM_SLATE, TERM_SLATE, TERM_L_WHITE, TERM_L_WHITE, TERM_VIOLET,
 ...
 *
 * Skeletons of Uniques, although considered artifacts, already have a name
 * and thus do not append another name.
 *
...
void object_desc(char *buf, object_type *o_ptr, int pref, int mode)
{
	...
	if (p_ptr->image)
	{
      mode = 0;
		...
		/* Prevent corpses of player */
		o_ptr->r_idx = randint(RACE_MAX_NORMAL - 1);
	}
	...
	switch (o_ptr->tval)
	{
		case TV_SKELETON:
		{
			...
			if (o_ptr->sval != SV_SKELETON && o_ptr->sval != SV_CORPSE &&
				 o_ptr->sval != SV_SKULL && o_ptr->sval != SV_HEAD)
			{
				break;
			}

			/* What type of creature were we? */
			...
			if((r_ptr->flags1 & RF1_UNIQUE) || (o_ptr->r_idx == 0))
			{
				/* "The skull of Farmer Maggot" */
				t = object_desc_str(t, basenm);
				t = object_desc_str(t, " of ");
				t = object_desc_str(t, modstr);
			}
			else if(k_ptr->sval == SV_HEAD)
			{
				/* "A Kobold's severed head" */
				t = object_desc_str(t, modstr);
				t = object_desc_str(t, "'s ");
				t = object_desc_str(t, basenm);
			}
         else
         {
	...
	if (basenm[0] == '&')
	{
		 ...
		 * This makes Unique Pounds of raw meat look silly.
		 */
		else if (known && artifact_p(o_ptr) &&
				 !(o_ptr->tval == TV_SKELETON &&
					o_ptr->sval == SV_RAW_MEAT))
		{
	...
	*t = '\0';
	...
	if (known && (o_ptr->tval != TV_SKELETON))
	{
	...
	/* Indicate firing status of crossbows */
	if ((o_ptr->tval == TV_BOW) && (o_ptr->sval >= SV_LIGHT_XBOW))
	{
		if (o_ptr->timeout)
		{
			t = object_desc_str(t, " (wound)");
		}
	}
	/* Indicate "charging" artifacts XXX XXX XXX */
	else if (known && (!decays(o_ptr)) && o_ptr->timeout)
	{
		/* Hack -- Dump " (charging)" if relevant */
	...
	/* Note murders */
	else if (o_ptr->discount == 95)
	{
		strcpy(tmp_val, "murderous");
	}

	/* Note the discount, if any */
	else if (o_ptr->discount)
	{
		if (o_ptr->discount <= 100)
		{
		...
		{
			strcpy(tmp_val, "special");
		}
	}

	/* Note rotten meat */
	else if (decays(o_ptr))
	{
		if (o_ptr->pval < o_ptr->wt)
		{
			strcat(tmp_val, "spoiled");
		}
		else if (o_ptr->timeout >= o_ptr->wt)
		{
			strcat(tmp_val, "cured");
		}
	}
...
bool identify_fully_aux(object_type *o_ptr)
{
	...
	if (f3 & (TR3_HEAVY))
	{
		info[i++] = "It induces rapid falling.";
	}
...
/*
 * How does a given object react to being destroyed?
 */
bool hates_dam(object_type *o_ptr, int *typ, int *dam, int *rad)
{
	object_kind *k_ptr = &k_info[o_ptr->k_idx];
	...
	switch (o_ptr->tval)
	{
		...
		case TV_POTION:
		{
			*typ = GF_SHARD;
			...
			/* High level potions are dangerous */
			*rad = k_ptr->level / 20;
			*dam = damroll(k_ptr->level / 5, k_ptr->level / 5);
		...
		{
			*typ = GF_MANA;
			*dam = o_ptr->pval * 7 + k_ptr->level * 3;
...
s16b label_to_equip(int c)
{
	...
	/* Convert */
	i = (islower(c) ? A2I(c) + INVEN_WIELD : -1);
...
cptr mention_use(int i)
{
	...
	if (i == INVEN_WIELD)
	{
		object_type *o_ptr;
		o_ptr = &inventory[i];
		if (adj_str_hold[p_ptr->stat_ind[A_STR]] < o_ptr->wt / 10)
		{
			p = "Just lifting";
		}
	}
...
bool item_edible(object_type *o_ptr)
{
   switch (o_ptr->tval)
   {
		case TV_SKELETON:
...
bool item_tester_okay(object_type *o_ptr)
{
	...
	if (item_tester_char)
	{
		if (item_tester_char != tval_to_char[o_ptr->tval & 0x7F]) return (FALSE);
	}
...
void display_inven(void)
{
	...
	for (i = 0; i < z; i++)
	{
		...
		if (show_weights && o_ptr->wt)
		{
			s32b wt = o_ptr->wt * o_ptr->number;
			sprintf(tmp_val, "%3d.%1d lb", wt / 10, wt % 10);
...
void show_equip(void)
{
	...
	for (j = 0; j < k; j++)
	{
		...
		{
			int wt = o_ptr->wt * o_ptr->number;
			sprintf(tmp_val, "%3d.%d lb", wt / 10, wt % 10);
			put_str(tmp_val, j+1, 71);
		}
	}
...
void toggle_inven_equip(void)
{
	...
	for (j = 0; j < 8; j++)
	{
		...
		/* Flip inven to equip */
		if (op_ptr->term_flag[j] & (PW_INVEN))
		{
			/* Flip flags */
			op_ptr->term_flag[j] &= ~(PW_INVEN);
			op_ptr->term_flag[j] |= (PW_EQUIP);
		...
		/* Flip inven to equip */
		else if (op_ptr->term_flag[j] & (PW_EQUIP))
		{
			/* Flip flags */
			op_ptr->term_flag[j] &= ~(PW_EQUIP);
			op_ptr->term_flag[j] |= (PW_INVEN);
 ...
 * If it is FALSE then we are viewing inventory, else equipment.
 *
	...
	while (!done)
	{
		...
		if (show_choices)
		{
			...
			for (j = 0; j < 8; j++)
			{
				...
				if (op_ptr->term_flag[j] & (PW_INVEN)) ni++;

				/* Count windows displaying equip */
				if (op_ptr->term_flag[j] & (PW_EQUIP)) ne++;
			}
		...
		/* Find the character info, and describe it */
		if (item_tester_char)
		{
			for (j = 0; ident_info[j]; ++j)
			{
				if (item_tester_char == ident_info[j][0]) break;
			}

			/* Describe */
			sprintf(out_val, "[%s] ", ident_info[j] + 2);
		}
		else
		{
			sprintf(out_val, "[All items] ");
		}

		if (use_floor)
		{
			/* Begin the prompt */
			strcat(out_val, "Floor:");

			/* Append */
			if (equip && p_ptr->command_wrk) strcat(out_val, " ' for Equip,");
			else if (inven && !p_ptr->command_wrk) strcat(out_val, " ' for Inven,");
		}

		/* Viewing inventory */
		else if (!p_ptr->command_wrk)
		{
			/* Begin the prompt */
		...
		if (use_floor)
		{
			which = show_floor(tmp_val, &line);
		}

		/* Inventory screen */
		else if (!p_ptr->command_wrk)
		{
			which = show_inven(tmp_val, &line);
		}

		/* Equipment screen */
		else
		{
			which = show_equip(tmp_val, &line);
		}
		...
		switch (which)
		{
			...
			default:
			{
				...
				{
					/* Scan all objects in the grid */
					for (j = -1, this_o_idx = cave_o_idx[py][px]; this_o_idx; this_o_idx = next_o_idx)
					{
						object_type *o_ptr;
