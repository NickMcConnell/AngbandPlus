...
void py_attack(int y, int x)
{
	...
	while (num++ < p_ptr->num_blow)
	{
		...
		if (test_hit(chance, r_ptr->ac, m_ptr->ml))
		{
			...
			/* Alex */
			update_abil_fail_dam(a_to_d, k);
			update_abil_fail(a_to_h, r_ptr->ac/40);
			update_abil_fail(a_remove_to_h_pen, r_ptr->ac/150);
			update_abil_fail(a_dex, r_ptr->ac/100);
			update_abil_fail(a_str, r_ptr->ac/100);
			if (!m_ptr->ml) update_abil_fail(a_see_invis, r_ptr->level/5);
			if (k > m_ptr->maxhp/5)
			{
				update_abil_fail(a_dd, r_ptr->level/30);
			}

			/* Damage, check for fear and death */
			kill = mon_take_hit(cave_m_idx[y][x], k, &fear, NULL);
			if (kill) break;
	...
	if (kill)
	{
		...
		if (r_ptr->flags1 & (RF1_UNIQUE)) rlev *= 10;

		if (r_ptr->flags3 & RF3_ORC) update_abil_fail(a_slay_orc, rlev);
		if (r_ptr->flags3 & RF3_TROLL) update_abil_fail(a_slay_troll, rlev/2);
		if (r_ptr->flags3 & RF3_GIANT) update_abil_fail(a_slay_giant, rlev/2);
		if (r_ptr->flags3 & RF3_DRAGON) update_abil_fail(a_slay_dragon, rlev/5);
		if (r_ptr->flags3 & RF3_DEMON) update_abil_fail(a_slay_demon, rlev/10);
		if (r_ptr->flags3 & RF3_UNDEAD) update_abil_fail(a_slay_undead, rlev);
		if (r_ptr->flags3 & RF3_ANIMAL) update_abil_fail(a_slay_animal, rlev/2);
		if (r_ptr->flags3 & RF3_EVIL) update_abil_fail(a_slay_evil, rlev/5);

		/* Extract the known flags */
		object_flags_known(o_ptr, &f1, &f2, &f3);

		if ((old_hp - m_ptr->hp) > m_ptr->maxhp/2)
		{
			if ((f1 & (TR1_SLAY_DRAGON | TR1_KILL_DRAGON)) && (r_ptr->flags3 & RF3_DRAGON)) update_abil_fail(a_kill_dragon, rlev/20);
			if ((f1 & (TR1_SLAY_DEMON | TR1_KILL_DEMON)) && (r_ptr->flags3 & RF3_DEMON)) update_abil_fail(a_kill_demon, rlev/30);
			if ((f1 & (TR1_SLAY_UNDEAD | TR1_KILL_UNDEAD)) && (r_ptr->flags3 & RF3_UNDEAD)) update_abil_fail(a_kill_undead, rlev/10);

			update_abil_fail(a_blow, rlev/40);
			update_abil_fail(a_to_h_to_d, rlev/20);
		}

		if ((f1 & (TR1_BRAND_ACID)) && !(r_ptr->flags3 & (RF3_IM_ACID))) update_abil_fail(a_acid, rlev/5);
		if ((f1 & (TR1_BRAND_ELEC)) && !(r_ptr->flags3 & (RF3_IM_ELEC))) update_abil_fail(a_elec, rlev/4);
		if ((f1 & (TR1_BRAND_FIRE)) && !(r_ptr->flags3 & (RF3_IM_FIRE))) update_abil_fail(a_fire, rlev/3);
		if ((f1 & (TR1_BRAND_COLD)) && !(r_ptr->flags3 & (RF3_IM_COLD))) update_abil_fail(a_cold, rlev/3);
		if ((f1 & (TR1_BRAND_POIS)) && !(r_ptr->flags3 & (RF3_IM_POIS))) update_abil_fail(a_pois, rlev/4);
	}
