...
/*
 * Possible tval's for monster inventories
 */
static cptr r_info_tval[] =
{
	"",
	"TV_SKELETON",
	"TV_BOTTLE",
...
/*
 * Monster race flags
 * Note all treasure drops but "DROP_CHOSEN" are obsolete.
 */
...
errr init_a_info_txt(FILE *fp, char *buf)
{
	...
	/* Save some space for the random artifacts */
	for (i = ART_MAX_NORMAL; i < MAX_A_IDX; i++)
	{
			...
			/* Append chars to the name */
			strcpy(a_name + a_ptr->name, "Without a Definition");
...
errr init_r_info_txt(FILE *fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'D')
		{
			...
			if (s[strlen(s) - 1] != ' ')
			{
				/* Some punctuation needs two spaces */
				if (strchr(".?!", s[strlen(s) - 1]))
				{
					strcat(r_text + r_head->text_size++, " ");
				}
				strcat(r_text + r_head->text_size++, " ");
			}
		...
		if (buf[0] == 'B')
		{
			...
			for (i = 0; i < 4; i++) if (!x_ptr->blow[i].method) break;
			...
			/* Save the method */
			x_ptr->blow[i].method = n1;

			/* Save the effect */
			x_ptr->blow[i].effect = n2;

			/* Extract the damage dice and sides */
			x_ptr->blow[i].d_dice = atoi(s);
			x_ptr->blow[i].d_side = atoi(t);

			/* Next... */
			continue;
		}

		/* Process 'C' for "Carries" (up to ten lines) */
		if (buf[0] == 'C')
		{
			int n1, n2, n3, good, great, chance;

			/* Find the next empty inventory slot (if any) */
			for (i = 0; i < 10; i++) if (!x_ptr->inv[i].tval) break;
			...
			/* Analyze the tval */
			for (n1 = 0; r_info_tval[n1]; n1++)
			{
				if (streq(s, r_info_tval[n1])) break;
			}

			/* Invalid tval */
			if (!r_info_tval[n1]) return (1);
			...
			/* Scan for the values */
			if (4 != sscanf(t, "%d:%d:%d:%d",
								 &n3, &good, &great, &chance)) return (1);

			/* Save the tval */
			x_ptr->inv[i].tval = n1;

			/* Extract the damage dice and sides */
			x_ptr->inv[i].q_dice = n2;
			x_ptr->inv[i].q_side = n3;
			x_ptr->inv[i].good = (good != 0);
			x_ptr->inv[i].great = (great != 0);
			x_ptr->inv[i].chance = chance;
	...
	/* XXX XXX XXX The ghost _will_be_used!_ */

	/* Save some space for the player ghosts */
