...
static void sense_inventory(void)
{
        ...
        /* No sensing when confused and berserk */
        if (p_ptr->confused || p_ptr->shero) return;
...
static void process_world(void)
{
        ...
        if (!cave_floor_bold(py, px))
        {
                ...
                if (randint(10) == 1)
                  {
                  msg_print("The fog shifts to block your movement.");
	...
	if (p_ptr->exp_drain)
	{
        ...
        for (j = 0, i = INVEN_WIELD; i < INVEN_TOTAL; i++)
        {
                ...
                /* TY Curse */
                if ((f3 & TR3_TY_CURSE) && (randint(TY_CURSE_CHANCE) == 
		    (p_ptr->sign == SIGN_PISCES) ? 1 : 2))
                {
                        activate_ty_curse();
                }
