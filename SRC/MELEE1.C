 ...
 * an adjacent monster, including descriptions and effects. Repeat the
 * same for attacking the player.
 *
 ...
 */
static int check_hit(int power, int level, int terrain_bonus, monster_race *r_ptr)
{
        int i, k, ac;
        ...
        /* Percentile dice */
        k = rand_int(100); /* with account for luck()?? - might be really unbalancing */
...
static cptr desc_insult[] =
{
        ...
        "moons you!!!"
	"insults your grandmother!",
	"insults your homeland!",
	"throws rotted fruit at you!"
};
...
bool make_attack_normal(int m_idx)
{
	...
	if (cave[py][px].feat == FEAT_RUBBLE)
	{
		terrain_bonus = ac / 8 + 5;
	}
	/* Players in trees can take advantage of cover, especially rangers -LM- */
	...
	/* Players in deep water are vulnerable */
	if (cave[py][px].feat == FEAT_DEEP_WATER)
	{
        ...
        for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
        {
                ...
                switch (effect)
                {
                        ...
                        case RBE_ELEC:      power = 20; break;
                        case RBE_FIRE:      power = 20; break;
                        case RBE_COLD:      power = 20; break;
                        ...
                        case RBE_INSANITY:  power = 60; break;
                ...
                /* Monster hits player */
                if (!effect || check_hit(power, rlev, terrain_bonus, r_ptr))
                {
                        /* Always disturbing */
                        disturb(1, 0);
                        ...
                        switch (method)
                        {
                                ...
                                case RBM_INSULT:
                                {
                                        act = desc_insult[rand_int(MAX_INSULTS)];
                                        sound(SOUND_MOAN);
                                        break;
                                }
					...
					if (p_ptr->mind & MIND_BIOFEEDBACK)
					    damage /= 2;
                                        ...
                                        if (wizard)
                                        {
                                        msg_format("%^s does %d damage by hurting.", m_name, damage);
                                        }
					...
					if (p_resist_pois) chance *= 3;
					if (p_ptr->oppose_pois) chance *= 3;

					if (p_resist_pois) damage = (damage + 2) / 3;
					if (p_ptr->oppose_pois) damage = (damage + 2) / 3;
					
                                        /* Take some damage */
                                        take_hit(damage, ddesc);

                                        /* Take "poison" effect */
                                        if (((p_resist_pois) || p_ptr->oppose_pois) && (randint(chance)==1))
                                        {
                                                if (set_poisoned(p_ptr->poisoned + randint(rlev + damage) + 1))
                                                {
                                                        obvious = TRUE;
                                                }
                                        }
                                        ...
                                        {
                                                if (set_poisoned(p_ptr->poisoned + randint(rlev + damage) * 6 / (6 + randint(6))))
                                                {
                                                        obvious = TRUE;
                                                }
                                        }
							...
							heal = MIN(heal, m_ptr->maxhp - m_ptr->hp);
                                                        ...
                                                        if (wizard) msg_format("%^s gains %d hp by draining.", m_name, heal);
                                                        ...
                                                        if (wizard) msg_format("%^s does %d damage by draining.", m_name,damage);
					...
					if (m_ptr->confused) break;
					         ...
					         ((p_ptr->sign == SIGN_SAGGITARIUS) ? 20 : 0))
                                ...
                                case RBE_LOSE_LUC:
                                {
                                        ...
                                        /* Damage (stat) */
                                        if (do_dec_stat(A_LUC, STAT_DEC_NORMAL))
                                                obvious = TRUE;

                                        if (wizard)
                                        {
                                        msg_format("%^s does %d damage by lowering luck.", m_name,damage);
                                        }
                                ...
                                case RBE_LOSE_ALL:
                                {
                                        ...
                                        if (do_dec_stat(A_WIS, STAT_DEC_NORMAL)) obvious = TRUE;
                                        if (do_dec_stat(A_CHR, STAT_DEC_NORMAL)) obvious = TRUE;
                                        if (do_dec_stat(A_LUC, STAT_DEC_NORMAL)) obvious = TRUE;
                                ...
                                case RBE_INSANITY:
                                {
                                    ...
                                    take_sanity_hit(damage, ddesc);
                                     ...
                                     if (wizard)
                                     {
                                        msg_format("%^s damages your sanity for %d.", m_name,damage);
                                     }
					...
					if (p_ptr->mind & MIND_BIOFEEDBACK)
					    damage /= 2;
                                ...
                                case RBE_EXP_10:
                                {
                                        ...
                                        if (p_hold_life && (rand_int(100) < 95))
                                        {
                                ...
                                case RBE_EXP_20:
                                {
                                        ...
                                        if (p_hold_life && (rand_int(100) < 90))
                                        {
                                                msg_print("You keep hold of your life force!");
                                        }
                                        else
					...
					if (p_ptr->oppose_pois) damage = (damage + 2) / 3;
                                        ...
                                        if ((p_resist_pois || p_ptr->oppose_pois) && randint(chance) == 1)
                                        {
                                                if (set_poisoned(p_ptr->poisoned + randint(rlev + damage / 2) + 1))
                                                {
                                                        obvious = TRUE;
                                                }
                                        }
                                        ...
                                        {
                                                if (set_poisoned(p_ptr->poisoned + randint(rlev + damage / 2) * 6 / (6 + randint(6))))
                                                {
                                                        obvious = TRUE;
                                                }
                                        }
                                        ...
                                        if (wizard)
                                        {
                                        msg_format("%^s does %d damage by transmitting disease.", m_name,damage);
                                        }
                                ...
                                case RBE_TIME:
                                {
                                        ...
                                        if (wizard)
                                        {
                                        msg_format("%^s does %d damage by releasing a time stream.", m_name,damage);
                                        }
                                     ...
                                     case RBE_HALLU:
                                       {
                                        ...
                                        {
                                                msg_print("Your vision blurs for a moment.,");
                                                obvious = TRUE;
                                        }
                                        else
                                        {
                                                if (set_image(p_ptr->image + 3 + randint(rlev)))
                                                {
                                                        obvious = TRUE;
                                                }
                                        }
                                        ...
                                        if (wizard)
                                        {
                                        msg_format("%^s does %d damage by inducing hallucination.", m_name,damage);
                                        }
                                ...
                                case RBE_HUNGER:
                                {
                                        ...
                                        /* Reduce food counter, but don't bash the player
                                         * too hard. */
                                        if (p_ptr->food <= PY_FOOD_STARVE)
                                                set_food(p_ptr->food - PY_FOOD_STARVE / div);
                                        else if (p_ptr->food < PY_FOOD_FAINT)
                                                set_food(p_ptr->food - PY_FOOD_FAINT / div);
                                        else
                                                set_food(p_ptr->food - PY_FOOD_WEAK / div);

                                        if (wizard)
                                        {
                                        msg_format("%^s does %d damage by inducing hunger.", m_name,damage);
                                        }
                                ...
                                case RBE_DEATH:
                                  {
                                       ...
                                       {
                                       msg_format("Was that the %s of death?",
                                             ((method == RBM_GAZE) ? "gaze": "touch"));
                                       break;
                                       }
                                       else
                                       msg_format("Oh no! It's the %s of death!",
                                             ((method == RBM_GAZE) ? "gaze": "touch"));
                                       if (rand_int(100) < p_ptr->skill_sav + luck())
                                       msg_print("Luckily, you are unaffected.");
                                       else take_hit(damage, ddesc);
                                       break;
                                  }
                                case RBE_STONE:
                                  {
                                       if (p_ptr->stoning) break;
                                       if (p_ptr->prace == RACE_GOLEM)
                                       {
                                       msg_format("You are unaffected!");
                                       break;
                                       }
                                       else
                                       if (rand_int(100) < p_ptr->skill_sav + luck())
                                       msg_print("You resist the effects!");
                                       else (void)set_stone(damage);
                                       break;
                                  }
                                }
                        }
                        ...
                        if (touched)
                        {
                                ...
                                if (p_sh_cold && alive)
                                {
                                        ...
                                        {
                                                msg_format("%^s gets freezed!", m_name);
                                                if (mon_take_hit(m_idx, damroll(2,6), &fear,
        ...
        if (blinked)
        {
                msg_print("The thief sprints away very fast!");
