...
static void calc_mana(void)
{
	...
	over_wgt = (cur_wgt - max_wgt) / 10; /* (Heino Vander Sanden) */

	/* Heavy armor penalizes mana (Heino Vander Sanden) */
	if (over_wgt > 0)
	{
		/* Encumbered */
		...
		/* Reduce mana */
		msp -= over_wgt * ((over_wgt / 5) + 1);
	}

	/* Mages may not wield heave weapons (Heino Vander Sanden) */
	if (p_ptr->pclass == 1)
	{
		/* Assume player not encumbered by weapon */
		p_ptr->cumber_weapon = FALSE;

		/* Weigh the weapon */
		cur_wgt = inventory[INVEN_WIELD].weight;

		/* Determine the weight allowance */
		max_wgt = 100;
		over_wgt = (cur_wgt - max_wgt) / 10;

		/* Heavy weapon penalizes mana */
		if (over_wgt > 0)
		{
			/* Encumbered */
			p_ptr->cumber_weapon = TRUE;

			/* Reduce mana */
			msp -= over_wgt * ((over_wgt / 2) + 1);
		}
	}
	...
	/* Take note when "weapon state" changes (Heino Vander Sanden) */
	if (p_ptr->old_cumber_weapon != p_ptr->cumber_weapon)
	{
		/* Message */
		if (p_ptr->cumber_weapon)
		{
			msg_print("The weight of your weapon encumbers your movement.");
		}
		else
		{
			msg_print("You feel able to move more freely.");
		...
		/* Save it */
		p_ptr->old_cumber_weapon = p_ptr->cumber_weapon;
	}
}
...
static void calc_bonuses(void)
{
	...
	if (o_ptr->k_idx && !p_ptr->heavy_wield)
	{
		...
		switch (p_ptr->pclass)
		{
			...
			/* Mage (Heino Vander Sanden) */
			case CLASS_MAGE:    num = 2; wgt = 40; mul = 2; break;
