...
static errr init_r_info_raw(int fd)
{
	...
	/* Read and Verify the sub-header */
	if (fd_read(fd, (char*)(&test), sizeof(header)) ||
	...
	C_MAKE(x_info, x_head->info_num, monster_xtra);

	/* Allocate the "l_info" array */
	C_MAKE(l_info, r_head->info_num, monster_lore);
...
static errr init_r_info(void)
{
	...
	/* Save the "record" information */
	x_head->info_num = MAX_R_IDX;
	x_head->info_len = sizeof(monster_xtra);
	...
	C_KILL(x_info, x_head->info_num, monster_xtra);
...
static errr init_g_info(void)
{
	...
	/* Save the "record" information */
	g_head->info_num = MAX_G_IDX;
	g_head->info_len = sizeof(ghost_type);
	...
	/* Assume the size of "g_name" */
	fake_name_size = 20 * 1024L;

	/* Allocate the "g_info" array */
	C_MAKE(g_info, g_head->info_num, ghost_type);

	/* Hack -- make "fake" array */
	C_MAKE(g_name, fake_name_size, char);

	/* Prepare the arrays */
	if (!load_bones())
	{
		/* Quit */
		quit(format("Error in 'bones%02d' file.", cur_bones));
	}
	...
	C_KILL(g_name, fake_name_size, char);

	/* Forget the array sizes */
	fake_name_size = 0;
	fake_text_size = 0;

	/* Allocate the "g_name" array */
	C_MAKE(g_name, g_head->name_size, char);

	/* Store the information */
	load_bones();

	/* Actually create monster races for the ghosts */
	for (i = 0; i < cur_bones; i++)
	{
		/* Don't redefine ghosts */
		if (!g_info[i].und_type) make_ghost(i);
	}
...
static errr init_other(void)
{
	...
	(void)format("%s (%s).", "Eric Bock", MAINTAINER);
...
void init_angband(void)
{
	...
	/* Initialize feature info */
	note("[Initializing arrays... (ghosts)]");
	if (init_g_info()) quit("Cannot initialize ghosts");
