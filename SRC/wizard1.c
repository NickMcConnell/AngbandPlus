...
static void kind_info(char *buf, char *dam, char *wgt, int *lev, s32b *val, int k)
{
	...
	/* Weight */
	sprintf(wgt, "%3d.%d", i_ptr->wt / 10, i_ptr->wt % 10);
}
...
static void analyze_misc (object_type *o_ptr, char *misc_desc)
{
	...
	sprintf(misc_desc, "Level %u, Rarity %u, %d.%d lbs, %ld Gold",
	        ...
	        a_ptr->wt / 10, a_ptr->wt % 10, a_ptr->cost);
}
...
static void spoil_mon_info(cptr fname)
{
	 ...
	 */
	for (n = 1; n < RACE_MAX_NORMAL; n++)
	{
		monster_race *r_ptr = &r_info[n];
		monster_xtra *x_ptr = &x_info[n];
		...
		for (i = 0, j = 0; j < 4; j++)
		{
			if (x_ptr->blow[j].method) i++;
		}

		/* Examine the actual attacks */
		for (k = 0, j = 0; j < 4; j++)
		{
			if (!x_ptr->blow[j].method) continue;

			/* No method yet */
			p = "???";

			/* Acquire the method */
			switch (x_ptr->blow[j].method)
			{
			...
			if (x_ptr->blow[j].effect)
			{
				spoil_out(" to ");
				spoil_out(q);
				if (x_ptr->blow[j].d_dice && x_ptr->blow[j].d_side)
				{
					spoil_out(" with damage");
					if (x_ptr->blow[j].d_side == 1)
						sprintf(buf, " %d", x_ptr->blow[j].d_dice);
					else
						sprintf(buf, " %dd%d",
							  x_ptr->blow[j].d_dice, x_ptr->blow[j].d_side);
					spoil_out(buf);
				}
			}
