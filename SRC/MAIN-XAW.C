...
static void Initialize(AngbandWidget request, AngbandWidget wnew)
{
	...
	int n;
 	...
 	/* Add in all the colors greater than 15 */
 	int i;
 	for (i = 16; i < NUM_COLORS; i++)
 	  wnew->angband.color[i] = (angband_color_table[i][1] << 16) + 
 	    (angband_color_table[i][2] << 8) + angband_color_table[i][3];
