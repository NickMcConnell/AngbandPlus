...
static void castle_greet(void)
{
	...
	if (increment < 1)
	{
		msg_format("Ah, a young adventurer, return when: %s (Level %d)",
	...
	{
		msg_format("You have been rewarded, return when %s (Level %d)",
...
static void greet_char(void)
{
	...
	if (increment < 1)
	{
		msg_format("You are young yet, return when: %s (Level %d)",
...
static void building_recharge(void)
{
	...
	int         charges;
	char        tmp_str[80];
	int         limit = 20 + (p_ptr->lev / 2);


	/* Display some info */
	clear_bldg(5, 18);
	prt(format("  Based on your skill, we can recharge staves and wands up to %d charges.", limit), 6, 0);
	prt("  The prices depend on the type.", 7, 0);
	...
	if (((o_ptr->tval == TV_WAND) || (o_ptr->tval == TV_STAFF)) &&
	    (o_ptr->pval >= limit))
	{
		/* ToDo: Find a better message text. */
		msg_print("This is beyond your skill.");
		msg_print(NULL);
		return;
	}
	...
	else
	{
		...
		charges = get_quantity(format("Add how many charges for %d gold? ",
		              price), MIN(p_ptr->au / price, limit - o_ptr->pval));
...

	{
		switch (bact)
		{
			...
			case BACT_ENCHANT_ARROWS:
				...
				enchant_item(bcost, 1, 1, 0);
				/* fix_item(0, INVEN_WIELD, TV_ARROW, FALSE, BACT_ENCHANT_ARROWS, set_reward);*/
				break;
