...
void teleport_player(int dis)
{
	...
	update_act_fail(ACT_PHASE, 5);
	update_act_fail(ACT_TELEPORT, 2);
}
 ...
 * the player to "dead".
/* Alex: modify given ability fail. abil==a_last to not modify fails */

void take_hit(int dam, cptr kb_str, enum Art_Abil abil)
{
	...
	/* Alex */
	update_abil_fail_dam(a_con, 2*dam);
	if (abil != a_last) update_abil_fail_dam(abil, dam);

	/* Alex */
	message_format(MSG_HIT, 0, "(-%d)", dam);
...
static int minus_ac(void)
{
	...
	update_abil_fail_dam(a_res_acid, 10);
	update_abil_fail_dam(a_im_acid, 1);
	/* Item was damaged */
	return (TRUE);
}
...
void acid_dam(int dam, cptr kb_str)
{
	...
	/* Take damage */
	take_hit(dam, kb_str, a_res_acid);

	/* Inventory damage */
	inv = inven_damage(set_acid_destroy, inv);
	update_abil_fail(a_res_acid, 50*inv);
	update_abil_fail_dam(a_im_acid, dam/4);
	update_abil_fail(a_im_acid, inv);
}
...
void elec_dam(int dam, cptr kb_str)
{
	...
	/* Take damage */
	take_hit(dam, kb_str, a_res_elec);

	/* Inventory damage */
	inv = inven_damage(set_elec_destroy, inv);
	update_abil_fail(a_res_elec, 50*inv);
	update_abil_fail_dam(a_im_elec, dam/4);
	update_abil_fail(a_im_elec, inv);
}
...
void fire_dam(int dam, cptr kb_str)
{
	...
	/* Take damage */
	take_hit(dam, kb_str, a_res_fire);

	/* Inventory damage */
	inv = inven_damage(set_fire_destroy, inv);
	update_abil_fail(a_res_fire, 50*inv);
	update_abil_fail_dam(a_im_fire, dam/4);
	update_abil_fail(a_im_fire, inv);
}
...
void cold_dam(int dam, cptr kb_str)
{
	...
	/* Take damage */
	take_hit(dam, kb_str, a_res_cold);

	/* Inventory damage */
	inv = inven_damage(set_cold_destroy, inv);
	update_abil_fail(a_res_cold, 50*inv);
	update_abil_fail_dam(a_im_cold, dam/4);
	update_abil_fail(a_im_cold, inv);
}
...
bool dec_stat(int stat, int amount, int permanent)
{
	...
	{
		/* Alex */
		update_abil_fail(a_sust_str + stat, (p_ptr->stat_cur[stat] - cur) * 100 + 
							(p_ptr->stat_max[stat] - max) * 300);
...
bool apply_disenchant(int mode)
{
	...
	update_abil_fail(a_res_disen, fail);
...
static void apply_nexus(const monster_type *m_ptr)
{
	...
	switch (randint(7))
	{
		...
		{
			teleport_player(200);
			update_abil_fail(a_res_nexus, 1);
			break;
		}
		...
		case 4: case 5:
		{
			...
			update_abil_fail(a_res_nexus, 2);
			break;
		}
		...
		case 6:
		{
			if (rand_int(100) < p_ptr->skill_sav)
			{
				...
				update_abil_fail(a_wis, 10);
				break;
			}

			/* Teleport Level */
			teleport_player_level();
			update_abil_fail(a_res_nexus, 50);
			break;
		}
		...
		case 7:
		{
			...
			{
				msg_print("You resist the effects!");
				update_abil_fail(a_wis, 20);
				break;
			}

			msg_print("Your body starts to scramble...");
			update_abil_fail(a_res_nexus, 100);
	...
	switch (typ)
	{
		...
		case GF_KILL_DOOR:
		{
			     ...
			     (cave_feat[y][x] <= FEAT_DOOR_TAIL)))
			{
				...
				/* Alex */
				if (who < 0) update_act_fail(ACT_TRAP_DOOR_DEST, 1);
			}
		...
		case GF_KILL_WALL:
		{
			...
			/* Alex */
			if (who < 0) update_act_fail(ACT_STONE_TO_MUD, 1);
			break;
		}
		...
		switch (typ)
		{
			...
			case GF_KILL_DOOR:
			{
				...
				if (o_ptr->tval == TV_CHEST)
				{
					...
					/* Alex */
					if (who < 0) update_act_fail(ACT_TRAP_DOOR_DEST, 10);
				}
 ...
 *
 * Alex: call update_act_fail_dam if player cause damage.
 * For r == 0, update bolt activation, for r > 0 ball activation.
 * (Central point of ball contributes to bolt activation. XXX).
 */
static bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	switch (typ)
	{
		...
		case GF_MISSILE:
		{
			...
			/* Alex */
			if (who < 0 && !r) update_act_fail_dam(ACT_MISSILE, dam);
			break;
		}
		...
		case GF_ACID:
		{
			...
			/* Alex */
			if (who < 0 && !r) update_act_fail_dam(ACT_ACID1, dam);
			break;
		}
		...
		case GF_ELEC:
		{
			...
			if (who < 0)
			{
				if (r > 0)
				{
					update_act_fail_dam(ACT_ELEC2, dam);
					update_act_fail_dam(ACT_STAR_BALL, dam);
				}
				else
					update_act_fail_dam(ACT_LIGHTNING_BOLT, dam);
			}
			break;
		}
		...
		case GF_FIRE:
		{
			...
			if (who < 0)
			{
				if (r > 0)
				{
					update_act_fail_dam(ACT_FIRE2, dam);
					update_act_fail_dam(ACT_FIRE3, dam);
				}
				else
					update_act_fail_dam(ACT_FIRE1, dam);
			}
			break;
		}
		...
		case GF_COLD:
		{
			...
			if (who < 0)
			{
				if (r > 0)
				{
					update_act_fail_dam(ACT_FROST2, dam);
					update_act_fail_dam(ACT_FROST3, dam);
					update_act_fail_dam(ACT_FROST5, dam);
				}
				else
				{
					update_act_fail_dam(ACT_FROST1, dam);
					update_act_fail_dam(ACT_FROST4, dam);
				}
			}
			break;
		}
		...
		case GF_POIS:
		{
			...
			/* Alex */
			if (who < 0) update_act_fail_dam(ACT_STINKING_CLOUD, dam);
			break;
		}
		...
		case GF_ARROW:
		{
			if (seen) obvious = TRUE;
			/* Alex */
			if (who < 0 && !r) update_act_fail_dam(ACT_ARROW, dam);
			break;
		}
		...
		case GF_MANA:
		{
			if (seen) obvious = TRUE;
			/* Alex */
			if (who < 0 && !r) update_act_fail_dam(ACT_MANA_BOLT, dam);
			break;
		}
		...
		case GF_OLD_DRAIN:
		{
			...
			if (who < 0 && !r)
			{
				update_act_fail_dam(ACT_DRAIN_LIFE1, dam);
				update_act_fail_dam(ACT_DRAIN_LIFE2, dam);
			}
			break;
		}
		...
		case GF_OLD_SLEEP:
		{
			...
			else
			{
				...
				/* Alex */
				if (who < 0) update_act_fail(ACT_SLEEP, 1 + r_ptr->level/10);
			}
		...
		case GF_OLD_CONF:
		{
			...
			/* Alex */
			if (who < 0 && do_conf) update_act_fail(ACT_CONFUSE, 1 + r_ptr->level);
		...
		case GF_LITE_WEAK:
		{
			...
			/* Alex */
			if (who < 0) update_act_fail_dam(ACT_STARLIGHT, dam);
			break;
		}
		...
		case GF_KILL_WALL:
		{
			...
			if (r_ptr->flags3 & (RF3_HURT_ROCK))
			{
				...
				/* Alex */
				if (who < 0) update_act_fail_dam(ACT_STONE_TO_MUD, dam);
			}
		...
		case GF_AWAY_ALL:
		{
			...
			/* Alex */
			if (who < 0)
				update_act_fail(ACT_TELE_AWAY, 1);
			break;
		}
		...
		case GF_DISP_EVIL:
		{
			...
			if (r_ptr->flags3 & (RF3_EVIL))
			{
				...
				/* Alex */
				if (who < 0) update_act_fail_dam(ACT_DISP_EVIL, dam);
			}
...
static bool project_p(int who, int r, int y, int x, int dam, int typ)
{
	...
	switch (typ)
	{
		...
		case GF_POIS:
		{
			...
			take_hit(dam, killer, a_res_pois);
		...
		case GF_NETHER:
		{
			...
			take_hit(dam, killer, a_res_nether);
			break;
		}
		...
		case GF_CHAOS:
		{
			...
			take_hit(dam, killer, a_res_chaos);
			break;
		}
		...
		case GF_SHARD:
		{
			...
			take_hit(dam, killer, a_res_shard);
			break;
		}
		...
		case GF_SOUND:
		{
			...
			else
			{
				...
				(void)set_stun(p_ptr->stun + k);
				update_abil_fail(a_res_sound, k);
			}
			take_hit(dam, killer, a_res_sound);
			break;
		}
		...
		case GF_CONFUSION:
		{
			...
			take_hit(dam, killer, a_res_conf);
			break;
		}
		...
		case GF_DISENCHANT:
		{
			...
			take_hit(dam, killer, a_res_disen);
			break;
		}
		...
		case GF_NEXUS:
		{
			...
			take_hit(dam, killer, a_res_nexus);
			break;
		}
		...
		case GF_LITE:
		{
			...
			take_hit(dam, killer, a_res_lite);
			break;
		}
		...
		case GF_DARK:
		{
			...
			take_hit(dam, killer, a_res_dark);
			break;
		}
		...
		case GF_GRAVITY:
		{
			...
			take_hit(dam, killer, a_remove_teleport);
			break;
		}
...
bool project(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	{
		monster_type *m_ptr = &mon_list[cave_m_idx[y][x]];
		if (blind) update_abil_fail(a_res_blind, 10);
		if (!m_ptr->ml)
		{
			monster_race* r_ptr = &r_info[m_ptr->r_idx];
			update_abil_fail(a_see_invis, r_ptr->level/10);
			if (!(r_ptr->flags2 & RF2_COLD_BLOOD)) update_abil_fail(a_infra, r_ptr->level/4);
			if (!(r_ptr->flags2 & RF2_EMPTY_MIND)) update_abil_fail(a_telepathy, r_ptr->level/40);
		}
	}
