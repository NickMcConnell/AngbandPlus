 ...
 *  and add the 2 obligatory ones ( Sauron and Morgoth )
...
void initialise_quests()
{
	...
	/* Add a special quest Sauron */
	q_list[0].level = 99;
	q_list[0].r_idx = 546;
	q_list[0].max_num = 1;

	/* Add a second quest Morgoth */
	q_list[1].level = 100;
	q_list[1].r_idx = 547;
	q_list[1].max_num = 1;
}
...
int get_rnd_q_monster(int q_idx)
{
	...
	tmp = rand_range(1,10);
	/* first level 10 monster (135), last monster (545) */
	switch (tmp)
	{
		case 1 : r_idx = rand_range(135,234); break;
		case 2 : r_idx = rand_range(235,317); break;
		case 3 : r_idx = rand_range(318,429); break;
		case 4 : r_idx = rand_range(430,481); break;
		case 5 : r_idx = rand_range(482,513); break;
		case 6 : r_idx = rand_range(514,522); break;
		case 7 : r_idx = rand_range(523,534); break;
		case 8 : r_idx = rand_range(535,541); break;
		case 9 : r_idx = rand_range(542,545); break;
		default : r_idx = rand_range (135,545);
	}
	for (j = 2; j < q_idx; j++)
...
static void player_birth_quests(void)
{
	...
	for (i = DEFAULT_QUESTS; i < MAX_Q_IDX; i++)
	{
		...
		q_list[i].level = q_list[i].level - rand_range(2,3+(q_list[i].level / 6));
		...
		do
		{
			...
			for (j = 2; j < i; j++)
			{
				if (q_list[j].level == q_list[i].level)
				{
					q_list[i].level = q_list[i].level - 2;
					if (q_list[i].level < 3) q_list[i].level = 15;
						 same_level = TRUE;
				}
			}
		} while (same_level);
...
static bool player_birth_aux()
{
	...
	while (1)
	{
		put_str(format("Number of additional quest? (<%u) ", MAX_QUESTS), 20, 2);
