...
void get_realms()
{
	...
	if (pclas == 5 || pclas == 3 || pclas == CLASS_CHAOS_WARRIOR
		|| pclas == CLASS_MONK || pclas == CLASS_HIGH_MAGE
	    || pclas == CLASS_PSIONICIST) return;
	else
...
static bool player_birth_aux()
{
    ...
    if (mp_ptr->spell_book != TV_PSI_BOOK) /* until specialization &~ */
      {
