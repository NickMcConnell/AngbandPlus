...
static void chest_trap(int y, int x, s16b o_idx)
{
	...
	if (trap & (CHEST_LOSE_STR))
	{
		...
		take_hit(damroll(1, 4), "a poison needle", a_last);
	...
	if (trap & (CHEST_EXPLODE))
	{
		...
		o_ptr->pval = 0;
		take_hit(damroll(5, 8), "an exploding chest", a_search);
	}
}
...
static bool do_cmd_open_chest(int y, int x, s16b o_idx)
{
	...
	if (o_ptr->pval > 0)
	{
		...
		if (rand_int(100) < j)
		{
			...
			/* Alex */
			update_abil_fail(a_int, 1);
			update_abil_fail(a_dex, 1);
			gain_exp(1);
			flag = TRUE;
		}
...
static bool do_cmd_tunnel_aux(int y, int x)
{
	...
	else if (cave_feat[y][x] >= FEAT_WALL_EXTRA)
	{
		...
		if ((p_ptr->skill_dig > 40 + rand_int(1600)) && twall(y, x))
		{
			...
			/* Alex */
			update_abil_fail(a_tunnel, 50);
			update_abil_fail(a_impact, 1);
			update_abil_fail(a_str, 1);
		}
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			if (test_hit(chance2, r_ptr->ac, m_ptr->ml))
			{
				...
				/* Alex */
				update_abil_fail_dam(a_to_d, tdam);
				update_abil_fail(a_to_h, r_ptr->ac/40);
				update_abil_fail(a_dex, r_ptr->ac/150);
				if (!visible) update_abil_fail(a_see_invis, r_ptr->level/5);
				if (tdam >= m_ptr->maxhp)
				{
					update_abil_fail(a_might, r_ptr->level/10);
					update_abil_fail(a_shots, r_ptr->level/20);
					update_abil_fail(a_might_shots, r_ptr->level/30);
				}
