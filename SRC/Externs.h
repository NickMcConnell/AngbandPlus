...
/* Alex */
extern s32b file_a_max;	/*  Max size for "a_info[]" before new artifacts */
...
extern void take_hit(int dam, cptr kb_str, enum Art_Abil abil);
...
/* Alex */
extern bool create_ego(int power, int sacrifice_levels);
/* Alex */
extern bool create_artifact(int power, int sacrifice_levels, bool player_choose, int choices, int tries);
/* Alex */
extern bool is_special(byte tval);
/* Alex */
extern bool item_tester_hook_for_create_artifact(const object_type *o_ptr);
...
extern int weight_limit(void);/* Alex */
...
#endif /* GJW_RANDART */
extern s16b *kinds;
extern cptr names_list;
extern int bow_multiplier(int sval);
extern void do_curse(artifact_type *a_ptr);

/* Alex: NewArt.c */
extern bool make_rnd_art(object_type* base);
extern bool add_new_art(int power, const object_type* base, bool player_choose, int choices, int tries);
extern bool enchant_artifact(int a_idx, int power, bool player_choose, int choices, int tries);
extern bool realloc_art(int n);
extern void init_new_art();
extern void done_new_art();
extern char* new_art_names;
extern int last_power;
extern s32b art_power(const artifact_type *a_ptr, int k_idx);
extern int get_levels_sacrifice();
extern u16b abil_fail[a_last];
extern u16b act_fail[ACT_MAX];
extern void wipe_fails();
extern void update_abil_fail(enum Art_Abil a, int p);
extern void update_act_fail(byte a, int p);
extern void update_abil_fail_dam(enum Art_Abil a, int dam);
extern void update_act_fail_dam(byte a, int dam);
extern void update_pval_fail();
extern void do_cmd_knowledge_abils();
extern void do_cmd_knowledge_acts();
extern bool is_activation(enum Art_Abil a);
extern bool is_bad(enum Art_Abil a);
extern bool is_dependent(enum Art_Abil a);
extern void forget_abil_act();

/* script.c */
/* Alex: we need call_hook in add_activation() in NewArt.c */
extern bool call_hook(cptr hook, cptr fmt, cptr ret, ...);
...
/* wizard1.c */
extern bool make_fake_artifact(object_type *o_ptr, int name1);/* Alex: int instead of byte */
