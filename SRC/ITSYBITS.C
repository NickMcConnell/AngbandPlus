...
#else
VOID WINAPI ibAdjustWindowRect( LPRECT lprc, DWORD dwStyle, BOOL fMenu, UINT nCapSize )
{
    short    cx = 0, cy = 0 ;
    ...
    if ( dwStyle & IBS_VERTCAPTION )
        lprc->left -= nCapSize ;
    else
        if ( dwStyle & IBS_HORZCAPTION )
            lprc->top -= nCapSize ;
...
// ibDrawCaption( HDC hDC, HWND hWnd, LPRECT lprc,
