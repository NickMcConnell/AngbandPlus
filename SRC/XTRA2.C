...
void monster_death(int m_idx)
{
	...
	* Heino Vander Sanden
	*/
 	if ((is_quest(dun_level, TRUE)) && (r_ptr->flags1 & (RF1_QUESTOR)))
	{
	...
	if (m_ptr->hp < 0)
	{
		...
		if (speak_unique && (r_ptr->flags2 & (RF2_CAN_SPEAK)))
		{
			...
			if (randint(REWARD_CHANCE)==1)
			{
				...
				if (reward > 30000) reward = 30000;/* Force 'good' values */
		...
		if (r_ptr->mexp)
		{
			...
			mod = p_ptr->max_plv * div;
			new_exp_frac = ((long)r_ptr->mexp * r_ptr->mexp % mod) * r_ptr->level % mod;

			/* calculate the integer exp part */
			new_exp = ((long)r_ptr->mexp * r_ptr->level / p_ptr->max_plv) * r_ptr->mexp / div;
...
void gain_level_reward(int chosen_reward)
{
	...
	else if (!(p_ptr->lev % 13) || !(p_ptr->lev % 9)) nasty_chance = 3;
	...
	switch (chosen_reward?chosen_reward:effect)
	{
		...
		case REW_SUM_CYBE:
			...
			{
				summon_specific(py, px, 200, SUMMON_CYBER);
			}
			break;
		case REW_TY_CURSE:
		...
		case REW_SUM_DEMO:
			...
			for (dummy = 1; !rand_int(dummy) && (dummy <= 8); dummy++);
			{
				summon_specific(py, px, (dun_level < 10) ? 10 : dun_level, SUMMON_DEMON);
			}
			break;
		case REW_DO_HAVOC:
			msg_format("The voice of %s booms out:",
		...
		case REW_WRAITH:
			...
			(void)set_shadow(p_ptr->wraith_form + rand_int(100) + 50);
			break;
		case REW_INVULN:
			msg_format("The voice of %s thunders:",
				chaos_patrons[p_ptr->chaos_patron]);
		...
		case REW_SER_DEMO:
			...
			summon_specific_friendly(py, px, dun_level + 15, SUMMON_DEMON, FALSE);
			break;
		case REW_SER_MONS:
			msg_format("%s rewards you with a servant!",chaos_patrons[p_ptr->chaos_patron]);
			summon_specific_friendly(py, px, dun_level + 5, SUMMON_NO_UNIQUES, FALSE);
			break;
		case REW_SER_UNDE:
			msg_format("%s rewards you with an undead servant!",chaos_patrons[p_ptr->chaos_patron]);
			summon_specific_friendly(py, px, dun_level + 10, SUMMON_UNDEAD, FALSE);
			break;
		case REW_SER_DRAG:
			msg_format("%s rewards you with a draconic servant!",chaos_patrons[p_ptr->chaos_patron]);
			summon_specific_friendly(py, px, dun_level + 20, SUMMON_DRAGON, FALSE);
			break;
		case REW_SER_CYBE:
			msg_format("The voice of %s thunders:",
				chaos_patrons[p_ptr->chaos_patron]);
			msg_print("'Trample thine enemies, mortal!'");
			summon_specific_friendly(py, px, 200, SUMMON_CYBER, FALSE);
			break;
		default:
					...
					else
					{
						...
						if (i < MUT1_MAX)
						{
							switch(muta_which)
							{
								...
								case MUT1_RECALL:
								{
									if (!dun_level) msg_print("The dungeon seems very distant.");
								...
								{
									msg_print("Your hands stop throbbing.");
