...
void spell_info(char *p, int spell)
{
	...
	if (mp_ptr->spell_book == TV_MAGIC_BOOK)
	{
		...
		/*
		 * Mage does double damage
		 * Heino Vander Sanden
		 */
		int plevdam = ((p_ptr->pclass == 1) ? (plev * 2) : plev);
		int pbasis = ((p_ptr->pclass == 1) ? ((p_ptr->lev >=35) ? 2 : 1) : 0);

		/* Analyze the spell */
		switch (spell)
		{
			case 0: sprintf(p, " dam %dd%d", 3 + ((plevdam - 1) / 5), 4 + pbasis); break;
			case 2: strcpy(p, " range 10"); break;
			case 5: strcpy(p, " heal 2d8"); break;
			case 8: sprintf(p, " dam %d", 10 + (pbasis * 5) + (plevdam / 2)); break;
			case 10: sprintf(p, " dam %dd%d", (3 + ((plevdam - 5) / 4)), 8 + pbasis); break;
			case 14: sprintf(p, " range %d", plev * 10); break;
			case 15: strcpy(p, " dam 6d8"); break;
			case 16: sprintf(p, " dam %dd%d", (5 + ((plevdam - 5) / 4)), 8 + pbasis); break;
			case 24: sprintf(p, " dam %dd%d", (8 + ((plevdam - 5) / 4)), 8 + pbasis); break;
			case 26: sprintf(p, " dam %d", 30 + (pbasis * 10) + plevdam); break;
			case 29: sprintf(p, " dur %d+d20", plev); break;
			case 30: sprintf(p, " dam %d", 55 + (pbasis * 15) + plevdam); break;
			case 38: sprintf(p, " dam %dd%d", (6 + ((plevdam - 5) / 4)), 8 + pbasis); break;
			case 39: sprintf(p, " dam %d", 40 + (pbasis * 10) + plevdam / 2); break;
			case 40: sprintf(p, " dam %d", 40 + (pbasis * 10) + plevdam); break;
			case 41: sprintf(p, " dam %d", 70 + (pbasis * 15) + plevdam); break;
			case 42: sprintf(p, " dam %d", 65 + (pbasis * 15) + plevdam); break;
			case 43: sprintf(p, " dam %d", 400 + (pbasis * 100) + plevdam * 3); break;
			...
			case 58: strcpy(p, " dur 5+d5"); break;
		}
	}
