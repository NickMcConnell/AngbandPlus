...
bool apply_disenchant(int mode)
{
    ...
    switch (randint(10)) {
	 ...
	 case 9: t = INVEN_LEFT; break;
	 case 10: t = INVEN_RIGHT; break;
    }
...
static bool hates_fire(inven_type *i_ptr)
{
	...
	return (TRUE);
      ...
      case TV_MAGIC_BOOK:
      case TV_PRAYER_BOOK:
	if (i_ptr->sval < SV_BOOK_MIN_GOOD) return TRUE;
	return (FALSE);
...
static int minus_ac(void)
{
    ...
    switch (randint(p_ptr->prace<MIN_DRAGON?6:2)) {
	case 1: i_ptr = &inventory[INVEN_HEAD]; break;
	case 2: i_ptr = &inventory[INVEN_OUTER]; break;
	case 3: i_ptr = &inventory[INVEN_BODY]; break;
	case 4: i_ptr = &inventory[INVEN_ARM]; break;
	case 5: i_ptr = &inventory[INVEN_HANDS]; break;
...
static bool project_m(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	if (r_ptr->rflags3 & RF3_IM_ACID) {
	    note = " resists a lot.";
	    dam /= 9;
	    if (m_ptr->ml) l_ptr->flags3 |= RF3_IM_ACID;
	}
	break;
    ...
    /* Always indicate total lack of effect */
    if (!note && !obvious && !dam) note = " is unaffected!";
    

    /* Give detailed messages if visible or destroyed */
    if (note && (m_ptr->ml || (dam > m_ptr->hp))) {
	message(m_name, 0x03);
	message(note, 0);
    }   
...
#if 0
		/* Hack -- wizards can see everything */
		else if (wizard && panel_contains(y, x)) {
		    drawn = TRUE;
		    print_rel('+', spell_color(typ), y, x);
		}
#endif
