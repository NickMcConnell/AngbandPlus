...
void get_count(int number, int max)
{
	...
	p_ptr->command_arg = get_number(p_ptr->command_arg, max, 0, 10, &cmd);
...
void request_command(bool shopping)
{
	...
	if (cmd == '0')
	{
		...
		/* Actually get a number */
		p_ptr->command_arg = get_number(p_ptr->command_arg, 9999, 0, 7, &cmd);

		/* Handle "zero" */
		if ((cmd != ESCAPE) && (p_ptr->command_arg == 0))
		{
			/* Default to 99 */
