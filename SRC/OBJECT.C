...
int lookup_kind(int tval, int sval)
{
    ...
    /* Oops */
    msg_print(format("No object (%d,%d)", tval, sval));
 ...
 * This entire function is more-or-less a complete hack.
 * Completely ignore the contents of the given object
 */
static bool make_artifact_special(inven_type *i_ptr)
{
	...
	/* don't bother with nonexistant artifacts!!! -ADM */
	if (!v_list[i].level) continue;
...
static void give_1_hi_resist(inven_type *i_ptr)
{
	...
	case 11: i_ptr->flags2 |= TR2_RES_POIS; break;
    }
}
...
static void give_1_lo_resist(inven_type *i_ptr)
{
	...
	case 11: i_ptr->flags3 |= TR3_FEATHER; break;
    }
}
...
static void a_m_aux_2(inven_type *i_ptr, int level, bool okay, bool good, bool great)
{
	...
	if (good || magik(chance)) {
		...
		switch (randint(30)) {
		  ...
		  case 2:
		    i_ptr->flags1 |= (TR1_STEALTH);
		    i_ptr->flags2 |= (TR2_FREE_ACT | TR2_RES_POIS | 
		  ...
		  case 11: case 12:
		    ...
		    /* One in three is also a blessed wis booster */
		    if (rand_int(3) == 0) {
			i_ptr->flags1 |= (TR1_WIS);
			i_ptr->flags3 |= (TR3_BLESSED);
			i_ptr->pval = randint(3);        /* +1 Wisdom */
		    }
		  ...
		  case 13: case 14:
		    ...
		    /* One in two is also a Life Holder */
		    if (rand_int(2) == 0) {
	...
	switch (i_ptr->sval) {
	  ...
	  case SV_AMULET_DOOM:
	    ...
	    /* Permanently broken */
	    i_ptr->ident |= ID_BROKEN;
	    while (randint(2)==1) give_1_hi_resist(i_ptr); /*why not? :)*/
	    i_ptr->flags3 |= TR3_CURSED;
...
void place_object(int y, int x)
{
    ...
    /* Notice "useful" non-cursed out-of-depth objects */
    if (!cursed_p(i_ptr) && !broken_p(i_ptr) &&
	(k_list[tmp].level > dun_level)) {
...
void place_good(int y, int x, bool great)
{
	...
	/* XXX Hack -- Books are never "great" */
	if (great) continue;
	
	/* XXX Hack -- High spell books are good.  Highest is great. */
	if ((tv == TV_MAGIC_BOOK) && (sv >= SV_BOOK_MIN_GOOD)) break;

	/* XXX Hack -- High prayer books are good.  Highest is great. */
	if ((tv == TV_PRAYER_BOOK) && (sv >= SV_BOOK_MIN_GOOD)) break;
    }
