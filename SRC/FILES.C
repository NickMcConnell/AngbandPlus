...
int file_character(cptr filename1)
{
    ...
    if (st_ptr->store_ctr == 0) {
        ...
        for (i = 0; i < st_ptr->store_ctr; i++) {
            if (i == 12) fprintf(fp, "\n");
            objdes(prt2, &st_ptr->store_item[i], 3);
            fprintf(fp, "%c) %s\n", (i%12)+'a', prt2);
        }
    }
...
void process_player_name(bool sf)
{
    ...
    /* Use the whole name */
    strcpy(player_base, player_name);
    
#else

    /* Extract "useful" letters */
    for (i = k = 0; player_name[i]; i++) {
...
static void make_bones(void)
{
    ...
    if (death && !(noscore & 0x0002)) {
        ...
        if (dun_level) {
            ...
            sprintf(str, "%s%sbone.%d", ANGBAND_DIR_BONE, PATH_SEP, dun_level);
