...
static void display_player_middle(void)
{
	...
	/* Heino Vander Sanden */
	prt_num("Max Depth      ",p_ptr->max_depth, 13, 52, TERM_L_GREEN);
}
...
void display_player(int mode)
{
	...
	if ((mode == 0) || (mode == 1))
	{
		...
		put_str("Maximize Mode", 6, 32);
		c_put_str(TERM_L_BLUE, (p_ptr->maximize ? " On" : "Off"), 6, 51);
		put_str("Preserve Mode", 7, 32);
		c_put_str(TERM_L_BLUE, (p_ptr->preserve ? " On" : "Off"), 7, 51);
 ...
 * Heino Vander Sanden
 */
static bool verify_dump(cptr prompt)
{
	char	out_val[160];

	/* Prompt */
	(void)sprintf(out_val, "%s? ", prompt);
...
{
	int i, x, y, z, k, lenln;		/* Heino Vander Sanden */
	...
	/* Dump resistance screen (Heino Vander Sanden)*/
	if(verify_dump("Dump resistance screen"))
	{
		/* Dump various things */
		display_player(2);
	...
	/* Dump Artifacts Seen (Heino Vander Sanden) */
	if (verify_dump("Dump Artifacts seen"))
	{
		/* Scan the artifacts */
		fprintf(fff,"Artifacts Seen \n");
		...
		for (k = 0; k < MAX_A_IDX; k++)
		{
			...
			lenln=lenln+5+strlen(base_name);
			if (lenln>80)
			{
			  fprintf(fff, "\n");
			  lenln=6+strlen(base_name);
			}
			fprintf(fff, " The %s,", base_name);
		}
		fprintf(fff,"\n\n");
		lenln=0;
	}

	/* Dump Uniques (Heino Vander Sanden) */
	if (verify_dump("Dump Uniques killed"))
	{
		/* Scan the monster races */
		fprintf(fff,"Uniques Killed \n");
		for (k = 1; k < MAX_R_IDX-1; k++)
		{
			...
			if (r_ptr->flags1 & (RF1_UNIQUE))
			{
				...
				if (dead)
				{
					lenln=lenln+2+strlen(r_name + r_ptr->name);
					if (lenln>80)
					{
						fprintf(fff, "\n");
						lenln=2+strlen(r_name + r_ptr->name);
					}
					/* Print a message */
					fprintf(fff, " %s;",r_name + r_ptr->name);
				}
			}
		}
		fprintf(fff,"\n\n");
