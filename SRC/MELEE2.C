...
static bool get_moves_aux(int m_idx, int *yp, int *xp)
{
	...
	if ((c_ptr->cost > r_ptr->aaf) && !(m_ptr->smart & SM_FRIEND)) return (FALSE);
...
static bool get_fear_moves_aux(int m_idx, int *yp, int *xp)
{
	...
	if ((cave[fy][fx].cost > r_ptr->aaf) && !(m_ptr->smart & SM_FRIEND)) return (FALSE);
...
void process_monsters(void)
{
	...
	for (i = m_max - 1; i >= 1; i--)
	{
		...
		if ((m_ptr->cdis <= r_ptr->aaf) || (m_ptr->smart && SM_FRIEND))
		{
		...
		/* Note that most monsters have "aaf" of "20" or so */
			 ...
			 ((cave[fy][fx].cost < r_ptr->aaf) || (m_ptr->smart & SM_FRIEND)))
		{
