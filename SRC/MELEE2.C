...
/* do "smart_cheat" once */
void learn_about_player(monster_type *m_ptr)
{
  /* Know basic info */
...
static void remove_bad_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p)
{
	...
	if (m_ptr->status & STATUS_DOMINATE) return;
	...
	if (smart & (SM_RES_CONF))
	{
		...
		if (int_outof(r_ptr, 50)) f4 &= ~RF4_P_BLAST;
		if (int_outof(r_ptr, 30)) f4 &= ~RF4_P_CRUSH;
		if (int_outof(r_ptr, 30)) f6 &= ~RF6_P_WAVE;
		if (int_outof(r_ptr, 50)) f6 &= ~RF6_BR_INSA;
		if (int_outof(r_ptr, 20)) f6 &= ~RF6_M_WRACK;
	}
	...
	/* add SM_RES_FORCE and SM_RES_PSI... */
...
static bool summon_possible(int y1, int x1)
{
    ...
    for (y= y1-2; y<= y1+2; y++)
    {
	    ...
	    if (cave[y][x].feat == FEAT_FORCE_WALL) continue;
...
static bool clean_shot(int y1, int x1, int y2, int x2)
{
   ...
   for (dist = 0; dist <= MAX_RANGE; dist++)
   {
       ...
       {
        if (!(m_list[cave[y][x].m_idx].status & STATUS_DOMINATE))
           break;
        }
...
static bool spell_attack(byte spell)
{
   ...
   if (spell == 160 + 6) return TRUE; /* mind wrack */
   if (spell == 160 + 7) return TRUE; /* psychic wave */
   if (spell == 160 + 15) return TRUE; /* breath insanity */
...
static void breath(int m_idx, int typ, int dam_hp, int rad)
{
	...
	if ((typ != GF_PSI) && (typ != GF_PSI2) && (typ != GF_PSI3))
	  if (p_ptr->inertial_barrier) dam_hp /= 2;
 ...
 static bool monst_spell_monst(int m_idx)
 {
   ...
   if ((m_ptr->status & STATUS_DOMINATE) == (t_ptr->status & STATUS_DOMINATE))  continue;
   ...
   switch (thrown_spell) {
	 ...
	 disturb(1,0);
	 if (seen)
	   msg_format("%^s evokes a blast of psionic energy in %s's direction!",m_name,t_name);
	 monst_breath_monst(m_idx,y,x,damroll(3,rlev),GF_PSI,2);
	 break;
       }
       ...
       case 96+2:
       {
           disturb(1,0);
	   if (seen)
	     msg_format("%^s attmempts to crush %s's psyche!",m_name,t_name);
	   project(m_idx,0,y,x,damroll(3,rlev),GF_PSI2,PROJECT_KILL);
	   break;
       }
       ...
       case 96+29:
   {
       ...
       case 160+6:
       {
           disturb(1,0);
	   if (seen) 
	     msg_format("%^s's eyes glow red as it gazes at %s.",m_name,t_name);
	   project(m_idx,0,y,x,randint(damroll(2,rlev)),GF_PSI3,PROJECT_KILL);
	   break;
       }
   ...
   case 160 + 7:
     {
       disturb(1,0);
       msg_format("%^s casts a wave of psionic energy at %s.",m_name,t_name);
       project(m_idx,0,y,x,damroll(rlev,4), GF_PSI2, PROJECT_BEAM |
	       PROJECT_THRU | PROJECT_KILL);
       break;
       }
   ...
   case 160 + 15:
     {
       disturb(1,0);
       msg_format("%^s breathes insanity at %s.", m_name,t_name);
       breath(m_idx,GF_PSI,min(800,m_ptr->hp / 4),0);
       break;
     }
...
void curse_equipment(int chance, int heavy_chance)
{
	...
	|| ((o4 & TR4_PEACE) && (randint(5000) > chance)))
    {   
        char o_name[256];
...
{
  int                 i,k, chance, thrown_spell, rlev, failrate;
   ...
   bool no_inate = FALSE;
	...
	if (psi_flags & PEACE)
	  if (randint(200) < 25 + adj_dex_safe[p_ptr->stat_use[A_CHR]])
	    return FALSE;
	if (m_ptr->status & STATUS_DOMINATE) return (FALSE);
    ...
    i = (m_ptr->status & STATUS_AMNESIA1 ? 1 : 0) +
	...
	case 96+1:
	  {
	    ...
	    breath(m_idx,GF_PSI,damroll(3,rlev),2);
	  }
	...
	case 96+2:
	  {
		...
		msg_print("You feel something focusing on your mind.");
	      ...
	      {
		msg_format("%^s stares at you ominously.",m_name);
	      }
	    project(m_idx,0,py,px,damroll(3,rlev),GF_PSI2,PROJECT_KILL);
	    break;
	  }
	...
	case 128+9:
	  {
		...
		if ((p_ptr->mental_barrier > 0) && (randint(2) == 1))
		  {
		    msg_format("Your mental barrier prevents %^s from draining your psychic energy.", m_name);
		    break;
		  }

		if ((psi_flags & RES_PSI) && !rand_int(3))
		  {
		    msg_format("You resist %s's attempted mana drain!",m_name);
		    break;
		  }
		...
		case 128+10:
		{
			    ...
			    ((rand_int(100) < p_ptr->skill_sav) && (psi_flags & RES_PSI)))
			{
			  ...
			  if ((p_ptr->mental_barrier > 0) && rand_int(3))
		...
		case 128+11:
		{
			    ...
			    ((rand_int(100) < p_ptr->skill_sav) && (psi_flags & RES_PSI)))
			{
			  ...
			  if ((p_ptr->mental_barrier > 0) && rand_int(2))
			    msg_print("Your mental barrier deflects the attack.");
			  else
			    {
				msg_print("Your mind is blasted by psionic energy.");
	...
	case 160 + 6:
	  {
	    ...
	    project(m_idx,0,py,px,randint(damroll(2,rlev)),GF_PSI3,PROJECT_KILL);
	    break;
	  }
	...
	case 160 + 7:
	  {
	    ...
	    project(m_idx,0,py,px,damroll(rlev,4), GF_PSI2, PROJECT_BEAM |
		    PROJECT_THRU | PROJECT_KILL);
	  }
...

{
    ...
    if ((!stupid_monsters) && !(m_ptr->status & STATUS_DOMINATE))
    {
...
static void process_monster(int m_idx, bool is_friend)
{
	...
	if (m_ptr->status & STATUS_DOMINATE)
	  {
	    ...
	    if (!rand_int(10)) /* compensate for zangband mana costs */ 
	    if ((randint(p_ptr->lev * 1000 + 5000) < randint(rlev))
		|| ((r_info[m_ptr->r_idx].flags1 & RF1_UNIQUE) && !rand_int(200)))
	      {
		...
		if ((rand_int(150) < rlev) || !rand_int(5) ||
		    ((r_info[m_ptr->r_idx].flags1 & RF1_UNIQUE) && rand_int(2)))
		  {
		    ...
		    project(m_idx,0,py,px,damroll(5,rlev),
	...
	if (m_ptr->status & STATUS_AMNESIA)
	  if (((r_info[m_ptr->r_idx].flags1 & RF1_UNIQUE) && !rand_int(40))
	      || !rand_int(400))
	    {
	      /* keep message? */
	      char m_name[80];
	      int tmp;
	      monster_desc(m_name,m_ptr,0);
	      msg_format("%^s recovers some of its memories.",m_name);
	      tmp = ((m_ptr->status & STATUS_AMNESIA1 ? 1 : 0) +
		     (m_ptr->status & STATUS_AMNESIA2 ? 2 : 0));
	      m_ptr->status &= ~STATUS_AMNESIA;
	      if (tmp == 3) m_ptr->mspeed = r_info[m_ptr->r_idx].speed;
	      tmp--;
	      m_ptr->status |= ((tmp & 1) ? STATUS_AMNESIA1 : 0) |
		((tmp & 2) ? STATUS_AMNESIA2 : 0);
	    }
 	 
	if (m_ptr->status & STATUS_EMPATHY)
	  {
	    int lev = r_info[m_ptr->r_idx].level;
	    if (randint(lev * lev) > randint(p_ptr->lev * p_ptr->lev))
	      {
		char m_name[80];
		monster_desc(m_name,m_ptr,0);
		msg_format("%^s stops sharing its feelings.",m_name);
	...
	if (m_ptr->stunned)
	{
		...
		if (r_ptr->flags1 & RF1_UNIQUE)
		  while (randint(15) * randint(20) * randint(3) < r_ptr->level) d += randint(4);		
	...
	/* to do: "psi" domination includes empathy, eliminates this... */
         if ((m_ptr->status & STATUS_DOMINATE) && (p_ptr->aggravate))
            gets_angry = TRUE;

        /* Paranoia... no friendly uniques outside wizard mode -- TY*/
         if ((m_ptr->status & STATUS_DOMINATE) && !(wizard) &&
            (r_ptr->flags1 & (RF1_UNIQUE)))
            gets_angry = TRUE;
		 ...
		 (rand_int(100) < 25))
	{
   ...
   else if (m_ptr->status & STATUS_DOMINATE) {
       ...
       for (i = 1; i < 10; i++)
           ...
           (!(m_list[cave[m_ptr->fy+ddy[i]][m_ptr->fx+ddx[i]].m_idx].status & STATUS_DOMINATE))) {
           /* set direction */
           mm[0] = i;
           break;
           }
           ...
           else if ((ny == py) && (nx == px))
       {
		...
		if (do_move && c_ptr->m_idx)
		{
			...
			if ((r_ptr->flags2 & (RF2_KILL_BODY)) &&
                ...
                !((m_ptr->status & STATUS_DOMINATE) &&  (m2_ptr->status & STATUS_DOMINATE)))
                /* Friends don't kill friends... */
			{
               ...
               /* Attack 'enemies' */
               else if (((m_ptr->status & STATUS_DOMINATE) != (m2_ptr->status & STATUS_DOMINATE))
                        || (m_ptr->confused))
                {
               do_move = FALSE;
               /* attack */
		...
		if (do_move)
		{
			...
			{
				/* Disturb */
                if (!(m_ptr->status & STATUS_DOMINATE) || disturb_pets)
                    disturb(0, 0);
			}
			...
			for (this_o_idx = c_ptr->o_idx; this_o_idx; this_o_idx = next_o_idx)
			{
				    ...
				    (r_ptr->flags2 & (RF2_KILL_ITEM)))
				{
					...
					if (f4 & TR4_PEACE) flg3 |= RF3_EVIL;
					
					/* something for unlife/fury/psi_wpn &? */
