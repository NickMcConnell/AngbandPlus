 ...
 * If you're trying to compile this without the Angband 2.8.2 source,
 * it won't work.
 */
...
s16b randnor(int mean,int stand) {return Rand_normal(mean,stand);}
...
int scramble (void)
{
   ...
   /* No refried artifacts - psi */
	   artifact_type *big_backup = malloc(MAX_A_IDX * sizeof(artifact_type));
      if (!big_backup)
	   {
		  msg_format ("Memory allocation error");
		  return 1;
      }
      memcpy(big_backup,a_info,MAX_A_IDX * sizeof(artifact_type));

   do
	{
		int a_idx;
                memcpy(a_info,big_backup,MAX_A_IDX * sizeof(artifact_type));
...
int scramble_artifact (int a_idx)
{
	...
	/* One last hack: if the artifact is very powerful, raise the rarity.
	   ...
	   kinds. */
        /*
	if ((ap > 0) && ((ap / 8) > a_ptr->rarity)) a_ptr->rarity = ap / 8;
        */
...
void choose_item (int a_idx, u32b activates)
{
	...
	switch (a_ptr->tval)
	{
		...
		case TV_HARD_ARMOR:
			...
			switch (a_ptr->tval)
			{
                        case TV_SOFT_ARMOR: case TV_HARD_ARMOR: /* changed from 2 to 4 */
...
int artifact_power (int a_idx, bool cannot_use_kind_cache)
{
	...
	if (a_ptr->flags3 & TR3_HOLD_LIFE) p += 10;
        if (a_ptr->flags2 & TR2_RES_ACID) p += 5; /* changed from 6 */
        if (a_ptr->flags2 & TR2_RES_ELEC) p += 5;
        if (a_ptr->flags2 & TR2_RES_FIRE) p += 5;
        if (a_ptr->flags2 & TR2_RES_COLD) p += 5;
