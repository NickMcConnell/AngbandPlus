...
static void regenhp(int percent)
{
	...
	{
		if (!rand_int(5)) update_abil_fail(a_regen, 1);
		/* Redraw */
		p_ptr->redraw |= (PR_HP);
...
static void process_world(void)
{
	...
	if (p_ptr->cut)
	{
		...
		/* Take damage */
		take_hit(i, "a fatal wound", a_con);
	}
	...
	if (p_ptr->food < PY_FOOD_STARVE)
	{
		...
		/* Take damage */
		take_hit(i, "starvation", a_slow_digest);
	}
	...
	if (p_ptr->exp_drain)
	{
		if ((rand_int(100) < 10) && (p_ptr->exp > 0))
		{
			p_ptr->exp--;
			p_ptr->max_exp--;
			update_abil_fail(a_remove_drain_exp, 1);
			check_experience();
		}
	}
...
//			}
	...
	/* Alex */
	i = p_ptr->total_weight - weight_limit()/2;
	if (i > 0)
	{
		update_abil_fail(a_low_weight, i/250);
		update_abil_fail(a_str, i/500);
	}
	if (p_ptr->heavy_wield)
	{
		update_abil_fail(a_low_weight, 1);
	...
	if (p_ptr->icky_wield) update_abil_fail(a_blessed, 1);
	if (!p_ptr->cur_lite && !(cave_info[p_ptr->py][p_ptr->px] & CAVE_GLOW)) update_abil_fail(a_lite, 1);
	...
	{
		/* Alex */
		msg_print("You are teleported by your equipment.");
		update_abil_fail(a_remove_teleport, 1);
		/* Teleport player */
		teleport_player(40);
	}
