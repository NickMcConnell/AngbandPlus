...
/*
 * Alex: Output artifact information
 */
static bool describe_artifact(const object_type *o_ptr)
{
	int a_idx = o_ptr->name1;
	artifact_type* a_ptr = &a_info[a_idx];

	if (!a_idx || !(o_ptr->ident & IDENT_MENTAL)) return FALSE;

	text_out(format("Artifact power: <%d>, level: %d, rarity: %d. ", art_power(a_ptr, 0), a_ptr->level, a_ptr->rarity));
	return TRUE;
}
...
bool object_info_out(const object_type *o_ptr)
{
	...
	/* Describe the object */
	if (describe_artifact(o_ptr)) something = TRUE; /* Alex */
