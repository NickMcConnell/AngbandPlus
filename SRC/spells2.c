...
void self_knowledge(void)
{
	...
	if (p_ptr->food >= PY_FOOD_PAIN)
	{
		info[i++] = "You have overeaten.";
	}
	if (p_ptr->food >= PY_FOOD_DEATH)
	{
		info[i++] = "You are bleeding internally.";
	}
	if (p_ptr->blind)
	{
 ...
 *
 * An attempt to balance the weaponry here... -EJB
 *
 * Heavy weapons may be enchanted to +20 to-hit, and light weapons to
 * +5 to-hit. Those that do more damage may be enchanted to +20, while
 * ineffective weapons cannot be enchanted much at all.
 *
...
bool enchant(object_type *o_ptr, int n, int eflag)
{
	...
	/* Heavier weapons can hold more magic */
	max_h = o_ptr->wt * 6 / 50;
	max_h = (max_h < 5) ? 5 : max_h;
	max_h = (max_h > 20) ? 20 : max_h;

	/* Weapons designed for better damage are easier to enhance */
	max_d = maxroll(o_ptr->dd, o_ptr->ds) * 3 / 2;
	max_d = (max_d <= 0) ? max_h : max_d;
	max_d = (max_d > 20) ? 20 : max_d;
	...
	for (i=0; i<n; i++)
	{
		...
		if (eflag & (ENCH_TOHIT))
		{
			...
			else if (o_ptr->to_h > max_h) chance = 1000;
			else chance = enchant_table[o_ptr->to_h * 16 / max_h];
            ...
            /* Curses block enchantment */
            if (cursed_p(o_ptr) && o_ptr->to_h > 0) o_ptr->to_h--;
			}
		}
		...
		if (eflag & (ENCH_TODAM))
		{
			...
			else if (o_ptr->to_d > max_d) chance = 1000;
			else chance = enchant_table[o_ptr->to_d * 16 / max_d];
            ...
            /* Curses block enchantment */
            if (cursed_p(o_ptr) && o_ptr->to_d > 0) o_ptr->to_d--;
			}
		}
		...
		if (eflag & (ENCH_TOAC))
		{
            ...
            /* Curses block enchantment */
            if (cursed_p(o_ptr) && o_ptr->to_a > 0) o_ptr->to_a--;
			}
		}
	}
 ...
 */
bool fire_ball(int typ, int dir, int dam, int rad, bool breath)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
 ...
 * Affect grids, objects, and monsters
 */
bool shower(int typ, int dir, int dam, int rad)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
	...
	else
	{
		...
		for (dist = 0; dist <= MAX_RANGE; dist++)
		{
			...
			if (dist && cave_m_idx[y][x] > 0) break;
...
bool sanctuary(void)
{
	...
	return (project(-1, 1, py, px, 0, GF_MAKE_SANC, flg));
}
