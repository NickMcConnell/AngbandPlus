...
bool set_blind(int v)
{
	...
	/* Alex */
	if (v > p_ptr->blind) update_abil_fail(a_res_blind, (v - p_ptr->blind));
...
bool set_confused(int v)
{
	...
	/* Alex */
	if (v > p_ptr->confused) update_abil_fail(a_res_conf, (v - p_ptr->confused));
...
bool set_poisoned(int v)
{
	...
	else
	{
		if (p_ptr->poisoned)
		{
			...
			/* Alex */
			update_act_fail(ACT_REM_FEAR_POIS, 20);
			notice = TRUE;
		}
	}

	/* Alex */
	if (v > p_ptr->poisoned) update_abil_fail(a_res_pois, v - p_ptr->poisoned);
...
bool set_afraid(int v)
{
	...
	else
	{
		if (p_ptr->afraid)
		{
			...
			/* Alex */
			update_act_fail(ACT_REM_FEAR_POIS, 10);
			notice = TRUE;
		}
	}

	/* Alex */
	if (v > p_ptr->afraid) update_abil_fail(a_res_fear, (v - p_ptr->afraid)*10);
...
bool set_paralyzed(int v)
{
	...
	/* Alex */
	if (v > p_ptr->paralyzed) update_abil_fail(a_free_act, (v - p_ptr->paralyzed)*20);
...
bool set_fast(int v)
{
	...
	if (v > p_ptr->fast)
	{
		update_abil_fail(a_speed, (v - p_ptr->fast)/5);
		update_act_fail(ACT_HASTE1, 1);
		update_act_fail(ACT_HASTE2, 1);
	}
	/* Use the value */
	p_ptr->fast = v;
...
bool set_blessed(int v)
{
	...
	if (v)
	{
		if (!p_ptr->blessed)
		{
			...
			notice = TRUE;
			/* Alex */
			update_act_fail(ACT_RAGE_BLESS_RESIST, 1);
		}
	}
...
bool set_shero(int v)
{
	...
	if (v)
	{
		if (!p_ptr->shero)
		{
			...
			/* Alex */
			update_act_fail(ACT_RAGE_BLESS_RESIST, 1);
			update_act_fail(ACT_BERSERKER, 1);
			notice = TRUE;
		}
	}
...
bool set_protevil(int v)
{
	...
	if (v)
	{
		if (!p_ptr->protevil)
		{
			...
			/* Alex */
			update_act_fail(ACT_PROT_EVIL, 1);
			notice = TRUE;
		}
	}
...
bool set_tim_invis(int v)
{
	...
	/* Alex */
	if (v > p_ptr->tim_invis) update_abil_fail(a_see_invis, (v - p_ptr->tim_invis));
...
bool set_tim_infra(int v)
{
	...
	/* Alex */
	if (v > p_ptr->tim_infra) update_abil_fail(a_infra, (v - p_ptr->tim_infra));
...
bool set_food(int v)
{
	...
	{
		new_aux = 0;
		update_abil_fail(a_slow_digest, 100);
	}
	...
	{
		new_aux = 1;
		update_abil_fail(a_slow_digest, 50);
	}
	...
	{
		new_aux = 2;
		update_abil_fail(a_slow_digest, 10);
	}
...
void lose_exp(s32b amount)
{
	...
	update_abil_fail_dam(a_hold_life, amount/100 + 1);
...
bool mon_take_hit(int m_idx, int dam, bool *fear, cptr note)
{
	...
	if (m_ptr->hp < 0)
	{
		...
		/* Was: Player level */
		/* Alex: maximum player level (due to Create Artifact spell) */
		div = p_ptr->max_lev;
