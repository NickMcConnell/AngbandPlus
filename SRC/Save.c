...
/*
 * Write a player-class record
 */
static void wr_pclass(void)
{
	int i;
	player_class *p_class = &(p_ptr->pl_class);

	/* Type of class */
	wr_string(p_class->title);

	/* Class stat modifier */
	for (i = 0; i < 6; i++)
	{
		wr_s16b(p_class->c_adj[i]);
	}

	wr_s16b(p_class->c_dis); /* class disarming */
	wr_s16b(p_class->c_dev); /* class magic devices */
	wr_s16b(p_class->c_sav); /* class saving throws */
	wr_s16b(p_class->c_stl); /* class stealth */
	wr_s16b(p_class->c_srh); /* class searching ability */
	wr_s16b(p_class->c_fos); /* class searching frequency */
	wr_s16b(p_class->c_thn); /* class to hit (normal) */
	wr_s16b(p_class->c_thb); /* class to hit (bows) */

	wr_s16b(p_class->x_dis); /* extra disarming */
	wr_s16b(p_class->x_dev); /* extra magic devices */
	wr_s16b(p_class->x_sav); /* extra saving throws */
	wr_s16b(p_class->x_stl); /* extra stealth */
	wr_s16b(p_class->x_srh); /* extra searching ability */
	wr_s16b(p_class->x_fos); /* extra searching frequency */
	wr_s16b(p_class->x_thn); /* extra to hit (normal) */
	wr_s16b(p_class->x_thb); /* extra to hit (bows) */

	wr_s16b(p_class->c_mhp); /* Class hit-dice adjustment */
	wr_s16b(p_class->c_exp); /* Class experience factor */
}
