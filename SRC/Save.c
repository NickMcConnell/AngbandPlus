...
static void wr_item(const object_type *o_ptr)
{
	...
	wr_s32b(o_ptr->name1);/* Alex: was wr_byte */
	wr_byte(o_ptr->name2);
...
static void wr_extra(void)
{
	...
	wr_u32b(PATCH_NEW_ART);
	/* Alex: new artifact version */
	wr_u32b(VERSION_NEW_ART);
...
static void wr_randarts(void)
{
	...
	for (i = 0; i < z_info->a_max; i++)
	{
		...
		/* Alex: write name and text for new artifacts*/
		if (i >= file_a_max)
		{
			wr_u32b(a_ptr->name);
			wr_u32b(a_ptr->text);
...
/*
 * Alex: dump artifact probabilities info
 */
static void wr_fails(void)
{
	u16b a;
...
static bool wr_savefile_new(void)
{
	...
	/* Alex: always write all artifacts as they could be changed */
	wr_randarts();

	/* Alex: write abilities/activation probabilities */
	wr_fails();
