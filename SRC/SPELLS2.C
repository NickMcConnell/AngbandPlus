...
void set_recall(void)
{
	...
	if (!p_ptr->word_recall)
	{
		/* D11-Hack: Ask for and set the recall depth */
		if (!p_ptr->depth)
			p_ptr->recall_depth = input_recall_depth();
		/* D11-Hack: Continue the recall */
...
bool genocide(void)
{
	...
	/*
	 * D11-Hack: The Genocide is extended by dumping a complete Char/Monster
	 * (Part I)  List (external routine) on a blank screem to simplify the
	 *           input of the asked char.
	 */
	/* Save the screen and clear the term */
	screen_save();
	Term_clear();
	extend_genocide();
	/* End of D11-Hack Part I */
	...
	/* D11-Hack Part II: Restore the screen */
	screen_load();
	/* End of D11-Hack Part II */
