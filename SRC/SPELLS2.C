...
bool genocide(void)
{
	...
	for (i = 1; i < m_max; i++)
	{
		...
		/* Skip Quest Monsters (Heino Vander Sanden) */
		if (m_ptr->r_idx == get_quest_monster()) continue;
 ...
 * (Heino Vander Sanden)
 */
void make_rubble(int xx, int yy, int chance)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
	...
	int x, y, i, sn = 0, sx = 0, sy = 0, damage;

	if (!in_bounds_fully(yy, xx)) return FALSE;
	...
	if (cave_floor_bold(yy, xx))
	{
		...
		if (hurt)
		{
			...
			/* Take some damage */
			if (damage) take_hit(damage, "the ceiling");
		}
		...
		/* Create granite rubble */
		cave_feat[yy][xx] = FEAT_RUBBLE;

		for (i = 0; i < 8; i++)
		{
			make_rubble(xx + ddx_ddd[i], yy + ddy_ddd[i], chance / 2);
		}
	}
}
...
void collapse(int dir)
{
	...
	if (p_ptr->depth == 0)
	{
		msg_print("There is noting you can collapse!");
		return;
	}
