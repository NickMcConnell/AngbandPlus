...
static void process_world(void)
{
	...
	{
		/* Protection */
		s32b sp = (p_ptr->pspeed < 0) ? 0 : ((p_ptr->pspeed > 200) ? 200 : p_ptr->pspeed);

		sp = sp + 30;

		/* Adjust for hunger level */
		...
		if (p_ptr->food < PY_FOOD_PAIN) sp -= 10;
		if (p_ptr->food < PY_FOOD_MAX) sp -= 10;

		/* Basic digestion rate based on speed */
		i = extract_energy[sp] * 2;
		...
		if (p_ptr->food - i < 0) i = p_ptr->food;
	...
	/* Very uncomfortable */
	if ((p_ptr->food >= PY_FOOD_PAIN) && (p_ptr->food < PY_FOOD_DEATH))
	{
		/* Calculate damage */
		i = rand_int((p_ptr->food - PY_FOOD_PAIN) / 1000 + 2);

		if (!(rand_int(PY_FOOD_DEATH - p_ptr->food) / 100))
		{
			msg_print("You vomit!");
			(void)set_food((PY_FOOD_DEATH - p_ptr->food) / 2 + PY_FOOD_STARVE);
			(void)set_paralyzed(p_ptr->paralyzed + 4);
		}

		/* Take damage */
		take_hit(i, "overeating");
	}

   /* Digestive problems ;) */
	if (p_ptr->food >= PY_FOOD_DEATH)
	{
		i = (p_ptr->mhp - p_ptr->chp) / 100 + rand_int(5);

		/* Take damage */
		take_hit(i, "internal bleeding");
	}
   ...
   if (p_ptr->food >= PY_FOOD_DEATH) regen_amount = 0;
	...
	for (j = 0, i = INVEN_WIELD; i < INVEN_TOTAL; i++)
	{
		...
		if ((o_ptr->timeout > 0) &&
			 ((o_ptr->tval != TV_BOW) || (o_ptr->sval < SV_LIGHT_XBOW)))
		{
			/* Recharge */
			o_ptr->timeout--;
	...
	/* Recharge rods and decay corpses */
	for (k = 0, j = 0, i = 0; i < INVEN_PACK; i++)
	{
		o_ptr = &inventory[i];
		...
		if (decays(o_ptr))
		{
			...
			if ((o_ptr->timeout > 0) && o_ptr->timeout < o_ptr->wt) o_ptr->timeout--;
...
static void process_player_aux(void)
{
	...
	if (p_ptr->monster_race_idx)
	{
		...
		l_ptr = &l_info[p_ptr->monster_race_idx];
...
static void process_player(void)
{
	...
	p_ptr->energy += extract_energy[sp];
...
static void dungeon(void)
{
	...
	while (TRUE)
	{
		...
		/* Hack -- Compress the random artifact list occasionally */
		if (a_cnt + 32 > MAX_A_IDX) compact_artifacts();

		/* Hack -- Compress the random artifact list occasionally */
		if (a_cnt + 32 < a_max) compact_artifacts();
...
void play_game(bool new_game)
{
	...
	/* Attempt to protect against failed saves */
	while (p_ptr->playing)
	{
		/* Process */
		while (TRUE)
