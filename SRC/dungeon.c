...
static void process_world(void)
{
	...
	/* D11-Hack: Count the Ability-Turns */
	update_abilities();
...
/* D11-Lesezeichen: WOR */
		...
		if (!p_ptr->word_recall)
		{
			...
			else
			{
				...
				/* New depth */
				/* D11-Hack: the depth is set to the specified recall-depth */
...
static void process_command(void)
{
	...
	switch (p_ptr->command_cmd)
	{
		...
		/* D11-Hack: Use a special Drow-Power */
		case 'U':
		{
			do_cmd_use_drowpower();
			break;
		}
		...
		/* D11-Hack: Configure Quick Spells */
		case KTRL('Q'):
		{
			configure_quick_spells();
			break;
		}
...
static void dungeon(void)
{
	...
	/* Update stuff , D11-Hack: added Train and Abilities*/
	p_ptr->update |= (PU_BONUS | PU_HP | PU_MANA | PU_SPELLS | PU_TRAIN | PU_ABILITIES);
