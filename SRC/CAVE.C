 ...
 * cave_clean_bold and cave_naked_bold.
 */
...
void map_info(int y, int x, byte *ap, char *cp)
{
	...
	else
	{
		...
		if (cave_info[y][x] & (CAVE_MARK))
		{
			...
			/* Special lighting effects */
			if (view_granite_lite && (a == TERM_WHITE) && (is_wall(feat)))
			{
				/* Handle "blind" */
				if (p_ptr->blind)
				{
	...
	/* Objects in water are actually *under* the water */
	if (is_liquid(feat))
	{
		/* Find the color of water */
		f_ptr = &f_info[feat];
...
static void update_flow_aux(int y, int x, int n)
{
	...
	if (is_wall(cave_feat[y][x])) return;
...
void wiz_lite(void)
{
	...
	for (y = 1; y < DUNGEON_HGT-1; y++)
	{
		...
		{
			/*
			 * Process all non-walls unless the player is a wizard,
			 * in which case we process everything.
			 */
			if (!is_wall(cave_feat[y][x]) || p_ptr->wizard)
			{
				/* Scan all neighbors */
				for (i = 0; i < 9; i++)
				{
