...
static void describe_monster_exp(int r_idx, const monster_lore *l_ptr)
{
	...
	if (l_ptr->tkills)
	{
		...
		/* Alex: use p_ptr->max_lev instead of p_ptr->lev due to Create Artifact spell/prayer */
		i = (long)r_ptr->mexp * r_ptr->level / p_ptr->max_lev;

		/* calculate the fractional exp part scaled by 100, */
		/* must use long arithmetic to avoid overflow */
		j = ((((long)r_ptr->mexp * r_ptr->level % p_ptr->max_lev) *
