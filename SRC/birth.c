	...
	while (TRUE)
	{
		...
		while (autoroll)
		{
			...
			if (flag || (auto_round < last_round + 100))
			{
				...
				/* Delay 1/10 second if the stats are ridiculous */
				if (flag && (auto_round>25000)) Term_xtra(TERM_XTRA_DELAY, 100);
      ...
      /* Be nice to the players that pick reasonable stats */
		if (auto_round <= 25000) auto_round = 0;
