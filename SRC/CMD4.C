...
static void do_cmd_knowledge_quests(void)
{
	...
	for (i = 0; i < MAX_Q_IDX; i++)
	{
		...
		r_ptr = &r_info[q_list[i].r_idx];
		name = (r_name + r_ptr->name);

		if (q_list[i].max_num == 1)
			fprintf(fff, "On level %d you have killed %s \n",q_list[i].level, name);
		else
			fprintf(fff, "On level %d you have killed %d %ss \n",q_list[i].level,
							q_list[i].max_num, name);
	}
	...
	/* Display the file contents */
	show_file(file_name, "Finished Quests", 0, 0);
...
void do_cmd_knowledge(void)
{
	...
	while (1)
	{
		...
		prt("(4) Display finished quests", 7, 5);
