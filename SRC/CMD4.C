...
void do_cmd_redraw(void)
{
	...
	/* Update stuff */
	p_ptr->update |= (PU_BONUS | PU_HP | PU_MANA | PU_SPELLS | PU_ABILITIES);
	...
	while (TRUE)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			sprintf(buf, "%-48s: %s  (%s)",
			        ...
			        option_text[opt[i]]);
			/* D11-Hack: No rogue-like commands */
			if (opt[i] == OPT_rogue_like_commands)
			sprintf(buf, "%-48s: n/a  (not available)",
			        option_desc[opt[i]],
			        option_text[opt[i]]);
			c_prt(a, buf, i + 2, 0);
		}
		...
		switch (ch)
		{
			...
			case '5':
			{
				...
				/* D11-Hack: No rogue-like commands */
				if (opt[k] == OPT_rogue_like_commands)
