 ...
 * out, etc.).  Shuffle store owners.  Maintain and initialize stores.
 * Move an object stolen by an AoBM to the black market.
 *
...
#define MAX_COMMENT_2B  17

static cptr comment_2b[MAX_COMMENT_2B] =
{
	"Ha!  No less than %s.",
	"This is an insult!  Try %s gold pieces.",
	"I can take no less than %s gold pieces.",
	"%s?!?  Go try Londis instead!",
	...
	"I paid more than %s for it myself!",
	...
	"Your mother was a Troll!  %s or I'll tell!",
	"May you meet a cult of Troll Priests!  I'm saying %s gold pieces."
};

#define MAX_COMMENT_3A  6
...
{
	msg_print(comment_4[rand_int(MAX_COMMENT_4)]);
}
...
static byte rgold_adj[MAX_RACES][MAX_RACES] =
{
	...
	/* Ent: Theoretical, copied from elf */
	{ 110, 105, 100, 105, 110, 120, 125, 130, 110, 100, 110,
	  120, 120, 105, 120, 110, 105, 125, 125, 110, 115, 108,
	  120, 115, 110, 110, 120, 110, 100, 125},
...
static void mass_produce(object_type *o_ptr)
{
	...
	else if (rand_int(50 - luck()) == 0)
	{
		discount = 25;
	}
	else if (rand_int(150 - luck() * 2) == 0)
	{
		discount = 50;
	}
	else if (rand_int(300 - luck() * 4) == 0)
	{
		discount = 75;
	}
	else if (rand_int(500 - luck() * 5) == 0)
	{
		discount = 90;
	}
...
static void store_create(void)
{
	...
	for (tries = 0; tries < 4; tries++)
	{
		...
		/* Mega-Hack -- no corpses in stores */
		if (q_ptr->tval == TV_CORPSE) continue;
		...
		if (cur_store_num == 6)
		{
			...
			/* Hack -- No "cheap" items */
			if (object_value(q_ptr) < 100) continue;
...
static void display_entry(int pos)
{
	...
	if (cur_store_num == STORE_HOME)
	{
		...
		if (show_weights)
		{
			...
			if (metric) wgt *= 0.5;
			(void)sprintf(out_val, "%3d.%d %s", wgt / 10, wgt % 10, (metric) ? "kg" : "lb");
...
static void display_store(void)
{
	...
	else
	{
		...
		sprintf(buf, "%s (%ld)", store_name, ot_ptr->max_cost);
		prt(buf, 3, 50);
...
{
	/* Increase insults */
	if (p_ptr->sign == SIGN_GEMINI && (randint(3) == 1))
	st_ptr->insult_cur++;
	else
	st_ptr->insult_cur++;
...
static void store_sell(void)
{
	...
	/* Prepare a prompt */
	if (cur_store_num == STORE_HOME) q = "Drop which item? ";
