...
/*
 * Hack -- Check if store acts as a home
 */
bool store_home(int n)
{
        town_type *t_ptr = &t_info[p_ptr->town];

	/* Hack -- The non-existant stores act like the home */
        if (!t_ptr->store[n].d_char) return (TRUE);

	/* The "home" acts like the home */
        if (t_ptr->store[n].d_char == '8') return (TRUE);

	/* The "special locations" act like the home */
        if ((t_ptr->store[n].d_char < '0') || (t_ptr->store[n].d_char > '9')) return (TRUE);
...
static bool store_will_buy(object_type *o_ptr)
{
	...
	if (store_home(store_num)) return (TRUE);
...
static bool black_market_crap(object_type *o_ptr)
{
	...
	for (i = 0; i < MAX_STORES; i++)
	{
		...
		if (i == STORE_B_MARKET || store_home(i))
		  continue;
...
#if 0
		/* Hack -- warn player */
		msg_format("Your home town is %s",t_name+t_ptr->name);

		/* Confirm */
		if (!get_choice("Make this town your home?")) return;
#endif
		/* Change the home */
...
void store_shuffle(int which)
{
	int i, j;
        ...
        /* Not a store */
        if (!t_ptr->store[which].d_char) return;


	/* Ignore home */
	if (store_home(which)) return;
...
void store_maint(int which)
{
	...
	/* Ignore home */
	if (store_home(store_num)) return;
...
void store_init(int which)
{
	...
	/* Stock special locations */
        if ((t_ptr->store[store_num].d_char < '0') || (t_ptr->store[store_num].d_char > '9'))
	{
		/* Create 4 objects here */
		store_create();
		store_create();
		store_create();
		store_create();
	...
	{
		/* Maintain the store 10 times over */
                for (k=0;k<10;k++) store_maint(which);
