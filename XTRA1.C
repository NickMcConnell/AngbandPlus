...
static void prt_poisoned(void)
{
    if (p_ptr->nasty_pois)
    {
        c_put_str(TERM_L_DARK, "Poisoned", ROW_POISONED, COL_POISONED);
    }
...
static void health_redraw(void)
{
    ...
    else
    {
        ...
        /*  Customized monster health bar -- colorless fear/sleep status */
         ...
         const char hb_sleeping[] = "ZZZZZZZZZZ";
         const char hb_poisoned[] = "PPPPPPPPPP";
         const char hb_stunned[] =  "SSSSSSSSSS";
         const char hb_invuln[]  =  "IIIIIIIIII";
         const char hb_confus[]  =  "CCCCCCCCCC";
        ...
        /* Confused */
        if (m_ptr->confused){ attr = TERM_UMBER; hb_ptr = hb_confus; }

        /* Invuln */
        if (m_ptr->invuln) { attr = TERM_L_DARK; hb_ptr = hb_invuln; }

        /* poisoned */
        if (m_ptr->poisoned) { attr = TERM_L_UMBER; hb_ptr = hb_poisoned; }
...
static void calc_mana(void)
{
    ...
    /* Only mages are affected. Dragons can't wair gloves! */
    if (mp_ptr->spell_book == TV_MAGIC_BOOK && p_ptr->prace < RACE_MIN_DRAGON)
    {
        u32b f1, f2, f3;
    ...
    {
        cur_wgt += inventory[INVEN_D_OUTER].weight;
        cur_wgt += inventory[INVEN_D_HEAD].weight;
    }
...
static void calc_bonuses(void)
{
    ...
    /* -TM- Save old cursed status */
    old_tmcursed = p_ptr->tmcursed;
    ...
    p_ptr->resist_acid = 100;
    p_ptr->resist_elec = 100;
    p_ptr->resist_fire = 100;
    p_ptr->resist_cold = 100;
    p_ptr->resist_pois = 100;
    p_ptr->resist_fear = 100;
    p_ptr->resist_lite = 100;
    p_ptr->resist_dark = 100;
    p_ptr->resist_blind = 100;
    p_ptr->resist_confu = 100;
    p_ptr->resist_sound = 100;
    p_ptr->resist_chaos = 100;
    p_ptr->resist_disen = 100;
    p_ptr->resist_shard = 100;
    p_ptr->resist_nexus = 100;
    p_ptr->resist_nethr = 100;
    ...
    /* Elf */
    if (p_ptr->prace == RACE_ELF) p_ptr->resist_lite /= 2;
    ...
    /* Dwarf */
    if (p_ptr->prace == RACE_DWARF) p_ptr->resist_blind /= 2;

    /* Half-Orc */
    if (p_ptr->prace == RACE_HALF_ORC) p_ptr->resist_dark /= 2;
    ...
    /* High Elf */
    if (p_ptr->prace == RACE_HIGH_ELF) p_ptr->resist_lite /= 2;
    if (p_ptr->prace == RACE_HIGH_ELF) p_ptr->see_inv = TRUE;

    /* Pixie */
    if (p_ptr->prace == RACE_PIXIE) p_ptr->pspeed += 10;
    if (p_ptr->prace == RACE_PIXIE) p_ptr->ffall = TRUE;
    if (p_ptr->prace == RACE_PIXIE && p_ptr->lev > 40) p_ptr->telepathy = TRUE;
    ...
    if (p_ptr->prace == RACE_CRYSTALDRAG) p_ptr->to_a += p_ptr->lev + 35;
    if (p_ptr->prace == RACE_CRYSTALDRAG) p_ptr->dis_to_a += p_ptr->lev + 35;
    ...
    if (p_ptr->prace == RACE_COPPERDRAG) p_ptr->dis_to_a += p_ptr->lev + 10;
    ...
    if (p_ptr->prace == RACE_BRONZEDRAG) p_ptr->dis_to_a += p_ptr->lev + 10;
    ...
    if (p_ptr->prace == RACE_GOLDDRAG) p_ptr->dis_to_a += p_ptr->lev + 10;

    /* Pseudo Dragon */
    if (p_ptr->prace == RACE_PSEUDODRAG) p_ptr->ffall = TRUE;
    if (p_ptr->prace == RACE_PSEUDODRAG) p_ptr->resist_lite /= 2;
    if (p_ptr->prace == RACE_PSEUDODRAG) p_ptr->resist_dark /= 2;
    if (p_ptr->prace == RACE_PSEUDODRAG) p_ptr->resist_blind /= 2;
    if (p_ptr->prace == RACE_PSEUDODRAG) p_ptr->resist_fear /= 2;
    ...
    if (p_ptr->prace == RACE_PSEUDODRAG && p_ptr->lev >= 50) p_ptr->invis += 1;
    ...
    {
        if (p_ptr->lev >= 30) p_ptr->resist_fear /= 2;
    }
    ...
    for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
    {
        ...
        if (f3 & (TR3_INVIS)) p_ptr->invis ++;
        if (f3 & (TR3_2HANDS)) p_ptr->to_h += 10;
        if (f3 & (TR3_2HANDS)) p_ptr->to_d += 5;
        if (f3 & (TR3_2HANDS)) extra_blows += 1;
        ...
        /* Resistance flags */
        if (f2 & (TR2_RES_ACID)) p_ptr->resist_acid /= 2;
        if (f2 & (TR2_RES_ELEC)) p_ptr->resist_elec /= 2;
        if (f2 & (TR2_RES_FIRE)) p_ptr->resist_fire /= 2;
        if (f2 & (TR2_RES_COLD)) p_ptr->resist_cold /= 2;
        if (f2 & (TR2_RES_POIS)) p_ptr->resist_pois /= 2;
        if (f2 & (TR2_RES_FEAR)) p_ptr->resist_fear /= 2;
        if (f2 & (TR2_RES_LITE)) p_ptr->resist_lite /= 2;
        if (f2 & (TR2_RES_DARK)) p_ptr->resist_dark /= 2;
        if (f2 & (TR2_RES_BLIND)) p_ptr->resist_blind /= 2;
        if (f2 & (TR2_RES_CONFU)) p_ptr->resist_confu /= 2;
        if (f2 & (TR2_RES_SOUND)) p_ptr->resist_sound /= 2;
        if (f2 & (TR2_RES_SHARD)) p_ptr->resist_shard /= 2;
        if (f2 & (TR2_RES_NEXUS)) p_ptr->resist_nexus /= 2;
        if (f2 & (TR2_RES_NETHR)) p_ptr->resist_nethr /= 2;
        if (f2 & (TR2_RES_CHAOS)) p_ptr->resist_chaos /= 2;
        if (f2 & (TR2_RES_DISEN)) p_ptr->resist_disen /= 2;
        ...
        if (f3 & (TR3_2HANDS)) p_ptr->dis_to_h += 10;
        if (f3 & (TR3_2HANDS)) p_ptr->dis_to_d += 5;
    ...
    if (p_ptr->oppose_acid) p_ptr->resist_acid /= 2;
    if (p_ptr->oppose_elec) p_ptr->resist_elec /= 2;
    if (p_ptr->oppose_fire) p_ptr->resist_fire /= 2;
    if (p_ptr->oppose_cold) p_ptr->resist_cold /= 2;
    if (p_ptr->oppose_pois) p_ptr->resist_pois /= 2;
    if (p_ptr->protevil)    p_ptr->resist_nethr /= 2;
    ...
    if (o_ptr->k_idx)
    {
        ...
        if (o_ptr->k_idx && !p_ptr->heavy_shoot)
        {
            ...
            /* -TM- Warriors _need_ extra shots */
            if (p_ptr->pclass == CLASS_WARRIOR)
            {
                /* Extra shot at level 30 regardless of bow type */
                if (p_ptr->lev >= 30) p_ptr->num_fire++;
            }

            /* -TM- Rogues are good with slings */
          ...
          (p_ptr->prace >= RACE_MIN_DRAGON))
    {
        ...
        {
            /* Warrior */
            case CLASS_WARRIOR: num = 5; wgt = 30; mul = 5; break;
            ...
            /* Ranger */
            case CLASS_RANGER:  num = 4; wgt = 35; mul = 4; break;
    ...
    /* -TM- TMCursed Change */
    if (p_ptr->tmcursed != old_tmcursed)
    {
        if (p_ptr->tmcursed)
        {
        ...
        {
            msg_print("You feel the curse lifting...");
        }
    }
