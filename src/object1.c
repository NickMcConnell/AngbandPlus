...
cptr item_activation(const object_type *o_ptr)
{
   ...
   if (o_ptr->activate == 0) return ("nothing");

   if (o_ptr->activate != 0)
   {
   	s_ptr = &s_info[o_ptr->activate];
      return (act_desc( p_ptr->px, p_ptr->py, o_ptr->activate,
                            0, 0, o_ptr->pval2, 0, 1, 1, 0, 0));
   }
...
/*
 * Anything containing a spell
 */
bool item_tester_hook_has_spell(const object_type *o_ptr)
{
   int   i,j;
   i = 0; j = 0;
   for (i = 0; i < 8; i++)
