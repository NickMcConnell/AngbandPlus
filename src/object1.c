...
cptr item_activation(const object_type *o_ptr)
{
	...
	if (o_ptr->activate < 128)
	{
		switch (o_ptr->activate)
		{
			...
			case ACT_AURA_FIRE:
			{
				return "flaming aura (dur 50+d50) every 300 turns";
			}
			case ACT_AURA_ELEC:
			{
				return "electric aura (dur 50+d50) every 300 turns";
			}
			case ACT_FREE_FALL:
			{
				return "flying + freedom (dur 100+level) every 200+d200 turns";
			}
			case ACT_INVISIBLE:
			{
				return "invisibility (dur 25+d(level)) every 400 turns";
			}
			case ACT_MAGIC_STORM:
			{
				return "resist magic(level*3) + mana storm(500) every 550 turns";
			}
			case ACT_CONFUSE:
			{
	...
	else
	{
		...
		switch (o_ptr->activate - 128)
		{
			...
			case ART_GRENADE:
			{
				return "holy explosion (500) every 250 turns";
			}
			case ART_INGWE:
			{
...
//	}
	...
	if (f1 & (TR1_SAV))
	{
		info[i++] = "It affects your saving throws.";
	}
	if (f1 & (TR1_SEARCH))
	{
	...
	if (f2 & (TR2_RES_MAGIC))
	{
		info[i++] = "It provides magic resistance.";
	}
	...
	if (f3 & (TR3_SH_COLD))
	{
		info[i++] = "It produces an icy shield.";
	}
	if (f3 & (TR3_SH_ACID))
	{
		info[i++] = "It produces an acidic bubble.";
	}
	if (f3 & (TR3_NO_MAGIC))
	{
	...
	{
		info[i++] = "It does extra damage with magic.";
	}
	if (f4 & (TR4_BRAND_SLEEP))
	{
		info[i++] = "It puts your enemies to sleep.";
	}
	if (f4 & (TR4_CHAOTIC))
	{
	...
	if (f4 & (TR4_XTRA_MIGHT))
	{
		info[i++] = "It fires missiles with extra might.";
	}
	if (f4 & (TR4_XTRA_SHOTS))
	{
		info[i++] = "It fires missiles excessively fast.";
	}
	if (f4 & (TR4_XTRA_FORCE))
	{
		if (o_ptr->sval == SV_SLING)
		{
			info[i++] = "Its missiles can stun your foes.";
		}
		else
		{
			info[i++] = "Its missiles can go straight through you foes.";
		}
	}
...
cptr mention_use(int i)
{
	...
	if (i == INVEN_WIELD)
	{
		object_type *o_ptr;
		o_ptr = &inventory[i];
		if (adj_str_hold[change_form(A_STR)] < o_ptr->weight / 10)
		{
			p = "Just lifting";
		}
	}
...
void display_inven(void)
{
	...
	for (i = 0; i < z; i++)
	{
		...
		if (o_ptr->timeout && o_ptr->tval != TV_WAND && o_ptr->tval != TV_STAFF && 
