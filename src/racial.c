...
static int racial_aux(power_desc_type *pd_ptr)
{
	...
	if ((pd_ptr->number >= -10) && (pd_ptr->number <= -4))
	{
		...
		{
#ifdef JP
			msg_format("この能力を使用するにはクラスレベル %d に達していなければなりません。", min_level);
#else
...
void do_cmd_racial_power(void)
{
	...
	case CLASS_LORD:
	{
		...
		power_desc[num++].number = -6;
#ifdef JP
		strcpy(power_desc[num].name, "増援要請");
#else
		strcpy(power_desc[num].name, "Summon Pumpkin-Heads");
#endif
	...
	case CLASS_GENERAL:
	{
		...
		power_desc[num++].number = -6;
#ifdef JP
		strcpy(power_desc[num].name, "音速剣");
#else
	...
	case CLASS_FREYA:
	{
		...
		power_desc[num++].number = -6;
#ifdef JP
		strcpy(power_desc[num].name, "スパイラル");
#else
	...
	case CLASS_VAMPIRE:
	{
		...
		power_desc[num++].number = -5;
#ifdef JP
		strcpy(power_desc[num].name, "チャーム");
#else
		...
		power_desc[num++].number = -6;
#ifdef JP
		strcpy(power_desc[num].name, "影の扉");
#else
