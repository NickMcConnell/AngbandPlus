...
/* randart.c: randart generator */

#include "posband.h"

#include "init.h"

#define randart_verbose 0
 ...
 * Updated for PosBand 0.3.0 by Alexander Ulyanov (Sep 30 2004).
 *
...
static long eval_max_dam(int r_idx)
{
	 ...
	 */
	/* -AU- Updated for NPP/Pos flags */

	if ((rptr->flags4 & RF4_LASH) && dam < rptr->blow[0].d_dice * rptr->blow[0].d_side / 2)
		dam = rptr->blow[0].d_dice * rptr->blow[0].d_side / 2;
	if ((rptr->flags4 & (RF4_BOULDER | RF4_SHOT | RF4_ARROW | RF4_BOLT)) && dam < spower * 4)
		dam = spower * 4;
	if ((rptr->flags4 & (RF4_MISSL | RF4_PMISSL)) && dam < spower * 3)
		dam = spower * 3;
	/* Assume single resist for the elemental attacks */
	breath_dam = ((hp / 3) > 1600 ? 533 : (hp / 9));
	if ((rptr->flags4 & RF4_BRTH_ACID) && dam < breath_dam) dam = breath_dam;
	if ((rptr->flags4 & RF4_BRTH_ELEC) && dam < breath_dam) dam = breath_dam;
	if ((rptr->flags4 & RF4_BRTH_FIRE) && dam < breath_dam) dam = breath_dam;
	if ((rptr->flags4 & RF4_BRTH_COLD) && dam < breath_dam) dam = breath_dam;
	/* Same for poison, but lower damage cap */
	breath_dam = ((hp / 3) > 800 ? 266 : (hp / 9));
	if ((rptr->flags4 & RF4_BRTH_POIS) && dam < breath_dam) dam = breath_dam;
	/*
	 ...
	 * XXX Doesn't take into account POWERFUL flag!
	 */
	breath_dam = ((hp / 6) > 550 ? 471 : ((hp * 6) / 42));
	if ((rptr->flags4 & RF4_BRTH_NETHR) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 500 ? 428 : ((hp * 6) / 42));
	if ((rptr->flags4 & RF4_BRTH_CHAOS) && dam < breath_dam) dam = breath_dam;
	if ((rptr->flags4 & RF4_BRTH_DISEN) && dam < breath_dam) dam = breath_dam;
	if ((rptr->flags4 & RF4_BRTH_SHARD) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 400 ? 228 : ((hp * 4) / 42));
	if ((rptr->flags4 & RF4_BRTH_LITE) && dam < breath_dam) dam = breath_dam;
	if ((rptr->flags4 & RF4_BRTH_DARK) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 400 ? 285 : ((hp * 5) / 42));
	if ((rptr->flags4 & RF4_BRTH_CONFU) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 500 ? 357 : ((hp * 5) / 42));
	if ((rptr->flags4 & RF4_BRTH_SOUND) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 400 ? 342 : ((hp * 6) / 42));
	if ((rptr->flags4 & RF4_BRTH_NEXUS) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 3) > 150 ? 150 : (hp / 9));
	if ((rptr->flags4 & RF4_BRTH_TIME) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 200 ? 200 : (hp / 6));
	if ((rptr->flags4 & RF4_BRTH_INER) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 3) > 200 ? 200 : (hp / 3));
	if ((rptr->flags4 & RF4_BRTH_GRAV) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 150 ? 150 : (hp / 6));
	if ((rptr->flags4 & RF4_BRTH_PLAS) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 200 ? 200 : (hp / 6));
	if ((rptr->flags4 & RF4_BRTH_FORCE) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 6) > 150 ? 150 : (hp / 6));
	if ((rptr->flags4 & RF4_BRTH_WIND) && dam < breath_dam) dam = breath_dam;
	breath_dam = ((hp / 4) > 250 ? 250 : (hp / 4));
	if ((rptr->flags4 & RF4_BRTH_MANA) && dam < breath_dam) dam = breath_dam;

	/* Handle the attack spells, again assuming single resists */

	if ((rptr->flags5 & RF5_BALL_ACID) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BALL_ELEC) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BALL_FIRE) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BALL_COLD) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BALL_POIS) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_NETHR) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_WATER) && dam < spower * 3)
		dam = spower * 3;
	if ((rptr->flags5 & RF5_BALL_MANA) && dam < spower * 3)
		dam = spower * 3;
	if ((rptr->flags5 & RF5_BALL_DARK) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_LITE) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_CONFU) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_SOUND) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_SHARD) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_WIND) && dam < spower * 5 / 4)
		dam = spower * 5 / 4;
	if ((rptr->flags5 & RF5_BALL_STORM) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BALL_CHAOS) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;

	/* Small annoyance value */
	if ((rptr->flags6 & RF6_DRAIN_MANA) && dam < 5)
		dam = 5;
	/* For all attack forms the player can save against, damage is halved */
	if ((rptr->flags6 & RF6_MIND_BLAST) && dam < 32)
		dam = 32;
	if ((rptr->flags6 & RF6_BRAIN_SMASH) && dam < 90)
		dam = 90;
	if ((rptr->flags5 & RF5_BOLT_ACID) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BOLT_ELEC) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BOLT_FIRE) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BOLT_COLD) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BOLT_POIS) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BOLT_NETHR) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_BOLT_WATER) && dam < spower * 3)
		dam = spower * 3;
	if ((rptr->flags5 & RF5_BOLT_MANA) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BOLT_PLAS) && dam < spower * 3)
		dam = spower * 3;
	if ((rptr->flags5 & RF5_BOLT_ICE) && dam < spower * 3)
		dam = spower * 3;
	if ((rptr->flags5 & RF5_BEAM_ELEC) && dam < spower * 2)
		dam = spower * 2;
	if ((rptr->flags5 & RF5_BEAM_ICE) && dam < spower * 4)
		dam = spower * 4;
	if ((rptr->flags5 & RF5_BEAM_NETHR) && dam < spower * 3 / 2)
		dam = spower * 3 / 2;
	if ((rptr->flags5 & RF5_HOLY_ORB) && dam < spower * 5 / 4)
		dam = spower * 5 / 4;
	if ((rptr->flags6 & RF6_WOUND) && dam < spower * 5 / 4)
		dam = spower * 5 / 4;



	/* Small annoyance value */
	if ((rptr->flags6 & RF6_SCARE) && dam < 5)
		dam = 5;
	/* Somewhat higher annoyance values */
	if ((rptr->flags6 & RF6_BLIND) && dam < 10)
		dam = 8;
	if ((rptr->flags6 & RF6_CONF) && dam < 10)
		dam = 10;
	/* A little more dangerous */
	if ((rptr->flags6 & RF6_SLOW) && dam < 15)
		dam = 15;
	/* Quite dangerous at an early level */
	if ((rptr->flags6 & RF6_HOLD) && dam < 25)
		dam = 25;
	...
	/* XXX There are some more summons in PosBand ... */
	/* Summon kin is more dangerous at deeper levels */
	if ((rptr->flags7 & RF7_S_KIN) && dam < rlev * 2)
		dam = rlev * 2;
	/* Dangerous! */
	if ((rptr->flags7 & RF7_S_HI_DEMON) && dam < 250)
		dam = 250;
	/* Somewhat dangerous */
	if ((rptr->flags7 & RF7_S_MONSTER) && dam < 40)
		dam = 40;
	/* More dangerous */
	if ((rptr->flags7 & RF7_S_MONSTERS) && dam < 80)
		dam = 80;
	/* Mostly just annoying */
	if ((rptr->flags7 & RF7_S_ANIMAL) && dam < 30)
		dam = 30;
	if ((rptr->flags7 & RF7_S_SPIDER) && dam < 20)
		dam = 20;
	/* Can be quite dangerous */
	if ((rptr->flags7 & RF7_S_HOUND) && dam < 100)
		dam = 100;
	/* Dangerous! */
	if ((rptr->flags7 & RF7_S_HYDRA) && dam < 150)
		dam = 150;
	/* Dangerous! */
	if ((rptr->flags7 & RF7_S_ENT) && dam < 175)
		dam = 175;
	/* Can be quite dangerous */
	if ((rptr->flags7 & RF7_S_AINU) && dam < 150)
		dam = 200;
	/* All of these more dangerous at higher levels */
	if ((rptr->flags7 & RF7_S_DEMON) && dam < (rlev * 3) / 2)
		dam = (rlev * 3) / 2;
	if ((rptr->flags7 & RF7_S_UNDEAD) && dam < (rlev * 3) / 2)
		dam = (rlev * 3) / 2;
	if ((rptr->flags7 & RF7_S_DRAGON) && dam < (rlev * 3) / 2)
		dam = (rlev * 3) / 2;
	/* Extremely dangerous */
	if ((rptr->flags7 & RF7_S_HI_UNDEAD) && dam < 400)
		dam = 400;
	/* Extremely dangerous */
	if ((rptr->flags7 & RF7_S_HI_DRAGON) && dam < 400)
		dam = 400;
	/* Extremely dangerous */
	if ((rptr->flags7 & RF7_S_WRAITH) && dam < 450)
		dam = 450;
	/* Most dangerous summon */
	if ((rptr->flags7 & RF7_S_UNIQUE) && dam < 500)
		dam = 500;
...
static bool init_mon_power (void)
{
	...
	for (i = 0; i < z_info->r_max; i++)
	{
		...
		if (randart_verbose)
		{
			...
			dmg = eval_max_dam(i);
			fprintf(randart_log, "Max dam: %ld ", dmg);
			fprintf(randart_log, "Rating: %ld\n", mon_power[i]);
			fflush(randart_log);
		}
...
static s32b slay_power(int a_idx)
{
	...
	if (randart_verbose)
	{
		...
		fprintf(randart_log,"times 1000 is: %ld\n", (1000 * sv) / tot_mon_power);
		fflush(randart_log);
	}
...
s32b artifact_power(int a_idx)
{
	...
	/* Hack -- cache is disabled in Pos for on-the-fly randart generation */
#if 0
...
#define ADD_POWER(string, val, flag, flgnum, extra) \
	...
	ADD_POWER("invisibility",	10, TR3_INVISIBILITY, 3,);
...
static struct item_choice {
	...
	{135, TV_RING,		"a ring"},
	{150, TV_AMULET,	"an amulet"}
};

static int orig_depth;
bool preserve_base_type_hack = FALSE;
object_type *b_ptr = NULL;
...
static s16b choose_item(int a_idx)
{
	 ...
	 * (Only for "predefined" randarts)
	 */
	if (a_idx != RART_BUFFER)
	{
		k_idx = kinds[a_idx]; /* FIXME */ /* huh? */
		k_ptr = &k_info[k_idx];
	...
	{
		target_level = orig_depth;
	}
	
	if (preserve_base_type_hack)
	{
		sval = b_ptr->sval;
		tval = b_ptr->tval;
		goto gonext;
	 ...
	 */
	/* Allow rings and amulets only for rand-rand-arts */
	r = rand_int(100 + 50 * (a_idx == RART_BUFFER));
	...
	case TV_DIGGING:
		if (r2 < 12) sval = SV_SHOVEL;
		else if (r2 < 25) sval = SV_PICK;
		else if (r2 < 50) sval = SV_GNOMISH_SHOVEL;
		else if (r2 < 70) sval = SV_ORCISH_PICK;
		else if (r2 < 95) sval = SV_DWARVEN_SHOVEL;
		else if (r2 < 120) sval = SV_DWARVEN_PICK;
	...
	case TV_POLEARM:
		if (r2 < 8) sval = SV_SPEAR;
		else if (r2 < 17) sval = SV_TRIDENT;
		else if (r2 < 22) sval = SV_LANCE;
	...
	case TV_SOFT_ARMOR:
		...
		else if (r2 < 144) sval = SV_DRAGON_SHINING;
	...
	case TV_RING:
	case TV_AMULET:
		/* Hack -- Pick a random, non-INSTA_ART base type */
		while (TRUE)
		{
			sval = randint(50);
			...
			{
				k_ptr = &k_info[k_idx];
				if (!(k_ptr->flags3 & (TR3_INSTA_ART))) break;
			}
		}
		break;
	}
gonext:
	...
	k_idx = lookup_kind(tval, sval);
	k_ptr = &k_info[k_idx];
	if (a_idx != RART_BUFFER) kinds[a_idx] = k_idx;
	...
	/* Base type is preserved. Ego-item and object flags are also preserved */
	if (preserve_base_type_hack)
	{
		/* Replace flags, if desired */
		if (b_ptr->useof)
		{
			a_ptr->flags1 = b_ptr->flags1;
			a_ptr->flags2 = b_ptr->flags2;
			a_ptr->flags3 = b_ptr->flags3;
			a_ptr->flags4 = b_ptr->flags4;
		}
		
		/* Preserve base bonuses */
		a_ptr->pval = b_ptr->pval;
		a_ptr->to_h = b_ptr->to_h;
		a_ptr->to_d = b_ptr->to_d;
		a_ptr->to_a = b_ptr->to_a;
		a_ptr->ac = b_ptr->ac;
		a_ptr->dd = b_ptr->dd;
		a_ptr->ds = b_ptr->ds;
		a_ptr->weight = b_ptr->weight;

		/* Hack -- for lites zero the pval */
		if (a_ptr->tval == TV_LITE) a_ptr->pval = 0;
	}
	...
	switch (a_ptr->tval)
	{
		...
		case TV_SWORD:
		case TV_POLEARM:
			th = (s16b)(mean_hit_startval / 2 +
				randint( mean_hit_startval ) );
			td = (s16b)(mean_dam_startval / 2 +
				randint( mean_dam_startval ) );
			if (th > a_ptr->to_h) a_ptr->to_h += (th / 2); else a_ptr->to_h += th;
			if (td > a_ptr->to_d) a_ptr->to_d += (td / 2); else a_ptr->to_d += td;
		...
		case TV_DRAG_ARMOR:
			...
			ta = (s16b)(mean_ac_startval / 2 +
				randint( mean_ac_startval ) );
			if (ta > a_ptr->to_a) a_ptr->to_a += (ta / 2); else a_ptr->to_a += ta;
...
static void parse_frequencies ()
{
	...
	for(i = 0; i < z_info->a_max; i++)
	{
			...
			(a_ptr->flags2 & TR2_IM_POIS) )
		{
			...
			if (a_ptr->flags2 & TR2_IM_POIS) temp++;
...
static void add_invisibility(artifact_type *a_ptr)
{
	a_ptr->flags3 |= TR3_INVISIBILITY;
	LOG_PRINT("Adding ability: invisibility\n");
}
...
/* At init1.c */
extern void gen_rart_desc(object_type *o_ptr);
...
static void scramble_artifact(int a_idx)
{
	...
	else
	{
		...
		a_ptr->flags1 = a_ptr->flags2 = a_ptr->flags4 = 0;
	...
	else
	{
		...
		if (randart_verbose && tries >= MAX_TRIES)
		{
			...
			/* Hack: set the cost to zero to handle inhibited cases */
			a_ptr->cost = 0;
...
/*
 * Makes a rand-rand-art
 */
bool scramble_artifact_random(int oa_idx, object_type *o_ptr)
{
	artifact_type *oa_ptr = &a_info[oa_idx], *a_ptr = &a_info[RART_BUFFER];
	int a_idx = RART_BUFFER;
	artifact_type a_old;
	u32b activates = a_ptr->flags3 & TR3_ACTIVATE;
	int power, tries, k_idx, ap;
	bool curse_me = FALSE;
	bool success = FALSE;
	char word[80], name[80];
	int count = 0;
	s32b ap2;

	b_ptr = o_ptr;
	orig_depth = oa_ptr->level;
	
	/* WIPE the buffer artifact */
	WIPE(a_ptr, artifact_type);

	/* Skip unused artifacts, too! */
	if (oa_ptr->tval == 0) return FALSE;

	/* Evaluate the original artifact to determine the power level. */
	power = base_power[oa_idx];

	/* If it has a restricted ability then don't randomize it. */
	if (power > 10000)
	{
		LOG_PRINT1("Skipping artifact number %d - too powerful to randomize!", oa_idx);
		return FALSE;
	}

	if (power < 0) curse_me = TRUE;

	LOG_PRINT("+++++++++++++++++ CREATING NEW RANDOM ARTIFACT ++++++++++++++++++\n");
	LOG_PRINT2("Base artifact %d: power = %d\n", oa_idx, power);
	...
	/* 
	 * Hack -- don't allow activation if recharge time is zero
	 * (Otherwise, we could get an artifact which "activates for illumination"
	 * every turn.)
	 */
	if (!oa_ptr->time) a_ptr->flags3 &= ~(TR3_ACTIVATE);	
	...
	/* The artifact is ready. Now ensure nobody sees it */
	a_ptr->rarity = 0;
	
	/* Copy all fields to object */
	o_ptr->k_idx = lookup_kind(a_ptr->tval, a_ptr->sval);
	o_ptr->tval = a_ptr->tval;
	o_ptr->sval = a_ptr->sval;
	o_ptr->pval = a_ptr->pval;
	o_ptr->weight = a_ptr->weight;
	/* Hack -- preserve ego-item flags */
	if (o_ptr->flags2)
	{
	...
	o_ptr->name1 = 0;  /* Randrandarts are identified by rart_name */
	o_ptr->name2 = 0;
	o_ptr->xtra1 = oa_idx;  /* Original artifact */
	o_ptr->xtra2 = 0;
	...
	o_ptr->flags3 |= a_ptr->flags3 | TR3_IGNORE_MASK;
	o_ptr->flags4 |= a_ptr->flags4;

	/* Think a name for it */
	make_random_name(word, sizeof(word));
	if (rand_int(3) == 0)
	{
		strnfmt(name, sizeof(name), "'%s'", word);
	}
	else
	{
		strnfmt(name, sizeof(name), "of %s", word);
	}
	o_ptr->rart_name = quark_add(name);
	gen_rart_desc(o_ptr);
