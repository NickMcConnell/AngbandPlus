 ...
 */
void get_monster_saying(monster_race* r_ptr, char* buff) {
  if (r_ptr->sayings_inx) {
    get_monster_saying(&r_info[r_ptr->sayings_inx], buff);
    ...
    get_random_line("insult.txt", buff);
  }    
}
			...
			switch (effect)
			{
				...
				case RBE_EAT_ITEM:
				{
					...
					for (k = 0; k < 10; k++)
					{
						...
						/* Message */
						mformat(MSG_WARNING, "%sour %s (%c) was stolen!",
				...
				case RBE_EAT_FOOD:
				{
					...
					for (k = 0; k < 10; k++)
					{
						...
						/* Message */
						mformat(MSG_WARNING, "%sour %s (%c) was eaten!",
						           ((o_ptr->number > 1) ? "One of y" : "Y"),
						           o_name, index_to_label(i));
