 ...
 */
static int 
monster_critical(int dice, int sides, int dam)
{
  int max = 0;
...
static s16b attack_effect_to_spell_type(s16b effect) {
  switch (effect) {
  case RBE_HURT:       return GF_MANA;
  case RBE_POISON:     return GF_POIS;
  case RBE_UN_POWER:
  case RBE_UN_BONUS:   return GF_DISENCHANT;
  case RBE_ACID:       return GF_ACID;
  case RBE_ELEC:       return GF_ELEC;
  case RBE_FIRE:       return GF_FIRE;
  case RBE_COLD:       return GF_COLD;
  case RBE_BLIND:      return GF_LITE;
  case RBE_CONFUSE:    return GF_CONFUSION;
  case RBE_TERRIFY:    return GF_DISP_ALL;
  case RBE_PARALYZE:   return GF_SLEEP;
  case RBE_LOSE_STR:   return GF_DEC_STR;
  case RBE_LOSE_INT:   return GF_DEC_INT;
  case RBE_LOSE_WIS:   return GF_DEC_WIS;
  case RBE_LOSE_DEX:   return GF_DEC_DEX;
  case RBE_LOSE_CON:   return GF_DEC_CON;
  case RBE_LOSE_CHR:   return GF_DEC_CHR;
  case RBE_LOSE_ALL:   return GF_RUINATION;
  case RBE_SHATTER:    return GF_QUAKE;
  case RBE_EXP_10:
  case RBE_EXP_20:
  case RBE_EXP_40:
  case RBE_EXP_80:     return GF_DRAIN;
  case RBE_INSANITY:   return GF_CAUSE_INSANITY;
...
make_attack_normal(int m_idx)
{
  ...
  for (ap_cnt = 0; ap_cnt < 4; ap_cnt++) {
	...
	project(m_idx, 5, m_ptr->fy, m_ptr->fx, damage, 
		...
		PROJECT_BLAST | PROJECT_KILL | PROJECT_ITEM | PROJECT_GRID);
	...
	case RBE_UN_POWER:
	  {
	    ...
	    for (o_ptr = inventory;
		 ...
		 o_ptr = o_ptr->next) {
	      ...
	      int foo = o_ptr->weight * 100 / p_ptr->total_weight;
		...
		continue;
		  ...
		  (o_ptr->pval)) {
	      /* Message */
		mprint(MSG_WARNING, "Energy drains from your pack!");
      ...
      case RBE_EAT_ITEM:
	{
	  ...
	  for (o_ptr = inventory; o_ptr != NULL; o_ptr = o_ptr->next) {
		...
		if (protect_equipment) skip = TRUE;
	       
		if (cursed_p(o_ptr)) skip = TRUE;
	      }
	    }
      ...
      case RBE_EAT_FOOD:
	{
	  ...
	  for (o_ptr = inventory; o_ptr != NULL; o_ptr = o_ptr->next) {
	    ...
	    /* Message */
	    mformat(MSG_WARNING, "Your %s %s eaten!",
		    o_name,
      ...
      case RBE_SHATTER:
	{
	  ...
	  if (damage > 23) {
	    fire_mega_blast(m_ptr->fy, m_ptr->fx,
			    GF_QUAKE, 8,
			    damroll(2, 5));
	  }
	  break;
