...
bool make_attack_normal(int m_idx)
{
	...
	for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
	{
		...
		switch (effect)
		{
			...
			case RBE_DISEN:	power = 20; break;
			case RBE_DR_CHARGE:	power = 15; break;
			...
			case RBE_DR_MANA:	power = 10; break;
			case RBE_DR_LIFE:	power = 10; break;
		...
		if (!effect || check_hit(power, rlev))
		{
			...
			switch (effect)
			{
				...
				case RBE_HURT:
				{
					...
					reduce = ((ac < 150) ? ac : 150) ;
					if (p_ptr->biofeedback)
					{
						reduce += 50;
						if (reduce < 125) reduce = 125;
					}
					
                                        if (p_ptr->shadow_form) reduce = 225;

                                        damage -= damage * reduce / 250;
				...
				case RBE_DR_MANA:
				{
					...
					/* Basic message */
					if (r1) msg_print("Your mana is drained!");
