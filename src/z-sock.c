...
bool zsock_write_win(ip_connection *conn, cptr str)
{
	SOCKET *c = conn->socket;

        if (!conn->connected) return FALSE;

	if (send(*c, str, strlen(str), 0) <= 0)
        {
...
bool zsock_read_win(ip_connection *conn, char *str, int len)
{
	char c;
	int l = 0;
...
bool zsock_write_unix(ip_connection *conn, cptr str)
{
	int *c = conn->socket;
...
bool zsock_read_unix(ip_connection *conn, char *str, int len)
{
	char c;
	int l = 0;
	int *cc = conn->socket;
...
bool zsock_write_dummy(ip_connection *conn, cptr str)
{
        return FALSE;
}

bool zsock_read_dummy(ip_connection *conn, char *str, int len)
{
        return FALSE;
}
