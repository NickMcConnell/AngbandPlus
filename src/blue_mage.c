...
static void _save_player(savefile_ptr file)
{
    ...
    for (i = 0; i < _MAX_SLOTS; i++)
    {
        ...
        if (spell->spell)
        {
            ...
            savefile_write_u16b(file, spell->race->id);
        }
    }
...
static void _list_spell_aux(doc_ptr doc, mon_spell_ptr spell, mon_race_ptr race, int options)
{
    int cost = mon_spell_cost_plr(spell, race);
    int fail = mon_spell_fail_plr(spell, race);
    cptr stat = "   ";

    if (spell->id.type == MST_BREATH)
        stat = "Con";
    else if (race->body.spell_stat != A_NONE)
        stat = stat_abbrev_true[race->body.spell_stat];

    if (options & _SHOW_GRAPHICS)
    {
        doc_insert_char(doc, race->x_attr, race->x_char);
        doc_insert_char(doc, TERM_WHITE, ' ');
    }
    else
    {
        doc_insert_char(doc, race->d_attr, race->d_char);
        doc_insert_char(doc, TERM_WHITE, ' ');
    }
    mon_spell_doc(spell, doc);
    doc_printf(doc, "<tab:30>%s %3d %3d %3d%%", stat, race->level, cost, fail);

    if (options & _SHOW_INFO)
        mon_spell_list_info(doc, spell, race);
...
static void _list_spell(doc_ptr doc, _spell_ptr spell, int choice, int options)
{
    ...
    _list_spell_aux(doc, spell->spell, spell->race, options);
}
...
static void _list_spells(doc_ptr doc, int options)
{
    ...
    if (options & _LEARNING)
    {
        ...
        _list_spell_aux(doc, _learning->spell, _learning->race, options);
    }
        ...
        if ('a' <= cmd && cmd < 'a' + _MAX_SLOTS)
        {
            ...
            else
            {
                if (_spells[slot].spell || (options & _ALLOW_EMPTY))
                {
                    ...
                    {
                        char       c;
                        string_ptr name = string_alloc();
                        string_ptr prompt;

                        mon_spell_print(result->spell, name);
                        prompt = string_alloc_format(
                            "Really replace %s? <color:y>[y/N]</color>", string_buffer(name));
...
void _learning_spell(int cmd, var_ptr res)
{
    ...
    case SPELL_DESC:
        ...
        break;
    case SPELL_CAST:
        if (p_ptr->action == ACTION_LEARN) set_action(ACTION_NONE);
...
static void _learn_aux(mon_spell_cast_ptr cast, _spell_ptr spell)
{
    ...
    msg_format("You have learned %s.", string_buffer(name));
    string_free(name);
...
void blue_mage_learn(mon_spell_cast_ptr cast)
{
    ...
    if (_find(cast->spell, cast->race))
    {
        if (0 || p_ptr->wizard)
        {
            ...
            msg_format("<color:D>You already know %s from this race.</color>", string_buffer(s));
            string_free(s);
        }
        return;
    }

    ml = 20 + 2*cast->race->level;
    pl = p_ptr->lev + p_ptr->stat_ind[A_INT];
    if (0 || p_ptr->wizard)
    {
        ...
        msg_format("<color:D>Attempting to learn %s (1d%d >= 1d%d => %d.%d%% Fail)...</color>",
            string_buffer(s), ml, pl, odds/10, odds%10);
        string_free(s);
    }
...
void blue_mage_cast(void)
{
    ...
    if (spell)
    {
        ...
        if (mon_spell_cast_blue_mage(spell->spell, spell->race))
            energy_use = 100;
    }
}
...
plr_class_ptr blue_mage_get_class(void)
{
    ...
    skills_t xs = {  7,  13,  10,   0,   0,   0,  12,  10 };
