...
void search(void)
{
	...
	/* Notice object flags */
	object_notice_flag(0, TR0_SEARCH1);
	object_notice_flag(0, TR0_SEARCH2);
...
byte py_pickup(int pickup)
{
	...
	size_t floor_num = 0;
	s16b floor_list[MAX_FLOOR_STACK + 1], floor_o_idx = 0;
	...
	if (pickup == 1)
	{
		...
		if (floor_num > 1)
			...
			this_o_idx = INT2S16B(floor_o_idx);
	}
