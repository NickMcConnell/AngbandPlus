...
void do_cmd_go_up(void)
{
	...
	{
		/* Verify probability travel */
		if (p_ptr->prob_travel)
		{
			if (!p_ptr->depth)
			{
				msg_print("You cannot travel up from here.");
				return;
			}
			else
			{
				...
				/* Success */
			 	msg_print("You pass through a probability gate.");
...
void do_cmd_go_down(void)
{
	...
	if (cave_feat[p_ptr->py][p_ptr->px] != FEAT_MORE)
	{
		...
		if (p_ptr->prob_travel)
		{
			if ((p_ptr->depth == 127) || is_quest(p_ptr->depth))
			{
				msg_print("A mysterious force prevents you from travelling down.");
				return;
			}
			else
			{
...
static bool do_cmd_disarm_aux(int y, int x)
{
	...
	if (cave_feat[y][x] == FEAT_MON_TRAP) power = 0;
...
void do_cmd_disarm(void)
{
	...
	if (easy_open)
	{
		...
		if ((count_feats(&y, &x, FEAT_TRAP_START, FEAT_TRAP_TAIL) +
...
void do_cmd_fire(void)
{
	...
	/* Find out if the ammo is blessed */
	object_flags(i_ptr, &f1, &f2, &f3);
	
	/* Penalize Priests */
	if (p_ptr->pclass == CLASS_PRIEST)
	{	
		if ((i_ptr->tval != TV_SHOT) && !(f3 & TR3_BLESSED)) unholy = TRUE;
	}
	if (unholy) bonus -= 15;
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			{
				msg_print("Your unholy attack misses.");
			}
...
/*
 * Allow user to choose a power (end force walls etc.) to activate
 */
void do_cmd_extra_power(void)
{
	int i = 0;
	...
	p_ptr->energy_use = 100;
		...
		{
			no_fw--;
			cave_set_feat(fw_y[no_fw], fw_x[no_fw], FEAT_FLOOR);
			p_ptr->update |= (PU_UPDATE_VIEW | PU_MONSTERS);
		}
		break;
		...
		case POWER_END_ONE_FORCE_WALL:
		{
			...
			int x,y;
			p_ptr->energy_use = 0;
			if (!tgt_pt(&y,&x, TARGET_FLOOR)) break;
