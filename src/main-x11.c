...
static errr Term_xtra_x11_react(void)
{
	...
	if (Metadpy->color)
	{
		...
		for (i = 0; i < 256; i++)
		{
			if ((color_table[i][0] != angband_colour_table[i][0]) ||
			    (color_table[i][1] != angband_colour_table[i][1]) ||
			    (color_table[i][2] != angband_colour_table[i][2]) ||
			    (color_table[i][3] != angband_colour_table[i][3]))
			{
				Pixell pixel;
				
				/* Save new values */
				color_table[i][0] = angband_colour_table[i][0];
				color_table[i][1] = angband_colour_table[i][1];
				color_table[i][2] = angband_colour_table[i][2];
				color_table[i][3] = angband_colour_table[i][3];
...
errr init_x11(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (prefix(argv[i], "-b"))
		{
			plog_fmt("Ignoring bigtile option");
			continue;
		}
	...
	switch (arg_graphics)
	{
		case GRAPHICS_ADAM_BOLT:
			...
			if (0 == fd_close(fd_open(filename, O_RDONLY)))
			{
				...
				/*use_transparency = TRUE;  Aint ready for it yet */
			...
			case GRAPHICS_DAVID_GERVAIS:
				...
				/*use_transparency = TRUE; aint ready for it*/
