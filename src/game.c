...
static void sense_inventory(void)
{
	...
	for (i = 0; i < INVEN_TOTAL; i++)
	{
		...
		if ((i < INVEN_EQUIP) && (0 != rand_int(5))) continue;
...
/*
 * Process inventory for destructive 'elemental touch' effect
 */

#define TOUCH_CHANCE 25

static void touch_inven(void)
{
	int i;
	...
	for (i = 0; i < INVEN_TOTAL; i++)
	{
		...
		if (!o_ptr->k_idx) continue;
                ...
                /* Cold touch: shatter potions */
                if (p_ptr->cold_touch)
                {
                        okay = FALSE;
                        
                        /* Potions, except water (any non-artifact - ?) */
                        if (o_ptr->tval == TV_POTION && o_ptr->sval != SV_POTION_WATER
                        	&& !artifact_p(o_ptr)) okay = TRUE;
                        /* Flasks (any non-artifact - ?) */
                        if (o_ptr->tval == TV_FLASK && !artifact_p(o_ptr)) okay = TRUE;
                        
                        if (okay && rand_int(TOUCH_CHANCE) == 0)
                        {
		                /* Get an object description */
		                object_desc(o_name, sizeof(o_name), o_ptr, FALSE, 0);

                                /* Whoops! */
                                msg_format("Your cold touch freezes your %s!", o_name);
                ...
                /* Fire touch: burn scrolls and staves */
                if (p_ptr->fire_touch)
                {
                	okay = FALSE;
                	
                	/* Scrolls (any non-artifact - ?) */
                	if (o_ptr->tval == TV_SCROLL && !artifact_p(o_ptr)) okay = TRUE;
                	/* Staves (any non-artifact - ?) */
                	if (o_ptr->tval == TV_STAFF && !artifact_p(o_ptr)) okay = TRUE;
		                ...
		                object_desc(o_name, sizeof(o_name), o_ptr, FALSE, 0);
                                ...
                                /* Whoops! */
                                msg_format("Your fiery touch burns your %s!", o_name);
		                inven_item_increase(i, -1);
		                inven_item_describe(i);
		                inven_item_optimize(i);
                        }
                ...
                /* Shocking touch: destroy rings, wands, rods */
                if (p_ptr->elec_touch)
                {
		    	u32b f1, f2, f3, f4;
			...
			object_flags(o_ptr, &f1, &f2, &f3, &f4);
                	...
                	/* Rings (any non-artifact, non-resistant) */
                	if (o_ptr->tval == TV_RING && !artifact_p(o_ptr) &&
				!(f3 & (TR3_IGNORE_ELEC))) okay = TRUE;
                	/* Wands (any non-artifact - ?, non-resistant) */
                	if (o_ptr->tval == TV_WAND && !artifact_p(o_ptr) &&
				!(f3 & (TR3_IGNORE_ELEC))) okay = TRUE;
                	/* Rods (any non-artifact - ?, non-resistant) */
                	if (o_ptr->tval == TV_ROD && !artifact_p(o_ptr) &&
				!(f3 & (TR3_IGNORE_ELEC))) okay = TRUE;
		                ...
		                object_desc(o_name, sizeof(o_name), o_ptr, FALSE, 0);
                                ...
                                /* Whoops! */
                                msg_format("Your shocking touch destroys your %s!", o_name);
		                inven_item_increase(i, -1);
		                inven_item_describe(i);
		                inven_item_optimize(i);
                        }
 ...
 */
static void terrain_affect(void)
{
	/* m_r_idx can be zero, but it is not really relevant here */
	monster_race *r_ptr = &r_info[p_ptr->m_r_idx];
	
	switch (cave_feat[p_ptr->py][p_ptr->px])
	{
		/* Water -> no effect (drench items?) */
		case FEAT_WATER:
			break;
		
		/* 
		 * Ice -> little cold damage; implement occasional random moves,
		 * like in ToME, elsewhere.
		 */
		case FEAT_ICE:
		{
		    	if (one_in_(4) && !p_ptr->resist_cold && !p_ptr->immune_cold)
			{
				msg_print("It's cold here!");
			}
                        cold_dam(damroll(1, 4), "freezing to death");
			break;
		}
		
		/* Fire -> Moderate fire damage */
		case FEAT_FIRE:
		{
		    	if ((one_in_(4) || !p_ptr->resist_fire) && !p_ptr->immune_fire)
			{
			    	msg_print("You are burned by the fires.");
			}
			fire_dam(damroll(6, 8), "fatal burns");
			break;
		}
		
		/* Lava -> *LARGE* damage, only immunity can help */
		case FEAT_LAVA:
		{
			if (!p_ptr->immune_fire)
			{
				msg_print("The lava burns you terribly!");
				take_hit(145 + rand_int(100), "falling to lava");
				/*
				 * Hack -- Take minor fire damage with fire_dam() to handle
				 * inventory damage properly.
				 */
				fire_dam(5, "falling to lava");
			}
			break;
		}
		...
		case FEAT_ROCKS:
		{
			if (!(r_ptr->flags2 & (RF2_PASS_WALL | RF2_KILL_WALL)))
			{
			    	if (one_in_(4))
				{
				    	msg_print("The rocky terrain slows you down.");
				}
				p_ptr->energy_use += rand_int(50);
			}
			break;
		}
		
		/* Fog -> invisibility; elsewhere */
		case FEAT_FOG:
			break;
		
		/* Abyss -> fall down (if we don't have f/fall) */
		/* XXX Stop on lev 99 is hardcoded */
		/* Maybe use saving throw? */
		case FEAT_ABYSS:
		{
			if (!p_ptr->ffall)
			{
				...
				msg_print("Aaaargh! You fall into the abyss!");
				while (1)
				{
				    	if (p_ptr->depth >= 99 && !p_ptr->total_winner)
					{
					    	if (!levs)
						{
							msg_print("Some force keeps you from falling!");
						}
						else
						{
						    	msg_format("After flying %d feet down, you are stopped by some force.", levs * 50);
						}
						break;
					}
					p_ptr->leaving = TRUE;
					...
					{
					    	msg_print("You crash into the bottom of the dungeon!");
						break;
					}
					else if (one_in_(7))
					{
					    	msg_format("You fall %d feet down.", levs * 50);
						break;
					}
				}
				take_hit(damroll(dice, 8), "falling into the abyss");
				break;
			}
		}
		
		/* Grass, tree -> no effect */
		/* Trees never slow down the player, but can slow down the monster */
		case FEAT_GRASS:
		case FEAT_TREE:
			break;
	}
...
static void regen_monsters(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		{
			frac = (m_ptr->maxmana + smooth) / 100 + m_ptr->level / 5;
			...
			if (m_ptr->mana == m_ptr->maxmana) m_ptr->min_range = 0;
		...
		{
			frac = (m_ptr->maxhp + smooth) / 100 + m_ptr->level;
...
static void process_world(void)
{
	...
	/*** Damage over Time ***/
        ...
        p_ptr->in_wall = !cave_floor_bold(p_ptr->py, p_ptr->px);

        /* Take damage from walking into walls (but not die this way!) */
        if (p_ptr->in_wall && p_ptr->chp)
        {
                /* Take damage */
                take_hit(1, "THIS IS A BUG");
        }
	...
	/* Take possible damage from terrain */
	/* if (!(turn % 100)) */
		terrain_affect();
	...
	if (p_ptr->food < PY_FOOD_MAX)
	{
		...
		if (!(turn % 100))
		{
			...
			/* Super-regeneration takes even more food */
			if (p_ptr->s_regenerate) i += 30;
	...
	/* Super-regeneration ability */
	if (p_ptr->s_regenerate)
	{
	...
	if (p_ptr->cut) regen_amount = 0;
        if (p_ptr->in_wall) regen_amount = 0;
	...
	if (p_ptr->tim_invis)
	{
		(void)set_tim_seeinvis(p_ptr->tim_invis - 1);
	}
	...
	{
		(void)set_tim_immaterial(p_ptr->tim_immaterial - 1);
	}

	/* Timed invisibility */
	if (p_ptr->tim_invisible)
	{
	    	(void)set_tim_invisibility(p_ptr->tim_invisible - 1);
	}
	...
	if (p_ptr->hero)
	{
		/* Never expire if the king is dead */
		if (!(p_ptr->hero == 1 && r_info[rp_ptr->king_r_idx].max_num == 0))
	...
	{
		/* Never expire if the king is dead */
		if (!(p_ptr->blessed == 1 && rp_ptr->king_r_idx && r_info[rp_ptr->king_r_idx].max_num == 0))
	...
	/* Lightning Cloud */
	if (p_ptr->project_elec)
	{
	    	p_ptr->project_elec--;
		if (!p_ptr->project_elec) msg_print("The electric charges around you dissipate.");
	}
	...
	/* Hack -- this doesn't allow >1 light */
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	if (r_ptr->body.light_mask & EQUIP_SLOT(i)) break;
	}
	o_ptr = &inventory[i];
	...
	/* Process equipment */
	for (j = 0, i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
		/* Get the object */
		o_ptr = &inventory[i];
	...
	switch (p_ptr->command_cmd)
	{
		...
		case 'u':
		{
			...
			break;
                ...
                case 'U':
                {
                        do_cmd_monster_power();
                        break;
                }
                
                /* Incarnate/Deincarnate */
                case 'N':
                {
                	do_cmd_possess();
                	break;
...
static void process_player(void)
{
	...
	do
	{
		...
		/* Hack -- no monster-monster out-of-LOS combat message yet */
		mon_mon_nonlos_combat_msg = FALSE;
