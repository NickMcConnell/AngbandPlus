...
void delete_monster(int y, int x)
{
	...
	if (cave_monster_bold(y, x)) delete_monster_idx(cave_m_idx[y][x]);
}
...
void monster_desc(char *desc, monster_type *m_ptr, int mode)
{
	...
	if (p_ptr->image)
	{
		if (!rand_int(2))
		{
			r_ptr = &r_info[randint(MAX_R_IDX - 1)];
			strcpy(silly_name, (r_name + r_ptr->name));
		}
		/* Oops */
		else if (get_rnd_line("silly.txt", "", silly_name))
		{
			strcpy(silly_name, "Something strange");
		}
...
void sanity_blast (monster_type * m_ptr, bool necro)
{
    ...
    if (!necro)
    {
       ...
       if (!(r_ptr->flags1 & RF1_UNIQUE))
       {
        ...
        if ((m_ptr->smart & SM_FRIEND) && (randint(8)!=1))
        ...
        {
            if (randint(100) < (25 + (p_ptr->level))) return;
        }
    }
    else
    {
...
void update_mon(int m_idx, bool full)
{
	...
	{
		byte info = cave_info[fy][fx];

		/* Normal line of sight, and player is not blind */
		if ((info & (CAVE_VIEW)) && (!p_ptr->blind))
		{
			/* Use "infravision" */
...
{
	/* Paranoia XXX XXX */
	if (!cave_dead_bold(y, x)) return (0);
 ...
 */
static bool place_monster_one(int y, int x, int r_idx, bool slp, bool charm)
{
	int i;

	monster_type monster_type_body;

	monster_type *m_ptr = &monster_type_body;
	...
	if (cave_feat[y][x] == FEAT_MINOR_GLYPH) return (FALSE);
	...
	if (!monster_place(y, x, m_ptr)) return (FALSE);
...
static bool place_monster_okay(int r_idx)
{
	...
	/* Hack - require same alignment */
   if ((r_ptr->flags3 & (RF3_DOOM)) != (z_ptr->flags3 & (RF3_DOOM))) return (FALSE);
...
bool place_monster_aux(int y, int x, int r_idx, bool slp, bool grp, bool charm)
{
	...
	if ((r_ptr->flags1 & (RF1_FRIENDS)) || (r_ptr->flags2 & RF2_MULTIPLY))
	{
...
bool alloc_horde(int y, int x)
{
	...
	{
		/* Attempt to place the monster */
		if (place_monster_aux(cy, cx, r_idx, FALSE, TRUE, FALSE)) break;
	}
	...
	for (attempts = randint(10) + 5; attempts; attempts--)
	{
		...
		(void)summon_specific(cy, cx, p_ptr->depth + 5, SUMMON_KIN, TRUE, FALSE);
...
static bool summon_specific_okay(int r_idx)
{
	...
	if (summon_specific_type == SUMMON_MONSTER)  return (TRUE);
	...
	switch (summon_specific_type)
	{
		...
		case SUMMON_CYBER:
		{
			okay = ((strstr(r_ptr->name + r_name, "Cyber") ||
					   strstr(r_ptr->name + r_name, "cyber")) &&
 ...
 */
bool summon_specific(int y1, int x1, int lev, int type, bool group, bool friendly)
{
	int i, x, y, r_idx;
	...
	if (!place_monster_aux(y, x, r_idx, FALSE, group, friendly)) return (0);
...
bool multiply_monster(int m_idx, bool charm, bool clone)
{
	...
	if (clone && result) m_list[cave_m_idx[y][x]].smart |= SM_CLONED;
