...
static bool place_monster_one(int y, int x, int r_idx, bool slp)
{
	...
	if (r_ptr->level > p_ptr->depth)
	{
		...
		if (r_ptr->flags1 & (RF1_UNIQUE))
		{
			...
			/* Message for Precognition */
			if (p_ptr->precognition) msg_format("The thought of meeting %s terrifies you.", name);
		...
		else
		{
			...
			/* Message for Precognition */
			if (p_ptr->precognition) msg_format("You feel afraid of meeting %ss.", name);
	...
	else if (r_ptr->flags1 & (RF1_UNIQUE))
	{
		...
		/* Precognition */
		if (p_ptr->precognition) msg_format("You will soon meet %s.", name);
	}
