...
static void player_flags(u32b *f1, u32b *f2, u32b *f3)
{
	...
	if (p_ptr->prace == RACE_MUTANT) {
		...
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_SEE_INVIS);
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_FREE_ACT);
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_SLOW_DIGEST);
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_REGEN);
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_HOLD_LIFE);
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_TELEPATHY);
		if(!rand_int(MUTANT_ATTRIB)) (*f3) |= (TR3_LITE);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_SUST_STR);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_SUST_INT);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_SUST_WIS);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_SUST_CON);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_SUST_DEX);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_SUST_CHR);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_ACID);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_ELEC);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_FIRE);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_COLD);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_POIS);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_FEAR);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_LITE);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_DARK);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_BLIND);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_CONFU);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_SOUND);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_CHAOS);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_DISEN);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_SHARD);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_NEXUS);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_RES_NETHR);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_IM_ACID);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_IM_ELEC);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_IM_FIRE);
		if(!rand_int(MUTANT_ATTRIB)) (*f2) |= (TR2_IM_COLD);
		Rand_quick = FALSE;
	}
}
