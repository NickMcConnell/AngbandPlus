 ...
 *
 * Modified 2-17-98 by Dag Arneson
 *
...
/*
 * Check if an object is a wand/rod/staff/battery
 */
static bool item_tester_hook_zap(object_type *o_ptr)
{
	if(o_ptr->tval==TV_BATTERY||o_ptr->tval==TV_WAND||
		o_ptr->tval==TV_ROD||o_ptr->tval==TV_STAFF) return TRUE;
	else return FALSE;
}
 ...
 */
void use_staff(int item, object_type *o_ptr)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
...
/* Use a battery. */
void use_battery(object_type *o_ptr)
{
	int amt;
	...
	if (!amt)
	{
		msg_print("There is no mana stored in the battery.");
		return;
	}

	/* No mana above maximum */
	if(amt > p_ptr->msp - p_ptr->csp) amt = p_ptr->msp - p_ptr->csp;

	if (!amt)
	{
		msg_print("Your mana is already at its maximum!");
		return;
	}

	msg_print("The energies from the battery flow through your body.");
	o_ptr->pval = 0;
	p_ptr->csp += amt;
...
void aim_wand(int item, object_type *o_ptr)
{
 ...
 */
void zap_rod(int item, object_type *o_ptr)
{
...
/*
 * Use a magical item.
 * This includes staffs, sands, rods and batteries.
 */
void do_cmd_use_magic_item(void)
{
	int item;
	...
	/* Restrict choices to wands, batteries, staffs and rods */
	item_tester_hook = item_tester_hook_zap;

	/* Get an item */
	q = "Use which magical item? ";
	s = "You have no magical item to use.";
	if (!get_item(&item, q, s, (USE_INVEN | USE_FLOOR))) return;
	...
	if ((item < 0) && (o_ptr->number > 1))
	{
		msg_print("You must first pick up the magical items.");
		return;
	}

	if (o_ptr->tval==TV_BATTERY) use_battery(o_ptr);
	else if (o_ptr->tval==TV_STAFF) use_staff(item, o_ptr);
	else if (o_ptr->tval==TV_WAND) aim_wand(item, o_ptr);
	else if (o_ptr->tval==TV_ROD) zap_rod(item, o_ptr);
}
 ...
 * Fill a battery
 */
void do_cmd_charge_battery(void)
{
	int item, amt, tmp;
	...
	/* Prepare the hook */
	item_tester_tval = TV_BATTERY;

	/* Get an item */
	q = "Charge which battery? ";
	s = "You have nothing to charge.";
	if (!get_item(&item, q, s, (USE_INVEN | USE_FLOOR))) return;
	...
	if (p_ptr->command_arg)
	{
		...
		/* Enforce the maximum */
		if (amt > p_ptr->csp) amt = p_ptr->csp;
	}

	else

	{
		msg_print("You must use a repeat value with the charge command.");
		return;
	}

	/* The battery can't glow when it doesn't add a charge */
	if(amt < BATTERY_DEV)
	{
		msg_print("That's not enough mana to charge the battery.");
		return;
	}

	msg_print("The battery glows with power.");
	p_ptr->csp -= amt;
	o_ptr->pval += amt / BATTERY_DEV;

	p_ptr->redraw |= (PR_MANA);
	p_ptr->window |= (PW_PLAYER_0 | PW_PLAYER_1);

	/* Bad things might happen if you try to charge batteries too much */
	if (rand_int(o_ptr->pval) >= (p_ptr->msp / BATTERY_DEV) * 2 ||
		rand_int(200) == 0)
	{
		tmp = rand_int(30);
		...
		if (tmp == 0)
		{
			msg_print("The battery explodes from over charging!");
			take_hit(rand_int(o_ptr->pval * BATTERY_DEV) + o_ptr->pval * BATTERY_DEV,
				"an exploding mana battery");

			/* Destroy a battery in the pack */
			if (item >= 0)
			{
				inven_item_increase(item, -1);
				inven_item_describe(item);
			...
			/* Destroy a battery on the floor */
			else
			{
				floor_item_increase(0 - item, -1);
				floor_item_describe(0 - item);
		...
		{
			msg_print("The battery loses its charge.");
			o_ptr->pval=0;
		}
		else
		{
			msg_print("The battery rejects the charge.");
			o_ptr->pval -= amt;
		}
	}
