...
static errr Term_user_lfb(int n)
{
	...
	while (1)
	{
		...
		prt(" Linux framebuffer display module for Angband, v." LFB_VERSION 
#ifndef USE_GRAPHICS
			"-text"
#endif /* !USE_GRAPHICS */
			, 1, 20);
		...
		prt("", 19, 20);
#else /* USE_GRAPHICS */
		strnfmt(buf, sizeof(buf), " Font: %s", fb_font);
		prt(buf, 7, 20);

		prt("", 8, 20);

		prt("  a) Change font", 9, 20);
...
/*
 * Display the double-width cursor
 */
                          ...
                          const byte *tap, const char *tcp)
{
	...
	for (k = 0; k < n; k++, tcp++, tap++, cp++, ap++)
	{
		...
		int x2 = (*(*tcp ? tcp : cp) & 0x7f) * tilew;
		int y2 = (*(*tap ? tap : ap) & 0x7f) * tileh;
 ...
 */
const char help_lfb[] = "Linux framebuffer, subopts -d<device> -f<font>"
#ifdef USE_GRAPHICS
			"-c<wid>x<hgt>"
...
#ifdef USE_GRAPHICS
	/* These hooks are only used with graphics */
