...
/* This spell should become more useful (more
   ...
   some potent effects only at high level. */
	...
	/* Attacks type */
	int attacks_chaos[6] = {GF_MANA, GF_TIME, GF_ELEC, GF_DISENCHANT, GF_ACID, GF_CHAOS};
	int attacks_normal[6] = {GF_MISSILE, GF_POIS, GF_ELEC, GF_COLD, GF_ACID, GF_FIRE};
	int *attacks = (p_ptr->al_special == PAL_CHAOS ? attacks_chaos : attacks_normal);
	...
	if (die < 8) clone_monster(dir);
		...
		fire_bolt_or_beam(beam - 10, attacks[0], dir,
		                  damroll(3 + ((plev - 1) / 5), 4));
	else if (die < 41) confuse_monster(dir, plev);
	else if (die < 46) fire_ball(attacks[1], dir, 20 + (plev / 2), 3);
	else if (die < 51) lite_line(dir);
	else if (die < 56)
		fire_beam(attacks[2], dir, damroll(3+((plev-5)/6), 6));
	else if (die < 61)
		fire_bolt_or_beam(beam-10, attacks[3], dir,
		                  damroll(5+((plev-5)/4), 8));
	else if (die < 66)
		fire_bolt_or_beam(beam, attacks[4], dir,
		                  damroll(6+((plev-5)/4), 8));
	else if (die < 71)
		fire_bolt_or_beam(beam, attacks[5], dir,
		                  damroll(8+((plev-5)/4), 8));
	else if (die < 76) drain_life(dir, 75);
	else if (die < 81) fire_ball(attacks[2], dir, 30 + plev / 2, 2);
	else if (die < 86) fire_ball(attacks[3], dir, 40 + plev, 2);
	else if (die < 91) fire_ball(attacks[4], dir, 70 + plev, 3);
	else if (die < 96) fire_ball(attacks[5], dir, 80 + plev, 3);
...
void self_knowledge(void)
{
	...
	for (k = INVEN_EQUIP; k < INVEN_TOTAL; k++)
	{
		u32b t1, t2, t3, t4;
	...
	if (p_ptr->cold_touch)
	{
		info[i++] = "Your body is extremely cold.";
	}
	if (p_ptr->fire_touch)
	{
		info[i++] = "Your body is extremely hot.";
	}
	if (p_ptr->acid_touch)
	{
		info[i++] = "Your body is highly acidic.";
	}
	if (p_ptr->elec_touch)
	{
		info[i++] = "Your body is electrocuting.";
	}
	
	if (p_ptr->undead)
	{
		info[i++] = "You are an undead creature.";
	}
	if (p_ptr->pass_wall)
	{
	...
	if (p_ptr->aura_fear)
	{
	    	info[i++] = "You are surrounded with an aura of despair.";
	}
...
#if 0   /* XXX Since multiple weapons can be wielded now, the knowledge would be ambiguous */
...
/*
 * Detect all "lawful" monsters on current panel
 */
bool detect_monsters_lawful(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		/* Detect lawful monsters */
		if (r_ptr->flags3 & (RF3_LAWFUL))
		{
			/* Take note that they are lawful */
			l_ptr->flags3 |= (RF3_LAWFUL);
	...
	{
		/* Describe result */
		msg_print("You sense the presence of lawful beings!");
	}
...
/*
 * Detect all "chaotic" monsters on current panel
 */
bool detect_monsters_chaotic(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		/* Detect chaotic monsters */
		if (r_ptr->flags3 & (RF3_CHAOTIC))
		{
			/* Take note that they are chaotic */
			l_ptr->flags3 |= (RF3_CHAOTIC);
	...
	{
		/* Describe result */
		msg_print("You sense the presence of chaotic beings!");
	}
...
void stair_creation(void)
{
	...
	{
		if (one_in_(2))	cave_set_feat(py, px, FEAT_LESS);
		else cave_set_feat(py, px, FEAT_LESS_SHAFT);
	}
	else if (one_in_(2))
	{
...
bool ident_spell(void)
{
	...
	/* Squelch it? */
	if (item < INVEN_EQUIP) squelch = squelch_itemp(o_ptr, 0, 1);
...
void aggravate_monsters(int who)
{
	...
	else if (sleep2) msg_print("You hear a sudden stirring in the distance!");
...
/*
 * Cast a "jumping ball" spell
 * Assumes there is a monster target
 * Jumps randomly between monsters, eventually losing its force
 * If limit is >0, then it means "jump until damage is lower than limit".
 * If limit is <0, then it means "jump -limit times".
 * Warning: limit = 0 >= no limit!
 */
bool fire_jump_ball(int typ, int dam, int rad, int diminish, int limit)
{
	int x0 = p_ptr->px, y0 = p_ptr->py, x1, y1, iter = 0, cycle;
	
	/* Ensure there is a monster target */
	if (!target_okay() || !p_ptr->target_who) return (FALSE);
	x1 = p_ptr->target_col;
	y1 = p_ptr->target_row;
	...
	{
		/* Project the ball */
		project(-1, rad, y0, x0, y1, x1, dam, typ, PROJECT_BOOM | PROJECT_KILL |
			PROJECT_GRID | PROJECT_ITEM, 0, 0);
		...
		if (limit < 0 && iter >= -(limit)) return (TRUE);
		if (limit > 0 && dam < limit) return (TRUE);
		
		/* Pick new monster target */
		/* Note: ineffective! */
		cycle = 0;
		while (++cycle < 100)
		{
			int m_idx = randint(mon_max);
			monster_type *m_ptr = &mon_list[m_idx];
			...
			/* Must be some other monster */
			if (m_ptr->fx == x0 && m_ptr->fy == y0) continue;
			
			/* Must be projectable */
			if (!los(y1, x1, m_ptr->fy, m_ptr->fx)) continue;
		...
		if (cycle >= 100) return (TRUE);	/* Failed to find anyone */
...
{
	u32b flg = PROJECT_STOP;
	return (fire_bolt_beam_special(GF_OLD_SPEED, dir, 0, MAX_RANGE, flg));
}
...
{
	u32b flg = PROJECT_STOP;
	return (fire_bolt_beam_special(GF_CHARM, dir, damroll (3, plev), MAX_RANGE, flg));
}
...
bool curse_armor(void)
{
	...
	do
	{
	    	slot = rand_int(INVEN_TOTAL - INVEN_EQUIP) + INVEN_EQUIP;
		o_ptr = &inventory[slot];
	}
	while (!(r_ptr->body.body_mask & EQUIP_SLOT(slot)));
...
bool curse_weapon(void)
{
	...
	while (!(r_ptr->body.weapon_mask & EQUIP_SLOT(slot)));
...
/*
 * Brand the weapon
 * This used to brand the current weapon, but with multiple weapons possible
 * things get too compicated
 */
...
/* 
 * "Artifact reforging", available in Adventurers' guild
 * for characters with high enough fame
 *
 * "Reforging" is a very powerful thing. It requires two objects --
 * an artifact and a normal object (not artifact or randart, but
 * possibly an ego-item). Reforging destroys the artifact, and
 * uses its "power" and activation to create randart from object.
 * Object flags, bonuses etc. are preserved. The operation
 * costs (artifact power ^ 2 * 75) gold. Note that it is not
 * recommended to reforge very powerful objects (e.g. PDSM), as
 * it won't really add any significant properties (And, really,
 * what else can you need from a PDSM?)
 */
void do_reforge_artifact()
{
	int item_f, item_o, cost, a_idx;
	object_type *o_ptr, *f_ptr;
	char o_name[80], a_name[80];
	cptr q, s;
	
	/* Let user choose an artifact */
	q = "Use what artifact for reforging? ";
	s = "You have no items at all.";
	if (!get_item(&item_f, q, s, (USE_EQUIP | USE_INVEN))) return;
	
	/* Get the item (in the pack) */
	f_ptr = &inventory[item_f];
	
	/* Ensure it is an artifact */
	if (!f_ptr->name1)
	{
		msg_print("You need a real artifact to perform reforging!");
		return;
	}
	
	a_idx = f_ptr->name1;
	cost = artifact_power(a_idx);
	cost = cost * cost * 75;
	...
	/* Are we really sure */
	object_desc(o_name, sizeof(o_name), f_ptr, FALSE, 0);
	if (!get_check(format("Really use %s? (It will be destroyed!) ", o_name))) return;
	
	/* Let user choose an object for reforging */
	q = "Reforge what? ";
	s = "You have no items at all.";
	if (!get_item(&item_o, q, s, (USE_EQUIP | USE_INVEN))) return;

	/* Get the item (in the pack) */
	o_ptr = &inventory[item_o];

	/* Ensure that it can be reforged */
	
	if (o_ptr->number > 1)
	{
		msg_print("One should be enough!");
		return;
	}
	...
	/* No more artifacts. How sad */
	inven_item_increase(item_f, -1);
	inven_item_describe(item_f);
	...
	/* Create that randart */
	preserve_base_type_hack = TRUE;
	(void)scramble_artifact_random(a_idx, o_ptr);
	preserve_base_type_hack = FALSE;
	...
	/* Allow user to think a name */
	strnfmt(a_name, sizeof(a_name), "%s", quark_str(o_ptr->rart_name));
	if (get_string("Name: ", a_name, sizeof(a_name)))
	{
		o_ptr->rart_name = quark_add(a_name);
	}
	
	/* This is it. Reorder the pack now */
	inven_item_optimize(item_f);
}
