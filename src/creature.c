  ...
  if ((m_ptr->cdis <= MAX_SIGHT) &&
      ...
      if (los(char_row, char_col, (int)m_ptr->fy, (int)m_ptr->fx))
	{
	  ...
	  if (c_ptr->pl || c_ptr->tl ||
	      (find_flag && m_ptr->cdis <= player_light))
	    {
#ifdef ATARIST_MWC
	      holder = CM_INVISIBLE;
...
register int *mm;
{
  ...
  i = m_list[monptr].fy;
  j = m_list[monptr].fx;
  if (cave[i][j].fval==LAVA_FLOOR &&
      !(c_list[m_list[monptr].mptr].cmove & FLIES) &&
      !(c_list[m_list[monptr].mptr].cdefense & IM_FIRE))
   {     /* Burning?  Panic! -DGK */
    for (k=0; k<=4; k++)
     do
      {
     ...
     while ((cave[y][x].fval == LAVA_FLOOR) ||
	    (cave[y][x].fval >= MIN_CLOSED_SPACE));
    return;  /* This'll be a problem if there isn't any free space about;
		shouldn't happen though. -DGK */
   }
  y = i - char_row;
  x = j - char_col;
  ...
  do
      ...
      if (c_ptr->fval != BOUNDARY_WALL)
	{
	  ...
	  if (c_ptr->fval <= MAX_OPEN_SPACE)
	    {
	     if (c_ptr->fval != LAVA_FLOOR ||
		 c_list[m_ptr->mptr].cdefense & IM_FIRE ||
		 movebits & FLIES)
	       do_move = TRUE;
	    }
...
/* Returns true if point is in lava and no egress is available -DGK */
int immerse_lava(y, x)
int y, x;
{
 ...
 for (i=y-1; i<=y+1; i++)
  for (j=x-1; j<=x+1; x++)
   if (in_bounds(i, j))
    flag |= (cave[y][x].fval<=MAX_OPEN_SPACE && cave[y][x].fval!=LAVA_FLOOR);
 return flag;
}
...
int32u *rcmove;
{
  ...
  else if ((m_ptr->confused)||(cave[m_ptr->fy][m_ptr->fx].fval==LAVA_FLOOR &&
			       immerse_lava(m_ptr->fy,m_ptr->fx)))
    {
      mm[0] = randint(9);
      mm[1] = randint(9);
      mm[2] = randint(9);
