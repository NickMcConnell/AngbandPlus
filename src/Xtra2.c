...
bool set_stun(int v)
{
	...
	else
	{
        ...
        if (ring(SV_RING_SAFETY) && !ability(3) && !ability(18)
         ...
         && !ability(50) && !ability(53) && !ability(68)) old_aux = 0;
	...
	else
	{
        ...
        if (ring(SV_RING_SAFETY) && !ability(3) && !ability(18)
         && !ability(23) && !ability(24) && !ability(49) 
         && !ability(50) && !ability(53) && !ability(68))
         {
                new_aux = 0;
         }
...
void gain_exp(s32b amount)
{
	...
	p_ptr->exp += amount;
        ...
        /* Gain some ability points */
        p_ptr->ability_points += amount / 3;
...
void monster_death(int m_idx)
{
	...
	else if (r_ptr->flags1 & (RF1_DROP_CHOSEN))
	{
                ...
                else
		{
			...
			{
                                msg_print("**Ryenna is begging you to spare her life!!!!**");
                                if (!get_com("Spare Ryenna's life? [Y]es, [N]o", &ch)) ch = 0;
                                if (ch == 'Y' || ch == 'y')
                                {
                                        msg_print("Oh thanks a lot!");
                                        msg_print("I will give you this Warlord Sword as a symbol of my defeat!");
                                        /* Get local object */
                                        q_ptr = &forge;
                                        ...
                                        /* Mega-Hack -- Prepare to make a ring of invisibility */
                                        object_prep(q_ptr, lookup_kind(TV_SWORD, SV_WARLORD_SWORD));
                                        q_ptr->number = 1;

                                        apply_magic(q_ptr, 3, TRUE, TRUE, TRUE);
                                ...
                                {
                                        msg_print("I suppose this is the end for me...");
                                        a_idx = ART_DRAGOS;
                                        chance = 100;
                                }
			}
