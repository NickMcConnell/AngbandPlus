...
static errr init_u_info(void)
{
	...
	/* Init the header */
	init_header(&u_head, z_info->u_max, sizeof(artifact_type));

	/* Save a pointer to the parsing function */
	u_head.parse_info_txt = parse_u_info;

	err = init_info("uniq_art", &u_head);
 ...
 */
static const byte store_table[MAX_STORES-3][STORE_CHOICES][2] =
{
	{
		/* General Store */
...
static errr init_other(void)
{
	...
	for (i = 0; i < MAX_STORES; i++)
	{
		...
		/* No table for the black market, guild or home */
	...
	(void)format("%s", PACKAGE_BUGREPORT);
...
void init_angband(void)
{
	...
	/* Initialize artifact info */
	note("[Initializing arrays... (unique art)]");
	if (init_u_info()) quit("Cannot initialize unique artifacts");
