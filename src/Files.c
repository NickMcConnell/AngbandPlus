...
void display_player(int mode)
{
	...
	if ((mode == 0) || (mode == 1))
	{
        ...
        if ((p_ptr->pclass == CLASS_CHAOS_WARRIOR) || (p_ptr->pclass == CLASS_CHAOS_MAGE))
          c_put_str(TERM_L_BLUE, chaos_patron_longs[p_ptr->chaos_patron], 7, 15);
...
long total_points(void)
{
	...
	{
		j=p_ptr->max_exp + (100 * (p_ptr->max_dlv[i]) + dun_defs[i].offset);
		if(j > best) best = j;
	}
	return (best);
}
 ...
 * -KMW-
 */
void show_highclass(int pclass)
{
