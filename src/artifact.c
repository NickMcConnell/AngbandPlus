...
static int random_misc(object_type *o_ptr, int artifact_bias)
{
	...
	{
		case BIAS_RANGER:
			if (!(o_ptr->flags1 & TR1_SUST_CON))
			{
				o_ptr->flags1 |= TR1_SUST_CON;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		
		case BIAS_STR:
			if (!(o_ptr->flags1 & TR1_SUST_STR))
			{
				o_ptr->flags1 |= TR1_SUST_STR;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		
		case BIAS_WIS:
			if (!(o_ptr->flags1 & TR1_SUST_WIS))
			{
				o_ptr->flags1 |= TR1_SUST_WIS;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		
		case BIAS_INT:
			if (!(o_ptr->flags1 & TR1_SUST_INT))
			{
				o_ptr->flags1 |= TR1_SUST_INT;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
			
		case BIAS_DEX:
			if (!(o_ptr->flags1 & TR1_SUST_DEX))
			{
				o_ptr->flags1 |= TR1_SUST_DEX;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		...
		case BIAS_CHR:
			if (!(o_ptr->flags1 & TR1_SUST_CHR))
			{
				o_ptr->flags1 |= TR1_SUST_CHR;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
...
static int random_slay(object_type *o_ptr, int artifact_bias)
{
	...
	if (o_ptr->tval == TV_BOW)
	{
		...
		{
			case 1:
				o_ptr->flags4 |= TR4_XTRA_MIGHT;
				
				if (!artifact_bias && one_in_(9))
					artifact_bias = BIAS_RANGER;
				break;
			case 2:
				o_ptr->flags4 |= TR4_XTRA_SHOTS;
				
				if (!artifact_bias && one_in_(9))
					artifact_bias = BIAS_RANGER;
			case 3:
				o_ptr->flags4 |= TR4_XTRA_FORCE;
				
				if (!artifact_bias && one_in_(9))
					artifact_bias = BIAS_RANGER;
			case 4:
				o_ptr->flags4 |= TR4_PIERC_AC;
				
				if (!artifact_bias && one_in_(9))
					artifact_bias = BIAS_RANGER;
			default:
				o_ptr->flags4 |= TR4_VELOCITY;
	...
	{
		case BIAS_CHAOS:
			if (!(o_ptr->flags4 & TR4_CHAOTIC))
			{
				o_ptr->flags4 |= TR4_CHAOTIC;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		...
		case BIAS_NECROMANTIC:
			if (!(o_ptr->flags4 & TR4_VAMPIRIC))
			{
				o_ptr->flags4 |= TR4_VAMPIRIC;
				if (one_in_(2)) return (artifact_bias);
			}
			if (!(o_ptr->flags4 & TR4_BRAND_POIS) && one_in_(2))
			{
		...
		case BIAS_POIS:
			if (!(o_ptr->flags4 & TR4_BRAND_POIS))
			{
				o_ptr->flags4 |= TR4_BRAND_POIS;
				if (one_in_(2)) return (artifact_bias);
			}
		...
		case BIAS_FIRE:
			if (!(o_ptr->flags4 & TR4_BRAND_FIRE))
			{
				o_ptr->flags4 |= TR4_BRAND_FIRE;
				if (one_in_(2)) return (artifact_bias);
			}
			break;

		case BIAS_COLD:
			if (!(o_ptr->flags4 & TR4_BRAND_COLD))
			{
				o_ptr->flags4 |= TR4_BRAND_COLD;
				if (one_in_(2)) return (artifact_bias);
			}
			break;

		case BIAS_ELEC:
			if (!(o_ptr->flags4 & TR4_BRAND_ELEC))
			{
				o_ptr->flags4 |= TR4_BRAND_ELEC;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		
		case BIAS_ACID:
			if (!(o_ptr->flags4 & TR4_BRAND_ACID))
			{
				o_ptr->flags4 |= TR4_BRAND_ACID;
				if (one_in_(2)) return (artifact_bias);
			}
			break;
		...
		case BIAS_LAW:
			...
			if (!(o_ptr->flags4 & TR4_SLAY_EVIL))
			{
				o_ptr->flags4 |= TR4_SLAY_EVIL;
				if (one_in_(2)) return (artifact_bias);
			}
...
static void give_activation_power(object_type *o_ptr, int artifact_bias)
{
	...
	switch (artifact_bias)
	{
		case BIAS_ELEC:
			chance = 101;
			if (!(o_ptr->flags3 |= TR3_SH_ELEC) && (one_in_(5)))
				type = ACT_AURA_ELEC;
			if (!one_in_(3))
				type = ACT_BO_ELEC_1;
			else if (!one_in_(5))
		...
		case BIAS_FIRE:
			chance = 101;
			if (!(o_ptr->flags3 |= TR3_SH_FIRE) && (one_in_(5)))
				type = ACT_AURA_FIRE;
			if (!one_in_(3))
				type = ACT_BO_FIRE_1;
			else if (!one_in_(5))
		...
		case BIAS_COLD:
			chance = 101;
			if (!(o_ptr->flags3 |= TR3_SH_COLD) && (one_in_(5)))
				type = ACT_AURA_COLD;
			if (!one_in_(3))
				type = ACT_BO_COLD_1;
			else if (!one_in_(3))
	...
	if (a_scroll && one_in_(4))
	{
		switch (p_ptr->pclass)
		{
			...
			case CLASS_THIEF_MAGE:
				if (one_in_(2)) artifact_bias = BIAS_MAGE;
	...
	if (activate > 127)
	{
		switch (activate - 128)
		{
			...
			case ART_GRENADE:
			{
				sound(65);
				if (!get_aim_dir(&dir)) return FALSE;
				msg_print("You pull the pin an throw...");
				(void)fire_ball(GF_HOLY_FIRE, dir, 500, 3);
				o_ptr->timeout = 250;
				break;
			}
...
//				bool rain_effect(GF_ELEC, damroll(5, 3), 1000, 3, int range)
				o_ptr->timeout = 100;
				break;
			}
			...
			case ART_CASPANION:
			{
				...
				(void)destroy_doors(3);
				(void)destroy_traps(3);
				o_ptr->timeout = 10;
				break;
			}
	...
	switch (o_ptr->activate)
	{
		...
		case ACT_MAGIC_STORM:
		{
			msg_print("You feel the mana in the area rumble.");
			(void)fire_ball(GF_MANA, 0, 500, 5);
			(void)set_resist_magic(p_ptr->resist_magic + p_ptr->lev * 3);
			o_ptr->timeout = 550;
			break;
		}
		...
		case ACT_AURA_FIRE:
		{
			msg_print("It flares bright red...");
			(void)set_sh_fire(p_ptr->tim_sh_fire + rand_range(50, 100));
			o_ptr->timeout = 300;
			break;
		}
		...
		case ACT_AURA_ELEC:
		{
			msg_print("It crackles with lightning...");
			(void)set_sh_elec(p_ptr->tim_sh_elec + rand_range(50, 100));
			o_ptr->timeout = 300;
			break;
		}
		...
		case ACT_AURA_COLD:
		{
			msg_print("It releases a white fog...");
			(void)set_sh_cold(p_ptr->tim_sh_cold + rand_range(50, 100));
			o_ptr->timeout = 300;
			break;
		}
		...
		case ACT_AURA_ACID:
		{
			msg_print("It grows threads of green liquid...");
			(void)set_sh_acid(p_ptr->tim_sh_acid + rand_range(50, 100));
			o_ptr->timeout = 300;
			break;
		}
		...
		case ACT_FREE_FALL:
		{
			int time = 100 + p_ptr->lev;
			msg_print("It glows pale blue...");
			set_slow(0);
			(void)set_fast(time);
			(void)set_ffall(p_ptr->ffall + time);
			(void)set_free_act(p_ptr->free_act + time);
		...
		case ACT_INVISIBLE:
		{
			int time = rand_range(25, 25 + p_ptr->lev);
			msg_print("It fades for a moment...");
			(void)set_tim_invis(p_ptr->tim_invis + time);
			(void)set_invisible(p_ptr->tim_nonvis + time);
			o_ptr->timeout = 400;
			break;
		}
		...
		case ACT_DIM_DOOR:
		{
			...
			if (!dimension_door(1)) return FALSE;
			o_ptr->timeout = 100;
			break;
		}
