...
/* parse.c: parsing of various files (mostly lib/edit/xxx.txt ones) */
 ...
 *
 * Actually, I think it might be a good idea. -uav
 */
 ...
 */
	...
	{"LAWFUL", RF3, RF3_LAWFUL},
	{"RF3XXX1", RF3, RF3_RF3XXX1},
	{"CHAOTIC", RF3, RF3_CHAOTIC},
	{"VALA", RF3, RF3_VALA},
 ...
 */
	...
	{"S_KIN", RF7, RF7_S_KIN},
	{"S_ELEM_WATER", RF7, RF7_S_ELEM_WATER},
	{"S_ELEM_FIRE", RF7, RF7_S_ELEM_FIRE},
	...
	{"S_ELEM_AIR", RF7, RF7_S_ELEM_AIR},
	{"S_ELEM_EARTH", RF7, RF7_S_ELEM_EARTH},
 ...
 */
	...
	{"INVISIBILITY", TR3, TR3_INVISIBILITY},
...
/*
 * Object flags 4 (new for PosBand)
 */
 	{"ACID_TOUCH", TR4, TR4_ACID_TOUCH},
 	{"ELEC_TOUCH", TR4, TR4_ELEC_TOUCH},
 	{"FIRE_TOUCH", TR4, TR4_FIRE_TOUCH},
 	{"COLD_TOUCH", TR4, TR4_COLD_TOUCH},
 	{"PASS_WALL", TR4, TR4_PASS_WALL},
 	{"UNDEAD_RACE", TR4, TR4_UNDEAD_RACE},
 	{"ALIGN_CHAOS", TR4, TR4_ALIGN_CHAOS},
 	{"ALIGN_LAW", TR4, TR4_ALIGN_LAW},
 	{"GIANT_WIELD", TR4, TR4_GIANT_WIELD},
 	{"GIANT_WEAR", TR4, TR4_GIANT_WEAR},
 	{"THROW_BOULDER", TR4, TR4_THROW_BOULDER},
 	{"POIS_BLOOD", TR4, TR4_POIS_BLOOD},
 	{"EAT_WALL", TR4, TR4_EAT_WALL},
 	{"SUPER_REGEN", TR4, TR4_SUPER_REGEN},
	{"EAT_STATUE", TR4, TR4_EAT_STATUE},
	{"AURA_FEAR", TR4, TR4_AURA_FEAR},
	{"NO_INNATE", TR4, TR4_NO_INNATE},
...
/* Powers */
/* powers_strings.inc must be generated from powers.m4 via GNU M4 */
#include "powers_strings.inc"
...
errr parse_v_info(char *buf, header *head)
{
	...
	else if (buf[0] == 'X')
	{
		...
		if ((v_ptr->typ == 7) && ((v_ptr->wid > 100) || (v_ptr->hgt > 100)))
			return (PARSE_ERROR_VAULT_TOO_BIG);

		if ((v_ptr->typ == 8) && ((v_ptr->wid > 100) || (v_ptr->hgt > 100)))
			return (PARSE_ERROR_VAULT_TOO_BIG);
	}
	
	/* Process 'R' for requirements */
	else if (buf[0] == 'R')
	{
		int typ, inf;
		...
		if (nreq >= MAX_VAULT_REQ) return (PARSE_ERROR_GENERIC);
		
		if (2 != sscanf(buf+2, "%d:%d", &typ, &inf)) return (PARSE_ERROR_GENERIC);
		
		v_ptr->req[nreq].type = typ;
		v_ptr->req[nreq].info = inf;
		nreq++;
	}

	/* Process 'C' for encodings */
	else if (buf[0] == 'C')
	{
		char cod;
		int rac, ter;
		...
		if (ncod >= MAX_VAULT_COD) return (PARSE_ERROR_GENERIC);
		
		if (3 != sscanf(buf+2, "%c:%d:%d", &cod, &rac, &ter)) return (PARSE_ERROR_GENERIC);
		
		v_ptr->cod[ncod].code = cod;
		v_ptr->cod[ncod].race = rac;
		v_ptr->cod[ncod].terr = ter;
		ncod++;
	}
 ...
 * Grab one flag in an object_kind from a textual string
 */
static errr grab_one_body_flag(monster_race *ptr, cptr what)
{
	u32b *f[MAX_FLAG_SETS];
	C_WIPE(f, MAX_FLAG_SETS, sizeof(u32b*));
	f[TR1] = &(ptr->body.flags1);
	f[TR2] = &(ptr->body.flags2);
	f[TR3] = &(ptr->body.flags3);
	f[TR4] = &(ptr->body.flags4);
	return grab_one_flag(f, "body", what);
}
...
errr parse_u_info(char *buf, header *head)
{
	...
	else if (buf[0] == 'W')
	{
		...
		if (3 != sscanf(buf+2, "%d:%d:%ld",
			            &level, &wgt, &cost)) return (PARSE_ERROR_GENERIC);
...
errr parse_r_info(char *buf, header *head)
{
	...
	else if (buf[0] == 'W')
	{
		...
		/* Scan for the values */
		k = sscanf(buf+2, "%d:%d:%d:%ld:%d", &lev, &rar, &mana, &exp, &extra);
		if (k < 4) return (PARSE_ERROR_GENERIC);
		...
		r_ptr->mana = mana;
		r_ptr->mexp = exp;
		if (k < 5) r_ptr->extra = 0;
		else r_ptr->extra = extra;
	}
	...
	/* Process 'Y' for unique artifact */
		...
		if (1 != sscanf(buf+2, "%d", &u_index)) return (PARSE_ERROR_GENERIC);
		
		r_ptr->artifact = u_index;
	}

	/* Process 'L' for player levels */
	else if (buf[0] == 'L') {

		int bl, el, ei[6] = {0,0,0,0,0,0};

		/* There better be a current r_ptr */
		if (!r_ptr) return (PARSE_ERROR_MISSING_RECORD_HEADER);
		
		if (3 > sscanf(buf+2, "%d:%d:%d:%d:%d:%d:%d:%d", &bl, &el, &ei[0],
			&ei[1], &ei[2], &ei[3], &ei[4], &ei[5])) return (PARSE_ERROR_GENERIC);
		
		r_ptr->body.base_level = bl;
		r_ptr->body.evolve_level = el;
		for (i = 0; i < 6; i++) r_ptr->body.evolve_index[i] = ei[i];
	    
	}
	
	/* Process 'E' for emulated class */
	else if (buf[0] == 'E')
	{
		int class;
		...
		if (1 != sscanf(buf+2, "%d", &class)) return (PARSE_ERROR_GENERIC);
		
		r_ptr->body.class = class;
	}

	/* Process 'A' for player stats */
	else if (buf[0] == 'A') {

		int as, ai, aw, ad, ac, ah, min_toa, min_spd,
		    max_toa, max_spd;
		...
		if (10 != sscanf(buf+2, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d",
		    &as, &ai, &aw, &ad, &ac, &ah, &min_toa, &max_toa, &min_spd, &max_spd))
                    return (PARSE_ERROR_GENERIC);
				
		r_ptr->body.to_stat[A_STR] = as;
		r_ptr->body.to_stat[A_INT] = ai;
		r_ptr->body.to_stat[A_WIS] = aw;
		r_ptr->body.to_stat[A_DEX] = ad;
		r_ptr->body.to_stat[A_CON] = ac;
		r_ptr->body.to_stat[A_CHR] = ah;

		r_ptr->body.to_ac_base = min_toa;
		r_ptr->body.to_spd_base = min_spd;
		r_ptr->body.to_ac_max = max_toa;
		r_ptr->body.to_spd_max = max_spd;

	}
        
	/* Process 'C' for abilities (one line only) */
	else if (buf[0] == 'C')
	{
		int dis, dev, sav, stl, srh, fos;
		...
		if (6 != sscanf(buf+2, "%d:%d:%d:%d:%d:%d",
			            &dis, &dev, &sav, &stl,
			            &srh, &fos)) return (PARSE_ERROR_GENERIC);
	...
	/* Process 'c' for fighting skills (one line only) */
	else if (buf[0] == 'c')
	{
		int bare, edged, hafted, polearm, axe, sling, bow, xbow, throw;
		...
		if (9 != sscanf(buf+2, "%d:%d:%d:%d:%d:%d:%d:%d:%d",
			    &bare, &edged, &hafted, &polearm, &axe,
			    &sling, &bow, &xbow, &throw)) return (PARSE_ERROR_GENERIC);

		/* Save the values */
		r_ptr->body.thn_bare    = bare;
		r_ptr->body.thn_edged   = edged;
		r_ptr->body.thn_hafted  = hafted;
		r_ptr->body.thn_polearm = polearm;
		r_ptr->body.thn_axe     = axe;
		r_ptr->body.thb_sling   = sling;
		r_ptr->body.thb_bow     = bow;
		r_ptr->body.thb_xbow    = xbow;
		r_ptr->body.tht         = throw;
	}

	/* Process 'M' for misc. body information */
	else if (buf[0] == 'M') {

		int hd, infra, pwrstat;

		/* There better be a current r_ptr */
		if (!r_ptr) return (PARSE_ERROR_MISSING_RECORD_HEADER);

		if (3 != sscanf(buf+2, "%d:%d:%d",
		    &hd, &infra, &pwrstat)) return (PARSE_ERROR_GENERIC);
		
		r_ptr->body.hitdie = hd;
		r_ptr->body.infra = infra;
                r_ptr->body.pwrstat = pwrstat;

	}

	/* Process 'K' for body parts */
	else if (buf[0] == 'K') {

		int weapon, bow, ring, amulet, light, body, cloak, shield, helm, glove, boot;
		int cslot = 0;
		...
		if (11 != sscanf(buf+2, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d",
		    &weapon, &bow, &ring, &amulet, &light, &body, &cloak, &shield, &helm, &glove, &boot))
		    	return (PARSE_ERROR_GENERIC);
	
		for (i = 0; i < weapon; i++)
		{
			r_ptr->body.weapon_mask |= 1 << cslot++;
		}
		for (i = 0; i < bow; i++)
		{
			r_ptr->body.bow_mask |= 1 << cslot++;
		}
		for (i = 0; i < ring; i++)
		{
		    	r_ptr->body.ring_mask |= 1 << cslot++;
		}
		for (i = 0; i < amulet; i++)
		{
			r_ptr->body.amulet_mask |= 1 << cslot++;
		}
		for (i = 0; i < light; i++)
		{
			r_ptr->body.light_mask |= 1 << cslot++;
		}
		for (i = 0; i < body; i++)
		{
		    	r_ptr->body.body_mask |= 1 << cslot++;
		}
		for (i = 0; i < cloak; i++)
		{
		    	r_ptr->body.cloak_mask |= 1 << cslot++;
		}
		for (i = 0; i < shield; i++)
		{
		    	r_ptr->body.shield_mask |= 1 << cslot++;
		}
		for (i = 0; i < helm; i++)
		{
		    	r_ptr->body.helm_mask |= 1 << cslot++;
		}
		for (i = 0; i < glove; i++)
		{
		    	r_ptr->body.glove_mask |= 1 << cslot++;
		}
		for (i = 0; i < boot; i++)
		{
		    	r_ptr->body.boot_mask |= 1 << cslot++;
		}

		r_ptr->body.slot_mask =
			(r_ptr->body.weapon_mask | r_ptr->body.bow_mask |
			 r_ptr->body.ring_mask | r_ptr->body.amulet_mask |
			 r_ptr->body.light_mask |
			 r_ptr->body.body_mask | r_ptr->body.cloak_mask |
			 r_ptr->body.shield_mask | r_ptr->body.helm_mask |
			 r_ptr->body.glove_mask | r_ptr->body.boot_mask);
	}

	/* Process 'O' for body flags (multiple lines) */
	else if (buf[0] == 'O')
	{
		/* There better be a current r_ptr */
		if (!r_ptr) return (PARSE_ERROR_MISSING_RECORD_HEADER);
		...
		for (s = buf + 2; *s; )
		{
			...
			/* Parse this entry */
			if (0 != grab_one_body_flag(r_ptr, s)) return (PARSE_ERROR_INVALID_FLAG);
	...
	else if (buf[0] == 'P')
	{
		...
		/* Find the next empty power slot (if any) */
		for (i = 0; i < PWR_BODY_MAX; i++) if (!r_ptr->body.pwr[i].type) break;

		/* Oops, no more slots */
		if (i == PWR_BODY_MAX) return (PARSE_ERROR_GENERIC);
		...
		/* Analyze type */
		for (n = 1; r_info_pwr[n]; n++)
		{
			if (streq(s, r_info_pwr[n])) break;
		}

		/* Invalid type */
		if (!r_info_pwr[n]) return (PARSE_ERROR_GENERIC);

		/* Save the type */
		r_ptr->body.pwr[i].type = n;
		...
		if (*t == 'd') *t++ = '\0';
                ...
                /* Get failrate */
                r_ptr->body.pwr[i].fail = atoi(s);
                
		/* Analyze the fifth field */
		for (s = t; *t && (*t != 'd'); t++) /* loop */;
		...
		if (*t == 'd') *t++ = '\0';
                ...
                /* Get extra */
                r_ptr->body.pwr[i].extra = atoi(s);
	}

	/* Process 'Z' for corpse info */
	else if (buf[0] == 'Z')
	{
		int wgt;
		...
		if (1 != sscanf(buf+2, "%d", &wgt)) return (PARSE_ERROR_GENERIC);
		
		s = strchr(buf+2, ':');
		if (!s) return (PARSE_ERROR_GENERIC);
		s++;
		
		strncpy(r_ptr->body.corpsename, s, 15);
		r_ptr->body.weight = wgt;
	}
...
errr parse_p_info(char *buf, header *head)
{
	...
	/* Process 'r' for fighting skills (one line only) */
	else if (buf[0] == 'r')
	{
		int bare, edged, hafted, polearm, axe, sling, bow, xbow, throw;
		...
		/* Save the values */
		pr_ptr->r_thn_bare    = bare;
		pr_ptr->r_thn_edged   = edged;
		pr_ptr->r_thn_hafted  = hafted;
		pr_ptr->r_thn_polearm = polearm;
		pr_ptr->r_thn_axe     = axe;
		pr_ptr->r_thb_sling   = sling;
		pr_ptr->r_thb_bow     = bow;
		pr_ptr->r_thb_xbow    = xbow;
		pr_ptr->r_tht         = throw;
	}
	...
	{
		int mhp, exp, infra, comp;

		/* There better be a current pr_ptr */
		if (!pr_ptr) return (PARSE_ERROR_MISSING_RECORD_HEADER);

		/* Scan for the values */
		if (3 > sscanf(buf+2, "%d:%d:%d:%d",
			            &mhp, &exp, &infra, &comp)) return (PARSE_ERROR_GENERIC);
	...
	/* Hack -- Process 'M' for "Monster Race" */
	else if (buf[0] == 'M')
	{
		int m_r_idx;
		...
		/* Scan for the values */
		if (1 != sscanf(buf+2, "%d", &m_r_idx)) return (PARSE_ERROR_GENERIC);
		
		pr_ptr->m_r_idx = m_r_idx;
	}

	/* Hack -- Process 'K' for "King Unique" */
	else if (buf[0] == 'K')
	{
		int king_r_idx;
		...
		/* Scan for the values */
		if (1 != sscanf(buf+2, "%d", &king_r_idx)) return (PARSE_ERROR_GENERIC);
		
		pr_ptr->king_r_idx = king_r_idx;
	}

	/* Hack -- Process 'P' for "Pet" */
	else if (buf[0] == 'P')
	{
		int pet_r_idx;
		...
		/* Scan for the values */
		if (1 != sscanf(buf+2, "%d", &pet_r_idx)) return (PARSE_ERROR_GENERIC);
		
		pr_ptr->pet_r_idx = pet_r_idx;
	}
...
errr parse_c_info(char *buf, header *head)
{
	...
	else if (buf[0] == 'c')
	{
		...
		/* Save the values */
		pc_ptr->c_thn_bare    = bare;
		pc_ptr->c_thn_edged   = edged;
		pc_ptr->c_thn_hafted  = hafted;
		pc_ptr->c_thn_polearm = polearm;
		pc_ptr->c_thn_axe     = axe;
		pc_ptr->c_thb_sling   = sling;
		pc_ptr->c_thb_bow     = bow;
		pc_ptr->c_thb_xbow    = xbow;
		pc_ptr->c_tht         = throw;
	}
	...
	/* Process 'x' for fighting skill modifiers (one line only) */
	else if (buf[0] == 'x')
	{
		int bare, edged, hafted, polearm, axe, sling, bow, xbow, throw;
		...
		/* Save the values */
		pc_ptr->x_thn_bare    = bare;
		pc_ptr->x_thn_edged   = edged;
		pc_ptr->x_thn_hafted  = hafted;
		pc_ptr->x_thn_polearm = polearm;
		pc_ptr->x_thn_axe     = axe;
		pc_ptr->x_thb_sling   = sling;
		pc_ptr->x_thb_bow     = bow;
		pc_ptr->x_thb_xbow    = xbow;
		pc_ptr->x_tht         = throw;
	}
...
errr parse_g_info(char *buf, header *head)
{
	...
	if (buf[0] == 'A')
	{
		...
		/* Hack -- Initialize the counter to max normal races */
		j = MON_RACE_MIN;
...
/*
 * Generate a random description for the artifact
 */
void gen_rart_desc(object_type *o_ptr)
{
	char desc[2048], buf[120];
	FILE *fff = NULL;
	int k;
	bool rn;
	...
	/* Open template file */
	path_build(buf, sizeof(buf), ANGBAND_DIR_FILE, "randart.txt");
	fff = my_fopen(buf, "r");
	if (!fff)
	{
		msg_print("randart.txt file missing!");
		return;
	}
	...
	while (1)
	{
		...
		{
			msg_print("randart.txt file broken! (no version stamp)");
			return;
		}
		...
		if (3 != sscanf(buf, "V:%d.%d.%d", &v1, &v2, &v3) ||
			v1 != VERSION_MAJOR || v2 != VERSION_MINOR || v3 != VERSION_PATCH)
		{
			msg_format("randart.txt version mismatch: %d.%d.%d", v1, v2, v3);
			return;
		}
		else break;
	}
	...
	/* Generate beginning */
	k = rand_int(100); /* Random */
	rn = FALSE;
	while (1)
	{
		int tval, sval, chance;
		
		if (my_fgets(fff, buf, sizeof(buf))) return;
		
		if (buf[0] == '#' || buf[0] == '\0') continue;
		
		if (buf[0] == 'B')
		{
			sscanf(buf+2, "%d:%d:%d", &tval, &sval, &chance);
			if (o_ptr->tval == tval && (o_ptr->sval == sval || sval == 0) && k < chance)
				rn = TRUE;
		}
	...
	/* Hack -- add pseudo-beginning if none found */
	if (k != 101) strcat(desc, "It is a great artifact");
	...
	while (1)
	{
		...
		{
			bool do_that = FALSE;
			if (one_in_(chance2) || p_ptr->wizard)
			{
				/* Check whether this flag is present */
				
				int i;
				
				for (i = 0; i < N_ELEMENTS(info_flags); i++)
				{
					if (!strcmp(flg, info_flags[i].name))
					{
/* msg_format("set: %d name: %s flag: %x o_ptr->flags3: %x", info_flags[i].set, info_flags[i].name, info_flags[i].flag, o_ptr->flags3); */
						/* Special handle of curse */
						if ((info_flags[i].set == TR3) && (info_flags[i].flag == TR3_LIGHT_CURSE) && cursed_p(o_ptr))
						{
							do_that = TRUE;
							break;
						}
						if ((info_flags[i].set == TR1) && (o_ptr->flags1 & info_flags[i].flag))
						{
							do_that = TRUE;
							/* Certain flags would be reasonable only with enough pval */
							if (info_flags[i].flag & TR1_PVAL_MASK && o_ptr->pval < 1) do_that = FALSE;
							if (info_flags[i].flag & (TR1_ALL_STATS | TR1_STEALTH |
								TR1_SEARCH | TR1_INFRA | TR1_SPEED) && o_ptr->pval < 3) do_that = FALSE;
						}
						if ((info_flags[i].set == TR2) && (o_ptr->flags2 & info_flags[i].flag)) do_that = TRUE;
						if ((info_flags[i].set == TR3) && (o_ptr->flags3 & info_flags[i].flag)) do_that = TRUE;
						if ((info_flags[i].set == TR4) && (o_ptr->flags4 & info_flags[i].flag)) do_that = TRUE;
						break;
					}
				}
			}
			...
			if (do_that)
			{
				/* More sophisticated than just strcat() */
				char *s, *t;
	...
	if (desc[strlen(desc) - 2] == ',') desc[strlen(desc) - 2] = '.';
	
	/* Store description as quark */
	o_ptr->rart_desc = quark_add(desc);
}
