...
bool borg_check_lite(void)
{
	...
	if (borg_cave_info[py][px] & (CAVE_DARK))
	{
		...
		for (i = 0; i < 8; i++)
		{
			...
			if (!(borg_cave_info[y][x] & CAVE_DARK)) continue; 
	...
	/* Hack -- Wizard Lite */
	if (!borg_when_wizard_lite || (borg_time - borg_when_wizard_lite >= 1000))
	{
		/* Wizard lite */
...
bool borg_leave_level(bool bored)
{
	...
	int goal = borg_fleeing ? GOAL_FLEE : GOAL_BORE;
	...
	if (bored && !borg_completed)
	{
		...
		/* Continue if above happy depth and not in town */
		if (b_ptr->depth && (b_ptr->depth < borg_happy_depth)) bored = FALSE;

		/* Track happiness */
		if (b_ptr->depth >= borg_happy_depth)
		{
			/* Track happiness */
			if (++borg_happy_count >= 10)
			{
				/* Reset happiness */
				borg_happy_count = 0;
	...
	if (!b_ptr->depth)
	{
		...
		if ((b_ptr->max_depth >= 5) &&
		    (amt_recall > 4) &&
			 borg_prepared(borg_happy_depth) &&
		    borg_recall())
		{
			/* Note */
		...
		if (borg_flow_stair_more(goal)) return (TRUE);
	...
	/* Dive when really bored */
	if (borg_prepared(b_ptr->depth + 1))
	{
		if ((b_ptr->depth < borg_happy_depth) || (bored && (borg_time - borg_when_began >= 5000)))
		{
			/* Note */
			borg_note("# Diving.");
	...
	/* Go to an easier level when really bored */
	else if (bored && (borg_time - borg_when_began >= 5000))
	{
		/* Note */
		borg_note("# Climbing.");
	...
	if ((g == 1) && !borg_prepared(b_ptr->depth + 1)) g = 0;
	...
	if (g && (borg_time - borg_when_began <
		 (u32b)value_feeling[borg_feeling] * b_ptr->depth / borg_happy_depth)) g = 0;

	/* Use random stairs when scared */
	if (borg_fleeing)
	{
		/* Note */
	...
	if (!borg_prepared(b_ptr->depth))
	{
		/* Note */
		borg_note("# Fleeing level (unprepared)");
	...
	if (bored)
	{
		...
		for (i = 0; i < 6; i++)
		{
			if (borg_base_fix_stat[i]) borg_rising = TRUE;
		}
	}
	...
	if (g < 0)
	{
		...
		if (borg_flow_stair_less(goal)) return (TRUE);
...
static bool borg_good_sell(auto_item *item, int who)
{
	...
	if (!item->able && !item->average)
	{
		switch(item->tval)
		{
			...
			case TV_BOLT:
			{
				if ((!item->dd && k_info[item->kind].dd) ||
					 (!item->ds && k_info[item->kind].ds))
				{
					return FALSE;
				}
			}
			...
			case TV_BOOTS:
			{
				if (!item->ac && k_info[item->kind].ac)
				{
					return FALSE;
				}
			}
...
static bool borg_think_dungeon_brave(void)
{
	...
	/* Flee bad levels */
	if (!borg_prepared(b_ptr->depth)) borg_leaving = TRUE;

	/* Leave the level */
	if (borg_leaving && borg_leave_level(FALSE)) return TRUE;
...
bool borg_think_dungeon(void)
{
	...
	{
		/* Reset */
		borg_time = borg_when_began + 10000;
	}
	...
	if (borg_boost < b_ptr->chp)
	{
		...
		/* Try something brave */
		if (borg_think_dungeon_brave()) return (TRUE);
	}
