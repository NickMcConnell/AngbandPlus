...
{
  "Angband.angband.iconName:   FAangband",
  "Angband.angband.title:      FAangband",
...
# else /* USE_TRANSPARENCY */
     static errr Term_pict_xaw(int x, int y, int n, const byte *ap, 
			       const char *cp)
# endif /* USE_TRANSPARENCY */
{
  term_data *td = (term_data*)(Term->data);
...
errr init_xaw(int argc, char *argv[])
{
  ...
  if (use_graphics)
      ...
      for (i = 0; i < num_term; i++)
	{
	  ...
	  td->widget->angband.TmpImage = 
	    XCreateImage(dpy, visual,depth, ZPixmap, 0, TmpData,
			 td->widget->angband.fontwidth,
