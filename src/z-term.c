...
/*
 * Dump (a part of) a terminal to file. Only the characters, not the attributes are dumped.
 * this function may not be compatible like all others since it calls fprintf directly.
 */
errr Term_dump_chars(FILE *f, int x1, int y1, int x2, int y2, int x_off, int y_off)
{
   int i,j;
   ...
   if (my_x1 < 0) my_x1 = 0;
   if (my_y1 < 0) my_y1 = 0;
   if (my_x2 > Term->wid) my_x2 = Term->wid-1;
   if (my_y2 > Term->hgt) my_y2 = Term->hgt-1;

   fprintf(f, "dump in: %d,%d to %d,%d doing: %d,%d to %d,%d min %d,%d max %d,%d\n",
               x1,y1,x2,y2, my_x1,my_y1,my_x2,my_y2, 0, 0, Term->wid, Term->hgt);
   /*
    * first print lines like
    *
    *     1      2         3
    *     345678901234567890
    * 23
    * 24  <insert real screen dump from 13,23 to 30,25 here>
    * 25
    */
   fprintf(f, "    ");
   for (j = my_x1; j < my_x2; j++)
   {
      if ( (j==my_x1) || (j == (my_x2-1) ) || ( ((j+x_off) % 10) == 0) )
      {
         fprintf(f, "%d", (j+x_off) / 100);
      }
      else
      {
         fprintf(f, " ");
   ...
   for (j = my_x1; j < my_x2; j++)
   {
      fprintf(f, "%d", (j+x_off) % 10);
   }
   fprintf(f, "\n");
   /* Scan every row */
   for (i = my_y1; i < my_y2; i++)
   {
      scr_cc = Term->scr->c[i];

      fprintf(f, "%03d ", i+y_off);
      for (j = my_x1; j <= my_x2; j++)
      {
         fprintf(f, "%c", scr_cc[j]);
      }
      fprintf(f, " %03d\n", i+y_off);
   }
