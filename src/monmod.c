...
{
        int r_idx, y, x, slp = FALSE, grp = TRUE, chr = FALSE;
	int retval;

	/* Parse arguments */
        if (!PyArg_ParseTuple(args, "(ii)i|ii", &y, &x, &r_idx, &slp, &grp, &chr))
		return NULL;

	/* Summon */
        retval = place_monster_aux(y, x, r_idx, slp, grp, chr);
