...
static int value_check(object_type *o_ptr, int mode)
{
	int value = value_check_aux(o_ptr);
	
	switch (mode)
	{
		/* no curse sensing, no quality sensing */
		case 0:
		{
			switch (value)
			{
				...
				default:
					return (INSCRIP_MAGIC);
			}
		}
		
		/* curse sensing, no quality sensing */
		case 1:
		{
			switch (value)
			{
		...
		/* no curse sensing, but quality sensing */
		case 2:
		{
			switch (value)
			{
				...
				case INSCRIP_SPECIAL:
					return (INSCRIP_UNIQUE);
				case INSCRIP_WORTHLESS:
				case INSCRIP_EXCELLENT:
					return (INSCRIP_EXTRAORDINARY);
				case INSCRIP_CURSED:
				case INSCRIP_GOOD:
					return (INSCRIP_MAGIC);
				default:
 ...
 * Sense the inventory
 *
 * Pseudo-ID works every X turns, where X is determined by
 * class, level, and magical enhancement.
 * In case of magical enhancement, pseudo-ID is always "strong",
 * otherwise it depends on class.
 * Pseudo-ID is impossible in case of too large minusses to pseudo-ID,
 * and severely by any minus to pseudo-ID.
 *
 */
...
static void sense_inventory(void)
{
	...
	/* Level affects pseudo-ID speed */
	ps_speed += plev;
	
	/* Magical pseudo-ID enhancement */
	ps_speed += p_ptr->pseudo_id * 10;
	
	/* Do we have pseudo-ID at all? */
	if (ps_speed < 0) return;
	
	/* Force sane values */
	if (ps_speed > 160 - 1) ps_speed = 160 - 1;
	
	/* Sense now? */
	if (0 != rand_int(ps_id_table[ps_speed])) return;
	
	/* Some people have a better pseudo-ID */
	if (p_ptr->pseudo_id > 0) mode = 3;

	/* Some people have really bad pseudo-ID */
	if (p_ptr->pseudo_id < 0) mode = 0;
...
static void process_world(void)
{
	...
	else
	{
		...
		if (!(turn % (10L * STORE_TURNS)))
		{
			...
			if (cheat_xtra) msg_print("Updating Shops...");
                        if (p_ptr->precognition) msg_print("New stuff is arriving in the stores.");
			...
			if (rand_int(STORE_SHUFFLE) == 0)
			{
				...
				if (p_ptr->precognition) msg_print("One of the shopkeepers is leaving town.");
	...
	/* Handle psionic stuff */
	{
		/* Awareness */
		if (p_ptr->awareness)
		{
			(void)set_awareness(p_ptr->awareness - 1);	
		}		

		/* Adrenaline */
		if (p_ptr->adrenaline)
		{
			(void)set_adrenaline(p_ptr->adrenaline - 1);
		}
		
		/* Biofeedback */				
		if (p_ptr->biofeedback)
                {
                	(void)set_biofeedback(p_ptr->biofeedback - 1);
                }
		...
		{
			(void)set_inertial_barrier(p_ptr->inertial_barrier - 1);
		}
		...
		{
			(void)set_precognition(p_ptr->precognition - 1);
                }
                
		/* Time/Space Anchor */	
		if (p_ptr->ts_anchor)
		{
			(void)set_ts_anchor(p_ptr->ts_anchor - 1);
		}
		...
		if (p_ptr->intensify)
		{
			i = p_ptr->csp_frac - (0x10000 * p_ptr->intensify * p_ptr->intensify) / 100;
			if (i < 0)
			{
				if (p_ptr->csp == 0)
				{
		...
		for (i = 0 ; i < no_fw ; i++)
		{
			...
			if (p_ptr->csp == 0)
			{
				...
				for (; no_fw > i ; no_fw--)
					cave_set_feat(fw_y[no_fw-1], fw_x[no_fw-1], FEAT_FLOOR);
				p_ptr->redraw |= PR_MAP;
			}
			else
			{
	...
	if (p_ptr->word_recall)
	{
		...
		if (!p_ptr->word_recall)
		{
			...
			/* Time/space anchor prevents recalling */
			if (p_ptr->ts_anchor)
			{
...
static void process_command(void)
{
	...
	switch (p_ptr->command_cmd)
	{
		...
		/* Cast a spell or a psionic power*/
		case 'm':
		{
...
static void process_player(void)
{
	...
	if (p_ptr->resting < 0)
	{
		/* Intensify doesn't work well with resting */
		if (p_ptr->intensify)
		{
			disturb(0, 0);
		}
