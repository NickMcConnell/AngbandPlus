...
static void process_world(void)
{
	...
	/* Oppose Light */
	if (p_ptr->oppose_light)
	{
                (void)set_oppose(p_ptr->oppose_light - 1, &(p_ptr->oppose_light), "light");
	}

        /* Oppose Darkness */
        if (p_ptr->oppose_dark)
        {
                (void)set_oppose(p_ptr->oppose_dark-1, &(p_ptr->oppose_dark), "darkness");
        }

        /* Oppose Confusion */
        if (p_ptr->oppose_conf)
        {
                (void)set_oppose(p_ptr->oppose_conf-1, &(p_ptr->oppose_conf), "confusion");
        }

        /* Oppose Shards */
        if (p_ptr->oppose_shards)
        {
                (void)set_oppose(p_ptr->oppose_shards-1, &(p_ptr->oppose_shards), "shards");
        }

        /* Oppose Time */
        if (p_ptr->oppose_time)
        {
                (void)set_oppose(p_ptr->oppose_time-1, &(p_ptr->oppose_time), "time");
        }
...
static void process_command(void)
{
	...
	switch (p_ptr->command_cmd)
	{
		...
		case '_':
		{
			int id = cave_feat[p_ptr->py][p_ptr->px];

			if ((id < FEAT_SHOP_HEAD) || (id > FEAT_SHOP_TAIL))
			{
				msg_print("There is no entrance here.");
				break;
			}

			if (id == FEAT_SHOP_HEAD+7)
			{
				do_cmd_home();
			}
			else
			{
				do_cmd_guild(id - FEAT_SHOP_HEAD);
			}
