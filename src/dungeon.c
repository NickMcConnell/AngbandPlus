...
#ifdef TC_COLOR
    {
     colr(&t_list[cave_ptr->tptr]);
     return t_list[cave_ptr->tptr].tchar;
    } /* else-if clause */	 
#else
    return t_list[cave_ptr->tptr].tchar;
#endif
  else if (cave_ptr->fval == LAVA_FLOOR)
	{
#ifdef TC_COLOR
	 if (!no_color_flag)
	   {
...
void dungeon()
{
  ...
  if (unfelt) feel_chance = 1000; /* 1 in 1000 chance */
	...
	alloc_monster(1, MAX_SIGHT, FALSE);
      if (!(turn % 20))
	regen_monsters();
      if ((cave[char_row][char_col].fval == LAVA_FLOOR)&&(!free_turn_flag))
       {
	msg_print("Lava!!");
	fire_dam(randint(30)+30,"lava");
       }
      /* Check light status			       */
      i_ptr = &inventory[INVEN_LIGHT];
      if (player_light)
      ...
      if (f_ptr->shield > 0)
	{
	  ...
	  if (f_ptr->shield == 0)
	    {
	      disturb (0, 0);
	      msg_print("Your feel less protected.");
...
*/
    ...
    case 't':		/* (t)hrow something	(f)ire something */
      ...
      throw_object(FALSE);
      break;
    case 's':            /* (s)hoot something from quiver if ready */
      feel_chance = 30;
      if (using_quiver)
	throw_object(TRUE);
       else
    ...
    case 'r':		/* (r)ead */
      ...
      read_scroll();
      break;
    case 'I':		/* (I)nvestigate (search) for a turn */
      feel_chance = 1000;
...
static void activate() {
  ...
  while (!flag && get_com(out_str, &choice)) {
	...
	if (inventory[i].name2 == SN_CUBRAGOL) {
	  if (using_quiver && inventory[INVEN_QUIVER].tval == TV_BOLT &&
	      inventory[INVEN_QUIVER].name2==SN_NULL)
	    a = INVEN_QUIVER;
	  else {
	   for (a=0; a<INVEN_WIELD; a++)
	    if (inventory[a].tval == TV_BOLT  && inventory[a].name2==SN_NULL)
	      break;
	  }
	  if (a<INVEN_WIELD && (inventory[a].name2 == SN_NULL)) {
	    ...
	    i = 3 + randint(3);
	    for (j=0; j<i; j++)
	      enchant(&i_ptr->tohit,10);
	    i = 3 + randint(3);
	    for (j=0; j<i; j++)
	      enchant(&i_ptr->todam,10);
	    i_ptr->cost+=25;
	    calc_bonuses ();
	  } else {
	...
	if (get_dir(NULL, &dir)) {
	  ...
	  choice = randint(2);
	  sprintf(tmp2, "You breathe %s...",
		  ((choice==1?"darkness":"nether")));
	  msg_print(tmp2);
	  sprintf(tmp2, "huge %s of %s", ((choice==1)?"sphere":"ball"),
		  ((choice==1?"Darkness":"Nether")));
	  fire_ball((choice==1 ? GF_DARK : GF_NETHER), dir,
	...
	if (get_dir(NULL, &dir)) {
	  ...
	  fire_ball(GF_SHARDS, dir, char_row, char_col,
		    250, 2, "huge ball of Shards");
	  inventory[i].timeout=300+randint(300);
        }
        break;
      case (418): /* Pseudo */
	if (get_dir(NULL, &dir)) {
	  ...
	  fire_ball((choice==1 ? GF_LIGHT : GF_DARK), dir,
		char_row, char_col, 100, 2, tmp2);
	  inventory[i].timeout=300+randint(300);
	}
	break;
      case (419): /* Celestial Dragon Scale Mail */
	msg_print("Tiamat's scales glow a bright blue...");
	py.flags.invuln += randint(12)+12;
	inventory[i].timeout=100;
	break;
      case (SPECIAL_OBJ+3):
...
/* Make the door harder to open.  Split out of jamdoor() -DGK */
int spikedoor(y,x,numspikes)
int y,x,numspikes;
{
 register inven_type *t_ptr;
 register cave_type *c_ptr;
