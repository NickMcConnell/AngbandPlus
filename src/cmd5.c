...
void do_poly_wounds(void)
{
	...
	s16b change = damroll(p_ptr->level, 5);
...
void do_poly_self(void)
{
	...
	msg_print("A change comes over you...");
		...
		switch (randint(14))
		{
			...
			case 12:
				...
				if (randint(6)==1)
				{
					...
					take_hit(damroll(randint(p_ptr->level),p_ptr->level), "a lethal mutation");
				}
				/* No break; here! */
			default:
				mutate_player();
...
static void call_the_(void)
{
	...
	else
	{
		...
		destroy_area(py, px, 20+(p_ptr->level), TRUE);
...
void fetch(int dir, int wgt, bool require_los)
{
	...
	else
	{
		...
		do
		{
			...
			if ((distance(py, px, ty, tx) > MAX_RANGE) || !cave_floor_bold(ty, tx)) return;
	...
	cave_o_idx[ty][tx] = 0;
	cave_o_idx[py][px] = o_idx; /* 'move' it */
	o_ptr->iy = py;
	o_ptr->ix = px;
...
void wild_magic(int spell)
{
	...
	switch(randint(spell) + randint(8) + 1)
	{
		...
		case 35:
			while (counter++ < 8)
			{
				(void)summon_specific(py, px, (p_ptr->depth * 3) / 2, type, TRUE, FALSE);
			}
			break;
		case 36:
		case 37:
	...
	if (rand_int(100) < chance)
	{
		...
		if (randint(100) < (chance/2))
		{
			...
			else
			{
				...
				project(0, 2+plev/10, py, px, plev * 2,
