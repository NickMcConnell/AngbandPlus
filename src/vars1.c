...
int8u equippy_chars = FALSE;
int8u quick_msgs = FALSE;
int8u delay_spd = 5; /* 1-10 for delays -DGK */
#ifdef TC_COLOR
...
int delay_spd = 5; /* 1-10 for delays -DGK */
#endif
...
/* Global variable to replace use of INVEN_WIELD as an index into the
   struct, inventory, indicating where the equipment list begins. If
   the quiver exists then the last slot of the pack inventory (which
   will always be 'v' if the savefile format is not changed) is "stolen"
   for use as a quiver.  Since bolts/arrows/shots/etc already sort to
   here, this is very easy to implement.  Note that equip_top should always
   == using_quiver ? INVEN_QUIVER : INVEN_WIELD if the quiver code is
   included or always INVEN_WIELD otherwise.                    -JLS*/
#ifdef MSDOS
...
#ifdef MSDOS
int8u using_quiver = FALSE; /* stores status of the quiver -JLS */
#else
...
#ifdef MSDOS
int16u feel_chance;  /* Feeling variables -SAC */
int8u unfelt;
bigvtype feeling;
int8u spec_level;    /* Special Level Flag -DGK */
int8u in_store_flag; /* Flag so equippy chars work right -DGK */
#else
int feel_chance;     /* Feeling variables -SAC */
int unfelt;
bigvtype feeling;
int spec_level;      /* Special Level Flag -DGK */
int in_store_flag;   /* Flag so equippy chars work right -DGK */
#endif
