...
struct _term_data
{
	...
	/* Font information for 25-line mode */
	cptr font_want_25;
...
static void term_getsize(term_data *td)
{
	...
	{
		/* Paranoia */
		if (td->cols_25 < 1) td->cols_25 = 1;
		if (td->rows_25 < 1) td->rows_25 = 1;

		wid = td->cols_25 * td->tile_wid_25 + td->size_ow1 + td->size_ow2;
		hgt = td->rows_25 * td->tile_hgt_25 + td->size_oh1 + td->size_oh2;
	}
	else
	{
		/* Paranoia */
		if (td->cols_50 < 1) td->cols_50 = 1;
		if (td->rows_50 < 1) td->rows_50 = 1;

		wid = td->cols_50 * td->tile_wid_50 + td->size_ow1 + td->size_ow2;
		hgt = td->rows_50 * td->tile_hgt_50 + td->size_oh1 + td->size_oh2;
	}
	...
	if (screen_rows != 50)
	{
		td->size_wid_25 = rc.right - rc.left;
		td->size_hgt_25 = rc.bottom - rc.top;
	}
	else
	{
		td->size_wid_50 = rc.right - rc.left;
		td->size_hgt_50 = rc.bottom - rc.top;
	}
...
static void save_prefs_aux(term_data *td, cptr sec_name)
{
	...
	/* Font */
	strcpy(buf, td->font_file_25 ? td->font_file_25 : "9x15B.FON");
	WritePrivateProfileString(sec_name, "Font_25", buf, ini_file);

	/* Tile size (x) */
	wsprintf(buf, "%d", td->tile_wid_25);
	WritePrivateProfileString(sec_name, "TileWid_25", buf, ini_file);

	/* Tile size (y) */
	wsprintf(buf, "%d", td->tile_hgt_25);
	WritePrivateProfileString(sec_name, "TileHgt_25", buf, ini_file);

	/* Window size (x) */
	wsprintf(buf, "%d", td->cols_25);
	WritePrivateProfileString(sec_name, "NumCols_25", buf, ini_file);

	/* Window size (y) */
	wsprintf(buf, "%d", td->rows_25);
	WritePrivateProfileString(sec_name, "NumRows_25", buf, ini_file);


	/* Font */
	strcpy(buf, td->font_file_50 ? td->font_file_50 : "8X8B.FON");
	WritePrivateProfileString(sec_name, "Font_50", buf, ini_file);

	/* Tile size (x) */
	wsprintf(buf, "%d", td->tile_wid_50);
	WritePrivateProfileString(sec_name, "TileWid_50", buf, ini_file);

	/* Tile size (y) */
	wsprintf(buf, "%d", td->tile_hgt_50);
	WritePrivateProfileString(sec_name, "TileHgt_50", buf, ini_file);

	/* Window size (x) */
	wsprintf(buf, "%d", td->cols_50);
	WritePrivateProfileString(sec_name, "NumCols_50", buf, ini_file);

	/* Window size (y) */
	wsprintf(buf, "%d", td->rows_50);
	WritePrivateProfileString(sec_name, "NumRows_50", buf, ini_file);
	...
	if (screen_rows != 50)
	{
		WritePrivateProfileString(sec_name, "PositionX_25", buf, ini_file);
	}
	else
	{
		WritePrivateProfileString(sec_name, "PositionX_50", buf, ini_file);
	}
	...
	if (screen_rows != 50)
	{
		WritePrivateProfileString(sec_name, "PositionY_25", buf, ini_file);
	}
	else
	{
		WritePrivateProfileString(sec_name, "PositionY_50", buf, ini_file);
	}
...
static void load_prefs_aux(term_data *td, cptr sec_name)
{
	...
	/* Desired font for 25-line mode, with default */
	GetPrivateProfileString(sec_name, "Font_25", "9X15B.FON", tmp, 127, ini_file);

	/* Analyze font, save desired font name */
	td->font_want_25 = string_make(analyze_font(tmp, &wid, &hgt));

	/* Tile size */
	td->tile_wid_25 = GetPrivateProfileInt(sec_name, "TileWid_25", wid, ini_file);
	td->tile_hgt_25 = GetPrivateProfileInt(sec_name, "TileHgt_25", hgt, ini_file);

	/* Window size */
	td->cols_25 = GetPrivateProfileInt(sec_name, "NumCols_25", td->cols_25, ini_file);
	td->rows_25 = GetPrivateProfileInt(sec_name, "NumRows_25", td->rows_25, ini_file);

	/* Window position */
	td->pos_x_25 = GetPrivateProfileInt(sec_name, "PositionX_25", td->pos_x_25, ini_file);
	td->pos_y_25 = GetPrivateProfileInt(sec_name, "PositionY_25", td->pos_y_25, ini_file);


	/* Desired font for 50-line mode, with default */
	GetPrivateProfileString(sec_name, "Font_50", "8X8B.FON", tmp, 127, ini_file);

	/* Analyze font, save desired font name */
	td->font_want_50 = string_make(analyze_font(tmp, &wid, &hgt));

	/* Tile size */
	td->tile_wid_50 = GetPrivateProfileInt(sec_name, "TileWid_50", wid, ini_file);
	td->tile_hgt_50 = GetPrivateProfileInt(sec_name, "TileHgt_50", hgt, ini_file);

	/* Window size */
	td->cols_50 = GetPrivateProfileInt(sec_name, "NumCols_50", td->cols_50, ini_file);
	td->rows_50 = GetPrivateProfileInt(sec_name, "NumRows_50", td->rows_50, ini_file);

	/* Window position */
	td->pos_x_50 = GetPrivateProfileInt(sec_name, "PositionX_50", td->pos_x_50, ini_file);
	td->pos_y_50 = GetPrivateProfileInt(sec_name, "PositionY_50", td->pos_y_50, ini_file);
}
...
static void load_prefs(void)
{
	...
	/* Paranoia */
	if (data[0].cols_25 < 80) data[0].cols_25 = 80;
	if (data[0].rows_25 < 25) data[0].rows_25 = 25;
	if (data[0].cols_50 < 80) data[0].cols_50 = 80;
	if (data[0].rows_50 < 24) data[0].rows_50 = 24;
}
...
static void term_window_resize(const term_data *td)
{
	...
	if (screen_rows != 50)
	{
		SetWindowPos(td->w, 0, 0, 0,
						 td->size_wid_25, td->size_hgt_25,
						 SWP_NOMOVE | SWP_NOZORDER);
	}
	else
	{
		SetWindowPos(td->w, 0, 0, 0,
						 td->size_wid_50, td->size_hgt_50,
						 SWP_NOMOVE | SWP_NOZORDER);
	}
...
static errr term_force_font(term_data *td, cptr path)
{
	...
	if (screen_rows != 50)
	{
		if (td->font_id_25) DeleteObject(td->font_id_25);
	}
	else
	{
		if (td->font_id_50) DeleteObject(td->font_id_50);
	}

	/* Forget old font */
	if (((screen_rows != 50) || (!path)) && (td->font_file_25))
	{
		bool used = FALSE;
		...
		for (i = 0; i < MAX_TERM_DATA; i++)
		{
			...
			if ((td != &data[i]) &&
				 (data[i].font_file_25) &&
				 (streq(data[i].font_file_25, td->font_file_25)))
			{
				used = TRUE;
			}
		}
		...
		if (!used) RemoveFontResource(td->font_file_25);

		/* Free the old name */
		string_free(td->font_file_25);

		/* Forget it */
		td->font_file_25 = NULL;
	}

	if (((screen_rows == 50) || (!path)) && (td->font_file_50))
	{
		bool used = FALSE;
		...
		for (i = 0; i < MAX_TERM_DATA; i++)
		{
			...
			if ((td != &data[i]) &&
				 (data[i].font_file_50) &&
				 (streq(data[i].font_file_50, td->font_file_50)))
			{
				used = TRUE;
			}
		}
		...
		/* Remove unused font resources */
		if (!used) RemoveFontResource(td->font_file_50);

		/* Free the old name */
		string_free(td->font_file_50);

		/* Forget it */
		td->font_file_50 = NULL;
	}
	...
	/* Save new font name */
	if (screen_rows != 50) td->font_file_25 = string_make(base);
	else                   td->font_file_50 = string_make(base);
	...
	if (screen_rows != 50)
	{
		td->font_id_25 = CreateFont(hgt, wid, 0, 0, FW_DONTCARE, 0, 0, 0,
	...
	{
		td->font_id_50 = CreateFont(hgt, wid, 0, 0, FW_DONTCARE, 0, 0, 0,
											 ANSI_CHARSET, OUT_DEFAULT_PRECIS,
											 CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
											 FIXED_PITCH | FF_DONTCARE, base);
	}
	...
	if (!wid || !hgt)
	{
		...
		if (screen_rows != 50)
		{
			hfOld = SelectObject(hdcDesktop, td->font_id_25);
		}
		else
		{
			hfOld = SelectObject(hdcDesktop, td->font_id_50);
		}
...
static void term_change_font(term_data *td)
{
	...
	if (screen_rows != 50)
	{
		if (td->font_file_25) strcpy(tmp, td->font_file_25);
	}
	else
	{
		if (td->font_file_50) strcpy(tmp, td->font_file_50);
	}
	...
	ofn.lpstrFilter = "Sangband Font Files (*.fon)\0*.fon\0";
	ofn.nFilterIndex = 1;
	ofn.lpstrFile = tmp;
	ofn.nMaxFile = 128;
	...
	if (GetOpenFileName(&ofn))
	{
		...
		if (term_force_font(td, tmp))
		{
			...
			path_build(tmp, sizeof(tmp), ANGBAND_DIR_XTRA_FONT,
				(screen_rows != 50 ? "9X15B.FON" : "8X8B.FON"));
...
static errr Term_xtra_win_react(void)
{
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		if (screen_rows != 50)
		{
			if ((td->cols_25 != td->t.wid) || (td->rows_25 != td->t.hgt))
			{
				/* Activate */
				Term_activate(&td->t);

				/* Hack -- Resize the term */
				Term_resize(td->cols_25, td->rows_25);
		...
		{
			/* Update resized windows */
			if ((td->cols_50 != td->t.wid) || (td->rows_50 != td->t.hgt))
			{
				/* Activate */
				Term_activate(&td->t);

				/* Hack -- Resize the term */
				Term_resize(td->cols_50, td->rows_50);
...
static errr Term_xtra_win_clear(void)
{
	...
	{
		rc.left = td->size_ow1;
		rc.right = rc.left + td->cols_25 * td->tile_wid_25;
		rc.top = td->size_oh1;
		rc.bottom = rc.top + td->rows_25 * td->tile_hgt_25;
	}
	else
	{
		rc.left = td->size_ow1;
		rc.right = rc.left + td->cols_50 * td->tile_wid_50;
		rc.top = td->size_oh1;
		rc.bottom = rc.top + td->rows_50 * td->tile_hgt_50;
	}

	/* Erase it */
	hdc = GetDC(td->w);
	SetBkColor(hdc, RGB(0, 0, 0));
	if (screen_rows != 50) SelectObject(hdc, td->font_id_25);
	else                   SelectObject(hdc, td->font_id_50);
...
static errr Term_curs_win(int x, int y)
{
	...
	else
	{
		if (screen_rows != 50)
		{
			tile_wid = td->tile_wid_25;
			tile_hgt = td->tile_hgt_25;
		}
		else
		{
			tile_wid = td->tile_wid_50;
			tile_hgt = td->tile_hgt_50;
		}
	}
...
static errr Term_wipe_win(int x, int y, int n)
{
	...
	if (screen_rows != 50)
	{
		rc.left = x * td->tile_wid_25 + td->size_ow1;
		rc.right = rc.left + n * td->tile_wid_25;
		rc.top = y * td->tile_hgt_25 + td->size_oh1;
		rc.bottom = rc.top + td->tile_hgt_25;
	}
	else
	{
		rc.left = x * td->tile_wid_50 + td->size_ow1;
		rc.right = rc.left + n * td->tile_wid_50;
		rc.top = y * td->tile_hgt_50 + td->size_oh1;
	    ...
	    (td->tile_wid_50 != td->font_wid_50))
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (screen_rows != 50)
			{
				...
				rc.right += td->tile_wid_25;
			}
			else
			{
				rc.left += td->tile_wid_50;
				rc.right += td->tile_wid_50;
			}
		}
	}
 ...
 */
static errr Term_rows_win(bool fifty_rows)
{
	term_data *td = &data[0];
...
static void windows_map_aux(void)
{
	...
	if (screen_rows != 50)
	{
		td->map_tile_wid = (td->tile_wid_25 * td->cols_25) / DUNGEON_WID_MAX;
		td->map_tile_hgt = (td->tile_hgt_25 * td->rows_25) / DUNGEON_HGT_MAX;
	}
	else
	{
		td->map_tile_wid = (td->tile_wid_50 * td->cols_50) / DUNGEON_WID_MAX;
		td->map_tile_hgt = (td->tile_hgt_50 * td->rows_50) / DUNGEON_HGT_MAX;
	}

	min_x = 0;
	min_y = 0;
	max_x = DUNGEON_WID_MAX;
	max_y = DUNGEON_HGT_MAX;
...
static void term_data_link(term_data *td)
{
	...
	if (screen_rows != 50)
	{
		term_init(t, td->cols_25, td->rows_25, td->keys);
	}
	else
	{
		term_init(t, td->cols_50, td->rows_50, td->keys);
	}
...
static void init_windows(void)
{
	...
	td->rows_25 = 25;
	td->rows_50 = 50;
	td->cols_25 = td->cols_50 = 80;
	td->visible = TRUE;
	td->size_ow1 = 2;
	td->size_ow2 = 2;
	td->size_oh1 = 2;
	td->size_oh2 = 2;
	td->pos_x_25 = td->pos_x_50 = -24;
	td->pos_y_25 = td->pos_y_50 = 20;
	...
	for (i = 1; i < MAX_TERM_DATA; i++)
	{
		...
		td->size_oh1 = 1;
		td->size_oh2 = 1;
		td->pos_x_25 = td->pos_x_50 = (7 - i) * 30;
		td->pos_y_25 = td->pos_y_50 = (7 - i) * 20;
	}
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		/* Access the standard font file (25 lines) */
		path_build(buf, sizeof(buf), ANGBAND_DIR_XTRA_FONT, td->font_want_25);

		/* Activate the chosen font */
		if (term_force_font(td, buf))
		{
			/* Access the specified font file (25 lines) */
			path_build(buf, sizeof(buf), ANGBAND_DIR_XTRA_FONT, "9X15B.FON");
		...
		/* Access the standard font file (50 lines) */
		path_build(buf, sizeof(buf), ANGBAND_DIR_XTRA_FONT, td->font_want_50);

		/* Activate the chosen font */
		if (term_force_font(td, buf))
		{
			/* Access the specified font file (50 lines) */
			path_build(buf, sizeof(buf), ANGBAND_DIR_XTRA_FONT, "8X8B.FON");
...
static void process_menus(WORD wCmd)
{
	...
	switch (wCmd)
	{
		...
		case IDM_FILE_OPEN:
		{
			...
			else
			{
				...
				if (GetOpenFileName(&ofn))
				{
					...
					/* if (character_icky > 0) screen_load(); */
		...
		case IDM_WINDOW_I_WID_7:
		{
			...
			if (screen_rows != 50) td->tile_wid_25 += 1;
			else                   td->tile_wid_50 += 1;
		...
		case IDM_WINDOW_I_HGT_7:
		{
			...
			if (screen_rows != 50) td->tile_hgt_25 += 1;
			else                   td->tile_hgt_50 += 1;
...
static void handle_wm_paint(HWND hWnd)
{
	...
	else
	{
		...
		if (screen_rows != 50)
		{
			x1 = (ps.rcPaint.left / td->tile_wid_25) - 1;
			x2 = (ps.rcPaint.right / td->tile_wid_25) + 1;
			y1 = (ps.rcPaint.top / td->tile_hgt_25) - 1;
			y2 = (ps.rcPaint.bottom / td->tile_hgt_25) + 1;
		}
		else
		{
			x1 = (ps.rcPaint.left / td->tile_wid_50) - 1;
			x2 = (ps.rcPaint.right / td->tile_wid_50) + 1;
			y1 = (ps.rcPaint.top / td->tile_hgt_50) - 1;
			y2 = (ps.rcPaint.bottom / td->tile_hgt_50) + 1;
		}
                                          ...
                                          WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_GETMINMAXINFO:
		{
			...
			if (screen_rows != 50)
			{
				rc.right = rc.left + 80 * td->tile_wid_25 +
					td->size_ow1 + td->size_ow2;
				rc.bottom = rc.top + 24 * td->tile_hgt_25 +
					td->size_oh1 + td->size_oh2 + 1;
			}
			else
			{
				rc.right = rc.left + 80 * td->tile_wid_50 +
					td->size_ow1 + td->size_ow2;
				rc.bottom = rc.top + 24 * td->tile_hgt_50 +
					td->size_oh1 + td->size_oh2 + 1;
			}
		...
		case WM_SIZE:
		{
			...
			switch (wParam)
			{
				...
				case SIZE_RESTORED:
				{
					if (screen_rows != 50)
					{
						int cols = (LOWORD(lParam) - td->size_ow1) / td->tile_wid_25;
						int rows = (HIWORD(lParam) - td->size_oh1) / td->tile_hgt_25;

						/* New size */
						if ((td->cols_25 != cols) || (td->rows_25 != rows))
						{
							/* Save the new size */
							td->cols_25 = cols;
							td->rows_25 = rows;
					...
					{
						int cols = (LOWORD(lParam) - td->size_ow1) / td->tile_wid_50;
						int rows = (HIWORD(lParam) - td->size_oh1) / td->tile_hgt_50;

						/* New size */
						if ((td->cols_50 != cols) || (td->rows_50 != rows))
						{
							/* Save the new size */
							td->cols_50 = cols;
							td->rows_50 = rows;
                                          ...
                                          WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_GETMINMAXINFO:
		{
			...
			if (screen_rows != 50)
			{
				rc.right = rc.left + 8 * td->tile_wid_25 +
					td->size_ow1 + td->size_ow2;
				rc.bottom = rc.top + 2 * td->tile_hgt_25 +
					td->size_oh1 + td->size_oh2;
			}
			else
			{
				rc.right = rc.left + 8 * td->tile_wid_50 +
					td->size_ow1 + td->size_ow2;
				rc.bottom = rc.top + 2 * td->tile_hgt_50 +
					td->size_oh1 + td->size_oh2;
			}
			...
			/* Maximum window size is 50x80 */
			if (screen_rows != 50)
			{
				rc.left = rc.top = 0;
				rc.right = rc.left + 80 * td->tile_wid_25 +
					td->size_ow1 + td->size_ow2;
				rc.bottom = rc.top + 25 * td->tile_hgt_25 +
					td->size_oh1 + td->size_oh2;

				/* Paranoia */
				rc.right += (td->tile_wid_25 - 1);
				rc.bottom += (td->tile_hgt_25 - 1);
			}
			else
			{
				rc.left = rc.top = 0;
				rc.right = rc.left + 80 * td->tile_wid_50 +
					td->size_ow1 + td->size_ow2;
				rc.bottom = rc.top + 50 * td->tile_hgt_50 +
					td->size_oh1 + td->size_oh2;

				/* Paranoia */
				rc.right += (td->tile_wid_50 - 1);
				rc.bottom += (td->tile_hgt_50 - 1);
			}
		...
		case WM_SIZE:
		{
			...
			if (screen_rows != 50)
			{
				cols = (LOWORD(lParam) - td->size_ow1) / td->tile_wid_25;
				rows = (HIWORD(lParam) - td->size_oh1) / td->tile_hgt_25;
			...
			{
				cols = (LOWORD(lParam) - td->size_ow1) / td->tile_wid_50;
				rows = (HIWORD(lParam) - td->size_oh1) / td->tile_hgt_50;
...
static void hook_quit(cptr str)
{
	...
	for (i = MAX_TERM_DATA - 1; i >= 0; --i)
	{
		...
		if (data[i].font_want_50) string_free(data[i].font_want_50);
...
/* Standard Windows operation -- wait for user to do something */
#if 0
	/* Prompt the user */
	prt("[Choose 'New' or 'Open' from the 'File' menu]", 46, 17);
	Term_fresh();
...
/* Sangband operation -- assume user wants to play a game */
#else
	/* Wait for response */
	prt("[Press any key to continue]",
		screen_rows != 50 ? screen_rows - 2 : screen_rows - 1, 26);
	Term_fresh();
	(void)inkey();
