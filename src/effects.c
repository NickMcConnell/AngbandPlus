...
bool set_effect_lingering_cloud(int f_idx, byte y, byte x,	u16b power, s16b source, u16b flag)
{
	...
	/* Calculate countdown */
	countdown = (effect_power / 50) + f_ptr->x_timeout_set + randint(f_ptr->x_timeout_rand);
...
bool set_effect_trap_smart(int f_idx, byte y, byte x, u16b flags)
{
	...
	trap_power = (f_ptr->x_damage * (MAX(1, danger(p_ptr->depth))) / 100);

	/*
	 * Avoid unfair instadeaths for the player by creating the trap only partially charged.
	 */
	countdown = (f_ptr->x_timeout_set + rand_int(f_ptr->x_timeout_rand)) / 4;
 ...
 */
void effect_copy(effect_type *x_ptr, const effect_type *x2_ptr)
{
	/* Copy the structure */
	COPY(x_ptr, x2_ptr, effect_type);
}
		...
		switch (x_ptr->x_type)
		{
			...
			case EFFECT_TRAP_SMART:
			{
				...
				if (!player_can_fire_bold(y, x))
				{
					...
					else x_ptr->x_countdown += randint(2);
				}
