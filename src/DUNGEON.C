...
static void process_world(void)
{
	...
	for (j = 0, i = 0; i < INVEN_PACK; i++)
	{
		...
		{
			/*error trapping, should not make final code*/
			if (o_ptr->pval - (k_ptr->pval * o_ptr->number) != 0)
			{
				...
				msg_format("Status of %s.", o_name);

				msg_format("pval %d number %d, item pval %d, timeout %d",
				o_ptr->pval,
				o_ptr->number,
				k_ptr->pval,
				o_ptr->timeout);
				...
				if (o_ptr->number > 1)
				{
					msg_format("Your %s pvals are off.", o_name);
				}
				else msg_format("Your %s pval is off.", o_name);
