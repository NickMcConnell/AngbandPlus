...
static char KEY_READABLE[] = "readable";
...
static bool autopick_new_entry(autopick_type *entry, cptr str, bool allow_default)
{
	...
	while (old_ptr != ptr)
	{
		...
		if (MATCH_KEY(KEY_READABLE)) ADD_FLG(FLG_READABLE);
...
static bool object_is_favorite(const object_type *o_ptr)
{
	...
	case CLASS_PRIEST:
	{
		...
		if (!(f3 & TR3_BLESSED) && (o_ptr->tval != TV_HAFTED)) return FALSE;
		break;
	}
...
static bool object_is_rare(const object_type *o_ptr)
{
	...
	case TV_POLEARM:
		if (o_ptr->sval == SV_SCYTHE_OF_SLICING) return TRUE;
		break;
	...
	case TV_SHIELD:
		if (o_ptr->sval == SV_DRAGON_SHIELD ||
		    o_ptr->sval == SV_SHIELD_OF_DEFLECTION) return TRUE;
		break;
	...
	case TV_BOOTS:
		if (o_ptr->sval == SV_PAIR_OF_DRAGON_BOOTS) return TRUE;
		break;
...
cptr autopick_line_from_entry(autopick_type *entry)
{
	...
	*buf = '\0';
	...
	if (IS_FLG(FLG_READABLE)) ADD_KEY(KEY_READABLE);
...
static bool is_autopick_aux(const object_type *o_ptr, autopick_type *entry, cptr o_name)
{
	...
	{
		if (!object_known_p(o_ptr) || !object_is_artifact(o_ptr))
			return FALSE;
	}
	...
	if ((IS_FLG(FLG_REALM1) || IS_FLG(FLG_READABLE)) &&
...
static void describe_autopick(char *buff, autopick_type *entry)
{
	...
	/*** Readable spellbooks ***/
	if (IS_FLG(FLG_READABLE))
	{
		before_str[before_n++] = "あなたが読める領域の";
		body_str = "魔法書";
	}
	...
	if (IS_FLG(FLG_FIRST))
	{
		before_str[before_n++] = "全3冊の内の1冊目の";
		body_str = "魔法書";
	}
	...
	if (IS_FLG(FLG_SECOND))
	{
		before_str[before_n++] = "全3冊の内の2冊目の";
		body_str = "魔法書";
	}
	...
	if (IS_FLG(FLG_THIRD))
	{
		before_str[before_n++] = "全3冊の内の3冊目の";
		body_str = "魔法書";
	}
	...
	*buff = '\0';
	...
	{
		body_str = "spellbooks";
		after_str[after_n++] = "of your realms";
	}
	...
	if (IS_FLG(FLG_FIRST))
	{
		before_str[before_n++] = "first one of three";
		body_str = "spellbooks";
	}
	...
	if (IS_FLG(FLG_SECOND))
	{
		before_str[before_n++] = "second one of three";
		body_str = "spellbooks";
	}
	...
	if (IS_FLG(FLG_THIRD))
	{
		before_str[before_n++] = "third one of three";
		body_str = "spellbooks";
	}
...
static char MN_UNIQUE[] = "ユニーク・モンスターの (像)";
/* static char MN_HUMAN[] = "人間の (死体)"; */
static char MN_READABLE[] = "読める (魔法書)" ;
...
static char MN_UNIQUE[] = "unique (statues)";
/* static char MN_HUMAN[] = "human (corpse)"; */
static char MN_READABLE[] = "readable (spellbooks)";
...
command_menu_type menu_data[] =
{
	...
	{MN_READABLE, 1, -1, EC_OK_READABLE},
...
static bool do_editor_command(text_body_type *tb, int com_id)
{
...
#ifdef JP
		(void)show_file("jeditor.txt", NULL, 0, 0);
#else
		(void)show_file("editor.txt", NULL, 0, 0);
#endif
		/* Redraw all */
		tb->dirty_flags |= DIRTY_SCREEN;
	...
	case EC_OK_READABLE:
		toggle_keyword(tb, FLG_READABLE);
