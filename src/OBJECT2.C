...
s16b get_obj_num(int level)
{
	...
	for (i = 0; i < alloc_kind_size; i++)
	{
		...
		/* Hack -- prevent embedded chests and chests generated in town */
...
bool object_similar(const object_type *o_ptr, const object_type *j_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_WAND:
		{
			...
			/* Wand charges combine in NPPangband.  */
...
static bool make_artifact_special(object_type *o_ptr)
{
	...
	/* No artifacts in the town, unless opening a chest */
	if ((!p_ptr->depth) && (!opening_chest)) return (FALSE);
...
static void a_m_aux_4(object_type *o_ptr, int level, int power, bool good, bool great)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_CHEST:
		{
			...
			o_ptr->xtra1 = choose_chest_contents (getchesttheme);
...
void apply_magic(object_type *o_ptr, int lev, bool okay, bool good, bool great)
{
	...
	if (o_ptr->name1)
	{
		 ...
		 * this would be the cause of some major bugs. JG
		 */
...
static bool kind_is_great(int k_idx)
{
	...
	switch (k_ptr->tval)
	{
		...
		/* Chests -- Chests are great. NPP Angband edit*/
		case TV_CHEST:
		{
				return (TRUE);
		}
...
static bool kind_is_scroll(int k_idx)
{
	...
	switch (k_ptr->tval)
	{
		...
		/* Books -- HACK - High level books are good only if out of depth */
		case TV_MAGIC_BOOK:
		case TV_PRAYER_BOOK:
		{
			if (k_ptr->level <= object_level) return (FALSE);
...
static bool kind_is_potion(int k_idx)
{
	...
	switch (k_ptr->tval)
	{
...

		{
			...
			if (k_ptr->sval == SV_POTION_INC_STR) return (TRUE);
			if (k_ptr->sval == SV_POTION_INC_INT) return (TRUE);
			if (k_ptr->sval == SV_POTION_INC_WIS) return (TRUE);
			if (k_ptr->sval == SV_POTION_INC_DEX) return (TRUE);
			if (k_ptr->sval == SV_POTION_INC_CON) return (TRUE);
			if (k_ptr->sval == SV_POTION_INC_CHR) return (TRUE);
 ...
 */
bool make_object(object_type *j_ptr, bool good, bool great, int chesttype)
{
	int prob, base;
	...
	if ((rand_int(prob) != 0) || !make_artifact_special(j_ptr))
	{
		...
		/*
		 * First check if it is a chest is being opened or generated, and
		 * only include objects from a pre-set theme.  The type of theme
		 * is determined during creation".  But, it can be
		 * called from anywhere.  Note that 1 serves adual prupose.  In chest_death,
		 * 1 means a  treasure chest, and make-object is not called.  1 here means .
		 * create a chest.
		 * First check to skip all these checks when unnecessary. JG
		 */


		 if ((good) || (great) || (chesttype >=2))
			{
					/*note - theme 1 is gold, sent to the make_gold function*/
					if (chesttype == DROP_TYPE_POTION)					get_obj_num_hook = kind_is_potion;
					else if (chesttype == DROP_TYPE_ROD_WAND_STAFF) 	get_obj_num_hook = kind_is_rod_wand_staff;
					else if (chesttype == DROP_TYPE_SCROLL) 			get_obj_num_hook = kind_is_scroll;
					else if (chesttype == DROP_TYPE_SHIELD) 			get_obj_num_hook = kind_is_shield;
					else if (chesttype == DROP_TYPE_WEAPON) 			get_obj_num_hook = kind_is_weapon;
					else if (chesttype == DROP_TYPE_ARMOR) 				get_obj_num_hook = kind_is_armor;
					else if (chesttype == DROP_TYPE_BOOTS) 				get_obj_num_hook = kind_is_boots;
					else if (chesttype == DROP_TYPE_BOW) 				get_obj_num_hook = kind_is_bow;
					else if (chesttype == DROP_TYPE_CLOAK)				get_obj_num_hook = kind_is_cloak;
					else if (chesttype == DROP_TYPE_GLOVES)				get_obj_num_hook = kind_is_gloves;
					else if (chesttype == DROP_TYPE_HAFTED)				get_obj_num_hook = kind_is_hafted;
					else if (chesttype == DROP_TYPE_HEADGEAR)			get_obj_num_hook = kind_is_headgear;
					else if (chesttype == DROP_TYPE_JEWELRY)			get_obj_num_hook = kind_is_jewelry;
					else if (chesttype == DROP_TYPE_DRAGON_ARMOR)		get_obj_num_hook = kind_is_dragarmor;
					else if (chesttype == DROP_TYPE_CHEST)				get_obj_num_hook = kind_is_chest;
				/*
				 *	If it isn't a chest, check good and great flags.
				 *  They each now have their own templates.  JG
				 *
				 */
			...
			if ((good) || (great) || (chesttype >=1)) get_obj_num_prep();

			/* Pick a random object */
			k_idx = get_obj_num(base);

			/* Clear acceptable objects template */
			if ((good) || (great) || (chesttype >=1))
			{
				/* Clear restriction */
...
bool make_gold(object_type *j_ptr)
{
	...
	if (opening_chest) j_ptr->pval += ((randint(4) + randint(4) + object_level / 4 ) * 50);
...
void place_object(int y, int x, bool good, bool great, int droptype)
{
	...
	if (make_object(i_ptr, good, great, droptype))
	{
		/* Give it to the floor */
...
s16b inven_carry(object_type *o_ptr)
{
	...
	if (i < INVEN_PACK)
	{
		...
		for (j = 0; j < INVEN_PACK; j++)
		{
			...
			/* Staffs sort by decreasing charges */
 ...
 *
 */
int choose_chest_contents (int chest_theme)
{
	 ...
	 * JG
	 */
	else if (chestlevel <=20) chest_theme = (randint (3)) + 1;

	/*
	 * The next nine themes are armor/weapons, all with equal chances
	 *
	 ...
	 * JG
	 */
	else if (chestlevel <=60) chest_theme = (randint (9)) + 4;
