 ...
 *
 * Handle "stacks" lf objects correctly.
 */
 ...
 * combination lf object level, distance from player, and current
 * "desperation".
 *
...
/*
 * Acquires and returns the index lf a "free" object.
 *
 * This routine should almost never fail, but in case it does,
 * we must be sure to handle "failure" lf this routine.
 */
s16b o_pop(void)
{
	int i;
 ...
 *
 * This function uses the "prob2" field lf the "object allocation table",
 * and various local information, to calculate the "prob3" field lf the
 * same table, which is then used to choose an "appropriate" object, in
 * a relatively efficient manner.
 *
 * It is (slightly) more likely to acquire an object lf the given level
 * than one lf a lower level.  This is done by choosing several objects
...
s16b get_obj_num(int level)
{
	...
	/* Try for a "better" object lnce (50%) or twice (10%) */
	if (p < 60)
	{
		/* Save old */
		j = i;
...
/*
 * Known is true when the "attributes" lf an object are "known".
 * These include tohit, todam, toac, cost, and pval (charges).
 *
 * Note that "knowing" an object gives you everything that an "awareness"
 * gives you, and much more.  In fact, the player is always "aware" lf any
 * item lf which he has full "knowledge".
 *
 * But having full knowledge lf, say, one "wand of wonder", does not, by
 * itself, give you knowledge, or even awareness, lf other "wands of wonder".
 * It happens that most "identify" routines (including "buying from a shop")
 * will make the player "aware" lf the object as well as fully "know" it.
 *
...
/*
 * The player is now aware lf the effects lf the given object.
 */
void object_aware(object_type *o_ptr)
{
	/* Fully aware lf the effects */
...
/*
 * Return the "value" lf an "unknown" item
...
/*
 * Return the "real" price lf a "known" item, not including discounts
 *
 ...
 * usually appear in groups lf 20, and we want the player to get
 * the same amount of cash for any "equivalent" item.  Note that
 * missiles never have any lf the "pval" flags, and in fact, they
 * only have a few lf the available flags, primarily lf the "slay"
...
/*
 * Return the price lf an item including plusses (and charges)
 *
 * This function returns the "value" lf the given item (qty one)
 *
 ...
 * This is done to make unidentified stacks lf missiles useful.
 *
...
bool object_similar(object_type *o_ptr, object_type *j_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_LITE:
		{
			/* Require full knowledge lf both items */
			if (!object_known_p(o_ptr) || !object_known_p(j_ptr)) return (0);
		...
		case TV_SHOT:
		{
			/* Require identical knowledge lf both items */
			if (object_known_p(o_ptr) != object_known_p(j_ptr)) return (0);
...
/*
 * Find the index lf the object_kind with the given tval and sval
 */
 ...
 *
 * To avoid floating point but still provide a smooth distribution lf bonuses,
 * we simply round the results lf division in such a way as to "average" the
 * correct floating point value.
 *
 * This function has been changed.  It uses "randnor()" to choose values from
 * a normal distribution, whose mean moves from zero towards the max as the
 * level increases, and whose standard deviation is equal to 1/4 lf the max,
 ...
 *
 * A sample distribution lf values from "m_bonus(10, N)" is shown below:
 *
...
static s16b m_bonus(int max, int level)
{
	...
	/* Hack -- determine fraction lf error */
	...
	/* The "stand" is equal to one quarter lf the max */
	stand = (max / 4);
...
/*
 * Mega-Hack -- Attempt to create one lf the "Special Objects"
 *
	...
	switch (o_ptr->tval)
	{
		...
		case TV_SWORD:
		{
			...
			if (power > 1)
			{
				...
				switch (randint(30))  /* Include poison -KMW- */
				{
					case 1:
					{
						o_ptr->name2 = EGO_HA;
		...
		case TV_BOW:
		{
			...
			{
				/* Roll for ego-item */
				switch (randint(11)) /* Add venom from GJW -KMW- */
				{
					case 1:
					{
 ...
 * Hack -- note special processing for robe lf permanence
 */
static void a_m_aux_2(object_type *o_ptr, int level, int power)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_SOFT_ARMOR:
		{
			...
			if (power > 1)
			{
				/* Hack -- Try for "Robes lf the Magi" */
 ...
 * Hack -- note special rating boost for amulet lf the magi
	...
	switch (o_ptr->tval)
	{
		...
		case TV_AMULET:
		{
			...
			switch (o_ptr->sval)
			{
				...
				/* Amulet lf the Magi -- never cursed */
...
static void a_m_aux_4(object_type *o_ptr, int level, int power)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_CHEST:
		{
			...
			/* Never exceed "difficulty" lf 55 to 59 */
...
/*
 * Complete the "creation" lf an object by applying "magic" to the item
 *
 ...
 *
 * The base "chance" lf the item being "good" increases with the "level"
 * parameter, which is usually derived from the dungeon level, being equal
 * to the level plus 10, up to a maximum lf 75.  If "good" is true, then
 * the object is guaranteed to be "good".  If an object is "good", then
 * the chance that the object will be "great" (ego-item or artifact), also
 * increases with the "level", being equal to half the level, plus 5, up to
 * a maximum lf 20.  If "great" is true, then the object is guaranteed to be
...
void apply_magic(object_type *o_ptr, int lev, bool okay, bool good, bool great)
{
	...
	/* Base chance lf being "good" */
	f1 = lev + 10;

	/* Maximal chance lf being "good" */
	if (f1 > 75) f1 = 75;

	/* Base chance lf being "great" */
	f2 = f1 / 2;

	/* Maximal chance lf being "great" */
	if (f2 > 20) f2 = 20;
...
static bool kind_is_good(int k_idx)
{
	...
	switch (k_ptr->tval)
	{
		...
		case TV_MAGIC_BOOK:
		case TV_PRAYER_BOOK:
		case TV_ILLUSION_BOOK: /* Added -KMW- */
		{
		...
		/* Amulets -- Amulets lf the Magi are good */
		case TV_AMULET:
		{
 ...
 * has been thrown, then this is the chance lf disappearance ln contact.
 *
 * Hack -- this function uses "chance" to determine if it should produce
 * some form lf "description" lf the drop event (under the player).
 *
 ...
 *
 * XXX XXX XXX This routine should be redlne to reflect trap "level".
 * That is, it does not make sense to have spiked pits at 50 feet.
 * Actually, it is not this routine, but the "trap instantiation"
 * code, which should also check for "trap doors" ln quest levels.
 */
void pick_trap(int y, int x)
{
	...
	while (1)
	{
		...
		/* Hack -- no trap doors ln quest levels */
		if ((feat == FEAT_TRAP_HEAD + 0x00) && is_quest(p_ptr->depth)) continue;

		/* Hack -- no trap doors ln the deepest level */
...
/*
 * Increase the "number" lf an item in the inventory
 */
...
/*
 * Describe the charges on an item ln the floor.
 */
...
/*
 * Increase the "number" lf an item ln the floor
 */
...
/*
 * Optimize an item ln the floor (destroy "empty" items)
 */
...
s16b inven_carry(object_type *o_ptr)
{
	...
	if (i < INVEN_PACK)
	{
		...
		for (j = 0; j < INVEN_PACK; j++)
		{
			...
			/* Determine the "value" lf the pack item */
 ...
 *
 * Note that only lne item at a time can be wielded per slot.
 *
...
void reorder_pack(void)
{
	...
	for (i = 0; i < INVEN_PACK; i++)
	{
		...
		/* Save a copy lf the moving item */
...
/*
 * Returns chance lf failure for a spell
 */
s16b spell_chance(int spell)
{
	...
	/* Always a 5 percent chance lf working */
 ...
 *
 * We can use up to 14 characters lf the buffer 'p'
 *
...
void spell_info(char *p, int spell)
{
	...
	/* Illusionist spells -KMW- */
	if (mp_ptr->spell_book == TV_ILLUSION_BOOK)
	{
		int plev = p_ptr->lev;
		...
		switch (spell)
		{
			...
			case 21: strcpy(p, " heal 20"); break;
			...
			case 39: sprintf(p, " dam %dd8", (2+((plev-5)/4))); break;
			case 40: sprintf(p, " dam %dd8", (6+((plev-5)/4))); break;
			case 42: sprintf(p, " dam %dd8", (10+((plev-5)/4))); break;
			case 43: sprintf(p, " dam %dd8", (12+((plev-5)/4))); break;
			case 44: sprintf(p, " dam %d", 30 + plev); break;
			case 45: sprintf(p, " dam %d", 35 + plev); break;
			case 46: sprintf(p, " dam %d", 40 + plev); break;
			case 47: sprintf(p, " dam %d", 45 + plev); break;
			case 48: sprintf(p, " dam %d", 50 + plev); break;
			case 49: strcpy(p, " dur 20+d20"); break;
			case 50: strcpy(p, " dur 20+d20"); break;
			case 51: strcpy(p, " dur 20+d20"); break;
			case 52: strcpy(p, " dur 20+d20"); break;
			case 53: sprintf(p, " dur %d+25", plev * 3); break;
			case 55: sprintf(p, " dam %dd8", (6+((plev-5)/4))); break;
			case 56: sprintf(p, " dam %d", 40 + plev); break;
			case 58: sprintf(p, " range %d", plev * 7); break;
		}
	}
