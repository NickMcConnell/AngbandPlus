...
bool quest_poison_gen_hook(int q_idx)
{
	...
	/* Pick a monster, using the level calculation */
        ...
        for (y = cy - 25; y <= cy + 25; y++)
        {
                ...
                if (magik(80) && ((cave[y][x].feat == FEAT_DEEP_WATER) || (cave[y][x].feat == FEAT_SHAL_WATER))) cave[y][x].feat = FEAT_TAINTED_WATER;
...
bool quest_poison_finish_hook(int q_idx)
{
        ...
        c_put_str(TERM_YELLOW, "The beautifull Mallorns are safe. Take this as a proof of our gratitude.", 9, 0);
