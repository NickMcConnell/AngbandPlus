...
void leave_floor(void)
{
	...
	if (change_floor_mode & CFM_SAVE_FLOORS)
	{
		...
		if (change_floor_mode & CFM_DOWN) move_num = upward_dun ? -1 : 1;
		else if (change_floor_mode & CFM_UP) move_num = upward_dun ? 1 : -1;
		...
		if ((move_num > 0) && !dun_level) move_num = d_info[c_ptr->special].mindepth;
		else if (dun_level + move_num < d_info[dungeon_type].mindepth) move_num = -dun_level;
...
void stair_creation(void)
{
	...
	/* Forbid forward staircases on quest level */
	if (quest_number(dun_level) || (dun_level >= maxdepth))
	{
		if (d_info[dungeon_type].flags1 & DF1_UPWARD) up = FALSE;
		else down = FALSE;
	}

	/* No effect out of standard dungeon floor */
	if (!dun_level || (!up && !down) || (dungeon_type == DUNGEON_HEAVEN) ||
	    (p_ptr->inside_quest && quest_is_fixed(p_ptr->inside_quest)) ||
	    p_ptr->inside_arena || astral_mode)
	{
		/* arena or quest */
#ifdef JP
		msg_print("効果がありません！");
