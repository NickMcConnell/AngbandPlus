...
bool borg_dump_character(cptr str)
{
	...
	/* Send Yes to replace existing file */
	/* this is ignored when there isn't an existing file */
	borg_keypress('y');
...
/*
 * Look at the monster health bar
 */
void borg_update_health_bar(void)
{
	byte t_a;
	...
	/* Info (monster health) */
	if (0 == borg_what_text(COL_INFO + 1, ROW_INFO, -10, &t_a, buf))
	{
		/* Count asterisks */
		for (i = 0; i < 10; i++)
		{
			/* Check for one more unit of health */
			if (buf[i] == '*')
			{
				/* Count */
				amt++;
	...
	if (amt)
	{
		...
		/* Look at dungeon */
		m_idx = borg_cave_m_idx[xb_ptr->gy][xb_ptr->gx];
		...
		/* Compute hit points */
		kill->curhp = (kill->power * amt + 9) / 10;

		/* Note */
		borg_note(format("# Estimating '%s' HP at %d/%d", r_ptr->name + r_name, kill->curhp, kill->power));
	}
}
...
void borg_update_frame(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		{
			/* Note "Nnn" vs "NNN" */
			borg_base_fix_stat[i] = (0 != islower(buf[2]));
		}

        /* Check ':'/'!' */
        if (0 == borg_what_text(COL_STAT+3, ROW_STAT+i, -1, &t_a, buf))
        {
            borg_stat_maxed[i] = (buf[0] == '!');
        }
				...
				if (info & (CAVE_WALL))
				{
					...
					if (!(info & (CAVE_VIEW)))
					{
						...
						else if (info & (CAVE_GLOW))
						{
							...
							if ((!(borg_cave_info[yy][xx] & (CAVE_WALL)) &&
							      (borg_cave_info[yy][xx] & (CAVE_GLOW))) ||
							    (!(borg_cave_info[y][xx] & (CAVE_WALL)) &&
							      (borg_cave_info[y][xx] & (CAVE_GLOW))) ||
							    (!(borg_cave_info[yy][x] & (CAVE_WALL)) &&
