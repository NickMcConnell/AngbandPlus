...
void self_knowledge(void)
{
	...
	if (p_ptr->blind)
	{
		info[i++] = format("You cannot see (%d).", p_ptr->blind);
	}
	if (p_ptr->confused)
	{
		info[i++] = format("You are confused (%d).", p_ptr->confused);
	}
	if (p_ptr->afraid)
	{
		info[i++] = format("You are terrified (%d).", p_ptr->afraid);
	}
	if (p_ptr->cut)
	{
		info[i++] = format("You are bleeding (%d).", p_ptr->cut);
	}
	if (p_ptr->stun)
	{
		info[i++] = format("You are stunned (%d).", p_ptr->stun);
	}
	if (p_ptr->poisoned)
	{
		info[i++] = format("You are poisoned (%d).", p_ptr->poisoned);
	}
	if (p_ptr->image)
	{
		info[i++] = format("You are hallucinating (%d).", p_ptr->image);
	}
	...
	if (p_ptr->blessed)
	{
		info[i++] = format("You feel righteous (%d).", p_ptr->blessed);
	}
	if (p_ptr->hero)
	{
		info[i++] = format("You feel heroic (%d).", p_ptr->hero);
	}
	if (p_ptr->shero)
	{
		info[i++] = format("You are in a battle rage (%d).", p_ptr->shero);
	}
	if (p_ptr->protevil)
	{
		info[i++] = format("You are protected from evil (%d).", p_ptr->protevil);
	}
	if (p_ptr->shield)
	{
		info[i++] = format("You are protected by a mystic shield (%d).", p_ptr->shield);
	}
	if (p_ptr->invuln)
	{
		info[i++] = format("You are temporarily invulnerable (%d).", p_ptr->invuln);
	}
	if (p_ptr->biofeedback)
	{
		info[i++] = format("You have biofeedback control (%d).", p_ptr->biofeedback);
	}	
	if (p_ptr->adrenaline)
	{
		info[i++] = format("You control your adrenaline (%d).", p_ptr->adrenaline);
	}
	if (p_ptr->inertial_barrier)
	{
		info[i++] = format("You have set up an inertial barrier (%d).", p_ptr->inertial_barrier);
	}
	if (p_ptr->prob_travel)
	{
		info[i++] = format("You can travel through the astral plane (%d).", p_ptr->prob_travel);
	}
	if (p_ptr->precognition)
	{
		info[i++] = format("You can see the future (%d).", p_ptr->precognition);
	}
	if (p_ptr->mental_barrier)
	{
		info[i++] = format("There is a barrier around your mind (%d).", p_ptr->mental_barrier);
	}
	if (p_ptr->ts_anchor)
	{
		info[i++] = format("You are anchored in time and space (%d).", p_ptr->ts_anchor);
	}
	if (p_ptr->intensify)
	{
		info[i++] = format("Your psionics is intensified (%d).", p_ptr->intensify);
	}
	if (p_ptr->confusing)
	{
	...
	{
		info[i++] = format("You will be recalled in %d turns.", p_ptr->word_recall);
	}
	if (p_ptr->see_infra)
	{
	...
	{
		info[i++] = format("You resist acid exceptionally well (%d).", p_ptr->oppose_acid);
	}
	...
	{
		info[i++] = format("You are resistant to acid (%d).", p_ptr->oppose_acid);
	}
	...
	{
		info[i++] = format("You resist lightning exceptionally well (%d).", p_ptr->oppose_elec);
	}
	...
	{
		info[i++] = format("You are resistant to lightning (%d)", p_ptr->oppose_elec);
	}
	...
	{
		info[i++] = format("You resist fire exceptionally well (%d).", p_ptr->oppose_fire);
	}
	...
	{
		info[i++] = format("You are resistant to fire (%d).", p_ptr->oppose_fire);
	}
	...
	{
		info[i++] = format("You resist cold exceptionally well (%d).", p_ptr->oppose_cold);
	}
	...
	{
		info[i++] = format("You are resistant to cold (%d).", p_ptr->oppose_cold);
	}

	if ((p_ptr->resist_pois) && (p_ptr->oppose_pois))
	{
		info[i++] = format("You resist poison exceptionally well (%d).", p_ptr->oppose_pois);
	}
	...
	{
		info[i++] = format("You are resistant to poison (%d).", p_ptr->oppose_pois);
	}
	...
	if (p_ptr->resist_psi)
	{
		info[i++] = "You are resistant to psionic attacks.";
	}
...
/*
 * Forget info about one object only
 */
bool lose_some_info(void)
{
	int i, j;
	object_type *o_ptr;
	...
	/* Mega-Hack -- Forget the map sometimes */
...
bool dimension_door(int lev)
{
	...
	(distance(j,i,p_ptr->py,p_ptr->px) > lev + 2) ||
	(!rand_int(lev * lev / 2)))
	{
		msg_print("You fail to exit the astral plane correctly!");
		p_ptr->energy -= inverse_amplify(100);
		teleport_player(amplify_dam(10));
	}
	else teleport_player_to(j,i);
	return (TRUE);
}

bool telekinesis(int lev)
{
	int sx,sy;
	s16b o_idx;
	...
	/* Hack -- Distance -- Reward level, penalize weight */
	tdis = (adj_str_blow[2*lev] + 20) * mul / div;
	...
	/* Calculate the path */
	path_n = project_path(path_g, tdis, sy, sx, ty, tx, 0);
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			int chance2 = chance - distance(sy, sx, y, x);
