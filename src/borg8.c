...
void borg_think(void)
{
	    ...
	    (streq(buf, "Item Description")))
	{
		...
		if (borg_do_browse_more != borg_shops[borg_base_shop].more)
		{
			...
			/* Save the new one */
			borg_do_browse_more = (byte)borg_shops[borg_base_shop].more;
		}
		...
		for (i = 0; i < 12; i++)
		{
			    ...
			    (buf[0] == I2A(n)) && (buf[1] == ')') && (buf[2] == ' '))
			{
				...
				if (0 != borg_what_text(3, i + 6, -80, &t_a, desc))
				{
					desc[0] = '\0';
				}
	...
	if (borg_prompt &&
	    (0 == borg_what_text(0, 0, 16, &t_a, buf)) &&
	    (prefix(buf, "Map sector ")))
	{
		/* Hack -- get the panel info */
		b_ptr->wy = D2I(buf[12]) * PANEL_HGT;
		b_ptr->wx = D2I(buf[14]) * PANEL_WID;
	...
	if (borg_do_frame)
	{
		...
		{
		case (CLASS_WARRIOR):
			if (b_ptr->lev > 19) auto_scum = TRUE;
			break;
		case (CLASS_MAGE):
			if (b_ptr->lev > 29) auto_scum = TRUE;
			break;
		case (CLASS_PRIEST):
		case (CLASS_ROGUE):
		case (CLASS_RANGER):
		case (CLASS_PALADIN):
			if (b_ptr->lev > 24) auto_scum = TRUE;
			break;		
		}
	}

	/*** Analyze the Monster Health Bar ***/

	/* Analyze health bar */
	if (borg_do_health_bar)
	{
		/* Only once */
		borg_do_health_bar = FALSE;

		/* Analyze the health bar */
		borg_update_health_bar();
	}
...
static cptr suffix_spell[] =
{
	...
	" summon similar monsters!",					/* RF6_S_KIN */
	" summon Greater Demons!",						/* RF6_S_HI_DEMON */
...
static bool borg_parse_aux_monster_1(cptr msg, int len)
{
	...
	/* Invisible monsters */
	/* Exclude the "Oops! It feels deathly cold!" message from cursed items */
	if (prefix(msg, "It ") && !suffix(msg,"deathly cold!"))
	{
		middle1 = "It";
	}
...
static bool borg_parse_aux_player(cptr msg, int len)
{
	...
	if ((suffix(msg,"deathly cold!")) ||
		(suffix(msg,"appears to be cursed.")))
	{
		/* Handle cursed item */
		borg_note("# cursed item worn");
		borg_cursed = TRUE; 
		return (TRUE);
	}
	...
	if (prefix(msg, "Welcome to level "))
	{
		...
		/* If our level is high enough we might as well  */ 
		/* speed things up by turning on the autoscummer */
		switch(b_ptr->pclass)
		{
		case (CLASS_WARRIOR):
			if (b_ptr->lev > 19) auto_scum = TRUE;
	...
	/* Handle objects underneath us */
	if (streq(msg, "You feel something roll beneath your feet.") ||
	    prefix(msg, "You have no room for "))
	{
	...
	/* Handle broken runes */
	if (streq(msg, "The rune of protection is broken!"))
	{
		/* If rune under us, break it */
		if (borg_cave_feat[b_ptr->py][b_ptr->px] == FEAT_GLYPH)
		{
			/* Replace with normal floor */
			borg_cave_feat[b_ptr->py][b_ptr->px] = FEAT_FLOOR;
		}
	}
    ...
    if (prefix(msg, "Your memories fade"))
    {
		...
		borg_amnesia_inven = TRUE;
		borg_amnesia_equip = TRUE;

		borg_note("# Hit by amnesia");
        ...
        for (i = INVEN_WIELD; i <= INVEN_FEET; i++)
        {
			...
			/* skip more easy know itens */
			if ((item->tval == TV_AMULET) &&
				((item->sval == SV_AMULET_TELEPORT) ||
				 (item->sval == SV_AMULET_SLOW_DIGEST) ||
				 (item->sval == SV_AMULET_ADORNMENT) ||
				 (item->sval == SV_AMULET_RESIST_ACID))) continue;

			/* skip even more easy know itens */
			if ((item->tval == TV_RING) &&
				((item->sval == SV_RING_AGGRAVATION) ||
				 (item->sval == SV_RING_TELEPORTATION) ||
				 ((item->sval >= SV_RING_SLOW_DIGESTION) &&
				 (item->sval <= SV_RING_SUSTAIN_CHR)) ||
				 ((item->sval >= SV_RING_RESIST_POIS) &&
				 (item->sval <= SV_RING_SEE_INVIS)))) continue;
...
static bool borg_parse_aux_atmosphere(cptr msg, int len)
{
	...
	/* Handle unexpected failure messages. */
	if ((prefix(msg, "You may not ") && suffix(msg, " that spell")) ||
