...
/*			case 'w': mask |= (CAVE_WALL); break;*/
	...
	/* Command: debug -- prepared above */
	else if (cmd == '<')
	{
		msg_format("Borg is %sprepared for level %d.",
					  borg_prepared(b_ptr->depth - 1) ? "" : "not ", b_ptr->depth - 1);
	}

	/* Command: debug -- prepared here */
	else if (cmd == '.')
	{
		msg_format("Borg is %sprepared for level %d.",
					  borg_prepared(b_ptr->depth) ? "" : "not ", b_ptr->depth);
	}

	/* Command: debug -- prepared below */
	else if (cmd == '>')
	{
		msg_format("Borg is %sprepared for level %d.",
					  borg_prepared(b_ptr->depth + 1) ? "" : "not ", b_ptr->depth + 1);
	...
	/* Command: debug -- prepared somewhere */
	else if (cmd == ',')
	{
		/* Ask for level */
		if (p_ptr->command_arg <= 0)
		{
			...
			/* Prompt */
			sprintf(ppp, "Borg command: Prepared at: ");

			/* Default */
			sprintf(tmp_val, "%d", b_ptr->depth);
		...
		msg_format("Borg is %sprepared for level %d.",
					  borg_prepared(p_ptr->command_arg) ? "" : "not ", p_ptr->command_arg);
	}
