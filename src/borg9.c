...
static char borg_inkey_hack(int flush_first)
{
	...
	if (borg_active == 0)
	{
		...
		/* stop playing with yourself! DvE */
		// keep_playing = FALSE;
		/* Done */
		return (0);
	}
...
static void borg_prepare(void)
{
	...
	/* We prefer to ignore discounts DvE */
	...
	/* Do not verify special commands */
	verify_special = FALSE;

	/* auto scum will be off untill we are strong (high level) enough */
	auto_scum = FALSE;
	...
	borg_do_spell = TRUE;
	borg_do_spell_aux = 0;
	borg_amnesia_inven = FALSE;
	borg_amnesia_equip = FALSE;
...
static void borg_init_1(void)
{
	...
	C_MAKE(borg_glow_g, BORG_TEMP_MAX, u16b);

	borg_glow_y = ((byte*)(borg_glow_g)) + 0;
	borg_glow_x = ((byte*)(borg_glow_g)) + BORG_TEMP_MAX;
	...
	borg_happy_depth = b_ptr->max_depth;
}
...
void borg_write_map(bool ask)
{
	...
	/* Only for high-level borgs */
	if (b_ptr->lev < 20) return;
	...
	/* Terminate */
	buf[i] = '\0';
	sprintf(buf, "%smap.txt", buf);

    /* XXX XXX XXX Get the name and open the map file */
	if (ask && get_string("Borg map File: ", buf, 70))
	{
		/* Open a new file */
	...
	else if (!ask) borg_map_file = my_fopen(buf, "w");

	/* Hack -- grab permissions */
	safe_setuid_grab();

	fprintf(borg_map_file,"Borg map level: %d\n\n",b_ptr->depth);
	...
	for (i = 0; i < DUNGEON_HGT; i++)
	{
		for (j = 0; j < DUNGEON_WID; j++)
		{
			...
			if ((i == b_ptr->py) && (j == b_ptr->px)) ch = '@';
			if (borg_cave_o_idx[i][j])
			{
				auto_take *take = &borg_takes[borg_cave_o_idx[i][j]];
				object_kind *k_ptr = &k_info[take->k_idx];
				ch = k_ptr->d_char;
			}
			if (borg_cave_m_idx[i][j])
			{
				auto_kill *kill = &borg_kills[borg_cave_m_idx[i][j]];
	...
	if (borg_weapon_swap == 0)
	{
		fprintf(borg_map_file,"Swap Weapon: None\n");
	}
	else
	{
		item = &borg_items[borg_weapon_swap];
		fprintf(borg_map_file,"Swap Weapon:  %s, value= %d\n", item->desc, borg_weapon_swap_value);
	}
	if (borg_armour_swap == 0)
	{
		fprintf(borg_map_file,"Swap Armour: None\n");
	}
	else
	{
		item = &borg_items[borg_armour_swap];
		fprintf(borg_map_file,"Swap Armour:  %s, value= %d\n", item->desc, borg_armour_swap_value);
	}
...
void borg_status(void)
{
	...
	for (j = 0; j < 8; j++)
	{
		...
		if (op_ptr->window_flag[j] & (PW_BORG_2))
		{
			...
			/* Basic four */
			if (b_ptr->immune_acid) attr = TERM_WHITE;
			else if (b_ptr->oppose_acid) attr = TERM_GREEN;
			else if (b_ptr->resist_acid) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 1, -1, attr, "Acid");

			if (b_ptr->immune_elec) attr = TERM_WHITE;
			else if (b_ptr->oppose_elec) attr = TERM_GREEN;
			else if (b_ptr->resist_elec) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 2, -1, attr, "Elec");

			if (b_ptr->immune_fire) attr = TERM_WHITE;
			else if (b_ptr->oppose_fire) attr = TERM_GREEN;
			else if (b_ptr->resist_fire) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 3, -1, attr, "Fire");

			if (b_ptr->immune_cold) attr = TERM_WHITE;
			else if (b_ptr->oppose_cold) attr = TERM_GREEN;
			else if (b_ptr->resist_cold) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 4, -1, attr, "Cold");

			/* High resists */
			if (b_ptr->resist_pois) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 5, -1, attr, "Pois");

			if (b_ptr->resist_fear) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 6, -1, attr, "Fear");

			if (b_ptr->resist_lite) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 7, -1, attr, "Lite");

			if (b_ptr->resist_dark) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(1, 8, -1, attr, "Dark");

			if (b_ptr->resist_blind) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 1, -1, attr, "Blind");

			if (b_ptr->resist_confu) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 2, -1, attr, "Confu");

			if (b_ptr->resist_sound) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 3, -1, attr, "Sound");

			if (b_ptr->resist_shard) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 4, -1, attr, "Shard");

			if (b_ptr->resist_nexus) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 5, -1, attr, "Nexus");

			if (b_ptr->resist_nethr) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 6, -1, attr, "Nethr");

			if (b_ptr->resist_chaos) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 7, -1, attr, "Chaos");

			if (b_ptr->resist_disen) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(6, 8, -1, attr, "Disen");

			/* Other abilities */
			if (b_ptr->slow_digest) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 1, -1, attr, "S.Dig");

			if (b_ptr->ffall) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 2, -1, attr, "Feath");

			if (b_ptr->lite) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 3, -1, attr, "PLite");

			if (b_ptr->regenerate) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 4, -1, attr, "Regen");

			if (b_ptr->telepathy) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 5, -1, attr, "Telep");

			if (b_ptr->see_inv) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 6, -1, attr, "Invis");

			if (b_ptr->free_act) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			Term_putstr(12, 7, -1, attr, "FrAct");

			if (b_ptr->hold_life) attr = TERM_BLUE;
			else attr = TERM_SLATE;
			...
			if (b_ptr->sustain_str) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(21, 1, -1, attr, "STR");

			if (b_ptr->sustain_int) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(21, 2, -1, attr, "INT");

			if (b_ptr->sustain_wis) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(21, 3, -1, attr, "WIS");

			if (b_ptr->sustain_dex) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(21, 4, -1, attr, "DEX");

			if (b_ptr->sustain_con) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(21, 5, -1, attr, "CON");

			if (b_ptr->sustain_chr) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(21, 6, -1, attr, "CHR");

			/* Temporary effects */
			Term_putstr(28, 0, -1, TERM_WHITE, "Temp Effects");
			if (b_ptr->protevil) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 1, -1, attr, "Prot. Evil");

			if (b_ptr->invuln) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 2, -1, attr, "Invulnerable");

			if (b_ptr->hero) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 3, -1, attr, "Heroism");

			if (b_ptr->shero) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 4, -1, attr, "Super Heroism");

			if (b_ptr->shield) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 5, -1, attr, "Shielded");

			if (b_ptr->blessed) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 6, -1, attr, "Blessed");

			if (b_ptr->tim_invis) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 7, -1, attr, "See Invisible");

			if (b_ptr->tim_infra) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(28, 8, -1, attr, "See Infrared");

			if (b_ptr->fast) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(42, 1, -1, attr, "Fast");

			/* Power & Depth info */
			Term_putstr(47, 0, -1,TERM_WHITE, "Recall:        ");
			buf = format("%d",b_ptr->max_depth);
			Term_putstr(54, 0, -1,TERM_WHITE, buf);

			Term_putstr(47, 1, -1,TERM_WHITE, "Happy:         ");
			buf = format("%d",borg_happy_depth);
			Term_putstr(54, 1, -1,TERM_WHITE, buf);

			if (borg_unique_level) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(47, 3, -1, attr, "Unique on level");			
			
			if (vault_on_level) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(47, 4, -1, attr, "Vault on level");

			if (borg_close_doors) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(47, 5, -1, attr, "Breeders (closing doors)");

			if (borg_cursed) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(47, 6, -1, attr, "Wearing a cursed item");

			borg_notice(TRUE);
			borg_prepared(borg_happy_depth + 1);
			Term_putstr(47, 8, -1, TERM_WHITE, "Reason:                           ");
			Term_putstr(55, 8, -1, TERM_WHITE, borg_reason);

			/* Slays and Brands */
			Term_putstr(5, 10, -1, TERM_WHITE, "Slays and Brands");

			if (my_slay_animal) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(1, 11, -1, attr, "Animal");

			if (my_slay_evil) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(1, 12, -1, attr, "Evil");

			if (my_slay_undead) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(1, 13, -1, attr, "Undead");

			if (my_slay_demon) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(1, 14, -1, attr, "Demon");

			if (my_slay_orc) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(8, 11, -1, attr, "Orc");

			if (my_slay_troll) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(8, 12, -1, attr, "Troll");

			if (my_slay_giant) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(8, 13, -1, attr, "Giant");

			if (my_kill_dragon) attr = TERM_BLUE;
			else if (my_slay_dragon) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(8, 14, -1, attr, "Dragon");

			if (my_brand_acid) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(15, 11, -1, attr, "Acid");

			if (my_brand_elec) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(15, 12, -1, attr, "Elec");

			if (my_brand_fire) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(15, 13, -1, attr, "Fire");

			if (my_brand_cold) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(15, 14, -1, attr, "Cold");

			if (my_brand_pois) attr = TERM_WHITE;
			else attr = TERM_SLATE;
			Term_putstr(20, 11, -1, attr, "Pois");
...
void do_cmd_borg(void)
{
	...
	if (cmd == '?')
	{
        ...
        Term_putstr(5, i++, -1, TERM_WHITE, "Command 's' activates string search mode.");
		Term_putstr(5, i++, -1, TERM_WHITE, "Command 'i' displays grid info.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command 'a' displays current danger.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command 'd' displays memorized danger.");
		Term_putstr(5, i++, -1, TERM_WHITE, "Command 'k' displays monster info.");
		Term_putstr(5, i++, -1, TERM_WHITE, "Command 't' displays object info.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command 'p' displays projectable.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command '%' displays current flow.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command '_' displays depth info.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command '&' modifies timestamp.");
        Term_putstr(5, i++, -1, TERM_WHITE, "Command '$' displays current power.");
		Term_putstr(5, i++, -1, TERM_WHITE, "Command 'm' writes a mapfile.");
		Term_putstr(5, i++, -1, TERM_WHITE, "Command 'w' displays swap.");
		...
		/* Prompt for key */
        msg_print("Available commands: z,x,u,f,l,s,i,a,d,k,tp,%,_,&,$.");
		msg_print(NULL);
	...
	else if (cmd == 'i')
	{
        ...
        if (cmd == '?')
        {
            ...
            /* Dump commands */
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 'm' displays CAVE_MARK.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 'g' displays CAVE_GLOW.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 'd' displays CAVE_DARK.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 'h' displays CAVE_HACK.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 's' displays CAVE_SEEN.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 'v' displays CAVE_VIEW.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 't' displays CAVE_TEMP.");
            Term_putstr(5, i++, -1, TERM_WHITE, "Flag 'w' displays CAVE_WALL.");
            ...
            /* Prompt for key */
            msg_print("Available flags: m,g,d,h,s,v,t,w.");
            msg_print(NULL);
	...
	else if (cmd == 'a')
	{
		...
		int limit = (p_ptr->command_arg ? p_ptr->command_arg : borg_avoid/borg_avoid_factor);
	...
	else if (cmd == '$')
	{
        ...
        if (b_ptr->depth == 0)
            borg_note(format("# Home power: %ld", borg_power_home()));
	}
	...
	else if (cmd == 'w')
	{
		...
		{
			item = &borg_items[borg_weapon_swap];
			msg_format("Swap Weapon:  %s (%d), value= %d", item->desc, borg_weapon_swap, borg_weapon_swap_value);
		}
		if (borg_armour_swap)
		{
			item = &borg_items[borg_armour_swap];
			msg_format("Swap Armour:  %s (%d), value= %d", item->desc, borg_armour_swap, borg_armour_swap_value);
		}
		msg_format("cursed : %d, amt *Removecurse* : %d",borg_cursed,amt_star_decurse);
	}
	/* Command: Print map */
	else if (cmd == 'm')
	{
	...
	{
		s32b time = borg_time - borg_when_began;
