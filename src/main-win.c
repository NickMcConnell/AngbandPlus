 ...
 *
 * HACK - UNREFERENCED_PARAMETER will be redefined (slightly differently) 
 * by windows.h.  Undefining it here prevents a compiler warning.
 */
...
/* Temporarily disable a warning generated by mmsystem.h */
# pragma warning(push)
 ...
 */
static char *analyze_font(char *path, s16b *wp, s16b *hp)
{
	s16b wid, hgt;
	...
	for (s = p; *s; ++s)
	{
		...
		if (islower((byte)*s)) *s = INT2CHR(toupper((byte)*s));
	}
...
static void term_getsize(term_data *td)
{
	...
	/* Window sizes */
	wid = (s16b) (td->cols * td->tile_wid + td->size_ow1 + td->size_ow2);
	hgt = (s16b) (td->rows * td->tile_hgt + td->size_oh1 + td->size_oh2);
	...
	/* Total size */
	td->size_wid = (s16b) (rc.right - rc.left);
	td->size_hgt = (s16b) (rc.bottom - rc.top);
	...
	/* Save the location */
	td->pos_x = (s16b) rc.left;
	td->pos_y = (s16b) rc.top;
}
...
static void save_prefs(void)
{
	...
	my_strcpy(buf, use_bigtile ? "1" : "0",sizeof(buf));
...
static void load_prefs_aux(term_data *td, cptr sec_name)
{
	...
	/* Tile size */
	td->tile_wid = (s16b) GetPrivateProfileInt(sec_name, "TileWid", wid, ini_file);
	td->tile_hgt = (s16b) GetPrivateProfileInt(sec_name, "TileHgt", hgt, ini_file);

	/* Window size */
	tmpint = GetPrivateProfileInt(sec_name, "NumCols", td->cols, ini_file);
	ISBYTE(tmpint);
	td->cols = (byte) tmpint;
	tmpint = GetPrivateProfileInt(sec_name, "NumRows", td->rows, ini_file);
	ISBYTE(tmpint);
	td->rows = (byte) tmpint;

	/* Window position */
	td->pos_x = (s16b) GetPrivateProfileInt(sec_name, "PositionX", td->pos_x, ini_file);
	td->pos_y = (s16b) GetPrivateProfileInt(sec_name, "PositionY", td->pos_y, ini_file);
}
...
static void load_prefs(void)
{
	...
	use_bigtile = (0 != GetPrivateProfileInt("Angband", "Bigtile", FALSE, ini_file));
...
static int new_palette(void)
{
	...
	assert((nEntries + 16) <= USHRT_MAX); /* WORD_MAX is not defined but WORD is defined as unsigned short */
...
static bool init_graphics(void)
{
	...
	/* if (can_use_graphics != arg_graphics) */
	{
		...
		/* Graphics available */
		can_use_graphics = (arg_graphics != 0);
	}
...
static errr term_force_font(term_data *td, cptr path)
{
	...
	if (!wid || !hgt)
	{
		...
		/* Font size info */
		wid = (s16b) tm.tmAveCharWidth;
		hgt = (s16b) tm.tmHeight;
	}
 ...
 * Hack -- redraw a term_data
 */
static void term_data_redraw_section(term_data *td, s16b x1, s16b y1, s16b x2, s16b y2)
{
	/* Activate the term */
	Term_activate(&td->t);
 ...
 */
static errr Term_curs_win(s16b x, s16b y)
{
	term_data *td = (term_data*)(Term->data);
 ...
 * Draw a "cursor" at (x,y), using a "yellow box".
 */
static errr Term_bigcurs_win(s16b x, s16b y)
{
	term_data *td = (term_data*)(Term->data);
 ...
 */
static errr Term_wipe_win(s16b x, s16b y, s16b n)
{
	term_data *td = (term_data*)(Term->data);
 ...
 */
static errr Term_text_win(s16b x, s16b y, s16b n, byte a, cptr s)
{
	term_data *td = (term_data*)(Term->data);
	RECT rc;
	HDC hdc;
 ...
 */
static errr Term_pict_win(s16b x, s16b y, s16b n, const byte *ap, const char *cp, const byte *tap, const char *tcp)
{
	term_data *td = (term_data*)(Term->data);
	...
	x2 = x * w2 + (s16b) td->size_ow1;
	y2 = y * h2 + (s16b) td->size_oh1;
...
static void handle_wm_paint(HWND hWnd)
{
	...
	else
	{
		...
		x1 = (s16b) (ps.rcPaint.left / td->tile_wid) - 1;
		x2 = (s16b) (ps.rcPaint.right / td->tile_wid) + 1;
		y1 = (s16b) (ps.rcPaint.top / td->tile_hgt) - 1;
		y2 = (s16b) (ps.rcPaint.bottom / td->tile_hgt) + 1;
                                          ...
                                          WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_LBUTTONDOWN:
		{
			...
			Term_mousepress(xPos,yPos,INT2CHR(button));
		...
		case WM_SIZE:
		{
			...
			switch (wParam)
			{
				...
				case SIZE_RESTORED:
				{
					s16b cols = (LOWORD(lParam) - td->size_ow1) / td->tile_wid;
					s16b rows = (HIWORD(lParam) - td->size_oh1) / td->tile_hgt;
...
static errr get_init_cmd()
{
	MSG msg;
#ifndef NDEBUG
	prt("Debug compiled version. Define NDEBUG to remove asserts.",0,0);
#endif
	/* Prompt the user */
...
static void init_stuff(void)
{
	...
	path[strlen(path) - 4] = '\0'; /* Remove .exe */

	/* Get the name of the "*.ini" file */
	my_strcat(path, ".INI",sizeof(path));
	...
	/* Add "lib" to the path */
	my_strcat(path,"\\lib\\",sizeof(path));
