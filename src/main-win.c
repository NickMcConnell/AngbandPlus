 ...
 * If "WIN_USE_GRAPHICS" is defined, then "readdib.h" and "readdib.c" must
...
/* first undefine FALSE & TRUE, since windows is determined to define these */
#undef FALSE
#undef TRUE
#include <windows.h>
...
static void load_prefs(void)
{
   ...
   char buf[1024], ExecutableName[1024];
   ...
   /* Extract the "Force_Templates" flag */
   arg_sound = (GetPrivateProfileInt("Angband", "Force_Templates", 0, ini_file) != 0);


   /* Obtain the "ErrLogFile" (with a simple default) */
   GetPrivateProfileString("Angband", "ErrLogFile", "angband.log",
                           ANGBAND_ERRLOG_FILE, 1024, ini_file);

   errlog = 0;

   /* Analyze the path */
   i = strlen(ANGBAND_ERRLOG_FILE);
   ...
   if (i)
   {
      ...
      if (!errlog)
      {
         quit(format("Failed to open log file %s", ANGBAND_ERRLOG_FILE));
      }
      dlog(DEBUGALWAYS,"Angband/64 (%d.%d.%d) started\n",
                       VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH);
      debuglevel |= DEBUGEXTRA;
      dlog(DEBUGALWAYS,"debugging flags: %08lx\n", debuglevel);
   }

   /* Obtain the "argv0" executable name (with a simple default) */
   GetPrivateProfileString("Angband", "Executable", "angband.exe",
                           ExecutableName, 1024, ini_file);
   argv0=ExecutableName;
   dlog(DEBUGALWAYS,"debugging flags: ExecutableName %s, argv0 %s\n", ExecutableName, argv0);


   (void)GetPrivateProfileString("Angband", "Debug", "none", buf, 1024, ini_file);
   (void)test_debug_flags(buf);
...
#endif /* __MWERKS__ */
{
	...
	switch (uMsg)
	{
		...
		case WM_GETMINMAXINFO:
		{
			...
			/* Maximum window size */
			rc.left = rc.top = 0;
			rc.right = rc.left + td->cols * td->tile_wid + td->size_ow1 + td->size_ow2;
			rc.bottom = rc.top + td->rows * td->tile_hgt + td->size_oh1 + td->size_oh2;
