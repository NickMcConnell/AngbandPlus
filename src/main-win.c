...
/*
 * Tom Morton's Bigscreen patch
 */
#define USE_BIGSCREEN
...
static void load_prefs_aux(term_data * td, cptr sec_name)
{
	...
	/* Visible */
	td->visible =
		(GetPrivateProfileInt(sec_name, "Visible", td->visible,
  ini_file) != 0);

	/* Desired font, with default */
	GetPrivateProfileString(sec_name, "Font", "8X13.FON", tmp, 127,
		ini_file);

	/* Bizarre */
	td->bizarre =
		(GetPrivateProfileInt(sec_name, "Bizarre", td->bizarre,
  ini_file) != 0);
	...
	/* Tile size */
	td->tile_wid =
		GetPrivateProfileInt(sec_name, "TileWid", wid, ini_file);
	td->tile_hgt =
		GetPrivateProfileInt(sec_name, "TileHgt", hgt, ini_file);

	/* Window size */
	td->cols =
		GetPrivateProfileInt(sec_name, "NumCols", td->cols, ini_file);
	td->rows =
		GetPrivateProfileInt(sec_name, "NumRows", td->rows, ini_file);
	...
	if (td == &data[0])
	{
		...
		if (td->rows < 24)
			td->rows = 24;
		if (td->rows > (DUNGEON_HGT + 2))
			td->rows = DUNGEON_HGT + 2;

		/* Sanity */
		if (td->cols < 80)
			td->cols = 80;
		if (td->cols > (DUNGEON_WID + 2))
			td->cols = DUNGEON_WID + 2;
...
static void load_prefs(void)
{
	...
	arg_graphics =
		GetPrivateProfileInt("Angband", "Graphics", GRAPHICS_NONE,
		ini_file);

	/* Extract the "arg_sound" flag */
	arg_sound =
		(GetPrivateProfileInt("Angband", "Sound", 0, ini_file) != 0);
...
static int new_palette(void)
{
	...
	/* Size of palette */
	pLogPalSize =
		sizeof(LOGPALETTE) + (nEntries + 16) * sizeof(PALETTEENTRY);
...
static bool init_graphics(void)
{
	...
	if (!can_use_graphics)
	{
		...
		{
			plog_fmt("Cannot read mask file '%s'", name);
			return (FALSE);
		}
...
static errr term_force_font(term_data * td, cptr path)
{
	...
	td->font_id =
		CreateFont(hgt, wid, 0, 0, FW_DONTCARE, 0, 0, 0, ANSI_CHARSET,
...
static errr Term_text_win(int x, int y, int n, byte a, const char *s)
{
	...
	/* Bizarre size */
	if (td->bizarre || (td->tile_hgt != td->font_hgt) ||
		(td->tile_wid != td->font_wid))
	{
		int i;
	...
	{
		/* Dump the text */
		ExtTextOut(hdc, rc.left, rc.top, ETO_OPAQUE | ETO_CLIPPED, &rc, s,
			n, NULL);
	}
 ...
 */
#ifdef USE_TRANSPARENCY
static errr Term_pict_win(int x, int y, int n, const byte * ap,
	const char *cp, const byte * tap, const char *tcp)
#else /* not USE_TRANSPARENCY */
static errr Term_pict_win(int x, int y, int n, const byte * ap,
	const char *cp)
#endif /* not USE_TRANSPARENCY */
{
#ifdef USE_GRAPHICS
	...
	for (i = 0; i < n; i++, x2 += w2)
	{
		...
		if (arg_graphics == GRAPHICS_ADAM_BOLT)
		{
			...
			else
			{
				...
				/* Copy the terrain picture from the bitmap to the window */
				StretchBlt(hdc, x2, y2, w2, h2, hdcSrc, x3, y3, w1, h1,
					SRCCOPY);
				...
				{
					/* Mask out the tile */
					StretchBlt(hdc, x2, y2, w2, h2, hdcMask, x1, y1, w1,
						h1, SRCAND);

					/* Draw the tile */
					StretchBlt(hdc, x2, y2, w2, h2, hdcSrc, x1, y1, w1, h1,
						SRCPAINT);
				}
			}
		}
...
static void init_windows(void)
{
	...
	td->rows = screen_y;
	td->cols = screen_x;
#else /* not USE_BIGSCREEN */
	td->rows = 24;
	td->cols = 80;
#endif /* not USE_BIGSCREEN */
	td->visible = TRUE;
	td->size_ow1 = 2;
	td->size_ow2 = 2;
	td->size_oh1 = 2;
	...
	for (i = MAX_TERM_DATA - 1; i >= 1; --i)
	{
		...
		my_td = td;
		td->w =
			CreateWindowEx(td->dwExStyle, AngList, td->s, td->dwStyle,
	...
	my_td = td;
	td->w =
		CreateWindowEx(td->dwExStyle, AppName, td->s, td->dwStyle,
		td->pos_x, td->pos_y, td->size_wid, td->size_hgt, HWND_DESKTOP,
		NULL, hInstance, NULL);
	my_td = NULL;
	if (!td->w)
		quit("Failed to create Angband window");
...
static void setup_menus(void)
{
	...
	{
		char cmdChars[] =
			"abcdefgijklnoqrstuvwz" "ABCDEFGILMOQRSTV"
			"#$@%^&*(){}]-+=;:\",<.>\\/~?";
		int i;
		...
		case IDM_OPTIONS_SAVER:
		{
			...
			else
			{
				...
				hwndSaver =
					...
					"WindowsScreenSaverClass", "Angband Screensaver",
					WS_POPUP | WS_MAXIMIZE | WS_VISIBLE, 0, 0,
					GetSystemMetrics(SM_CXSCREEN),
					GetSystemMetrics(SM_CYSCREEN), NULL, NULL, hInstance,
					NULL);

				if (hwndSaver)
				{
					/* Push the window to the bottom XXX XXX XXX */
					SetWindowPos(hwndSaver, HWND_BOTTOM, 0, 0, 0, 0,
						SWP_NOMOVE | SWP_NOSIZE);
				}
				else
				{
...
#ifdef __MWERKS__
LRESULT FAR PASCAL AngbandWndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam);
LRESULT FAR PASCAL AngbandWndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam)
#else /* __MWERKS__ */
...
#endif /* __MWERKS__ */
{
	...
	switch (uMsg)
	{
		...
		case WM_GETMINMAXINFO:
		{
			...
			rc.left = rc.top = 0;
			rc.right =
				rc.left + 8 * td->tile_wid + td->size_ow1 + td->size_ow2;
			rc.bottom =
				rc.top + 2 * td->tile_hgt + td->size_oh1 + td->size_oh2 +
				1;
...
#ifdef USE_BIGSCREEN
			rc.right =
				rc.left + screen_x * td->tile_wid + td->size_ow1 +
				td->size_ow2;
			rc.bottom =
				rc.top + screen_y * td->tile_hgt + td->size_oh1 +
				td->size_oh2;
#else /* not USE_BIGSCREEN */
			rc.right =
				rc.left + 80 * td->tile_wid + td->size_ow1 + td->size_ow2;
			rc.bottom =
				rc.top + 24 * td->tile_hgt + td->size_oh1 + td->size_oh2;
#endif /* not USE_BIGSCREEN */
		...
		case WM_SIZE:
		{
			...
			switch (wParam)
			{
				...
				case SIZE_RESTORED:
				{
					...
					td->cols =
						(LOWORD(lParam) - td->size_ow1 -
						td->size_ow2) / td->tile_wid;
					td->rows =
						(HIWORD(lParam) - td->size_oh1 -
						td->size_oh2) / td->tile_hgt;

					term_getsize(td);

					MoveWindow(hWnd, td->pos_x, td->pos_y, td->size_wid,
						td->size_hgt, TRUE);
					...
					{
						if (data[i].visible)
							ShowWindow(data[i].w, SW_SHOWNOACTIVATE);
					}
...
#ifdef __MWERKS__
LRESULT FAR PASCAL AngbandListProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam);
LRESULT FAR PASCAL AngbandListProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam)
#else /* __MWERKS__ */
...
#ifdef __MWERKS__
LRESULT FAR PASCAL AngbandSaverProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam);
LRESULT FAR PASCAL AngbandSaverProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam)
#else /* __MWERKS__ */
	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	/* Prompt the user */
	s = "[Choose 'New' or 'Open' from the 'File' menu]";
	prt(s, Term->hgt - 1, (Term->wid - strlen(s)) / 2);
	Term_fresh();
