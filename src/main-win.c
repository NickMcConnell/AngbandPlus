 ...
 */
static void save_prefs_aux(term_data *td, cptr sec_name, int term_idx)
{
	char buf[1024];
	...
	/* Font */
	strcpy(buf, td->font_file_25 ? td->font_file_25 : DEFAULT_LARGE_FONT);
	...
	/* The main window can also show 50 rows */
	if (term_idx == 0)
	{
		/* Font */
		strcpy(buf, td->font_file_50 ? td->font_file_50 : DEFAULT_SMALL_FONT);
...
static void load_prefs_aux(term_data *td, cptr sec_name)
{
	...
	GetPrivateProfileString(sec_name, "Font_25", DEFAULT_LARGE_FONT, tmp, 127, ini_file);
	...
	GetPrivateProfileString(sec_name, "Font_50", DEFAULT_SMALL_FONT, tmp, 127, ini_file);
...
static void load_prefs(void)
{
	...
	if (data[0].rows_50 < 50) data[0].rows_50 = 50;
}
...
static errr term_force_font(term_data *td, cptr path)
{
	...
	/* The 25-line font is active and in use */
	...
	/* The 50-line font is active and in use */
	...
	if ((screen_rows != 50) || (!td->variable_rows))
	{
		td->font_file_25 = string_make(base);
	}
	else
	{
		td->font_file_50 = string_make(base);
	}
	...
	if (GetOpenFileName(&ofn))
	{
		...
		if (term_force_font(td, tmp))
		{
			...
			path_build(tmp, sizeof(tmp), ANGBAND_DIR_XTRA_FONT,
				((screen_rows != 50 || (!td->variable_rows)) ?
			      DEFAULT_LARGE_FONT : DEFAULT_SMALL_FONT));
...
static errr Term_xtra_win_clear(void)
{
	...
	if ((screen_rows != 50) || (!td->variable_rows))
	{
		SelectObject(hdc, td->font_id_25);
	}
	else
	{
		SelectObject(hdc, td->font_id_50);
	}
...
/*
 * Change the number of text lines shown on the main screen.
 */
 ...
 */
static void term_data_link(term_data *td, int term_idx)
{
	term *t = &td->t;

	/* Initialize the term */
	if ((screen_rows != 50) || (term_idx != 0))
	{
	...
	/* Only set the rows hook on the main term  XXX XXX */
	if (term_idx == 0) t->rows_hook = Term_rows_win;
...
static void init_windows(void)
{
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		if (term_force_font(td, buf))
		{
			...
			path_build(buf, sizeof(buf), ANGBAND_DIR_XTRA_FONT, DEFAULT_SMALL_FONT);

			/* Force the use of that font */
			(void)term_force_font(td, buf);

			/* Oops */
			td->tile_wid_50 = DEFAULT_SMALL_FONT_WID;
			td->tile_hgt_50 = DEFAULT_SMALL_FONT_HGT;
		...
		if (term_force_font(td, buf))
		{
			...
			/* Oops */
			td->tile_wid_25 = DEFAULT_LARGE_FONT_WID;
			td->tile_hgt_25 = DEFAULT_LARGE_FONT_HGT;
		...
		case IDM_FILE_OPEN:
		{
			...
			else
			{
				...
				if (GetOpenFileName(&ofn))
				{
					...
					if (character_icky > 0) screen_load();  /* XXX XXX */
                                          ...
                                          WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_GETMINMAXINFO:
		{
			...
			/* Minimum window size is 80x25 or 80x50 */
			rc.left = rc.top = 0;
			if ((screen_rows != 50) || (!td->variable_rows))
			{
