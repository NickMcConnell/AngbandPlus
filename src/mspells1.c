...
static void remove_bad_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p)
{
	...
	distance(p_ptr->py, p_ptr->px, m_ptr->fy, m_ptr->fx);
	if ((smart) && (d <= 5) && (int_outof(is_dumb, 100)))
	{
		f5 &= ~(RF6_QUAKE);
	}
...
bool make_attack_spell(int m_idx)
{
	...
	switch (thrown_spell)
	{
		...
		case 96+2:
		{
			...
			else msg_format("%^s throws a blob of lava.", m_name);
			bolt(m_idx, GF_LAVA, damroll(8, 8) + (rlev / 3));
		...
		case 128+9:
		{
			...
			if (p_ptr->csp)
			{
				...
				/* Basic message */
				msg_format("%^s draws magical energy from you!", m_name);
		...
		case 128+20:
		{
			...
			bolt(m_idx, GF_POIS, damroll(4, 8) + (rlev / 3));
			update_smart_learn(m_idx, DRS_POIS);
			update_smart_learn(m_idx, DRS_REFLECT);
			break;
		}
		...
		case 128+26:
		{
			...
			bolt(m_idx, GF_MANA, damroll(2, 6) + (rlev / 3));
			update_smart_learn(m_idx, DRS_REFLECT);
			break;
		}
		...
		case 160+6:
		{
			disturb(TRUE);
			if (blind) msg_format("%^s mumbles.", m_name);
			else msg_format("%^s gestures volcanicly.", m_name);
			msg_print("You are covered in lava.");
			breath(m_idx, GF_LAVA,
				randint1(rlev * 4) + 10, 3, FALSE);
			update_smart_learn(m_idx, DRS_FIRE);
			break;
		}
		...
		{
			disturb(TRUE);
			if (blind) msg_format("%^s stamps on the ground.", m_name);
			else msg_format("%^s creates a tremor.", m_name);
			bolt(m_idx, GF_QUAKE, damroll(6, 6) + (rlev));
			update_smart_learn(m_idx, DRS_REFLECT);
			break;
		}
		...
		{	
			if (blind) msg_format("%^s mumbles.", m_name);
			else msg_format("%^s smashes at the ground.", m_name);
			earthquake(p_ptr->py, p_ptr->px, 5, m_idx);
			break;
		}
