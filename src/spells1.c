...
/*
 * Retrieve a 'good' color for a spell, given the type
 */
...
void take_hit(int damage, cptr hit_from)
{
	...
	if (p_ptr->chp < 0)
	{
		...
		/* Hack -- Note death */
		if (last_words && !get_rnd_line("death.txt", hit_from, buf))
		{
			msg_print(buf);
		}
		else
...
static bool hates_acid(object_type *o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		/* Staffs/Scrolls/Books are wood/paper */
		case TV_STAFF:
		case TV_SCROLL:
		case TV_LIFE_BOOK:
...
static bool project_f(int who, int r, int y, int x, int dam, int typ)
{
	...
	bool wall = !cave_floor_bold(y, x);
	bool bare = cave_naked_bold(y, x);
	bool hear = player_has_los_bold(y, x);
	bool seen = player_can_see_bold(y, x);
	...
	switch (typ)
	{
		...
		case GF_DEST_MUND:
		{
			/* Only barrels of goo explode */
			if (feat != FEAT_BARREL_GOO) break;

			/* Usually explode */
			if (rand_int(20) > dam) break;
		...
		case GF_CRUSHER:
		{
			if (!wall)
			{
				...
				/* Track this crusher; handle overflow by forgetting last */
				if (crusher_n < CRUSHER_MAX) crusher_n++;
			}
			break;
		}
		...
		case GF_JAM_DOOR: /* Jams a door (as if with a spike) */
		{
				 ...
				 (feat <= FEAT_DOOR_TAIL))
			{
				...
				if (feat < FEAT_DOOR_HEAD + 0x08) draw_grid_feat(y, x, feat + 0x08);

				/* Add one spike to the door */
				if (feat < FEAT_DOOR_TAIL) draw_grid_feat(y, x, feat + 1);
		...
		case GF_LITE:
		{
			...
			if (m_idx) update_mon(m_idx, FALSE);
...
/*	dam = (dam + r) / (r + 1);*/
	...
	for (this_o_idx = cave_o_idx[y][x]; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		switch (typ)
		{
			...
			/* Mana and Crushers -- destroy everything */
			case GF_MANA:
			case GF_CRUSHER:
			{
				do_kill = TRUE;
			...
			case GF_DEST_MUND:
			{
				...
				/* Excellent items and artifacts are immune */
				if (excellent_p(o_ptr))
				{
					ignore = TRUE;
				}
		...
		if (do_kill)
		{
			...
			else
			{
				...
				if (is_potion)
				{
					(void)potion_smash_effect(y, x, o_sval);
				}
...
static bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	/* Can the player know about this monster? */
	...
	/* Nobody here */
	if (!cave_monster_bold(y, x)) return (FALSE);
	...
	if ((who < 0) && (m_ptr->smart & SM_FRIEND))
	{
		...
		switch (typ)
		{
			case GF_ACID:
			{
				if (!(r_ptr->flags2 & (RF2_HEAL_ACID))) get_angry = TRUE;
				break;
			}
			case GF_ELEC:
			{
				if (!(r_ptr->flags2 & (RF2_HEAL_ELEC))) get_angry = TRUE;
				break;
			}
			case GF_FIRE:
			{
				if (!(r_ptr->flags2 & (RF2_HEAL_FIRE))) get_angry = TRUE;
				break;
			}
			case GF_COLD:
			{
				if (!(r_ptr->flags2 & (RF2_HEAL_COLD))) get_angry = TRUE;
				break;
			}
			case GF_POIS:
			{
				if (!(r_ptr->flags2 & (RF2_HEAL_POIS))) get_angry = TRUE;
				break;
			}
			case GF_AWAY_UNDEAD:
			case GF_AWAY_EVIL:
	...
	switch (typ)
	{
		...
		case GF_ACID:
		{
			...
			if (r_ptr->flags2 & (RF2_HEAL_ACID))
			{
				...
				/* Remember it */
				if (seen) r_ptr->r_flags2 |= (RF2_HEAL_ACID);
			}
		...
		case GF_ELEC:
		{
			...
			if (r_ptr->flags2 & (RF2_HEAL_ELEC))
			{
				...
				/* Remember it */
				if (seen) r_ptr->r_flags2 |= (RF2_HEAL_ELEC);
			}
		...
		case GF_FIRE:
		{
			...
			if (r_ptr->flags2 & (RF2_HEAL_FIRE))
			{
				...
				/* Remember it */
				if (seen) r_ptr->r_flags2 |= (RF2_HEAL_FIRE);
			}
		...
		case GF_COLD:
		{
			...
			if (r_ptr->flags2 & (RF2_HEAL_COLD))
			{
				...
				/* Remember it */
				if (seen) r_ptr->r_flags2 |= (RF2_HEAL_COLD);
			}
			else if (r_ptr->flags3 & (RF3_IM_COLD))
			{
				note = " resists a lot.";
				dam /= 9;
		...
		case GF_POIS:
		{
			...
			if (r_ptr->flags2 & (RF2_HEAL_POIS))
			{
				...
				/* Remember it */
				if (seen) r_ptr->r_flags2 |= (RF2_HEAL_POIS);
			}
		...
		/* Rocket: Shard resistance helps - but not as much as sound */
		case GF_ROCKET:
		{
			if (seen) obvious = TRUE;
			...
			if ((r_ptr->flags4 & (RF4_BR_SOUN)) || (r_ptr->flags3 & (RF3_RES_SOUN)))
			{
				note = " resists somewhat.";
		...
		case GF_GRAVITY:
		{
			...
			else
			{
				...
				/* 2. stun */
				do_stun = damroll((p_ptr->level / 10) + 3 , (dam)) + 1;
		...
		case GF_DISINTEGRATE:
		{
			...
			{
				if (rand_int(r_ptr->level + 10) > rand_int(p_ptr->level))
				{
					note = " resists.";
					dam >>= 3;
				}
		...
		case GF_DOMINATION:
		{
			if (m_ptr->smart & SM_FRIEND) break;
			if (seen) obvious = TRUE;
		...
		case GF_OLD_CLONE:
		{
			...
			/* Attempt to clone. */
			if (multiply_monster(m_idx, !!(m_ptr->smart & SM_FRIEND), TRUE))
			{
				note = " spawns!";
			}
		...
		case GF_DEST_MUND:
		{
			/* Only affect non-evil non-unique animals */
			if ((!r_ptr->flags1 & (RF1_UNIQUE)) &&
				 (r_ptr->flags3 & (RF3_ANIMAL)) &&
				 !(r_ptr->flags3 & (RF3_EVIL)))
			{
	...
	else if (do_stun && !(r_ptr->flags3 & (RF3_NO_STUN | RF3_RES_SOUN)) &&
	...
	if (who >= 0)
	{
		...
		if (m_ptr->hp < 0)
		{
			...
			if (note && known) msg_format("%^s%s", m_name, note);
		...
		else
		{
			...
			/* Hack -- Pain message */
			else if ((dam > 0) && known) message_pain(m_idx, dam);
...
static bool project_p(int who, int r, int y, int x, int dam, int typ, int a_rad)
{
	...
	/* Player is not here */
	if (!cave_player_bold(y, x)) return (FALSE);

	/* Player cannot hurt himself */
	if (who < 0) return (FALSE);


	if (p_ptr->reflect && (rand_int(100) < p_ptr->skill_sav) && (typ != GF_CRUSHER))
	{
		if (!a_rad)
		{
			...
			/* Reflect directly to caster */
			if (rand_int(100) < p_ptr->skill_sav + p_ptr->level - r_ptr->level)
			{
				scatter(&ty, &tx, fy, fx, dist / 10, 0);
			}
			/* Bounce it somewhere randomly */
			else
			{
				scatter(&ty, &tx, py, px, MAX_SIGHT - dist, 0);
			}

			/* Affect features and grids if no monster targetted */
			if (cave_dead_bold(ty, tx)) flg |= PROJECT_GRID | PROJECT_ITEM;

			project(-1, 0, ty, tx, dam, typ, flg);
	...
	switch (typ)
	{
		...
		case GF_NETHER:
		{
			...
			else
			{
				...
				{
					msg_print("Your life slips away!");
		...
		case GF_ROCKET:
		{
			...
			{
				inven_damage(set_cold_destroy, 3);
				inven_damage(set_fire_destroy, 1);
			}
		...
		/* Pure damage unless player is a spectre or has wraith form */
		case GF_CRUSHER:
		{
			if ((p_ptr->prace != RACE_SPECTRE) && !p_ptr->wraith_form)
			{
				if (fuzzy) msg_print("You are being crushed!");
				take_hit(dam, "a crushing ceiling");
...
bool project(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	if ((typ == GF_ROCKET) || (typ == GF_BFG)) flg |= PROJECT_STOP;
  ...
  */
bool potion_smash_effect(int y, int x, int o_sval)
 {
	int       radius = 2;
	int       dt = 0;
	int       dam = 0;
	...
	(void)project(0, radius, y, x, dam, dt,
