...
static bool hates_acid(object_type *o_ptr)
{
	...
	{
		/* Wearable items */
		case TV_ARROW: case TV_BOLT: case TV_BOW: case TV_DIGGING:
		case TV_HAFTED: case TV_POLEARM: case TV_AXE: case TV_SWORD:
		case TV_HELM: case TV_CROWN: case TV_SHIELD: case TV_BOOTS:
...
static bool hates_fire(object_type *o_ptr)
{
	...
	{
		/* Wearable */
		case TV_LITE: case TV_ARROW: case TV_BOW: case TV_DIGGING:
		case TV_HAFTED: case TV_POLEARM: case TV_AXE: case TV_BOOTS:
		case TV_GLOVES:	case TV_CLOAK: case TV_SOFT_ARMOR:
		{
			return (TRUE);
		}
...
static bool project_f(int who, int r, int y, int x, int dam, int typ)
{
	...
	switch (typ)
	{
		...
		case GF_PLASMA: case GF_METEOR: case GF_ICE: case GF_SHARDS:
		case GF_FORCE: case GF_SOUND: case GF_MANA:
		case GF_HOLY_FIRE: case GF_HELL_FIRE: case GF_DISINTEGRATE:
		case GF_PSI: case GF_PSI_DRAIN: case GF_TELEKINESIS:
		case GF_DOMINATION:
		{
			break;
		}
...
static bool project_o(int who, int r, int y, int x, int dam, int typ)
{
	...
	for (this_o_idx = c_ptr->o_idx; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		switch (typ)
		{
			...
			/* Unlock chests */
			case GF_KILL_TRAP: case GF_KILL_DOOR:
			{
...
static bool project_m(int who, bool pet_attack, int r, int y, int x, int dam, int typ)
{
	...
	char		killer[80];
	bool		seen = m_ptr->ml; /* Is the monster "seen"? */
	bool		obvious = FALSE; /* Were the effects "obvious"? */
	bool		skipped = FALSE; /* Were the effects "irrelevant"? */
	int		do_poly = 0; /* Polymorph setting (true or false) */
	...
	if ((r_ptr->flags3 & (RF3_DEMON)) || (r_ptr->flags3 & (RF3_UNDEAD)) ||
	    (r_ptr->flags2 & (RF2_STUPID)) || (r_ptr->flags3 & (RF3_NONLIVING)) ||
	...
	if ((!who) && (m_ptr->smart & SM_FRIEND))
	{
		bool get_angry = FALSE;
		/* Grrr? */
		switch (typ)
		{
			case GF_AWAY_UNDEAD:	case GF_AWAY_EVIL:
			case GF_AWAY_ALL:	case GF_CHARM:
			case GF_CONTROL_UNDEAD:	case GF_CONTROL_ANIMAL:
			case GF_OLD_HEAL:	case GF_OLD_SPEED:
			case GF_DARK_WEAK:	case GF_JAM_DOOR:
			...
			case GF_HELL_FIRE:
				if (!(r_ptr->flags3 & (RF3_GOOD)))
					get_angry = TRUE;
				break;
			case GF_TURN_UNDEAD: case GF_DISP_UNDEAD:
				if (r_ptr->flags3 & RF3_UNDEAD)
					get_angry = TRUE;
				break;
			case GF_TURN_EVIL: case GF_DISP_EVIL:
			...
			case GF_DOMINATION:
				if (!(r_ptr->flags3 & (RF3_NO_CONF)))
					get_angry = TRUE;
				break;
			case GF_OLD_POLY: case GF_OLD_CLONE:
				if (randint(8) == 1)
					get_angry = TRUE;
				break;
			case GF_LITE: case GF_LITE_WEAK:
	...
	switch (typ)
	{
		...
		case GF_FIRE:
		{
			...
			if (r_ptr->flags3 & (RF3_HURT_FIRE))
			{
				...
				note_dies = " collapses into a pile of ash.";
				dam += (dam / 2);
		...
		case GF_PLASMA:
		{
			...
			if (r_ptr->flags3 & (RF3_RES_PLAS))
			{
				note = " resists.";
				dam /= 3;
				if (seen) r_ptr->r_flags3 |= (RF3_RES_PLAS);
			}
			break;
		}
		...
		case GF_CHAOS:
		{
			   ...
			   ((r_ptr->flags3 & (RF3_DEMON)) && (randint(3)==1)))
			{
				...
				dam /= 3;
				do_poly = FALSE;
				if ((seen) && r_ptr->flags3 & (RF3_RES_CHAO))
					r_ptr->r_flags3 |= (RF3_RES_CHAO);
				if ((seen) && r_ptr->flags3 & (RF3_DEMON))
		...
		case GF_DISINTEGRATE:
		{
			...
			if (r_ptr->flags1 & RF1_UNIQUE)
			{
				if (rand_int(r_ptr->level + 10) > rand_int(p_ptr->lev * 2))
				{
					note = " resists.";
					dam /= 3;
				}
		...
		case GF_PSI:
		{
			         ...
			         (r_ptr->level > randint(3 * dam)))
			{
				...
				/*
				 * Powerful demons & undead can turn a
				 * mindcrafter's attacks back on them
				 */
				if (((r_ptr->flags3 & RF3_UNDEAD) ||
				     (r_ptr->flags3 & RF3_DEMON)) &&
				     (r_ptr->level > ((3 * p_ptr->lev) / 2)) && 
				     (randint(2) == 1))
				{
					note = NULL;
		...
		case GF_OLD_CLONE:
		{
			...
			if (seen) obvious = TRUE;
			if ((m_ptr->smart & SM_FRIEND) && (randint(3)!=1))
				is_friend = TRUE;
	...
	/* Confusion breathers never confuse */
	else if (do_conf &&
		 !(r_ptr->flags3 & (RF3_NO_CONF)) &&
