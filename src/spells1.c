...
void take_hit(int damage, cptr hit_from)
{
	...
	if (p_ptr->invuln && (damage < 9000))
	{
        ...
        if((o_ptr->k_idx) && (randint(10)<4) && (!carried_monster_hit))
        {
                ...
                {
                        cmsg_print(TERM_L_RED, "Your monster dies from protecting you, you feel very sad...");
                ...
                {
                        msg_print("Your monster takes the damage instead of you.");
                        o_ptr->pval2 -= damage;
                        monster_take = TRUE;
                }
...
static bool hates_fire(object_type *o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_ARROW:
                {                        
                        if ((p_ptr->pclass == CLASS_ARCHER) && magik(100 - p_ptr->lev))
                        {
                                return FALSE;
                        }
                        else
...
sint project_path(u16b *gp, int range, int y1, int x1, int y2, int x2, int flg)
{
	...
	if ((x1 == x2) && (y1 == y2)) return (0);
		...
		while (1)
		{
			...
			/* Always stop at non-initial wall grids */
                        if ((n > 0) && !(cave_floor_bold(y, x) || (cave[y][x].feat == FEAT_MON_TRAP))) return n;
	...
	if (ay > ax)
	{
		...
		while (1)
		{
			...
			/* Always stop at non-initial wall grids */
                        if ((n > 0) && !(cave_floor_bold(y, x) || (cave[y][x].feat == FEAT_MON_TRAP)) && !(flg & PROJECT_WALL)) break;
	...
	switch (typ)
	{
		...
		case GF_METEOR:
				...
				/* Destroy the trap */
                        ...
                        else if ((c_ptr->feat == FEAT_SANDWALL) || (c_ptr->feat == FEAT_SANDWALL_H) || (c_ptr->feat == FEAT_SANDWALL_K))
                        {
                                int k = randint(100);
			...
			case GF_KILL_DOOR:
			{
				...
				break;
                        ...
                        case GF_STAR_IDENTIFY:
                        {
                                ...
                                process_hooks(HOOK_IDENTIFY, 0 - this_o_idx);
                        ...
                        case GF_RAISE:
                        {
                                ...
                                if (o_ptr->tval == TV_CORPSE)
                                {
                                        ...
                                        if (place_monster_one(y, x, o_ptr->pval2, ego, FALSE, MSTATUS_PET))
			...
			case GF_RAISE_DEMON:
				...
				if (randint(100) > r_ptr->level - p_ptr->lev)
				{
					if (r_ptr->level < 10) name="Manes";
                                        ...
                                        if (place_monster_one(y, x, test_monster_name(name), 0, FALSE, MSTATUS_PET))
...
bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	monster_desc(m_name, m_ptr, 0);
        ...
        {
                msg_print("%s is immune to magic.");
                return seen;
        }
	...
	/* Mega-Hack -- Handle "polymorph" -- monsters get a saving throw */
        else if (do_poly && (randint(90) > m_ptr->level))
	{
		int new_r_idx;
		int old_r_idx = m_ptr->r_idx;
                int status = m_ptr->status;
		...
		if (new_r_idx != old_r_idx)
		{
			...
			/* Create a new monster (no groups) */
                        if (!place_monster_aux(y, x, new_r_idx, FALSE, FALSE, status))
			{
				/* Placing the new monster failed */
                                place_monster_aux(y, x, old_r_idx, FALSE, FALSE, status);
				obvious = FALSE;
				note = " is unaffected!";
			}
...
static bool project_p(int who, int r, int y, int x, int dam, int typ, int a_rad)
{
	...
	if (who == -2) fuzzy = TRUE;
        ...
        if (who == -99)
        {
            ...
            sprintf(killer, "%s, deity of %s", deity_info[p_ptr->pgod - 1].name,
               deity_info[p_ptr->pgod - 1].god_of);
            }
        ...
        if (who == -2)
        {
                sprintf(killer, t_name + t_info[cave[py][px].t_idx].name);
        }
	...
	switch (typ)
	{
		...
		case GF_DARK:
		{
			...
			if (p_ptr->resist_dark)
			{
				dam *= 4; dam /= (randint(6) + 6);
                                ...
                                if ((p_ptr->pracem == RMOD_VAMPIRE)||(p_ptr->mimic_form == MIMIC_VAMPIRE)) dam = 0;
			}
			else if (!blind && !p_ptr->resist_blind)
			{
				(void)set_blind(p_ptr->blind + randint(5) + 2);
				...
				switch (randint(10))
				{
					...
					case 6: case 7: case 8: case 9:
					{
						switch (randint(6))
						{
							...
							case 5: k = A_CON; act = "hardy"; break;
                ...
                case GF_MAKE_GLYPH:
		{
                        ...
                        set_shield(p_ptr->shield + dam, 50, 0);
			break;
		}
...
bool project(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		/* Hack -- Balls explode before reaching walls */
                if (!(cave_floor_bold(ny, nx) || (cave[ny][nx].feat == FEAT_MON_TRAP)) && (rad > 0)) break;
...
void describe_attack_fully(int type, char* r) {
  ...
  case GF_SHARDS:      strcpy(r, "shards"); break;
