...
void take_hit(int damage, cptr hit_from)
{
	...
	if (p_ptr->ironwill)
	{
		...
		if (damage>p_ptr->chp)
		{
			p_ptr->ironwill-=damage/2;
			damage=p_ptr->chp;
		}
		if (d<p_ptr->ironwill || p_ptr->ironwill<10) /* Fini */
		{
			p_ptr->ironwill=0;
...
static bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	switch (typ)
	{
		...
		case GF_DISP:
		{
			/* Only affect creatures being dispelled */
			if (!p_flag || r_ptr->flags3 & p_flag)
			{
				/* Learn about type */
				if (seen)
				{
					r_ptr->r_flags3 |= p_flag;
