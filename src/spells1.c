...
void teleport_away(int m_idx, int dis)
{
	...
	if (p_ptr->ts_anchor)
	{
		msg_print("The time/space anchor prevents the monster from teleporting.");
		return;
	}
...
static byte spell_color(int type)
{
	...
	switch (type)
	{
		...
		case GF_BALEFIRE:	return (TERM_RED);
		case GF_EMPATHY:	return (TERM_SLATE);
		case GF_PSI:		return (TERM_L_GREEN);
		case GF_PSI2:		return (TERM_VIOLET);
		case GF_PSI3:		return (TERM_VIOLET);
		case GF_AMNESIA:	return (TERM_VIOLET);			
	}
...
static bool hates_elec(object_type *o_ptr)
{
	switch (o_ptr->tval)
	{
		...
		case TV_PSI_BOOK:
		{
			/* Hack - level 1 foci hate elec, 2 gets save, 3 immune */
...
static bool project_o(int who, int r, int y, int x, int dam, int typ)
{
	...
	for (this_o_idx = cave_o_idx[y][x]; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		switch (typ)
		{
			...
			/* Mana and disintegration -- destroys everything */
			case GF_MANA:
			case GF_DISINTEGRATE:
			{
				do_kill = TRUE;
...
/*
 * Count the number of psionic attacks a monster has.
 * This is used to determine psionic resistance.
 */

static int num_psi_attacks(monster_race *r_ptr)
{
	int x=0;
	
	/* Count the psionic attacks */
...
static bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	/* Does the monster resist the psi effect? */
	bool resist = FALSE;
	...
	dam = (dam + r) / (r + 1);
        ...
        /* Fading damage hack - not reduced by distance */
        if (fade_dam_on)
        {
        	dam = fade_dam;
	...
	switch (typ)
	{
		...
		case GF_PSI_DRAIN:
		{
			...
			{
				for (tmp = num_psi_attacks(r_ptr) ; tmp ; tmp--)
				{
		...
		case GF_PSI3:
		{
			...
			if ((r_ptr->flags2 & RF2_STUPID) || (r_ptr->flags3 & RF3_ANIMAL))
			{
				note = " is too stupid to be hurt.";
				dam /= 2;
			}
		...
		case GF_BALEFIRE: /* half fire, half time */
		{
			...
			if (bale_effect & BALE_CLONE)
			{
				...
				multiply_monster(cave_m_idx[y][x]);
			}

			if (bale_effect & BALE_HEAL)
			{
				msg_format("%^s jumps back to a healthier point in its existence.",m_name);
			...
			{
				msg_format("Your balefire tries to remove %s from existence completely!",m_name);
				dam *= 10;
	         ...
	         !(r_ptr->flags4 & (RF4_BR_CHAO)))
	{
		...
		/* Will get stunned and confused */
		if (do_stun) note = " looks dazed and confused.";
...
static bool project_p(int who, int r, int y, int x, int dam, int typ)
{
	...
	{	
       ...
       /* Psi Hack - these tests should be done somewhere else */
	...
	switch (typ)
	{
		...
		{
			/* Unresisted */	
			if (!p_ptr->resist_confu) set_confused(p_ptr->confused + rand_int(20) + 10);
		}
		...
		case GF_PSI3:
		{
			...
			if (rand_int(rand_int(100 + mlev )) < rand_int(p_ptr->skill_sav)) psi_resists++;
			if (p_ptr->resist_psi) psi_resists++;
			if (p_ptr->mental_barrier) psi_resists++;
			...
			if (typ == GF_PSI2)
			{
				switch(psi_resists)
				{
					...
					case -1:
					case  0:					
						if (!rand_int(20 + 4 * psi_resists))
						{
							msg_print("Your memories fade away.");
							lose_all_info();
						}
						if (!rand_int(7 + 2 * psi_resists)) dec_stat(A_INT,randint(100),!rand_int(5));
						if (!rand_int(7 + 2 * psi_resists)) dec_stat(A_WIS,randint(100),!rand_int(5));
			...
			{
				k = dam;
				if (psi_resists > 0) k -= (p_ptr->lev * psi_resists);
				...
				while (k > 0)
				{
					...
					switch (rand_int(24))
					{
						...
						case 7:
							...
							if (rand_int(10 + 2 * psi_resists)) break;
						case 8:
						case 9:
						case 10:
						case 11:
						...
						case 14:
						case 15:
							if (!rand_int(p_ptr->paralyzed + 1) && !p_ptr->free_act)
		...
		case GF_MISSILE:
		{
			...
			if (p_ptr->shadow_form) dam /= 3;
			if (p_ptr->biofeedback) dam /= 2;
			if (p_ptr->inertial_barrier) dam /= 2;
			take_hit(dam, killer);
			break;
		}
		...
		case GF_SOUND:
		{
			if (fuzzy) msg_print("You are hit by something!");
			if (p_ptr->biofeedback) dam /= 2;
			if (p_ptr->inertial_barrier) dam /= 3;
			if (p_ptr->resist_sound)
			{
		...
		{
			if (fuzzy) msg_print("You are hit by something strange!");
			if (p_ptr->inertial_barrier) dam *= 6;
		...
		{
			if (fuzzy) msg_print("You are hit by something!");
			if (p_ptr->inertial_barrier) dam = dam * 3 / 4;
			take_hit(dam, killer);
			break;
		}
...
bool project(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	for (dist = 0; dist <= rad; dist++)
	{
		...
		for (y = y2 - dist; y <= y2 + dist; y++)
		{
			for (x = x2 - dist; x <= x2 + dist; x++)
			{
				...
				/* Ball explosions are stopped by walls, but not disintegration */
				if (typ != GF_DISINTEGRATE)
				{
					if (!los(y2, x2, y, x)) continue;
     					...
     					{
						/* Disintegration is stopped by  permanent walls */
						if (cave_feat[y][x] >= FEAT_PERM_EXTRA) break;
					}
                                }
	...
	if (flg & (PROJECT_KILL))
	{
		...
		for (i = 0; i < grids; i++)
		{
			...
			/* Mega-Hack - allow balefire abort */
