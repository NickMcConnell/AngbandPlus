...
void take_hit(int dam, cptr kb_str)
{
	...
	int warning = (p_get_mhp() * op_ptr->hitpoint_warn / 10);
		...
		case GF_NETHER:
		{
			...
			else
			{
				...
				else
				{
					...
					{
						s32b d = 200 + (pc_array[pcidx].exp / 100) * MON_DRAIN_LIFE / PY_MAX_CLASSES;
						if (p_ptr->state.hold_life)
							lose_exp(d / 10, pcidx);
						else
							lose_exp(d, pcidx);
		...
		case GF_CHAOS:
		{
			...
			if (!p_ptr->state.resist_nethr && !p_ptr->state.resist_chaos)
			{
				...
				else
				{
					...
					for (pcidx = 0; pcidx < PY_MAX_CLASSES; pcidx++)
					{
						s32b d = 500 + (pc_array[pcidx].exp / 100) * MON_DRAIN_LIFE / PY_MAX_CLASSES;
						if (p_ptr->state.hold_life)
							lose_exp(d / 10, pcidx);
						else
							lose_exp(d, pcidx);
		...
		case GF_TIME:
		{
			...
			switch (randint1(10))
			{
				case 1: case 2: case 3: case 4: case 5:
				{
					...
					for (pcidx = 0; pcidx < PY_MAX_CLASSES; pcidx++)
					{
						lose_exp(100 + (pc_array[pcidx].exp / 100) * MON_DRAIN_LIFE, pcidx);
					}
					break;
				}
