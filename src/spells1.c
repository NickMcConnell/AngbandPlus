...
static int minus_ac(void)
{
	...
	/* ~ a quick player may be able to save the armor...*/
	if (!p_ptr->paralyzed &&
	    (rand_int(100) < ((adj_dex_safe[p_ptr->stat_ind[A_DEX]] +
	                      p_ptr->lev)/2)))
	{
		msg_format("You deftly save your %s from damage!", o_name);
...
void acid_dam(int dam, cptr kb_str)
{
	...
	/* ~ possibly disfigure player */
	if (randint(p_ptr->lev + 10) == 1)
	{
 ...
 * ~ can now disfigure player, but it's much less likely than with
 # ~ acid -- neko
 */
void fire_dam(int dam, cptr kb_str)
{
	...
	/* ~ disfigure player on rare occasions */
	if (randint(p_ptr->lev + 50) == 1)
	{
		if (dec_stat(A_CHR, 10, FALSE))
		{
			/* ~ Message */
