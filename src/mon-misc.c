...
/* mon-misc.c: monster manipulation routines */
	...
	if (size)
	{
		...
		for (i = 1; i < mon_max; i++)
		{
			...
			else
			{
				...
				/* Permanent monsters and Pets are protected */
				else if ((m_ptr->align & (AL_PET_MASK)) || m_ptr->mflag & (MFLAG_PERM)) mon_lev[i] += MAX_DEPTH;
...
void wipe_mon_list(void)
{
	...
	for (i = 1; i < mon_max; i++)
	{
		...
		/* Hack -- preserve permanent monsters */
		if (m_ptr->mflag & (MFLAG_PERM))
		{
			count++;

			/* Hack -- move monster */
			COPY(&mon_list[count], m_ptr, monster_type);

			/* Monster is gone... for a while, that is */
			cave_m_idx[m_ptr->fy][m_ptr->fx] = 0;

			/* Wipe the Monster */
			if (i != count) (void)WIPE(m_ptr, monster_type);
...
s16b get_mon_num(int level)
{
	...
	do
	{
		...
		for (i = 0; i < alloc_race_size; i++)
		{
			...
			/* Valar don't normally appear */
			if (r_ptr->flags3 & (RF3_VALA)) continue;
...
void update_mon(int m_idx, bool full)
{
	...
	if (d <= MAX_SIGHT)
	{
		...
		if (player_has_los_bold(fy, fx) && !p_ptr->blind)
		{
			...
			if (player_can_see_bold(fy, fx))
			{
				...
				/* Handle monsters which are invisible because of terrain */
				else if ((cave_feat[fy][fx] == FEAT_FOG) ||  /* Nothing is visible in the fog */
					(strchr("Ev.", r_ptr->d_char) && strstr(r_name + r_ptr->name, "ire") &&
					 cave_feat[fy][fx] == FEAT_FIRE) ||  /* Fire elementals/vortices are invisible in fire */
					(strchr("Ev.", r_ptr->d_char) && strstr(r_name + r_ptr->name, "agma") &&
					 cave_feat[fy][fx] == FEAT_LAVA) ||  /* Magma elementals are invisible in magma */
					(strchr("Ev.", r_ptr->d_char) && strstr(r_name + r_ptr->name, "ater") &&
					 cave_feat[fy][fx] == FEAT_WATER) ||  /* Water elementals/vortices are invisible in water */
					(strchr("Ev.", r_ptr->d_char) && strstr(r_name + r_ptr->name, "arth") &&
					 cave_wall_bold(fy, fx)))
				{
				    	; /* Do nothing */
				}
	...
	/* Mega-Mega-Mega Hack: in wizard mode, *everybody* *always* is *visible* */
...
bool place_monster_one(int y, int x, int r_idx, bool slp, int nlev)
{
	...
	/* The monster must be able to pass that grid safely */
	if (monster_pass_non_wall(r_idx, y, x, 100) < 65) return (FALSE);
	...
	if ((r_ptr->flags1 & (RF1_UNIQUE)) && !(r_ptr->flags3 & (RF3_VALA)) && (summon_pets_hack))
	{
		/* Cannot create */
		return (FALSE);
	}
	...
	if ((r_ptr->flags3 & (RF3_LAWFUL)) && (p_ptr->al_special == PAL_CHAOS) && (summon_pets_hack))
	{
		/* Cannot create */
		return (FALSE);
	}
	...
	if ((!p_ptr->depth) && (!r_ptr->level) && (!summon_pets_hack))
	{
	...
	/* Mega-Hack -- No sleepers in wizard mode */
	if (slp && r_ptr->sleep && !p_ptr->wizard)
	{
	...
	if (summon_pets_hack)
	{
		...
		else if (r_ptr->flags3 & (RF3_CHAOTIC))
	...
	/* Reset AC, spellpower, mana */
	n_ptr->ac = r_ptr->ac;
	n_ptr->maxmana = n_ptr->mana;
	n_ptr->spell_power = r_ptr->spell_power;
	
	/* Hack -- Set the timeout for the Valar */
	if (r_ptr->flags3 & (RF3_VALA)) n_ptr->timeout = 100;
	...
	if ((p_ptr->depth) && (recent_failed_thefts > 30)
		&& ((randint(5) + 30) > recent_failed_thefts) && !summon_pets_hack)
	{
		/*make them all awake.....*/
		n_ptr->csleep = 0;
	...
	/* Gain levels */
	if (nlev > 1) n_ptr->exp = MONSTER_EXP(nlev) + 1;
	while ((n_ptr->level < MONSTER_LEVEL_MAX) && (n_ptr->level < nlev))
	{
		n_ptr->level++;
		...
		if (!r_ptr->body.hitdie)
			n_ptr->maxhp += randint(r_ptr->level);
		else
			n_ptr->maxhp += randint(r_ptr->body.hitdie);
		
		/* +2 mana, spell power (only if already had some) */
		if (n_ptr->maxmana) n_ptr->maxmana += 2;
		if (n_ptr->spell_power) n_ptr->spell_power += 2;
		
		/* Evolve if needed and possible */
		if (n_ptr->level >= r_ptr->body.evolve_level && r_ptr->body.evolve_index[0])
		{
			int i = 0;

			/* Count evolution forms */
			while (r_ptr->body.evolve_index[i]) i++;
				
			/* Change race */
			n_ptr->r_idx = r_ptr->body.evolve_index[rand_int(i)];
			...
			n_ptr->maxmana = r_ptr->mana;
			n_ptr->spell_power = r_ptr->spell_power;
			n_ptr->mspeed = r_ptr->speed;
			n_ptr->ac = r_ptr->ac;

			/* Reroll bonuses for experience */
			n_ptr->ac += 2 * (n_ptr->level - 1);
			if (n_ptr->maxmana) n_ptr->maxmana += 2 * (n_ptr->level - 1);
			if (n_ptr->spell_power) n_ptr->spell_power += 2 * (n_ptr->level - 1);
			for (i = 1; i < n_ptr->level; i++)
			{
				if (!r_ptr->body.hitdie)
					n_ptr->maxhp += randint(r_ptr->level);
				else
	...
	else if (r_ptr->flags1 & (RF1_UNIQUE))
	{
		...
		/* King Uniques always induce a message */
		if (n_ptr->r_idx == rp_ptr->king_r_idx)
		{
...
/*
 * Maximum size of a group of pets
 */
#define GROUP_PET_MAX	6
...
bool place_monster_group(int y, int x, int r_idx, bool slp, s16b group_size)
{
	...
	if (summon_pets_hack && group_size > GROUP_PET_MAX) group_size = GROUP_PET_MAX;
	...
	for (n = 0; (n < hack_n) && (hack_n < group_size); n++)
	{
		...
		for (i = start; (i < 8 + start) && (hack_n < group_size); i++)
		{
			...
			if (place_monster_one(my, mx, r_idx, slp, 1))
			{
...
static void place_monster_escort(int y, int x, int leader_idx, bool slp)
{
	...
	for (n = 0; (n < hack_n) && (hack_n < escort_size); n++)
	{
		...
		for (i = start; (i < 8 + start) && (hack_n < escort_size); i++)
		{
			...
			/* Attempt to place another monster */
			else if (!place_monster_one(my, mx, escort_idx, slp, 1))
			{
				continue;
			}
...
bool place_monster_aux(int y, int x, int r_idx, bool slp, bool grp)
{
	...
	if (!place_monster_one(y, x, r_idx, slp, 1)) return (FALSE);
...
bool alloc_monster(int dis, bool slp)
{
	...
	attempts_left = 10;
	while (attempts_left-- && !place_monster(y, x, slp, TRUE))
		;
...
bool multiply_monster(int m_idx)
{
	...
	if (!grids)
	{
		/* Hack -- switch the alignment */
		if (m_ptr->align & (AL_PET_MASK))
		{
			/* Pet */
			if (m_ptr->align == AL_PET_C)
				m_ptr->align = AL_PET_L;
			else if (m_ptr->align == AL_PET_L)
				m_ptr->align = AL_PET_C;
			else  /* Randomly choose */
				m_ptr->align = (rand_int(2) == 1 ? AL_PET_C : AL_PET_L);
		}
		else
		{
			/* Enemy */
			if (m_ptr->align == AL_HOSTILE_C)
				m_ptr->align = AL_HOSTILE_L;
			else if (m_ptr->align == AL_HOSTILE_L)
				m_ptr->align = AL_HOSTILE_C;
			else  /* Randomly choose */
				m_ptr->align = (rand_int(2) == 1 ? AL_HOSTILE_C : AL_HOSTILE_L);
		}
	...
	/* Mega-Hack -- pets multiply as pets */
	summon_pets_hack = (m_ptr->align & (AL_PET_MASK));
