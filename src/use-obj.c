...
static bool eat_food(object_type *o_ptr, bool *ident)
{
	...
	switch (o_ptr->sval)
	{
		case SV_FOOD_POISON:
		{
			if (!(p_ptr->resist_pois || p_ptr->timed[TMD_OPP_POIS] || p_ptr->timed[TMD_SUSTAIN_HEALTH]))
			{
		...
		case SV_FOOD_LUCKFINDING: /* pricy luckfinding */
		{
            ...
            if ((price == 1) && ((badluck*2) - goodluck < rand_int(7) - rand_int(5))) 
                price = randint(5) + 1;
			switch (price)
			{
			...
			/* stat too low to get lower */
			if (p_ptr->stat_cur[sick] < 4)
			{
				take_hit(damroll(3, 6), "poisonous clover");
				msg_print("You feel pitiful.");
				*ident = TRUE;
				break;
			}
			sickold = p_ptr->stat_cur[sick];
			/* lower the stat */
			if (p_ptr->stat_cur[sick] > 18+20)
			{
				p_ptr->stat_cur[sick] = (p_ptr->stat_cur[sick] * 5) / 6;
			}
			else p_ptr->stat_cur[sick] -= 2;

			if (p_ptr->stat_cur[sick] < 3) p_ptr->stat_cur[sick] = 3;
			price = sickold - p_ptr->stat_cur[sick];

			/* strength drain isn't always permanent */
			if ((sick == A_STR) && (goodluck - badluck > 0)) /* skip */;
			else if (goodluck - badluck - randint(11) > 0) /* skip */;
			/* otherwise permanent & bypasses sustains */
			if (sickold > 18+20) p_ptr->stat_max[sick] -= price;
			else p_ptr->stat_max[sick] -= 2;
		...
		{
			/* SUSTAIN_HEALTH protects but doesn't let you know what type the shroom is */
			if (p_ptr->timed[TMD_SUSTAIN_HEALTH]) break;
		...
		case SV_FOOD_EMERGENCY:
		{
			...
			else if (((cp_ptr->flags & CF_ZERO_FAIL) && (p_ptr->msp > 50) &&
				(p_ptr->csp <= p_ptr->msp/10)) || ((p_ptr->msp > 100) &&
				(p_ptr->csp <= p_ptr->msp/10)))
			{
				p_ptr->csp += p_ptr->msp/2 + randint(p_ptr->msp/4);
	...
	/* notice changes to stats, slime and silver poison levels */
...
static bool quaff_potion(object_type *o_ptr, bool *ident)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_POTION_MULTIHUED_POISON:
		{
			...
			/* golems have no CON score */
			if ((p_ptr->prace == 16) && (sick == A_CON)) break;
		...
		case SV_POTION_DETECT_MON:
		{
            ...
            {
				if (detect_monsters_normal(2)) *ident = TRUE;
				if (detect_monsters_invis()) *ident = TRUE;
				if (((badluck*2)/3 >= 5) && (goodluck < 2))
				{
					if (set_timed(TMD_MDETECTION, randint(4) + 4)) *ident = TRUE;
				}
				else if (set_timed(TMD_MDETECTION, randint(5 + (goodluck*2)/3 - (badluck*2)/3) + 8)) 
					*ident = TRUE;
			}
			break;
		}
		...
		{
			int dir;
			msg_print("Breathe in what direction? (max.range 15)");
			*ident = TRUE;
		...
		case SV_POTION_INC_CON:
		{
			...
			if (p_ptr->prace == 16) value = 18+100;
    ...
    /* notice changes of stats, slime and silver poison levels */
    p_ptr->redraw |= (PR_SILVER);
    p_ptr->redraw |= (PR_SLIME);
	p_ptr->redraw |= (PR_STATS);
	...
	switch (o_ptr->sval)
	{
		case SV_SCROLL_DARKNESS:
		{
			if (cp_ptr->spell_book == TV_DARK_BOOK)
			{
				...
				{
					(void)inc_timed(TMD_BLIND, 3 + randint(3));
				}
				if (unlite_area(13, 11, TRUE)) *ident = TRUE;
				/* may extend DARKSTEP effect */
				if (p_ptr->timed[TMD_DARKSTEP]) (void)inc_timed(TMD_DARKSTEP, rand_int(4));
			}
			else
			{
				if (!p_ptr->resist_blind)
				...
				if (unlite_area(10, 8, FALSE)) *ident = TRUE;
			}
			break;
		}
		...
		{
            if ((goodluck > 16) && (rand_int(100) < goodluck))
            {
               msg_print("You feel as if you just escaped a nasty curse..");
               break; /* (very lucky) */
            }
            else if ((goodluck > 13) && (rand_int(100) < goodluck * 3))
            {
				if (curse_armor(TRUE)) *ident = TRUE;
               break; /* (less nasty version) */
            }
			if (curse_armor(FALSE)) *ident = TRUE;
			break;
		}
		...
		case SV_SCROLL_IDENTIFY:
		{
			*ident = TRUE;
			...
			{
			   msg_print("The writing on the scroll doesn't disappear!");
               used_up = FALSE;
		...
		case SV_SCROLL_LIGHT:
		{
			...
			if (p_ptr->timed[TMD_DARKSTEP]) take_hit(damroll(2, 6), "a scroll of light");
			break;
		}
...
/*		case SV_SCROLL_DETECT_GOLD:
		...
		case SV_SCROLL_DISPEL_UNDEAD:
		{
			...
			if (turn_undead(p_ptr->skills[SKILL_DEV]*4/5)) *ident = TRUE;
			break;
		}
...
static bool use_staff(object_type *o_ptr, bool *ident)
{
	...
	switch (o_ptr->sval)
	{
		case SV_STAFF_DARKNESS:
		{
			...
			else if ((randint(p_ptr->skills[SKILL_DEV] + (goodluck*2)) > strongodd) &&
				(cp_ptr->spell_book != TV_PRAYER_BOOK))
			{
				strongdark = TRUE;
				darkdam = damroll(3, (p_ptr->skills[SKILL_DEV]/12)) + enhanced/2;
			}
			if ((!p_ptr->resist_blind) && (rand_int(50) > enhanced))
			{
				if (!(cp_ptr->spell_book == TV_DARK_BOOK))
					(void)inc_timed(TMD_BLIND, 3 + randint(6));
				else if (randint(100) > 5 - badluck + goodluck*4)
					(void)inc_timed(TMD_BLIND, 3 + randint(3));
			}
			if (unlite_area(darkdam, darkrad, strongdark)) *ident = TRUE;
			break;
		}
		
		/* the only staff which aims */
		case SV_STAFF_STRIKING:
		{
			int damg = damroll(3, 9);
			*ident = TRUE;
  	        if (!get_aim_dir(&dir))
  	        {
	           use_charge = FALSE;
...
bool wand_effect(int sval, bool *ident, int enhanced, int dir)
{
	...
	if (plev) /* set wand range */
	{
		int rangedev = luckdev + p_ptr->lev/5 - badluck/2;
	
		/* don't always reach MAX_RANGE anymore */
		if (rangedev > 55) /* (use MAX_RANGE which is 20) */ range = 0;
		else if (rangedev > 40) range = 19;
		else if (rangedev > 25) range = 18;
		else if (rangedev > 15) range = 17;
		else range = 16;
	}
	...
	*ident = FALSE;
	...
	switch (sval)
	{
		...
		case SV_WAND_LITE:
		{
			...
			{
				int blek = plev/5 * p_ptr->skills[SKILL_DEV]/5;
				if (plev < 5) blek = p_ptr->skills[SKILL_DEV]/5;
                msg_print("A line of blue shimmering light appears.");
				if (blek > 31) lite_line(dir, 0);
		...
		{
			pwr = 160;
			if (randint(luckdev + enhanced) > 60)
				pwr += randint(p_ptr->skills[SKILL_DEV]/2);
			if (pwr + enhanced < 200) pwr += enhanced;
			else if (enhanced) pwr += (pwr + enhanced - 198) / 4;
...
static bool aim_wand(object_type *o_ptr, bool *ident)
{
	...
	/* thieves zap faster (usually) */
	if ((!p_ptr->timed[TMD_CONFUSED]) && (cp_ptr->flags & CF_CLASS_SPEED) && 
		(rand_int(p_ptr->skills[SKILL_DEV]) > 10 + badluck))
	{
	...
	if ((o_ptr->charges == 1) && (o_ptr->enhance))
	{
		o_ptr->enhance = 0;
		o_ptr->enhancenum = 0;
	}
	/* can't have more items enhanced than there are charges left */
	else if ((o_ptr->enhance) && (o_ptr->charges) && 
		(o_ptr->enhancenum < o_ptr->number))
	{
		if (o_ptr->enhancenum > o_ptr->charges) o_ptr->enhancenum = o_ptr->charges;
	}
...
bool rod_effect(int sval, bool *ident, int enhanced, int dir)
{
	...
	int rangedev = luckdev + plev/5 - badluck/2;

	/* don't always reach MAX_RANGE anymore */
	if (rangedev > 50) /* (use MAX_RANGE which is 20) */ range = 0;
	else if (rangedev > 35) range = 19;
	...
	*ident = FALSE;
	...
	switch (sval)
	{
		...
		case SV_ROD_TELEPORT_AWAY: /* SV_ROD_TELEPORT_OTHER */
		{
		...
		case SV_ROD_LITE: /* SV_ROD_LIGHT (why always use dumb spelling?) */
		{
			int blek;
            msg_print("A line of blue shimmering light appears.");
		...
		{
            int pwr = p_ptr->lev + 6 + adj_chr_charm[p_ptr->stat_ind[A_CHR]];
			if (randint(p_ptr->skills[SKILL_DEV] + goodluck + enhanced) > 70)
				pwr += 7;
		...
		case SV_ROD_DRAIN_LIFE:
		{
			...
			if (randint(p_ptr->skills[SKILL_DEV]) + goodluck > 65)
				draindam += p_ptr->skills[SKILL_DEV] / 5;
...
static bool zap_rod(object_type *o_ptr, bool *ident)
{
	...
	if (o_ptr->timeout > (o_ptr->charges - k_ptr->pval))
	{
		if (flush_failure) flush();
	...
	*ident = FALSE;
	...
	used_charge = rod_effect(o_ptr->sval, ident, enhanced, dir);
	...
	if (o_ptr->name1)
	{
		...
		switch (a_ptr->activation)
		{
			...
			case ACT_CLAIRVOYANCE:
			{
				...
				(void)detect_doorstairs(FALSE);
				(void)detect_traps();
				/* object detection no longer for whole level */
				(void)detect_treasure();
			...
			/* weaker version for dagger of Pippin */
			case ACT_WCLAIRVOYANCE:
			{
	...
	if (o_ptr->tval == TV_DRAG_ARMOR)
	{
		...
		switch (o_ptr->sval)
		{
			case SV_DRAGON_BLUE:
			{
				...
				o_ptr->timeout = rand_int(240) + 300;
				break;
			}
			...
			case SV_DRAGON_MULTIHUED:
			{
				...
				fire_ball(((chance == 1) ? GF_ELEC :
				          ...
				          dir, 250, 2);
				o_ptr->timeout = rand_int(180) + 300;
				break;
			}
...
{
	u32b f1, f2, f3, f4;
	s32b minute = 0, rminute = 0, sec = 0, rsec = 0;
	...
	if (o_ptr->name1)
	{
		...
		else if (a_ptr->randtime) rsec = a_ptr->randtime * 10;
		else rsec = 0;
	...
	switch (o_ptr->sval)
	{
		case SV_DRAGON_BLUE:
		{
			text_out("breathe lightning (100) every 50+d40 minutes");
			break;
		}
		case SV_DRAGON_WHITE:
		{
			text_out("breathe frost (110) every 50+d40 minutes");
			break;
		}
		case SV_DRAGON_BLACK:
		{
			text_out("breathe acid (130) every 50+d40 minutes");
			break;
		}
		case SV_DRAGON_GREEN:
		{
			text_out("breathe poison gas (150) every 50+d40 minutes");
			break;
		}
		case SV_DRAGON_RED:
		{
			text_out("breathe fire (200) every 50+d40 minutes");
			break;
		}
		case SV_DRAGON_MULTIHUED:
		{
			text_out("breathe multi-hued (250) every 50+d30 minutes");
			break;
		}
		case SV_DRAGON_BRONZE:
		{
			text_out("breathe confusion (120) every 40+d40 minutes");
			break;
		}
		case SV_DRAGON_GOLD:
		{
			text_out("breathe sound (130) every 40+d40 minutes");
			break;
		}
		case SV_DRAGON_CHAOS:
		{
			text_out("breathe chaos/disenchant (220) every 40+d40 minutes");
			break;
		}
		case SV_DRAGON_LAW:
		{
			text_out("breathe sound/shards (230) every 40+d40 minutes");
			break;
		}
		case SV_DRAGON_BALANCE:
		{
			text_out("breathe balance (250) every 40+d40 minutes");
			break;
		}
		case SV_DRAGON_SHINING:
		{
			text_out("breathe light/darkness (200) 40+d40 minutes");
			break;
		}
		case SV_DRAGON_POWER:
		{
			text_out("breathe the elements (300) every 40+d40 minutes");
			break;
		}
		case SV_DRAGON_ETHEREAL:
		{
			text_out("breathe light, dark, confusion, or nexus (230) every 40+d40 minutes");
			break;
		}
	}
}
