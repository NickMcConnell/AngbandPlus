...
/*
 * Sync team members' options with leader's
 */

void do_cmd_options_sync (int plr_sync)
{
	int opt_idx, plr_idx;

	/* Apply to each player */
	/* 	How much I want to remove all those '4's from code! -IB */
	for (plr_idx = 0; plr_idx < 4; plr_idx++)
	{
	
		/* Don't sync with self */
		if (plr_idx == plr_sync) continue;
		
		/* First, sync all options... */
		for (opt_idx = 0; opt_idx < OPT_MAX; opt_idx++)
			team_info[plr_idx].opt[opt_idx] = team_info[plr_sync].opt[opt_idx];

		/* ...then windows... */
		for (opt_idx = 0; opt_idx < 8; opt_idx++)
			team_info[plr_idx].window_flag[opt_idx] = team_info[plr_sync].window_flag[opt_idx];
			
		/* ...then hitpoint warning... */
		team_info[plr_idx].hitpoint_warn = team_info[plr_sync].hitpoint_warn;
		
		/* ...and delay factor */
		team_info[plr_idx].delay_factor = team_info[plr_sync].delay_factor;
	}
}
 ...
 */
void do_cmd_options(bool allow_sync)
{
	char ch, sync_prompt[80];

	/* Save screen */
	screen_save();

	sprintf (sync_prompt, "(S) Sync team members' options with %s's", op_ptr->full_name);
	...
	while (1)
	{
		...
		/* Why are we here */
		prt("Angband options", 0, 0);

		/* Give some choices */
		prt("(1) User Interface Options", 2, 5);
		prt("(2) Disturbance Options", 3, 5);
		prt("(3) Game-Play Options", 4, 5);
		prt("(4) Efficiency Options", 5, 5);
		prt("(5) Birth Options", 6, 5);
		prt("(6) Cheat Options", 7, 5);

		/* Window flags */
		prt("(W) Window flags", 9, 5);

		/* Load and Append */
		prt("(L) Load a user pref file", 11, 5);
		prt("(A) Append options to a file", 12, 5);
		...
		/* Sync team */
		if (allow_sync) prt(sync_prompt, 17, 5);
		...
		/* Sync team */
		else if (allow_sync && ((ch == 'S') || (ch == 's')))
		{
			do_cmd_options_sync (p_ptr->whoami);
			msg_format ("Options synchronized with %s", team_info[p_ptr->whoami].full_name);
		}
...
{
	/* Silly message */
	msg_format("You are playing Team Angband %d.%d.%d.  Type '?' for more info.",
