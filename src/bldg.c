...

	{
		switch (bact)
		{
			...
			case BACT_LOSE_MUTATION:
				...
				break;
			default: /* Prfnoff */
				msg_print("Oops. This is an invalid building action.");
				break;
		}
	}
...
/*
 * Entering or leaving a quest -- Prfnoff
 */
void set_inside_quest(int new_quest, bool by_stair)
{
	quest_type *q_ptr;

	/* Player enters a new quest */
	leaving_quest = p_ptr->inside_quest;
	p_ptr->inside_quest = new_quest;

	if (leaving_quest)
	{
		/* Access the quest */
		q_ptr = &quest[new_quest];

		/* Was quest completed? */
		if (by_stair && (q_ptr->type == QUEST_TYPE_FIND_EXIT))
		{
