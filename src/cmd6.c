...
static void do_cmd_quaff_potion_aux(int item)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_POTION_DETONATIONS:
		{
			...
			take_hit(damroll(50, 20), "a potion of Nitro 9");
...
static void do_cmd_read_scroll_aux(int item)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_SCROLL_RESOURCE:
		{
			if (p_ptr->crp < p_ptr->mrp)
			{
				p_ptr->crp += p_ptr->mrp / 2;
				ident = TRUE;
	...
	switch (o_ptr->sval)
	{
		...
		case SV_STAFF_REMOVE_CURSE:
		{
			if (remove_curse())
			{
				if (!p_ptr->blind)
				{
					msg_print("The staff glows yellow for a moment...");
				}
				ident = TRUE;
			}
			break;
...
void do_cmd_tech_item_aux(int item)
{
	...
	if ((item < 0) && (o_ptr->number > 1))
	{
		msg_print("You must first pick up the machinery.");
		return;
	}


	/* Get a direction (unless KNOWN not to need it) */
	if ((o_ptr->sval <= SV_TECH_MAX_DIRECTION) || !object_aware_p(o_ptr))
	{
		/* Get a direction, allow cancel */
		if (!get_aim_dir(&dir)) return;
	}
	...
	/* Take a turn */
	p_ptr->energy_use = MIN(75, 200 - 5 * tech_skill / 8);
	...
	{
		if (flush_failure) flush();
		msg_print("You failed to use the machinery properly.");
		sound(SOUND_FAIL);
		return;
	}
	...
	{
		if (flush_failure) flush();
		msg_print("The machinery is still reloading.");
		return;
	}
	/* A stack of rods lacks enough energy. */
	else if ((o_ptr->number > 1) && (o_ptr->timeout > o_ptr->pval - k_ptr->pval))
	{
		if (flush_failure) flush();
		msg_print("The machines are all still reloading.");
		return;
	}
	...
	switch (o_ptr->sval)
	{
		...
		case SV_TECH_DETECT_MONSTER:
		{
			ident = detect_monsters_normal();
			break;
		}
		
		case SV_TECH_DETECT_MAGIC:
		{
			ident = detect_objects_magic();
			break;
		}
...
void do_cmd_tech_item(void)
{
	int item;
	cptr q, s;

	/* Restrict choices to rods */
	item_tester_tval = TV_TECH;

	/* Get an item */
	q = "Use which machine? ";
	s = "You have no machines to use.";
	if (!get_item(&item, q, s, (USE_INVEN | USE_FLOOR))) return;

	/* Zap the rod */
	do_cmd_tech_item_aux(item);
}
