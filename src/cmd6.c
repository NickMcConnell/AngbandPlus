...
bool item_tester_hook_edible(object_type* o_ptr) {
  if (o_ptr->tval == TV_FOOD && 
      o_ptr->stuff == STUFF_FLESH) return TRUE;
  return FALSE;
}
...
{
  item_effect("potion", "drink", TRUE, TRUE, SOUND_QUAFF, NULL, TV_POTION);
}
...
void do_cmd_use_staff(void)
{
  ...
  o_ptr = item_effect("staff", "use", FALSE, FALSE, SOUND_ZAP, NULL, TV_STAFF);
...
void do_cmd_aim_wand(void)
{
  ...
  o_ptr = item_effect("wand", "zap", FALSE, FALSE, SOUND_ZAP, NULL, TV_WAND);
...
bool item_tester_hook_rod(object_type* o_ptr) {
  if (o_ptr->tval == TV_ROD && o_ptr->timeout == 0) return TRUE;
  return FALSE;
}
...
void do_cmd_zap_rod(void)
{
  ...
  item_effect("rod", "zap", FALSE, FALSE, SOUND_ZAP, item_tester_hook_rod, 0);
...
static bool item_tester_hook_activate(object_type *o_ptr)
{
	...
	if (f3 & (TR3_ACTIVATE)) {
	  ...
	  for (i = 0; i < EQUIP_MAX; i++) {
	    if (equipment[i] == o_ptr) return TRUE;
	  }
	}
...
object_type* item_effect(cptr name, cptr act, bool obvious, bool one_time_use,
			 int snd, bool (hook)(object_type*), s16b tval) {
  ...
  if (!obvious) {
    ...
    if ((chance < USE_DEVICE) || (randint(chance) < USE_DEVICE)) {
      ...
      if (one_time_use) remove_object(o_ptr);
  ...
  if (artifact_p(o_ptr)) {
    ...
    o_ptr->timeout = rand_int(a_ptr->timeout_rand) + 
      a_ptr->timeout_static;
    
  } else {
    success = cause_spell_effect(&activations[k_ptr->activation]);
    o_ptr->timeout = rand_int(k_ptr->timeout_rand) +
...
void do_cmd_brew_stuff(void) {
  ...
  while (TRUE) {
    ...
    o_ptr = get_item(q, s, p_ptr->py, p_ptr->px, 
...
void do_cmd_sacrifice(void) {
  ...
  /* Get sacrifice */
  o_ptr = get_item("Sacrifice what", "You don't have anything to sacrifice.",
		   p_ptr->py, p_ptr->px,
		   (USE_INVEN | USE_BY_PARTS | USE_REMOVE));
...
void do_cmd_change_shape(void) {
  ...
  o_ptr = get_item("Use which book", "You have no books of lore.",
...
void do_cmd_cast_helper(void) {
  ...
  if (p_ptr->shape) {
	...
	p_ptr->pclass == CLASS_BEASTMASTER) {
      foo = get_check("Attempt to use the magical powers of your shape? ");
    }
