...
void do_cmd_quaff_potion(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		{
			if (set_adrenaline(p_ptr->adrenaline + 20)) ident = TRUE;
			break;		
		}
	}
...
void do_cmd_activate(void)
{
	...
	if (o_ptr->name1)
	{
		...
		switch (a_ptr->activation)
		{
			...
			case ACT_INVULN:
			{
				msg_format("Your %s produces a protective globe...", o_name);
				(void)set_invuln(p_ptr->invuln + 8 + randint(8));
				break;
			}
			
			case ACT_MANA_STORM:
			{
				msg_format("Your %s glows with intense magical energy...", o_name);
				if (!get_aim_dir(&dir)) return;
			...
			case ACT_DIM_DOOR:
			{
				msg_format("Your %s opens a dimensional door...", o_name);
				(void)dimension_door(40);
				break;			
			}
			
			case ACT_AWARENESS:
			{
				msg_format("Your %s expands your mind...", o_name);
				(void)set_awareness(p_ptr->awareness + 20 + randint(20));
				break;
			}
			
			case ACT_ADRENALINE:
			{
				msg_format("Your %s shoots small needles into your body...", o_name);
				(void)set_adrenaline(p_ptr->adrenaline + 20 + randint(20));
				break;
			}
			
			case ACT_BIOFEEDBACK:
			{
				msg_format("Your %s helps your mind control your body...", o_name);
				(void)set_biofeedback(p_ptr->biofeedback + 10 + randint(20));
				break;
			}
			
			case ACT_INERTIAL_BARRIER:
			{
				msg_format("Your %s produces a thuunderous sound...", o_name);
				(void)set_inertial_barrier(p_ptr->inertial_barrier + 10 + randint(10));
				break;
			}
			
			case ACT_PROB_TRAVEL:
			{
				msg_format("Your %s opens a probability gate...", o_name);
				(void)set_prob_travel(p_ptr->prob_travel + 5 + randint(5));
				break;
			}
			...
			case ACT_TS_ANCHOR:
			{
				msg_format("Your %s starts to pulsate slowly...", o_name);
				(void)set_ts_anchor(p_ptr->ts_anchor + 5 + randint(5));
				break;
			}
			
			case ACT_MENTAL_BARRIER:
			{
				msg_format("Your %s lets out a shrill note...", o_name);
				(void)set_mental_barrier(p_ptr->mental_barrier + 10 + randint(10));
				break;
			}
			
			case ACT_ULTRABLAST:
			{
				msg_format("Your %s glows violet...", o_name);
				project_hack(GF_PSI,damroll(50,50));				
				break;
			}
			
			case ACT_STAR_ULTRABLAST:
			{
				msg_format("Your %s glows ultra violet...", o_name);
				project_hack(GF_PSI2,damroll(100,50));
				break;
			}
			
			case ACT_ANALYZE:
			{
				msg_format("Your %s glows in a bluish green...", o_name); 
				if (!get_aim_dir(&dir)) return;
				fire_ball(GF_ANALYZE,dir,100,0);
				break;
			}
			
			case ACT_DISINTEGRATE:
			{
				msg_format("Your %s starts vibrating quickly...", o_name);
				if (!get_aim_dir(&dir)) return;
				fire_ball(GF_DISINTEGRATE,dir,damroll(50,10),2);
				break;
			}
			...
			{
				int radius = 0;
				msg_format("Your %s roars extremely loud...", o_name);
				if (!get_aim_dir(&dir)) return;
				fire_ball(GF_METEOR,dir,damroll(15,50),2);
				if (!rand_int(8))
				{
