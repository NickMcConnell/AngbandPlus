...
void do_cmd_eat_food(void)
{
  ...
  if (p_ptr->schange == SHAPE_GOAT) goat = TRUE;
...
void do_cmd_quaff_potion(void)
{
  ...
  switch (o_ptr->sval)
    ...
    case SV_POTION_INVIS:
      {
	int t = 30 + randint(30);
	if (set_tim_invis(p_ptr->tim_invis + t))
	  {
	    ident = TRUE;
	  }
	...
	if (p_ptr->exp < PY_MAX_EXP)
	  {
	    s32b ee = (p_ptr->lev *p_ptr->lev *40);
	    msg_print("You feel more experienced.");
	    gain_exp(ee);
	    p_ptr->tot_exp -= ee;	/* Don't get points for this */
	    ident = TRUE;
	  }
	break;
      }
...
static bool curse_weapon(void)
{
  ...
  if (p_ptr->twoweap && randint(2)) o_ptr = &inventory[INVEN_ARM];
...
void do_cmd_use_staff(void)
{
  ...
  switch (o_ptr->sval)
    ...
    case SV_STAFF_POWER:
      {
	if (dispel_creature(120)) ident = TRUE;
	break;
      }
