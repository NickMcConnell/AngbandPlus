...
{
	/* DragonRiders, or rather, their dragons can eat firestone */
	if (PRACE_FLAG(PR1_TP) && (o_ptr->tval == TV_FIRESTONE)) return (TRUE);
...
void do_cmd_eat_food(void)
{
	...
	/* Ents */
	else if (PRACE_FLAG(PR1_NO_STUN) && PRACE_FLAG(PR1_AC_LEVEL) &&
...
void do_cmd_use_staff(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		{
                        project_hack(GF_HOLY_FIRE, 150 + get_skill_scale(SKILL_MAGIC, 150));
		...
		{
                        if (detect_treasure(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
                        if (detect_objects_gold(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
		...
		{
                        if (detect_objects_normal(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
		...
		{
                        if (detect_traps(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
		...
		{
                        if (detect_doors(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
                        if (detect_stairs(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
		...
		{
                        if (detect_monsters_invis(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
		...
		{
                        if (detect_monsters_evil(DEFAULT_RADIUS + get_skill_scale(SKILL_MAGIC, 30))) ident = TRUE;
		...
		{
                        if (dispel_evil(60 + get_skill_scale(SKILL_MAGIC, 60))) ident = TRUE;
		...
		case SV_STAFF_POWER:
		{
                        if (dispel_monsters(120) + get_skill_scale(SKILL_MAGIC, 120)) ident = TRUE;
...
void do_cmd_aim_wand(void)
{
	...
	switch (sval)
	{
		...
		{
                        if (confuse_monster(dir, 10 + get_skill_scale(SKILL_MAGIC, 90))) ident = TRUE;
		...
		{
                        if (fear_monster(dir, 10 + get_skill_scale(SKILL_MAGIC, 90))) ident = TRUE;
		...
		{
                        if (drain_life(dir, 75 + get_skill_scale(SKILL_MAGIC, 100))) ident = TRUE;
		...
		{
                        fire_cloud(GF_POIS, dir, 12 + get_skill_scale(SKILL_MAGIC, 50), 2, 6);
		...
		{
                        fire_bolt_or_beam(20, GF_MISSILE, dir, damroll(2 + get_skill_scale(SKILL_MAGIC, 10), 6 + get_skill_scale(SKILL_MAGIC, 6)));
		...
		{
                        fire_bolt_or_beam(20, GF_ACID, dir, damroll(3, 8) + get_skill_scale(SKILL_MAGIC, 90));
		...
		{
                        if (charm_monster(dir, 45 + get_skill_scale(SKILL_MAGIC, 100))) ident = TRUE;
		...
		{
                        fire_bolt_or_beam(20, GF_FIRE, dir, damroll(6, 8) + get_skill_scale(SKILL_MAGIC, 110));
		...
		{
                        fire_bolt_or_beam(20, GF_COLD, dir, damroll(3, 8) + get_skill_scale(SKILL_MAGIC, 110));
		...
		{
                        fire_ball(GF_ACID, dir, 60 + get_skill_scale(SKILL_MAGIC, 100), 2);
		...
		{
                        fire_ball(GF_ELEC, dir, 32 + get_skill_scale(SKILL_MAGIC, 150), 2);
		...
		{
                        fire_ball(GF_FIRE, dir, 72 + get_skill_scale(SKILL_MAGIC, 130), 2);
		...
		{
                        fire_ball(GF_COLD, dir, 48 + get_skill_scale(SKILL_MAGIC, 150), 2);
		...
		{
			project_hook(GF_STONE_WALL, dir, 1,
			             PROJECT_BEAM | PROJECT_KILL | PROJECT_GRID);
		...
		{
                        fire_ball(GF_FIRE, dir, 100 + get_skill_scale(SKILL_MAGIC, 220), 3);
		...
		{
                        fire_ball(GF_COLD, dir, 80 + get_skill_scale(SKILL_MAGIC, 220), 3);
		...
		case SV_WAND_DRAGON_BREATH:
		{
			switch (randint(5))
			{
				case 1:
				{
                                        fire_ball(GF_ACID, dir, 100 + get_skill_scale(SKILL_MAGIC, 220), 3);
				...
				case 2:
				{
                                        fire_ball(GF_ELEC, dir, 80 + get_skill_scale(SKILL_MAGIC, 220), 3);
				...
				default:
				{
                                        fire_ball(GF_POIS, dir, 60 + get_skill_scale(SKILL_MAGIC, 220), 3);
		...
		{
                        if (drain_life(dir, 125 + get_skill_scale(SKILL_MAGIC, 200))) ident = TRUE;
		...
		case SV_WAND_ROCKETS:
		{
			msg_print("You launch a rocket!");
                        fire_ball(GF_ROCKET, dir, 75 + (randint(50)) + get_skill_scale(SKILL_MAGIC, 200), 2);
...
void do_cmd_activate(void)
{
	...
	/* Handle special cases -- 1) Mage Staff of Spells */
...
static bool activate_random_artifact(object_type * o_ptr)
{
	...
	/* Paranoia */
	if ((!(o_ptr->art_name)) && (o_ptr->tval != TV_RANDART)) return (FALSE);
