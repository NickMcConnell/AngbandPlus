...
void do_cmd_eat_food(void)
{
	...
	{
		object_aware(o_ptr);
		gain_exp((lev + (p_ptr->level >> 1)) / p_ptr->level);
	}
...
void do_cmd_quaff_potion(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_POTION_LIFE:
		{
			msg_print("Life flows through your body!");
			restore_level();
			hp_player(5000);
			(void)set_poisoned(0);
			(void)set_blind(0);
	...
	if ((p_ptr->prace == RACE_SKELETON) && (randint(12)==1))
	{
		...
		potion_smash_effect(py, px, o_ptr->sval);
	}
...
void do_cmd_read_scroll(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_SCROLL_SUMMON_MONSTER:
		{
			...
			{
				if (summon_specific(py, px, p_ptr->depth + 5, SUMMON_MONSTER, TRUE, FALSE))
				{
					ident = TRUE;
				}
			}
		...
		case SV_SCROLL_SUMMON_UNDEAD:
		{
			for (k = 0; k < randint(3); k++)
			{
				if (summon_specific(py, px, p_ptr->depth + 5, SUMMON_UNDEAD, TRUE, FALSE))
				{
					ident = TRUE;
				}
			}
...
void do_cmd_aim_wand(void)
{
	...
	switch (sval)
	{
		...
		case SV_WAND_DESTROY_MUNDANE:
		{
			fire_ball(GF_DEST_MUND, dir, 75, 5);
			ident = TRUE;
			break;
		}
	}
...
void do_cmd_zap_rod(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		case SV_ROD_DESTROY_MUNDANE:
		{
			fire_ball(GF_DEST_MUND, dir, 50, 4);
			ident = TRUE;
			o_ptr->pval = 50;
			break;
		}
	...
	else if (o_ptr->name1)
	{
		...
		switch (o_ptr->name1)
		{
			...
			case ART_DAWN:
			{
				...
				(void)summon_specific(py, px, MAX_DEPTH, SUMMON_DAWN, TRUE, TRUE);
...
static bool activate_random_artifact(object_type * o_ptr)
{
	...
	switch (o_ptr->xtra2)
	{
		...
		{
			(void)summon_specific(py, px, plev, SUMMON_ANIMAL_RANGER, TRUE, TRUE);
		...
		case ACT_SUMMON_PHANTOM:
		{
			...
			(void)summon_specific(py, px, p_ptr->depth + 5, SUMMON_PHANTOM, TRUE, TRUE);
		...
		{
			msg_print("A thick mist swirls about you...");

			if (!rand_int(3))
			{
				if (summon_specific(py, px, (plev * 3) / 2, SUMMON_ELEMENTAL, TRUE, FALSE))
				{
