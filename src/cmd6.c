...
void do_cmd_read_scroll(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		{
			if (dispel_creature(RF3_UNDEAD, 60)) ident = TRUE;
			break;
		}
...
void do_cmd_use_staff(void)
{
	...
	switch (o_ptr->sval)
	{
		...
		{
			if (dispel_creature(RF3_EVIL, 120)) ident = TRUE;
			k = 3 * p_ptr->lev;
			if (set_protevil(p_ptr->protevil + randint(25) + k)) ident = TRUE;
			if (set_poisoned(0)) ident = TRUE;
			if (set_afraid(0)) ident = TRUE;
	...
	if (o_ptr->name1)
	{
		...
		switch (o_ptr->name1)
		{
			...
			case ART_INGWE:
			{
				...
				dispel_creature(RF3_EVIL, p_ptr->lev * 5);
