	...
	for (i = 1; i < m_max; i++)
	{
		...
		switch (thrown_spell)
		{
			...
			case 96+2:
			{
				if (known)
				{
					if (see_either)
					{
						...
						{
							msg_format("%^s throws a blob of lava.", m_name);
						}
					}
					else
					{
				...
				monst_bolt_monst(m_idx, y, x, GF_LAVA, damroll(8, 8) + (rlev / 3));
				break;
			}
			...
			case 128+9:
			{
				...
				{
					if ((!tr_ptr->flags4 && !tr_ptr->flags5 && !tr_ptr->flags6) || (RF7_RES_MAGIC))
					{
						if (see_both)
						{
			...
			case 128+12:
			{
				...
				{
					mon_take_hit_mon(t_idx, dam, &fear, " is destroyed.");
				}
			...
			case 160+6:
			{
				if (known)
				{
					if (see_either)
					{
						...
						{
							msg_format("%^s gestures volcanicly.", m_name);
						}
					}
					else
					{
				...
				monst_breath_monst(m_idx, y, x, GF_LAVA, randint1(rlev * 4) + 10, 3, FALSE);
			...
			case 160+7:
			{
				if (known)
				{
					if (see_either)
					{
						...
						if (blind) 
						{
							msg_format("%^s stamps on the ground.", m_name);
						}
						else 
						{
							msg_format("%^s creates a tremor.", m_name);
						}
					}
					else
					{
				...
				monst_bolt_monst(m_idx, y, x, GF_QUAKE, damroll(6, 6) + (rlev));
				break;
			}
			...
			case 160+11:
			{
				if (known)
				{
					if (see_either)
					{
						...
						{
							msg_format("%^s smashes at the ground.", m_name);
						}
					}
					else
					{
