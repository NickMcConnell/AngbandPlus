...
static void borg_update_kill_new(int i)
{
	...
	/* Guess at the monster speed and ac DvE*/
	...
	if (r_ptr->flags1 & RF1_UNIQUE)
	{
		...
		if ((r_ptr->level <= b_ptr->depth) && (b_ptr->lev > 9))
		{
			/* Boost bravery */
		...
		for (dx = SCREEN_WID; dx; --dx, ++x, ++g)
		{
			...
			switch (feat)
			{
				...
				case FEAT_PERM_SOLID:
				{
                    ...
                    if (borg_cave_feat[y][x] == FEAT_PERM_INNER) break;
                    ...
                    {
                        borg_cave_feat[y][x] = FEAT_PERM_INNER;
						borg_note("# Found a vault");
                        vault_on_level = TRUE;
                        break;
                    }
                    ...
                    if (feat >= FEAT_WALL_EXTRA &&
                        feat <= FEAT_PERM_EXTRA) break;

                    /* Assume granite */
                    borg_cave_feat[y][x] = FEAT_WALL_EXTRA;
...
void borg_update(void)
{
	...
	if (xb_ptr->old_depth != b_ptr->depth)
	{
        ...
        /* reset our vault check */
        vault_on_level = FALSE;

		/* Reset danger avoidance */
		borg_avoid = 0;

		/* Set borg_avoid_factor DvE */
		...
		/* I didn't close any doors yet */
		track_door_num = 0;
		borg_close_doors = FALSE;
        ...
        /* In town, need to compute best equipment -RML */
        borg_town_need_check_best = TRUE;
	...
	else
	{
        ...
        if ((xb_ptr->old_wx != b_ptr->wx) || (xb_ptr->old_wy != b_ptr->wy))
        {
			borg_note("# Changed panel");
