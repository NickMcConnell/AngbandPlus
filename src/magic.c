  ...
  if (py.flags.blind > 0)
      ...
      else if (result > 0)
	{
	  ...
	  if (randint(100) <= chance) /* changed -CFT */
		...
		case 14: /* Wizlock -DGK */
		 {
		  ...
		  int temp = target_mode;
			/* targetting will screw up get_dir.. -CFT */
		  ...
		  if (get_dir(NULL, &dir))
		   {
		    (void) mmove(dir, &y, &x);
		    if (spikedoor(y,x,py.misc.lev/5))
		     count_msg_print ("The door becomes harder to open...");
		   }
		  target_mode = temp;
		 }
		  break;
		...
		case 24:
		  if (get_dir(NULL, &dir)) {
		     msg_print("A cone of frost appears.");
		     frost_line(dir, char_row, char_col,
			      damroll(4+((py.misc.lev-5)/4),8));
		  }
		  break;
		case 25:
		  if (get_dir(NULL, &dir))
