...
/* cmd-misc.c: miscellaneous commands */
...
static bool do_cmd_tunnel_aux(int y, int x)
{
	...
	else if (cave_feat[y][x] >= FEAT_WALL_EXTRA)
	{
		...
		if ((p_ptr->skill_dig > 40 + rand_int(1600)) && twall(y, x))
		{
			...
			/* Hack -- Make some boulders for giants */
			if (rp_ptr->flags4 & (TR4_THROW_BOULDER))
			{
				object_type object_type_body;
				object_type *i_ptr = &object_type_body;
					
				object_prep(i_ptr, lookup_kind(TV_JUNK, SV_BOULDER));
				i_ptr->number = damroll(2, 3);
				drop_near(i_ptr, -1, y, x);
			}
		}
...
static bool do_cmd_walk_test(int y, int x)
{
	...
	if (cave_mon_trap_bold(y,x)) return (TRUE);
        ...
        /* Big Hack -- require to stay inside dungeon */
        if (x < 0 || y < 0) return (FALSE);
        if (x >= p_ptr->cur_map_wid || y >= p_ptr->cur_map_hgt) return (FALSE);

	/* Require open space (unless immaterial); never allow permanent walls */
	if ((!cave_floor_bold(y, x) && !p_ptr->pass_wall) || (cave_feat[y][x] >= FEAT_PERM_EXTRA &&
...
void do_cmd_fire(void)
{
	...
	{
	    	if ((r_ptr->body.bow_mask & EQUIP_SLOT(i)) && inventory[i].tval)
		{
			nbows++;
			item = i;
		}
	...
	/* Single bow, no need to ask */
	else if (nbows == 1)
	{
	    	j_ptr = &inventory[item];
	}

	/* >=2 bows.  Need to choose one */
	else
	{
	    	q = "Fire from what? ";
		s = "You have nothing to fire with.";
		item_tester_tval = TV_BOW;
		if (!get_item(&item, q, s, USE_EQUIP)) return;
		j_ptr = &inventory[item];
	}

	/* Make sure that the right stats are used */
	analyze_launcher(j_ptr);
	
	/* Use the proper number of shots */
	thits = p_ptr->num_blow[item];
	...
	switch (p_ptr->ammo_tval)
	{
		case TV_SHOT:
		{
			chance = (p_ptr->skill_thb_sling + (bonus * BTH_PLUS_ADJ));
			break;
		}

		case TV_ARROW:
		{
			chance = (p_ptr->skill_thb_bow + (bonus * BTH_PLUS_ADJ));
			break;
		}

		case TV_BOLT:
		{
			chance = (p_ptr->skill_thb_xbow + (bonus * BTH_PLUS_ADJ));
			break;
		}
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			else if (test_hit(chance2, m_ptr->ac, m_ptr->ml))
			{
				bool fear = FALSE;
...
void do_cmd_throw(void)
{
	...
	/* Mega-Hack -- Adjust dice and to-hit for boulders thrown by giants */
	if (rp_ptr->flags4 & (TR4_THROW_BOULDER) && i_ptr->tval == TV_JUNK && i_ptr->sval == SV_BOULDER)
	{
		i_ptr->dd = i_ptr->ds = (p_ptr->lev < 10 ? 5 : p_ptr->lev / 2);
	}
	...
	else
	{
		chance = (3 * p_ptr->skill_tht / 2) + (BTH_PLUS_ADJ * (i_ptr->to_h +
			((i_ptr->tval == TV_JUNK && i_ptr->sval == SV_BOULDER && rp_ptr->flags4 & (TR4_THROW_BOULDER)) ?
				p_ptr->lev : 0)));
	}
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			else if (test_hit((chance2 + sleeping_bonus), m_ptr->ac, m_ptr->ml))
			{
				bool fear = FALSE;
...
/*
 * Incarnate or deincarnate a body (possessors)
 */
void do_cmd_possess(void)
{
	object_type *o_ptr;
	int item, min_value, max_value, i;
	cptr q, s;
	
	/* Hack - only possessors */
	if (p_ptr->prealclass != POSS_CLASS)
	{
		msg_print("You do not have possessing powers.");
		return;
	}
	
	/* If we are in special soul body, incarnate into a corpse */
	if (p_ptr->m_r_idx == POSS_SOUL)
	{
		/* Ask for object */
		q = "Incarnate into which corpse? ";
		s = "You have no corpses to incarnate into.";
		item_tester_tval = TV_CORPSE;
		if (!get_item(&item, q, s, (USE_INVEN | USE_FLOOR))) return;
		
		/* Hack -- remove all items (in particular, light source) */
		for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
		{
			/* Only perma-cursed objects can go here */
			o_ptr = &inventory[i];
			if (o_ptr->k_idx) inven_takeoff(i, 255);
		}
		
		/* Hack -- forget all spells */
		C_WIPE(p_ptr->spell_flags, PY_MAX_SPELLS, byte); 
		C_WIPE(p_ptr->spell_order, PY_MAX_SPELLS, byte); 
		...
		/* Alert player */
		msg_format("You incarnate into %s!", r_name + r_info[p_ptr->m_r_idx].name);

		/* Update bonuses - and get various things from monster body */
	...
	/* Otherwise, deincarnate */
	else
	{
		char out_val[160];
		
		/* We need empty equip to deincarnate */
		for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
		{
			o_ptr = &inventory[i];
			if (o_ptr->k_idx)
			{
				...
				/* Hack -- Perma-cursed objects can remain */
				object_flags(o_ptr, &f1, &f2, &f3, &f4);
				if (f3 & (TR3_PERMA_CURSE))
					continue;
				
				/* Otherwise fail */
				msg_print("You must take off everything to leave your body.");
				return;
			}
		}
		
		/* Ask for confirmation */
		strnfmt(out_val, sizeof(out_val), "Really deincarnate your body of %s? ",
			r_name + r_info[p_ptr->m_r_idx].name);
		message_flush();
		if (!get_check(out_val)) return;
		
		/* Reset body to lost soul */
		p_ptr->m_r_idx = POSS_SOUL;

		/* Reset class to possessor */
		p_ptr->pclass = POSS_CLASS;
		cp_ptr = &c_info[p_ptr->pclass];
		
		msg_print("You deicarnate your body; it quickly decays.");

		/* Hack -- remove perma-curse items */
		for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
		{
			/* Only perma-cursed objects can go here */
			o_ptr = &inventory[i];
			if (o_ptr->k_idx)
			{
				object_desc(out_val, sizeof(out_val), o_ptr, FALSE, 0);
				msg_format("Your accursed %s falls off your former body!", out_val);
				inven_takeoff(i, 255);
			}
		}

		/* Update bonuses - and get various things from possessor soul */
		p_ptr->update |= (PU_BONUS | PU_HP);
		update_stuff();
...
void do_cmd_pet()
{
	...
	/* Show the menu */
	prt("", 1, 20);
	prt("  a) Dismiss pet", 2, 20);
	prt("  b) Dismiss all pets", 3, 20);
	prt("  c) Switch pet/companion status", 4, 20);
	prt("  d) Give target to pet", 5, 20);
	prt("  e) Cancel target", 6, 20);
	prt("  f) Set enabled spells", 7, 20);
	prt("", 8, 20);
	
	/* Ask */
	while (!flag && get_com("(Orders a-f, ESC=exit) Give which order? ", &choice))
	{
		/* Lowercase */
		...
		switch (i)
		{
			...
			{
				/* Target someone */
				if (!target_set_interactive(TARGET_KILL)) return;
				...
				/* XXX We need to be able to target only pets here... */
				...
				/* Delete the monster */
				delete_monster_idx(p_ptr->target_who);
			...
			case 1:
			{
				...
				/* Make sure */
				if (!get_check("Are you sure you want to dismiss all pets? ")) return;
				
				/* Cycle through all monsters */
				for (k = 1; k < mon_max; k++)
				{
					m_ptr = &mon_list[k];
					...
					/* Skip non-pets */
					if (!(m_ptr->align & (AL_PET_MASK))) continue;
					
					/* Skip companions */
					if (m_ptr->mflag & (MFLAG_PERM)) continue;
					
					/* Only targetable monsters in LOS */
					/* if (!player_has_los_bold(m_ptr->fy, m_ptr->fx) || !m_ptr->ml) continue; */
				...
				if (!count)
				{
					msg_print("There are no pets that you can dismiss.");
				}
				else
				{
					msg_format("You dismissed %d pet%s.", count, (count > 1 ? "s" : ""));
				}
			...
			case 2:
			{
				...
				/* Turn companion into a simple pet */
				if (m_ptr->mflag & (MFLAG_PERM))
				{
					/* Make sure */
					if (!get_check(format("Really turn %s from a companion into a simple pet? ", m_name))) return;
				
					m_ptr->mflag &= ~(MFLAG_PERM);
				}
				/* Turn pet into a companion */
				else
				{
					/* Can we have any more? Count existing. */
					
					k = 0;
					
					for (i = 0; i < mon_max; i++)
					{
						monster_type *t_ptr = &mon_list[i];
						if (t_ptr->r_idx && (t_ptr->align & (AL_PET_MASK)) && t_ptr->mflag & (MFLAG_PERM)) k++;
					}
					
					/* Nope. */
					if (k >= adj_chr_comp[p_ptr->stat_ind[A_CHR]])
					{
					...
					/* Alright! */
					if (!get_check(format("Really turn %s into your companion? ", m_name))) return;
					m_ptr->mflag |= (MFLAG_PERM);
					msg_format("%^s becomes your companion.", m_name);
				}
			...
			case 3:
			{
				...
				if (!target_set_interactive(TARGET_KILL | TARGET_GRID)) return;
				if (!p_ptr->target_set) return;
				
				m_ptr->mflag |= (MFLAG_OVER);
				
				/* Attack (or maybe defend?) a monster */
				if (p_ptr->target_who > 0)
				{
					msg_print("(Monster target)");
					t_ptr = &mon_list[p_ptr->target_who];
					m_ptr->target = p_ptr->target_who;
					m_ptr->ty = p_ptr->target_row;
					m_ptr->tx = p_ptr->target_col;
				}
				else if (p_ptr->target_row == p_ptr->py && p_ptr->target_col == p_ptr->px)
				/* Defend player */
				{
					msg_print("(Player target)");
					m_ptr->target = -1;
					m_ptr->tx = p_ptr->px;
					m_ptr->ty = p_ptr->py;
				}
				...
				/* Defend location */
				{
					msg_print("(Grid target)");
					m_ptr->target = 0;
					m_ptr->mflag |= (MFLAG_STAY);
					m_ptr->ty = p_ptr->target_row;
					m_ptr->tx = p_ptr->target_col;
				}
			...
			case 4:
			{
				...
				{
					msg_print("Target cancelled.");
					m_ptr->mflag &= ~(MFLAG_OVER);
					m_ptr->mflag &= ~(MFLAG_STAY);
				}
				else
				{
					msg_print("This pet has no explicitly given target.");
				}
			...
			case 5:
			{
				...
				/* Print orders for the first time */
				screen_save();
				prt("", 1, 20);
				prt(format("  a) Ranged attacks (missiles, breath)        %s",
					(m_ptr->mflag & (MFLAG_NO_RANGED)) ? "disabled" : "enabled "), 2, 20);
				prt(format("  b) Offensive spells (bolts, balls, beams)   %s",
					(m_ptr->mflag & (MFLAG_NO_OFFENSE)) ? "disabled" : "enabled "), 3, 20);
				prt(format("  c) Teleportation spells                     %s",
					(m_ptr->mflag & (MFLAG_NO_TELEPORT)) ? "disabled" : "enabled "), 4, 20);
				prt(format("  d) Spells with a saving throw (scare etc.)  %s",
					(m_ptr->mflag & (MFLAG_NO_SAVING)) ? "disabled" : "enabled "), 5, 20);
				prt(format("  e) Summoning spells                         %s",
					(m_ptr->mflag & (MFLAG_NO_SUMM)) ? "disabled" : "enabled "), 6, 20);
				prt(format("  f) Mass summoning spells                    %s",
					(m_ptr->mflag & (MFLAG_NO_SUMM_MANY)) ? "disabled" : "enabled "), 7, 20);
				prt("", 8, 20);

				/* Ask -- submenu*/
				while (get_com("(Spells type a-f, ESC=continue) Enable/disable which spells? ", &choice2))
				{
					/* Lowercase */
					choice2 = tolower((unsigned char)choice2);
		
					/* Extract request */
					i = (islower((unsigned char)choice2) ? A2I(choice2) : -1);
					...
					switch (i)
					{
						case 0: m_ptr->mflag ^= (MFLAG_NO_RANGED); break;
						case 1: m_ptr->mflag ^= (MFLAG_NO_OFFENSE); break;
						case 2: m_ptr->mflag ^= (MFLAG_NO_TELEPORT); break;
						case 3: m_ptr->mflag ^= (MFLAG_NO_SAVING); break;
						case 4: m_ptr->mflag ^= (MFLAG_NO_SUMM); break;
						case 5: m_ptr->mflag ^= (MFLAG_NO_SUMM_MANY); break;
					}
