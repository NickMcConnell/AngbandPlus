...
static int bow_multiplier(int sval)
{
	switch (sval)
	{
		...
		case SV_LIGHT_RIFLE:
			return (2);
		case SV_HEAVY_RIFLE:
		case SV_LIGHT_SHOTGUN:
			return (3);
		case SV_HEAVY_SHOTGUN:
			return (4);
		default:
...
static s32b artifact_power(int a_idx)
{
	...
	switch (a_ptr->tval)
	{
		...
		case TV_SWORD:
		{
			...
			if (a_ptr->flags1 & TR1_SLAY_AUTOMATA) p = (p * 5) / 4;
...
static void choose_item(int a_idx)
{
	...
	if (r < 5)
	{
		...
		if (r2 < 3) sval = SV_PISTOL;
		else if (r2 < 10) sval = SV_LIGHT_RIFLE;
		else if (r2 < 30) sval = SV_HEAVY_RIFLE;
		else if (r2 < 45) sval = SV_LIGHT_SHOTGUN;
		else sval = SV_HEAVY_SHOTGUN;
	}
	else if (r < 9)
	{
	...
	else if (r < 64)
	{
		...
		else if (r2 < 10) sval = SV_LEATHER_WAISTCOAT;
		else if (r2 < 15) sval = SV_LEATHER_WAISTCOAT_VEST;
		else if (r2 < 20) sval = SV_HEAVY_JACKET;
		else sval = SV_HEAVY_TRENCHCOAT;
	}
	else if (r < 71)
	{
		/* Make shoes. */
	...
	else if (r < 78)
	{
		...
		else if (r2 < 30) sval = SV_SET_OF_SILK_GLOVES;
	...
	if (r < 5)		/* Pick something dependent on item type. */
	{
		...
		switch (a_ptr->tval)
		{
			...
			case TV_SWORD:
			{
				...
				{
					a_ptr->flags1 |= TR1_SLAY_AUTOMATA;
				...
				else if (r < 63)
				{
					...
					if (rand_int(2) == 0) a_ptr->flags1 |= TR1_SLAY_AUTOMATA;
					if (rand_int(2) == 0) a_ptr->flags1 |= TR1_SLAY_GIANT;
				}
				else if (r < 67)
				{
...
{
	int swords = 5, polearms = 5, blunts = 5, guns = 3;
	...
	if (swords > 0 || polearms > 0 || blunts > 0 || guns > 0 ||
