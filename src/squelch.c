...
int add_autoinscription(s16b kind, cptr inscription)
{
	...
	inscriptions[index].inscription_idx = (s16b) quark_add(inscription); /* RHS Max is probably 216 */
 ...
 *
 * These functions have standard argument set although not all arguments are required for all
 * of the functions.
 */
static void quality_display(menu_type *menu, int oid, bool cursor, s16b row, s16b col, s16b width)
{
 ...
 */
static void quality_subdisplay(menu_type *menu, int oid, bool cursor, s16b row, s16b col, s16b width)
{
 ...
 * Handle "Enter".  :(
 */
static bool quality_subaction(char cmd, void *db, int oid) /* TODO Still to be implemented? */
{
 ...
 */
static void sval_display(menu_type *menu, int oid, bool cursor, s16b row, s16b col, s16b width)
{
	char buf[80];
...
static void display_options_item(menu_type *menu, int oid, bool cursor, s16b row, s16b col, s16b width)
{
	size_t line = (size_t) oid;
	UNREFERENCED_PARAMETER(width);
	UNREFERENCED_PARAMETER(menu);
