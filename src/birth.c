...
* Won't apply in Ponyband - delete later
*/
...
static void player_outfit(struct player *p)
{
	...
	if (OPT(adult_thrall))
	object_prep(i_ptr, lookup_kind(TV_FOOD, SV_FOOD_SAAAPPLE), MINIMISE);
	else
	object_prep(i_ptr, lookup_kind(TV_FOOD, SV_FOOD_CARROT), MINIMISE);
		...
		case 2:
			{
				...
				if (!maxed[A_STR] && (stats[A_STR] <= 22 || attempt > 2)) {
					next_stat = A_STR;
				} else if (!maxed[A_DEX] && (stats[A_DEX] <= 22 || attempt > 2)) {
					next_stat = A_DEX;
				} else if (!maxed[A_INT] && mp_ptr->spell_stat != A_INT && (stats[A_INT] <= 22 || attempt > 1)) {
					next_stat = A_INT;
				} else if (!maxed[A_WIS] && mp_ptr->spell_stat != A_WIS && (stats[A_WIS] <= 22 || attempt > 1)) {
					next_stat = A_WIS;
				} else if (!maxed[A_CHR] && (stats[A_CHR] <= 22 || attempt > 0)) {
					next_stat = A_CHR;
				} else {
					step++;
					attempt++;
...
void player_birth(bool quickstart_allowed)
{
	...
	*/
	...
	sprintf(notes_start, "Began the quest to kill Nightmare Moon on %s\n", long_day);
