...
static void get_extra(void)
{
	...
	/* Initialize quest and rewards information -KMW- */
...
static bool get_player_race(void)
{
	...
	for (n = 0; n < MAX_RACES; n++)
	{
		...
		if (n < RACE_VAMPIRE)
			...
			sprintf(buf, "%d", (n - RACE_ZOMBIE)); /* HACK */
		c_put_str(attr, buf, 18 + (n/5), 2 + 15 * (n%5));

		sprintf(buf, "%c %s", p2, str);
		put_str(buf, 18 + (n/5), 3 + 15 * (n%5));
	}
...
static bool get_player_class(void)
{
	...
	for (n = 0; n < MAX_CLASS; n++)
	{
		...
		str = cp_ptr->title;
		attr = cp_ptr->c_att; /* Prfnoff */
		if (!attr) attr = TERM_WHITE;

		sprintf(buf, "%c", I2A(n));
		c_put_str(attr, buf, 19 + (n/3), 2 + 20 * (n%3));

		if (!(rp_ptr->choice & (1L << n)))
			sprintf(buf, "%c (%s)%s", p2, str, mod);
		else
			/* Display */
			sprintf(buf, "%c %s%s", p2, str, mod);

		put_str(buf, 19 + (n/3), 3 + 20 * (n%3));
	}
...
static bool player_birth_aux(void)
{
	...
	{
		/* Temporary buffer */
		(void)strncpy(buf, player_name, i);

		/* Increase number */
		if (!roman_make(k + 1, &buf[i], 32 - i))
		{
			(void)strcpy(player_name, buf);
		}
	}
