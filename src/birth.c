...
static bool player_birth_aux_1(void)
{
	...
	if (leader == NO_LEADER)	/* Ask for altering options for */
	{				/* the first character only -IB */
		...
		for (i = OPT_BIRTH; i < OPT_CHEAT; i++)
		{
			...
			/* All characters start with the same options -IB */
		do_cmd_options_sync(0);
	}
