...
static void wiz_display_item(object_type *o_ptr)
{
	...
	prt("+------------FLAGS3------------+", 10, j+32);
	prt("fe s    ehsi  st    iiiiadta  hp", 11, j+32);
	prt("il e n taihnf ee    ggggcregb vr", 12, j+32);
	prt("reslnowysdose eld   nnnntalrl ym", 13, j+32);
	prt("ecpfomrcyewta ieirmsrrrriieaeccc", 14, j+32);
	prt("aaictaauktmatlnpgeihaefcvnpvsuuu", 15, j+32);
	prt("uunuegirnyoahivaeggoclioaeoasrrr", 16, j+32);
	prt("rrerlitsopdretitsehtierltxrtesss", 17, j+32);
	prt("aassechewestreshtntsdcedeptedeee", 18, j+32);
...
void do_cmd_debug(void)
{
	...
	switch (cmd)
	{
		...
		/* Hack -- Help */
		case '?': do_cmd_help("wizcmd.txt"); break;
		...
		case 'C': wiz_create_named_art(command_arg); break;
		...
		/* Lose Mutation */
		case 'L': (void)lose_mutation(command_arg); break;

		/* Magic Mapping */
		case 'm': map_area(); break;

		/* Mutation */
		case 'M': (void)gain_random_mutation(command_arg); break;

		/* Specific reward */
		case 'r': (void)gain_level_reward(command_arg); break;

		/* Summon _friendly_ named monster */
		case 'N': do_cmd_wiz_named_friendly(command_arg, TRUE); break;
		...
		/* Hack -- whatever I desire (probing) */
