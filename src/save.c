...
void wr_player_hp(void)
{
	...
	for (i = 0; i < PY_MAX_LEVEL; i++)
		wr_s16b(0); // stored in pc_array now!
}
...
// Writes player class info. Changing PY_MAX_CLASSES or PY_MAX_LEVEL will break savefiles!
void wr_classes(void)
{
	size_t i, j;
	for (i = 0; i < PY_MAX_CLASSES; i++)
	{
		wr_byte(pc_array[i].hitdie);
		wr_byte(pc_array[i].expfact);
		wr_s16b(pc_array[i].max_lev);
		wr_s16b(pc_array[i].lev);
		wr_s32b(pc_array[i].max_exp);
		wr_s32b(pc_array[i].exp);
		wr_u16b(pc_array[i].exp_frac);
		wr_s16b(pc_array[i].mhp);
		wr_s16b(pc_array[i].msp);
		for (j = 0; j < PY_MAX_LEVEL; j++)
			wr_s16b(pc_array[i].player_hp[j]);
	}
}