...
#define WRITE_FN wr_long
  ...
  wr_byte(f_ptr->fear_resist);

  wr_short((int16u)missile_ctr);
  wr_long((int32u)turn);
  if (using_quiver) /* Patch inven_ctr and equip_ctr -JLS */
  {
    if (inven_ctr < INVEN_QUIVER) /* Need to move enquivered item to the
				     bottom of the pack if 'a' to 'v' are
				     not all in use -JLS */
      inventory[inven_ctr] = inventory[INVEN_QUIVER];
    inven_ctr++;
    equip_ctr--; /* Make quiver seem to be in pack -JLS */
  }
...
#define READ_FN rd_long
      ...
      if ((l & 0x80000000L) == 0)
	{
	  ...
	  { /* Patch inven_ctr and equip_ctr so the enquivered ammo is
	       at the bottom of the pack in the savefile if the quiver
	       is in use. -JLS */
	    inven_ctr--;
	    equip_ctr++; /* Patch ctrs so quiver is in equip */
	    if (inven_ctr < INVEN_QUIVER)
/* If not counting the enquivered item (ie patched inven_ctr) and there are
 less than INVEN_QUIVER items then the enquivered item is not in the proper
 slot!  We must fix this. -JLS */
	    {
