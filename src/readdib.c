...
static HPALETTE PASCAL NEAR MakeDIBPalette(LPBITMAPINFOHEADER lpInfo)
{
	...
	{
		npPal =
			(NPLOGPALETTE) LocalAlloc(LMEM_FIXED,
			sizeof(LOGPALETTE) +
	...
	HPALETTE * phPal, HBITMAP * phBitmap)
{
	...
	if ((hPalette = MakeDIBPalette(lpInfo)) != 0)
	{
		...
		lpBits =
			((LPSTR) lpInfo + (WORD) lpInfo->biSize +
			(WORD) lpInfo->biClrUsed * sizeof(RGBQUAD));
		hBitmap =
			CreateDIBitmap(hDC, lpInfo, CBM_INIT, lpBits,
...
BOOL ReadDIB(HWND hWnd, LPSTR lpFileName, DIBINIT * pInfo)
{
	...
	pInfo->hDIB =
		GlobalReAlloc(pInfo->hDIB,
		lpbi->biSize + nNumColors * sizeof(RGBQUAD) + lpbi->biSizeImage,
		0);
	...
	if (!bCoreHead)
	{
		_lread(fh, (LPSTR) (lpbi) + lpbi->biSize,
