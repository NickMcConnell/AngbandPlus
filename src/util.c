...
static char roguelike_commands(char command)
{
	...
	switch (command)
	{
		...
		/* Toggle shouting */
		/* This is a really stupid, arbitrary choice */
		case KTRL('Z'): return ('#');
...
byte message_prior(s16b age) {
  ...
  /* Forgotten */
  if ((age < 0) || (age >= message_num())) return MSG_NORMAL;
...
void mprint(byte priority, cptr msg)
{
	...
	if (character_generated) message_add(msg, priority);
