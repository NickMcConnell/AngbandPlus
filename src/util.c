...
static struct roman_digit roman_table[] =
{
	{ "I",  1    },
	{ "IV", 4    },
	{ "V",  5    },
...
s16b roman_extract(cptr buf, int *spacer)
{
	int prev_val, this_val;
	s16b value;
	int i, j;

	for (i = strlen(buf); i-- > 0; /* do nothing */)
	{
		if (buf[i] == ' ') break;
	}
	*spacer = i + 1;
	...
	for (prev_val = 0, value = 0; buf[++i]; /* do nothing */)
	{
		for (j = 0; roman_table[j].r_dig; j++)
		{
			if (roman_table[j].r_dig[0] == buf[i]) break;
		}
		this_val = roman_table[j].r_val;
		if (!this_val) return (-1);
...
errr roman_make(s16b value, char *buf, int buf_len)
{
	int i, j;
	...
	*buf = '\0';
	for (i = 0; j-- > 0; /* do nothing */)
	{
		while (value >= roman_table[j].r_val)
		{
			i += strlen(roman_table[j].r_dig);
			if ((i + 1) > buf_len) return (1);
			(void)strcat(buf, roman_table[j].r_dig);
			value -= roman_table[j].r_val;
		}
	}
