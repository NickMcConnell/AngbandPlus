 ...
 *
 * The messages are stored in a special manner that maximises "efficiency",
 * that is, we attempt to maximise the number of semi-sequential messages
...
void pause_line(int row)
{
	int i;
	prt("", row, 0);
	put_str("[Press any key to continue]", row, (screen_x-28)/2);
	i = inkey();
	prt("", row, 0);
}
