...
void user_name(char *buf, size_t len, int id)
{
	...
	if ((pw = getpwuid(id)))
	{
		...
		if (islower((unsigned char)buf[0])) buf[0] = toupper((unsigned char)buf[0]);
 ...
 *
 * Hack -- Note that "message_flush" will clear the top line even if no
 ...
 */
void message(u16b message_type, s16b extra, cptr msg)
{
	if (extra != -1) sound(message_type);

	msg_print_aux(message_type, msg);
}
...
s16b get_quantity(cptr prompt, int max)
{
	...
	else if (max != 1)
	{
		...
		/* Ask for a quantity */
		if (!get_string(prompt, buf, sizeof(buf))) return (0);
...
void request_command(bool shopping)
{
	...
	while (TRUE)
	{
		...
		if (act && !inkey_next)
		{
			...
			strnfmt(request_command_buffer, sizeof(request_command_buffer), "%s", act);
