...
errr init_ami(void)
{
	...
	for (i = 0; i < 32; i++)
	{
		...
		if (color_table[i][0] == 0 && color_table[i][1] == 0 &&
			color_table[i][2] == 0 && color_table[i][3] == 0)
		{
			color_table[i][0] = 1;
	...
	if (strlen(modestr) > 0)
	{
		...
		if (scr_m == 0)
		{
			...
			{
				FAIL
					("Public screen can only be used on kickstart 3.0 or later.");
			}
			...
			if (!pubscr)
			{
				sprintf(tmpstr, "Unable to get a lock on screen '%s'.",
					modestr);
				FAIL(tmpstr);
			}
			...
			{
				pen =
					ObtainBestPen(pubscr->ViewPort.ColorMap,
					color_table[i][1] << 24, color_table[i][2] << 24,
					color_table[i][3] << 24, OBP_Precision,
					PRECISION_EXACT);
				if (pen == -1)
				{
					FAIL
						("Unable to obtain suitable pens to use on public screen. ");
				}
	...
	{
		scr_m =
			BestModeID(BIDTAG_NominalWidth, ts->ww, BIDTAG_NominalHeight,
			ts->wh, BIDTAG_DesiredWidth, ts->ww, BIDTAG_DesiredHeight,
			ts->wh, BIDTAG_Depth, 4, TAG_END);
	}
	...
	if (!use_pub)
	{
		...
		if ((amiscr =
				OpenScreenTags(NULL, SA_Width, ts->ww, SA_Height, ts->wh,
			  SA_Depth, deep ? 5 : 4, SA_DisplayID, scr_m, SA_Font,
					scrattr, SA_Type, CUSTOMSCREEN, SA_Title,
					"Angband Screen", SA_ShowTitle, FALSE, SA_Quiet, TRUE,
					SA_Behind, TRUE, SA_AutoScroll, TRUE, SA_Overscan,
					OSCAN_TEXT, v39 ? SA_Interleaved : TAG_IGNORE, TRUE,
					TAG_END)) == NULL)
		{
	...
	else
	{
		...
		{
			sprintf(tmpstr,
				"Public screen is too small for window (%d x %d).", maxw,
				maxh);
			FAIL(tmpstr);
		}
	...
	{
		if ((visinfo =
				GetVisualInfo(use_pub ? pubscr : amiscr, TAG_END)) == NULL)
		{
			use_menus = FALSE;
		}
	}
	...
	if ((ts->win =
			OpenWindowTags(NULL, WA_Left, ts->wx, WA_Top, ts->wy,
		 WA_InnerWidth, ts->ww, WA_InnerHeight, ts->wh,
				use_pub ? WA_PubScreen : WA_CustomScreen,
				use_pub ? pubscr : amiscr, WA_Backdrop, backdrop,
				WA_Borderless, backdrop, WA_GimmeZeroZero, !backdrop,
				WA_DragBar, !backdrop &&
				!ts->notitle, WA_DepthGadget, !backdrop &&
				!ts->notitle, WA_NewLookMenus, TRUE,
				backdrop ? TAG_IGNORE : WA_ScreenTitle, VERSION, (backdrop
					|| ts->notitle) ? TAG_IGNORE : WA_Title, ts->name,
				WA_Activate, TRUE, WA_RMBTrap, !use_menus, WA_ReportMouse,
				TRUE, WA_IDCMP,
	...
	if (tm->use)
	{
		/* Open mirror window */
		if ((tm->win =
				OpenWindowTags(NULL, WA_Left, tm->wx, WA_Top, tm->wy,
			 WA_InnerWidth, tm->ww, WA_InnerHeight, tm->wh, WA_PubScreen,
					pubscr, WA_GimmeZeroZero, TRUE, WA_DragBar,
					!tm->notitle, WA_DepthGadget, !tm->notitle,
					WA_NewLookMenus, TRUE, WA_ScreenTitle, VERSION,
					tm->notitle ? TAG_IGNORE : WA_Title, tm->name,
					WA_ReportMouse, TRUE, TAG_END)) == NULL)
		{
	...
	if (tr->use)
	{
		/* Open recall window */
		if ((tr->win =
				OpenWindowTags(NULL, WA_Left, tr->wx, WA_Top, tr->wy,
			 WA_InnerWidth, tr->ww, WA_InnerHeight, tr->wh, WA_PubScreen,
					pubscr, WA_GimmeZeroZero, TRUE, WA_DragBar,
					!tr->notitle, WA_DepthGadget, !tr->notitle,
					WA_NewLookMenus, TRUE, WA_ScreenTitle, VERSION,
	...
	if (tc->use)
	{
		/* Open choice window */
		if ((tc->win =
				OpenWindowTags(NULL, WA_Left, tc->wx, WA_Top, tc->wy,
			 WA_InnerWidth, tc->ww, WA_InnerHeight, tc->wh, WA_PubScreen,
					pubscr, WA_GimmeZeroZero, TRUE, WA_DragBar,
					!tc->notitle, WA_DepthGadget, !tc->notitle,
					WA_NewLookMenus, TRUE, WA_ScreenTitle, VERSION,
	...
	if (tc->use)
		term_choice = link_term(tc);
	if (tr->use)
		term_recall = link_term(tr);
	if (tm->use)
		term_mirror = link_term(tm);
	if (ts->use)
	...
	if (use_graphics)
	{
		...
		if (use_pub ||
			(depth_of_bitmap(ts->rp->BitMap) !=
	 depth_of_bitmap(ts->gfxbm)))
		{
...
static int yesno(char *str)
{
	...
	return (!strcmp(tmp, "y") || !strcmp(tmp, "yes") ||
		!strcmp(tmp, "true") || !strcmp(tmp, "on"));
}
	...
	if (AslBase = OpenLibrary("asl.library", 37))
	{
		...
		if (req = AllocAslRequestTags(ASL_FontRequest, TAG_DONE))
		{
			...
			if (AslRequestTags(req, ASLFO_FixedWidthOnly, TRUE, TAG_DONE))
			{
				...
				sprintf(str, "%s/%d", req->fo_Attr.ta_Name,
...
int read_prefs(void)
{
	...
	while (FGets(file, line, 256))
	{
		...
		else if (!strncmp(tmp, "font", 4))
		{
			...
			attr.ta_Flags = (!strcmp(fname, "topaz.font") && (fsize == 8 ||
					fsize == 9)) ? FPF_ROMFONT : FPF_DISKFONT;
...
static errr amiga_event(int v)
{
	...
	if ((imsg =
			(struct IntuiMessage *) GetMsg(screen.win->UserPort)) == NULL)
	{
...
int amiga_tomb(void)
{
	...
	if ((filebm =
			alloc_bitmap(512, 168, 4, BMF_CLEAR,
	...
	if (depth != 4 || use_pub)
	{
		...
		if ((convbm =
				alloc_bitmap(512, 168, depth, BMF_CLEAR,
			  screen.rp->BitMap)) == NULL)
		{
			free_bitmap(filebm);
			return (FALSE);
		...
		else
		{
			...
			remap_bitmap(filebm, convbm, use_pub ? pubpens : stdpens, 512,
				168);
		}
	...
	if ((scalbm =
			alloc_bitmap(tw, th, depth, BMF_CLEAR,
		  screen.rp->BitMap)) == NULL)
	{
		free_bitmap(convbm);
		return (FALSE);
	...
	BltBitMapRastPort(scalbm, 0, 0, screen.rp, (screen.ww - tw) / 2, 0, tw,
		th, 0xc0);
...
void handle_rawkey(UWORD code, UWORD qual, APTR addr)
{
	...
	if ((qual & IEQUALIFIER_NUMERICPAD) && (qual & 0xff))
	{
		...
		if ((code >= 0x1d && code <= 0x1f) || (code == 0x2d ||
				code == 0x2f) || (code >= 0x3d && code <= 0x3f))
		{
...
static void cursor_off(term_data * td)
{
	if (td->cursor_lit && !iconified)
	{
		...
		if (CUR_A & 0xf0 && use_graphics)
		{
			put_gfx(td->wrp, td->cursor_xpos, td->cursor_ypos, CUR_C,
				CUR_A);
		}
...
static void cursor_anim(void)
{
	...
	if (td->cursor_map)
	{
		...
		{
			SetAPen(td->wrp, PEN(CURSOR_PEN));
			RectFill(td->wrp, td->map_x + i * td->mpt_w,
				td->map_y + j * td->mpt_h,
	...
	else if (td->cursor_visible && !iconified)
	{
		...
		else
		{
			SetAPen(td->wrp, PEN(CUR_A & 0x0f));
			SetBPen(td->wrp,
				(td->cursor_frame < 4) ? PEN(CURSOR_PEN) : PEN(0));
			Move(td->wrp, td->fw * td->cursor_xpos,
				td->fh * td->cursor_ypos + td->fb);
			Text(td->wrp, &CUR_C, 1);
		}
...
int load_gfx(void)
{
	...
	/* Allocate bitmaps */
	if ((ts->gfxbm =
			alloc_bitmap(GFXW, GFXH, 4, BMF_CLEAR,
	   ts->rp->BitMap)) == NULL) return (FALSE);
	if ((ts->mskbm =
			alloc_bitmap(GFXW, GFXH, 1, BMF_CLEAR,
	   ts->rp->BitMap)) == NULL) return (FALSE);
...
int conv_gfx(void)
{
	...
	else
	{
		...
		remap_bitmap(ts->gfxbm, tmpbm, use_pub ? pubpens : stdpens, GFXW,
			GFXH);
	}
...
int size_gfx(term_data * td)
{
	...
	if ((td->mapbm =
			alloc_bitmap(td->map_w, td->map_h, depth, BMF_CLEAR,
	   sbm)) == NULL) return (FALSE);
	scale_bitmap(ts->gfxbm, GFXW, GFXH, td->mapbm, td->map_w, td->map_h);

	/* Scale tile graphics */
	depth = depth_of_bitmap(ts->gfxbm);
	if ((tmpbm =
			alloc_bitmap(td->gfx_w, td->gfx_h, depth, BMF_CLEAR,
		   sbm)) == NULL) return (FALSE);
...
static void put_gfx(struct RastPort *rp, int x, int y, int chr, int col)
{
	...
	if (col & 0x40)
	{
		SetAPen(rp, PEN(0));
		RectFill(rp, x0, y0, x1, y1);
		BltMaskBitMapRastPort(td->gfxbm, c * fw, a * fh, rp, x0, y0, fw,
			fh, (ABC | ANBC | ABNC), td->mskbm->Planes[0]);
	}

	/* Draw full tile */
	else
	{
		BltBitMapRastPort(td->gfxbm, c * fw, a * fh, rp, x0, y0, fw, fh,
			0xc0);
	}
}
...
void load_palette(void)
{
	...
	if (v39)
	{
		...
		{
			palette32[i * 3 + 1] =
				color_table[use_graphics ? i : i + 16][1] << 24;
			palette32[i * 3 + 2] =
				color_table[use_graphics ? i : i + 16][2] << 24;
			palette32[i * 3 + 3] =
				color_table[use_graphics ? i : i + 16][3] << 24;
		}
	...
	else
	{
		...
		{
			palette4[i] =
				(color_table[use_graphics ? i : i + 16][1] >> 4) << 8;
			palette4[i] |=
				(color_table[use_graphics ? i : i + 16][2] >> 4) << 4;
			palette4[i] |=
				(color_table[use_graphics ? i : i + 16][3] >> 4);
		}
...
int create_menus(void)
{
	...
	if (menu)
	{
		...
		if (LayoutMenus(menu, visinfo,
				v39 ? GTMN_NewLookMenus : TAG_IGNORE, TRUE, TAG_END))
		{
...
//   ClearMenuStrip( screen.win );
	...
	if (item = ItemAddress(menu, FULLMENUNUM(6, 7, 0)))
	{
		item->Flags =
			use_graphics ? item->
			Flags | ITEMENABLED : item->Flags & ~ITEMENABLED;
	}
...
static void play_sound(int v)
{
	...
	if (has_sound)
	{
		...
		if (snd->Address)
		{
			...
			PlaySound(snd->Address, snd->Volume, channel, rate,
				snd->Repeats);
		}
...
static void put_gfx_map(term_data * td, int x, int y, int c, int a)
{
	BltBitMapRastPort(td->mapbm, c * td->mpt_w, a * td->mpt_h, td->wrp,
		td->map_x + x * td->mpt_w, td->map_y + y * td->mpt_h, td->mpt_w,
		td->mpt_h, 0xc0);
}
...
void free_bitmap(struct BitMap *bitmap)
{
	...
	else
	{
		...
		{
			FreeRaster(bitmap->Planes[p], bitmap->BytesPerRow * 8,
				bitmap->Rows);
		}
		/* Free bitmap structure */
		FreeMem(bitmap, sizeof(struct BitMap));
