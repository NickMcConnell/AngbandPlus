...
void handle_menupick( int mnum )
{
   ...
   while ( mnum != MENUNULL )
   {
      ...
      else if ( ud >= MNU_OPTION )
      {
         ...
         *options[ i ].variable = item->Flags & CHECKED ? TRUE : FALSE;
...
int create_menus( void )
{
   ...
   /* Find next option */
   for ( opt = options, o = 0; opt->descr; opt++, o++ )
   {
      /* Cheating options are skipped */
      if ( opt->page > 6 ) continue;
      ...
      item->nm_Label = (STRPTR) opt->descr;
...
//   ClearMenuStrip( screen.win );
   ...
   /* Find next option */
   for ( i = 0; item && options[ i ].descr; i++ )
   {
      /* Did we find it? */
      if ( item )
      {
