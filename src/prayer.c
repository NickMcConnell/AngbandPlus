  ...
  if (py.flags.blind > 0)
      ...
      else if (result > 0)
	{
	  ...
	  if (randint(100) <= chance) /* changed -CFT */
		...
		case 6:
		  if (get_dir(NULL, &dir))
		   fire_bolt(GF_FORCE, dir, char_row, char_col,
			     damroll(2, 5)+py.misc.lev/2, spell_names[5]);
		  break;
		case 7:
		  (void) detect_sdoor();
		  (void) detect_trap();
		...
		case 42: /* shielding */
		  ...
		  calc_bonuses();
		  prt_pac(); 
		  msg_print("You are surrounded by a glowing nimbus of light!");
		  break;
		case 43: /* banishment */
		  if (banish_creature(EVIL, 100))
		    msg_print("The power of your god banishes the creatures!");
		  break;
		case 44: /* word of destruction */
		  destroy_area(char_row, char_col);
		  break;
 		case 45: /* invulnerability */
		...
		case 50: /* enchant weapon */
		  i_ptr = &inventory[(using_quiver &&
				  inventory[INVEN_WIELD].tval==TV_NOTHING) ?
				  INVEN_QUIVER : INVEN_WIELD];
