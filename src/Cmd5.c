...
static bool item_tester_hook_hellqueen_book_browsable(object_type *o_ptr)
{
        if ((o_ptr->tval >= 111) && (o_ptr->tval <= 125)) return (TRUE);
...
void do_cmd_browse(void)
{
	...
	/* Restrict choices to "useful" books */
        ...
        {
                item_tester_hook = item_tester_hook_hellqueen_book_browsable;
        }
        else item_tester_tval = mp_ptr->spell_book;
                ...
                for (spell = 0; spell < 64; spell++)
		{
                        ...
                        if ((fake_spell_flags[o_ptr->tval - TV_VALARIN_BOOK + 1][sval][(spell < 32)] & (1L << (spell % 32))))
			{
				/* Skip non "okay" prayers */
                                if (!spell_okay(spell, FALSE, o_ptr->tval - TV_VALARIN_BOOK + 1) || p_ptr->pclass != CLASS_HELLQUEEN) continue;
...
static bool item_tester_hook_hellqueen_book(object_type *o_ptr)
{
        if ((o_ptr->tval >= 111) && (o_ptr->tval <= 125)) return (TRUE);
...
void do_cmd_cast(void)
{
	...
	/* Restrict choices to spell books */
        if (p_ptr->pclass == CLASS_HELLQUEEN)
        {
                item_tester_hook = item_tester_hook_hellqueen_book;
        }
        else item_tester_tval = mp_ptr->spell_book;
	...
	else
	{
                ...
                if ((!(spell_worked[realm][(spell < 32)] & (1L << (spell % 32)))) && (p_ptr->pclass != CLASS_SORCERER))
		{
			...
			/* Gain experience */
                        if (p_ptr->pclass != CLASS_HELLQUEEN) gain_exp(e * s_ptr->slevel);
		}
	}
...
void do_cmd_lite_charm()
{
        ...
        else msg_print("You need at least 3 mana points!");
        update_and_handle();
}

void do_cmd_dragon_charm()
{
        int     dir, beam;
        int     plev = p_ptr->lev;
        int to_s2=p_ptr->to_s/2;
        ...
        beam = plev / 2;
        if (!get_aim_dir(&dir)) return;
        fire_bolt(GF_DRAGON_CONTROL, dir, 0);
}
...
void do_cmd_bless_weapon(void)
{
                ...
                apply_magic(o_ptr, p_ptr->lev, TRUE, TRUE, TRUE);
                ...
                /* Devaluate the item to prevent cheating */
                k_ptr->cost = 0;
                }
...
void do_cmd_divine_bolt()
{
        ...
        beam = plev / 2;
        if (!get_aim_dir(&dir)) return;
        fire_bolt(GF_LITE, dir, 15 * p_ptr->lev * 3);
        p_ptr->csp -= 20;
        energy_use = 100;
        }
...
void wave_kick()
{
               int dir;
               if (!get_aim_dir(&dir)) return;
               fire_bolt_or_beam(p_ptr->lev, GF_MANA, dir,
                                 damroll(p_ptr->to_d * 10, 1));
...
void skatter_enchant_bow()
{
                ...
                /* This is to prevent players to sell this bow */
                o_ptr->ident |= (IDENT_BROKEN);
                /* Devaluate the bow to prevent cheating */
                k_ptr->cost = 0;
                }
...
void skatter_exploding_arrow()
{
                ...
                if (!get_aim_dir(&dir)) return;
                fire_ball(GF_MANA, dir, arrowpower * (p_ptr->lev / 5), 2);
...
void hellqueen_mana_blast()
{
        ...
        if (!get_aim_dir(&dir)) return;
        fire_beam(GF_MANA, dir, beampower);
        p_ptr->csp -= beampower;
        energy_use = 100;
}
...
void special_weapon_charge()
{
                ...
                object_flags(o_ptr, &f1, &f2, &f3, &f4);
                if (f4 & (TR4_CHARGEABLE) || ability(38))
                {
                        while (TRUE)
			{
                                if (!get_com("Do what? [N]ame item, [T]weak, [L]ist of tweaking prices", &ch))
				{
					amber_power = 0;
					break;
				}
...
void repair_weapon()
{
                ...
                if (o_ptr->pval3 < 20 && (o_ptr->tval == TV_SWORD || o_ptr->tval == TV_HAFTED || o_ptr->tval == TV_POLEARM
                 || o_ptr->tval == TV_MSTAFF || o_ptr->tval == TV_ROD || o_ptr->tval == TV_SWORD_DEVASTATION
                 || o_ptr->tval == TV_HELL_STAFF || o_ptr->tval == TV_VALKYRIE_SPEAR))
                {
