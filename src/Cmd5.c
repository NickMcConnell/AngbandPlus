...
while (effects-- && more_effects)
    {
        ...
        case 8: /* Purposedly "leaks" into default */
            ...
            if (randint(6)==1)
            {
                ...
                take_hit(damroll(randint(p_ptr->lev),p_ptr->lev), "a lethal chaos feature");
            }
            /* No break; here! */
        default:
...
void do_cmd_cast(void)
{
	...
	energy_use = 100+(s_ptr->slevel-p_ptr->lev);
	if(energy_use < 1) energy_use = 1;
	/* Sufficient mana */
	if (s_ptr->smana <= p_ptr->csp)
	{
		/* Use some mana */
...
{
    msg_print
    ("Praying is not used in Cthangband. Use magic spell casting instead.");
}
...
void do_cmd_mindcraft(void)
{
	...
	/* Take a turn */
	energy_use = 100; /* set cost as these need concentration */
