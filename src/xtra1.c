...
static void calc_bonuses(void)
{
	...
	/* Reset metapsionics level */
	p_ptr->meta_psi_lev = 0;
	...
	if (f2 & (TR2_RES_DISEN)) p_ptr->resist_disen = TRUE;
        if (f2 & (TR2_RES_PSI)) p_ptr->resist_psi = TRUE;
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
	{
		...
		if (f2 & (TR2_RES_DISEN)) p_ptr->resist_disen = TRUE;
                ...
                /* Calculate metapsionics level */
                p_ptr->meta_psi_lev = MAX(p_ptr->meta_psi_lev, ((f3 & TR3_META_PSI1) ? 1: 0) +
                ((f3 & TR3_META_PSI2) ? 2 : 0));
	...
	/* Take note when "inhibit psi" changes */
	if (p_ptr->old_inhibit_psi != p_ptr->inhibit_psi)
	{
		/* Message */
		if (p_ptr->inhibit_psi)
		{
			msg_print("Your equipment clouds your mind.");
		}
		else
		{
			msg_print("Your mind feels less clouded.");
		}
