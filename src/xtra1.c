...
static void prt_title(void)
{
	...
	/* Winner */
	else if (total_winner || (p_ptr->level > PY_MAX_LEVEL))
	{
		p = "***WINNER***";
	}
	...
	{
		p = player_title[p_ptr->pclass][(p_ptr->level-1)/5];
...
static void calc_spells(void)
{
	...
	levels = p_ptr->level - mp_ptr->spell_first + 1;
	...
	for (i = 63; i >= 0; i--)
	{
		...
		if (s_ptr->slevel <= p_ptr->level) continue;
...
static void calc_hitpoints(void)
{
	...
	/* Calculate hitpoints */
	mhp = player_hp[p_ptr->level-1] + (bonus * p_ptr->level / 2);

	/* Always have at least one hitpoint per level */
	if (mhp < p_ptr->level + 1) mhp = p_ptr->level + 1;
...
static void calc_torch(void)
{
	...
	/* Vampires have 'dark' light */
	if (p_ptr->prace == RACE_VAMPIRE)
	{
		p_ptr->cur_lite = MIN(3, (p_ptr->level / 5) + 1);
	}
	/* Fiends have good night vision */
	else if (p_ptr->prace == RACE_FIEND)
	{
		p_ptr->cur_lite = MIN(3, (p_ptr->level / 13) + 1);
	}
	/* Assume no light */
	else
	{
...
static void calc_bonuses(void)
{
	...
	{
		case CLASS_WARRIOR:
			if (p_ptr->level > 29) p_ptr->resist_fear = TRUE;
			break;
		case CLASS_PALADIN:
			if (p_ptr->level > 39) p_ptr->resist_fear = TRUE;
			break;
		case CLASS_CHAOS_WARRIOR:
			if (p_ptr->level > 29) p_ptr->resist_chaos = TRUE;
			if (p_ptr->level > 39) p_ptr->resist_fear = TRUE;
			break;
		case CLASS_MINDCRAFTER:
			if (p_ptr->level >  9) p_ptr->resist_fear = TRUE;
			if (p_ptr->level > 19) p_ptr->sustain_wis = TRUE;
			if (p_ptr->level > 29) p_ptr->resist_conf = TRUE;
			if (p_ptr->level > 39) p_ptr->telepathy = TRUE;
			break;
		case CLASS_MONK:
	...
	switch (p_ptr->prace)
	{
		...
		case RACE_YEEK:
			...
			if (p_ptr->level > 19) p_ptr->immune_acid = TRUE;
			break;
		case RACE_KLACKON:
		...
		case RACE_DARK_ELF:
			p_ptr->resist_dark = TRUE;
			if (p_ptr->level > 19) p_ptr->see_inv = TRUE;
			break;
		case RACE_DRACONIAN:
			p_ptr->ffall = TRUE;
			if (p_ptr->level >  4) p_ptr->resist_fire = TRUE;
			if (p_ptr->level >  9) p_ptr->resist_cold = TRUE;
			if (p_ptr->level > 14) p_ptr->resist_acid = TRUE;
			if (p_ptr->level > 19) p_ptr->resist_elec = TRUE;
			if (p_ptr->level > 34) p_ptr->resist_pois = TRUE;
			break;
		case RACE_MIND_FLAYER:
			p_ptr->sustain_int = TRUE;
			p_ptr->sustain_wis = TRUE;
			if (p_ptr->level > 14) p_ptr->see_inv = TRUE;
			if (p_ptr->level > 29) p_ptr->telepathy = TRUE;
			break;
		case RACE_IMP:
			p_ptr->resist_fire = TRUE;
			if (p_ptr->level > 9) p_ptr->see_inv = TRUE;
			break;
		case RACE_GOLEM:
			p_ptr->slow_digest = TRUE;
			p_ptr->free_act = TRUE;
			...
			p_ptr->resist_fear = TRUE;
			p_ptr->resist_conf = TRUE;
			if (p_ptr->level > 34) p_ptr->hold_life = TRUE;
			break;
		case RACE_SKELETON:
			p_ptr->resist_shard = TRUE;
		...
		case RACE_VAMPIRE:
			...
			p_ptr->resist_pois = TRUE;
			p_ptr->lite = TRUE;
			if (p_ptr->level >= 10) p_ptr->see_inv = TRUE;
			if (p_ptr->level >= 25)
			{
				p_ptr->regenerate = TRUE;
				p_ptr->free_act = TRUE;
		...
		case RACE_SPECTRE:
			...
			p_ptr->slow_digest = TRUE;
			p_ptr->resist_cold = TRUE;
			if (p_ptr->level > 34) p_ptr->telepathy = TRUE;
			break;
		case RACE_SPRITE:
			p_ptr->ffall = TRUE;
			p_ptr->resist_lite = TRUE;
		...
		case RACE_FIEND:
			...
			p_ptr->hold_life = TRUE;
			p_ptr->slow_digest = TRUE;
			p_ptr->to_h += p_ptr->level / 3;
			p_ptr->to_d += p_ptr->level / 3;
			p_ptr->dis_to_h += p_ptr->level / 3;
			p_ptr->dis_to_d += p_ptr->level / 3;
			p_ptr->pspeed += (p_ptr->level) * 2 / 13;
			break;
		default:
			/* Do nothing */
			;
	...
	/* I'm adding the mutations here for the lack of a better place... */
		  if (p_ptr->muta3)
		  {
                ...
                {
						  p_ptr->skill_sav += (15 + (p_ptr->level / 5));
                }
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
	{
		...
		if (f3 & (TR3_WRAITH))
		{
			set_wraith_form(MAX(p_ptr->wraith_form, 20));
		}
		...
		/* Hack -- berserk strength */
		if (f2 & (TR2_BERSERK))
		{
			set_shero(MAX(p_ptr->shero, 50));
		}
    ...
    if ((p_ptr->pclass == CLASS_MONK) && !(monk_heavy_armor()))
    {
        ...
        {
            p_ptr->to_a += (p_ptr->level * 3) / 2;
            p_ptr->dis_to_a += (p_ptr->level * 3) / 2;
        }
        if (!(inventory[INVEN_OUTER].k_idx) && (p_ptr->level > 15))
        {
            p_ptr->to_a += ((p_ptr->level - 13) / 3);
            p_ptr->dis_to_a += ((p_ptr->level - 13) / 3);
        }
        if (!(inventory[INVEN_ARM].k_idx) && (p_ptr->level > 10))
        {
				p_ptr->to_a += ((p_ptr->level - 8) / 3);
            p_ptr->dis_to_a += ((p_ptr->level - 8) / 3);
        }
        if (!(inventory[INVEN_HEAD].k_idx)&& (p_ptr->level > 4))
        {
				p_ptr->to_a += (p_ptr->level - 2) / 3;
            p_ptr->dis_to_a += (p_ptr->level -2) / 3;
        }
        if (!(inventory[INVEN_HANDS].k_idx))
        {
            p_ptr->to_a += (p_ptr->level / 2);
            p_ptr->dis_to_a += (p_ptr->level / 2);
        }
		  if (!(inventory[INVEN_FEET].k_idx))
        {
            p_ptr->to_a += (p_ptr->level / 3);
            p_ptr->dis_to_a += (p_ptr->level / 3);
        }
    }
	...
	{
		p_ptr->to_a += 20 + (p_ptr->level / 5);
		p_ptr->dis_to_a += 20 + (p_ptr->level / 5);
	}
	...
	if (o_ptr->k_idx && !p_ptr->heavy_shoot)
	{
		...
		if (((p_ptr->pclass == 4) && (p_ptr->tval_ammo == TV_ARROW)))
		{
			...
			if (p_ptr->level >= 20) p_ptr->num_fire++;

			/* Extra shot at level 40 */
			if (p_ptr->level >= 40) p_ptr->num_fire++;
		}
		    ...
		    (p_ptr->tval_ammo >= TV_SHOT))
		{
			...
			if (p_ptr->level >= 25) p_ptr->num_fire++;

			/* Extra shot at level 50 */
			if (p_ptr->level >= 50) p_ptr->num_fire++;
		}
	...
	if (o_ptr->k_idx && !p_ptr->heavy_wield)
	{
		...
		if (p_ptr->pclass == CLASS_WARRIOR) p_ptr->num_blow += (p_ptr->level) / 15;
	...
	else if ((p_ptr->pclass == CLASS_MONK) && !inventory[INVEN_WIELD].k_idx)
	{
		p_ptr->num_blow = 0;

		if (p_ptr->level >  9) p_ptr->num_blow++;
		if (p_ptr->level > 19) p_ptr->num_blow++;
		if (p_ptr->level > 29) p_ptr->num_blow++;
		if (p_ptr->level > 34) p_ptr->num_blow++;
		if (p_ptr->level > 39) p_ptr->num_blow++;
		if (p_ptr->level > 44) p_ptr->num_blow++;
		if (p_ptr->level > 49) p_ptr->num_blow++;
	...
	if (p_ptr->pclass == CLASS_WARRIOR)
	{
		p_ptr->to_h += (p_ptr->level/5);
		p_ptr->to_d += (p_ptr->level/5);

		p_ptr->dis_to_h += (p_ptr->level/5);
		p_ptr->dis_to_d += (p_ptr->level/5);
	}
	...
	p_ptr->skill_dis += (cp_ptr->x_dis * p_ptr->level / 10);

	/* Affect Skill -- magic devices (Level, by Class) */
	p_ptr->skill_dev += (cp_ptr->x_dev * p_ptr->level / 10);

	/* Affect Skill -- saving throw (Level, by Class) */
	p_ptr->skill_sav += (cp_ptr->x_sav * p_ptr->level / 10);

	/* Affect Skill -- stealth (Level, by Class) */
	p_ptr->skill_stl += (cp_ptr->x_stl * p_ptr->level / 10);

	/* Affect Skill -- search ability (Level, by Class) */
	p_ptr->skill_srh += (cp_ptr->x_srh * p_ptr->level / 10);

	/* Affect Skill -- search frequency (Level, by Class) */
	p_ptr->skill_fos += (cp_ptr->x_fos * p_ptr->level / 10);

	/* Affect Skill -- combat (normal) (Level, by Class) */
	p_ptr->skill_thn += (cp_ptr->x_thn * p_ptr->level / 10);

	/* Affect Skill -- combat (shooting) (Level, by Class) */
	p_ptr->skill_thb += (cp_ptr->x_thb * p_ptr->level / 10);

	/* Affect Skill -- combat (throwing) (Level, by Class) */
	p_ptr->skill_tht += (cp_ptr->x_thb * p_ptr->level / 10);
...
bool monk_heavy_armor()
{
	...
	return (monk_arm_wgt > ( 100 + (p_ptr->level * 4))) ;
}
