...
/* #endif                                                                      */
	...
	switch (p_ptr->prace)
	{
                case RACE_DWARF:
                        if (plev > 14)
                                info[i++] = "You can find secret passages (cost 30).";
                case RACE_NIBELUNG:
			if (plev > 4)
...
bool dark_lord_aura(int dam, int rad)
{
	int flg = PROJECT_GRID | PROJECT_KILL;
...
bool valkyrie_aura(int dam, int rad)
{
	int flg = PROJECT_GRID | PROJECT_KILL;
...
bool firelord_fireaura()
{
	...
	/* Hook into the "project()" function */
        if (p_ptr->stat_ind[A_CON] >= 7)
        {
        (void)project(0, 3, py, px, (p_ptr->chp / 2), GF_FIRE, flg);
        p_ptr->chp -= p_ptr->chp / 3;
        p_ptr->stat_cur[A_CON] -= 10;
        }
        else msg_print("You are too weak to use this!");
        update_and_handle();
...
bool project_hook_pets(monster_type *m_ptr, int typ, int dir, int dam, int flg)
{
	int tx, ty;
...
bool fire_ball_pets(monster_type *m_ptr, int typ, int dam, int rad)
{
	int tx, ty;
        char ch = 0;
