 ...
 * and others.
 *
 * Also modified for PosBand by Alexander Ulyanov.
 */
...
bool sees_player_mon(monster_type *m_ptr)
{
	...
	/*
	 * Who can see through any invisibility:
	 * - Ainur are demi-gods, they can see whatever they like
	 * - Beholders have magical eyeballs
	 * - Mushrooms, golems, molds, quylthulgs, skeletons, vortices
	 *   don't have eyes and "see" using some supernatural
	 *   abilities
	 * - Ghosts, liches and wraiths are powerful undeads, who
	 *   are partially/fully spectral themselves
	 * - Questers (Sauron/Morgoth) mustn't be hindered
	 * - Hack -- powerful uniques also mustn't be tricked
	 * Adding another monster racial flag would be a cleaner
	 * solution, but I don't feel like going through the whole
	 * list one more time...
	 */
	if (strchr(",AegGLmQsvW", r_ptr->d_char) ||
		r_ptr->flags1 & (RF1_QUESTOR) ||
		(r_ptr->flags1 & (RF1_UNIQUE) && r_ptr->level >= 30)) return (TRUE);
	
	/*
	 * Being in fog makes invisible.
	 * Being in water or fire makes poorly visible.
	 */
	...
	/*
	 * If the player has temporary or permanent invisibility,
	 * then, you guessed it, he's invisible.
	 */
	if (p_ptr->invisible || p_ptr->tim_invisible) return (FALSE);
...
static void find_range(monster_type *m_ptr)
{
	...
	/* Some monsters run when low on mana */
	    ...
	    (m_ptr->mana < m_ptr->maxmana / 6)) m_ptr->min_range = FLEE_RANGE;
	...
	else
	{
		...
		else if (m_lev + 3 < p_lev)
		{
			...
			if (p_val * m_mhp > m_val * p_mhp && !(m_ptr->align & (AL_PET_MASK)))
	...
	if (r_ptr->freq_ranged > 24)
	{
		...
		if (m_ptr->mana > m_ptr->maxmana / 5) m_ptr->best_range += 3;
...
/*
 * Calculate minimum and desired combat ranges for monsters.  -BR-
 */
static void find_range_mon(monster_type *m_ptr, monster_type *t_ptr)
{
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
	...
	else
	{
		...
		else if (m_lev + 3 < p_lev)
		{
			...
			/* Strong monsters scare strong monsters */
	...
	/* Nearby monsters that cannot run away will stand and fight */
	if ((m_ptr->cdis < TURN_RANGE) && (m_ptr->mspeed < t_ptr->mspeed))
		m_ptr->min_range = 1;
...
static void apply_monster_trap(monster_type *m_ptr, int y, int x)
{
	...
	if (trap_hit)
	{
		...
		switch (feat)
		{
			...
			{
				/*uniques, and monsters already slowed are unaffected*/
				if ((r_ptr->flags1 & (RF1_UNIQUE)) || (m_ptr->mspeed <= 60) ||
		...
		/*mark the creatures who saw wary*/
...
/*
 * Mega-Hack - Fully update monster's knowledge of the monster.
 * Used by everybody - monsters know each other well, right?
 * Not really good... but may work.
 */
static void update_smart_cheat_mon(int m_idx, int t_idx)
{
	monster_type *m_ptr = &mon_list[m_idx];
	monster_type *t_ptr = &mon_list[t_idx];
	monster_race *r_ptr = &r_info[t_ptr->r_idx];

	/* Know weirdness */
	if (r_ptr->flags3 & (RF3_NO_SLEEP | RF3_NO_SLOW)) m_ptr->smart |= (SM_IMM_FREE);
	if (!r_ptr->mana) m_ptr->smart |= (SM_IMM_MANA);
	/* Most of monsters below these levels cannot be really hindered. */
	if (r_ptr->level >= 30) m_ptr->smart |= (SM_GOOD_SAVE);
	if (r_ptr->level >= 55) m_ptr->smart |= (SM_PERF_SAVE);

	/* Know immunities */
	if (r_ptr->flags3 & (RF3_IM_ACID)) m_ptr->smart |= (SM_IMM_ACID);
	if (r_ptr->flags3 & (RF3_IM_ELEC)) m_ptr->smart |= (SM_IMM_ELEC);
	if (r_ptr->flags3 & (RF3_IM_FIRE)) m_ptr->smart |= (SM_IMM_FIRE);
	if (r_ptr->flags3 & (RF3_IM_COLD)) m_ptr->smart |= (SM_IMM_COLD);
	if (r_ptr->flags3 & (RF3_IM_POIS)) m_ptr->smart |= (SM_RES_POIS);
	if (r_ptr->flags3 & (RF3_NO_FEAR)) m_ptr->smart |= (SM_RES_FEAR);
	if (r_ptr->flags4 & (RF4_BRTH_LITE)) m_ptr->smart |= (SM_RES_LITE);
	if (r_ptr->flags4 & (RF4_BRTH_DARK)) m_ptr->smart |= (SM_RES_DARK);
	/* XXX */ m_ptr->smart |= (SM_RES_BLIND);
	if (r_ptr->flags3 & (RF3_NO_CONF)) m_ptr->smart |= (SM_RES_CONFU);
	if (r_ptr->flags4 & (RF4_BRTH_SOUND)) m_ptr->smart |= (SM_RES_SOUND);
	if (r_ptr->flags4 & (RF4_BRTH_SHARD)) m_ptr->smart |= (SM_RES_SHARD);
	if (r_ptr->flags3 & (RF3_RES_NEXUS)) m_ptr->smart |= (SM_RES_NEXUS);
	if (r_ptr->flags3 & (RF3_RES_NETHR | RF3_EVIL)) m_ptr->smart |= (SM_RES_NETHR);
	if (r_ptr->flags3 & (RF3_RES_CHAOS)) m_ptr->smart |= (SM_RES_CHAOS);
	if (r_ptr->flags3 & (RF3_RES_DISEN)) m_ptr->smart |= (SM_RES_DISEN);
...
static int find_resist(int m_idx, int spell_lrn)
{
	...
	switch (spell_lrn)
	{
		...
		case LRN_ELEC:
		{
			if (smart & (SM_IMM_ELEC)) return (100ll);
		...
		/* FixMe: Following spells check player afraid/etc.
		 * Wrong for monster-monster combat
		 */
...
static void remove_useless_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p, u32b *f7p, bool require_los)
{
	monster_type *m_ptr = &mon_list[m_idx];
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
	int range = (m_ptr->target < 0 ? m_ptr->cdis : distance(m_ptr->fy, m_ptr->fx, m_ptr->ty, m_ptr->tx));
	...
	if (m_ptr->mana >= m_ptr->maxmana) f6 &= ~(RF6_ADD_MANA);
	...
	if (!(range > m_ptr->best_range) && require_los)
		f6 &= ~(RF6_TELE_SELF_TO);

	if (range > 5) f6 &= ~(RF6_TELE_SELF_TO);
...
static int choose_ranged_attack(int m_idx)
{
	...
	/* Remove explicitly disabled spells */
	if (m_ptr->mflag & (MFLAG_NO_RANGED)) f4 &= ~(RF4_RANGED);
	if (m_ptr->mflag & (MFLAG_NO_OFFENSE)) f5 &= ~(RF5_OFFENSE);
	if (m_ptr->mflag & (MFLAG_NO_TELEPORT)) f6 &= ~(RF6_TELEPORT);
	if (m_ptr->mflag & (MFLAG_NO_SAVING)) f6 &= ~(RF6_SAVING);
	if (m_ptr->mflag & (MFLAG_NO_SUMM)) f7 &= ~(RF7_SUMM);
	if (m_ptr->mflag & (MFLAG_NO_SUMM_MANY)) f7 &= ~(RF7_SUMM_MANY);
	...
	if (m_ptr->mana < m_ptr->maxmana/4) want_mana +=2;
	else if (m_ptr->mana < m_ptr->maxmana/2) want_mana++;
...
/*
 * Have a monster choose a spell to use at other monster.
 *
 * Monster at m_idx uses this function to select a legal attack spell.
 * Spell casting AI is based here.
 *
 ...
 *-BR-
 */
static int choose_ranged_attack_mon(int m_idx, int t_idx)
{
	monster_type *m_ptr = &mon_list[m_idx];
	monster_type *t_ptr = &mon_list[t_idx];
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
	...
	int range = distance(m_ptr->fy, m_ptr->fx, m_ptr->ty, m_ptr->tx);
	...
	/* Check what kinds of spells can hit player */
	path = projectable(m_ptr->fy, m_ptr->fx, t_ptr->fy, t_ptr->fx, PROJECT_CHCK);
	...
	/* Don't lash if too far or close */
	if ((range > 3) || (range < 2)) f4 &= ~(RF4_LASH);
	...
	/* Desire to keep minimum distance */
	if (range < m_ptr->best_range) want_tactic++;
	if (range < m_ptr->min_range)
		want_tactic += (m_ptr->min_range - range + 1) / 2;
	if (want_tactic > 3) want_tactic=3;
	...
	/* Cheat! Cheat! Cheat! */
	update_smart_cheat_mon(m_idx, t_idx);
	...
	if (p_ptr->wizard)
	{
		/* msg_format("Spell rating: %i.", best_spell_rating); */ /* Annoys me greatly -AU- */
	}
...
/*
 * Returns the chance of passing the given non-wall grid.
 * Moved into the separate function -- place_monster_one() calls it too.
 */
int monster_pass_non_wall(int r_idx, int y, int x, int move_chance)
{
	...
	/* Water or ice */
	else if (feat == FEAT_WATER || feat == FEAT_ICE)
	{
		/* Fire elementals really don't want to go there */
		if (r_ptr->d_char == 'E' && (strstr(m_name, "ire") || strstr(m_name, "agma")))
			return (0);
		
		/* Red dragons avoid water wherever possible */
		if (r_ptr->flags3 & (RF3_DRAGON) &&
			(((r_ptr->flags4 & (RF4_BRTH_ALL)) == RF4_BRTH_FIRE) ||  /* Breathes fire and only fire */
			r_idx == 475 || r_idx == 481))           /* Hack -- Smaug or Glaurung */
			return (move_chance / 4);
		
		/* Fire-using demons don't like water */
		if (r_ptr->flags3 & (RF3_DEMON) && 
			(r_ptr->flags4 & (RF4_BRTH_FIRE) ||                    /* Breathe fire */
			 r_ptr->flags5 & (RF5_BALL_FIRE | RF5_BOLT_FIRE)))      /* Cast fire spells */
			return (move_chance / 3);
		
		/* Non-watery creatures generally try to avoid water (but not ice) */
		if (!(r_ptr->flags3 & (RF3_RES_WATER)) && feat == FEAT_WATER)
	...
	/* Fire or lava */
	else if (feat == FEAT_FIRE || feat == FEAT_LAVA)
	{
		/* Water elementals really don't want to go there */
		if (r_ptr->d_char == 'E' && (strstr(m_name, "ater") || strstr(m_name, "ce")))
			return (0);
		/* So do ents */
		if (r_ptr->d_char == '%')
			return (0);

		/* Fire elementals feel at home */
		if (r_ptr->d_char == 'E' && (strstr(m_name, "ire") || strstr(m_name, "agma")))
			return (move_chance);
		
		/* Non-fire-resistant creatures will be hurt greatly */
		if (!(r_ptr->flags3 & (RF3_IM_FIRE)))
			return (feat == FEAT_LAVA ? 0 : move_chance / 4);
		
		/* Only the fire creatures can willingly enter fire/lava */
		if (!(r_ptr->flags3 & (RF3_DEMON) || r_ptr->flags4 & (RF4_BRTH_FIRE)))
			return (move_chance / 3);
		
		return (move_chance);
	}
	
	/* Rock (sand is passable by everyone) */
	else if (feat == FEAT_ROCKS)
	{
		/* Non-earth creatures will usually try to find a better way */
	...
	/* Abyss (fog is passable by everyone) */
	else if (feat == FEAT_ABYSS)
	{
		/* Only certain races can fly above the abyss */
		/* XXX It would be nice to have FLYING flag, as in other variants */
		if (!(strchr("AbBdDEFGUvV", r_ptr->d_char) ||
		      r_ptr->flags1 & (RF1_QUESTOR)))
		      return (0);
	...
	/* Trees (passable by everyone but slow down) */
	else if (feat == FEAT_TREE)
	{
		/* Trees will slow down many monsters */
		if (!(strchr("%aAbBcCfFGIJKlqrSwy", r_ptr->d_char) ||
			r_ptr->flags1 & (RF1_QUESTOR)))
		{
			return (rand_range(30, MAX(30, move_chance / 2)));
		}
	}
...
static int cave_passable_mon(monster_type *m_ptr, int y, int x, bool *bash)
{
	...
	else if (cave_m_idx[y][x] > 0)
	{
		...
		/* Ensure that dread will not push an orc into the wall! */
		if (cave_info[m_ptr->fy][m_ptr->fx] & (CAVE_WALL))
		{
			/* Can the monster move easily through walls? */
			bool move_wall = FALSE;
		
			if ((nr_ptr->flags2 & (RF2_PASS_WALL)) ||
			    (nr_ptr->flags2 & (RF2_KILL_WALL)))
			{
				move_wall = TRUE;
			}
		...
		/* Check for hostility */
		if (m_ptr->align != n_ptr->align)
			return (100);
			
		/* Check for chaotic/lawful */
		/* Shouldn't be needed now */
		/* if (r_ptr->flags3 & (RF3_CHAOTIC) && nr_ptr->flags3 & (RF3_LAWFUL))
			return (100);
		if (r_ptr->flags3 & (RF3_LAWFUL) && nr_ptr->flags3 & (RF3_CHAOTIC))
			return (100); */
	...
	/* Webs */
	if (feat == FEAT_WEB && r_ptr->d_char != 'S')
	{
		/* Webs are hard to break */
/*		if (move_chance > 100 * r_ptr->level / BREAK_GLYPH * 3) */
/*			 move_chance = 100 * r_ptr->level / BREAK_GLYPH * 3; */
	}
	...
	if (!(cave_info[y][x] & (CAVE_WALL)))
	{
		return (monster_pass_non_wall(m_ptr->r_idx, y, x, move_chance));
	}
...
/*
 * Helper function for monsters that want to advance toward other monster.
 * Assumes that the monster isn't frightened, and is not in LOS of the
 * other monster.
 *
 * Simpler, as monster (currently) cannot use sound/scent to chase other monsters.
 */
static void get_move_advance_mon(monster_type *m_ptr, int py, int px, int *ty, int *tx)
{
	int i, y, x, y1, x1;
                     ...
                     bool must_use_target)
{
	...
	/*
	 * Hack -- the monster will blindly wander around target,
	 * if it cannot see it.
	 */
	if ((!sees_player_mon(m_ptr) && m_ptr->target == -1) || m_ptr->blinded)
	{
	    	do
		{
	...
	*tx = m_ptr->fx;
	...
	if (r_ptr->flags1 & (RF1_NEVER_MOVE))
	{
		...
		if (m_ptr->cdis <= 1 && m_ptr->target == -1)
		{
		...
		/* Are any hostile monsters in range? */
		for (i = 0; i < 8; i++)
		{
			y = m_ptr->fy + ddy_ddd[i];
			x = m_ptr->fx + ddx_ddd[i];
			...
			{
				/* Check attitude */
				if (m_ptr->align != mon_list[cave_m_idx[y][x]].align)
				{
					/* KILL!!! */
					*fear = FALSE;
					*ty = y;
	...
	if ((!*fear) && (m_ptr->cdis <= 1) && m_ptr->target == -1)
	{
	...
	/* If the target is near, kill it! */
	if ((!*fear) && (m_ptr->target >= 0) && (distance(m_ptr->fy, m_ptr->fx, m_ptr->ty, m_ptr->tx) <= 1))
	{
		*ty = m_ptr->ty;
		*tx = m_ptr->tx;
		return (TRUE);
	...
	if ((!*fear) && (r_ptr->flags1 & (RF1_FRIENDS)) &&
	    ...
	    (!(r_ptr->flags2 & (RF2_PASS_WALL | RF2_KILL_WALL))) && m_ptr->target == -1)
	{
	...
	if ((!*fear) && (r_ptr->flags1 & (RF1_FRIENDS)) &&
	    (m_ptr->cdis <= 3) && (player_has_los_bold(m_ptr->fy, m_ptr->fx)) && m_ptr->target == -1)
	{
		start = rand_int(8);
	...
	if ((!*fear) && ((r_ptr->flags2 & (RF2_PASS_WALL)) ||
	                 (r_ptr->flags2 & (RF2_KILL_WALL))) && m_ptr->target == -1)
	{
		*ty = py;
		*tx = px;
		return (TRUE);
	...
	{
		/*
		 * XXX XXX -- The monster cannot see the target.  Make it
		 * advance, so the player can have fun ambushing it.
		 */
		if (!player_has_los_bold(m_ptr->fy, m_ptr->fx) && m_ptr->target == -1)
		{
			/* Advance */
			get_move_advance(m_ptr, ty, tx);
		}
		else if (!los(m_ptr->fy, m_ptr->fx, m_ptr->ty, m_ptr->tx) && m_ptr->target > 0)
		{
			/* Advance */
			get_move_advance_mon(m_ptr, mon_list[m_ptr->target].fy, mon_list[m_ptr->target].fx, ty, tx);
		}
		else if (!los(m_ptr->fy, m_ptr->fx, m_ptr->ty, m_ptr->tx) && m_ptr->target == 0)
		{
			/* Advance */
			get_move_advance_mon(m_ptr, m_ptr->ty, m_ptr->tx, ty, tx);
		}
	...
	bool *bash)
{
	...
	if (TRUE)
	{
		...
		for (i = 0; i <= 8; i++)
		{
			...
			if (fear && !(m_ptr->align & (AL_PET_MASK)))
			{
	...
	if ((fear) && (cave_m_idx[*ty][*tx] < 0) && !(m_ptr->align & (AL_PET_MASK)))
	{
		/* Cancel fear */
		set_mon_fear(m_ptr, 0, FALSE);
		...
		/* Forget target */
		m_ptr->target = -2; m_ptr->ty = 0;    m_ptr->tx = 0;
...
static void process_move(monster_type *m_ptr, int ty, int tx, bool bash)
{
	...
	/* Webs */
	if (cave_feat[oy][ox] == FEAT_WEB && (r_ptr->d_char != 'S' &&
	...
	if (do_move)
	{
		...
		if (cave_m_idx[ny][nx] > 0)
		{
			...
			/* Attack hostile monsters */
			if (!(r_ptr->flags1 & (RF1_NEVER_BLOW)))
			{
				/* Pets can attack hostiles, and vice versa */
				if (m_ptr->align != n_ptr->align)
				{
				...
				/* Chaotics cat attack lawfuls, and vice versa */
				else if ((r_ptr->flags3 & (RF3_CHAOTIC) && nr_ptr->flags3 & (RF3_LAWFUL)) ||
					 (r_ptr->flags3 & (RF3_LAWFUL) && nr_ptr->flags3 & (RF3_CHAOTIC)))
				{
					make_attack_normal_mon(m_ptr, n_ptr);
				...
				/* Otherwise, we don't kill friends! */
	...
	if (do_move)
	{
		...
		if (m_ptr->ml)
		{
			...
			if (m_ptr->cdis == 1 && !(m_ptr->align & (AL_PET_MASK)))
			{
				disturb(0, 0);
			}
			...
			else if (disturb_move && !(m_ptr->align & (AL_PET_MASK))) disturb(0, 0);

			/* Option -- be disturbed by monster movement in LOS */
			else if ((disturb_near) &&
			         (player_has_los_bold(ny, nx)) && !(m_ptr->align & (AL_PET_MASK)))
			{
				disturb(0, 0);
			}
		}
		...
		if ((r_ptr->flags2 & (RF2_TAKE_ITEM) && !(m_ptr->align & (AL_PET_MASK))) ||
			 (r_ptr->flags2 & (RF2_KILL_ITEM)))
		{
			     ...
			     this_o_idx = next_o_idx)
			{
				...
				if (artifact_p(o_ptr) || (r_ptr->flags3 & flg3) ||
 				   (f3 & (TR3_NEVER_PICKUP)) || (o_ptr->ident & IDENT_QUEST))
				{
				...
				/* Pick up the item */
				/* Hack -- monsters are not interested in corpses */
				/* Well, some of them could be, but it would be strange
				 * to see novice rogue collecting dozens of corpses...
				 */
...
/*
 * Acquire (random) target for a monster
 * XXX This function is coded in rather naive way, and
 * probably needs some work.
 */
void acquire_target(monster_type *m_ptr)
{
	int last_target = -2, last_dist = 0, dist, i;
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
	
	/* Scan through monsters */
	for (i = 1; i < mon_max; i++)
	{
		...
		if ((m_ptr->align & (AL_HOSTILE_MASK)) && (n_ptr->align & (AL_HOSTILE_MASK)))
		{
			if (m_ptr->align == AL_HOSTILE || n_ptr->align == AL_HOSTILE ||
				m_ptr->align == n_ptr->align) continue;
		}
		
		/* Pets will never attack pets */
		if ((m_ptr->align & (AL_PET_MASK)) && (n_ptr->align & (AL_PET_MASK))) continue;
		...
		if (nr_ptr->flags1 & (RF1_UNIQUE) && !(r_ptr->flags3 & (RF3_VALA)) &&
			!(nr_ptr->flags3 & (RF3_VALA)) &&
			!(n_ptr->align & (AL_PET_MASK)) &&
			r_info[p_ptr->m_r_idx].d_char != 'Q') continue;
		
		/* Skip non-los */
		if (!los(m_ptr->fy, m_ptr->fx, n_ptr->fy, n_ptr->fx)) continue;
		
		dist = distance(m_ptr->fy, m_ptr->fx, n_ptr->fy, n_ptr->fx);
		
		/* Choose the closest monster, somewhat random */
		if (!last_dist || (one_in_(3) && (dist <= last_dist)))
		{
			last_target = i;
			last_dist = dist;
		}
	...
	/* For hostile monsters consider also player */
	if ((m_ptr->align & (AL_HOSTILE_MASK)) && m_ptr->cdis <= r_ptr->aaf &&
	    (!last_dist || (m_ptr->cdis <= last_dist && one_in_(2))))
	{
		last_target = -1;
	}
	
	/* For pets, if all else fails, run to player */
	if ((m_ptr->align & (AL_PET_MASK)) && last_target == -2)
	{
		last_target = -1;
	}
	
	m_ptr->target = last_target;

#if 0
	/* Debug messages -- who targets who */
	if (last_target == -1)
	{
		msg_format("%s X Player.", r_name + r_info[m_ptr->r_idx].name);
	}
	else if (last_target == -2)
	{
		msg_format("%s X (None).", r_name + r_info[m_ptr->r_idx].name);
	}
	else
	{
		msg_format("%s X %s.", r_name + r_info[m_ptr->r_idx].name,
			r_name + r_info[mon_list[last_target].r_idx].name);
	}
#endif /* 0 */
}
...
static void process_monster(monster_type *m_ptr)
{
	...
	if (m_ptr->min_range == 0)
	{
		...
		{
			find_range_mon(m_ptr, &mon_list[m_ptr->target]);
		}
	}
	...
	if (m_ptr->mflag & (MFLAG_ACTV))
	{
		 ...
		 */
		/* This code applies now only to breeders -AU */
		if ((m_ptr->cdis >= FLEE_RANGE) && (m_ptr->cdis > r_ptr->aaf) &&
		    /* (m_ptr->target == -2) && */ !(m_ptr->align & (AL_PET_MASK)) &&
	...
	else
	{
		...
		/* Monster has a target */
		else if (m_ptr->target > -2) m_ptr->mflag |= (MFLAG_ACTV);
	...
	/* Hack -- don't allow if the breeder is our pet! */
	if ((r_ptr->flags2 & (RF2_MULTIPLY)) &&
	    (!m_ptr->confused) && (!m_ptr->monfear) &&
	    (mon_cnt < z_info->m_max - 50) &&
	    (m_ptr->align & (AL_HOSTILE_MASK)))
	{
	...
	/* If reached "guard post", stay there */
	if (m_ptr->target == 0 && m_ptr->fx == m_ptr->tx && m_ptr->fy == m_ptr->ty)
	{
		/* Reset target */
		m_ptr->target = -2;
	}

	/* Acquire target, if don't have one, or sometimes randomly */
	if (m_ptr->target == -2 || /* No current target */
	   (m_ptr->target == -1 && (m_ptr->align & (AL_PET_MASK))
	       && !(r_ptr->flags2 & RF2_MULTIPLY)) || /* Pet guarding player, not a breeder */
	   (one_in_(7) && !(m_ptr->mflag & (MFLAG_OVER)))) /* Random chance otherwise, if target is not overridden */
	{
		acquire_target(m_ptr);
	}
	
	/* Reacquire target if targeted monster is dead */
	if (m_ptr->target > 0 && !mon_list[m_ptr->target].r_idx)
	{
		/* Reset "override" flag */
		if (!(m_ptr->mflag & (MFLAG_STAY))) m_ptr->mflag &= ~(MFLAG_OVER);
		acquire_target(m_ptr);
	}
	
	/* If failed to acquire, just wander around */
	if (m_ptr->target == -2)
	{
		/* ...Townsmen use special AI */
		if (!(m_ptr->mflag & MFLAG_TOWN))
		{
			/* Pick a random route up to ten grids */
			if (!m_ptr->tx || (m_ptr->tx == m_ptr->fx && m_ptr->ty == m_ptr->fy))
			{
				int start = rand_int(8);
				bool dummy;
	...
	*/
	...
	{
		/* Monster */
		m_ptr->tx = mon_list[m_ptr->target].fx;
		m_ptr->ty = mon_list[m_ptr->target].fy;
	}
	...
	if ((chance) && (rand_int(100) < chance))
	{
		...
		{
			choice = choose_ranged_attack_mon(cave_m_idx[m_ptr->fy][m_ptr->fx], m_ptr->target);
		}
	}
	...
	if (choice != 0)
	{
		...
		{
			make_attack_ranged_mon(m_ptr, &mon_list[m_ptr->target], choice);
		}
	...
	/* Monster is using the special "townsman" AI, and has no other monster target */
	else if (m_ptr->mflag & (MFLAG_TOWN) && m_ptr->target < 0)
	{
	...
	/* Monster isn't confused, just moving semi-randomly or blind */
	else if (random_move || m_ptr->blinded)
	{
	...
	/* Guardians allowed to move only if they will actually melee */
	if (m_ptr->mflag & (MFLAG_STAY) && m_ptr->target > 0)
	{
		if (cave_m_idx[ty][tx])
			/* Change terrain, move the monster, handle secondary effects. */
			process_move(m_ptr, ty, tx, bash);
	}
	else process_move(m_ptr, ty, tx, bash);
...
static void recover_monster(monster_type *m_ptr)
{
	...
	/* Handle the terrain effects */
	switch (cave_feat[fy][fx])
	{
		case FEAT_WATER:
		case FEAT_ICE:
		{
			...
			/* Fire elementals disintegrate */
			if (r_ptr->d_char == 'E' &&  (strstr(m_name, "ire") || strstr(m_name, "agma")))
			{
				if (cave_feat[fy][fx] == FEAT_WATER)
				{
					if (visible) msg_format("%^s extinguishes with a scream of agony!", m_name);
				...
				{
					if (visible) msg_format("The heat of %s melts the ice.", m_name);
					cave_set_feat(fy, fx, FEAT_WATER);
				}
			}
			
			/* Red dragons are stunned by water */
			if (r_ptr->flags3 & (RF3_DRAGON) &&
				(((r_ptr->flags4 & (RF4_BRTH_ALL)) == RF4_BRTH_FIRE) ||  /* Breathes fire and only fire */
				m_ptr->r_idx == 475 || m_ptr->r_idx == 481))           /* Hack -- Smaug or Glaurung */
			{
				if (m_ptr->stunned < 10) m_ptr->stunned = 10;
				mon_hurt_misc(m_ptr, damroll(10, 15));
			}
			
			/* Fire-using demons are stunned by water */
			if (r_ptr->flags3 & (RF3_DEMON) && 
				(r_ptr->flags4 & (RF4_BRTH_FIRE) ||                    /* Breathe fire */
				 r_ptr->flags5 & (RF5_BALL_FIRE | RF5_BOLT_FIRE)))      /* Cast fire spells */
			{
				if (m_ptr->stunned < 3) m_ptr->stunned = 3;
				mon_hurt_misc(m_ptr, damroll(7, 5));
			}
			
			/* Non-cold-resistant creatures are hurt by ice */
			if (!(r_ptr->flags3 & (RF3_IM_COLD)) && cave_feat[fy][fx] == FEAT_ICE)
			{
				mon_hurt_misc(m_ptr, damroll(7, 5));
			}
		}
		...
		case FEAT_FIRE:
		{
			...
			/* Water elementals evaporate */
			if (r_ptr->d_char == 'E' && (strstr(m_name, "ater") || strstr(m_name, "ce")))
			{
				if (visible) msg_format("%^s evaporates with a scream of agony!", m_name);
				delete_monster_idx(cave_m_idx[fy][fx]);
				return;
			}
			
			/* White dragons are stunned by fire */
			if (r_ptr->flags3 & (RF3_DRAGON) &&
				(((r_ptr->flags4 & (RF4_BRTH_ALL)) == RF4_BRTH_COLD)))
			{
				if (m_ptr->stunned < 10) m_ptr->stunned = 10;
				mon_hurt_misc(m_ptr, damroll(10, 15));
			}

			/* Non-resistant undeads are stunned by fire */
			if (r_ptr->flags3 & (RF3_UNDEAD) && !(r_ptr->flags3 & (RF3_IM_FIRE)))
			{
				if (m_ptr->stunned < 3) m_ptr->stunned = 3;
				mon_hurt_misc(m_ptr, damroll(7, 5));
			}
			...
			if (r_ptr->d_char == '%')
			{
				mon_hurt_misc(m_ptr, 300);
			}
			
			/* Non-fire-resistant creatures are hurt by fire */
			if (!(r_ptr->flags3 & (RF3_IM_FIRE)))
			{
				mon_hurt_misc(m_ptr, damroll(7, 5));
			}
		...
		case FEAT_LAVA:
		{
			/* Non-fire resistant creatures burn */
			if (!(r_ptr->flags3 & (RF3_IM_FIRE)))
			{
				char m_name[80];
				...
				{
					if (visible) msg_format("%^s is burnt by lava.", m_name);
					if (visible) msg_format("%^s runs away, screaming in pain!", m_name);
				}
				else
				{
					if (visible) msg_format("%^s burns to ashes with a scream of agony!", m_name);
				}
				delete_monster_idx(cave_m_idx[fy][fx]);
				return;
			}
		...
		case FEAT_ABYSS:
		{
			      ...
			      r_ptr->flags1 & (RF1_QUESTOR)))
			{
				...
				if (visible) msg_format("%^s falls into the abyss with a scream of agony!", m_name);
	...
	/* Hack -- the monster may be already dead at this point... */
	if (!m_ptr->r_idx) return;
	    ...
	    (m_ptr->mflag & (MFLAG_ACTV)))
	{
		...
		else if (m_ptr->cdis <= 5 && (player_can_fire_bold(m_ptr->fy, m_ptr->fx))) affect = TRUE;
		
		/* Cloud-surrouning pets are way too annoying */
		if (m_ptr->align & (AL_PET_MASK)) affect = FALSE;
	...
	if (m_ptr->blinded)
	{
		...
		else
		{
			...
			if (visible)
			{
				...
				/* Dump a message */
				msg_format("%^s is able to see again.", m_name);
			}
		}
	}
	...
	if (m_ptr->monfear)
	{
		...
		int d = randint(div_round(r_ptr->level, 20) + 1);
	...
	/* Recover offense counter */
	if (m_ptr->off)
	{
		m_ptr->off--;
		
		/* Messages for calming pet */
		if ((m_ptr->align & (AL_PET_MASK)) && (visible))
		{
			char m_name[80];
			monster_desc(m_name, sizeof(m_name), m_ptr, 0);

			if (m_ptr->off == OFFENSE_WARNING)
			{
				msg_format("%^s seems to calm down a bit.", m_name);
			}
			else if (m_ptr->off == 0)
			{
				msg_format("%^s seems to calm down.", m_name);
			}
		}
	}
	...
	if ((m_ptr->mspeed > r_ptr->speed + 4) || (m_ptr->mspeed < r_ptr->speed - 4))
	{
		...
		if ((m_ptr->mspeed < r_ptr->speed) && (one_in_(chance)))
		{
			...
			/* Hack -- restore a small racial variety*/
			if (!(r_ptr->flags1 & (RF1_UNIQUE)))
			{
				int i;
	...
	/* Hack -- handle aura of fear */
	/* XXX This if (...) is scary */
	if (p_ptr->aura_fear && !(r_ptr->flags2 & (RF2_STUPID)) && !m_ptr->csleep && m_ptr->cdis < 10 &&
		!(((r_ptr->flags1 & (RF1_UNIQUE)) && (one_in_(2))) ||
		(r_ptr->flags3 & (RF3_NO_FEAR)) || ((r_ptr->level + randint(MAX(4, r_ptr->level / 2))) >
		(randint(p_ptr->lev + 15) + charisma_adjustment(fy, fx)))))
	{
	    	set_mon_fear(m_ptr, m_ptr->monfear + damroll(3, p_ptr->lev + 15), TRUE);
	}

	/* Hack -- handle electric cloud */
	if (p_ptr->project_elec && (m_ptr->cdis < p_ptr->lev / 5))
	{
		char m_name[80];
		...
		monster_desc(m_name, sizeof(m_name), m_ptr, 0);
	    	...
	    	msg_format("%^s is electrocuted by the discharge.", m_name);
		project_m(0, m_ptr->fy, m_ptr->fx, damroll(p_ptr->lev / 5, 8), GF_ELEC, 0);
	}
