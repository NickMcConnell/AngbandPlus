...
void get_attack_colors(int melee_colors[RBE_MAX], int spell_colors[RSF_MAX])
{
	...
	for (i = 0; i < INVEN_TOTAL; i++)
	{
		...
		if (i < INVEN_PACK && (!known || !artifact_p(o_ptr)) &&
				p_get_lev() + adj_dex_safe[st.stat_ind[A_DEX]] < 100)
	...
	/* Gold theft */
	if (p_get_lev() + adj_dex_safe[st.stat_ind[A_DEX]] < 100 && p_ptr->au)
...
static void describe_monster_exp(int r_idx, const monster_lore *l_ptr)
{
	...
	i = (long)r_ptr->mexp * r_ptr->level / p_get_lev();

	/* calculate the fractional exp part scaled by 100, */
	/* must use long arithmetic to avoid overflow */
	j = ((((long)r_ptr->mexp * r_ptr->level % p_get_lev()) *
		  (long)1000 / p_get_lev() + 5) / 10);
