...
void update_smart_learn(int m_idx, int what)
{
	...
	switch (what)
	{
		...
		case DRS_MANA:
		{
			if (!p_get_msp()) m_ptr->smart |= (SM_IMM_MANA);
			break;
		}
...
bool mon_take_hit(int m_idx, int dam, bool *fear, cptr note)
{
	...
	if (m_ptr->hp < 0)
	{
		...
		for (i = 0; i < PY_MAX_CLASSES; i++)
		{
			...
			new_exp = ((long)r_ptr->mexp * r_ptr->level) / div / PY_MAX_CLASSES;

			/* Handle fractional experience */
			new_exp_frac = ((((long)r_ptr->mexp * r_ptr->level) % (div * PY_MAX_CLASSES))
							* 0x10000L / div) + pc_array[i].exp_frac;

			/* Keep track of experience */
			if (new_exp_frac >= 0x10000L)
			{
				new_exp++;
				pc_array[i].exp_frac = (u16b)(new_exp_frac - 0x10000L);
			}
			else
			{
				pc_array[i].exp_frac = (u16b)new_exp_frac;
			}
