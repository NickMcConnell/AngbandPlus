...
int y, x;
{
  ...
  t_ptr = &t_list[cur_pos];
  invcopy(t_ptr, sorted_objects[tmp]);
  ...
  do
      ...
      do
	{
	  ...
	  if ((cave_ptr->fval <= MAX_CAVE_FLOOR) && (cave_ptr->tptr == 0))
	    {
	      if ((randint(100) < 75)||(cave_ptr->fval == LAVA_FLOOR))
		place_object(j, k);
	      else
		place_gold(j, k);
	      i = 9;
...
void prt_equippy_chars()
{
 ...
 out_val[1]='\0';
 for (i=INVEN_QUIVER; i < INVEN_ARRAY_SIZE; i++)
  {
   i_ptr=&inventory[i];
   j=i-INVEN_QUIVER;
#ifdef TC_COLOR
...
#endif
   if (i_ptr->tval != TV_NOTHING && (i>INVEN_QUIVER || using_quiver))
     {
...
register int item_val, drop_all;
{
  ...
  i_ptr = &inventory[item_val];
  if (cave[char_row][char_col].fval==LAVA_FLOOR && set_fire_destroy(i_ptr))
   {
    flag=TRUE;
    objdes(prt2, i_ptr, FALSE);
    (void) sprintf(prt1, "The %s lands in lava and is destroyed!", prt2);
    msg_print(prt1);
   }
  else
   {
...
register inven_type *i_ptr;
{
  ...
  if (subt >= ITEM_SINGLE_STACK_MIN) {
	...
	if (locn == INVEN_QUIVER) msg_print ("Adding to quiver!");
...
int spell;
{
  ...
  if (py.misc.pclass==2)  /* Big prayer penalty for edged weapons -DGK */
   {
    register inven_type *i_ptr = &inventory[INVEN_WIELD];
    if ((i_ptr->tval==TV_SWORD)||(i_ptr->tval==TV_POLEARM))
     if ((i_ptr->flags2 & TR_BLESS_BLADE)==0) chance+=25;
   }
  if (chance > 95)
    chance = 95;
...
int stat;
{
  ...
  if (spell_learned != 0 || spell_learned2 != 0)
	...
	new_mana=(3*new_mana)/4;
      ...
      /* Start of encumbrance calculation -DGK */
      amrwgt = 0;
      for (i=INVEN_WIELD; i < INVEN_ARRAY_SIZE; i++)
       {i_ptr=&inventory[i];
	switch(i)
	{
	 case INVEN_HEAD: case INVEN_BODY: case INVEN_ARM: case INVEN_HANDS:
	 case INVEN_FEET: case INVEN_OUTER: amrwgt+= i_ptr->weight;
	}
       }
      switch(py.misc.pclass)
       {
	case 1:  maxwgt=300; break;
	case 2:  maxwgt=350; break;
	case 3:  maxwgt=350; break;
	case 4:  maxwgt=400; break;
	case 5:  maxwgt=400; break;
	default: maxwgt=0;
       }
      if (amrwgt>maxwgt)
...
int *wtohit;
{
  ...
  if (s * 15 < weight)
	...
	s+=inventory[d].p1;
      if (py.misc.pclass==1)   /* Mage attack penalty */
	d = ((((int)blows_table[str_index][dex_index]+1)/2)+s);
       else
...
int dis;
{
  ...
  do {
    ...
    while ( ((cave[y][x].fval >= MIN_CLOSED_SPACE) ||
	     ...
	     (cave[y][x].fval == NT_LIGHT_FLOOR) ||
	     (cave[y][x].fval == LAVA_FLOOR)) && count<1000);
