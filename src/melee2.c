...
void py_steal(int y, int x)
{
	...
	int i;
	int effect, theft_protection, observed;
	...
	observed = count_monsters_made_wary (p_ptr->py, p_ptr->px, FALSE);
...
/*Count the monsters noticing a certain event at a certain coordinate,
 *and make them wary.  Currently used to see how many monsters notice a
 *player setting a trap.  Notice this is counts only monsters
 *the player would know see then.  Monsters out of the players line
 *of sight who can still see the player, or confused monsters
 *who the player doesn't know seeing them, aren't counted.  If
 *player_sees is true, it counts only the cretaures who saw the theft.  If
 *it is false, count of all monsters made wary are returned,
 *regardless if player knows they were or not
 */
int count_monsters_made_wary (int y, int x, int player_sees)
{
	int observed = 0;

	int i;

	/* Smart monsters in line of sight notice things, unless they are confused or asleep */
	for (i = 1; i < mon_max; i++)
	{
		...
		/* Skip "wrong" monsters */
		if (r_ptr->d_char != r_ptr->d_char) continue;
 ...
 * -LM-
 */
void py_set_trap(int y, int x, bool count_trap)
{
	int observed, max_traps;
	...
	/*Unless using a scroll, make sure player doesn't set more than
	his limit in traps*/
	if (count_trap)
	{
		/*More advanced rogues can set more traps*/
		if (p_ptr->lev > 25) max_traps = 2;
		else max_traps = 1;
...
void py_modify_trap(int y, int x)
{
	...
	if (cave_feat[y][x] == FEAT_MTRAP_BASE) return;
