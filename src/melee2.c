...
static void remove_bad_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p)
{
	...
	if (smart_cheat)
	{
		...
		if (p_ptr->oppose_blind) smart |= (SM_OPP_BLIND);
		if (p_ptr->oppose_conf) smart |= (SM_OPP_CONF);
	...
	if ((smart & (SM_RES_BLIND)) || (smart & (SM_OPP_BLIND)))
	{
		if (int_outof(r_ptr, 100)) f5 &= ~(RF5_BLIND);
	}

	if ((smart & (SM_RES_CONFU)) || (smart & (SM_OPP_CONF)))
	{
...
static void process_monster(int m_idx)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		if (do_move && (cave_feat[ny][nx] == FEAT_GLYPH))
		{
			...
			/*
			 * Monsters may attack the player, but not move onto
			 * a Glyph.  This is the new two-dimensional Glyph
			 * that covers the floor tile but doesn't extend a
			 * force field vertically. -GJW
			 */
			...
			/* Break the ward */
			if (!do_move && (randint(BREAK_GLYPH) < r_ptr->level))
			{
