...
static void mon_take_hit_mon(int m_idx, int dam, bool *fear, cptr note)
{
	...
	/* Can the player know about the attack? */
	...
	if (m_ptr->hp < 0)
	{
		...
		else
		{
			...
			if (note)
			{
				if (known) msg_format("%^s%s", m_name, note);
			}
			...
			{
				if (known) msg_format("%^s is destroyed.", m_name);
			}
			else
			{
				if (known) msg_format("%^s is killed.", m_name);
			}
...
static void remove_bad_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p)
{
	...
	/* Doom critters don't care */
	if (r_ptr->flags3 & (RF3_DOOM)) return;
	...
	{
		if (int_outof(r_ptr, 50)) f4 &= ~(RF4_BO_SHARD);
		if (int_outof(r_ptr, 50)) f4 &= ~(RF4_BL_SHARD);
		if (int_outof(r_ptr, 50)) f4 &= ~(RF4_BR_SHAR);
		if (int_outof(r_ptr, 20)) f4 &= ~(RF4_ROCKET);
		if (int_outof(r_ptr, 50)) f5 &= ~(RF5_CH_SHARD);
	}

	if (smart & (SM_IMM_REFLECT))
	{
		if (int_outof(r_ptr, 100)) f4 &= ~(RF4_BO_SHARD);
		if (int_outof(r_ptr, 100)) f4 &= ~(RF4_BL_SHARD);
		if (int_outof(r_ptr, 100)) f5 &= ~(RF5_BO_COLD);
		if (int_outof(r_ptr, 100)) f5 &= ~(RF5_BO_FIRE);
		if (int_outof(r_ptr, 100)) f5 &= ~(RF5_BO_ACID);
		if (int_outof(r_ptr, 100)) f5 &= ~(RF5_BO_ELEC);
		if (int_outof(r_ptr, 100)) f5 &= ~(RF5_CH_SHARD);
...
static bool summon_possible(int y1, int x1)
{
	...
	for (y= y1-2; y<= y1+2; y++)
	{
		for (x = x1-2; x<=x1+2; x++)
		{
			...
			&& (cave_feat[y][x] <= FEAT_PATTERN_XTRA2)) continue;
...
static bool clean_shot(int y1, int x1, int y2, int x2)
{
	...
	for (dist = 0; dist <= MAX_RANGE; dist++)
	{
		...
		if (dist && cave_monster_bold(y, x))
		{
			if (!(m_list[cave_m_idx[y][x]].smart & SM_FRIEND)) break;
		}
...
/*
 * Determine if a bolt spell can reach its target.
 *
 * This is exactly like "projectable", but it will return TRUE if nothing
 * is in the way.
 */
static bool good_shot(int y1, int x1, int y2, int x2)
{
	int dist, y, x;
	...
	for (dist = 0; dist <= MAX_RANGE; dist++)
	{
		...
		if (dist)
		{
			...
			/* Never pass through monsters */
			if (cave_monster_bold(y, x)) break;

			/* Never pass through player */
			if (cave_player_bold(y, x)) break;
		}
...
{
	/* All summon spells */
	if ((spell == 160 + 7) || (spell >= 160 + 14)) return (TRUE);
...
static int choose_attack_spell(int m_idx, byte spells[], byte num)
{
	...
	if ((distance(p_ptr->py, p_ptr->px, m_ptr->fy, m_ptr->fx) < 4) &&
		 attack_num && (rand_int(100) < 75))
	{
...
static bool monst_spell_monst(int m_idx)
{
	...
	if (m_ptr->smart & SM_FRIEND) friendly = TRUE;
	...
	if (rand_int(100) >= r_ptr->freq_spell) no_spell = TRUE;

	/* Sometimes forbid inate attacks (breaths) */
	if (rand_int(100) >= r_ptr->freq_inate) no_inate = TRUE;
	...
	for (i = 1; i < m_max; i++)
	{
		...
		/* Ignore spells which can't reach the target */
		if (!good_shot(fy, fx, y, x))
		{
			f4 &= ~(RF4_BOLT_MASK);
			f5 &= ~(RF5_BOLT_MASK);
			f6 &= ~(RF6_BOLT_MASK);
		...
		if (see_either || blind) disturb(1, 0);
		...
		case 96+0:
		{
			...
			if (!see_m)
			{
				if (known) msg_print("You hear a shriek.");
			}
			else
			{
		...
		case 96+3:
		{
			if (!see_either)
			{
				if (known) msg_print("You hear an explosion!");
			}
			else if (blind)
			{
				msg_format("%^s shoots something.", m_name);
			...
			monst_breath_monst(m_idx, y, x, GF_ROCKET,
				((m_ptr->hp / 6) > 800 ? 800 : (m_ptr->hp / 6)), 2);
			break;
		}
		...
		case 96+4:
		{
			if (!see_either)
			{
				if (known) msg_print("You hear a strange noise.");
			}
			else if (blind)
			{
		...
		case 96+6:
		{
			...
			{
				msg_format("%^s fires a missile at %s.", m_name, t_name);
			}
		...
		case 96+8:
		{
			if (!see_either)
			{
				if (known) msg_print("You hear breathing noise.");
			}
			else if (blind)
			{
		...
		case 96+27:
		{
			...
			{
				msg_format("%^s breathes magical energy at %s.", m_name, t_name);
			}
		...
		case 96+28:
		{
			if (!see_either)
			{
				if (known) msg_print("You hear someone mumble.");
			}
			else if (blind)
			{
		...
		case 96+30:
		{
			if (!see_either)
			{
				if (known) msg_print("You hear someone mumble frighteningly.");
			}
			else if (blind)
			{
			...
			{
				msg_format("%^s invokes a raw Logrus upon %s.", m_name, t_name);
			}
			monst_breath_monst(m_idx, y, x, GF_CHAOS,
		...
		case 128+7:
		{
			if (!see_either)
			{
				if (known) msg_print ("You hear someone mumble powerfully.");
			}
			else if (blind)
			{
		...
		case 128+12:
		{
			...
			if (blind || !see_m)
			{
				if (known) msg_format("%^s mumbles.", m_name);
			}
			else
			{
		...
		case 128+16:
		{
			...
			{
				msg_format("%^s casts a acid bolt at %s.", m_name, t_name);
			}
		...
		case 128+27:
		{
			...
			if (blind || !see_m)
			{
				if (known) msg_format("%^s mumbles, and you hear scary noises.", m_name);
			}
			else
			{
				msg_format("%^s casts a fearful illusion at %s.", m_name, t_name);
			}
		...
		case 128+28:
		{
			...
			{
				if (known) msg_format("%^s casts a spell, burning %s%s eyes.", m_name, t_name,
		...
		case 128+29:
		{
			...
			if (blind || !see_m)
			{
				if (known) msg_format("%^s mumbles, and you hear puzzling noises.", m_name);
			}
			else
			{
				msg_format("%^s creates a mesmerising illusion in front of %s.", m_name, t_name);
			}
			if (tr_ptr->flags3 & RF3_NO_CONF)
			{
		...
		case 128+30:
		{
			...
			if (!blind && see_either)
			{
				if (known) msg_format("%^s drains power from %s%s muscles.", m_name, t_name,
						(!strcmp(t_name, "it") ? "s" : "'s"));
			}
			if (tr_ptr->flags1 & RF1_UNIQUE)
			{
		...
		case 128+31:
		{
			...
			if (!blind && see_m)
			{
				if (known) msg_format("%^s stares intently at %s.", m_name, t_name);
			}
			if ((tr_ptr->flags1 & RF1_UNIQUE) ||
		...
		case 160+1:
		{
			...
			if (blind || !see_m)
			{
				if (known) msg_print("You hear someone invoke the Hand of Doom!");
			}
			else
			{
				msg_format("%^s invokes the Hand of Doom on %s.", m_name, t_name);
			}
		...
		case 160+2:
		{
			...
			if (m_ptr->hp >= m_ptr->maxhp)
			{
				...
				{
					if (known) msg_format("%^s sounds completely healed!", m_name);
				}
			}
			...
			else
			{
				...
				{
					if (known) msg_format("%^s sounds healthier.", m_name);
				}
			}
		...
		case 160+7:
		{
			if (blind || !see_m)
			{
				if (known) msg_format("%^s shrieks.", m_name);
			}
			else
			{
				msg_format("%^s spawns a skull!", m_name);
			}

			for (k = 0; k < 1; k++)
			{
				count += summon_specific(fy, fx, rlev + 5, SUMMON_SKULL, FALSE, friendly);
			}

			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+9:
		{
			...
			else
			{
				...
				if (see_either) msg_format("%^s teleports %s away.", m_name, t_name);
		...
		case 160+14:
		{
			...
			{
				msg_format("%^s magically summons a servant of Id!", m_name);
			}

			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_DOOM, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+15:
		{
			...
			{
				msg_format("%^s magically summons servants of Id!", m_name);
			}

			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HI_DOOM, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+16:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_KIN, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear many things appear nearby.");
		...
		case 160+17:
		{
			...
			if (friendly)
			{
				summon_specific(y, x, MAX_DEPTH, SUMMON_CYBER, TRUE, TRUE);
			}
			else
			{
				summon_cyber();
		...
		case 160+18:
		{
			...
			for (k = 0; k < 1; k++)
			{
				if (friendly)
				{
					count += summon_specific(y, x, rlev + 5, SUMMON_NO_UNIQUES, TRUE, TRUE);
				}
				else
				{
					count += summon_specific(y, x, rlev + 5, SUMMON_MONSTER, TRUE, FALSE);
				}
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		...
		case 160+20:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_ANT, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+21:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_SPIDER, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+22:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HOUND, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+23:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HYDRA, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+24:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_ANGEL, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+25:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_DEMON, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+26:
		{
			...
			{
				msg_format("%^s magically summons an undead adversary!", m_name);
			}

			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_UNDEAD, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+27:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_DRAGON, TRUE, friendly);
			}
			if (blind && count && known) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+28:
		{
			...
			for (k = 0; k < 8; k++)
			{
				if (friendly)
				{
					count += summon_specific(y, x, rlev + 5, SUMMON_HI_UNDEAD_NO_UNIQUES, TRUE, TRUE);
				}
				else
				{
					count += summon_specific(y, x, rlev + 5, SUMMON_HI_UNDEAD, TRUE, FALSE);
				}
			}
			if (blind && count && known)
			{
		...
		case 160+29:
		{
			...
			for (k = 0; k < 8; k++)
			{
				if (friendly)
				{
					count += summon_specific(y, x, rlev + 5, SUMMON_HI_DRAGON_NO_UNIQUES, TRUE, TRUE);
				}
				else
				{
					count += summon_specific(y, x, rlev + 5, SUMMON_HI_DRAGON, TRUE, FALSE);
				}
			}
			if (blind && count && known)
			{
		...
		case 160+30:
		{
			...
			for (k = 0; k < 8; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_AMBER, TRUE, FALSE);
			}
		...
		case 160+31:
		{
			...
			for (k = 0; k < 8; k++)
			{
				if (!friendly) count += summon_specific(y, x, rlev + 5, SUMMON_UNIQUE, TRUE, FALSE);
			}
			for (k = 0; k < 8; k++)
			{
				if (friendly)
...
/*
 * Curse an item in the equipment
 */
void curse_equipment(int chance, int heavy_chance)
{
	...
	object_type *o_ptr = &inventory[rand_range(INVEN_WIELD, INVEN_TOTAL - 1)];

	/* Skip curse sometimes */
	if (rand_int(100) >= chance) return;
	...
	/* Some items may be heavily cursed */
	if ((rand_int(100) < heavy_chance) && excellent_p(o_ptr))
	{
		/* Notice the change */
	...
	/* Most items get lightly cursed */
	else
	{
		/* Notice the change */
		if (!(o_ptr->ident & (IDENT_CURSED))) changed = TRUE;
	...
	/* Notice the change */
	if (changed) msg_print("There is a malignant black aura surrounding you...");
}
...
bool make_attack_spell(int m_idx)
{
		 ...
		 !(r_ptr->flags3 & RF3_DOOM))
	{
		...
		if (!clean_shot(m_ptr->fy, m_ptr->fx, py, px))
		{
	...
	switch (thrown_spell)
	{
		...
		case 160+7:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(m_ptr->fy, m_ptr->fx, rlev + 5, SUMMON_SKULL, FALSE, FALSE);
			}
		...
		case 160+14:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_DOOM, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+15:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HI_DOOM, TRUE, FALSE);
			}
		...
		case 160+16:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_KIN, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear many things appear nearby.");
		...
		case 160+20:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_ANT, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+21:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_SPIDER, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+22:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HOUND, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+23:
		{
			...
			for (k = 0; k < 6; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HYDRA, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear many things appear nearby.");
			break;
		}
		...
		case 160+24:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_ANGEL, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+25:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_DEMON, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+26:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_UNDEAD, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+27:
		{
			...
			for (k = 0; k < 1; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_DRAGON, TRUE, FALSE);
			}
			if (blind && count) msg_print("You hear something appear nearby.");
			break;
		}
		...
		case 160+31:
		{
			...
			for (k = 0; k < 8; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_UNIQUE, TRUE, FALSE);
			}
			for (k = 0; k < 8; k++)
			{
				count += summon_specific(y, x, rlev + 5, SUMMON_HI_UNDEAD, TRUE, FALSE);
...
static bool get_moves_aux(int m_idx, int *yp, int *xp)
{
	...
	if (cave_when[y1][x1] < cave_when[py][px])
	{
		...
		if (!cave_when[y1][x1]) return (FALSE);
	...
	if ((cave_cost[y1][x1] > r_ptr->aaf) && !(m_ptr->smart & SM_FRIEND)) return (FALSE);
...
static bool get_fear_moves_aux(int m_idx, int *yp, int *xp)
{
	...
	if ((cave_cost[fy][fx] > r_ptr->aaf) && !(m_ptr->smart & SM_FRIEND)) return (FALSE);
...
static bool get_moves(int m_idx, int *mm)
{
	...
	{
		/* Flow towards the player */
		done = get_moves_aux(m_idx, &y2, &x2);
	}
#endif
	...
	if (!stupid_monsters && !(m_ptr->smart & (SM_FRIEND)))
	{
		/*
		* Animal packs try to get the player out of corridors
		* (...unless they can move through walls -- TY)
		*/
		if ((r_ptr->flags1 & (RF1_FRIENDS | RF1_ESCORT | RF1_ESCORTS)) &&
			 ...
			 !(r_ptr->flags3 & (RF3_DOOM)))
		{
			...
			if ((s_ptr < 5) && (p_ptr->chp > p_ptr->mhp / 2))
			{
				/* Find hiding place */
...
static bool monst_attack_monst(int m_idx,int t_idx)
{
	...
	{
		if (known) msg_print("You hear noise.");
	}
	...
	for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
	{
		...
		if (!effect || check_hit2(power, rlev,ac))
		{
			...
			if (act)
			{
				...
				if (m_ptr->ml || t_ptr->ml) msg_format("%^s %s", m_name, temp);
			}
...
static void process_monster(int m_idx, bool is_friend)
{
	...
	/* Take damage from being in walls */
	if (!(r_ptr->flags2 & (RF2_PASS_WALL)) && !cave_floor_bold(fy, fx))
	{
		project(0, 0, fy, fx, damroll(30, 6),
				  GF_CRUSHER, PROJECT_KILL | PROJECT_HIDE | PROJECT_JUMP);
	...
	if ((m_ptr->smart & (SM_FRIEND)) && !p_ptr->wizard &&
		(r_ptr->flags1 & (RF1_UNIQUE)))
	{
		gets_angry = TRUE;
	}
	...
	if (speak_unique && !(m_ptr->smart & (SM_FRIEND)))
	{
		if (!rand_int(SPEAK_CHANCE))
		{
			if (player_has_los_bold(oy, ox) && (r_ptr->flags2 & (RF2_CAN_SPEAK)))
			{
				...
				if (m_ptr->monfear)
				{
					if (!get_rnd_line("monfear.txt", r_name + r_ptr->name, buf))
					{
						msg_format("%^s %s", m_name, buf);
					}
				}
				else if (!get_rnd_line("monspeak.txt", r_name + r_ptr->name, buf))
				{
					msg_format("%^s %s", m_name, buf);
				}
			}
	...
	*/
	...
	for (i = 0; mm[i]; i++)
	{
		...
		/* Hack -- player 'in' wall */
		else if (cave_player_bold(ny, nx))
		{
			do_move = TRUE;
		}

		else if (cave_monster_bold(ny, nx))
		{
		...
		else if (r_ptr->flags2 & (RF2_KILL_WALL))
		{
			...
			/* Notice */
			draw_grid_feat(ny, nx, FEAT_FLOOR);
					...
					(feat == FEAT_SECRET))
		{
...

			{
				...
				else if (feat < FEAT_DOOR_HEAD + 0x08)
				{
					...
					/* Door power */
					k = ((feat - FEAT_DOOR_HEAD) & 0x07);

					/* Try to unlock it XXX XXX XXX */
					if (rand_int(m_ptr->hp / 10) > k)
					{
						/* Unlock the door */
						draw_grid_feat(ny, nx, FEAT_DOOR_HEAD + 0x00);
			...
			if (did_open_door || did_bash_door)
			{
				...
				{
					draw_grid_feat(ny, nx, FEAT_BROKEN);
				}

				/* Open the door */
				else
				{
					draw_grid_feat(ny, nx, FEAT_OPEN);
				}
		...
		else if (do_move && (feat == FEAT_MINOR_GLYPH) &&
					!(m_ptr->smart & SM_FRIEND) &&
					!(r_ptr->flags1 & (RF1_NEVER_BLOW)))
		{
			/* Assume no move allowed */
			...
			if (randint(BREAK_MINOR_GLYPH) < r_ptr->level)
			{
				...
				if (cave_info[ny][nx] & (CAVE_MARK))
				{
					if (cave_player_bold(ny, nx))
					{
						msg_print("The rune explodes!");
						fire_ball(GF_MANA, 0,
							2 * ( ((p_ptr->level)/2) + damroll(7,7)), 2);
					}
					else
		...
		if (do_move && cave_player_bold(ny, nx) &&
			(r_ptr->flags1 & (RF1_NEVER_BLOW)))
		{
			/* Do not move */
			do_move = FALSE;
		...
		if (do_move && cave_monster_bold(ny, nx))
		{
			monster_race *z_ptr = &r_info[y_ptr->r_idx];
			monster_type	*m2_ptr = &m_list[cave_m_idx[ny][nx]];
				 ...
				 m_ptr->confused)
			{
				...
				if (r_ptr->flags2 & RF2_KILL_BODY) did_kill_body = TRUE;
		...
		if (do_move)
		{
			...
			{
				/* Disturb */
				if (!(m_ptr->smart & (SM_FRIEND)) || disturb_pets)
				disturb(0, 0);
			}
...
void process_monsters(void)
{
	...
	for (i = m_max - 1; i >= 1; i--)
	{
		...
		if ((m_ptr->cdis <= r_ptr->aaf) || total_friends)
		{
		...
		/* Note that most monsters have "aaf" of "20" or so */
					...
					(cave_when[py][px] == cave_when[fy][fx]) &&
					(cave_cost[fy][fx] < MONSTER_FLOW_DEPTH) &&
					((cave_cost[fy][fx] < r_ptr->aaf) || (m_ptr->smart & SM_FRIEND)))
		{
		...
		if (m_ptr->smart & (SM_FRIEND)) is_friend = TRUE;
