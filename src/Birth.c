	...
	{"You have ", 100, 134, 120, 50 },
	  ...
	  /* Demon */
        ...
        {"and large brown wings. ", 100, 214, 0, 50},
          ...
          /* Hell Queen!! */
        {"You are a hell queen, neither undead nor demon. A cross between those two. ", 100, 215, 216, 50},

        {"You are an *extremly* beautiful, powerful and evil lady. ", 100, 216, 217, 50},

        {"You have beautiful dark black hairs, ", 50, 217, 218, 25},
        {"You have beautiful dark red hairs, ", 100, 217, 218, 50},

        {"red eyes, ", 100, 218, 219, 50},

        {"a reddish skin ", 100, 219, 220, 50},

        {"and a very sexy thin body. ", 100, 220, 221, 50},

        {"You have two dark horns on your head. ", 33, 221, 0, 15},
        {"You have two red horns on your head. ", 66, 221, 0, 32},
        {"You have two white horns on your head. ", 100, 221, 0, 50},

          /* Skatts! */
        {"You were born in the jungle. ", 50, 222, 223, 25},
        {"You were born in the mountains. ", 100, 222, 223, 50},

        {"You are a cross between a human and a feline. ", 100, 223, 224, 50},

        {"You have cat-like ears, ", 100, 224, 225, 50},

        {"blue eyes, ", 20, 225, 226, 10},
        {"green eyes, ", 40, 225, 226, 20},
        {"red eyes, ", 60, 225, 226, 30},
        {"pale green eyes, ", 80, 225, 226, 40},
        {"brown eyes, ", 100, 225, 226, 50},

        {"a ocher tail, ", 50, 226, 227, 25},
        {"a orange tail, ", 100, 226, 227, 50},

        {"and very powerful feline legs. ", 100, 227, 0, 50},

          /* Benemals */
        {"You are a cross between an evil demon and a good human. ", 100, 228, 229, 50},

        {"You have a human 'white' skin, ", 25, 229, 230, 15},
        {"You have a pale red skin, ", 50, 229, 230, 30},
        {"You have a human 'black' skin, ", 75, 229, 230, 40},
        {"You have a totally black skin, ", 100, 229, 230, 50},
        ...
        {"You are a gray devling. ", 50, 235, 0, 30},
...
byte choose_realm(s32b choices)
{
	...
	{
                sprintf(buf, "%c%c %s", I2A(n), p2, "Magic");
		put_str(buf, 21 + (n/5), 2 + 15 * (n%5));
                picks[n]=10;
		n++;
	}
	...
	{
                sprintf(buf, "%c%c %s", I2A(n), p2, "Prayer");
		put_str(buf, 21 + (n/5), 2 + 15 * (n%5));
                picks[n]=11;
		n++;
	}

        if ((choices & CH_BATTLE) && (p_ptr->realm1 != REALM_BATTLE))
	{
                sprintf(buf, "%c%c %s", I2A(n), p2, "Battle");
		put_str(buf, 21 + (n/5), 2 + 15 * (n%5));
                picks[n]=15;
		n++;
	}
...
static void player_outfit(void)
{
	...
	q_ptr = &forge;
        ...
        if ((p_ptr->pclass == CLASS_MAGE) || (p_ptr->pclass == CLASS_SORCERER) || (p_ptr->pclass == CLASS_LICH))
	{
...
static bool player_birth_aux()
{
	...
	str = cp_ptr->title;
        ...
        if (p_ptr->pclass == CLASS_LICH && (p_ptr->prace != RACE_SPECTRE || p_ptr->prace != RACE_SPECTRE))
        {
            msg_print("As a lich, you MUST be an undead! You will now be turned into one!");
            if (rand_int(50) >= 26) p_ptr->prace = 16;
            else p_ptr->prace = 17;
            rp_ptr = &race_info[p_ptr->prace];
            str = rp_ptr->title;
	...
	process_dungeon_file("q_info.txt", &ystart, &xstart, 0, 0);
        ...
        p_ptr->inside_quest = QUEST_MARYLENE;
	process_dungeon_file("q_info.txt", &ystart, &xstart, 0, 0);
        quest[QUEST_MARYLENE].status = QUEST_STATUS_TAKEN;

        p_ptr->inside_quest = QUEST_VARIAZ;
	process_dungeon_file("q_info.txt", &ystart, &xstart, 0, 0);
        quest[QUEST_VARIAZ].status = QUEST_STATUS_TAKEN;

        p_ptr->inside_quest = 22;
	process_dungeon_file("q_info.txt", &ystart, &xstart, 0, 0);
        quest[22].status = QUEST_STATUS_TAKEN;
...
static void validate_bg(void)
{
        ...
        race_chart[RACE_SKATT] = 222;
        race_chart[RACE_BENEMAL] = 228;
        race_chart[RACE_DEVLING] = 232;
        /* class_chart[CLASS_HELLQUEEN] = 215; */
...
void do_cmd_change_class(void)
{
     ...
     cptr str;
     int cchoic;
     cchoic = get_quantity("Become which class? ", 35);
     if (cchoic >= 0 && cchoic <= 35) c_put_str(TERM_L_BLUE, "             ", 2, 0);
     if (cchoic >= 0 && cchoic <= 35) c_put_str(TERM_L_BLUE, "             ", 3, 0);
     if (cchoic == 0)
     {
     msg_print("You can no longer change back to an apprentice that way!");
     msg_print("You must go to the training center and pay to change back!");
     }

     else if (cchoic == 1 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 14 && p_ptr->stat_ind[A_CON] >= 12)
        {
        msg_print("You become a warrior!");
        ...
        else msg_print("You do not have the required stats! Type '?' and check the stats table!");
     }

     else if (cchoic == 2 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 14 && p_ptr->stat_ind[A_CHR] >= 9)
        {
        msg_print("You become a mage!");
     ...
     else if (cchoic == 3 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_WIS] >= 14 && p_ptr->stat_ind[A_CHR] >= 11 && p_ptr->pgod != GOD_NONE)
        {
        msg_print("You become a priest!");
     ...
     else if (cchoic == 4 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 10 && p_ptr->stat_ind[A_INT] >= 10 && p_ptr->stat_ind[A_DEX] >= 14 && p_ptr->stat_ind[A_CHR] >= 9)
        {
        msg_print("You become a rogue!");
     ...
     else if (cchoic == 5 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 11 && p_ptr->stat_ind[A_INT] >= 11 && p_ptr->stat_ind[A_DEX] >= 14 && p_ptr->stat_ind[A_CON] >= 10)
        {
        msg_print("You become a ranger!");
     ...
     else if (cchoic == 6 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 11 && p_ptr->stat_ind[A_WIS] >= 11 && p_ptr->stat_ind[A_CHR] >= 12 && p_ptr->pgod != GOD_NONE)
        {
        msg_print("You become a paladin!");
     ...
     else if (cchoic == 7 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 11 && p_ptr->stat_ind[A_INT] >= 11 && p_ptr->stat_ind[A_CON] >= 9 && p_ptr->lev >= 5)
        {
        msg_print("You become a warrior-mage!");
     ...
     else if (cchoic == 8 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 11 && p_ptr->stat_ind[A_INT] >= 11 && p_ptr->stat_ind[A_CON] >= 9 && p_ptr->stat_ind[A_CHR] <= 8 && (p_ptr->realm1 == REALM_CHAOS || p_ptr->realm2 == REALM_CHAOS) && p_ptr->lev >= 5)
        {
        msg_print("You become a chaos warrior!");
     ...
     else if (cchoic == 9 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 15 && p_ptr->stat_ind[A_INT] >= 10 && p_ptr->stat_ind[A_WIS] >= 10 && p_ptr->stat_ind[A_DEX] >= 15 && p_ptr->stat_ind[A_CON] >= 14)
        {
        msg_print("You become a monk!");
     ...
     else if (cchoic == 10 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 14 && p_ptr->stat_ind[A_WIS] >= 14)
        {
        msg_print("You become a mindcrafter!");
     ...
     else if (cchoic == 11 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 18 && p_ptr->stat_ind[A_CHR] >= 9 && p_ptr->lev >= 10)
        {
        msg_print("You become a high-mage!");
     ...
     else if (cchoic == 12 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 15)
        {
        msg_print("You become a mimic!");
     ...
     else if (cchoic == 13 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 16 && p_ptr->stat_ind[A_INT] >= 14 && p_ptr->stat_ind[A_WIS] >= 14 && p_ptr->stat_ind[A_DEX] >= 15 && p_ptr->stat_ind[A_CON] >= 13 && p_ptr->stat_ind[A_CHR] >= 16)
        {
        msg_print("You become a beastmaster!");
     ...
     else if (cchoic == 14 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 17 && p_ptr->stat_ind[A_WIS] >= 11 && p_ptr->lev >= 15)
        {
        msg_print("You become an alchemist!");
     ...
     else if (cchoic == 15 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 12 && p_ptr->stat_ind[A_WIS] >= 12 && p_ptr->stat_ind[A_CHR] >= 16 && (p_ptr->realm1 == REALM_MUSIC || p_ptr->realm2 == REALM_MUSIC))
        {
        msg_print("You become an harper!");
     ...
     else if (cchoic == 16 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 18 && p_ptr->lev >= 10)
        {
        msg_print("You become a power mage!");
        p_ptr->pclass = CLASS_POWERMAGE;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 17 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 18 && p_ptr->stat_ind[A_WIS] >= 13 && p_ptr->lev >= 10)
        {
        msg_print("You become a runecrafter!");
        p_ptr->pclass = CLASS_RUNECRAFTER;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 18 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 19 && p_ptr->lev >= 20)
        {
        msg_print("You become a possessor!");
     ...
     else if (cchoic == 19 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 22 && p_ptr->stat_ind[A_CHR] >= 11 && p_ptr->lev >= 20)
        {
        msg_print("You become a sorceror!");
     ...
     else if (cchoic == 20 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_DEX] >= 15)
        {
        msg_print("You become an archer!");
     ...
     else if (cchoic == 21 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 14 && (p_ptr->realm1 == REALM_ILLUSION || p_ptr->realm2 == REALM_ILLUSION) && p_ptr->stat_ind[A_CHR] >= 12)
        {
        msg_print("You become an illusionist!");
     ...
     else if (cchoic == 22 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 15 && p_ptr->stat_ind[A_WIS] >= 15 && (p_ptr->realm1 == REALM_DRUID || p_ptr->realm2 == REALM_DRUID))
        {
        msg_print("You become a druid!");
     ...
     else if (cchoic == 23 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 16 && p_ptr->stat_ind[A_WIS] >= 15 && p_ptr->stat_ind[A_CHR] <= 8)
        {
        msg_print("You become a necromancer!");
        p_ptr->pclass = CLASS_NECRO;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 24 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 20 && p_ptr->stat_ind[A_INT] >= 20 && p_ptr->stat_ind[A_DEX] >= 20 && p_ptr->stat_ind[A_CON] >= 15 && (p_ptr->realm1 == REALM_BATTLE || p_ptr->realm2 == REALM_BATTLE) && p_ptr->lev >= 15)
        {
        msg_print("You become a magi warrior!");
        p_ptr->pclass = CLASS_MAGIWARRIOR;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 25 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 25 && p_ptr->stat_ind[A_DEX] >= 14 && p_ptr->stat_ind[A_CON] >= 18 && p_ptr->prace != RACE_DRAGONRIDER)
        {
        msg_print("You become a berserker!");
     ...
     else if (cchoic == 26 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 17 && p_ptr->stat_ind[A_INT] >= 23 && p_ptr->stat_ind[A_CON] >= 14 && p_ptr->stat_ind[A_CHR] <= 0 && p_ptr->lev >= 25 && p_ptr->psex == SEX_MALE)
        {
        msg_print("You become a dark lord!");
        p_ptr->pclass = CLASS_DARK_LORD;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 27 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 17 && p_ptr->stat_ind[A_INT] >= 15 && p_ptr->stat_ind[A_WIS] >= 19 && p_ptr->stat_ind[A_CON] >= 11 && p_ptr->stat_ind[A_CHR] >= 17 && p_ptr->prace != RACE_DEMON && p_ptr->pgod == GOD_GOD)
        {
        msg_print("You become a justice warrior!");
        p_ptr->pclass = CLASS_JUSTICE_WARRIOR;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 28 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_INT] >= 22 && p_ptr->stat_ind[A_CHR] <= 7 && (p_ptr->prace == RACE_VAMPIRE || p_ptr->prace == RACE_SPECTRE) && p_ptr->lev >= 20)
        {
        msg_print("You become a lich!");
     ...
     else if (cchoic == 29 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_CHR] >= 18)
        {
        msg_print("You become a leader!");
        p_ptr->pclass = CLASS_LEADER;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 30 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 17 && p_ptr->stat_ind[A_DEX] >= 18 && p_ptr->stat_ind[A_INT] >= 14 && p_ptr->stat_ind[A_WIS] >= 11 && p_ptr->stat_ind[A_CON] >= 12 && p_ptr->prace != RACE_DRAGONRIDER)
        {
        msg_print("You become a Zelar!");
        p_ptr->pclass = CLASS_ZELAR;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 31 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_DEX] >= 20 && p_ptr->prace == RACE_SKATT)
        {
        msg_print("You become a Skatter!");
        p_ptr->pclass = CLASS_SKATTER;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 32 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 10 && p_ptr->stat_ind[A_INT] >= 12 && p_ptr->stat_ind[A_DEX] >= 19 && p_ptr->stat_ind[A_CHR] >= 12)
        {
        msg_print("You become an Assassin!");
     ...
     else if (cchoic == 33 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 16 && p_ptr->stat_ind[A_CON] >= 17)
        {
        msg_print("You become a Fire Lord!");
        p_ptr->pclass = CLASS_FIRE_LORD;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else if (cchoic == 34 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_STR] >= 15 && p_ptr->stat_ind[A_CON] >= 12 && p_ptr->stat_ind[A_CHR] >= 22)
        {
        msg_print("You become a Commander!");
     ...
     else if (cchoic == 35 && p_ptr->pclass == CLASS_APPRENTICE)
     {
        if (p_ptr->stat_ind[A_CON] >= 10 && p_ptr->stat_ind[A_INT] >= 15)
        {
        msg_print("You become a Monster Mage!");
        p_ptr->pclass = CLASS_MONSTER_MAGE;
        cp_ptr = &class_info[p_ptr->pclass];
        mp_ptr = &magic_info[p_ptr->pclass];
        str = cp_ptr->title;
        c_put_str(TERM_L_BLUE, cp_ptr->title, 2, 0);
     ...
     else msg_print("You can no longer change class!");
     handle_stuff();
}
...
void change_back_to_apprentice(void)
{
     ...
     msg_print("However, you lose 25% of your experience. That is the cost for changing class!");
...
void apply_valkyrie_weapon_magic()
{
        ...
        if (p_ptr->pclass == CLASS_SKATTER && o_ptr->tval == TV_VALKYRIE_SPEAR)
        {
