 ...
 *
 * Cameron Browne (2010) added code to GetTileImage() to avoid image tiles 
 * interpolating across tile boundaries and leaking into neighbours.
 *
...
static CGImageRef GetTileImage(int row, int col, bool has_alpha)
{
	...
	int y1 = y0 + graf_height; 
	
	CGRect src_left   = {{ x0-2, y0-2} , { 2, graf_height+4 } };
	CGRect src_right  = {{ x1, y0-2} , { 2, graf_height+4 } };
	...
	CGContextRef context_copy_ref = CGBitmapContextCreate(NULL, 
														  ...
														  8, 
														  CGImageGetBytesPerRow(frame.image), 
														  CGImageGetColorSpace(frame.image), 
														  kCGImageAlphaPremultipliedFirst); 
	int dx = CGImageGetWidth(frame.image);
	...
	CGContextAddRect(context_copy_ref, src_top); 
	CGContextAddRect(context_copy_ref, src_bottom);
	CGContextFillPath(context_copy_ref);		
	
	CGImageRef image_copy_ref = CGBitmapContextCreateImage(context_copy_ref);
	
	DrawSubimage(map, dst_r, image_copy_ref, src_r);
	
	CGContextRelease(context_copy_ref);
#else
