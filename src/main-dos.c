...
/*
 * This is a BETA version that works with Allegro 4.#.  Please test before
* trusting.  -LM-
 */

/*
 * This file does not currently work with Sangband
 */
...
#include <allegro.h>
#include <allegro/internal/aintern.h>   /* XXX XXX XXX */
...
static void Term_xtra_dos_react(void)
{
	...
	for (i = 0; i < MAX_COLORS; i++)
	{
		...
		char rv = color_table[i].rv >> 2;
		char gv = color_table[i].gv >> 2;
		char bv = color_table[i].bv >> 2;
	...
	while (TRUE)
	{
		...
		switch (k)
		{
			...
			case 'r':
			{
				...
				/* Save the resolution */
				set_config_int("Angband", "Resolution1", resolution);
...
static void Term_nuke_dos(term *t)
{
	...
	{
		/*
		 * This code needs to be verified.  It does not seem necessary to
		 * call "free(td->font)"; how about "destroy_font(td->font)"?
		 * XXX XXX XXX
		 */
		if (td->uses_grx_font)
		{
			/* free(td->font->data.dat_prop); */
			/* free(td->font); */
		}
		else
		{
 ...
 */
static errr Term_rows_dos(bool fifty_rows)
{
	term_data *td;
...
/* return (TRUE); */
	...
	/* Get screen size */
	resolution = get_config_int(section,
		(fifty_rows ? "Resolution2" : "Resolution1"), 1);

	/* Section name */
	sprintf(section, "Mode-%d", resolution);

	/* Section name */
	sprintf(section, "Term-%d-%d", resolution, 0);
...
/*	td->number = 0; */
	...
	/* Draw the cursor sprite (yellow line) */
	rectfill(cursor, 0, td->tile_hgt - 2, td->tile_wid - 1, td->tile_hgt - 1,
...
static void term_data_link(term_data *td)
{
	...
	/* Initialize the term */
	(void)term_init(t, td->cols, td->rows, 255);
...
static bool init_windows(void)
{
	...
	for (i = 0; i < num_windows; i++)
	{
		...
		{
			/* Load the font file */
			if (!(td->font = font_25 = import_grx_font(filename)))
			{
		...
		else if (suffix(filename, ".dat"))
		{
			...
			/* Save the font data */
			td->font = font_25 = fontdata[1].dat;
...
errr init_dos(int argc, char **argv)
{
	...
	/* Get screen size */
	resolution = get_config_int(section, "Resolution1", 1);
