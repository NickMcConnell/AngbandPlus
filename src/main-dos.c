 ...
 *
 * Ben Harrison, Robert Ruehlmann, and others released all changes to the Angband code under the terms of the GNU General Public License (version 2),
...
static void Term_xtra_dos_react(void)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		char rv = angband_colour_table[i][1] >> 2;
		char gv = angband_colour_table[i][2] >> 2;
		char bv = angband_colour_table[i][3] >> 2;
	...
	while (1)
	{
		...
		switch (k)
		{
			...
			case 'r':
			{
				...
				prt("Restart Hellband to get the new screenmode.", 3, 0);
				...
				/* Save the resolution */
				set_config_int("Hellband", "Resolution", resolution);
...
#endif /* USE_SOUND */
				set_config_int("Hellband", "Graphics", arg_graphics);
				set_config_int("Hellband", "Sound", arg_sound);
...
static void dos_dump_screen(void)
{
	...
	path_build(filename, 1024, ANGBAND_DIR_XTRA, "dump.bmp");
...
errr init_dos(int argc, char **argv)
{
	...
	set_config_file("hellband.cfg");

	/* Main section */
	strcpy(section, "Hellband");
	...
	if (!arg_sound)
	{
		arg_sound = get_config_int("Hellband", "Sound", TRUE);
	}
	...
	if (!arg_graphics)
	{
		arg_graphics = get_config_int("Hellband", "Graphics", TRUE);
	}
