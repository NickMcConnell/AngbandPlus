...
/* $Id: opponent.c,v 1.4 2003/04/07 20:53:55 cipher Exp $ */
...
IH_RenderOverlay_Opponent(Overlay * overlay)
{
     ...
     flags = overlay->var[0];
     cur_hp = overlay->var[1];
     max_hp = MIN(1, overlay->var[2]); /* prevent divide by 0 */
     attr = (byte) overlay->var[3];
     flags = overlay->flags;
     ...
     overlay->position.x = (ih.display_width - back_width) / 2;
     ...
     if(m_name)
     {
          ...
          IH_RenderText(IH_OVERLAY_OPPONENT_NAME_FONT, m_name, &pos, color,
                        NULL);
