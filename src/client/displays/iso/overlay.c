...
/* $Id: overlay.c,v 1.6 2003/04/07 20:53:55 cipher Exp $ */
...
static int      ov_display_order = 0;

static void
condense_overlay_orders(void)
{
     int             overlay_num;

     for(overlay_num = DISPLAY_CHARACTER;
         overlay_num < DISPLAY_MAX; overlay_num++)
     {
          if(!overlays[overlay_num].shown)
          {
               int             i;

               for(i = overlay_num; i < DISPLAY_MAX; i++)
               {
                    if(overlays[i].order > i)
                         overlays[i].order--;
               }
          }
     }
...
overlay_init(int overlay_num)
{
     ...
     if(!SDL_SemWait(ih.sem.overlay))
     {
          ...
          overlay->order = ov_display_order++;
...
IH_RenderOverlays(void)
{
     ...
     for(order = 0; order < DISPLAY_MAX; order++)
     {
          for(overlay_num = DISPLAY_CHARACTER;
              overlay_num < DISPLAY_MAX; overlay_num++)
          {
