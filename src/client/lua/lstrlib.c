...
/*
** $Id: lstrlib.c,v 1.2 2003/03/17 22:45:30 cipher Exp $
           ...
           struct Capture *cap)
{
     ...
     while((s + i) < cap->src_end &&
           luaI_singlematch((unsigned char) *(s + i), p, ep))
          i++;
     ...
     for(;;)
     {
          ...
          if(res != NULL)
               return res;
                  ...
                  luaI_singlematch((unsigned char) *s, p, ep))
             ...
             dflt:{            /* it is a pattern item */
                    ...
                    switch (*ep)
                    {
                         ...
                         case '+': /* 1 or more repetitions */
                              return (m ? max_expand(L, s + 1, p, ep, cap)
                                      : NULL);
...
str_find(lua_State * L)
{
     ...
     luaL_arg_check(L, 0 <= init &&
                    (size_t) init <= l1, 3, "out of range");
     ...
     if(lua_isstring(L, 3))
     {
          ...
          for(i = 0; i < l; i++)
          {
               ...
               else
               {
                    ...
                    {
                         int             level =
                             check_capture(L, news[i], cap);
                         luaL_addlstring(b, cap->capture[level].init,
                                         cap->capture[level].len);
                    }
               }
          }
     }
     ...
     while(*strfrmt)
     {
          ...
          {                     /* format item */
               ...
               switch (*strfrmt++)
               {
                    ...
                    case 'X':
                         sprintf(buff, form,
                                 (unsigned int) luaL_check_number(L, arg));
                         break;
                    case 'e':
                    case 'E':
                    case 'f':
