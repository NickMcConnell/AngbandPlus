...
/*
** $Id: liolib.c,v 1.1 2003/03/19 21:53:06 cipher Exp $
...
static int
io_readfrom(lua_State * L)
{
     return io_fromto(L, INFILE, "r");
}

static int
io_writeto(lua_State * L)
{
     return io_fromto(L, OUTFILE, "w");
}

static int
io_appendto(lua_State * L)
{
     IOCtrl         *ctrl = (IOCtrl *) lua_touserdata(L, -1);
     FILE           *current;
...
io_read(lua_State * L)
{
     ...
     {                          /* no arguments? */
          ...
          luaL_checkstack(L, lastarg - firstarg + 1 + LUA_MINSTACK,
                          "too many arguments");
...
io_write(lua_State * L)
{
     ...
     for(; arg <= lastarg; arg++)
     {
          ...
          {                     /* LUA_NUMBER */
               ...
               status = status &&
                   fprintf(f, "%ld", lua_tonumber(L, arg)) > 0;
          }
          else
          {
               size_t          l;
...
setloc(lua_State * L)
{
     ...
     int             op =
         luaL_findstring(luaL_opt_string(L, 2, "all"), catnames);
               ...
               default:
                    {
                         if(*ar.what == 'm') /* main? */
                              ...
                              sprintf(buff, "function <%d:%.70s>",
                                      ar.linedefined, ar.short_src);
