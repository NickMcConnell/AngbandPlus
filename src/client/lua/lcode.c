...
/*
** $Id: lcode.c,v 1.1 2003/03/19 21:53:06 cipher Exp $
...
*/
static          Instruction
previous_instruction(FuncState * fs)
{
...
void
luaK_setcallreturns(FuncState * fs,
                    int nresults)
{
...
static void
luaK_patchlistaux(FuncState * fs,
                  int list,
                  int target,
                  OpCode special,
                  int special_target)
            ...
            OpCode jump)
{
     ...
     LUA_ASSERT(*previous == previous_instruction(fs),
                "no jump allowed here");
             ...
             int onlyone)
{
     ...
     {                          /* `v' is an expression? */
          ...
          if(!ISJUMP(previous) && v->u.l.f == NO_JUMP &&
             v->u.l.t == NO_JUMP)
          {
           ...
           int arg2)
{
     Instruction     i = previous_instruction(fs);
     int             delta =
         luaK_opproperties[o].push - luaK_opproperties[o].pop;
     ...
     switch (o)
     {
          ...
          case OP_ADDI:
               {
                    LUA_INTERNALERROR
                        ("instruction used only for optimizations");
                    break;
               }
          default:
               {
