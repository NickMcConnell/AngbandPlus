...
errr Term_fresh(void)
{
    ...
    if (Term->total_erase)
    {
        ...
        memset(old->va, na, w * h);
        memset(old->vc, nc, w * h);
        memset(old->vta, na, w * h);
        memset(old->vtc, nc, w * h);
    ...
    if (Term->soft_cursor)
    {
        ...
        if (!scr->cu && scr->cv && !Term->no_cursor)
        {
            ...
            {
                /* Call the big cursor display routine */
...
errr Term_clear(void)
{
    ...
    /* Wipe the content arrays */
    memset(Term->scr->va, na, w * h);
    memset(Term->scr->vc, nc, w * h);
    memset(Term->scr->vta, 0, w * h);
    memset(Term->scr->vtc, 0, w * h);
...
errr Term_info(int x, int y, byte *a, char *c, byte *ta, char *tc)
{
    int w = Term->wid;
    int h = Term->hgt;
...
static void Term_increase_queue()
{
    ...
    new_queue = C_ZNEW(Term->key_size * 2, ui_event);
