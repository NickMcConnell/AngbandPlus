...
static void store_create(void)
{
	...
	object_type object_type_body;
     ...
     fprintf(stderr, "Check for no room left.\n");
	/* Paranoia -- no room left */
	if (st_ptr->stock_num >= st_ptr->stock_size) return;


	/* Hack -- consider up to four items */
     fprintf(stderr, "Give it 4 tries.\n");
	for (tries = 0; tries < 4; tries++)
	{
          fprintf(stderr, "Try number %d.\n", tries);
		/* Black Market */
		if (store_num == STORE_B_MARKET)
		{
               fprintf(stderr, "Black market.\n");
			/* Pick a level for object/magic */
			level = 25 + rand_int(25);

			/* Random object kind (usually of given level) */
               fprintf(stderr, "Random object of level %d.\n", level);
		...
		/* Get local object */
          fprintf(stderr, "Get the object pointer.\n");
		i_ptr = &object_type_body;

		/* Create a new object of the chosen kind */
          fprintf(stderr, "Prepare an object.\n");
		object_prep(i_ptr, k_idx);

		/* Apply some "low-level" magic (no artifacts) */
          fprintf(stderr, "Apply magic.\n");
		apply_magic(i_ptr, level, FALSE, FALSE, FALSE);

		/* Hack -- Charge lite's */
		if (i_ptr->tval == TV_LITE)
		{
               fprintf(stderr, "Charge lights.\n");
		...
		/* The object is "known" */
          fprintf(stderr, "Object is known.\n");
		object_known(i_ptr);

		/* Item belongs to a store */
          fprintf(stderr, "Set item ident.\n");
		i_ptr->ident |= IDENT_STORE;

		/* Mega-Hack -- no chests in stores */
          fprintf(stderr, "Check for chest.\n");
		if (i_ptr->tval == TV_CHEST) continue;

		/* Prune the black market */
		if (store_num == STORE_B_MARKET)
		{
               fprintf(stderr, "Handle crap in the black market.\n");
		...
		{
               fprintf(stderr, "Check for worthless item.\n");
			/* No "worthless" items */
		...
		/* Mass produce and/or Apply discount */
          fprintf(stderr, "Mass produce item.\n");
		mass_produce(i_ptr);

		/* Attempt to carry the (known) object */
          fprintf(stderr, "Call store_carry.\n");
...
void store_maint(int which)
{
	...
	if (store_num == STORE_B_MARKET)
	{
          fprintf(stderr, "Prune the black market.\n");
	...
	/* Destroy objects until only "j" slots are left */
     fprintf(stderr, "Destroy objects.\n");
	...
	/* Create some new items */
     fprintf(stderr, "Create new items.\n");
