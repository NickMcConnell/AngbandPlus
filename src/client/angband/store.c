 ...
 */
static          s32b
price_item(const object_type * o_ptr,
           int greed,
           bool flip)
{
     int             factor;
 ...
 */
static          bool
store_object_similar(const object_type * o_ptr,
 ...
 */
static void
store_object_absorb(object_type * o_ptr,
                    object_type * j_ptr)
{
 ...
 */
static          bool
store_check_num(const object_type * o_ptr)
{
     int             i;
     object_type    *j_ptr;
 ...
 */
static          bool
store_will_buy_tester(const object_type * o_ptr)
{
 ...
 */
static          bool
black_market_crap(const object_type * o_ptr)
{
     int             i, j;
           ...
           int final)
{
     ...
     {
          strnfmt(buf, sizeof(buf), "%s [-%ld] ", pmt,
                  (long) (ABS(last_inc)));
     }
     ...
     while(TRUE)
     {
          ...
          if(!get_string(buf, out_val, sizeof(out_val)))
               return (FALSE);
 ...
 */
static          bool
purchase_haggle(object_type * o_ptr,
                s32b * price)
{
     ...
     *price = 0;
     ...
     for(flag = FALSE; !flag;)
     {
          ...
          {
               strnfmt(out_val, sizeof(out_val), "%s :  %ld", pmt,
                       (long) cur_ask);
               put_str(out_val, 1, 0);
               cancel = receive_offer("What do you offer? ",
                                      &offer, last_offer, 1, cur_ask,
                                      final);
...
store_purchase(void)
{
     ...
     else
     {
          ...
          object_desc(o_name, sizeof(o_name), &inventory[item_new], TRUE,
                      3);
