 ...
 */
static void
uncurse_object(object_type * o_ptr)
{
     /* Uncurse it */
 ...
 */
static          bool
item_tester_hook_weapon(const object_type * o_ptr)
{
     switch (o_ptr->tval)
     {
          case TV_SWORD:
 ...
 */
static          bool
item_tester_hook_armour(const object_type * o_ptr)
{
     switch (o_ptr->tval)
     {
          case TV_DRAG_ARMOR:
...
static          bool
item_tester_unknown(const object_type * o_ptr)
{
...
static          bool
item_tester_unknown_star(const object_type * o_ptr)
{
     if(o_ptr->ident & IDENT_MENTAL)
          return FALSE;
     else
 ...
 */
static          bool
item_tester_hook_recharge(const object_type * o_ptr)
{
     /* Recharge staffs */
          ...
          for(dx = -r; dx <= r; dx++)
          {
               ...
               if(cave_m_idx[yy][xx] > 0)
               {
                       ...
                       !(r_ptr->flags2 & (RF2_PASS_WALL)))
                    {
                         ...
                         {
                              /* Message */
                              msg_format("%^s is embedded in the rock!",
                                         m_name);
           ...
           int rad)
{
     ...
     {
          /* Analyze the "dir" and the "target".  Hurt items on floor. */
          if(project(-1, rad, ty, tx, dam, typ, flg))
               noticed = TRUE;
     }
...
curse_weapon(void)
{
     ...
     if(artifact_p(o_ptr) && (rand_int(100) < 50))
     {
          /* Cool */
          msg_format("A %s tries to %s, but your %s resists the effects!",
                     "terrible black aura", "surround your weapon",
                     o_name);
     }
 ...
 */
static          bool
item_tester_hook_ammo(const object_type * o_ptr)
{
     switch (o_ptr->tval)
     {
          case TV_BOLT:
