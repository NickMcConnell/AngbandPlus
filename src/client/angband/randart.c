...
/* $Id: randart.c,v 1.3 2003/03/17 22:45:28 cipher Exp $ */
...
artifact_power(int a_idx)
{
          ...
          case TV_SWORD:
               {
                    ...
                    if(a_ptr->flags1 & TR1_KILL_UNDEAD)
                         p = (p * 3) / 2;
                    if(a_ptr->flags1 & TR1_SLAY_ANIMAL)
                         p = (p * 4) / 3;
                    ...
                    if(a_ptr->flags1 & TR1_SLAY_ORC)
                         p = (p * 5) / 4;
                    if(a_ptr->flags1 & TR1_SLAY_GIANT)
                         p = (p * 6) / 5;

                    if(a_ptr->flags1 & TR1_BRAND_ACID)
                         p = p * 2;
     ...
     if(a_ptr->flags1 & TR1_INFRA)
          p += (a_ptr->pval + sign(a_ptr->pval)) / 2;
     if(a_ptr->flags1 & TR1_SPEED)
          p += (a_ptr->pval * 3) / 2;

     if(a_ptr->flags2 & TR2_SUST_STR)
          p += 6;
     if(a_ptr->flags2 & TR2_SUST_INT)
          p += 4;
     if(a_ptr->flags2 & TR2_SUST_WIS)
          p += 4;
     if(a_ptr->flags2 & TR2_SUST_DEX)
          p += 4;
     if(a_ptr->flags2 & TR2_SUST_CON)
          p += 4;
     if(a_ptr->flags2 & TR2_SUST_CHR)
          p += 1;
...
choose_item(int a_idx)
{
     ...
     else if(r < 9)
     {
          ...
          if(r2 < 15)
               ...
               sval = SV_GNOMISH_SHOVEL;
          else if(r2 < 90)
               sval = SV_ORCISH_PICK;
          else if(r2 < 120)
               sval = SV_DWARVEN_SHOVEL;
          else
               sval = SV_DWARVEN_PICK;
     }
     else if(r < 19)
     {
     ...
     else if(r < 33)
     {
          ...
          if(r2 < 0)
               ...
               sval = SV_RAPIER; /* or at least pointy ;-) */
          else if(r2 < 12)
               sval = SV_SMALL_SWORD;
          else if(r2 < 14)
               sval = SV_SHORT_SWORD;
     ...
     switch (a_ptr->tval)
     {
          ...
          case TV_POLEARM:
               ...
               a_ptr->to_d +=
                   (s16b) (rand_int((a_ptr->dd * a_ptr->ds) / 2 + 1));
               break;
          case TV_BOOTS:
          case TV_GLOVES:
          case TV_HELM:
 ...
 */
static void
do_pval(artifact_type * a_ptr)
{
     if(a_ptr->pval == 0)
          a_ptr->pval = (s16b) (1 + rand_int(3));
     else if(a_ptr->pval < 0)
     {
          if(rand_int(2) == 0)
               a_ptr->pval--;
...
static void
remove_contradictory(artifact_type * a_ptr)
{
     if(a_ptr->flags3 & TR3_AGGRAVATE)
 ...
 */
static void
add_ability(artifact_type * a_ptr)
{
     int             r;
               ...
               case TV_SWORD:
                    {
                         ...
                         else if(r < 89)
                         {
                              ...
                              a_ptr->to_a += (s16b) (3 + rand_int(3));
                         else if(r < 98)
                              a_ptr->weight = (a_ptr->weight * 9) / 10;
                         else if(a_ptr->tval != TV_DIGGING)
                         {
...
scramble_artifact(int a_idx)
{
     ...
     if((a_idx == ART_POWER) ||
        (a_idx == ART_GROND) || (a_idx == ART_MORGOTH))
          return;
...
static          errr
do_randart_aux(bool full)
{
     errr            result;

     /* Generate random names */
     if((result = init_names()) != 0)
          return (result);
