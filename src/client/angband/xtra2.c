 ...
 */
static int
get_coin_type(const monster_race * r_ptr)
{
...
monster_death(int m_idx)
{
     ...
     bool            good =
         (r_ptr->flags1 & (RF1_DROP_GOOD)) ? TRUE : FALSE;
     bool            great =
         (r_ptr->flags1 & (RF1_DROP_GREAT)) ? TRUE : FALSE;
             ...
             cptr note)
{
     ...
     if(m_ptr->hp < 0)
     {
          ...
          if(note)
          {
               message_format(MSG_KILL, m_ptr->r_idx, "%^s%s", m_name,
                              note);
          }
             ...
             int wx)
{
     ...
     if(p_ptr->depth == 0)
     {
          if(wy > TOWN_HGT - SCREEN_HGT)
               wy = TOWN_HGT - SCREEN_HGT;
          else if(wy < 0)
               wy = 0;
     }
     ...
     if(p_ptr->depth == 0)
     {
          if(wx > TOWN_WID - SCREEN_WID)
               wx = TOWN_WID - SCREEN_WID;
          else if(wx < 0)
               wx = 0;
     }
              ...
              int m_idx)
{
     ...
     else
     {
          ...
          if(perc >= 60)
               my_strcpy(buf,
                         (living ? "somewhat wounded" :
                          "somewhat damaged"), max);
          else if(perc >= 25)
               ...
               my_strcpy(buf,
                         (living ? "almost dead" : "almost destroyed"),
                         max);
     }
 ...
 */
static          bool
ang_sort_comp_distance(const void *u,
                       const void *v,
                       int a,
                       int b)
{
 ...
 */
static          bool
target_set_interactive_accept(int y,
                              int x)
{
     object_type    *o_ptr;
 ...
 */
static int
target_set_interactive_aux(int y,
                           int x,
                           int mode,
                           cptr info)
{
     ...
     while(1)
     {
          ...
          if(easy_floor)
          {
               ...
               floor_num =
                   scan_floor(floor_list, MAX_FLOOR_STACK, y, x, 0x02);
...
target_set_interactive(int mode)
{
     ...
     while(!done)
     {
               ...
               if(d)
               {
                    ...
                    if(scroll_target && (i < 0))
                    {
                         ...
                         if(change_panel(d))
                         {
                              ...
                              /* Find a new monster */
                              i = target_pick(old_y, old_x, ddy[d],
                                              ddx[d]);
          ...
          else
          {
               ...
               query =
                   target_set_interactive_aux(y, x, mode | TARGET_LOOK,
                                              info);
