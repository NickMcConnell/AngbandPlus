...
/* $Id: z-term.h,v 1.6 2003/03/18 22:03:34 cipher Exp $ */
...
struct term
{
     ...
     void            (*nuke_hook) (term * t);
                     ...
                     errr(*setup_input_hook) (const byte * prompt,
                                              byte * buf,
                                              int len);
};
 ...
 * The "TERM_XTRA_MUSIC" action uses "v" for the index of a video
 * The "TERM_XTRA_SCENE" action uses "v" as a "scene" value
 * The "TERM_XTRA_STAGE" action uses "v" as a scene's "stage" value
 * The "TERM_XTRA_OVERL" action uses "v" for the overlay number
 * The "TERM_XTRA_OVER0" action uses "v" for the overlay number
 * The "TERM_XTRA_OVER1" action uses "v" for the overlay number
 *
...
#define TERM_XTRA_MUSIC 14      /* Play a song (optional) */
#define TERM_XTRA_VIDEO 15      /* Play a video (optional) */
#define TERM_XTRA_SCENE 16      /* Change the display scene */
#define TERM_XTRA_STAGE 17      /* Change the display scene's stage */
#define TERM_XTRA_OVERL 18      /* Toggle the display of an overlay */
#define TERM_XTRA_OVER1 19      /* Explicitly activate an overlay */
#define TERM_XTRA_OVER0 20      /* Explicitly deactivate an overlay */
...
extern errr     Term_xtra(int n,
                          int v);

extern void     Term_queue_char(int x,
                                int y,
                                byte a,
                                char c,
                                byte ta,
                                char tc);
extern void     Term_queue_chars(int x,
                                 int y,
                                 int n,
                                 byte a,
                                 cptr s);
...
extern errr     Term_gotoxy(int x,
                            int y);
extern errr     Term_draw(int x,
                          int y,
                          byte a,
                          char c);
extern errr     Term_addch(byte a,
                           char c);
extern errr     Term_addstr(int n,
                            byte a,
                            cptr s);
extern errr     Term_putch(int x,
                           int y,
                           byte a,
                           char c);
extern errr     Term_putstr(int x,
                            int y,
                            int n,
                            byte a,
                            cptr s);
extern errr     Term_erase(int x,
                           int y,
                           int n);
extern errr     Term_clear(void);
extern errr     Term_redraw(void);
extern errr     Term_redraw_section(int x1,
                                    int y1,
                                    int x2,
                                    int y2);

extern errr     Term_get_cursor(bool * v);
extern errr     Term_get_size(int *w,
                              int *h);
extern errr     Term_locate(int *x,
                            int *y);
extern errr     Term_what(int x,
                          int y,
                          byte * a,
                          char *c);
...
extern errr     Term_inkey(char *ch,
                           bool wait,
                           bool take);
extern errr     Term_setup_input(const byte * prompt,
                                 byte * buf,
                                 int len);
...
extern errr     Term_resize(int w,
                            int h);

extern errr     Term_activate(term * t);

extern errr     term_nuke(term * t);
extern errr     term_init(term * t,
                          int w,
                          int h,
                          int k);
