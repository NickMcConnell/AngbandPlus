               ...
               bool full)
{
     ...
     if(fd >= 0)
     {
          ...
          /* Build query */
          strnfmt(out_val, sizeof(out_val), "Replace existing file %s? ",
                  buf);
     ...
     if(st_ptr->stock_num)
     {
          ...
          {
               object_desc(o_name, sizeof(o_name), &st_ptr->stock[i], TRUE,
                           3);
               fprintf(fff, "%c) %s\n", I2A(i), o_name);
          ...
          int mode)
{
     ...
     while(TRUE)
     {
          ...
          {
               /* Wait for it */
               prt("[Press Space to advance, or ESC to exit.]", hgt - 1,
                   0);
          }
...
process_player_name(bool sf)
{
     ...
     if(sf)
     {
          ...
          strnfmt(temp, sizeof(temp), "%d.%s", player_uid,
                  op_ptr->base_name);
#else
 ...
 */
static int
highscore_write(const high_score * score)
{
 ...
 */
static int
highscore_where(const high_score * score)
{
     int             i;
 ...
 */
static int
highscore_add(const high_score * score)
{
     int             i, slot;
     bool            done = FALSE;
     ...
     for(k = from, j = from, place = k + 1; k < count; k += 5)
     {
          ...
          for(n = 0; j < count && n < 5; place++, j++, n++)
          {
               ...
               for(when = the_score.day; isspace((unsigned char) *when);
                   when++) /* loop */ ;
               for(gold = the_score.gold; isspace((unsigned char) *gold);
                   gold++) /* loop */ ;
               for(aged = the_score.turns; isspace((unsigned char) *aged);
                   aged++) /* loop */ ;
               ...
               /* Append a "maximum level" */
               if(mdun > cdun)
                    my_strcat(out_val, format(" (Max %d)", mdun),
                              sizeof(out_val));
...
top_twenty(void)
{
     ...
     else
     {
          ...
          display_scores_aux(score_idx - 2, score_idx + 7, score_idx,
                             NULL);
     }
               ...
               case 'F':
                    {
                         ...
                         strnfmt(ftmp, sizeof(ftmp), "%s.txt",
                                 op_ptr->base_name);
