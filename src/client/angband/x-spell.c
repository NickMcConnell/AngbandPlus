...
/* $Id: x-spell.c,v 1.3 2003/03/17 22:45:29 cipher Exp $ */
          ...
          switch (spell)
          {
               ...
               case PRAYER_ORB_OF_DRAINING:
                    sprintf(p, " %d+3d6", plev +
                            (plev /
                             ((cp_ptr->flags & CF_BLESS_WEAPON) ? 2 : 4)));
                    break;
...
cast_priest_spell(int spell)
{
     ...
     switch (spell)
     {
          ...
          case PRAYER_ENCHANT_WEAPON:
               {
                    (void) enchant_spell(rand_int(4) + 1, rand_int(4) + 1,
                                         0);
                    break;
               }
