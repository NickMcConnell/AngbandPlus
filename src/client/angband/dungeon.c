 ...
 */
static int
value_check_aux1(const object_type * o_ptr)
{
     /* Artifacts */
     if(artifact_p(o_ptr))
     {
 ...
 */
static int
value_check_aux2(const object_type * o_ptr)
{
...
sense_inventory(void)
{
     ...
     int             plev = p_ptr->lev;
     bool            heavy =
         ((cp_ptr->flags & CF_PSEUDO_ID_HEAVY) ? TRUE : FALSE);
     int             feel;
     object_type    *o_ptr;
     char            o_name[80];
 ...
 /*** Check for "sensing" ***/
     ...
     {
          if(0 !=
             rand_int(cp_ptr->sense_base /
                      (plev * plev + cp_ptr->sense_div)))
               return;
     }
     else
     {
          if(0 !=
             rand_int(cp_ptr->sense_base / (plev + cp_ptr->sense_div)))
               return;
     }
 ...
 /*** Sense everything ***/
     ...
     for(i = 0; i < INVEN_TOTAL; i++)
     {
          ...
          {
               msg_format
                   ("You feel the %s (%c) in your pack %s %s...",
...
process_world(void)
{
 ...
 /*** Poison and Stun and Cut ***/
     ...
     {
          int             adjust =
              (adj_con_fix[p_ptr->stat_ind[A_CON]] + 1);
...
process_command(void)
{
     ...
     if(process_command_hook(p_ptr->command_cmd))
          return;
