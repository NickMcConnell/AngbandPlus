 ...
 */
static void
display_parse_error(cptr filename,
                    errr err,
                    cptr buf)
{
     cptr            oops;
  ...
  /*** Dump the binary image file ***/
          ...
          if(fd < 0)
          {
               ...
               if(fd < 0)
               {
                    ...
                    /* Message */
                    strnfmt(why, sizeof(why),
                            "Cannot create the '%s' file!", buf);
...
init_b_info(void)
{
     ...
     /* Init the header */
     init_header(&b_head, (u16b) (MAX_STORES * z_info->b_max),
                 sizeof(owner_type));
...
init_g_info(void)
{
     ...
     /* Init the header */
     init_header(&g_head, (u16b) (z_info->p_max * z_info->p_max),
                 sizeof(byte));
...
init_angband(void)
{
 ...
 /*** Initialize some arrays ***/
     ...
     if(script_init())
          quit("Cannot initialize scripts");
