 ...
 */
static          bool
wearable_p(const object_type * o_ptr)
{
     /* Valid "tval" codes */
     switch (o_ptr->tval)
     {
...
rd_dungeon(void)
{
 ...
 /*** Basic info ***/
     ...
     if((ymax != DUNGEON_HGT) || (xmax != DUNGEON_WID))
     {
          /* XXX XXX XXX */
          note(format
               ("Ignoring illegal dungeon size (%d,%d).", ymax, xmax));
          return (0);
     }
     ...
     {
          note(format
               ("Ignoring illegal player location (%d,%d).", py, px));
          return (1);
     }
...
load_player(void)
{
     ...
     if(!err)
     {
          ...
          if((version_major != sf_major) ||
             (version_minor != sf_minor) || (version_patch != sf_patch))
          {
               /* Message */
