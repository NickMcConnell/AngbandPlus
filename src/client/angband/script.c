...
static int
xxx_build_script_path(lua_State * L)
{
     char            buf[1024];
     cptr            filename;
 ...
 *
 * Calls a Lua event handler with the name in 'hook',
     ...
     while(fmt[i])
     {
          switch (fmt[i++])
          {
               ...
               case 's':
                    ...
                    break;
               case 'O':
                    tolua_pushusertype(L, va_arg(ap, object_type *),
                                       tolua_tag(L, "object_type"));
                    num_args++;
                    break;
               case '(':
               case ')':
               ...
               case 'O':
                    {
                         ...
                         if(tolua_istype
                            (L, -num_res + i, tolua_tag(L, "object_type"),
                             0))
                              *tmp =
                                  tolua_getuserdata(L, -num_res + i, NULL);
                         break;
                    }
          }
     }
           ...
           bool * ident)
{
     ...
     if(!call_hook("use_object", "(O)", "bb", o_ptr, ident, &used_up))
     {
          /* Error */
          *ident = FALSE;
          used_up = FALSE;
...
int
get_spell_index(const object_type * o_ptr,
                int index)
{
     int             spell = -1;

     if(!call_hook("get_spell_index", "(O,d)", "d", o_ptr, index, &spell))
     {
          /* Error */
          return -1;
     }
               ...
               int index)
{
     ...
     if(!call_hook("get_spell_name", "(d,d)", "s", tval, index, &name))
     {
          /* Error */
          return "";
     }
               ...
               int index)
{
     ...
     if(!call_hook("get_spell_info", "(d,d)", "s", tval, index, &info))
     {
          /* Error */
          return "";
     }
           ...
           int index)
{
     ...
     if(!call_hook("cast_spell", "(d,d)", "b", tval, index, &result))
     {
          /* Error */
          return FALSE;
     }
...
describe_item_activation(const object_type * o_ptr)
{
     ...
     if(call_hook("describe_item_activation", "(O)", "s", o_ptr, &desc))
     {
          if(desc)
               text_out(desc);
     }
 ...
 */
int
get_store_choice(int store_num)
{
     int             choice = -1;

     if(!call_hook("get_store_choice", "(d)", "d", store_num, &choice))
     {
          /* Error */
          return -1;
     }
 ...
 */
bool
store_will_buy(int store_num,
               const object_type * o_ptr)
{
     bool            result = FALSE;

     if(!call_hook("store_will_buy", "(d,O)", "b",
                   store_num, o_ptr, &result))
     {
          /* Error */
          return FALSE;
     }
...
void
player_birth_done_hook(void)
{
     call_hook("player_birth_done", "", "");
}

void
player_calc_bonus_hook(void)
{
     call_hook("player_calc_bonus", "", "");
}

void
start_game_hook(void)
{
     call_hook("start_game", "", "");
}

void
enter_level_hook(void)
{
     call_hook("enter_level", "", "");
}

void
leave_level_hook(void)
{
     call_hook("leave_level", "", "");
}

bool
process_command_hook(int command)
{
     int             result = 0;

     if(!call_hook("process_command", "(d)", "d", command, &result))
     {
          /* Error */
          return FALSE;
     }
...
bool
generate_level_hook(int level)
{
     int             result = 0;

     if(!call_hook("generate_level", "(d)", "d", level, &result))
     {
          /* Error */
          return FALSE;
     }
          ...
          lua_Debug * ar)
{
     ...
     for(j = 0; j < ANGBAND_TERM_MAX; j++)
     {
          ...
          if(op_ptr->window_flag[j] & PW_SCRIPT_SOURCE)
          {
               ...
               lua_getstack(L, 0, ar);
               lua_getinfo(L, "S", ar);
               show_file(ar->source + 1, ar->short_src,
                         ar->currentline - 1, 1);
...
bool
script_do_string(cptr script)
{
     if(!L)
          return FALSE;

     if(!lua_dostring(L, script))
          return TRUE;
