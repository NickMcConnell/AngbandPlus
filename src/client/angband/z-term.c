...
/*
 * Execute the "Term->input_hook" hook, if available (see above).
 */

errr
Term_setup_input(const byte * prompt,
                 byte * buf,
                 int len)
{
     /* Verify the hook */
     if(!Term->setup_input_hook)
          return (-1);

     /* Call the hook */
     return ((*Term->setup_input_hook) (prompt, buf, len));
}
...
Term_fresh(void)
{
     ...
     if((y1 > y2) &&
        ...
        (scr->cv == old->cv) &&
        (scr->cx == old->cx) &&
        (scr->cy == old->cy) && !(Term->total_erase))
     {
          /* Nothing */
          return (1);
     }
