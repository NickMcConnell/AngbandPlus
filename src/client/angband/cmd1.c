 ...
 */
static          bool
auto_pickup_okay(const object_type * o_ptr)
{
     cptr            s;
...
py_pickup(int pickup)
{
     ...
     if(easy_floor && (can_pickup + not_pickup > 0))
     {
          ...
          if(!pickup)
          {
               ...
               {
                    /* Message */
                    msg_format("You see a pile of %d objects.",
                               not_pickup);
               }
          ...
          case FEAT_TRAP_HEAD + 0x0C:
               {
                    ...
                    {
                         (void) set_blind(p_ptr->blind + rand_int(50) +
                                          25);
                    }
                    break;
               }
          ...
          int x)
{
     ...
     while(num++ < p_ptr->num_blow)
     {
          ...
          {
               /* Message */
               message_format(MSG_HIT, m_ptr->r_idx, "You hit %s.",
                              m_name);
          ...
          {
               /* Message */
               message_format(MSG_MISS, m_ptr->r_idx, "You miss %s.",
                              m_name);
          }
     }
...
run_step(int dir)
{
     ...
     if(dir)
     {
          ...
          p_ptr->running =
              (p_ptr->command_arg ? p_ptr->command_arg : 1000);
