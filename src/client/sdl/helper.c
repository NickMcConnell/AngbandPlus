...
#ifdef DEBUG
     fprintf(stderr, "Checking rectangle position of rect (%d, %d, %d, %d) against (%d,%d)\n",
                   ...
                   SDL_Rect *dr)
{
     ...
     for(ty = dr->y, sy = (float)sr->y;
         ty < ly;
         ++ty, sy+=dsy)
     {
          for(tx = dr->x, sx = (float)sr->x;
              tx < lx;
              ++tx, sx+=dsx)
          {
               rs = gs = bs = 0.0;
                       ...
                       SDL_Rect *dr)
{
      ...
      */
     for (ty = dr->y, sy = sr->y << 16;
          ty < ly;
          ++ty, sy+=dsy)
     {
          for (tx = dr->x, sx = sr->x << 16;
               tx < lx;
               ++tx, sx+=dsx)
          {
               rs = gs = bs = 0;
...
errr SDL_StretchBlit(SDL_Surface *src,
                     SDL_Rect *sr,
                     SDL_Surface *dst,
                     SDL_Rect *dr)
{
