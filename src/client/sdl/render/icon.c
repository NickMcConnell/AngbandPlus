...
static struct IconObjectInit
{
     char *name;
     int object;
};

static struct IconObjectPos
{
     int object;
     int dir, anchor;
...
{
     { IH_SCENE_OBJECT_CHARACTER, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 0 },
     { IH_SCENE_OBJECT_ARMOR_CLASS, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 1 },
     { IH_SCENE_OBJECT_CHARACTER_LEVEL, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 2 },
     { IH_SCENE_OBJECT_STRENGTH, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 3 },
     { IH_SCENE_OBJECT_INTELLIGENCE, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 4 },
     { IH_SCENE_OBJECT_WISDOM, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 5 },
     { IH_SCENE_OBJECT_DEXTERITY, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 6 },
     { IH_SCENE_OBJECT_CONSTITUTION, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 7 },
     { IH_SCENE_OBJECT_CHARISMA, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 8 },
     { IH_SCENE_OBJECT_GOLD, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 9 },
     { IH_SCENE_OBJECT_BURDEN, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPLEFT, 0, 10 },
     { IH_SCENE_OBJECT_INVENTORY, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_TOPRIGHT, 0, 0 },
     { IH_SCENE_OBJECT_INVENTORY_SLOT, IH_ICON_DIR_DOWN, IH_ICON_ANCHOR_TOPRIGHT, 0, 1 },
     { IH_SCENE_OBJECT_SPELL_BOOK, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_TOPRIGHT, 1, 0 },
     { IH_SCENE_OBJECT_DEPTH, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 0, 0 },
     { IH_SCENE_OBJECT_HUNGRY, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 1, 0 },
     { IH_SCENE_OBJECT_GRAZE, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 2, 0 },
     { IH_SCENE_OBJECT_BLIND, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 3, 0 },
     { IH_SCENE_OBJECT_CONFUSED, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 4, 0 },
     { IH_SCENE_OBJECT_AFRAID, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 5, 0 },
     { IH_SCENE_OBJECT_POISONED, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 6, 0 },
     { IH_SCENE_OBJECT_STUN, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 7, 0 },
     { IH_SCENE_OBJECT_STUDY, IH_ICON_DIR_LEFT, IH_ICON_ANCHOR_BOTTOMRIGHT, 8, 0 },
     { IH_SCENE_OBJECT_END }
};
...
errr IH_LoadIcons(void)
{
     errr rc = 0;
     cptr path_data;
     cptr path_general; // general icons; regardless of selected size
     cptr size;
     cptr path_size; // icons of the user-selected size
     int i;
         ...
         i++)
     {
          ...
          filename_len = strlen(size_icons[i].name) + strlen(IH_IMAGE_FORMAT_EXT) + 2;
          ...
          if(file)
          {
               ...
               {
#ifdef DEBUG
                    fprintf(stderr, "SceneObject allocated.\n");
#endif
                    IH_ListAppend(&ih.icons, (void *)object);

                    object->data.image = IMG_Load_RW(SDL_RWFromFile(file, "rb"), 1);
...
void IH_PositionIcons(void)
{
         ...
         node = IH_ListNext(&ih.icons, node))
     {
          ...
          for(i = 0;
              icon_pos[i].object != IH_SCENE_OBJECT_END;
              i++)
          {
          ...
          object->x = base_x + (pos->x_offset * icon_size * dir_x) + (slot_x * icon_size * dir_x);
#ifdef DEBUG
          fprintf(stderr, "object->x = %d\n", object->x);
#endif
          
          object->y = base_y + (pos->y_offset * icon_size * dir_y) + (slot_y * icon_size * dir_y);
#ifdef DEBUG
...
#ifdef DEBUG
               fprintf(stderr, "Blitting icon %d.\n", object->object);
#endif
               SDL_BlitSurface(object->data.image, NULL,
                               ih.screen, &rect);
          }
     }          
}
