...
errr IH_LoadPointers(void)
{
         ...
         i++)
     {
          ...
          my_strcat(pointer_name, pointers_init[i].name, sizeof(pointer_name) - 1);
          my_strcat(pointer_name, "." IH_IMAGE_FORMAT_EXT, sizeof(pointer_name) - 1);
          fprintf(stderr, "pointer_name = %s\n", pointer_name);
          
          path_pointer = IH_PathBuild(path_misc, pointer_name, NULL);
          fprintf(stderr, "path_pointer = %s\n", path_pointer);

          pointers[pointers_init[i].array_pos] = IMG_Load_RW(SDL_RWFromFile(path_pointer, "rb"), 1);
