...
errr IH_LoadLMX(void)
{
     ...
     errr rc = 0;
     cptr path_data;
     cptr path_lmx, path_xp_bar, path_life_bar, path_mana_bar;
     ...
     /* Load the decoration.
      */
     path_lmx = IH_PathBuild(path_data, "lmx-deco." IH_IMAGE_FORMAT_EXT, NULL);
#ifdef DEBUG
     fprintf(stderr, "path_lmx = %s\n", path_lmx);
#endif
     ...
     if(object)
     {
          ...
          object->data.image = IMG_Load_RW(SDL_RWFromFile(path_lmx, "rb"), 1);
     ...
     /* Load the decoration.
      */
     path_life_bar = IH_PathBuild(path_data, "gfx", "life-bar." IH_IMAGE_FORMAT_EXT, NULL);
#ifdef DEBUG
     fprintf(stderr, "path_life_bar = %s\n", path_life_bar);
#endif
     ...
     if(object)
     {
          ...
          object->data.image = IMG_Load_RW(SDL_RWFromFile(path_life_bar, "rb"), 1);
          if(!object->data.image)
          {
               fprintf(stderr, "Unable to load icon image: %s: %s\n",
                       path_life_bar,
     ...
     /* Load the decoration.
      */
     path_mana_bar = IH_PathBuild(path_data, "gfx", "mana-bar." IH_IMAGE_FORMAT_EXT, NULL);
#ifdef DEBUG
     fprintf(stderr, "path_mana_bar = %s\n", path_mana_bar);
#endif
     ...
     if(object)
     {
          ...
          object->data.image = IMG_Load_RW(SDL_RWFromFile(path_mana_bar, "rb"), 1);
          if(!object->data.image)
          {
               fprintf(stderr, "Unable to load icon image: %s: %s\n",
                       path_mana_bar,
     ...
     /* Load the decoration.
      */
     path_xp_bar = IH_PathBuild(path_data, "gfx", "xp-bar." IH_IMAGE_FORMAT_EXT, NULL);
#ifdef DEBUG
     fprintf(stderr, "path_xp_bar = %s\n", path_xp_bar);
#endif
     ...
     if(object)
     {
          ...
          object->data.image = IMG_Load_RW(SDL_RWFromFile(path_xp_bar, "rb"), 1);
          if(!object->data.image)
          {
               fprintf(stderr, "Unable to load icon image: %s: %s\n",
                       path_xp_bar,
...
errr IH_LoadImages(void)
{
      ...
      */
     {
          ...
          file = IH_PathBuild(path_images, "splash." IH_IMAGE_FORMAT_EXT, NULL);
          ...
          file = IH_PathBuild(path_images, "background." IH_IMAGE_FORMAT_EXT, NULL);
         ...
         i++)
     {
          ...
          filename_len = strlen(graphics[i].name) + strlen(IH_IMAGE_FORMAT_EXT) + 2;
         ...
         node = IH_ListNext(&ih.misc, node))
     {
          ...
          switch(object->object)
          {
               case IH_SCENE_OBJECT_LMXDECO:
                    lmx_x = object->x = (ih.display_width - object->data.image->w) / 2;
                    object->y = 0;
                    break;
         ...
         node = IH_ListNext(&ih.misc, node))
     {
          ...
          switch(object->object)
          {
               case IH_SCENE_OBJECT_LIFE:
                    ...
                    perc = (p_ptr->chp / p_ptr->mhp) * 100;
                    break;

               case IH_SCENE_OBJECT_MANA:
                    perc = (p_ptr->csp / p_ptr->msp) * 100;
                    break;

               case IH_SCENE_OBJECT_XP:
                    perc = (p_ptr->exp / p_ptr->max_exp) * 100;
                    break;
          }
          srect.w *= perc;

#ifdef DEBUG
          fprintf(stderr, "Blitting misc object %d.\n", object->object);
#endif
          SDL_BlitSurface(object->data.image, &srect,
                          ih.screen, &drect);
