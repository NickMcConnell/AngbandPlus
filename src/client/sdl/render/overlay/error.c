...
/* $Id: error.c,v 1.2 2003/03/18 19:17:42 cipher Exp $ */
...
IH_RenderOverlay_Error(Overlay * overlay)
{
     ...
     if(!overlay->position.x)
     {
          ...
          overlay->gfx.error.w = widest + 6;
          overlay->gfx.error.h =
              (IH_FONT_NORMAL_SIZE * (overlay->gfx.error.n_lines + 1)) + 6;

          overlay->position.x =
              (ih.display_width - overlay->gfx.error.w) / 2;
          overlay->position.y =
              (ih.display_height - overlay->gfx.error.h) / 2;
     }
     ...
     if(ih.input_buffer)
     {
          ...
          while(1)
          {
                ...
                */
               if(width <= overlay->gfx.error.w - 6)
               {
                    SDL_Color       color;
                    ihFontPos       pos;
