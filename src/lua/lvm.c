...
/*
** $Id: lvm.c,v 1.1 2003/04/09 00:19:01 cipher Exp $
** Lua virtual machine
              ...
              StkId t)
{
     ...
     {                          /* or no TM? */
          ...
          if(ttype(h) != LUA_TNIL ||
             ((tm = luaT_gettm(L, tg, TM_INDEX)) == NULL))
     ...
     for(;;)
     {
          ...
          switch (GET_OPCODE(i))
          {
               ...
               case OP_FORPREP:
                    {
                         ...
                         if(nvalue(top - 1) > 0 ? nvalue(top - 3) >
                            nvalue(top - 2) : nvalue(top - 3) <
                            nvalue(top - 2))
                         {      /* `empty' loop? */
               ...
               case OP_CLOSURE:
                    {
                         L->top = top;
                         luaV_Lclosure(L, tf->kproto[GETARG_A(i)],
                                       GETARG_B(i));
                         top = L->top;
                         luaC_checkGC(L);
                         break;
