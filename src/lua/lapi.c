...
/*
** $Id: lapi.c,v 1.1 2003/04/09 00:19:01 cipher Exp $
** Lua API
...
static TObject *
luaA_indexAcceptable(lua_State * L,
                     int index)
{
     if(index >= 0)
     {
...
LUA_API void
lua_setgcthreshold(lua_State * L,
                   int newthreshold)
{
...
LUA_API void
lua_unref(lua_State * L,
          int ref)
{
     if(ref >= 0)
     {
          LUA_ASSERT(ref < L->refSize &&
                     L->refArray[ref].st < 0, "invalid ref");
     ...
     else
     {
          ...
          while(i--)
          {
               if(ttype(key(n)) == LUA_TNUMBER &&
                  ttype(val(n)) != LUA_TNIL && nvalue(key(n)) > max)
                    max = nvalue(key(n));
               n++;
          }
          return (int) max;
