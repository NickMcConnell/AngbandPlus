...
/*
** $Id: lparser.c,v 1.1 2003/04/09 00:19:01 cipher Exp $
...
static int
checkname(LexState * ls)
{
     return string_constant(ls->fs, str_checkname(ls));
}

static int
luaI_registerlocalvar(LexState * ls,
                      TString * varname)
{
     Proto          *f = ls->fs->f;

     luaM_growvector(ls->L, f->locvars, f->nlocvars, 1, LocVar, "",
                     MAX_INT);
...
static void
new_localvar(LexState * ls,
             TString * name,
             int n)
{
     FuncState      *fs = ls->fs;

     luaX_checklimit(ls, fs->nactloc + n + 1, MAXLOCALS,
                     "local variables");
     fs->actloc[fs->nactloc + n] = luaI_registerlocalvar(ls, name);
}

static void
adjustlocalvars(LexState * ls,
                int nvars)
{
     FuncState      *fs = ls->fs;
          ...
          expdesc * var)
{
     ...
     if(level >= 1)             /* neither local (0) nor global (-1)? */
          luaX_syntaxerror(ls, "cannot access a variable in outer scope",
                           n->str);
             ...
             expdesc * v)
{
     ...
     for(i = 0; i < fs->nupvalues; i++)
     {
          if(fs->upvalues[i].k == v->k &&
             fs->upvalues[i].u.index == v->u.index)
               return i;
     }
     /* new one */
            ...
            TString * n)
{
     ...
     {                          /* global? */
          if(fs->prev == NULL)
               luaX_syntaxerror(ls, "cannot access upvalue in main",
                                n->str);
...
static void
adjust_mult_assign(LexState * ls,
                   int nvars,
                   int nexps)
{
     FuncState      *fs = ls->fs;
...
listfields(LexState * ls)
{
     ...
     while(ls->t.token == ',')
     {
          ...
          if(n % LFIELDS_PER_FLUSH == 0)
               luaK_code2(fs, OP_SETLIST, n / LFIELDS_PER_FLUSH - 1,
                          LFIELDS_PER_FLUSH);
     }
     luaK_code2(fs, OP_SETLIST, n / LFIELDS_PER_FLUSH,
                n % LFIELDS_PER_FLUSH);
     return n;
}

static void
constructor_part(LexState * ls,
                 Constdesc * cd)
{
     switch (ls->t.token)
     {
...
constructor(LexState * ls)
{
     ...
     if(optional(ls, ';'))
     {
          ...
          check_condition(ls, (cd.k != other_cd.k),
                          "invalid constructor syntax");
          nelems += other_cd.n;
     }
     check_match(ls, '}', '{', line);
     luaX_checklimit(ls, nelems, MAXARG_U,
        ...
        TString * indexname)
{
     ...
     check_condition(ls,
                     (ls->t.token == TK_NAME &&
                      ls->t.seminfo.ts == luaS_new(ls->L, "in")),
                     "`in' expected");
     next(ls);                  /* skip `in' */
     exp1(ls);                  /* table */
