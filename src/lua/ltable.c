...
/*
** $Id: ltable.c,v 1.1 2003/04/09 00:19:01 cipher Exp $
** Lua tables (hash)
...
*/
Node           *
luaH_mainposition(const Hash * t,
                  const TObject * key)
{
     unsigned long   h;
     ...
     LUA_ASSERT(h % (unsigned int) t->size ==
                (h & ((unsigned int) t->size - 1)),
                "a&(x-1) == a%x, for x power of 2");
     return &t->node[h & (t->size - 1)];
}

static const TObject *
luaH_getany(lua_State * L,
            const Hash * t,
            const TObject * key)
{
...
const TObject  *
luaH_getnum(const Hash * t,
            Number key)
{
     Node           *n =
         &t->node[(unsigned long) (long) key & (t->size - 1)];
     do
     {
          ...
          const TObject * key)
{
     ...
     else
     {
          ...
          i = (int) (((const char *) v -
                      (const char *) (&t->node[0].val)) / sizeof(Node)) +
              1;
     }
     for(; i < t->size; i++)
     {
