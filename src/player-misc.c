...
/* player-misc.c: screen refreshing etc. */
...
static void prt_title(void)
{
	...
	/* Possessor */
	else if (p_ptr->pclass != p_ptr->prealclass)
	{
	    	p = "(Possessed)";
	}
...
static void prt_equippy(void)
{
	...
	if (use_bigtile) return;
        ...
        /* XXX XXX XXX Disabled: twice as more equipment slots in posband */
        return;
	...
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
		...
		/* Dump */
		Term_putch(COL_EQUIPPY + i - INVEN_EQUIP, ROW_EQUIPPY, a, c);
	}
}
...
static void prt_sp(void)
{
	...
	/* Do not show mana if we're a monster and have no powers */
	if (cp_ptr->spell_book == TV_MONSTER_BOOK && !r_info[p_ptr->m_r_idx].body.pwr[0].type) return;
...
static void prt_frame_basic(void)
{
	...
	/* Race and Class */
	/* Hack -- Wrap long (monster) race name. */
	rp = p_name + rp_ptr->name;
	if (strlen(rp) < 13)
	{
	...
	else
	{
		...
		else
		{
			*rp = '\0';
			...
			prt_field(rp, ROW_CLASS, COL_CLASS);
		}
	}
	if (p_ptr->pclass != MON_CLASS)
	{
...
static void calc_spells(void)
{
	...
	if (!cp_ptr->spell_book) return;
        ...
        /* Hack -- must not be monster */
        if (cp_ptr->spell_book == 99) return;
...
static void calc_mana(void)
{
	...
	/* Extract total mana */
        /* Hack - for monsters, calculate mana based on body stat */
        if (cp_ptr->spell_book == 99)
                msp = (long)adj_mag_mana[p_ptr->stat_ind[r_info[p_ptr->m_r_idx].body.pwrstat]] * levels / 100;
        else
	...
	if (cp_ptr->flags & CF_CUMBER_GLOVE)
	{
		...
		/* Chech all the gloves */
		for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
		{
		    	if (!(r_ptr->body.glove_mask & EQUIP_SLOT(i))) continue;
	...
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	if ((r_ptr->body.body_mask | r_ptr->body.helm_mask | 
		     r_ptr->body.shield_mask | r_ptr->body.cloak_mask |
		     r_ptr->body.glove_mask | r_ptr->body.boot_mask) & EQUIP_SLOT(i))
		{
		    	cur_wgt += inventory[i].weight;
		}
	}
...
static void calc_torch(void)
{
	...
	if (p_ptr->lite) p_ptr->cur_lite++;
        ...
        /* -1 to light radius for nether beings */
        if (p_ptr->undead && p_ptr->cur_lite) p_ptr->cur_lite--;
...
/*
 * "Analyze" current missile launcher,
 * and extract various stats from it.
 */
void analyze_launcher(object_type *o_ptr)
{
	int extra_might = 0;
	u32b f1, f2, f3, f4;
...
/*
 * Evolve our body. Shows the menu if there is >1 possible form.
 */
static void evolve(void)
{
	monster_race *r_ptr = &r_info[p_ptr->m_r_idx];
	monster_body *b_ptr = &r_ptr->body;
	char notebuf[80];
	object_type temp[INVEN_TOTAL];
	int i, k = 0;

	/*
	 * The tricky part here is equipment. Slot configuration
	 * for the new form could be different. If we just preserve
	 * the old equipment intact, some items may end up in wrong
	 * slots.  Here, we silently take off everything, and
	 * then re-equip.
	 */
	
	/* Clear the temporary storage for items */
	C_WIPE(temp, INVEN_TOTAL, object_type);

	/* Move the old equipment in this storage */
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	/* If the slot exists, and is not empty ... */
	    	if ((r_ptr->body.slot_mask & EQUIP_SLOT(i)) && inventory[i].number)
		{
		    	/* ... copy the object from inventory to temp array ... */
		   	object_copy(&temp[k++], &inventory[i]);
			/* ... and delete it from the inventory */
		    	inven_item_increase(i, -99);
			inven_item_optimize(i);
		}
	}
	p_ptr->total_weight = p_ptr->equip_cnt = 0;    

	if (!b_ptr->evolve_index[1])
	{
		/* No choice */
		p_ptr->m_r_idx = b_ptr->evolve_index[0];
	}
	else
	{
		char key;
...
repeat:
		...
		for (i = 0; i < 6; i++)
		{
			if (!b_ptr->evolve_index[i]) break;
			prt(format(" %c) %s", I2A(i), r_name + r_info[b_ptr->evolve_index[i]].name), i + 3, 20);
		}
		prt("", i + 3, 20);
		prt(" h) Help", i + 4, 20);
		prt("", i + 5, 20);
		...
		while (1)
		{
			...
			if (key == 'h')
			{
				strnfmt(buf, sizeof(buf), "monrace.txt#%s", p_name + rp_ptr->name);
		...
		p_ptr->m_r_idx = b_ptr->evolve_index[A2I(key)];
	}
	
	r_ptr = &r_info[p_ptr->m_r_idx];

	/* Give the message and note the evolution */
	
	msg_format("You become a%s %s!", is_a_vowel((r_name + r_ptr->name)[0]) ? "n" : "",
	    r_name + r_ptr->name);

	sprintf(notebuf, "New form: %s", r_name + r_ptr->name);
	do_cmd_note(notebuf, p_ptr->depth);

	/* Re-equip all items */
	for (i = 0; i < k; i++)
	{
	    	int slot = wield_slot(&temp[i]);

		if (inventory[slot].number)
		{
			/* Slot is (all slots are) already occupied */
		    	char o_name[120];
			object_desc(o_name, sizeof(o_name), &temp[i], FALSE, 0);
			msg_format("You cannot equip your %s in your new body.", o_name);
			inven_carry(&temp[i]);
		}
		else
		{
		    	/* Copy the item in the inventory (manually) */
		    	object_copy(&inventory[slot], &temp[i]);
			p_ptr->total_weight += inventory[slot].weight;
			p_ptr->equip_cnt++;
		}
	}
}          
...
static void calc_bonuses(void)
{
	...
	/* Reset "blow" info */
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++) p_ptr->num_blow[i] = 1;
	extra_blows = 0;
	extra_shots = 0;
	...
	for (i = 0; i < A_MAX; i++) p_ptr->stat_add[i] = 0;
        ...
        /* Clear the skills */
        p_ptr->skill_dis = p_ptr->skill_dev = p_ptr->skill_sav = 0;
        p_ptr->skill_stl = p_ptr->skill_srh = p_ptr->skill_fos = 0;
        p_ptr->skill_thn_bare = p_ptr->skill_thn_edged = p_ptr->skill_thn_hafted = 0;
	p_ptr->skill_thn_polearm = p_ptr->skill_thn_axe = p_ptr->skill_thb_sling = 0;
	p_ptr->skill_thb_bow = p_ptr->skill_thb_xbow = p_ptr->skill_tht = 0;
	...
	p_ptr->cold_touch = FALSE;
	p_ptr->fire_touch = FALSE;
	p_ptr->acid_touch = FALSE;
	p_ptr->elec_touch = FALSE;
	p_ptr->pois_blood = FALSE;
	p_ptr->aura_fear = FALSE;
	...
	p_ptr->al_special = 0;
        ...
        {
        	int level;
		monster_lore *l_ptr = &l_list[p_ptr->m_r_idx];

        	/* First of all, evolve if needed */
	        /* Hack -- disable for possessors! */
	        if (p_ptr->lev >= r_ptr->body.evolve_level &&
		    b_ptr->evolve_index[0] && p_ptr->prealclass != POSS_CLASS)
		{
		    	evolve();
	         	r_ptr = &r_info[p_ptr->m_r_idx];
			b_ptr = &r_ptr->body;
	        ...
	        /* Adjustment for armor and speed */
	        if (p_ptr->lev <= b_ptr->base_level)
        		level = 0;
	        else if (p_ptr->lev >= b_ptr->evolve_level)
        	        level = 100;
	        else
        	        level = 100 * (p_ptr->lev - b_ptr->base_level) /
        	            (b_ptr->evolve_level - b_ptr->base_level);
        
		/* Mega-Hack -- adjust rp_ptr! */
		
	        /* Infravision */
         	rp_ptr->infra = b_ptr->infra;

		/* Emulated class */
		if (b_ptr->class)
		{
			p_ptr->pclass = b_ptr->class - 1;
			cp_ptr = &c_info[p_ptr->pclass];
			mp_ptr = &cp_ptr->spells;
		}

         	/* Abilities */
	        rp_ptr->r_dis = b_ptr->dis;
	        rp_ptr->r_dev = b_ptr->dev;
        	rp_ptr->r_sav = b_ptr->sav;
        	rp_ptr->r_stl = b_ptr->stl;
	        rp_ptr->r_srh = b_ptr->srh;
	        rp_ptr->r_fos = b_ptr->fos;
        	rp_ptr->r_thn_bare    = b_ptr->thn_bare;
        	rp_ptr->r_thn_edged   = b_ptr->thn_edged;
        	rp_ptr->r_thn_hafted  = b_ptr->thn_hafted;
        	rp_ptr->r_thn_polearm = b_ptr->thn_polearm;
        	rp_ptr->r_thn_axe     = b_ptr->thn_axe;
        	rp_ptr->r_thb_sling   = b_ptr->thb_sling;
        	rp_ptr->r_thb_bow     = b_ptr->thb_bow;
        	rp_ptr->r_thb_xbow    = b_ptr->thb_xbow;
        	rp_ptr->r_tht         = b_ptr->tht;
                    
	        /* Hack -- Adjust the preloaded p_info array */
	        rp_ptr->r_adj[A_STR] = b_ptr->to_stat[A_STR];
  	        rp_ptr->r_adj[A_INT] = b_ptr->to_stat[A_INT];
	        rp_ptr->r_adj[A_WIS] = b_ptr->to_stat[A_WIS];
        	rp_ptr->r_adj[A_DEX] = b_ptr->to_stat[A_DEX];
 	        rp_ptr->r_adj[A_CON] = b_ptr->to_stat[A_CON];
  	        rp_ptr->r_adj[A_CHR] = b_ptr->to_stat[A_CHR];
  	        
  	        /* We may need to reroll the hit points */
  	        /* TODO: Move the hit points reroll in a separate function */
        	if (p_ptr->hitdie != b_ptr->hitdie)
        	{
 		       	int min_value, max_value, i;

		        p_ptr->hitdie = rp_ptr->r_mhp = b_ptr->hitdie;
		...
		{
			/* No need to reroll from the beginning */
		        p_ptr->hitdie = rp_ptr->r_mhp = b_ptr->hitdie;
		}
		
		/* Flags */
		rp_ptr->flags2 = b_ptr->flags2;
		rp_ptr->flags3 = b_ptr->flags3;
		rp_ptr->flags4 = b_ptr->flags4;

		p_ptr->ac = p_ptr->dis_ac = b_ptr->to_ac_base +
		    (b_ptr->to_ac_max - b_ptr->to_ac_base) * level / 100;
		p_ptr->pspeed += b_ptr->to_spd_base +
		    (b_ptr->to_spd_max - b_ptr->to_spd_base) * level / 100;

		/* Ensure that we know everything about our body */
		...
		{
			if (r_ptr->blow[i].effect || r_ptr->blow[i].method) l_ptr->blows[i] = MAX_UCHAR;
		}
	...
	p_ptr->skill_thn_bare += rp_ptr->r_thn_bare + cp_ptr->c_thn_bare;
	p_ptr->skill_thn_edged += rp_ptr->r_thn_edged + cp_ptr->c_thn_edged;
	p_ptr->skill_thn_hafted += rp_ptr->r_thn_hafted + cp_ptr->c_thn_hafted;
	p_ptr->skill_thn_polearm += rp_ptr->r_thn_polearm + cp_ptr->c_thn_polearm;
	p_ptr->skill_thn_axe += rp_ptr->r_thn_axe + cp_ptr->c_thn_axe;

	/* Base skill -- combat (shooting) */
	p_ptr->skill_thb_sling += rp_ptr->r_thb_sling + cp_ptr->c_thb_sling;
	p_ptr->skill_thb_bow += rp_ptr->r_thb_bow + cp_ptr->c_thb_bow;
	p_ptr->skill_thb_xbow += rp_ptr->r_thb_xbow + cp_ptr->c_thb_xbow;
	...
	if (f4 & (TR4_SUPER_REGEN)) p_ptr->s_regenerate = TRUE;
	if (f3 & (TR3_TELEPATHY)) p_ptr->telepathy = TRUE;
	if (f3 & (TR3_SEE_INVIS)) p_ptr->see_inv = TRUE;
	if (f3 & (TR3_FREE_ACT)) p_ptr->free_act = TRUE;
	if (f3 & (TR3_HOLD_LIFE)) p_ptr->hold_life = TRUE;
	if (f3 & (TR3_INVISIBILITY)) p_ptr->invisible = TRUE;

        /* Elemental touches */
        if (f4 & (TR4_COLD_TOUCH)) p_ptr->cold_touch = TRUE;
        if (f4 & (TR4_FIRE_TOUCH)) p_ptr->fire_touch = TRUE;
        if (f4 & (TR4_ACID_TOUCH)) p_ptr->acid_touch = TRUE;
        if (f4 & (TR4_ELEC_TOUCH)) p_ptr->elec_touch = TRUE;

	/* Weird flags */
	if (f3 & (TR3_BLESSED)) p_ptr->bless_blade = TRUE;

	/* Misc. special race flags */
        if (f4 & (TR4_UNDEAD_RACE)) p_ptr->undead = TRUE;
        if (f4 & (TR4_PASS_WALL)) p_ptr->pass_wall = TRUE;
        if (f4 & (TR4_POIS_BLOOD)) p_ptr->pois_blood = TRUE;
	if (f4 & (TR4_AURA_FEAR)) p_ptr->aura_fear = TRUE;
	...
	/* Special align */
	if (f4 & (TR4_ALIGN_CHAOS)) p_ptr->al_special = PAL_CHAOS;
	if (f4 & (TR4_ALIGN_LAW)) p_ptr->al_special = PAL_LAW;
	...
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
		...
		/* Don't count blows and shots if this is a weapon/bow! */
		
		/* Affect blows */
		if (!(r_ptr->body.weapon_mask & EQUIP_SLOT(i)) && (f1 & (TR1_BLOWS))) extra_blows += o_ptr->pval;

		/* Affect shots */
		if (!(r_ptr->body.bow_mask & EQUIP_SLOT(i)) && (f1 & (TR1_SHOTS))) extra_shots += o_ptr->pval;
		...
		/* Hack -- do not apply "weapon"/"bow" bonuses */
		if ((r_ptr->body.weapon_mask | r_ptr->body.bow_mask) & EQUIP_SLOT(i)) continue;
	...
	p_ptr->skill_thn_bare += (cp_ptr->x_thn_bare * p_ptr->lev / 10);
	p_ptr->skill_thn_edged += (cp_ptr->x_thn_edged * p_ptr->lev / 10);
	p_ptr->skill_thn_hafted += (cp_ptr->x_thn_hafted * p_ptr->lev / 10);
	p_ptr->skill_thn_polearm += (cp_ptr->x_thn_polearm * p_ptr->lev / 10);
	p_ptr->skill_thn_axe += (cp_ptr->x_thn_axe * p_ptr->lev / 10);

	/* Affect Skill -- combat (shooting) (Level, by Class) */
	p_ptr->skill_thb_sling += (cp_ptr->x_thb_sling * p_ptr->lev / 10);
	/* Hack - Rogues get increased skill with slings */
	if (cp_ptr->flags & CF_ROGUE_COMBAT)
	{
		p_ptr->skill_thb_sling += 3 + p_ptr->lev / 4;
	}
	p_ptr->skill_thb_bow += (cp_ptr->x_thb_bow * p_ptr->lev / 10);
	p_ptr->skill_thb_xbow += (cp_ptr->x_thb_xbow * p_ptr->lev / 10);
	...
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	if (!(r_ptr->body.bow_mask & EQUIP_SLOT(i))) continue;
		...
		if (o_ptr->k_idx)
		{
		    	analyze_launcher(o_ptr);
		}

		/* Add extra shots, if necessary */
		if (!p_ptr->heavy_shoot)
		{
			...
			{
				if (p_ptr->lev > 25) p_ptr->num_blow[i]++;
			}

			/* Add extra shots from other equipment */
			p_ptr->num_blow[i] += extra_shots;
		
			/* Add extra shots from the bow */
			object_flags(o_ptr, &f1, &f2, &f3, &f4);
			if (f1 & (TR1_SHOTS)) p_ptr->num_blow[i] += o_ptr->pval;
		}
	...
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
		...
		if (o_ptr->k_idx && !p_ptr->heavy_wield)
		{
			...
			/* Maximal value */
			if (p_ptr->num_blow[i] > cp_ptr->max_attacks) p_ptr->num_blow[i] = cp_ptr->max_attacks;

			/* Add in the "bonus blows" from equipment */
			p_ptr->num_blow[i] += extra_blows;

			/* Add in the "bonus blows" from this weapon */
			object_flags(o_ptr, &f1, &f2, &f3, &f4);
			if (f1 & (TR1_BLOWS)) p_ptr->num_blow[i] += o_ptr->pval;

			/* Require at least one blow */
			if (p_ptr->num_blow[i] < 1) p_ptr->num_blow[i] = 1;
			
			/* Add extra attack for those who have the flag */
			if ((p_ptr->lev > 25) && (cp_ptr->flags & CF_EXTRA_ATTACK)) p_ptr->num_blow[i]++;
		}
		...
		/* Giants have trouble with small weapons */
		if ((rp_ptr->flags4 & (TR4_GIANT_WIELD)) && o_ptr->weight < 180 && o_ptr->k_idx)
		{
			/* Reduce the real bonus */
			p_ptr->to_h -= (180 - o_ptr->weight) / 5;

			/* Reduce the displayed bonus */
			p_ptr->dis_to_h -= (180 - o_ptr->weight) / 5;
		}
	}
			...
			else if (i == A_WIS)
			{
				if (cp_ptr->spell_stat == A_WIS)
				{
					p_ptr->update |= (PU_MANA | PU_SPELLS);
                        ...
                        /* For monster race any change may affect mana */
                        if (cp_ptr->spell_book == 99)
