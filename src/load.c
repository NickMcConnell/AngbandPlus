...
/*
 * This file loads savefiles from Kangband 2.8.2d and later.
 *
...
static errr rd_extra(void)
{
	...
	for (i = 0; i < MAX_REWARDS; i++) rd_s16b(&p_ptr->rewards[i]);

	for (i = 0; i < MAX_MON_QUEST; i++) rd_s16b(&p_ptr->cqmon[i]);
	for (i = 0; i < MAX_MON_QUEST; i++) rd_s16b(&p_ptr->cqmonc[i]);
	for (i = 0; i < MAX_ITEM_QUEST; i++) rd_s16b(&p_ptr->cqitem[i]);
	for (i = 0; i < MAX_ITEM_QUEST; i++) rd_s16b(&p_ptr->cqitemc[i]);
