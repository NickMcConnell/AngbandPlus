...
s32b flag_cost(const object_type *o_ptr, int plusses)
{
	...
	if (f1 & TR1_ILL_STR) total -= (500 * plusses);
	if (f1 & TR1_ILL_INT) total -= (500 * plusses);
	if (f1 & TR1_ILL_WIS) total -= (500 * plusses);
	if (f1 & TR1_ILL_DEX) total -= (500 * plusses);
	if (f1 & TR1_ILL_CON) total -= (500 * plusses);
	if (f1 & TR1_ILL_CHR) total -= (250 * plusses);
	if (f2 & TR2_CHAOTIC) total += 5000;
	if (f2 & TR2_VAMPIRIC) total += 5000;
	...
	if (f2 & TR2_VORPAL) total += 1500;
	if (f2 & TR2_IMPACT) total += 1500;
	...
	if (f3 & TR3_REFLECT) total += 5000;
	if (f3 & TR3_FREE_ACT) total += 3000;
	if (f3 & TR3_HOLD_LIFE) total += 2000;
	...
	if (f3 & TR3_IGNORE_ELEM) total += 200;
...
s32b object_value_real(const object_type *o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_RING:
		{
			...
			/* Give credit for stat bonuses */
			if (f1 & (TR1_ILL_STR)) value -= (o_ptr->pval * 100L);
			if (f1 & (TR1_ILL_INT)) value -= (o_ptr->pval * 100L);
			if (f1 & (TR1_ILL_WIS)) value -= (o_ptr->pval * 100L);
			if (f1 & (TR1_ILL_DEX)) value -= (o_ptr->pval * 100L);
			if (f1 & (TR1_ILL_CON)) value -= (o_ptr->pval * 100L);
			if (f1 & (TR1_ILL_CHR)) value -= (o_ptr->pval * 100L);
...
static void init_ego_item(object_type *o_ptr, byte ego)
{
	...
	else
	{
		...
		if (e_ptr->max_pval)
		{
			// if the item already has a pval
			if (o_ptr->pval)
			{
				// roll another, keep the lowest
				rand = randint1(e_ptr->max_pval);
				if (rand < o_ptr->pval) o_ptr->pval = rand;
			}
			else
			{
				// roll a pval
...
void add_ego_flags(object_type *o_ptr, byte ego)
{
	...
	if (!o_ptr->xtra_name) o_ptr->xtra_name = quark_add(e_name + e_ptr->name);
	...
	switch (o_ptr->tval)
	{
		...
		case TV_SWORD:
		{
			...
			if (flags & OC_FORCE_GOOD)
			{
				...
				if (ego)
				{
					if (one_in_(5 * o_ptr->dd * o_ptr->ds))
					{
						o_ptr->ds += (o_ptr->ds * randint1(5)) / 5;
					}
				}
	...
	switch (o_ptr->tval)
	{
		...
		case TV_CLOAK:
		{
			...
			else if (flags & OC_FORCE_BAD)
			{
				...
				/* Roll for ego-item */
				if (get_ego_prep(ES_OUTER, FALSE))
				{
					ego = get_ego_num(level);
				}
			}
...
static void a_m_aux_4(object_type *o_ptr, int level, byte flags)
{
	...
	switch (o_ptr->tval)
	{
		case TV_LITE:
		{
			/* Big Hack -- if the pval is greater than 20, it is probably fuel */
			if (o_ptr->pval > 20)
			{
				if ( (o_ptr->sval == SV_LITE_TORCH) || (o_ptr->sval == SV_LITE_LANTERN) )
				{
					o_ptr->timeout = randint1(o_ptr->pval);
	...
	switch (power)
	{
		...
		case EGO_XTRA_SLAY:
		{
			...
			switch (randint0(11))
			{
				...
				case 7:
				{
					(o_ptr->flags1) |= (TR1_SLAY_ORC | TR1_SLAY_TROLL | TR1_SLAY_GIANT);
					break;
				}
				case 8:
				{
					(o_ptr->flags1) |= (TR1_SLAY_UNDEAD | TR1_SLAY_DEMON);
					break;
				}
				case 9:
				{
					(o_ptr->flags2) |= (TR2_VAMPIRIC);
					break;
				}
				case 10:
				{
					if (o_ptr->tval == TV_SWORD)
					{
						(o_ptr->flags2) |= (TR2_VORPAL);
					}
					else
					{
						(o_ptr->flags2) |= (TR2_IMPACT);
					}
