...
/* Chance of creating a random artifact; probably hardcoded -- Prfnoff */
#define RANDART_CHANCE 20
...
bool object_similar(object_type *o_ptr, object_type *j_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		/* Weapons, Armor, Rings, Amulets, Lites */
		case TV_BOW:
		case TV_DIGGING:
		case TV_HAFTED:
		case TV_POLEARM:
		...
		case TV_SWORD:
		{
			...
			if (power > 1)
			{
				...
				switch (randint((o_ptr->tval == TV_POLEARM) ? 40 : 42))
				{
					...
					case 35:
					{
						create_artifact(o_ptr, FALSE, FALSE);
						break;
					}
					case 36: case 37:
					{
			...
			{
				if (randint(RANDART_CHANCE) == 1) /* Prfnoff */
					create_artifact(o_ptr, FALSE, TRUE);
				/* Roll for ego-item */
				else if ((rand_int(MAX_DEPTH) < level) || (power < -2))
				{
...
void apply_magic(object_type *o_ptr, int lev, bool okay, bool good, bool great, bool curse)
{
	...
	if (o_ptr->name1)
	{
		...
		/* Munchkin option *mega-hack* -- Prfnoff */
		if (munchkin_weapon && (o_ptr->name1 == ART_GROO) && (o_ptr->pval == 2))
		{
			o_ptr->pval = 3;
		}
...
s16b drop_near(object_type *j_ptr, int chance, int y, int x)
{
	...
	for (dy = -3; dy <= 3; dy++)
	{
		...
		for (dx = -3; dx <= 3; dx++)
		{
			...
			/* Require floor space */
			if (!cave_safe_grid(c_ptr) && /* Prfnoff */
	...
	for (i = 0; !flag; i++)
	{
		...
		if (!cave_safe_grid(c_ptr)) continue;
...
/* Prfnoff deleted pick_trap and place_trap; now in trap.c */
