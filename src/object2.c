...
void wipe_o_list(void)
{
	...
	for (i = 1; i < o_max; i++)
	{
		...
		{
			/* Hack -- see above */
			cave_o_idx[o_ptr->iy][o_ptr->ix] = 0;
		}
...
static void charge_wand(object_type *o_ptr)
{
	switch (o_ptr->sval)
	{
		...
		case SV_WAND_DESTROY_MUNDANE:				 o_ptr->pval = randint(5)  + 3; break;
	}
}
...
static void a_m_aux_1(object_type *o_ptr, int level, int power)
{
	...
	int new_dd = rand_norm(o_ptr->dd);
	int new_ds = rand_norm(o_ptr->ds);
	...
	switch (o_ptr->tval)
	{
		...
		case TV_SHOT:
		{
			...
			if (power > 1)
			{
				...
				/* Hack -- super-charge the damage dice */
				while (o_ptr->dd && o_ptr->ds && (rand_int(10L * o_ptr->dd * o_ptr->ds) == 0)) o_ptr->dd++;
		...
		case TV_RING:
		{
			...
			switch (o_ptr->sval)
			{
				...
				case SV_RING_SLAYING:
				{
					...
					o_ptr->to_d = randint(10) + m_bonus(10, level);
					o_ptr->to_h = randint(10) + m_bonus(10, level);
...
void drop_near(object_type *j_ptr, int chance, int y, int x)
{
	...
	if (!artifact_p(j_ptr) && (chance > 0) && (rand_int(100) < chance))
	{
		/* Message */
		msg_format("The %s %s destroyed.",
	...
	for (d = 0; d <= MAX_SIGHT; d++)
	{
		...
		for (i = 0; i <= 4 * n; i++)
		{
			...
			/* Require floor space */
			if (!cave_barren_bold(ty, tx)) continue;
	...
	if (chance && cave_player_bold(by, bx))
	{
...
void pick_trap(int y, int x)
{
	...
	while (1)
	{
		...
		/* Mega-hack -- allow crusher traps occasionally */
		if (!rand_int(17)) feat = FEAT_TRAP_CRUSHER;
...
void display_spell_list(void)
{
	...
	if (p_ptr->pclass == CLASS_MINDCRAFTER)
	{
		...
		for (i = 0; i < MAX_MINDCRAFT_POWERS; i++)
		{
			...
			chance -= 3 * (p_ptr->level - spell.min_lev);
...
s16b spell_chance(int spell,int realm)
{
	...
	/* Reduce failure rate by "effective" level adjustment */
	chance -= 3 * (p_ptr->level - s_ptr->slevel);
...
bool spell_okay(int spell, bool known, int realm)
{
	...
	/* Spell is illegal */
	if (s_ptr->slevel > p_ptr->level) return (FALSE);
