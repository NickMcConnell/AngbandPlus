...
static s32b object_value_base(object_type *o_ptr)
{
	...
	/* Analyze the type of 'un-aware' items */
	switch (o_ptr->tval)
	{
...
s32b flag_cost(object_type * o_ptr, int plusses)
{
	...
	if (f2 & TR2_RAND_SUSTAIN) total += 0;
	if (f2 & TR2_RAND_ABILITY) total += 0;
	...
	if (f3 & TR3_SPINES) total += 4000; /* was XXX3 */
	if (f3 & TR3_AUTO_CURSE) total -= 5000;
	...
	if ((o_ptr->art_name) && (o_ptr->art_flags3 & (TR3_ACTIVATE)))
	{
		...
		else if (type == ACT_MANA_STORM) total += 10000;
...
s32b object_value_real(object_type *o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		/* Bows/Weapons */
		case TV_BOW: case TV_DIGGING: case TV_HAFTED: case TV_AXE:
...
bool object_similar(object_type *o_ptr, object_type *j_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		/* Weapons and Armor */
		case TV_BOW: case TV_DIGGING: case TV_HAFTED: case TV_AXE:
		case TV_POLEARM: case TV_SWORD: case TV_BOOTS: case TV_HELM:
		case TV_GLOVES: case TV_CROWN: case TV_SHIELD: case TV_CLOAK:
		case TV_SOFT_ARMOR: case TV_HARD_ARMOR: case TV_DRAG_ARMOR:
		{
			/* Require permission */
...
static bool make_artifact_special(object_type *o_ptr)
{
	 ...
	 * Try up to 200 times to get a match. It's a bit wasteful, but it
	 * means that the deeper and rarer 'specials' will show up in more
	 * games. Thanks to Greg Wooledge for cleaning it up a bit. -- Gumby
	 */
	for (i = 0; i < ((dun_level * 2) + 100); i++)
	{
		artifact_type *a_ptr;

		/* Pick one of the 'special' artifact slots */
		k = randint(ART_MIN_NORMAL - 1);
			...
			if (power > 1)
			{
				...
				switch (randint(45))
				{
					...
					case 37:
					{
						...
						/* Add a bunch of slays -- Gumby */
						if (randint(20)==1) o_ptr->art_flags1 |= TR1_SLAY_HUMANOID;
						if (randint(2)==1) o_ptr->art_flags1 |= TR1_SLAY_ELEMENTAL;
						if (randint(4)==1) o_ptr->art_flags1 |= TR1_SLAY_ANIMAL;
						if (randint(5)==1) o_ptr->art_flags1 |= TR1_SLAY_EVIL;
						if (randint(3)==1) o_ptr->art_flags1 |= TR1_SLAY_UNDEAD;
						if (randint(3)==1) o_ptr->art_flags1 |= TR1_SLAY_DEMON;
						if (randint(2)==1) o_ptr->art_flags1 |= TR1_SLAY_ORC;
						if (randint(2)==1) o_ptr->art_flags1 |= TR1_SLAY_TROLL;
						if (randint(2)==1) o_ptr->art_flags1 |= TR1_SLAY_GIANT;
						if (randint(4)==1) o_ptr->art_flags1 |= TR1_SLAY_DRAGON;
						if (randint(20)==1) o_ptr->art_flags1 |= TR1_KILL_DRAGON;
...
static void a_m_aux_2(object_type *o_ptr, int level, int power)
{
	...
	switch (o_ptr->tval)
	{
		case TV_DRAG_ARMOR:
		{
			/*
			 * Rating boost
			 * Black, Blue, White, and Red DSMs aren't all
			 * that special. -- Gumby
			 */
			if (o_ptr->sval >= 5)
				rating += 30;
		...
		case TV_SHIELD:
		{
			...
			/*
			 * Shields of Deflection & Dragon Shields cannot be
			 * ego-items or random artifacts. -- Gumby
			 */
				...
				if (power > 1)
				{
					...
					else
					{
						...
						switch (randint(15))
						{
							...
							{
								o_ptr->name2 = EGO_SEEING;
								if (randint(6)==1) o_ptr->art_flags3 |= TR3_TELEPATHY;
								break;
							}
...
static void a_m_aux_3(object_type *o_ptr, int level, int power)
{
	...
	switch (o_ptr->tval)
	{
        ...
        /* Allow some Ring & Amulet artifacts - stolen from PernAngband - G */
        if((power > 1) && (randint(100) <= 4))
        {
                create_artifact(o_ptr, FALSE);
        }
}
...
void apply_magic(object_type *o_ptr, int lev, bool okay, bool good, bool great)
{
	...
	if (o_ptr->name1)
	{
		...
		if (a_ptr->cost > 50000L) rating += 5;

		/* Increase it even more for awesome artifacts. -- Gumby */
...
static bool kind_is_good(int k_idx)
{
	...
	switch (k_ptr->tval)
	{
		...
		case TV_HARD_ARMOR: case TV_SOFT_ARMOR: case TV_DRAG_ARMOR:
		case TV_CLOAK: case TV_BOOTS: case TV_GLOVES: case TV_CROWN:
		case TV_HELM: case TV_SHIELD:
		{
		...
		case TV_LIFE_BOOK: case TV_SORCERY_BOOK: case TV_TRUMP_BOOK:
		case TV_NATURE_BOOK: case TV_CHAOS_BOOK: case TV_DEATH_BOOK:
		{
		...
		case TV_RING:
		{
			if (k_ptr->sval == SV_RING_SHADOWS) return (TRUE);
		...
		case TV_AMULET:
		{
			if (k_ptr->sval == SV_AMULET_HOLDING) return (TRUE);
...
void display_spell_list(void)
{
	...
	/* Warriors & Weaponmasters are illiterate */
...
s16b spell_chance(int spell,int realm)
{
	...
	/*
	 * Most spellcasters never get too good.
	 */
...
#ifdef DRS_SHOW_SPELL_INFO
	{
		...
		switch (realm)
		{
			case 0: /* Life */
				switch (spell)
				{
					case  1: sprintf(p, " heal %dd5", (2 + ((plev - 2) / 6))); break;
					case  2: sprintf(p, " dur %d turns", 10 + plev); break;
					case  3: sprintf(p, " dam %dd4", (3 + ((plev - 1) / 7))); break;
					case  4: sprintf(p, " dam %d", 10 + (plev / 2)); break;
					case  6: sprintf(p, " heal %dd10", (3 + ((plev - 2) / 6))); break;
					case 10: sprintf(p, " heal %dd10", (6 + ((plev - 5) / 3))); break;
					case 11: sprintf(p, " dur %d turns", (25 + ((3 * plev) / 2))); break;
			...
			case 2: /* Nature */
				switch (spell)
				{
					case  1: sprintf(p, " dam %d", 3 + ((plev-1)/7)); break;
					case  4: sprintf(p, " dam %d", 10 + (plev / 2)); break;
					case  6: strcpy (p, " dur 20+d20"); break;
					case  7: strcpy (p, " heal 6d8"); break;
			...
			case 4: /* Death */
				switch(spell)
				{
					...
					case 13: sprintf(p, " dam %d", 200 + (plev*2)); break;
