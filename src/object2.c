	...
	switch (o_ptr->tval)
	{
		...
		case TV_RING:
		{
			...
			if (f1 & (TR1_TUNNEL))
			{
				 ...
				 * digger possesses intrinsically. -LM-
				 */
...
static void a_m_aux_2(object_type *o_ptr, int level, int power)
{
	...
	/* Boots and handgear aren't as important to total AC. -LM- */
...
void apply_magic(object_type *o_ptr, int lev, bool okay, bool good, bool great)
{
	...
	if (o_ptr->name2)
	{
		...
		switch (o_ptr->name2)
		{
			...
			/* Dwarven armour is strong and light. -LM- */
			case EGO_DWARVEN:
			{
		...
		/* Apply extra bonuses or penalties. -LM- */
...
void drop_near(object_type *j_ptr, int chance, int y, int x)
{
	...
	for (dy = -3; dy < 4; dy++)
	{
		...
		for (dx = -3; dx < 4; dx++)
		{
			 ...
			 * the item, encourage it to pile with up to 19 other items. -LM-
			 */
			if (cave_m_idx[y][x] < 0)
			{
				s = 1000 - (d + (k > 20 ? k * 5 : 0));
