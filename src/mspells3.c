    ...
    switch (power)
    {
        ...
        case MS_SHOOT:
        {
            ...
            if (slot)
            {
                ...
                sprintf(p, " %s%dd%d+%d", s_dam, o_ptr->dd, spell_power(o_ptr->ds), spell_power(o_ptr->to_h));
            }
            else
                sprintf(p, " %s1", s_dam);
            break;
...
static int get_learned_power(int *sn)
{
    ...
    *sn = (-1);
	...
	for (i = 0; i < MS_MAX; i++)
	{
		if (p_ptr->magic_num2[i] && blue_mage_mask[i] == mode)
			spellnum[num++] = i;
	}
...
static bool cast_learned_spell(int spell, bool success)
{
    ...
    case MS_SHOOT:
    {
        ...
        {
            object_type *o_ptr = equip_obj(slot);
            damage = damroll(o_ptr->dd, o_ptr->ds)+ o_ptr->to_h;
...
int get_monspell(byte type, s16b effect)
{
	int monspell = -1;
	...
	case MST_ANNOY:
		switch (effect)
		{
		case 0: /*ANNOY_AMNESIA*/
			monspell = 78;
			break;
		case 2: /*ANNOY_BLIND*/
			monspell = 60;
			break;
		case 3: /*ANNOY_CONFUSE*/
			monspell = 61;
			break;
		case 4: /*ANNOY_DARKNESS*/
			monspell = 76;
			break;
		case 5: /*ANNOY_PARALYZE*/
			monspell = 63;
			break;
		case 6: /*ANNOY_SCARE*/
			monspell = 59;
		...
		case 8: /*ANNOY_SLOW*/
			monspell = 62;
			break;
		case 9: /*ANNOY_TELE_LEVEL*/
			monspell = 74;
			break;
		case 10: /*ANNOY_TELE_TO*/
			monspell = 72;
			break;
		case 11: /*ANNOY_TRAPS*/
			monspell = 77;
		...
		break;
	case MST_BIFF:
		if (effect == 1) /* BIFF_DISPEL_MAGIC */
			monspell = 2;
		break;
	case MST_ESCAPE:
		if (effect == 1) /* ESCAPE_TELE_OTHER */
			monspell = 73;
		break;
	case MST_CURSE:
		switch (effect)
...
void learn_monster_spell(int monspell)
{
