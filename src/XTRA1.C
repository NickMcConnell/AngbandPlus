...
static void health_redraw(void)
{
	...
	else
	{
		...
		/* Dump the current "health" (use '*' symbols) */
                sprintf(thelevel, "Level %d", m_ptr->level);
                Term_putstr(COL_INFO + 1, ROW_INFO, 12, attr, thelevel);
                sprintf(thehealth, "%d/%d", m_ptr->hp, m_ptr->maxhp);
                Term_putstr(COL_INFO, ROW_INFO + 1, 12, attr, thehealth);
	}
...
static void calc_spells(void)
{
	...
	/* Count the number of spells we know */
        ...
        switch(p_ptr->pclass)
        {
                ...
                /* Skatters don't use magic, but still get mana for some skills */
                case CLASS_SKATTER:
                        class_max = 0;
                        break;
...
static void calc_mana(void)
{
	...
	if (msp && (p_ptr->pclass == CLASS_HIGH_MAGE)) msp += msp / 4;
        ...
        /* Dark Lords have 50% more mana */
        if (msp && (p_ptr->pclass == CLASS_DARK_LORD)) msp += msp / 2;

        /* Hack: Power mages have a 75% mana bonus */
        if (msp && (p_ptr->pclass == CLASS_POWERMAGE)) msp += msp * 3 / 4;
        if (msp && (ability(36))) msp += msp * 3 / 4;

        /* Leaders and Commanders have a 50% mana PENALITY!! */
        if (msp && (p_ptr->pclass == CLASS_LEADER || p_ptr->pclass == CLASS_COMMANDER)) msp += msp / 2;

        /* Hack: Sorcerer have a 150% mana bonus */
        if (msp && (p_ptr->pclass == CLASS_SORCERER)) msp += msp * 3 / 2;

	  /* Hack: MagiWarriors have a mana bonus */
        if (msp && (p_ptr->pclass == CLASS_MAGIWARRIOR)) msp += msp * 3 / 2;

          /* The Lich have a formidale amount of mana! */
        if (msp && (p_ptr->pclass == CLASS_LICH)) msp += msp * 3;

          /* Beautiful, powerful and evil... */
        if (msp && (p_ptr->pclass == CLASS_HELLQUEEN)) msp += msp * 8;

        if (msp && (ability(5))) msp += 30;
        if (msp && (ability(6))) msp += msp / 4;
        if (msp && (ability(17))) msp += msp / 10;
        if (msp && (ability(23))) msp += msp / 10;
        if (msp && (ability(24))) msp += msp / 4;
        if (msp && (ability(30))) msp += msp / 2;
        if (msp && (ability(41))) msp += msp;
        if (msp && (ability(49))) msp += msp / 2;
        if (msp && (ability(50))) msp += msp * 3 / 4;
...
void calc_hitpoints(void)
{
	...
	if (mhp < p_ptr->lev + 1) mhp = p_ptr->lev + 1;
        ...
        /* Hack: Assassins have a little hp penality */
        if (mhp && (p_ptr->pclass == CLASS_ASSASSIN)) mhp -= mhp /10;

        /* Hack: Leaders have a -25% hp penality */
        if (mhp && (p_ptr->pclass == CLASS_LEADER)) mhp -= mhp / 4;

        /* Hack: Hell Queen have a -33% hp penality */
        if (mhp && (p_ptr->pclass == CLASS_HELLQUEEN)) mhp -= mhp / (1.5);
        ...
        /* Extra hp bonuses for some warrior classes */
        if (mhp && (p_ptr->pclass == CLASS_RANGER)) mhp += mhp / 4;
        if (mhp && (p_ptr->pclass == CLASS_PALADIN)) mhp += mhp / 4;
        if (mhp && (p_ptr->pclass == CLASS_WARRIOR_MAGE)) mhp += mhp / 4;
        if (mhp && (p_ptr->pclass == CLASS_MONSTER_MAGE)) mhp += mhp / 4;

        /* Dark Lords have more hp due to their dark energy */
        if (mhp && (p_ptr->pclass == CLASS_DARK_LORD)) mhp += mhp;

        if (mhp && (p_ptr->pclass == CLASS_WARRIOR)) mhp += mhp / 2;
        if (mhp && (p_ptr->pclass == CLASS_FIRE_LORD)) mhp += mhp / 2;
        if (mhp && (p_ptr->pclass == CLASS_MAGIWARRIOR)) mhp += mhp / 2;
        if (mhp && (p_ptr->pclass == CLASS_JUSTICE_WARRIOR)) mhp += mhp / 2;
        if (mhp && (p_ptr->pclass == CLASS_COMMANDER)) mhp += mhp / 2;

        if (mhp && (ability(3))) mhp += mhp / 2;
        if (mhp && (ability(18))) mhp += mhp / 4;
        if (mhp && (ability(23))) mhp += mhp / 10;
        if (mhp && (ability(24))) mhp += mhp / 4;
        if (mhp && (ability(49))) mhp += mhp / 2;
        if (mhp && (ability(50))) mhp += mhp * 3 / 4;
        if (mhp && (ability(53))) mhp += mhp * 2;
        if (mhp && (ability(68))) mhp += mhp;

        /* Zelar get double hp */
        if (mhp && (p_ptr->pclass == CLASS_ZELAR)) mhp += mhp;
        /* Valkyries too! */
        if (mhp && (p_ptr->pclass == CLASS_VALKYRIE)) mhp += mhp;
                                     
	/* Berserkers gain lot of hp quickly */
        if (mhp && (p_ptr->pclass == CLASS_BERSERKER)) mhp += mhp * 2;

        /* Battle Masters gain lot of hp quickly */
        if (mhp && (p_ptr->pclass == CLASS_MASTER)) mhp += mhp * 2;
	...
	if (p_ptr->shero) mhp += 30;
        ...
        while (p_ptr->body_parts[i] == INVEN_WIELD)
        {
                ...
                {
                        /* Augment Hitpoint */
                        if (o_ptr->tval == TV_SWORD && o_ptr->sval == 42) mhp = 30000;
                        else mhp += mhp * o_ptr->pval / 5;
                }
        ...
        i = 0;
        while (p_ptr->body_parts[i] == INVEN_NECK)
        {
                /* Get the weapon */
                o_ptr = &inventory[INVEN_NECK + i];
...
void calc_body()
{
        ...
        p_ptr->body_parts[INVEN_ABILITY - INVEN_WIELD] = INVEN_ABILITY;
...
void calc_bonuses(void)
{
	...
	switch (p_ptr->pclass)
	{
		...
		case CLASS_CHAOS_WARRIOR:
                        p_ptr->resist_chaos = TRUE;
                        if (p_ptr->lev > 15) p_ptr->resist_fear = TRUE;
                        p_ptr->stat_add[A_INT] += 1;
                        p_ptr->stat_add[A_CHR] -= 1;
			break;
		case CLASS_MINDCRAFTER:
		...
		case CLASS_MONK:
			...
			break;
                ...
                case CLASS_MAGIWARRIOR:
                        if (p_ptr->lev > 15) p_ptr->resist_fear = TRUE;
                        if (p_ptr->lev > 15) p_ptr->resist_conf = TRUE;
                        if (p_ptr->lev >= 20)
                        {
                            p_ptr->to_s += 3;
                            p_ptr->to_h += 10;
			...
			break;
                case CLASS_DARK_LORD:
                        ...
                        p_ptr->resist_disen = TRUE;
                        p_ptr->see_inv = TRUE;
                        if (p_ptr->lev >= 15) p_ptr->telepathy = TRUE;
                        p_ptr->stat_add[A_INT] += 5;
                        p_ptr->stat_add[A_STR] += 5;
                        p_ptr->stat_add[A_CON] += 5;
			...
			break;
                ...
                case CLASS_FIRE_LORD:
                        ...
                        p_ptr->sustain_str = TRUE;
                        p_ptr->to_d += 8;
                        if (p_ptr->lev >= 25) p_ptr->immune_fire = TRUE;
			break;
                case CLASS_VALKYRIE:
                        p_ptr->resist_lite = TRUE;
                        p_ptr->resist_elec = TRUE;
                        ...
                        p_ptr->stat_add[A_CON] += 2;
                        p_ptr->to_d += 10;
                        if (p_ptr->lev >= 16) p_ptr->immune_elec = TRUE;
                        break;
	...
	switch (p_ptr->prace)
	{
		...
		case RACE_HALF_TROLL:
			...
			if (p_ptr->lev > 14)
			{
				...
				p_ptr->regenerate = TRUE;
                                ...
                                if (p_ptr->pclass == CLASS_WARRIOR || p_ptr->pclass == CLASS_BERSERKER || p_ptr->pclass == CLASS_MASTER || p_ptr->pclass == CLASS_FIRE_LORD)
				{
					p_ptr->slow_digest = TRUE;
		...
		case RACE_SPECTRE:
		    ...
		    case RACE_DEMON:
                        ...
                        p_ptr->sustain_con = TRUE;
                        p_ptr->slow_digest = TRUE;
                        if (p_ptr->lev >= 30) p_ptr->immune_fire = TRUE;
                        break;
   ...
   p_ptr->skill_stl += move_info[p_ptr->movement].to_stealth;
	...
	if (p_ptr->see_inv != old_see_inv)
	{
        ...
        if (ability(64) && p_ptr->pclass != CLASS_ZELAR)
        {
                p_ptr->num_blow += p_ptr->lev /10;
        }
	if (ring(SV_RING_SAFETY) && !ability(3) && !ability(18)
         && !ability(23) && !ability(24) && !ability(49) 
         && !ability(50) && !ability(53) && !ability(68)) p_ptr->stun = 0;

        /* Extra bonus for Daggers-wielders! */
        if (o_ptr->tval == TV_SWORD && o_ptr->sval == SV_DAGGER) p_ptr->num_blow += p_ptr->lev / 5;

        /* Valkyries: Spear mistresses */
        if (o_ptr->tval == TV_POLEARM) p_ptr->num_blow += p_ptr->lev / 5 + 1;
	...
	if (o_ptr->k_idx && !p_ptr->heavy_shoot)
	{
                ...
                if (p_ptr->pclass == CLASS_ARCHER)
		{
                ...
                /* Hack -- Reward High Level Skatters */
                if (p_ptr->pclass == CLASS_SKATTER)
		{
                        /* Extra shot at level 10 */
                        if (p_ptr->lev >= 10) p_ptr->num_fire++;
	...
	if (o_ptr->k_idx && !p_ptr->heavy_wield)
	{
		...
		switch (p_ptr->pclass)
		{
			...
			/* Illusionist -KMW- */
                        ...
                        /* Berserkers and Ninjas use the ultimate TABLE 11!! */
                        case CLASS_BERSERKER:
                        case CLASS_MASTER:
                        case CLASS_ZELAR:
                        case CLASS_DARK_LORD:
                        case CLASS_VALKYRIE:
                                num = 11; wgt = 25; mul = 6; break;
		...
		/* Level bonus for warriors (1-3) */
                if ((p_ptr->pclass == CLASS_WARRIOR) || (p_ptr->pclass == CLASS_BERSERKER) || (p_ptr->pclass == CLASS_MASTER) || (p_ptr->pclass == CLASS_FIRE_LORD) || (p_ptr->pclass == CLASS_VALKYRIE) || (p_ptr->pclass == CLASS_COMMANDER)) p_ptr->num_blow += (p_ptr->lev) / 15;

		/* MagiWarriors have bonuses, less than warriors */
                if (p_ptr->pclass == CLASS_MAGIWARRIOR ) p_ptr->num_blow += (p_ptr->lev) / 20;
                /* Even less for Justice Warriors, but still a small bonus... */
                if (p_ptr->pclass == CLASS_JUSTICE_WARRIOR ) p_ptr->num_blow += (p_ptr->lev) / 25;

            /* Berserkers have the best blows bonuses */
                if (p_ptr->pclass == CLASS_BERSERKER ) p_ptr->num_blow += (p_ptr->lev) / 5;

                if (p_ptr->pclass == CLASS_MASTER ) p_ptr->num_blow += (p_ptr->lev) / 3;

                if (ability(2)) p_ptr->num_blow += 1;
                if (ability(54)) p_ptr->num_blow += 3;
	...
	else if (p_ptr->pclass == CLASS_MONK && monk_empty_hands())
	{
        ...
        /* The multiplier!! */
        p_ptr->multiplier = 1;
        if (p_ptr->pclass == CLASS_WARRIOR || p_ptr->pclass == CLASS_FIRE_LORD ||
        p_ptr->pclass == CLASS_VALKYRIE || p_ptr->pclass == CLASS_DARK_LORD || p_ptr->pclass == CLASS_JUSTICE_WARRIOR || 
        p_ptr->pclass == CLASS_MONK || p_ptr->pclass == CLASS_ZELAR || p_ptr->pclass == CLASS_MAGIWARRIOR
         || p_ptr->pclass == CLASS_COMMANDER || p_ptr->pclass == CLASS_MASTER || p_ptr->pclass == CLASS_BERSERKER || ability(4))
        p_ptr->multiplier += p_ptr->lev / 3;
        else p_ptr->multiplier += p_ptr->lev / 6;

        /* Check the current ability */
        o_ptr = &inventory[INVEN_ABILITY];
        if (o_ptr->sval != p_ptr->current_ability) p_ptr->current_ability = o_ptr->sval;
        o_ptr = &inventory[INVEN_WIELD];
	...
	/* Extra bonus for warriors... */
        if ((p_ptr->pclass == CLASS_WARRIOR) || (p_ptr->pclass == CLASS_FIRE_LORD) || ability(1))
	{
	...
	/* Extra bonus for MagiWarriors... */
        if (p_ptr->pclass == CLASS_MAGIWARRIOR || p_ptr->pclass == CLASS_JUSTICE_WARRIOR || p_ptr->pclass == CLASS_COMMANDER)
        {   
                p_ptr->to_h += (p_ptr->lev/7);
                p_ptr->to_d += (p_ptr->lev/7);

                p_ptr->dis_to_h += (p_ptr->lev/7);
                p_ptr->dis_to_d += (p_ptr->lev/7);
	}

        /* Extra bonus for Berserkers... */
        ...
        if (p_ptr->pclass == CLASS_VALKYRIE )
	{
        ...
        /* Extra bonus for Masters... */
        if (p_ptr->pclass == CLASS_MASTER )
	{
                p_ptr->to_h += (p_ptr->lev * 6);
                p_ptr->to_d += (p_ptr->lev * 6);

                p_ptr->dis_to_h += (p_ptr->lev * 6);
                p_ptr->dis_to_d += (p_ptr->lev * 6);
	}
        /* Extra bonus for Dark Lords... */
	...
	/* Priest weapon penalty for non-blessed edged weapons */
        if (((p_ptr->pclass == CLASS_PRIEST)) && (!p_ptr->bless_blade) &&
            ((o_ptr->tval == TV_SWORD) || (o_ptr->tval == TV_POLEARM)) && !ability(4))
	{
        ...
        /* No hafted or polearm weapons for Assassins */
        if (p_ptr->pclass == CLASS_ASSASSIN && (o_ptr->tval == TV_HAFTED || o_ptr->tval == TV_POLEARM || o_ptr->tval == TV_MSTAFF
        || o_ptr->tval == TV_ROD || o_ptr->tval == TV_HELL_STAFF) && !ability(4))
        {
                p_ptr->to_h -= 200;
                p_ptr->dis_to_h -= 200;
                p_ptr->icky_wield = TRUE;
        }
        /* If a Zelar uses a weapon, VERY BAD!!!! Else, very good! */
        if (p_ptr->pclass == CLASS_ZELAR || ability(64))
        {
        if ((o_ptr->tval == TV_SWORD || o_ptr->tval == TV_HAFTED || o_ptr->tval == TV_POLEARM ||
        o_ptr->tval == TV_MSTAFF || o_ptr->tval == TV_SWORD_DEVASTATION || o_ptr->tval == TV_HELL_STAFF || o_ptr->tval == TV_ROD) && !ability(4))
	{
                /* Augment the real bonuses */
                p_ptr->to_h -= 500;

                /* Augment the mental bonuses */
                p_ptr->dis_to_h -= 500;
        ...
        {
                p_ptr->to_h += (20 + p_ptr->lev * 5);
                p_ptr->dis_to_h += (20 + p_ptr->lev * 5);
                p_ptr->to_d += (10 +  p_ptr->lev * 10);
                p_ptr->dis_to_d += (10 +  p_ptr->lev * 10);

        }

        }

        /* Skatters CAN use weapons, but we need a strong one... */
        if (p_ptr->pclass == CLASS_SKATTER)
        {
        if ((o_ptr->tval == TV_SWORD || o_ptr->tval == TV_HAFTED || o_ptr->tval == TV_POLEARM ||
        o_ptr->tval == TV_MSTAFF || o_ptr->tval == TV_SWORD_DEVASTATION || o_ptr->tval == TV_HELL_STAFF) && !ability(4))
	{
                /* Augment the real bonuses */
                p_ptr->to_h -= 20;
        ...
        if (o_ptr->tval == TV_SWORD_DEVASTATION && (p_ptr->pclass != CLASS_DARK_LORD && !ability(55)))
        {
                  msg_print("Because you're not a dark lord, you die by wielding this sword!");
                  take_hit(20000, "a sword of devastation");
	}
        if (o_ptr->name1 == ART_ASMODIS && o_ptr->asmodis_energy > 15000) o_ptr->asmodis_energy = 15000;
        /* Sorcerer can't wield a weapon unless it's a mage staff */
        if (p_ptr->pclass == CLASS_SORCERER)
        {
                if (o_ptr->tval != TV_MSTAFF && o_ptr->tval != TV_ROD && !ability(4))
                {
                        /* Reduce the real bonuses */
                        p_ptr->to_h -= 200;
                        p_ptr->to_d -= 200;
        ...
        /* Valkyries can't wield a weapon unless it's a Polearm */
        if (p_ptr->pclass == CLASS_VALKYRIE)
        {
                if (o_ptr->tval != TV_POLEARM && o_ptr->tval != TV_VALKYRIE_SPEAR && !ability(4))
                {
                        /* Reduce the real bonuses */
                        p_ptr->to_h -= 500;
                        p_ptr->to_d -= 500;
        ...
        /* Hell Queens need a HELL staff to fight! */
        if (p_ptr->pclass == CLASS_HELLQUEEN)
        {
                if (o_ptr->tval != TV_HELL_STAFF && !ability(4))
                {
                        if (!ability(64))
                        {
                        /* Reduce the real bonuses */
        ...
        /* Hack: Assassins are limited to soft armors */
        o_ptr = &inventory[INVEN_BODY];
        if (p_ptr->pclass == CLASS_ASSASSIN && o_ptr->tval == TV_HARD_ARMOR)
        {
                p_ptr->to_h -= 200;
                p_ptr->to_d -= 200;
                p_ptr->dis_to_h -= 200;
        ...
        for (v = 0; v < INVEN_TOTAL; v++)
        {
                ...
                p_ptr->pclass == CLASS_LICH || p_ptr->pclass == CLASS_HELLQUEEN ||
                p_ptr->pclass == CLASS_LEADER || p_ptr->pclass == CLASS_NECRO ||
                p_ptr->pclass == CLASS_RUNECRAFTER || p_ptr->pclass == CLASS_MINDCRAFTER ||
        ...
        if (ability(21))
        {
                p_ptr->stat_ind[A_CHR] += 2;
        }
        if (ability(25))
        {
                p_ptr->pspeed += 5;
                p_ptr->stat_ind[A_STR] -= 5;
                p_ptr->stat_ind[A_INT] -= 5;
                p_ptr->stat_ind[A_WIS] -= 5;
                p_ptr->stat_ind[A_DEX] -= 5;
                p_ptr->stat_ind[A_CON] -= 5;
                p_ptr->stat_ind[A_CHR] -= 5;
        }
        if (ability(26))
        {
                p_ptr->stat_ind[A_CON] += 2;
        }
        if (ability(31))
        {
                p_ptr->to_s += 4;
	...
	/* while Rangers and Monks get one every 10 levels. */
        if (!p_ptr->aggravate)
	{
                if (p_ptr->pclass == CLASS_ROGUE || p_ptr->pclass == CLASS_ASSASSIN)
		{
