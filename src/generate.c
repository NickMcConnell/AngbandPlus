...
{
	if ( rand_int(100) < 50 )
		/* 50%: Create locked door */
		cave_set_feat(y, x, FEAT_DOOR_HEAD + randint(7));
	else
		/* 50%: Create trapped locked door */
		cave_set_feat(y, x, FEAT_TRAPPED_DOOR_HEAD + rand_int(8));
}
...
static void place_random_door(int y, int x)
{
	...
	{
		place_locked_door(y, x);  /* could be trapped! */
	}
