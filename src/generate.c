...
void repl_spot(int, int, int);
static void place_lava(void);
static void place_destroyed(void);
#else
char *strcpy(ARG_CHAR_PTR ARG_COMMA ARG_CHAR_PTR);
void repl_spot(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
static void place_lava(ARG_VOID);
...
/* Places "streamers" of lava through dungeon -DGK */
static void place_lava()
{
  register int i, tx, ty, dist;
  int y, x, t1, t2, dir, numstr, degr;

for (numstr=0;numstr<5;numstr++)
 {
  dist=0;
  /* Choose starting point and direction                */
  switch (randint(4))
   {
    case 1: y=1;            x=randint(cur_width-22)+10;
	    degr=3;           dir=2;             break;
    case 2: y=cur_height-2; x=randint(cur_width-22)+10;
	    degr=11;          dir=8;             break;
    case 3: y=randint(cur_height-22)+10;           x=1;
	    degr=7;           dir=6;             break;
    case 4: y=randint(cur_height-22)+10; x=cur_width-2;
	    degr=15;          dir=4;             break;
   }

  /* Place streamer into dungeon                        */
  t1 = randint(DUN_STR_RNG*3+2);
  t2 = DUN_STR_RNG + 1; /* Constant */
  do
    {
     for (i = 0; i < ((DUN_STR_DEN*t1/(2*DUN_STR_RNG+1))+1); i++)
       {
	ty = y + randint(t1) - t2;
	tx = x + randint(t1) - t2;
	if (in_bounds(ty, tx))
	 {
	  int8u tty,ttx;
	  cave[ty][tx].fval = LAVA_FLOOR;
	  for(tty=ty-1;tty<=ty+1;tty++)
	   for(ttx=tx-1;ttx<=tx+1;ttx++)
	    if ((cave[tty][ttx].fval>=MIN_CAVE_WALL)&&
		 in_bounds(tty,ttx))
	     cave[tty][ttx].fval=MAGMA_WALL;
	 }
       }
     if (t1<2)
       t1++;
      else if (t1 > (DUN_STR_RNG*2+3) )
       t1--;
      else
       t1+=randint(3)-2;
     dist++;
       ...
       if (randint(2)==1)
	{
	 ...
	 switch (degr)
	  {
	   case 1:  dir=1; break;
	   case 2:  dir=((dist&1)?1:2); break;
	   case 3:  dir=2; break;
	   case 4:  dir=((dist&1)?2:3); break;
	   case 5:  dir=3; break;
	   case 6:  dir=((dist&1)?3:6); break;
	   case 7:  dir=6; break;
	   case 8:  dir=((dist&1)?6:9); break;
	   case 9:  dir=9; break;
	   case 10: dir=((dist&1)?9:8); break;
	   case 11: dir=8; break;
	   case 12: dir=((dist&1)?8:7); break;
	   case 13: dir=7; break;
	   case 14: dir=((dist&1)?7:4); break;
	   case 15: dir=4; break;
	   case 16: dir=((dist&1)?4:1); break;
	  }
	}
      }
    }
...
int16 *y, *x;
{
  ...
  while (c_ptr->fval >= MIN_CLOSED_SPACE || (c_ptr->cptr != 0)
	 ...
	 || (c_ptr->fval == NT_DARK_FLOOR) || (c_ptr->fval == LAVA_FLOOR));
  *y = i;
  *x = j;
}
  ...
  int yval, xval;
{
  ...
  else if (tmp < 20) special_pit(yval, xval,2); /* orc */
  else if (tmp < 43)
    if (randint(3)==1) special_pit(yval, xval,7); /* giant -CFT */
    else special_pit(yval, xval, 3); /* troll */
  else if (tmp < 57) special_pit(yval, xval,4); /* undead */
  else if (tmp < 73) special_pit(yval, xval,5); /* dragon */
  else               special_pit(yval, xval,6); /* demon */
}

/* Allocates an object for tunnels and rooms		-RAK-	*/
/* Moved from misc2.c -DGK */
...
static void cave_gen()
{
  ...
  spec_level=0;
  pit_ok=FALSE;
  if ((randint(DUN_LAVA) == 1) && (dun_level>10) && (!is_quest(dun_level)))
    {
     if (wizard) msg_print ("Lava Level");
     spec_level=SPEC_LAVA;
    }
   else if ((randint(DUN_DEST)==1)&&(dun_level>10)&&(!is_quest(dun_level)))
    {
  ...
  if (spec_level==SPEC_LAVA)
    alloc_object(set_lava, 4, randint(TREAS_GOLD_ALLOC*2)+alloc_level);
  if (place_ghost())
    good_item_flag = TRUE;
