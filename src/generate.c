...
static void build_type5(int y0, int x0)
{
	...
	if (cheat_room)
	{
        ...
        {
        	/* Silly message */
        	msg_format("You will soon meet %ss.", name);
        }
...
static void build_type6(int y0, int x0)
{
	...
	/* Hack -- increase danger level */
	if (tmp < p_ptr->depth / 2) tmp += randint(p_ptr->depth-tmp);
	...
	for (t = data, dy = 0; dy < ymax; dy++)
	{
		for (dx = 0; dx < xmax; dx++, t++)
		{
			...
			switch (*t)
			{
				...
				/* The guarded treasure - great objects */
				case '|':
				{
					object_level = p_ptr->depth + 20;
					place_object(y, x, TRUE, TRUE);
				...
				/* The guarded treasure - good objects */
				case '\\':
				{
					object_level = p_ptr->depth + 7;
					place_object(y, x, TRUE, FALSE);
