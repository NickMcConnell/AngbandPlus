...
/* wizard.c: wizard and debug modes */
...
static void wiz_statistics(object_type *o_ptr)
{
	...
	if (artifact_p(o_ptr) && !o_ptr->name3) a_info[o_ptr->name1].cur_num = 0;
	...
	while (TRUE)
	{
		...
		for (i = 0; i <= TEST_ROLL; i++)
		{
			...
			/* Mega-Hack -- allow multiple artifacts XXX XXX XXX */
			if (artifact_p(i_ptr) && !o_ptr->name3) a_info[i_ptr->name1].cur_num = 0;
	...
	if (artifact_p(o_ptr) && !o_ptr->name3) a_info[o_ptr->name1].cur_num = 1;
}
 ...
 * Summon some pets
 */
static void do_cmd_wiz_summon_pet(int num)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
	...
	for (i = 0; i < num; i++)
	{
		(void)summon_specific_pet(py, px, p_ptr->depth, 0);
	}
}
...
/*
 * Summon a pet of the specified type
 *
 * This function is rather dangerous XXX XXX XXX
 */
static void do_cmd_wiz_named_pet(int r_idx, bool slp)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
...
void acquire_uniq_art(int y, int x, int idx)
{
	artifact_type *u_ptr = &u_info[idx];
	object_type object_type_body;
	object_type *i_ptr;
...
void do_cmd_debug(void)
{
	...
	switch (cmd)
	{
		...
		case 'N':
		{
			do_cmd_wiz_named_pet(p_ptr->command_arg, TRUE);
			break;
		}
		...
		{
			if (p_ptr->command_arg <= 0) p_ptr->command_arg = 1;
			do_cmd_wiz_summon_pet(p_ptr->command_arg);
			break;
		}
		...
		{
			if (p_ptr->command_arg <= 0) p_ptr->command_arg = 1;
			acquire_uniq_art(py, px, p_ptr->command_arg);
			break;
		}
		...
		case 'y':
		{
			...
			if (p_ptr->command_arg <= 0) p_ptr->command_arg = randint(z_info->a_max);
			/* Get an item */
			q = "Make randart from what object? ";
			s = "You have nothing to create randart from.";
			if (!get_item(&item, q, s, (USE_EQUIP | USE_INVEN | USE_FLOOR))) return;
			...
			if (!scramble_artifact_random(p_ptr->command_arg, o_ptr)) msg_print("Failed!");
		...
		case 'X':
		{
			...
			monster_exp(p_ptr->target_who, p_ptr->command_arg * 10, 0);
