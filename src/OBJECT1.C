 ...
 */
void object_flags_old(object_type *o_ptr, u32b *f1, u32b *f2, u32b *f3)
{
...
{
  if (!o_ptr->art_flag_init)
  {
    object_flags_old(o_ptr, f1, f2, f3);
    o_ptr->art_flag_init = TRUE;
    o_ptr->art_flags1 = (*f1);
    o_ptr->art_flags2 = (*f2);
    o_ptr->art_flags3 = (*f3);
  }
	...
	switch (o_ptr->name1)
	{
        ...
        if (o_ptr->tval == TV_RING)
        {
            switch (o_ptr->sval)
            {
                   ...
                   {
                        return "teleport every 100+d200 turns";
                   }
                   case SV_RING_ICE:
                   {
                        return "resist cold every 40+d20 turns";
                   }
                   case SV_RING_FLAMES:
                   {
                        return "resist fire every 40+d20 turns";
                   }
                   case SV_RING_ACID:
                   {
                        return "resist acid every 40+d20 turns";
                   }
            }
        }
