...
static int Term_xtra_ibm_react(void)
{
	...
	if (use_color_complex)
	{
		...
		for (i = 0; i < 16; i++)
		{
			...
			rv = angband_colour_table[i][1] >> 2;
			gv = angband_colour_table[i][2] >> 2;
			bv = angband_colour_table[i][3] >> 2;
	...
	else
	{
		...
		for (i = 0; i < 16; i++)
		{
			...
			ibm_color_simple[i] = angband_colour_table[i][0];
		}
	}
...
errr init_ibm(void)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		angband_colour_table[i][0] = ibm_color_simple[i];
	}
