...
static grouper group_item[] = {
	...
	{TV_SPELLBOOK, "Spellbooks"},
	{TV_MIMIC_BOOK, "Books of Lore"},
...
static flag_desc brand_flags_desc[] = {
	...
	{TR1_BRAND_POIS, "Venom"}, /* Added by GJW -KMW- */
};
...
static void spoil_mon_info(cptr fname)
{
	...
	for (mon = 1; mon < MAX_R_IDX - 1; mon++)
	{
		...
		if (flags4 & (RF4_BR_MANA))
			vp[vn++] = "mana";
		if (flags4 & (RF4_BR_QUAKE))
			vp[vn++] = "space-time distortion";
		if (flags4 & (RF4_XXX8))
			vp[vn++] = "something";
		...
		vn = 0;
		if (flags7 & RF7_RES_BASH)
			vp[vn++] = "hafted weapons";
		if (flags7 & RF7_RES_STAB)
			vp[vn++] = "polearms";
		if (flags7 & RF7_RES_SLASH)
			vp[vn++] = "slashing weapons";
		...
		if (vn)
		{
			...
			{
				if (n == 0)
					spoil_out(" is protected against ");
				else if (n < vn - 1)
					spoil_out(", ");
				else
					spoil_out(" and ");
		...
		vn = 0;
		if (flags7 & RF7_IMM_BASH)
			vp[vn++] = "hafted weapons";
		if (flags7 & RF7_IMM_STAB)
			vp[vn++] = "polearms";
		if (flags7 & RF7_IMM_SLASH)
			vp[vn++] = "slashing weapons";
