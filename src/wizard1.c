...
static cptr attr_ttext(byte a)
{
   switch (a)
   {
      ...
      case TERM_L_BROWN: return ("L.Umber");
   }
 ...
 * Describe the kind
 */
static void kind_info(char *buf, char *dam, char *wgt, s16b *lev, s32b *val, s16b k)
{
   object_kind *k_ptr;
 ...
 * the other p1val affected traits to simplify the case where an object
 * affects all stats.  In this case, "All stats" is used instead of
 * listing each stat individually.
 */

static flag_desc stat1_flags_desc[] =
{
   { TR1_STR1,      "STR" },
   { TR1_INT1,      "INT" },
   { TR1_WIS1,      "WIS" },
...
static flag_desc stat2_flags_desc[] =
{
   { TR1_STR2,      "STR" },
   { TR1_INT2,      "INT" },
   { TR1_WIS2,      "WIS" },
 ...
 * which may be affected by an object's p1val
 */

static flag_desc p1val_flags1_desc[] =
{
   { TR1_STEALTH1,  "Stealth" },
   { TR1_SEARCH1,   "Searching" },
   { TR1_INFRA1,    "Infravision" },
   { TR1_TUNNEL1,   "Tunneling" },
   { TR1_BLOWS1,    "Attacks" },
   { TR1_SPEED1,    "Speed" },
   { TR1_SHOTS1,    "Shots" },
   { TR1_MIGHT1,    "Might" },
   { TR1_MAGIC1,    "Magic" },
   { TR1_VAMPIRIC1, "Vampirism" },
   { TR1_LITE1,     "Perm Lite" }
};

static flag_desc p2val_flags1_desc[] =
{
   { TR1_STEALTH2,  "Stealth" },
   { TR1_SEARCH2,   "Searching" },
   { TR1_INFRA2,    "Infravision" },
   { TR1_TUNNEL2,   "Tunneling" },
   { TR1_BLOWS2,    "Attacks" },
   { TR1_SPEED2,    "Speed" },
   { TR1_SHOTS2,    "Shots" },
   { TR1_MIGHT2,    "Might" },
   { TR1_MAGIC2,    "Magic" },
   { TR1_VAMPIRIC2, "Vampirism" },
   { TR1_LITE2,     "Perm Lite" }
};
...
/*
 * A special type used just for deailing with p1vals
 */
...
typedef struct
{
   ...
   */
   ...
   /*
   * A list of various player traits affected by an object's p1val such
   ...
   */
   cptr p1val_affects[N_ELEMENTS(stat1_flags_desc) - 1 +
                N_ELEMENTS(p1val_flags1_desc) + 1];
   cptr p2val_affects[N_ELEMENTS(stat2_flags_desc) - 1 +
                N_ELEMENTS(p2val_flags1_desc) + 1];
...
typedef struct
{
   ...
   /* Description of what is affected by an object's p1val */
 ...
 */
static void spoiler_out_n_chars(s16b n, char c)
{
 ...
 */
static void spoiler_blanklines(s16b n)
{
...
static cptr *spoiler_flag_aux(const u32b art_flags, const flag_desc *flag_ptr,
                              cptr *desc_ptr, const s16b n_elmnts)
{
   s16b i;
...
/*
 * List "player traits" altered by an artifact's p1val. These include stats,
 * speed, infravision, tunneling, stealth, searching, and extra attacks.
 */

static void analyze_p1val(object_type *i_ptr, pval_info_type *p_ptr)
{
   const u64b all_stats1 = (TR1_STR1 | TR1_INT1 | TR1_WIS1 |
                           TR1_DEX1 | TR1_CON1 | TR1_CHR1);

   u64b f1, f2, f3;

   cptr *affects_list;

   /* If p1val == 0, there is nothing to do. */
   if (!i_ptr->p1val)
   {
       /* An "empty" p1val description indicates that p1val == 0 */
       p_ptr->p1val_desc[0] = '\0';
       return;
   }
   ...
   affects_list = p_ptr->p1val_affects;

   /* Create the "+N" string */
   sprintf(p_ptr->p1val_desc, "%s%ld", POSITIZE(i_ptr->p1val), i_ptr->p1val);

   /* First, check to see if the p1val affects all stats */
   if ((f1 & all_stats1) == all_stats1)
   {
      *affects_list++ = "All stats";
   }

   /* Are any stats affected? */
   else if (f1 & all_stats1)
   {
      affects_list = spoiler_flag_aux(f1, stat1_flags_desc,
                                      affects_list,
                                      N_ELEMENTS(stat1_flags_desc));
   }

   /* And now the "rest" */
   affects_list = spoiler_flag_aux(f1, p1val_flags1_desc,
                                   affects_list,
                                   N_ELEMENTS(p1val_flags1_desc));

   /* Terminate the description list */
   *affects_list = NULL;
}

static void analyze_p2val (object_type *i_ptr, pval_info_type *p_ptr)
{
   const u64b all_stats2 = (TR1_STR2 | TR1_INT2 | TR1_WIS2 |
                           TR1_DEX2 | TR1_CON2 | TR1_CHR2);

   u64b f1, f2, f3;

   cptr *affects_list;

   /* If p2val == 0, there is nothing to do. */
   if (!i_ptr->p2val)
   {
       /* An "empty" p2val description indicates that p2val == 0 */
       p_ptr->p2val_desc[0] = '\0';
       return;
   }
   ...
   affects_list = p_ptr->p2val_affects;

   /* Create the "+N" string */
   sprintf(p_ptr->p2val_desc, "%s%d", POSITIZE(i_ptr->p2val), i_ptr->p2val);

   /* First, check to see if the p2val affects all stats */
   if ((f1 & all_stats2) == all_stats2)
   {
      *affects_list++ = "All stats";
   }

   /* Are any stats affected? */
   else if (f1 & all_stats2)
   {
      affects_list = spoiler_flag_aux(f1, stat2_flags_desc,
                                      affects_list,
                                      N_ELEMENTS(stat2_flags_desc));
   }

   /* And now the "rest" */
   affects_list = spoiler_flag_aux(f1, p2val_flags1_desc,
                                   affects_list,
                                   N_ELEMENTS(p2val_flags1_desc));
...
static void analyze_sustains (object_type *i_ptr, cptr *sustain_list)
{
   ...
   {
      sustain_list = spoiler_flag_aux(f2, sustain_flags_desc, sustain_list,
...
static void object_analyze(object_type *i_ptr, obj_desc_list *desc_ptr)
{
   ...
   analyze_p1val(i_ptr, &desc_ptr->pval_info);

   analyze_p2val(i_ptr, &desc_ptr->pval_info);
...
static void spoiler_print_art(obj_desc_list *art_ptr)
{
   ...
   /* An "empty" p1val description indicates that the p1val affects nothing */
   if (pval_ptr->p1val_desc[0])
   {
      /* Mention the effects of p1val */
      sprintf(buf, "%s to", pval_ptr->p1val_desc);
      spoiler_outlist(buf, pval_ptr->p1val_affects, ITEM_SEP);
   }

   /* An "empty" p2val description indicates that the p1val affects nothing */
   if (pval_ptr->p2val_desc[0])
   {
      /* Mention the effects of p1val */
      sprintf(buf, "%s to", pval_ptr->p2val_desc);
      spoiler_outlist(buf, pval_ptr->p2val_affects, ITEM_SEP);
   }
 ...
 */
static bool forge_artifact(s16b name1, object_type *i_ptr)
{
   s16b i;
...
static void spoil_mon_desc(cptr fname)
{
   ...
   for (i = 0; i < n; i++)
   {
      ...
      sprintf(exp, "%s '%c'", attr_ttext(r_ptr->d_attr), r_ptr->d_char);
 ...
 */
static void spoil_mon_info(cptr fname, bool ghost)
{
   char buf[1024];
   s16b msex, vn, i, j, k, n;
   s16b start, end;
   bool breath, magic, sin;
   cptr p, q;
   cptr vp[64];
   u64b flags1, flags2, flags3, flags4, flags5, flags6;
   ...
   for (n = start; n < end; n++)
   {
      ...
      /* Color */
      spoil_out(attr_ttext(r_ptr->d_attr));
      ...
      if (flags6 & RF6_S_TROLLS)          vp[vn++] = "summon trolls";
      ...
      if (flags6 & RF6_S_NUMBER16)
      {
         vp[vn++] = "in alarming numbers";
      }
      else if (flags6 & RF6_S_NUMBER8)
      {
         vp[vn++] = "in great numbers";
      }
      else if (flags6 & RF6_S_NUMBER4)
      {
         vp[vn++] = "in impressive numbers";
      }
      else if (flags6 & RF6_S_NUMBER2)
      {
         vp[vn++] = "in small numbers";
      }
...
void do_cmd_spoilers(void)
{
   ...
   while (1)
   {
      ...
      prt("(4) Full Monster Info (mon-info.spo + ghst-inf.spo)", 5, 8);
      ...
      {
         spoil_mon_info("mon-info.spo", FALSE);
         spoil_mon_info("ghst-inf.spo", TRUE);
      }

      /* Oops */
      else
      {
         bell("Unrecognized option");
      }
