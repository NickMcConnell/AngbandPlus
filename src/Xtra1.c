...
static void prt_frame_basic(void)
{
	...
	prt_field(class_sub_name[p_ptr->pclass][p_ptr->realm1], ROW_CLASS, COL_CLASS);
...
static void calc_bonuses(void)
{
        ...
        if (p_ptr->lev>14)
            {
                ...
                {
                    p_ptr->slow_digest = TRUE; /* Let's not make Regeneration
    ...
    /* Chaos Mage: resist chaos at level 20 */
    if ((p_ptr->pclass == CLASS_CHAOS_MAGE) && (p_ptr->lev > 19))
    {
            p_ptr->resist_chaos = TRUE;
    }
    ...
    /* Mystic: free action if unencumbered at level 25 */
    ...
    if ((p_ptr->pclass == CLASS_MYSTIC) && !(mystic_heavy_armor()))
    {
    ...
    if ((p_ptr->prace == RACE_KLACKON) || (p_ptr->prace == RACE_SPRITE) ||
        ((p_ptr->pclass == CLASS_MYSTIC) && !(mystic_heavy_armor())))
    {
	...
	if (o_ptr->k_idx && !p_ptr->heavy_wield)
	{
		...
		switch (p_ptr->pclass)
		{
			...
			/* Mage */
            case CLASS_MAGE: case CLASS_HIGH_MAGE: case CLASS_CHAOS_MAGE:
                num = 4; wgt = 40; mul = 2; break;

            /* Priest, Mindcrafter */
            case CLASS_PRIEST: case CLASS_MINDCRAFTER: case CLASS_DRUID:
            ...
            /* Chaos Warrior */
            case CLASS_CHAOS_WARRIOR: num = 5; wgt = 30; mul = 4; break;
	...
	/* Weapon penalty for Chaos Mages if not using a blade of chaos*/
	if ((p_ptr->pclass == CLASS_CHAOS_MAGE) && (inventory[INVEN_WIELD].k_idx) &&
		((o_ptr->tval != TV_SWORD) || (o_ptr->sval != SV_BLADE_OF_CHAOS)))
	{
		u32b f1,f2,f3;
		/* 'Chaotic' weapons are allowed */
		object_flags(o_ptr,&f1,&f2,&f3);
		if(!(f1 & TR1_CHAOTIC))
		{
			/* Reduce the real bonuses */
	...
	if (p_ptr->old_icky_wield != p_ptr->icky_wield)
	{
		...
		if (p_ptr->icky_wield)
		{
			if(p_ptr->pclass == CLASS_CHAOS_MAGE)
			{
