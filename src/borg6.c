...
static int borg_prepare_aux_big_heal(int p)
{
	...
	/* Perform healing */
	if (borg_quaff_potion(SV_POTION_STAR_HEALING) ||
	    borg_prayer(6, 2))
	{
		/* Return value */
		return (q);
...
static int borg_prepare_aux_haste(int p)
{
	...
	/* Haste */
	if (borg_activate_artifact(ART_FEANOR) ||
...
static int borg_prepare_aux_prot_evil(int p)
{
	...
	/* Apply protection */
	if (borg_read_scroll(SV_SCROLL_PROTECTION_FROM_EVIL) ||
	    borg_prayer(2, 4))
	{
		/* Return value */
		return (q);
...
bool borg_prepare(int p)
{
	...
	if (borg_unique_near)
	{
		...
		if (b_q > (borg_avoid * 3 / 4)) return (FALSE);
	}
	else
	{
		...
		/* Better to teleport than to die */
		if (b_q > (borg_avoid * 1 / 2)) return (FALSE);
	}
...
bool borg_caution(void)
{
	...
	/* Excessive danger */
	else if ((b_mq > b_ptr->mhp) && !borg_unique_level)
	{
		/* Start leaving */
		if (!borg_leaving)
		{
	...
	/* Moderate danger */
	else if ((b_q > b_ptr->mhp / 2) && !borg_unique_level)
	{
		/* Start leaving */
		if (!borg_leaving)
		{
	...
	if (borg_base_is_confused && (p > borg_avoid / 2))
	{
