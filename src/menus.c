...
void do_cmd_change_name(void)
{
	...
	/* Prompt */
	p = "[(c)hange name, (f)ile, (h/+)prev page, (-/l)next page, or ESC]";
	...
	while (1)
	{
		...
		else if ((ch == '+') || (ch == 'l'))
		{
			...
			else if ((!st_ptr->stock_num) && (mode == 4)) mode = 0;
		}
...
void do_cmd_colors(void)
{
 ...
 void do_cmd_note(char *note, int what_depth)
 {
	...
	{
		sprintf(depths, "Chest");
	}
	else if (what_depth == QUEST_LEVEL)
	{
		sprintf(depths, "Quest");
	}
	...
	if((length + length_info) > LINEWRAP)
	{
		...
		while (keep_going)
		{
			...
			if (startpoint) note_printf("|  continued...   |     |  ");
...
static void do_cmd_knowledge_notes(void)
{
	...
	/*open notes file for writing*/
	notes_file = my_fopen(buff, "w");

	/* Dump the notes */
	fwrite(note_base, note_cur - note_base, 1, notes_file);
...
void do_cmd_knowledge(void)
{
	...
	while (1)
	{
		...
		/* Allow the player to see the notes taken */
		prt("(6) Display character notes file", 9, 5);
