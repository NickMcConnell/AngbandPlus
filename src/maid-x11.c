 ...
 *
 * Hack -- Stolen from "util.c", where "gamma_load" should move to.
 */
...
/*
 * Hack -- Load a gamma table.
 */
static byte *gamma_load(cptr file)
{
	byte *gamma;
	...
	/* Allocate gamma table */
	C_MAKE(gamma, 256, byte);

	/* Load each line */
	for (y = 0; y < 16; y++)
	{
		/* Read next line of data, or die */
		if ((my_fgets(fff, buf, 1024) != 0) || (strlen(buf) < 32))
		{
			/* Free gamma table */
			C_KILL(gamma, 256, byte);
		...
		for (x = 0; x < 16; x++)
		{
			gamma[16*y+x] = 16 * dehex(buf[x*2]) + dehex(buf[x*2+1]);
...
static unsigned long create_pixel(Display *dpy, byte red, byte green, byte blue)
{
	...
	if (!checked)
	{
		/* Look up gamma filename */
		cptr str = getenv("ANGBAND_X11_GAMMA");

		/* Load gamma table (if requested) */
		if (str != NULL) gamma = gamma_load(str);
