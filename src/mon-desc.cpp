 ...
 */
static char *wd_he[3] = { "it", "he", "she" };
static char *wd_his[3] = { "its", "his", "her" };
...
/*
 * Buffer text, dumping full lines.
 *
    ...
    for (p = str; *p; p++) {
        ...
        if (wrap)
        {
            ...
            *roff_p = '\0';
            ...
            {
                box(0, roff_old*16, 639, roff_old*16+15, COLOR_BLACK);
                roff_old++;
            }

            /* Dump the line, advance the row */
            put_string(0, (roff_row++)*16, roff_buf, COLOR_WHITE);        
 ...
 */
static bool know_armor(int r_idx)
{
...
static void roff_aux(int r_idx)
{
    ...
    if (r_ptr->r_tkills)
    {
        ...
        if (TRUE)
        {
            ...
            i = (long)r_ptr->mexp * r_ptr->level / p_ptr->GetLev();

            /* calculate the fractional exp part scaled by 100, */
            /* must use long arithmetic to avoid overflow  */
            j = ((((long)r_ptr->mexp * r_ptr->level % p_ptr->GetLev()) *
                   (long)1000 / p_ptr->GetLev() + 5) / 10);
    ...
    if (flags6 & RF6_S_WRAITH)          vp[vn++] = "summon Ringwraiths";
    ...
    if (flags3 & RF3_RES_NETHER) vp[vn++] = "nether";
    if (flags3 & RF3_RES_WATER) vp[vn++] = "water";
    if (flags3 & RF3_RES_PLASMA) vp[vn++] = "plasma";
    if (flags3 & RF3_RES_NEXUS) vp[vn++] = "nexus";
    if (flags3 & RF3_RES_DISEN) vp[vn++] = "disenchantment";
    if (flags3 & RF3_RES_MAGIC) vp[vn++] = "magic";
...
static void roff_top(int r_idx)
{
    ...
    // Put together some of the string
    sprintf(out_val, "%s%s:",
        (r_ptr->flags1 & RF1_UNIQUE) ? "" : "The ", r_name + r_ptr->name);
