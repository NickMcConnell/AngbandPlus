...
const int MAX_IRQS = 4;          // timer + keyboard + soundcard + spare
const int STACK_SIZE = 8192;     // 8k stack should be plenty
...
byte *_irq_stack[IRQ_STACKS];

extern "C" void _irq_wrapper_0(), _irq_wrapper_1(),
...
int _install_irq(int num, int (*handler)())
{
   ...
   if (irq_virgin) {                /* first time we've been called? */
	 ...
	 _irq_stack[c] = (byte *) malloc(STACK_SIZE);
	 if (_irq_stack[c]) {
