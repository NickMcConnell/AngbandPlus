...
void player_flags(bitflag f[OF_SIZE])
{
	...
	if (player_has(PF_BRAVERY_30) && p_get_lev() >= 30)
		of_on(f, OF_RES_FEAR);
}
...
static const char *show_title(void)
{
	if (p_ptr->wizard)
		...
		return c_text + cp_ptr->title[(p_get_lev() - 1) / 5];
}

static const char *show_adv_exp(void)
{
	if (p_get_lev() < PY_MAX_LEVEL)
	{
		int pcidx;
		static char buffer[30];
		// find minimum XP to advance to next lvl in any class
		s32b advance = -1;
		for (pcidx = 0; pcidx < PY_MAX_CLASSES; pcidx++)
		{
			s32b cadv = player_exp[pc_array[pcidx].lev - 1] * pc_array[pcidx].expfact / 100L;
			if (advance < 0 || advance > cadv)
				advance = cadv;
		}
...
static int get_panel(int oid, data_panel *panel, size_t size)
{
  ...
  case 1:
  {
	...
	P_I(TERM_L_BLUE, "HP",	"%y/%y",	i2u(p_ptr->chp), i2u(p_get_mhp())  );
	P_I(TERM_L_BLUE, "SP",	"%y/%y",	i2u(p_ptr->csp), i2u(p_get_msp())  );
	P_I(TERM_L_BLUE, "Level",	"%y/%y",	i2u(p_get_lev()), i2u(p_get_best_lev())  );
  ...
  case 2:
  {
	...
	int pcidx;
	int max_lev = 0;
	s32b exp = 0, max_exp = 0;
	assert( ret >= boundaries[2].page_rows);
	ret = boundaries[2].page_rows;
	for (pcidx = 0; pcidx < PY_MAX_CLASSES; pcidx++)
	{
		max_lev = MAX(max_lev, pc_array[pcidx].max_lev);
		exp += pc_array[pcidx].exp;
		max_exp += pc_array[pcidx].max_exp;
	}
	P_I(max_color(p_get_lev(), max_lev), "Level", "%y/%y", i2u(p_get_lev()), i2u(p_get_best_lev())  );
	P_I(max_color(exp, max_exp), "Cur Exp", "%y", i2u(exp), END  );
	P_I(TERM_L_GREEN, "Max Exp",	"%y",	i2u(max_exp), END  );
