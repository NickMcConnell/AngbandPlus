...
static void display_player_stat_info(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		/* Deduct race and mod bonuses if in maximize */
		if (p_ptr->maximize)
		{
			e_adj -= rp_ptr->r_adj[i];
			e_adj -= p_ptr->stat_mod[i];
		}
...
{
	return (p_ptr->tot_exp + (100 * p_ptr->max_dlv));
}
