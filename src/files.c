...
static named_num gf_desc[] =
{
	...
	{"GF_CRUSHER_MOD",		GF_CRUSHER_MOD		},
	{"GF_BFG",					GF_BFG				},
	{"GF_DEST_MUND",			GF_DEST_MUND		},
	{NULL, 						0						}
};
...
static void display_player_middle(void)
{
	...
	prt_num("Level      ", (int)p_ptr->level, 9, 28, TERM_L_GREEN);
	...
	{
		prt_lnum("Exp to Adv.",
				(s32b)(player_exp[p_ptr->level - 1] * p_ptr->expfact / 100L),
				12, 28, TERM_L_GREEN);
	}
...
static void player_flags(u32b *f1, u32b *f2, u32b *f3)
{
	...
	/* Warriors... */
	if (((p_ptr->pclass == CLASS_WARRIOR) && (p_ptr->level>29))||
		((p_ptr->pclass == CLASS_PALADIN) && (p_ptr->level>39))||
		((p_ptr->pclass == CLASS_CHAOS_WARRIOR) && (p_ptr->level>39)))
		{
			(*f2) |= (TR2_RES_FEAR);
		}

	if ((p_ptr->pclass == CLASS_CHAOS_WARRIOR) && (p_ptr->level>29))
	{
		(*f2) |= (TR2_RES_CHAOS);
	}

	if ((p_ptr->pclass == CLASS_MONK) && (p_ptr->level > 9) &&
		!(monk_heavy_armor()))
	{
		(*f1) |= TR1_SPEED;
	}

	if ((p_ptr->pclass == CLASS_MONK) && (p_ptr->level>24) &&
		!(monk_heavy_armor()))
	{
		(*f2) |= (TR2_FREE_ACT);
	}
	...
	if (p_ptr->resist_chaos) (*f2) |= TR2_RES_CONF;
}
...
static void display_player_misc_info(void)
{
	...
	(void) sprintf(buf, "%d", (int) p_ptr->level);
...
static void print_tomb(void)
{
	...
	if (!done)
	{
		...
		/* King or Queen */
		if (total_winner || (p_ptr->level > PY_MAX_LEVEL))
		{
			p = "Magnificent";
		}
		...
		(void)sprintf(tmp, "Level: %d", (int)p_ptr->level);
...
static errr top_twenty(void)
{
	...
	sprintf(the_score.cur_lev, "%3d", p_ptr->level);
	sprintf(the_score.cur_dun, "%3d", p_ptr->depth);
	sprintf(the_score.max_lev, "%3d", p_ptr->max_level);
	...
	while (TRUE)
	{
		...
		if (my_fgets(fp, buf, 1024) == 0)
		{
			...
			if ((buf[0] == 'N') && (buf[1] == ':'))
			{
				...
				/* Check the entry */
				else if (!strncmp(&buf[2], entry, strlen(entry)))
				{
					found = TRUE;
					break;
				}
