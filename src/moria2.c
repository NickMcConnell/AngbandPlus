	  ...
	  if (pickup && inven_check_num(i_ptr))	   /* Too many objects?	    */
	      ...
	      if (pickup && !inven_check_weight(i_ptr))
		{
		  ...
		  if (locn == INVEN_QUIVER && using_quiver)
		    locn = 0; /*Force (a) to be printed for quiver ammo -JLS*/
...
int32u win;
{
  ...
  if (win & CM_WIN) {          /* MORGOTH */
   ...
   else if (win & SPECDROP)   /* Tiamat: Celestial Dragon Scale Mail -DGK */
   {
    register int i, j, k;
    register cave_type *c_ptr;
    int cdsm=FALSE;
	...
	if (!cdsm) {
	  ...
	  cur_pos = popt();
	  cave[j][k].tptr = cur_pos;
	  invcopy(&t_list[cur_pos], 419);
	  t_ptr = &t_list[cur_pos];
	  t_ptr->flags |= (TR_RES_FIRE|TR_RES_COLD|TR_RES_ACID|TR_POISON|
			   TR_RES_LIGHT);
	  t_ptr->flags2 |= (TR_RES_CONF|TR_RES_SOUND|TR_RES_LT|TR_RES_DARK|
			    TR_RES_CHAOS|TR_RES_DISENCHANT|TR_RES_SHARDS|
  ...
  if (m_ptr->hp < 0)
	...
	if (found) {
      ...
      i = monster_death((int)m_ptr->fy, (int)m_ptr->fx,
			...
			(c_list[m_ptr->mptr].cmove & (WINNER|SPECDROP)));
  ...
  else
   {
    ...
    if (!(m_ptr->monfear) && !(c_ptr->cmove & CM_ATTACK_ONLY) &&
...
int y, x;
{
  ...
  feel_chance = 20; /* Good chance of feeling -SAC */
  crptr = cave[y][x].cptr;
	  ...
	  if (mon_take_hit(crptr, k) >= 0)
	    {
		  ...
		  (c_list[monptr].cchar=='E' || c_list[monptr].cchar=='g'))
...
int dir, do_pickup;
{
  ...
  feel_chance = 500; /* General chance of feeling -SAC */
  ...
  if (mmove(dir, &y, &x))    /* Legal move?	      */
	  ...
	  || (!m_list[c_ptr->cptr].ml && c_ptr->fval >= MIN_CLOSED_SPACE))
	{
	  ...
	  if ((c_ptr->fval <= MAX_OPEN_SPACE) && /* Open floor spot	*/
	      ((cave[char_row][char_col].fval==LAVA_FLOOR) ||
	       (cave[y][x].fval!=LAVA_FLOOR) ||
	       (lava_flag=get_Yn("Lava! Continue?")) ))
	    {
	      ...
	      if (c_ptr->tptr != 0)
		{
		  ...
		  if (i == TV_INVIS_TRAP || i == TV_VIS_TRAP
		   ...
		   if (prompt_carry_flag && i!= TV_OPEN_DOOR
...
int *tbth, *tpth, *tdam, *tdis, *thits;
{
  ...
  *tdis = (((py.stats.use_stat[A_STR]+20)*10)/tmp_weight);
  ...
  /* And triple crossbows shoot three bolts -DGK */
  *thits = 1;
  ...
  if (inventory[INVEN_WIELD].tval == TV_BOW)
	...
	break;
      case 10: case 12:
	if (i_ptr->tval == TV_BOLT)      /* Light/Triple Crossbow and Bolt*/
	  {
	    *tbth = py.misc.bthb;
	    *tpth += 2 * inventory[INVEN_WIELD].tohit;
	    *tdam += inventory[INVEN_WIELD].todam;
	    *tdam = *tdam * 3;
	    *tdis = 25;
	    if (inventory[INVEN_WIELD].subval==12) /*Triple crossbow*/
	     *thits = 3;
	  }
	break;
      case 11:
...
inven_type *t_ptr;
{
  ...
  j = x;
  k = 0;
  if (randint(10) > 1 || t_ptr->flags2 & TR_ARTIFACT)
    {
      do
	{
	  if (in_bounds(i, j))
  ...
  if (flag && cave[i][j].fval==LAVA_FLOOR && set_fire_destroy(t_ptr))
    {
      objdes(tmp_str, t_ptr, FALSE);
      (void) sprintf(out_val, "The %s lands in lava and is destroyed!", tmp_str);
      msg_print(out_val);
    }
  else if (flag)
    {
...
int8u mode; /* TRUE if firing from quiver, FALSE from inventory -JLS */
{
  ...
  if (inven_ctr == 0 && !mode)
	...
	while (thits-- > 0)
	 {
	  if (inventory[INVEN_WIELD].subval==12) tpth-=10;
	  flag = FALSE;
	  y = char_row;
	  x = char_col;
	  oldy = char_row;
	      ...
	      if ((c_ptr->fval <= MAX_OPEN_SPACE) && (!flag))
		{
		  if (c_ptr->cptr > 1)
				   ...
				   (int)c_list[m_ptr->mptr].ac, CLA_BTHB))
			{
			  ...
			  if (throw_obj.flags2 & TR_ARTIFACT)
			   drop_throw(y,x,&throw_obj);
			}
		      else
...
int y, x;
{
  ...
  if (test_hit(base_tohit, (int)py.misc.lev,
      ...
      if (mon_take_hit(monster, k) >= 0)
	{
	      ...
	      (c_list[m_ptr->mptr].cdefense & UNDEAD) ||
	      (c_list[m_ptr->mptr].cchar=='E'||c_list[m_ptr->mptr].cchar=='g') )
	    (void) sprintf(out_val, "You have destroyed %s.", m_name);
          else
	    (void) sprintf(out_val, "You have slain %s.", m_name);
	  msg_print(out_val);
...
static char *look_mon_desc(int mnum){
  ...
  int8u living = !((c_list[m->mptr].cdefense & (UNDEAD | DEMON))||
		  c_list[m->mptr].cchar =='E'||c_list[m->mptr].cchar=='g');
