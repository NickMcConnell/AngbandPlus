...
/* cmd-move-attack.c: commands for moving and attacking monsters */
...
int tot_dam_aux(const object_type *o_ptr, int tdam, const monster_type *m_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_JUNK: /* Boulder */
		{
			...
			/* Brand (Cold) + Cold Touch */
			if (f1 & (TR1_BRAND_COLD) || p_ptr->cold_touch)
			{
				/* Notice immunity */
...
void py_attack(int y, int x)
{
	...
	monster_race *plr_ptr = &r_info[p_ptr->m_r_idx];
	...
	bool do_quake = FALSE;
        ...
        int flg = PROJECT_KILL | PROJECT_STOP | PROJECT_HIDE | PROJECT_PASS;
        int tx, ty;
	...
	/* Hack -- count the weapons */
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	if ((plr_ptr->body.weapon_mask & EQUIP_SLOT(i)) && inventory[i].tval)
		{
			nwpns++;
		}
	}

	/* No weapon equipped.  Try to use innate attacks. */
	if (!nwpns)
	{
	    	int num = 0;
		 ...
		 */
		if (plr_ptr->body.flags4 & (TR4_NO_INNATE))
		{
		    	plr_ptr = &r_info[0];
		}
		
		/* Notice if we have no attacks */
		if (!plr_ptr->blow[0].method)
		{
			msg_print("You cannot attack monsters!");
			return;
		}
		...
		/* Chance of hitting */
		chance = (p_ptr->skill_thn_bare + (bonus * BTH_PLUS_ADJ) + sleeping_bonus);
			
		for (num = 0; num < MONSTER_BLOW_MAX; num++)
		{
			int type = GF_ARROW, type2 = 0;

			if (!plr_ptr->blow[num].method)
				continue;

			/* Some monsters are great at dodging  -EZ- */
			if ((plr_ptr->flags2 & (RF2_EVASIVE)) && (!was_asleep) &&
				(!m_ptr->stunned) && (!m_ptr->confused) && (!m_ptr->monfear)
				&& (one_in_(2)))
			{
				message_format(MSG_MISS, 0, "%^s evades your attack!", m_name);
			...
			/* Test for hit */
			if (test_hit(chance, m_ptr->ac, m_ptr->ml))
			{
				int mul = 1;
			
				/* Get the method */
				switch (plr_ptr->blow[num].method)
				{
				...
				/* Get the effect */
				switch (plr_ptr->blow[num].effect)
				{
					...
					case RBE_EAT_LITE: type = GF_DARK; mul = 2; break;
				...
				k = damroll(plr_ptr->blow[num].d_dice * mul, plr_ptr->blow[num].d_side);
				/* Add to-dam bonus only if did some damage */
				if (k) k += p_ptr->to_d;
				if (k < 0) k = 0;
					
				/* Project rad. 0 ball on monster */
				project(-1, 0, y, x, y, x, k, type, flg, 0, 0);
				/* If there is an extra effect, project it also, using 4*level as power */
				if (type2)
				{
					project(-1, 0, y, x, y, x, 4 * p_ptr->lev, type2, flg, 0, 0);
				}
					
				/* Check death */
				if (m_ptr->fx != tx || m_ptr->fy != ty) return;
	...
	/* We have at least one weapon.  Fine, repeat for all of them. */
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	if ((plr_ptr->body.weapon_mask & EQUIP_SLOT(i)) && inventory[i].tval)
		{
			...
			/* Hack -- reduce for multiple weapons */
			bonus = bonus * (100 - 15 * (nwpns - 1)) / 100;
			...
			switch (o_ptr->tval)
			{
				case TV_SWORD:
				{
					chance = (p_ptr->skill_thn_edged + (bonus * BTH_PLUS_ADJ) + sleeping_bonus);
					break;
				}

				case TV_HAFTED:
				{
					chance = (p_ptr->skill_thn_hafted + (bonus * BTH_PLUS_ADJ) + sleeping_bonus);
					break;
				}

				/* Digging tools are close to polearms (and having a separate skill would be silly) */
				case TV_DIGGING:
				{
					chance = (p_ptr->skill_thn_polearm + (bonus * BTH_PLUS_ADJ) + sleeping_bonus);
					break;
				}
				...
				case TV_POLEARM:
				{
					...
					switch (o_ptr->sval)
					{
						case SV_BEAKED_AXE:
						case SV_BROAD_AXE:
						case SV_THROWING_AXE:
						case SV_BATTLE_AXE:
						case SV_GREAT_AXE:
						case SV_LOCHABER_AXE:
						{
							/* Axes */
							chance = (p_ptr->skill_thn_axe + (bonus * BTH_PLUS_ADJ) + sleeping_bonus);
							break;
						}
			...
			while (num++ < p_ptr->num_blow[i])
			{
				...
				/* Test for hit */
				else if (test_hit(chance, m_ptr->ac, m_ptr->ml))
				{
					if (was_asleep)
					{
						if (cp_ptr->flags & CF_ROGUE_COMBAT)
...
void move_player(int dir, int jumping)
{
	...
	x = px + ddx[dir];
        ...
        if (x < 0 || y < 0) return;
        if (x >= p_ptr->cur_map_wid || y >= p_ptr->cur_map_hgt) return;

	/* Hack -- attack monsters */
	if (cave_m_idx[y][x] > 0 && (mon_list[cave_m_idx[y][x]].align & (AL_HOSTILE_MASK)))
	{
		py_attack(y, x);
	}
	...
	/* Player can not walk through "walls" unless immaterial, but can go through traps */
	else if ((!cave_floor_bold(y, x) && !p_ptr->pass_wall) || (cave_feat[y][x] >= FEAT_PERM_EXTRA &&
								   cave_feat[y][x] <= FEAT_PERM_SOLID) ||
								  (cave_feat[y][x] == FEAT_LAVA && !p_ptr->immune_fire) ||
								  (cave_feat[y][x] == FEAT_ABYSS && !p_ptr->ffall))
        {
		...
		if (!(cave_info[y][x] & (CAVE_MARK)))
		{
			...
			/* Abyss */
			else if (cave_feat[y][x] == FEAT_ABYSS)
			{
				message(MSG_HITWALL, 0, "You nearly fall into an abyss!");
				cave_info[y][x] |= (CAVE_MARK);
				lite_spot(y, x);
			}
			...
			{
				message(MSG_HITWALL, 0, "You stop at the edge of a pool of hot lava.");
				cave_info[y][x] |= (CAVE_MARK);
				lite_spot(y, x);
			}
		...
		else
		{
			...
			{
				message(MSG_HITWALL, 0, "You stop on the edge of the abyss.");
				cave_info[y][x] |= (CAVE_MARK);
				lite_spot(y, x);
			}
			...
			{
				message(MSG_HITWALL, 0, "You can't make yourself enter the lava.");
				cave_info[y][x] |= (CAVE_MARK);
				lite_spot(y, x);
			}
	...
	else
	{
		...
		/* If there's a pet there, print message */
		if (cave_m_idx[y][x] > 0)
		{
