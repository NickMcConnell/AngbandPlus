...
void do_cmd_mimic()
{
  ...
  if (randint(100) < chance) {
    ...
    if (randint(100) < p_ptr->stat_ind[A_WIS]) {
        ...
        if (p_ptr->blind)
	{
        ...
        if (ext == 1)
        {
                ...
                if (c_ptr->feat == FEAT_RUBBLE)
                {
                        ...
                        object_aware(q_ptr);
                        object_known(q_ptr);
                        apply_magic(q_ptr, dun_level, TRUE, TRUE, (magik(20))?TRUE:FALSE, FALSE);
...
/* Check if we must call the flooding function */
void do_cmd_check_flooding()
{
	object_type	*o_ptr;

        o_ptr = &inventory[INVEN_NECK];
        if(o_ptr->name1 == ART_FLOODING)
        {
             do_cmd_flooding();
        }
}
