...
/* spoilers.c: spoilers generation */
...
static void kind_info(char *buf, char *dam, char *wgt, int *lev, s32b *val, int k)
{
	...
	/* Weight */
	sprintf(wgt, "%3ld.%ld", i_ptr->weight / 10, i_ptr->weight % 10);
}
...
static void spoil_artifact(cptr fname)
{
	...
	for (i = 0; group_artifact[i].tval; i++)
	{
		...
		for (j = 1; j < z_info->a_max; ++j)
		{
			...
			/* Mega-Hack -- Skip fake-artifact for randart generation */
			if (j == (z_info->a_max - 1)) continue;
