...
errr init_s_info_txt(FILE *fp, char *buf)
{
  ...
  while (0 == my_fgets(fp, buf, 1024))
      ...
      if (buf[0] == 'C')
	{
	  ...
	  /* Scan for the values */
	  if (1 != sscanf(buf+2, "%d", &number)) return (1);
      ...
      if (buf[0] == 'P')
	{
	  ...
	  /* Scan for the values */
	  if (2 != sscanf(buf+2, "%d:%d", &a_idx, &pval)) return (1);
	  ...
	  for (s = buf + 2; *s; )
	      ...
	      if (*t)
		{
		  *t++ = '\0';
	      ...
	      /* Parse this entry */
	      if (0 != grab_one_racial_flag(rp_ptr, s)) 
	  ...
	  for (s = buf + 2; *s; )
	      ...
	      if (*t)
		{
		  *t++ = '\0';
	      ...
	      /* Parse this entry */
	      if (0 != grab_one_special_racial_flag(rp_ptr, s)) 
		return (PARSE_ERROR_INVALID_FLAG);
 ...
 * Grab one special flag in a player_class from a textual string
 */
static errr grab_one_specialty(player_class *cp_ptr, cptr what, 
			       int cur_specialty)
{
  int i;
  ...
  while (0 == my_fgets(fp, buf, 1024))
	  ...
	  for (s = buf + 2; *s; )
	      ...
	      if (*t)
		{
		  *t++ = '\0';
	      ...
	      /* Parse this entry */
	      if (0 != grab_one_special_class_flag(cp_ptr, s)) 
		return (PARSE_ERROR_INVALID_FLAG);
      ...
      if (buf[0] == 'L') 
	{
	  ...
	  for (s = buf + 2; *s; )
	      ...
	      if (*t)
		{
		  *t++ = '\0';
	      ...
	      /* Parse this entry */
	      if (0 != grab_one_specialty(cp_ptr, s, cur_specialty)) 
		return (PARSE_ERROR_INVALID_FLAG);
	  ...
	  for (j = cur_specialty; j < CLASS_SPECIALTIES; j++) 
	    cp_ptr->specialties[j] = SP_NO_SPECIALTY;
