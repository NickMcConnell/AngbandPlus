...
/* spells-util.c: low-level spell function */
...
static byte spell_color(int type)
{
	...
	switch (type)
	{
		...
		case GF_WEBBING:	return (TERM_YELLOW);
		case GF_CRYO:		return (TERM_L_WHITE);
	}
...
static int minus_ac(void)
{
	...
	do
	{
	    	i = rand_int(INVEN_TOTAL - INVEN_EQUIP) + INVEN_EQUIP;
		o_ptr = &inventory[i];
	}
	while (!((r_ptr->body.body_mask | r_ptr->body.helm_mask | 
...
bool apply_disenchant(int mode)
{
	...
	while (!((r_ptr->body.body_mask | r_ptr->body.helm_mask | 
		     ...
		     r_ptr->body.weapon_mask | r_ptr->body.bow_mask) & EQUIP_SLOT(i)));
...
int charisma_adjustment(int y, int x)
{
	monster_type *m_ptr;
	monster_race *r_ptr;
...
static bool project_f(int who, int y, int x, int dist, int dam, int typ)
{
	...
	switch (typ)
	{
		/* Acid burns trees and grass */
		case GF_ACID:
		{
			if ((cave_feat[y][x] == FEAT_GRASS || cave_feat[y][x] == FEAT_TREE) &&
				rand_int(100) < dam)
			{
		 ...
		 * Cold and ice can:
		 *  - extinguish fire
		 *  - extinguish lava (unlikely)
		 *  - freeze water
		 *  - cover floor with ice (unlikely)
		 */
		case GF_COLD:
		case GF_ICE:
		{
			if (cave_feat[y][x] == FEAT_FIRE && rand_int(40) < dam)
			{
				cave_set_feat(y, x, FEAT_FLOOR);
			}
			else if (cave_feat[y][x] == FEAT_LAVA && dam > 100 && rand_int(100) < 2)
			{
				cave_set_feat(y, x, FEAT_ROCKS);
			}
			else if (cave_feat[y][x] == FEAT_WATER && rand_int(30) < dam)
			{
				cave_set_feat(y, x, FEAT_ICE);
			}
			else if (cave_feat[y][x] == FEAT_FLOOR && dam > 50 && rand_int(100) < 5)
			{
				cave_set_feat(y, x, FEAT_ICE);
			}
			break;
		...
		/* Holy orbs, electricity, meteors, shards, force, sound don't affect anything */
		/* Possible: - meteor works like fire; - shards destroy trees */
		case GF_HOLY_ORB:
		case GF_ELEC:		
		case GF_METEOR:
		case GF_SHARD:
		...
		/*
		 * Mana (sufficiently powerful attacks) can destroy any
		 * extra terrain feature (except abyss)
		 */
		case GF_MANA:
		{
			if (cave_feat[y][x] >= FEAT_WEB && cave_feat[y][x] != FEAT_ABYSS &&
				dam > 125 && rand_int(2) == 1)
			{
				cave_set_feat(y, x, FEAT_FLOOR);
			}
			break;
		...
		case GF_PLASMA:
		{
			if (cave_feat[y][x] == FEAT_WEB && rand_int(25) < dam)
			{
				cave_set_feat(y, x, FEAT_FLOOR);
				msg_print("Some webs are burned!");
			}
			else if (cave_feat[y][x] == FEAT_WATER && rand_int(40) < dam)
			{
				cave_set_feat(y, x, FEAT_FLOOR);
			}
			else if (cave_feat[y][x] == FEAT_ICE && rand_int(30) < dam)
			{
				cave_set_feat(y, x, FEAT_WATER);
			}
			else if ((cave_feat[y][x] == FEAT_GRASS || cave_feat[y][x] == FEAT_TREE) &&
				rand_int(30) < dam)
			{
				cave_set_feat(y, x, FEAT_FLOOR);
			}
		...
		{
			/* Need floor */
			if (cave_perma_bold(y, x) || !cave_floor_bold(y, x)) break;
			
			/*
			 * Do not allow to place webs over fire and lava.
			 * Note that we DO allow to place webs over the abyss,
			 * although in real life it would be possible only if
			 * the webs formed some bridge.
			 * Even more serious problem -- if the webs are removed
			 * (by fire), the abyss magically turns into the floor!
			 * This is hard to fix, and the fix will likely be a big hack.
			 */
			if (cave_feat[y][x] == FEAT_FIRE || cave_feat[y][x] == FEAT_LAVA) break;
		...
		case GF_MAKE_WATER:
		{
			...
			/* Disallow lava and abyss */
			if (cave_feat[y][x] == FEAT_LAVA || cave_feat[y][x] == FEAT_ABYSS) break;
		...
		case GF_MAKE_FIRE:
		{
			...
			/* Disallow ice and rocks */
			if (cave_feat[y][x] == FEAT_ICE || cave_feat[y][x] == FEAT_ROCKS) break;
			
			/* Make the fire */
			cave_set_feat(y, x, FEAT_FIRE);
		...
		case GF_MAKE_FOG:
		{
			...
			/* Disallow lava and rocks */
			if (cave_feat[y][x] == FEAT_LAVA || cave_feat[y][x] == FEAT_ROCKS) break;
		...
		case GF_MAKE_GRASS:
		{
			...
			/* Disallow extra features */
			if (cave_feat[y][x] >= FEAT_WATER && cave_feat[y][x] <= FEAT_ABYSS) break;
		...
		case GF_MAKE_TREE:
		{
			...
			/* Need grass */
			if (cave_feat[y][x] != FEAT_GRASS) break;
		...
		case GF_MAKE_WALL:
		{
			...
			/* Disallow "extreme" features */
			if (cave_feat[y][x] == FEAT_ICE || cave_feat[y][x] == FEAT_LAVA ||
				cave_feat[y][x] == FEAT_ROCKS || cave_feat[y][x] == FEAT_ABYSS) break;
			
			/* Determine the kind of wall and make it */
			k = rand_int(3);
			switch (k)
			{
				case 0: cave_set_feat(y, x, FEAT_WALL_SOLID); break;
				case 1: cave_set_feat(y, x, FEAT_MAGMA);      break;
				case 2: cave_set_feat(y, x, FEAT_QUARTZ);     break;
			}
		...
		case GF_AWAKE_TREE:
		{
			...
			/* Summon */
			summon_specific_pet(y, x, 40 + dam, SUMMON_ENT);
...
bool project_m(int who, int y, int x, int dam, int typ, u32b flg)
{
	...
	/* Were the effects "beneficial" for monster? */
	bool good = FALSE;
	...
	if (!cave_floor_bold(y,x) && !(flg & PROJECT_PASS)) return (FALSE);
	...
	switch (typ)
	{
		...
		/* Freezing -- Slow down, small chance of instakill */
		case GF_CRYO:
		{
			if (seen) obvious = TRUE;
			
			if (one_in_(30) && !(r_ptr->flags1 & (RF1_UNIQUE)))
			{
				note_dies = " freezes to solid state and shatters!";
				dam = m_ptr->hp + 1;
			}
			else if (strchr("acdDeFiIJKlMnRSw", r_ptr->d_char))
			{
				note = " has its metabolism slowed down.";
				if (m_ptr->mspeed > 70) m_ptr->mspeed -= (5 + randint(5));
			}
			else
			{
				if (r_ptr->flags3 & (RF3_IM_COLD))
		...
		case GF_OLD_DRAIN:
		{
			...
			if (monster_nonliving(r_ptr))
			{
				...
				obvious = FALSE;
				dam = 0;
				/* Still remains "offending", even if mob is actually immune */
			}
		...
		case GF_OLD_POLY:
		{
			...
			/* No "real" damage */
			dam = 0;
			/* Poly'd monsters lose pet status anyway */
		...
		case GF_AWAY_UNDEAD:
		{
			...
			/* No "real" damage */
			dam = 0;
			good = TRUE; /* Assume TP-away is not offending */
			break;
		}
		...
		case GF_MAKE_WARY:
		{
			...
			/* So are pets */
			if (m_ptr->align & (AL_PET_MASK)) skipped = TRUE;
			...
			/* Offence? None taken! I'll just watch you a little more */
			good = TRUE;
		...
		case GF_CHARM:
		{
			/* Hack -- don't affect pets. They're already charmed. */
			if (m_ptr->align & (AL_PET_MASK))
			{
				dam = 0;
				skipped = TRUE;
			...
			/* Cannot charm uniques. Others get a good saving throw (though no resist) */
			if ((r_ptr->flags1 & (RF1_UNIQUE)) ||
			    ((r_ptr->level + randint(MAX(4, r_ptr->level / 2))) >
				 (randint(dam) + charisma_adjustment(y, x))))
			{
	...
	/* -AU- Quest monsters canot be polymorphed too */
	if (do_poly)
	{
		int i;
		for (i = 0; i < z_info->q_max; i++)
		{
			...
			if (q_ptr->mon_idx == m_ptr->r_idx && q_ptr->active_level == p_ptr->depth) do_poly = FALSE;
		}
	}
	...
	if (r_ptr->flags1 & (RF1_UNIQUE))
	{
		...
		/* Hack -- or by quylthulg's pets */
		/* Hack -- or by Valar */
		/* Hack -- or by anyone if they are our pets */
		if ((who > 0) && (dam > m_ptr->hp))
		{
			monster_type *sm_ptr = &mon_list[who];
			monster_race *sr_ptr = &r_info[sm_ptr->r_idx];
			
			if (((sm_ptr->align & (AL_PET_MASK)) && r_info[p_ptr->m_r_idx].d_char == 'Q') ||
				(sr_ptr->flags3 & (RF3_VALA)) || (m_ptr->align & (AL_PET_MASK)))
			{
				/* Do nothing, apply that damage as usual */
			}
			else
			{
				dam = m_ptr->hp;
	...
	if (who > 0)
	{
		...
		else
		{
			...
			/* Hack -- Pain message -- only if seen! */
			else if (dam > 0 && m_ptr->ml) message_pain(cave_m_idx[y][x], dam);
			
			/* Hack -- If no current target, get angry! */
			if (dam > 0 && m_ptr->target < 0 && m_ptr->align != mon_list[who].align)
			{
				m_ptr->target = who;
			}
	...
	else
	{
		...
		else
		{
			...
			/* Damaged a pet? You scum! */
			if ((m_ptr->align & (AL_PET_MASK)) && !good)
			{
				msg_format("%^s is enraged by your attack!", m_name);
				m_ptr->off += OFFENSE_ONE;
				
				if (m_ptr->off > OFFENSE_CRITICAL)
				{
					msg_format("%^s refuses to be the friend of a traitor!", m_name);
					if (m_ptr->align == AL_PET_L)
						m_ptr->align = AL_HOSTILE_L;
					else if (m_ptr->align == AL_PET_C)
						m_ptr->align = AL_HOSTILE_C;
					else
						m_ptr->align = AL_HOSTILE;
				}
				else if (m_ptr->off > OFFENSE_WARNING)
				{
					msg_format("%^s seems to be very angry.", m_name);
					/* Just a warning */
				}
			}
...
static bool project_p(int who, int y, int x, int dam, int typ)
{
	...
	/* Paranoia - if the monster is pet, no damage */
	/* XXX XXX XXX This may be unbalancing. Need to investigate */
	...
	switch (typ)
	{
		...
		case GF_FIRE:
		{
			if (blind) msg_print("You are hit by fire!");
                        ...
                        {
                                /* Damage 2.5x */
                                msg_print("You are hit hard by fire!");
                                dam *= 5;
                                dam /= 2;
                        }
                        if (p_ptr->cold_touch)
                        {
                        	/* Temp. unresistable blindness */
                        	msg_print("You partially evaporate.");
                        	msg_print("Cloud of thick steam appears!");
                        	set_blind(p_ptr->blind + 2);
			}
			fire_dam(dam, killer);
			break;
		}
		...
		case GF_POIS:
		{
			if (blind) msg_print("You are hit by poison!");
                        if (p_ptr->undead || p_ptr->immune_pois)
                        {
                                dam = 0;
                        }
                        else
		...
		case GF_HOLY_ORB:
		{
			if (blind) msg_print("You are hit by a holy force!");
                        ...
                        {
                                /* Damage 2.5x */
                                msg_print("You are hit hard by the holiness!");
                                dam *= 5;
                                dam /= 2;
                        }          
			take_hit(dam, killer);
		...
		case GF_NETHER:
		{
			if (blind) msg_print("You are hit by something strange!");
                        ...
                        {
                                /* Heal dam * 1/3 */
                                msg_print("Nether forces heal you!");
                                hp_player(dam / 3);
                                /* No damage */
                                dam = 0;
                        }
                        else if (p_ptr->resist_nethr)
			{
		...
		case GF_WIND:
		{
			if (blind) msg_print("You are buffeted by winds!");
			if (dam > randint(200))
			{
		...
		case GF_LITE:
		{
			if (blind) msg_print("You are hit by something!");
                        ...
                        {
                                /* Damage 2x */
                                msg_print("You are hit hard by light!");
                                dam *= 2;
                        }
		...
		case GF_TIME:
		{
			if (blind) msg_print("You are hit by something strange!");
                        ...
                        {
                                /* No damage */
                                msg_print("As an undead, you are immune to aging.");
                                dam = 0;
                        }
 ...
 */
void calc_starburst(int height, int width, byte *arc_first,
...
void spread_cave_temp(int y1, int x1, int range, bool room, bool walls)
{
	...
	for (i = 0; i < temp_n; i++)
	{
		...
		if (!cave_floor_bold(y, x) && !walls) continue;
		...
		if (flg & (PROJECT_STAR))
		{
			calc_starburst(1 + rad * 2, 1 + rad * 2, arc_first, arc_dist,
				&arc_num);
                        ...
                        spread_cave_temp(y0, x0, rad, FALSE, FALSE);
		}
