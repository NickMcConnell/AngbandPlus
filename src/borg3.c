...
void borg_item_analyze(auto_item *item, cptr desc)
{
	...
	switch (item->tval)
	{
		...
		case TV_ROD:
		{
			...
			if (item->iqty > 1)
			{
				if (streq(tail, format("(%d charging)",item->iqty))) item->pval = 0;
			}
			else
			{
		...
		case TV_RING:
		{
			...
			for (i = m; i >= 0; i--)
			{
				...
				if (prefix(tail, ab_ptr->artego_text[i]))
				{
					...
					{
						item->name1 = (byte)ab_ptr->artego_what[i];
					}
	...
	/* Remove close curly */
    /* XXX The game sometimes fails to put one on(!) */
    if (buf[strlen(buf)-1] == c2)
    {
        buf[strlen(buf)-1] = '\0';
    }
	...
	if (prefix(buf, "BORG_"))
	{
        ...
        /* XXX We try to recognize most enscriptions even if the game
           chops them off short */

        /* Randart */
        else if (prefix(buf+5, "ART"))
        {
            /* XXX XXX XXX Steal the info from the game */
        ...
        {
            if (prefix(buf+5, "RES_AC")) item->flags2 |= (TR2_RES_ACID);
            else if (prefix(buf+5, "RES_EL")) item->flags2 |= (TR2_RES_ELEC);
            else if (prefix(buf+5, "RES_FI")) item->flags2 |= (TR2_RES_FIRE);
            else if (prefix(buf+5, "RES_COL")) item->flags2 |= (TR2_RES_COLD);
            else if (prefix(buf+5, "RES_PO")) item->flags2 |= (TR2_RES_POIS);
            else if (prefix(buf+5, "RES_FE")) item->flags2 |= (TR2_RES_FEAR);
            else if (prefix(buf+5, "RES_LI")) item->flags2 |= (TR2_RES_LITE);
            else if (prefix(buf+5, "RES_DA")) item->flags2 |= (TR2_RES_DARK);
            else if (prefix(buf+5, "RES_BL")) item->flags2 |= (TR2_RES_BLIND);
            else if (prefix(buf+5, "RES_CON")) item->flags2 |= (TR2_RES_CONFU);
            else if (prefix(buf+5, "RES_SO")) item->flags2 |= (TR2_RES_SOUND);
            else if (prefix(buf+5, "RES_SH")) item->flags2 |= (TR2_RES_SHARD);
            else if (prefix(buf+5, "RES_NEX")) item->flags2 |= (TR2_RES_NEXUS);
            else if (prefix(buf+5, "RES_NET")) item->flags2 |= (TR2_RES_NETHR);
            else if (prefix(buf+5, "RES_CH")) item->flags2 |= (TR2_RES_CHAOS);
            else if (prefix(buf+5, "RES_DI")) item->flags2 |= (TR2_RES_DISEN);
		}
		...
		{
            if (prefix(buf+5, "SUST_ST")) item->flags2 |= (TR2_SUST_STR);
            else if (prefix(buf+5, "SUST_IN")) item->flags2 |= (TR2_SUST_INT);
            else if (prefix(buf+5, "SUST_WI")) item->flags2 |= (TR2_SUST_WIS);
            else if (prefix(buf+5, "SUST_DE")) item->flags2 |= (TR2_SUST_DEX);
            else if (prefix(buf+5, "SUST_CO")) item->flags2 |= (TR2_SUST_CON);
            else if (prefix(buf+5, "SUST_CH")) item->flags2 |= (TR2_SUST_CHR);
		}

		/* Misc (flags3) */
		else
		{
            if (prefix(buf+5, "SLO")) item->flags3 |= (TR3_SLOW_DIGEST);
            else if (prefix(buf+5, "FEA")) item->flags3 |= (TR3_FEATHER);
            else if (prefix(buf+5, "LIT")) item->flags3 |= (TR3_LITE);
            else if (prefix(buf+5, "REG")) item->flags3 |= (TR3_REGEN);
            else if (prefix(buf+5, "TEL")) item->flags3 |= (TR3_TELEPATHY);
            else if (prefix(buf+5, "SEE")) item->flags3 |= (TR3_SEE_INVIS);
            else if (prefix(buf+5, "FRE")) item->flags3 |= (TR3_FREE_ACT);
            else if (prefix(buf+5, "HOL")) item->flags3 |= (TR3_HOLD_LIFE);
		}
	}
...
void borg_send_item_index(int i)
{
	...
	else
	{
		...
		borg_keypress((char)I2A(i - INVEN_WIELD));
	}
}
...
/*
 * Determine two items are very similar.
 *
 ...
 *
 * This is very similar to, but slightly different from, object_similar().
 *
 * Adapted by -RML
 */
bool borg_object_similar(const auto_item *o_ptr, const auto_item *j_ptr)
{
    int total = o_ptr->iqty + j_ptr->iqty;


    /* Hack -- require full knowlege of both items */
    if (!o_ptr->kind) return (FALSE);
    if (!j_ptr->kind) return (FALSE);
...
/*
 * Determine fail rate of a spell 
 */
int borg_spell_fail(int book, int what, bool now)
{
	auto_magic *as = &borg_magics[book][what];
	int chance, minfail;

	if (now) if (!borg_spell_okay(book, what)) return (100);
	...
	if(now) if (borg_base_is_stun) chance += 25;
...
/*
 * Determine fail rate of a prayer
 */
int borg_prayer_fail(int book, int what, bool now)
{
	auto_magic *as = &borg_magics[book][what];
	int chance, minfail;

	if (now) if (!borg_prayer_okay(book,what)) return (100);
...
void borg_watch_equip(int which)
{
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
	{
		...
		if ((streq(buf, borg_items[i].desc)) && (!borg_amnesia_equip)) continue;

		/* Analyze the item (no price) */
		borg_item_analyze(&borg_items[i], buf);

		/* Hack -- Check if it is (still) cursed */
		...
		{
			borg_items[i].flags3 &= ~(TR3_LIGHT_CURSE);
			borg_items[i].flags3 &= ~(TR3_HEAVY_CURSE);
		}



		/* Hack remove the uncursed inscription */
		/* No inscription XXX XXX */
		if (!borg_items[i].note[0]) continue;
		
#if 0
		/* Ignore open curly */
		strcpy(buf, borg_items[i].note+1);
...
void borg_watch_inven(int which)
{
	...
	for (i = 0; i < INVEN_PACK; i++)
	{
		...
		/* Ignore "unchanged" items */
		if ((streq(buf, borg_items[i].desc)) && (!borg_amnesia_inven)) continue;
	...
	{
		{
			...
			BORG_MAGIC_EXT	/* D "Find Hidden Traps/Doors" */,
	...
	{
		{
			...
			BORG_MAGIC_EXT	/* D "Find Traps" */,
			BORG_MAGIC_EXT	/* D "Detect Doors/Stairs" */,
...

		{
            ...
            BORG_MAGIC_OBJ  /* O "Elemental Brand" */,
			BORG_MAGIC_ICK	/*   "(blank)" */,
			BORG_MAGIC_ICK	/*   "(blank)" */,
			BORG_MAGIC_ICK	/*   "(blank)" */
		},
...
void borg_prepare_book(int book)
{
	...
	for (what = 0; what < num; what++)
	{
		...
		const magic_type *s_ptr = &mb_ptr->info[spell[what]];
