...
/* cmd-object.c: commands for inventory manipulation */
...
static bool item_tester_hook_wear(const object_type *o_ptr)
{
	...
	if (wield_slot(o_ptr) >= INVEN_EQUIP) return (TRUE);
...
#if 0  /* Player can have six rings in PosBand */
...
static bool item_tester_refill_lantern(const object_type *o_ptr)
{
	...
	/* Non-empty, non-artifact lanterns are okay */
...
static void do_cmd_refill_lamp(void)
{
	...
	/* Get the lantern */
	/* XXX Having >1 lantern is NOT supported. */
...
{
	/* Non-artifact torches are okay */
	if ((o_ptr->tval == TV_LITE) &&
	    (o_ptr->sval == SV_LITE_TORCH) &&
	    !artifact_p(o_ptr)) return (TRUE);
...
void do_cmd_refill(void)
{
	...
	/* Get the light */
	/* XXX Multiple lights are not supported.  I don't think this is needed. */
	for (i = INVEN_EQUIP; i < INVEN_TOTAL; i++)
	{
	    	if (r_ptr->body.light_mask & EQUIP_SLOT(i)) break;
	}
	...
	/* It's a lamp */
	else if (o_ptr->sval == SV_LITE_LANTERN && !artifact_p(o_ptr))
	{
		do_cmd_refill_lamp();
	}

	/* It's a torch */
	else if (o_ptr->sval == SV_LITE_TORCH && !artifact_p(o_ptr))
	{
		do_cmd_refill_torch();
	}
...
static cptr ident_info[] =
{
	...
	"%:A vein (magma or quartz); an animated tree",
	/* "&:unused", */
	"':An open door",
	"(:Soft armor",
	"):A shield",
	"*:A vein with treasure",
	"+:A closed door; a tree",
	",:Food (or mushroom patch)",
	"-:A wand (or rod)",
	".:Floor (normal, sand, grass, water, lava etc.)",
