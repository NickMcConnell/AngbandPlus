...
{
    #define RF(a, b) #a,
    #include "../../common/list-mon-flags.h"
    #undef RF
    NULL
};
...
static int find_blow_method(const char *name)
{
    ...
    {
        if (streq(name, r_info_blow_method[i])) break;
    }
...
static int find_blow_effect(const char *name)
{
    ...
    {
        if (streq(name, r_info_blow_effect[i])) break;
    }
...
static enum parser_error parse_r_b(struct parser *p)
{
    ...
    {
        if (!r->blow[i].method) break;
    }
    if (i == MONSTER_BLOW_MAX) return PARSE_ERROR_TOO_MANY_ENTRIES;
    r->blow[i].method = find_blow_method(parser_getsym(p, "method"));
    if (!r_info_blow_method[r->blow[i].method]) return PARSE_ERROR_UNRECOGNISED_BLOW;
    if (parser_hasval(p, "effect"))
    {
        r->blow[i].effect = find_blow_effect(parser_getsym(p, "effect"));
        if (!r_info_blow_effect[r->blow[i].effect]) return PARSE_ERROR_INVALID_EFFECT;
    }
...
static enum parser_error parse_r_drop(struct parser *p)
{
    ...
    if ((parser_getuint(p, "min") >= MAX_STACK_SIZE) || (parser_getuint(p, "max") >= MAX_STACK_SIZE))
...
static int lookup_artifact_name(const char *name)
{
    ...
    for (i = 1; i < z_info->a_max; i++)
    {
        ...
        /* Test for equality */
        if (a_ptr->name && streq(name, a_ptr->name)) return i;
...
static enum parser_error parse_r_drop_artifact(struct parser *p)
{
    ...
    if (art < 0) return PARSE_ERROR_GENERIC;
    a = &a_info[art];
...
static enum parser_error parse_r_mimic(struct parser *p)
{
    ...
    if (!kind) return PARSE_ERROR_GENERIC;
...
static errr finish_parse_r(struct parser *p)
{
    ...
    while (r)
    {
        if (r->ridx > z_info->r_max) z_info->r_max = r->ridx;
        r = r->next;
    }
    z_info->r_max++;
