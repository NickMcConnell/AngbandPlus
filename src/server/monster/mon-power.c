...
static long eval_blow_effect(int effect, int atk_dam, int rlev)
{
    switch (effect)
    {
        ...
        /* PWMAngband effects */
        case RBE_FORGET: atk_dam += 10; break;
        case RBE_DISEASE:
        {
            /* Poison + lose CON */
            atk_dam *= 5;
        ...
        case RBE_TIME: atk_dam += 20; break;
        case RBE_DISARM: atk_dam += 30; break;
        case RBE_FAMINE: atk_dam += 5; break;
...
static long eval_max_dam(monster_race *r_ptr)
{
    ...
    int dam = 1;
    int boost = (rsf_has(r_ptr->spell_flags, RSF_HASTE)? 5: 0);
    ...
    if (flags_test(r_ptr->flags, RF_SIZE, RF_KILL_WALL, RF_PASS_WALL, FLAG_END))
        ...
        melee_dam = melee_dam * 3 + melee_dam * get_energy(r_ptr->speed + boost) / 7;

    /*
     * Scale based on attack accuracy.
     * We make a massive number of assumptions here and just use monster level.
     */
    ...
    r_ptr->spell_dam = spell_dam;
    r_ptr->melee_dam = melee_dam;
     ...
     */
    dam = dam * get_energy(r_ptr->speed + boost) / 10;

    /*
     * Adjust threat for speed -- multipliers are more threatening.
     */
    if (rf_has(r_ptr->flags, RF_MULTIPLY))
        r_ptr->highest_threat = r_ptr->highest_threat * get_energy(r_ptr->speed + boost) / 5;
...
static long eval_hp_adjust(monster_race *r_ptr)
{
    ...
    if (rf_has(r_ptr->flags, RF_NEVER_MOVE) && !r_ptr->freq_spell)
    {
        hp /= 2;
        if (hp < 1) hp = 1;
    }
    ...
    if (flags_test(r_ptr->spell_flags, RSF_SIZE, RSF_TPORT, RSF_TELE_AWAY, RSF_TELE_LEVEL,
        FLAG_END))
            hp = (hp * 6) / 5;
...
errr eval_r_power(struct monster_race *races)
{
    ...
    power = C_ZNEW(z_info->r_max, u32b);
    ...
    for (iteration = 0; iteration < 3; iteration++)
    {
        ...
        for (i = 0; i < z_info->r_max; i++)
        {
            /* Point at the "info" */
            r_ptr = &races[i];
            boost = (rsf_has(r_ptr->spell_flags, RSF_HASTE)? 5: 0);
            ...
            else
            {
                ...
                if ((lvl <= 100) && !rf_has(r_ptr->flags, RF_UNIQUE))
                {
                    long mexp = (hp * dam) / 25;
                    ...
                    /* Set level */
                    if (arg_rebalance) r_ptr->level = lvl;
                }
            ...
            {
                if (rf_has(r_ptr->flags, RF_FRIEND)) power[i] *= 2;
            ...
            {
                if (flags_test(r_ptr->flags, RF_SIZE, RF_KILL_WALL, RF_PASS_WALL, FLAG_END))
                    power[i] = MAX(power[i], power[i] * get_energy(r_ptr->speed + boost));
                else if (flags_test(r_ptr->flags, RF_SIZE, RF_OPEN_DOOR, RF_BASH_DOOR, FLAG_END))
                    power[i] = MAX(power[i], power[i] * get_energy(r_ptr->speed + boost) * 3 / 2);
                else
                    power[i] = MAX(power[i], power[i] * get_energy(r_ptr->speed + boost) / 2);
            }
            ...
            for (j = lvl; j < (lvl == 0 ? lvl + 1: MAX_DEPTH); j++)
            {
                 ...
                 * add several to the count so that the averages don't get
                 * thrown off
                 */
                ...
                {
                    if (flags_test(r_ptr->flags, RF_SIZE, RF_KILL_WALL, RF_PASS_WALL, FLAG_END))
                        count = MAX(1, get_energy(r_ptr->speed + boost)) * count;
                    else if (flags_test(r_ptr->flags, RF_SIZE, RF_OPEN_DOOR, RF_BASH_DOOR, FLAG_END))
                        count = MAX(1, get_energy(r_ptr->speed + boost) * 3 / 2) * count;
                    else
                        count = MAX(1, get_energy(r_ptr->speed + boost) / 2) * count;
                }
        ...
        for (i = 0; i < z_info->r_max; i++)
        {
            ...
            if ((tot_hp[lvl] != 0) && (tot_dam[lvl] != 0))
            {
                ...
                {
                    /* Get power */
                    u32b new_power = r_ptr->power;

                    /* Compute rarity algorithmically */
                    for (j = 1; ((new_power > j) && (j < 256)); new_power -= j * j, j++);
