 ...
 * the player.
 *
 * f is the set of flags we're checking
 * flag is the object flag for which we are checking.
 */
bool check_state_aux(struct player *p, bitflag f[OF_SIZE], int flag)
{
    const struct object_flag *of_ptr;
...
bool check_state(struct player *p, int flag)
{
    return check_state_aux(p, p->state.flags, flag);
}
