...
static void process_player_end(int Ind)
{
	...
	if (!(turn%(level_speed(p_ptr->dun_depth)/12)))
	{
		...
		{
			(void)set_tim_meditation(Ind, p_ptr->tim_meditation - 1);
		}
		...
		if (p_ptr->tim_wraith)
		{
			...
			{
				(void)set_tim_wraith(Ind, p_ptr->tim_wraith - 1);
			}
		}
		...
		{
			(void)set_invis(Ind, p_ptr->tim_invisibility - 1, p_ptr->tim_invis_power);
		}
