...
static errr init_v_info(void)
{
	...
	fake_name_size = 30 * 1024L;
	fake_text_size = 63 * 1024L;
...
static errr init_other(void)
{
	...
	/* Array of grids */
	C_MAKE(view_g, VIEW_MAX, grid);

	/* Array of grids */
	C_MAKE(temp_g, TEMP_MAX, grid);
	...
	/* Padded info array */
	C_MAKE(cave_info, MAX_HGT, byte_256);

	/* Feature array */
	C_MAKE(cave_feat, MAX_HGT, byte_256);

	/* Entity arrays */
	C_MAKE(cave_o_idx, MAX_HGT, s16b_wid);
	C_MAKE(cave_m_idx, MAX_HGT, s16b_wid);

	/* Flow arrays */
	C_MAKE(cave_cost, MAX_HGT, byte_wid);
	C_MAKE(cave_when, MAX_HGT, byte_wid);
