...
bool set_mental_barrier(int v)
{
	...
	if (v)
	{
		...
		{
			msg_print("Your psyche is shielded from mental attacks!");
			p_ptr->update |= PU_BONUS;
			notice = TRUE;
		}
	}
...
/*
 * Set "p_ptr->awareness", notice observable changes
 */
bool set_awareness(int v)
{
	bool notice = FALSE;
...
/*
 * Set "p_ptr->inertial_barrier", notice observable changes
 */
bool set_inertial_barrier(int v)
{
	bool notice = FALSE;
	...
	else
	{
		if (p_ptr->inertial_barrier)
		{
			msg_print("Your inertial barrier dissipates.");
			notice = TRUE;
		}
	}

	/* Use the value */
	p_ptr->inertial_barrier = v;
...
bool set_ts_anchor(int v)
{
	...
	else
	{
		if (p_ptr->ts_anchor)
		{
			msg_print("Your block on extradimensional space unravels.");
			notice = TRUE;
		}
	}
...
/*
 * Set "p_ptr->precognition", notice observable changes
 */
bool set_precognition(int v)
{
	bool notice = FALSE;
