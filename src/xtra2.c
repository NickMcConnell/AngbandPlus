...
bool gain_corruption(int choose_mut)
{
	...
	while (attempts_left--)
	{
		...
		case 85: case 86:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_LARGE_HORNS;
			muta_desc = "You grow large horns on your head!";
			break;
		case 87: case 88:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_ATT_DEMON;
			muta_desc = "You start attracting demons.";
			break;
		case 89:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_PROD_MANA;
			muta_desc = "You start producing magical energy uncontrollably.";
			break;
		case 90: case 91:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_SPEED_FLUX;
			muta_desc = "You become manic-depressive.";
			break;
		case 92: case 93:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_BANISH_ALL;
			muta_desc = "You feel a terrifying power lurking behind you.";
			break;
		case 94:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_EAT_LIGHT;
			muta_desc = "You feel a strange kinship with the night.";
			break;
		case 95: case 96:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_SMALL_HORNS;
			muta_desc = "You grow small horns on your head.";
			break;
		case 97:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_ATT_ANIMAL;
	...
	else
	{
		...
		*(muta_class) |= muta_which;
		...
		{
			if (muta_which == COR3_PUNY)
			{
				if (p_ptr->muta3 & COR3_HYPER_STR)
				{
					msg_print("You no longer feel super-strong!");
					p_ptr->muta3 &= ~(COR3_HYPER_STR);
				}
			}
			else if (muta_which == COR3_HYPER_STR)
			{
				if (p_ptr->muta3 & COR3_PUNY)
				{
					msg_print("You no longer feel puny!");
					p_ptr->muta3 &= ~(COR3_PUNY);
				}
			}
			else if (muta_which == COR3_IDIOTIC)
			{
				if (p_ptr->muta3 & COR3_HYPER_INT)
				{
					msg_print("You are no longer a genius.");
					p_ptr->muta3 &= ~(COR3_HYPER_INT);
				}
			}
			else if (muta_which == COR3_HYPER_INT)
			{
				if (p_ptr->muta3 & COR3_IDIOTIC)
				{
					msg_print("You are no longer an idiot.");
					p_ptr->muta3 &= ~(COR3_IDIOTIC);
				}
			}
			else if (muta_which == COR3_IRON_SKIN)
			{
				if (p_ptr->muta3 & COR3_SCALES)
				{
					msg_print("You lose your scales.");
					p_ptr->muta3 &= ~(COR3_SCALES);
				}
				if (p_ptr->muta3 & COR3_FLESH_ROT)
				{
					msg_print("Your flesh rots no longer.");
					p_ptr->muta3 &= ~(COR3_FLESH_ROT);
				}
				if (p_ptr->muta3 & COR3_WART_SKIN)
				{
					msg_print("You lose your warts.");
					p_ptr->muta3 &= ~(COR3_WART_SKIN);
				}
			}
			else if (muta_which == COR3_WART_SKIN || muta_which == COR3_SCALES
				|| muta_which == COR3_FLESH_ROT)
			{
				if (p_ptr->muta3 & COR3_IRON_SKIN)
				{
					msg_print("Your skin is no longer made of steel.");
					p_ptr->muta3 &= ~(COR3_IRON_SKIN);
				}
			}
			else if (muta_which == COR3_FEARLESS)
			{
				if (p_ptr->muta2 & COR2_COWARDICE)
				{
					msg_print("You are no longer cowardly.");
					p_ptr->muta2 &= ~(COR2_COWARDICE);
				}
			}
			else if (muta_which == COR3_FLESH_ROT)
			{
				if (p_ptr->muta3 & COR3_REGEN)
				{
					msg_print("You stop regenerating.");
					p_ptr->muta3 &= ~(COR3_REGEN);
				}
			}
			else if (muta_which == COR3_REGEN)
			{
				if (p_ptr->muta3 & COR3_FLESH_ROT)
				{
			...
			else if (muta_which == COR3_LIMBER)
			{
				if (p_ptr->muta3 & COR3_ARTHRITIS)
				{
					msg_print("Your joints stop hurting.");
					p_ptr->muta3 &= ~(COR3_ARTHRITIS);
				}
			}
			else if (muta_which == COR3_ARTHRITIS)
			{
				if (p_ptr->muta3 & COR3_LIMBER)
				{
					msg_print("You no longer feel limber.");
					p_ptr->muta3 &= ~(COR3_LIMBER);
				}
			}
		}
		else if (muta_class == &(p_ptr->muta2))
		{
			if (muta_which == COR2_COWARDICE)
			{
				if (p_ptr->muta3 & COR3_FEARLESS)
				{
					msg_print("You no longer feel fearless.");
					p_ptr->muta3 &= ~(COR3_FEARLESS);
				}
			}
			if (muta_which == COR2_LARGE_HORNS)
			{
				if (p_ptr->muta2 & COR2_SMALL_HORNS)
				{
					msg_print("Your horns grow larger.");
					p_ptr->muta2 &= ~(COR2_SMALL_HORNS);
				}
			}
			if (muta_which == COR2_SMALL_HORNS)
			{
				if (p_ptr->muta2 & COR2_LARGE_HORNS)
				{
					msg_print("Your horns shrink.");
					p_ptr->muta2 &= ~(COR2_LARGE_HORNS);
				}
			}
		}
		p_ptr->update |= PU_BONUS;
...
bool lose_corruption(int choose_mut)
{
	...
	while (attempts_left--)
	{
		...
		case 17: case 18:
			muta_class = &(p_ptr->muta1);
			muta_which = COR1_SLIME;
			muta_desc = "You lose the power to summon hellslime.";
			break;
		case 19: case 20:
			muta_class = &(p_ptr->muta1);
			muta_which = COR1_VAMPIRISM;
		...
		case 78:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_FORGET;
			muta_desc = "Your amnesia clears up!";
			break;
		case 79:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_HALLU;
		...
		case 80:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_BRIMSTONE;
			muta_desc = "You are no longer a conduit to hell fires.";
			break;
		case 81: case 82:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_POISON_FANGS;
			muta_desc = "You lose your poison fangs!";
			break;
		case 83: case 84:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_CLAWS;
		...
		case 85: case 86:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_LARGE_HORNS;
			muta_desc = "Your horns shrivel and fall off!";
			break;
		case 87: case 88:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_ATT_DEMON;
		...
		case 98:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_SPINES;
			muta_desc = "Your spines vanish from your joints.";
			break;
		case 99:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_RAW_CHAOS;
		...
		case 110: case 111:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_PATRON;
			muta_desc = "You lose the attention of the infernal patron.";
			break;
		case 112:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_INTUITION;
			muta_desc = "You lose your strange intuition.";
			break;
		case 113: case 114:
			muta_class = &(p_ptr->muta2);
			muta_which = COR2_WARNING;
		...
		case 143: case 144:
			muta_class = &(p_ptr->muta3);
			muta_which = COR3_FORKED_TONGUE;
			muta_desc = "Your tongue returns to normal.";
			break;
		case 145:
			muta_class = &(p_ptr->muta3);
			muta_which = COR3_ILL_NORM;
		...
		case 151: case 152: case 153:
			muta_class = &(p_ptr->muta3);
			muta_which = COR3_STENCH;
			muta_desc = "You stop smelling so bad!";
			break;
		case 154: case 155: case 156:
			muta_class = &(p_ptr->muta3);
			muta_which = COR3_INFRAVIS;
		...
		case 157: case 158:
			muta_class = &(p_ptr->muta3);
			muta_which = COR3_GOAT_LEGS;
			muta_desc = "Your hooves return to normal feet!";
			break;
		case 159: case 160:
			muta_class = &(p_ptr->muta3);
			muta_which = COR3_SHORT_LEG;
...
void dump_corruptions(FILE * OutFile)
{
	...
	if (p_ptr->muta1)
	{
		...
		{
			fprintf(OutFile, " You can summon waves of hellslime.\n");
		}
		...
		{
			fprintf(OutFile, " You can emit confusing, blinding lights.\n");
		}
	...
	if (p_ptr->muta2)
	{
		...
		{
			fprintf(OutFile, " You are subject to fits of amnesia.\n");
		}
		...
		{
			fprintf(OutFile, " You are occasionally surrounded with brimstone.\n");
		}
		...
		{
			fprintf(OutFile, " You have poison fangs (poison, 3d7).\n");
		}
		if (p_ptr->muta2 & COR2_CLAWS)
		{
			fprintf(OutFile, " You have razor sharp claws (dam. 2d6).\n");
		}
		if (p_ptr->muta2 & COR2_LARGE_HORNS)
		{
			fprintf(OutFile, " You have large horns on your head (dam. 2d4).\n");
		}
		...
		if (p_ptr->muta2 & COR2_SMALL_HORNS)
		{
			fprintf(OutFile, " You have small horns on your head (dam 1d4).\n");
		}
		...
		{
			fprintf(OutFile, " You have bony spines on your joints (dam 2d5).\n");
		}
		...
		if (p_ptr->muta2 & COR2_PATRON)
		{
			fprintf(OutFile, " An infernal patron gives you gifts.\n");
		}
		if (p_ptr->muta2 & COR2_INTUITION)
		{
			fprintf(OutFile, " You notice when miracles change the world.\n");
		}
	...
	if (p_ptr->muta3)
	{
		...
		if (p_ptr->muta3 & COR3_HYPER_INT)
		{
			fprintf(OutFile, " You are a genius (+4 INT/WIS).\n");
		}
		if (p_ptr->muta3 & COR3_IDIOTIC)
		{
			fprintf(OutFile, " You are an idiot (-4 INT/WIS).\n");
		}
		...
		if (p_ptr->muta3 & COR3_FLESH_ROT)
		{
			fprintf(OutFile, " Your flesh is rotting (-2 CON, -1 CHA).\n");
		}
		if (p_ptr->muta3 & COR3_SILLY_VOI)
		{
			fprintf(OutFile, " Your voice is a hoarse rasp (-4 CHA).\n");
		}
		if (p_ptr->muta3 & COR3_FORKED_TONGUE)
		{
			fprintf(OutFile, " You have a forked tongue (-1 CHA).\n");
		}
		...
		{
			fprintf(OutFile, " You have glowing eyes (+15 search).\n");
		}
		...
		{
			fprintf(OutFile, " You have a stench of the grave about you (-3 stealth).\n");
		}
		...
		{
			fprintf(OutFile, " You have goat legs with cloven hooves (+3 speed).\n");
		}
		...
		if (p_ptr->muta3 & COR3_WART_SKIN)
		{
			fprintf(OutFile, " Your skin is covered with warts (-2 CHA, +5 AC).\n");
		}
		if (p_ptr->muta3 & COR3_SCALES)
		{
			fprintf(OutFile, " Your skin has turned into scales (-1 CHA, +10 AC).\n");
		}
		if (p_ptr->muta3 & COR3_IRON_SKIN)
		{
