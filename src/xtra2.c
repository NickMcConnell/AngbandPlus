...
/*
 * Generic set "p_ptr->oppose_xxx" flag, notice observable changes
 */
bool set_oppose(int v, s16b* opposer, char* type)
{
	bool notice = FALSE;
	char buf[80];
	...
	if (v)
	{
		if (!(*opposer))
		{
			sprintf(buf, "You feel resistant to %s!", type);
			msg_print(buf);
			notice = TRUE;
		}
	}
	...
	else
	{
		if (*opposer)
		{
			sprintf(buf, "You feel less resistant to %s.", type);
			msg_print(buf);
			notice = TRUE;
		}
	}
