	...
	if (v)
	{
                if (!p_ptr->tim_invisible)
		{
			msg_print("You feel your body fade away.");
                        ...
                        {
                                p_ptr->csp += (p_ptr->skill_stealth + (p_ptr->skill_stealth * ((p_ptr->abilities[(CLASS_SHADOW * 10) + 8] * 10) / 100)));
                        }
			notice = TRUE;
                        update_and_handle();
		}
	       ...
	       (p_ptr->exp >= (player_exp[p_ptr->lev-1] * p_ptr->expfact / 100L)))
	{
		...
		if (p_ptr->lev > p_ptr->max_plv)
		{
			...
			if (p_ptr->muta2 & MUT2_CHAOS_GIFT)
			{
                ...
                /* You always gain 2 stat points. You gain at least 1 */
                /* skill point, maybe more if you have a high wisdom. */
                if (p_ptr->lev >= p_ptr->max_plv && nostats == FALSE)
                {
                        p_ptr->statpoints += 2;                        
                        if (p_ptr->prace == RACE_HIGH_ELF || p_ptr->prace == RACE_BENEMAL || p_ptr->prace == RACE_DEMON) p_ptr->skillpoints += 1 + (p_ptr->stat_ind[A_WIS] / 20);
                        else p_ptr->skillpoints += 2 + (p_ptr->stat_ind[A_WIS] / 20);
		...
		msg_format("Welcome to level %d.", p_ptr->lev);
                /* If you are a skatter with skatter arrows in quiver, upgrade them! */
                apply_skatter_quiver_magic();
                /* Same with Valkyrie Spear */
                apply_valkyrie_weapon_magic();
	...
	else if (r_ptr->flags1 & (RF1_DROP_CHOSEN))
	{
                if (strstr((r_name + r_ptr->name),"Morgoth, Lord of Darkness"))
		{
			...
			drop_near(q_ptr, -1, y, x);
                ...
                else if (strstr((r_name + r_ptr->name),"Bentz The Ranger"))
                {
			/* Get local object */
			q_ptr = &forge;
                        ...
                        /* Mega-Hack -- Prepare to make a blue dragon scale mail */
                ...
                else
		{
                        ...
                        else if (strstr((r_name + r_ptr->name),"Eol the Dark Elf"))
			{
                        ...
                        else if (strstr((r_name + r_ptr->name),"The Executioner"))
			{
			...
			q_ptr = &forge;
                        ...
                        /* Mega-Hack -- Prepare to make a ring of invisibility */
                        object_prep(q_ptr, lookup_kind(TV_TOOL, SV_TOOL_CLIMB));
                        q_ptr->number = 1;
			...
			drop_near(q_ptr, -1, y, x);
                        ...
                        else if (strstr((r_name + r_ptr->name),"PlatinumStone The Solid"))
			{
                                /* Get local object */
                                q_ptr = &forge;
                                ...
                                /* Mega-Hack -- Prepare to make a ring of invisibility */
                                object_prep(q_ptr, lookup_kind(TV_HELM, SV_VALKYRIAN_HELM));
                                q_ptr->number = 1;
                        ...
                        else if (strstr((r_name + r_ptr->name),"Seaneyen, Legendary Water Creature"))
			{
                                a_idx = ART_ASTRA;
                                chance = 100;
			}
                        else if (strstr((r_name + r_ptr->name),"Ryenna, Queen Of Battle"))
			{
                                a_idx = ART_DRAGOS;
                                chance = 100;
			}
                        else if (strstr((r_name + r_ptr->name),"Finisher"))
			{
			...
			q_ptr = &forge;
                        ...
                        /* Mega-Hack -- Prepare to make a Nuclear Blade */
                        object_prep(q_ptr, lookup_kind(TV_SWORD, 42));
                        q_ptr->number = 1;
			...
			drop_near(q_ptr, -1, y, x);
                        ...
                        else if (strstr((r_name + r_ptr->name),"Algatar The Handsome"))
			{
                                ...
                                /* Mega-Hack -- Prepare to make a parchement */
                                object_prep(q_ptr, lookup_kind(8, 21));
                                q_ptr->number = 1;
                        ...
                        else if (strstr((r_name + r_ptr->name),"Medeola, Young Dancer Of Chaos"))
			{
                                ...
                                object_prep(q_ptr, lookup_kind(36, 20));
                                q_ptr->number = 1;
                                ...
                                /* Message... */
                                msg_print("A voice echoes: 'You dared to kill my beautiful dancer! You have unleashed the wrath of Dalisabah The Heartbreaker! Come fight me if you dare, pathetic mortal! I'm waiting in my lair...");
			}
                        else if (strstr((r_name + r_ptr->name),"Dryad") && !is_pet(m_ptr) && m_ptr->angered_pet == 0 && randint(100) >= 75)
			{
                                ...
                                object_prep(q_ptr, lookup_kind(36, 19));
                                q_ptr->number = 1;
	...
	if (m_ptr->hp < 0)
	{
		...
		else
		{
			p_ptr->exp_frac = new_exp_frac;
                ...
                /* Gain more experience for higher level monsters */
                if (m_ptr->level >= 20) new_exp *= (m_ptr->level / 20);
...
bool gain_random_mutation(int choose_mut)
{
	...
	while (attempts_left--)
	{
                ...
                /* Disabled the abilities mutations for now... */
                /* They are kind of weak... */
                switch(choose_mut?choose_mut:(randint(120)+74))
		{
...
void great_side_effect(void) {
        ...
        switch(p_ptr->pgod)
        {
                ...
                case GOD_GOD:
                        msg_print("God summon some Archons to help you!");
                        for(a = 0; a < 5 + (5 * power); a++)
                        {
                                int y = py, x = px;

                                scatter(&y, &x, py, px, 5, 0);
                                place_monster_one(y, x, test_monster_name("Archon"), FALSE, TRUE);
                        }
                        break;
