	...
	else if (r_ptr->flags1 & (RF1_DROP_CHOSEN))
	{
                ...
                else
		{
			...
			int I_kind = 0;
                        ...
                        if (strstr((r_name + r_ptr->name),"T'ron , the rebel DragonRidder"))
			{
                                a_idx = ART_TRON;
                                chance = 75;
			}
        ...
        /* Hack - the Dragondriders give some firescone */
        else if (r_ptr->flags3 & RF3_DRAGONRIDDER)
	{
		/* Get local object */
		q_ptr = &forge;

                /* Prepare to make some Firescones */
                object_prep(q_ptr, lookup_kind(TV_FIRESCONE, SV_FIRESCONE));
...
bool gain_random_corruption(int choose_mut)
{
	...
	while (attempts_left--)
	{
		...
		case 115:
			muta_class = &(p_ptr->muta2);
			muta_which = MUT2_DIABLO;
			muta_desc = "You are blessed with fits of Diablo(tm)ness.";
			break;
		case 116: case 117:
			muta_class = &(p_ptr->muta2);
...
bool lose_corruption(int choose_mut)
{
	...
	while (attempts_left--)
	{
		...
		case 115:
			muta_class = &(p_ptr->muta2);
			muta_which = MUT2_DIABLO;
			muta_desc = "You are no longer blessed with Diablo(tm)ness.";
			break;
		case 116: case 117:
			muta_class = &(p_ptr->muta2);
			muta_which = MUT2_SP_TO_HP;
...
void dump_corruptions(FILE * OutFile)
{
	...
	if (p_ptr->muta2)
	{
		...
		{
			fprintf(OutFile, " You occasionally turn into Diablo(tm).\n");
		}
