...
static void wr_extra(void)
{
	...
	/* Dag - save all the racial stuff. */
	/* This is so the mutants don't get messed up. */
	for (i = 0; i < 6; i++) wr_s16b(rp_ptr->r_adj[i]);

	wr_s16b(rp_ptr->r_dis);
	wr_s16b(rp_ptr->r_dev);
	wr_s16b(rp_ptr->r_sav);
	wr_s16b(rp_ptr->r_stl);
	wr_s16b(rp_ptr->r_srh);
	wr_s16b(rp_ptr->r_fos);
	wr_s16b(rp_ptr->r_thn);
	wr_s16b(rp_ptr->r_thb);

	wr_byte(rp_ptr->r_mhp);
	wr_byte(rp_ptr->r_exp);

	wr_byte(rp_ptr->b_age);
	wr_byte(rp_ptr->m_age);

	wr_byte(rp_ptr->m_b_ht);
	wr_byte(rp_ptr->m_m_ht);
	wr_byte(rp_ptr->m_b_wt);
	wr_byte(rp_ptr->m_m_wt);

	wr_byte(rp_ptr->f_b_ht);
	wr_byte(rp_ptr->f_m_ht);
	wr_byte(rp_ptr->f_b_wt);
	wr_byte(rp_ptr->f_m_wt);

	wr_byte(rp_ptr->infra);
	wr_byte(rp_ptr->choice);
