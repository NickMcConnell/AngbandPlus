...
static hist_type bg[] =
{
	...
	/* Dag - the mutant descs */

	{"You are the product of ", 100, 67, 68, 50},

	{"a genetic defect.  ", 20, 68, 69, 50},
	{"a horrible accident.  ", 40, 68, 69, 50},
	{"a strange crossbreed. ", 60, 68, 69, 50},
	{"a mad wizard's experiment.  ", 80, 68, 69, 50},
	{"an unknown occurance.  ", 100, 68, 69, 50},
...
/*
 * Dag - the xp multiplier for mutants goes up with bonuses to
 * various things (stats, abilities)
 */
static int mutant_xp_mod;
...
static void get_stats(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		/* Set the mutants racial adjust randomly */
		if(p_ptr->prace == RACE_MUTANT)	rp_ptr->r_adj[i] = rand_int(9) - 4;

		/* Adjust the xp mod */
		mutant_xp_mod = (i==A_CHR) ? rp_ptr->r_adj[i] * 2 : rp_ptr->r_adj[i] * 8;
	...
	/*
	 * Dag - Set the extra skills for mutants and adjust the xp
	 * mod accodingly
	 */
	if(p_ptr->prace == RACE_MUTANT)
	{
		rp_ptr->r_dis = (rand_int(31)-15);
		rp_ptr->r_dev = (rand_int(31)-15);
		rp_ptr->r_sav = (rand_int(31)-15);
		rp_ptr->r_stl = (rand_int(23)-11);
		rp_ptr->r_srh = (rand_int(31)-15);
		rp_ptr->r_fos = rand_int(20);
		rp_ptr->r_thn = (rand_int(31)-15);
		rp_ptr->r_thb = (rand_int(31)-15);
...
static void get_extra(void)
{
	...
	/* Hitdice */
	/* Dag - Random mutant hp mod */
	if(p_ptr->prace==RACE_MUTANT) 
	{
		rp_ptr->r_mhp = rand_int(21);
		if (rp_ptr->r_mhp < 3) rp_ptr->r_mhp = 3;
	}
	p_ptr->hitdie = rp_ptr->r_mhp + cp_ptr->c_mhp;

	/* Set the xp_mod to the hp bonus */
	mutant_xp_mod += (rp_ptr->r_mhp - 8) * 4;

	/* Hack -- no really low xp factors */
	if(mutant_xp_mod < -10) mutant_xp_mod = -10;	


	/* Experience factor */
	if(p_ptr->prace==RACE_MUTANT) rp_ptr->r_exp = (110 + (mutant_xp_mod));
...
static void get_money(void)
{
	...
	/* Minimum 100 gold */ /* Dag - Added bonus */
	if (gold < 100) gold = 100 + rand_int(30);
...
static bool player_birth_aux()
{
...
#if 0
	if(p_ptr->prace==RACE_MUTANT && autoroll)
	{
		autoroll = 0;
		msg_print("Sorry, but the autoroller is disabled for mutants.");
		msg_print(NULL);
	}
#endif
	...
	if (autoroll)
	{
		...
		if (p_ptr->prace == RACE_MUTANT)
		{
		put_str("Mutants: Since your stats are random, the values below are the", 10, 2);
		put_str("concievable maximums, but it's unlikely they will reach that.", 11, 2);
		put_str("Also, to limit scumming the RNG for a really good mutant, the", 12, 2);
		put_str("autoroller is slowed down for mutants.", 13, 2);
		}
	...
	while (TRUE)
	{
		...
		while (autoroll)
		{
			...
			/* Hack -- Prevent overflow */ /* If anyone rolls this high... */
			if (auto_round >= 2000000000L) break;
			...
			if (flag || (auto_round < last_round + 100))
			{
				...
				/* Delay 1/10 second */
				if (flag && p_ptr->prace == RACE_MUTANT) Term_xtra(TERM_XTRA_DELAY, 200);
				else if (flag) Term_xtra(TERM_XTRA_DELAY, 100);
	...
	/* Hack -- Set the Mutant seed */
	seed_mutant = rand_int(0x10000000);
