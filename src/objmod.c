...
{
	int num_hit, num_dam, num_ac, retval;

	/* Parse arguments */
	if (!PyArg_ParseTuple(args, "iii", &num_hit, &num_dam, &num_ac))
		return NULL;

	/* Enchant */
	retval = enchant_spell(num_hit, num_dam, num_ac);
...
void initobject()
{
	...
	add_constant(d, "TV_JUNK", TV_JUNK);
	...
	add_constant(d, "ART_HITHLOMIR", ART_HITHLOMIR);
