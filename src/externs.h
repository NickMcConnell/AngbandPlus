...
extern int8u equippy_chars;
extern int8u quick_msgs;
extern int8u delay_spd; /* 1-10 for delays -DGK */
#ifdef TC_COLOR
...
#endif
extern int8u using_quiver; /* Boolean status of quiver variable.  Should be
			      set if quiver in use, cleared otherwise. -JLS */
extern int8u equip_top;    /* Should always be equivalent to (using_quiver?
			      INVEN_QUIVER:INVEN_WIELD).  Needed because
			      sometimes the slot INVEN_QUIVER is a quiver
			      so becomes the top of the equipment list.  The
			      rest of the time it's a slot at the bottom of
			      the inventory.  - JLS */
#else
...
extern int using_quiver; /* Boolean status of quiver variable.  Should be
			    set if quiver in use, cleared otherwise. -JLS */
extern int equip_top;    /* Should always be equivalent to (using_quiver?
			    INVEN_QUIVER:INVEN_WIELD).  Needed because
			    sometimes the slot INVEN_QUIVER is a quiver
			    so becomes the top of the equipment list.  The
			    rest of the time it's a slot at the bottom of
...
TULKAS, NECKLACE, BARAHIR, CASPANION, RAZORBACK, BLACKARR;
...
#ifdef MSDOS
extern int8u unfelt;
extern int16u feel_chance;        /* feeling chance -SAC */
extern int8u spec_level;          /* special level type -DGK */
extern int8u in_store_flag;       /* flag so equippy chars work right -DGK */
...
extern int8u light_rad;         /* Light radius */
...
#else
extern int unfelt;
extern int feel_chance;         /* feeling chance -SAC */
extern int spec_level;          /* special level type -DGK */
...
/* misc1.c */
void give_1_hi_resist(struct inven_type *);
...
void use_quiver(struct inven_type *, int);
...
int immerse_lava(ARG_INT ARG_COMMA ARG_INT);
...
int spikedoor(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
...
/* misc1.c */
void give_1_hi_resist(ARG_INV_PTR);
...
void prt_equippy_chars(ARG_VOID);
...
void use_quiver(ARG_INV_PTR ARG_COMMA ARG_INT);
...
int set_plasma_destroy(ARG_INV_PTR);
int set_meteor_destroy(ARG_INV_PTR);
int set_holy_destroy(ARG_INV_PTR);
int set_mana_destroy(ARG_INV_PTR);
...
int light_area(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
...
void mon_light_dam(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
void light_line(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
void frost_line(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
...
void fire_ball(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_CHAR_PTR);
...
int confuse_monster(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
int fear_monster(ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT ARG_COMMA ARG_INT);
...
int enchant(ARG_INT16_PTR ARG_COMMA ARG_INT16);
void elemental_brand(ARG_VOID);
