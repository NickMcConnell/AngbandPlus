...
/* INVEN_QUIVER is equal to the bottom slot in the inventory right above
   the equipment (ie. INVEN_QUIVER = INVEN_WIELD - 1).  If the user
   wears, takes off, or drops ammo in the quiver, the global variables:
   using_quiver and equip_top will be updated accordingly. - JLS */

void use_quiver(t_ptr, using)
inven_type *t_ptr;
int using; /* Boolean controlling whether quiver to be filled or emptied */
{
  if (using) /* Put ammo in quiver */
  {
    using_quiver = TRUE;
    equip_top = INVEN_QUIVER;
  }
  else if (using_quiver) /* Take ammo out of quiver */
  {
    using_quiver = FALSE;
...
/* Be sure to turn off the quiver before calling takeoff(...)
   to takeoff quiver ammo if the ammo will be kept in the pack.
   If the quiver is on, INVEN_QUIVER will always have its
   contents destroyed. -JLS */
  ...
  int item_val, posn;
{
  ...
  if (using_quiver && item_val == INVEN_QUIVER)
   {
    use_quiver(t_ptr, FALSE);
    invcopy(t_ptr, OBJ_NOTHING);
   }
  else if (item_val >= inven_ctr)
   /* If the quiver was already off, INVEN_QUIVER may be a part of the pack;
      if so, do _not_ destroy the item it holds. -JLS */
    invcopy(t_ptr, OBJ_NOTHING);
}
...
char command;
{
  ...
  do
	...
	case 't':	  /* Take off	   */
	  ...
	  else if (inven_ctr >= equip_top && !using_quiver && !doing_inven)
  ...
  /* Can always take off ammo in quiver since quiver is part of pack! -JLS */
		      ...
		      if (item >= 0)
			{
			  ...
			  if (command == 'r')
			  ...
			  else
			    {
				  ...
				  !inven_check_num(&inventory[item]))
				{
				  use_quiver(&inventory[item], FALSE);
		  /* Since inven_carry() would stack the quiver with itself
		     (because the quiver is a phantom member of the pack for
		     stacking), we will unload the quiver manually. - JLS */
				  slot = inven_ctr++;
				  i_ptr = &inventory[slot];
				  *i_ptr = inventory[INVEN_QUIVER];
				  /* takeoff will subtract weight back off */
		  ...
		  (because the quiver is a phantom member of the pack for
		  stacking), we will unload the quiver manually. -JLS */
				inventory[inven_ctr] = *i_ptr;
				/* takeoff will subtract weight */
				inven_weight += i_ptr->weight*i_ptr->number;
				tmp = inven_ctr++;
			       }
			      else
			  ...
			  /* third, wear new item */
			  *i_ptr = tmp_obj;
			  if (slot == INVEN_QUIVER) /* turn quiver on -JLS */
			    use_quiver(&inventory[INVEN_QUIVER], TRUE);
			  equip_ctr++;
			  py_bonuses(i_ptr, 1);
			  if (slot == INVEN_WIELD)
  ...
  if (scr_state != BLANK_SCR)
    restore_screen();
  if (light_chg)      /* Update screen for changed light radius. -DGK */
   {
    register int i, j;

    for (i = char_row - light_rad; i <= char_row + light_rad; i++)
     for (j = char_col - light_rad; j <= char_col + light_rad; j++)
      cave[i][j].tl = FALSE;
#ifdef TC_COLOR
    textcolor(LIGHTGRAY);
#endif
	...
	cave[i][j].tl = TRUE;
    for (i = char_row - tmp2; i <= char_row + tmp2; i++)
     for (j = char_col - tmp2; j <= char_col + tmp2; j++)
      lite_spot(i,j);
   }
  calc_bonuses();
...
int y1, y2;
{
  ...
  if (light_flag)
    {
      for (i = y1-light_rad; i <= y1+light_rad; i++) /* Turn off light */
	for (j = x1-light_rad; j <= x1+light_rad; j++)
	  cave[i][j].tl = FALSE;
  ...
  for (i = y2-light_rad; i <= y2+light_rad; i++)
    for (j = x2-light_rad; j <= x2+light_rad; j++)
     if (los(i,j,y2,x2) && distance(i,j,y2,x2)<=light_rad)
      {
	c_ptr = &cave[i][j];
	/* only light up if normal movement */
	if (light_flag)
	  c_ptr->tl = TRUE;
	if (c_ptr->fval >= MIN_CAVE_WALL || c_ptr->fval == LAVA_FLOOR)
	  c_ptr->pl = TRUE;
...
/* This fn controls the colorizing of inventory and equipment lists, to make
   it simple to change the colors.  The idea of colorizing inventory came
   from Druid Moria.  I'm not 100% sure that color inventory won't be more
   obnoxious than helpful, but there's 1 way to find out. -CFT */
