...
static errr rd_extra(void)
{
	...
	if(arg_fiddle) note("In rd_extra().");

	rd_string(op_ptr->full_name, 32);

	rd_string(p_ptr->died_from, 80);

	if(arg_fiddle) note("Read full_name and died_from.");
	...
	if(arg_fiddle) note("Read history strings.");
	...
	if(arg_fiddle) note("Read race, class, and sex.");
	...
	if (arg_fiddle) note("Read hitdie and expfact.");
	...
	if(arg_fiddle) note("Read age, height and weight.");

	/* Read the stat info */
	for (i = 0; i < 6; i++) rd_s16b(&p_ptr->stat_max[i]);
	for (i = 0; i < 6; i++) rd_s16b(&p_ptr->stat_cur[i]);

	if(arg_fiddle) note("Read stat_max and stat_cur.");
	...
	if(arg_fiddle) note("Read gold and exp stuff.");
	...
	if(arg_fiddle) note("Read level, hp/sp stuff and max_depth.");
	...
	if(arg_fiddle) note("Read player flags.");
	...
	if(arg_fiddle) note("Read more flags.");

	/* Dag - all the racial stuff. */
	/* This is so the mutants don't get messed up. */
	for (i = 0; i < 6; i++) rd_s16b(&rp_ptr->r_adj[i]);
	if(arg_fiddle) note("Read racial bonus stats ok.");

	rd_s16b(&rp_ptr->r_dis);
	rd_s16b(&rp_ptr->r_dev);
	rd_s16b(&rp_ptr->r_sav);
	rd_s16b(&rp_ptr->r_stl);
	rd_s16b(&rp_ptr->r_srh);
	rd_s16b(&rp_ptr->r_fos);
	rd_s16b(&rp_ptr->r_thn);
	rd_s16b(&rp_ptr->r_thb);

	if(arg_fiddle) note("Read racial bonus skills ok.");

	rd_byte(&rp_ptr->r_mhp);
	rd_byte(&rp_ptr->r_exp);

	if(arg_fiddle) note("Read racial bonus hp/exp ok.");

	rd_byte(&rp_ptr->b_age);
	rd_byte(&rp_ptr->m_age);

	if(arg_fiddle) note("Read racial age bytes ok.");

	rd_byte(&rp_ptr->m_b_ht);
	rd_byte(&rp_ptr->m_m_ht);
	rd_byte(&rp_ptr->m_b_wt);
	rd_byte(&rp_ptr->m_m_wt);

	rd_byte(&rp_ptr->f_b_ht);
	rd_byte(&rp_ptr->f_m_ht);
	rd_byte(&rp_ptr->f_b_wt);
	rd_byte(&rp_ptr->f_m_wt);

	if(arg_fiddle) note("Read racial height/weight bytes ok.");

	rd_byte(&rp_ptr->infra);
	rd_byte(&rp_ptr->choice);

	if(arg_fiddle) note("Read infravision and class choices ok.");
...
static errr rd_savefile_new_aux(void)
{
	...
	/* Important -- Initialize the class */
	/* Dag - Now the race info is stored in the save file */
	rp_ptr->title = race_info[p_ptr->pclass].title;
