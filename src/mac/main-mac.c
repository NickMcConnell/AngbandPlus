 ...
 *
 * #define ALLOW_BIG_SCREEN (V, Ey, O, and Z.
 *   Dr's big screen needs more work.  New S one needs some thought)
 * #define ANG281_RESET_VISUALS (Z-based variants)
 * #define USE_DOUBLE_TILES ("bigtile" patch, V)
 * #define AUTO_SAVE_ARG_REQUIRED (O and Z)
 * #define HAS_SCORE_MENU (V and some more)
...
/*
 * This is much more convenient than ancient dialogues
 * (OS 8.5 or greater, or System 7.5-- with a system extention)
 */

/* Turn off Navigation Service for backwards compatibility -- kend */
...
/*
 * MPW 68K compiler cannot process tolua generated crap...
...
static bool askfor_file(char *buf, int len)
{
	...
	/* Default file name */
	(void)strnfmt((char*)dflt + 1, 255, "%s's description", buf);
...
static SndListHandle find_sound(int num)
{
	...
	(void)strnfmt((char*)sound + 1, 255, "%.16s.wav", angband_sound_name[num]);
...
static void Term_init_mac(term *t)
{
	...
	/* Make the window */
	td->w = NewCWindow(0, &td->r, td->title, 0,
		(td == data? noGrowDocProc: documentProc), (WindowPtr)-1, 1, 0L);
...
static errr Term_xtra_mac(int n, int v)
{
	...
	switch (n)
	{
		...
		case TERM_XTRA_SOUND:
		{
			...
			/* Get the proper sound name */
			(void)strnfmt((char*)sound + 1, 255, "%.16s.wav", angband_sound_name[v]);
			sound[0] = strlen((char*)sound + 1);
 ...
 */
static errr Term_rows_mac(bool fifty_rows)
{
...
static void SetupAppDir(void)
{
	...
	if (err != noErr)
	{
		(void)strnfmt(errString, 100, "Fatal PBGetFCBInfo Error #%d.\r Exiting.", err);
	...
	if (err != noErr)
	{
		(void)strnfmt(errString, 100, "Fatal HSetVol Error #%d.\r Exiting.", err);
		mac_warning(errString);
		ExitToShell();
	}
}
 ...
 * an obsolete (and arcane!) feature -- pelpel
 */
static void SetupAppDir(void)
{
	OSErr err;
...
# ifndef USE_NAVIGATION_SERVICE
	...
	/* Standard File relies on "current working directory" */
	err = HSetVol(NULL, app_vol, app_dir);
...
static void do_menu_file_open(bool all)
{
	...
	/* Use what SetupAppDir thinks of current application's vol and dir */
	vrefnum = app_vol;
	drefnum = app_dir;
...
static void init_menubar(void)
{
	...
	for (i = 8; i <= 32; i += ((i / 16) + 1))
	{
		...
		/* Textual size */
		(void)strnfmt((char*)buf + 1, 15, "%d", i);
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		/* Describe the item */
		(void)strnfmt((char*)buf + 1, 15, "%.15s", angband_term_name[i]);
		buf[0] = strlen((char*)buf + 1);
		...
		case MENU_FILE:
		{
			switch (selection)
			{
				...
				case ITEM_SCORE:
				{
					...
					(void)path_build(buf, sizeof(buf), ANGBAND_DIR_APEX,
						"scores.raw");
...
#pragma unused(reply, handlerRefCon)
	...
	if (err)
	{
		(void)strnfmt(msg, 128, "Argh!  FSpGetFInfo failed with code %d", err);
		mac_warning(msg);
		return err;
	}
		...
		case mouseDown:
		{
			...
			switch (code)
			{
				...
				case inGrow:
				{
					...
					r.bottom = (td == data? 50: 24) * td->tile_hgt
...
static void init_stuff(void)
{
	...
	while (1)
	{
		...
		/* Build the filename */
		(void)path_build(path, 1024, ANGBAND_DIR_FILE, "news.txt");
...
# endif
	...
	/* Prompt the user */
	prt("[Choose 'New' or 'Open' from the 'File' menu]", screen_rows - 1, 15);
