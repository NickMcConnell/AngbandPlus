 ...
 * #define ANG281_RESET_VISUALS (Cth, Gum, Z, S)
 * #define AUTO_SAVE_ARG_REQUIRED (O, Z, S)
 ...
 */
int fsetfileinfo(cptr pathname, OSType fcreator, OSType ftype)
{
	OSErr err;
	FSSpec spec;
	FInfo info;
 ...
 * Please note that TR_OVER only works with 256 colour (8-bit) images.
 * I might consider resurrecting the dynamic mask creation code...
 */
#define TR_NONE	0
#define TR_OVER 1
...
static errr globe_init(void)
{
	...
	if (frameP == NULL)
	{
		/* Dispose of ususable image GWorld */
...
static void Term_init_mac(term *t)
{
	...
	/* Error */
	if (err != noErr) quit("Cannot create window!");
