...
void str_map_add_int(str_map_ptr map, const char *key, int val)
{
    str_map_add(map, key, (void *)(intptr_t)val);
}
...
int str_map_find_int(str_map_ptr map, const char *key)
{
    void *pv = str_map_find(map, key);
...
int str_map_iter_current_int(str_map_iter_ptr iter)
{
    assert(str_map_iter_is_valid(iter));
    return (int)(intptr_t)iter->node->val;
}
