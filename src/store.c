...
static bool black_market_crap(object_type *o_ptr)
{
	...
	/* Random artifacts -- not crappy */
	if (o_ptr->tval == TV_RANDART) return FALSE;
...
static void store_purchase(void)
{
	...
	{
		if (store_num == 7) mprint(MSG_TEMP, "Your home is empty.");
		else mprint(MSG_TEMP, "I am currently out of stock.");
		return;
	}
	...
	{
		mprint(MSG_TEMP, "You cannot carry that many items.");
		return;
	}
	...
	if (store_num != 7)
	{
		...
		/* Message */
		mformat(MSG_TEMP, "Buying %s (%c).", o_name, I2A(item));
		msg_print(NULL);
		...
		if (choice == 0)
		{
			...
			if (p_ptr->au >= price)
			{
				...
				if (st_ptr->stock_num == 0)
				{
					...
					{
						/* Message */
						mprint(MSG_TEMP, "The shopkeeper retires.");
					...
					{
						/* Message */
						mprint(MSG_TEMP, "The shopkeeper brings out some new stock.");
					}
			...
			else
			{
				...
				mprint(MSG_TEMP, "You do not have enough gold.");
			}
		}
	}
...
static void store_sell(void)
{
	...
	{
		/* Oops */
		mprint(MSG_STUPID, "Hmmm, it seems to be cursed.");
	...
	{
		if (store_num == 7) mprint(MSG_TEMP, "Your home is full.");
		else mprint(MSG_TEMP, "I have not the room in my store to keep it.");
		return;
	}
	...
	{
		/* Describe the transaction */
		mformat(MSG_TEMP, "Selling %s (%c).", o_name, index_to_label(item));
		msg_print(NULL);
...
static void store_process_command(void)
{
	...
	switch (p_ptr->command_cmd)
	{
		...
		default:
		{
			mprint(MSG_TEMP, "That command does not work in stores.");
			break;
		}
	}
}
...
void do_cmd_store(void)
{
	...
	while (!leave_store)
	{
		...
		if (inventory[INVEN_PACK].k_idx)
		{
			...
			{
				/* Message */
				mprint(MSG_TEMP, "Your pack is so full that you flee the store...");
			...
			{
				/* Message */
				mprint(MSG_TEMP, "Your pack is so full that you flee your home...");
