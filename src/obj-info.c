 ...
 */
static bool describe_misc_magic(const object_type *o_ptr, u32b f1, u32b f3) /* ~ f1 for ammunition */
{
	cptr good[6], bad[4];
	int gc = 0, bc = 0;
	...
	if (f3 & (TR3_REGEN))       good[gc++] = "speeds your regeneration";
        if (f1 & (TR1_AMMUNITION))  good[gc++] = "needs no extra ammunition"; /* ~ ammunition */
        if (f3 & (TR3_CONFER_FIRE)) good[gc++] = "makes your weapon burn with fire"; /* ~ confer */
        if (f3 & (TR3_CONFER_COLD)) good[gc++] = "makes your weapon very cold"; /* ~ confer */
        if (f3 & (TR3_CONFER_ELEC)) good[gc++] = "makes lightning arc from your weapon"; /* ~ confer 												*/
        if (f3 & (TR3_CONFER_ACID)) good[gc++] = "makes your weapon drip with acid"; /* ~ confer */
        if (f3 & (TR3_CONFER_POIS)) good[gc++] = "makes your weapon drip with venom"; /* ~ confer */
        if (f3 & (TR3_CONFER_EVIL)) good[gc++] = "makes your weapon smite evil"; /* ~ confer */
        if (f3 & (TR3_CONFER_DRGN)) good[gc++] =
                            "makes your weapon thirst for the black blood of dragons"; /* ~ confer */
...
bool object_info_out(const object_type *o_ptr)
{
	...
	if (describe_misc_magic(o_ptr, f1, f3)) something = TRUE;
