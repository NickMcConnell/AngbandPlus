...
/* mon-info.c: monster information dump */
...
static void describe_monster_spells(int r_idx, const monster_lore *l_ptr)
{
	...
	if (l_ptr->flags7)
	{
		...
		if (l_ptr->flags7 & (RF7_S_ENT))		vp[vn++] = "Ents";
...
static void describe_monster_movement(int r_idx, const monster_lore *l_ptr)
{
	...
	if (l_ptr->flags3 & RF3_LAWFUL) text_out_c(TERM_L_GREEN, " lawful");
	else if (l_ptr->flags3 & RF3_CHAOTIC) text_out_c(TERM_L_GREEN, " chaotic");
	...
	else if (l_ptr->flags3 & RF3_VALA) text_out_c(TERM_L_BLUE, " Vala");
...
/*
 * "Extra info" on monsters - level, exp and such.
 */
void extra_info(monster_type *m_ptr)
{
	...
	/* Extra info for pets, also in wizard mode */
	if ((m_ptr->align & (AL_PET_MASK)) || p_ptr->wizard)
	{
		if (m_ptr->align & (AL_PET_MASK)) text_out(format("%^s is friendly to you.  ", m_name));
		else text_out(format("%^s is hostile to you.  ", m_name));
		if (m_ptr->align == AL_PET || m_ptr->align == AL_HOSTILE)
			text_out(format("%^s is aligned neutrally.  ", wd_he[msex]));
		else if (m_ptr->align == AL_PET_L || m_ptr->align == AL_HOSTILE_L)
			text_out(format("%^s is aligned lawfully.  ", wd_he[msex]));
		else if (m_ptr->align == AL_PET_C || m_ptr->align == AL_HOSTILE_C)
			text_out(format("%^s is aligned chaotically.  ", wd_he[msex]));
		text_out(format("%^s is currently at experience level %d, ", wd_he[msex], m_ptr->level));
		text_out(format("and has %d experience points.  ", m_ptr->exp));
		text_out(format("%^s needs %d more points to advance to next level.  ", wd_he[msex], MONSTER_EXP(m_ptr->level+1) - m_ptr->exp));
		text_out(format("%^s has %d/%d hit points, and %d/%d mana points.  ", wd_he[msex],
			m_ptr->hp, m_ptr->maxhp, m_ptr->mana, m_ptr->maxmana));
		text_out(format("%^s spell power is %d.  ", wd_his[msex], m_ptr->spell_power));
		text_out(format("%^s armor class is %d.  ", wd_his[msex], m_ptr->ac));
		if ((m_ptr->align & (AL_PET_MASK)) && m_ptr->off)
		{
			text_out(format("%^s seems to be %s.  ", m_name, m_ptr->off < OFFENSE_WARNING ?
				"angry" : "very angry"));
		}
	}
	else
	/* Limited info */
	{
		text_out(format("%^s is hostile to you.  ", m_name));
		text_out(format("%^s seems to have experience level of %d.  ", wd_he[msex], m_ptr->level));
	}
	/* For both cases */
	if (m_ptr->mflag & (MFLAG_PERM)) text_out(format("%^s will follow you between levels.  ", wd_he[msex]));
	text_out("\n");
}
