 ...
 */
uint vstrnfmt(char *buf, uint max, char *fmt, va_list vp)
{
    char *s;
 ...
 */
static char *vformat(char *fmt, va_list vp)
{
    ...
    if (!format_buf) {
        format_len = 1024;
        format_buf = new char[format_len];
 ...
 */
uint strnfmt(char *buf, uint max, char *fmt, ...)
{
    uint len;
    va_list vp;
