...
s16b hitpoint_warn = 5;		/* Hitpoint warning (0 to 9) */

s16b delay_factor = 3;		/* Delay factor (0 to 9) */

bool autosave_l = TRUE;        /* Autosave before entering new levels */
bool autosave_t = TRUE;        /* Timed autosave */
...
/*
 * Array of grids for use by crushing ceilings (see "dungeon.c", and "spells1.c")
 */
s16b crusher_n;
s32b crusher_t[CRUSHER_MAX];
byte crusher_y[CRUSHER_MAX];
byte crusher_x[CRUSHER_MAX];


/*
 * Array[MAX_HGT][256] of cave grid info flags (padded)
 *
...
/*
 * Array[MAX_HGT][256] of cave grid feature codes
 *
 * This array is padded to a width of 256 to allow fast access to elements
 * in the array via "grid" values (see the GRID() macros).
 */
...
/*
 * Array[MAX_HGT][MAX_WID] of cave grid object indexes
 *
...
/*
 * Array[MAX_HGT][MAX_WID] of cave grid monster indexes
 *
...
/*
 * Array[MAX_HGT][MAX_WID] of cave grid flow "cost" values
 */
byte_wid *cave_cost;

/*
 * Array[MAX_HGT][MAX_WID] of cave grid flow "when" stamps
 */
byte_wid *cave_when;
...
/*
 * Quest array (Heino Vander Sanden)
 */
quest_type q_list[MAX_Q_IDX];
int level_quest[MAX_DEPTH];
int total_quests;
