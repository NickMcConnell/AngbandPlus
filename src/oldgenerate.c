...
static void build_type6(int yval, int xval)
{
	...
	for (x = xval - 9; x <= xval + 9; x++)
	{
		place_monster_aux(yval - 2, x, what[0], FALSE, FALSE, 0);
		place_monster_aux(yval + 2, x, what[0], FALSE, FALSE, 0);
	}
	...
	for (y = yval - 1; y <= yval + 1; y++)
	{
		place_monster_aux(y, xval - 9, what[0], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 9, what[0], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 8, what[1], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 8, what[1], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 7, what[1], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 7, what[1], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 6, what[2], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 6, what[2], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 5, what[2], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 5, what[2], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 4, what[3], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 4, what[3], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 3, what[3], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 3, what[3], FALSE, FALSE, 0);

		place_monster_aux(y, xval - 2, what[4], FALSE, FALSE, 0);
		place_monster_aux(y, xval + 2, what[4], FALSE, FALSE, 0);
	}
	...
	for (x = xval - 1; x <= xval + 1; x++)
	{
		place_monster_aux(yval + 1, x, what[5], FALSE, FALSE, 0);
		place_monster_aux(yval - 1, x, what[5], FALSE, FALSE, 0);
	}

	/* Next to the center monster */
	place_monster_aux(yval, xval + 1, what[6], FALSE, FALSE, 0);
	place_monster_aux(yval, xval - 1, what[6], FALSE, FALSE, 0);

	/* Center monster */
	place_monster_aux(yval, xval, what[7], FALSE, FALSE, 0);
}
 ...
 * -KMW-
 */
/* void old_wilderness_gen(int refresh)
{
	...
	cave_feat[63][196] = FEAT_DEEP_LAVA; 
...
void wilderness_gen(int refresh)
{
	...
	for (wh = 0;wh < DUNGEON_HGT;wh++) {
		for (ww = 0;ww < DUNGEON_WID;ww++) {
			...
			switch(wild_char) {
				case 'T':
					cave_feat[wh][ww] = FEAT_TREES;
					break;
				case 'W':
					cave_feat[wh][ww] = FEAT_DEEP_WATER;
					break;
				case 'w':
					cave_feat[wh][ww] = FEAT_SHAL_WATER;
					break;
				case 'L':
					cave_feat[wh][ww] = FEAT_DEEP_LAVA;
					break;
				case 'l':
					cave_feat[wh][ww] = FEAT_SHAL_LAVA;
					break;
				case ';':
					cave_feat[wh][ww] = FEAT_DIRT;
					break;
				case '#':
					cave_feat[wh][ww] = FEAT_PERM_SOLID;
					cave_info[wh][ww] |= (CAVE_GLOW | CAVE_MARK);
					break;
				case 'c':
					break;
				case '^':
					cave_feat[wh][ww] = FEAT_MOUNTAIN;
					break;
				case '.':
					cave_feat[wh][ww] = FEAT_FLOOR;
					break;
				default:
					cave_feat[wh][ww] = FEAT_PERM_SOLID;
					break;
...
build_bldg(int bldg_num, int y, int x, int bldgtype)
{
	...
	switch(bldgtype)
	{
		...
		case 7: /* druid grove corner */
			i = PANEL_HGT + y * 10 + 3;
			j = TOWN_WIDTH + x * 14 + 3;
			for (y_height = i;y_height > (i-3);y_height--)
				for (x_right = j; x_right > (j-4); x_right--)
					cave_feat[y_height][x_right] = FEAT_TREES;
			break;
	}
	/* Clear previous contents, add a building door */
	if ((bldg_num == 17) && (p_ptr->pclass == CLASS_DRUID))
		cave_feat[i][j] = FEAT_BLDG_HEAD + bldg_num;
	else if (bldg_num == 17)
		cave_feat[i][j] = FEAT_TREES;
	else
		cave_feat[i][j] = FEAT_BLDG_HEAD + bldg_num;
}
...
static void town_gen_hack(void)
{
	...
	build_bldg(17,0,0,7); /* druid grove */
