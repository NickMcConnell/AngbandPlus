...
void do_cmd_go_up(void)
{
	 ...
	 * chance of going up another level.
       idea taken from Oangband.  NPPAngband edit.	 */
...
void do_cmd_go_down(void)
{
	 ...
	 * there is a 50% chance of descending another level.
	 *  idea taken from Oangband.  NPPAngband edit.	 */
...
static void chest_death(int y, int x, s16b o_idx)
{
	...
	for (; number > 0; --number)
	{
		...
		quality = randint (object_level) + object_level / 5;
		...
		else if (chesttheme >= 2)
		{
			 ...
			 * with one in 4 guaranteed great getting 4 chances
			 * to become an applicable artifact.
...
static bool do_cmd_open_chest(int y, int x, s16b o_idx)
{
	...
	if (flag)
	{
		...
		if ((squelch_level[CHEST_INDEX]) == SQUELCH_OPENED_CHESTS)
		{
			...
			msg_print("Chest squelched upon opening.");
		}
	}
