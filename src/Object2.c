...
s16b spell_chance(int spell,int realm)
{
	...
	/* Hack -- Chaos Mage spell penalty for *any* weapons */
	if ((p_ptr->pclass == CLASS_CHAOS_MAGE) && (p_ptr->icky_wield)) chance += 25;
...
#ifdef DRS_SHOW_SPELL_INFO
{
   ...
   /* See below */
        int orb = (plev / ((p_ptr->pclass == CLASS_PRIEST || p_ptr->pclass == CLASS_HIGH_MAGE)
                     ? 2 : 4));
