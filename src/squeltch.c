...
static int automatizer_new_rule()
{
        ...
        while (1)
        {
                ...
                exec_lua(format("xml.write_out_y = 6; xml.write_out_x = 16; xml.write_out_h = %d; xml.write_out_w = %d; xml.write_y = 0; xml.write_x = 0; xml:print_xml(auto_aux.types_desc['%s'][2][1], '')", hgt - 3 - 5, wid - 1 - 15 - 1, types_list[sel]));
...
void do_cmd_automatizer()
{
        ...
        while (1)
        {
                ...
                if (max) exec_lua(format("xml.write_out_y = 1; xml.write_out_x = 16; xml.write_out_h = %d; xml.write_out_w = %d; xml.write_y = 0; xml.write_x = 0; xml:print_xml(__rules[%d].table, '')", hgt - 4 - 1, wid - 1 - 15 - 1, sel));
		...
		c = inkey();
				...
				screen_save();
                        ...
                        else if (c == 'n')
                        {
                                ...
                                {
                                        cptr styp = "nothing";
                                        typ = msg_box("[D]estroy, [P]ickup, [N]othing rule?", hgt / 2, wid / 2);
