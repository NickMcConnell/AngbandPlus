...
#define FSCALE (1<<8)
#if defined(Pyramid) && !defined(MSDOS)
#include <sys/time.h>
#else
#include <time.h>
#endif
...
int slp;
{
  ...
  if (cave[y][x].fval==LAVA_FLOOR && !(c_list[z].cdefense & IM_FIRE) &&
      !(c_list[z].cmove & FLIES))
    return FALSE; /* instant death on lava BAD! -DGK */
  ...
  int r, c, l;
{
  ...
  case 6:
    ...
    g->cmove|=(FLIES|MV_INVIS|MV_ATT_NORM|CARRY_OBJ|
  ...
  case 10:
    sprintf(g->name, "%s%s spirit", name, (name[strlen(name)-1]=='s')?
	    "'":"'s");
    g->cmove |= (FLIES|MV_INVIS|THRO_WALL|MV_ATT_NORM|CARRY_OBJ|HAS_1D2|GOOD);
    g->spells |= (NONE8);
  ...
  case 13:
    sprintf(g->name, "%s%s Wraith", name, (name[strlen(name)-1]=='s')?
	    "'":"'s");
    g->cmove |= (FLIES|THRO_DR|MV_ATT_NORM|CARRY_OBJ|HAS_4D2|HAS_2D2|GOOD);
  ...
  case 15:
     sprintf(g->name, "%s%s ghost", name, (name[strlen(name)-1]=='s')?
	    "'":"'s");
    g->cmove |= (FLIES|MV_INVIS|THRO_WALL|MV_ATT_NORM|CARRY_OBJ|
		 HAS_2D2|SPECIAL);
  ...
  if (!stricmp("& Longsword", name)) {
    ...
    case 1:
      ...
      t_ptr->flags = (TR_SEE_INVIS|TR_SLAY_UNDEAD|TR_SLAY_EVIL|TR_REGEN|
		      TR_RES_COLD|TR_FROST_BRAND|TR_FREE_ACT|
		      TR_SLOW_DIGEST);
      t_ptr->flags2|= (TR_SLAY_DEMON|TR_SLAY_TROLL|TR_LIGHT|TR_ACTIVATE
		      |TR_ARTIFACT|TR_RES_LT|TR_BLESS_BLADE|TR_ATTACK_SPD);
      t_ptr->p1    = 3;
      t_ptr->cost  = 300000L;
      RINGIL = 1;
      return 1;
    ...
    default:
      ...
      t_ptr->flags2|= (TR_SLAY_TROLL|TR_SLAY_ORC|TR_ARTIFACT|TR_ATTACK_SPD);
      t_ptr->p1    = 2;
      t_ptr->cost  = 30000L;
      ELVAGIL = 1;
      return 1;
  ...
  switch (t_ptr->tval)
      ...
      if (magik(chance)||good)
	{
	  ...
	  if (magik(3*special/2)||good==666) { /* was 2 */
		...
		t_ptr->todam += 5;
	      ...
	      switch(randint(30)) /* was 16 */   /* Was 27 -DGK */
		{
		case 1:	/* Holy Avenger	 */
	...
	case TV_BOW:
	  ...
	  if (magik(chance)||good) {
		...
		(t_ptr->subval != 12 || randint(5) == 1))
		    /* Triple crossbows less likely to be {excellent} -DGK */
	    switch (randint(15)) {
	    case 1:
	      if (((randint(3)==1)||(good==666)) && !not_unique &&
	...
	case TV_WAND:
	  if (t_ptr->subval <= 28)
	   {
	    t_ptr->p1 = randint(t_ptr->p1)+(t_ptr->toac);
	    t_ptr->toac = 0;
	   }
      break;
	  ...
	  if (magik(chance)||good)
	    {
	      ...
	      if (magik(5*special/2)||(good==666))
	       if (!not_unique &&
		   !stricmp(object_list[t_ptr->index].name, "& Arrow~") &&
		   randint(10)==1)
		 {
		  if (BLACKARR) break;
		  if (wizard || peek) msg_print("The Black Arrow");
		  else good_item_flag=TRUE;
		  t_ptr->name2=SN_BLACKARR;
		  t_ptr->tohit=15;
		  t_ptr->todam=30;
		  t_ptr->damage[0]=4;
		  t_ptr->damage[1]=4;
...
void set_options()
{
  ...
  for (max = 0; options[max].o_prompt != 0; max++)
    {
      if (options[max].o_var != &delay_spd)
