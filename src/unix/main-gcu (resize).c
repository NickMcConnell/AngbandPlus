...
static errr Term_xtra_gcu_react(void)
{
	...
	/* Return if not safe to resize */
	if (!resize_safe) return (0);
	...
	/*
	 * Hack -- Resize the term if grown only  XXX XXX
	 *
	 * If we are in split_window mode, we avoid resizing on shrink
	 * to save effort.
	 *
	 * No need to resize main term, unless split_window is FALSE.
	 *
	 * FIXME: Add sub-windows if resized enough.
	 */
	...
	if (x > x_max)
	{
		/* We use multi-windows, if possible */
		if (split_window)
		{
			/* Upper right */
			if (data[TERM_SUBWINDOW + 1].active)
			{
				cols = x - (term_size_min[WINDOW_DISPLAY][0] + 1);
				lines = data[TERM_SUBWINDOW + 1].t.rows;

				wresize(data[TERM_SUBWINDOW + 1].win, lines, cols);

				(void)Term_activate(&data[TERM_SUBWINDOW + 1].t);

				(void)Term_resize(cols, lines);

				(void)Term_redraw();
			}

			/* Lower right */
			if (data[TERM_SUBWINDOW + 2].active)
			{
				cols = x - (term_size_min[WINDOW_DISPLAY][0] + 1);
				lines = data[TERM_SUBWINDOW + 2].t.rows;

				wresize(data[TERM_SUBWINDOW + 2].win, lines, cols);

				(void)Term_activate(&data[TERM_SUBWINDOW + 2].t);
		...
		{
			wresize(data[TERM_MAIN].win, y, x);

			(void)Term_activate(&data[TERM_MAIN].t);

			(void)Term_resize(x, y);
	...
	if (y > y_max)
	{
		...
		{
			/* Lower left */
			if (data[TERM_SUBWINDOW + 0].active)
			{
				cols = data[TERM_SUBWINDOW + 0].t.cols;
				lines = y - (term_size_min[WINDOW_DISPLAY][1] + 1);

				wresize(data[TERM_SUBWINDOW + 0].win, lines, cols);

				(void)Term_activate(&data[TERM_SUBWINDOW + 0].t);
			...
			if (data[TERM_SUBWINDOW + 2].active)
			{
				cols = data[TERM_SUBWINDOW + 2].t.cols;
				lines = y - (term_size_min[WINDOW_DISPLAY][1] + 1);
 ...
 */
static errr switch_display_gcu(bool display)
{
        /* Assume success */
        return (0);
}
