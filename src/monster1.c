...
static void describe_monster_movement(int r_idx, const monster_lore *l_ptr)
{
	...
	else if (l_ptr->tkills)
	{
		...
		if (depth_in_feet)
		{
                        text_out_c(TERM_SLATE, format("at depths of "));
			text_out_c(((r_ptr->level > p_ptr->max_depth) ?
                                                    TERM_L_RED : TERM_UMBER),
                                                    format("%d ",
			                            r_ptr->level * 50));
                        text_out_c(TERM_SLATE, format("feet"));
		}
		else
		{
			text_out_c(TERM_SLATE, format("on dungeon level "));
                        text_out_c(((r_ptr->level > p_ptr->max_depth) ?
                                                    TERM_L_RED : TERM_UMBER),
