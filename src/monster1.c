...
static void roff_aux(int r_idx)
{
	...
	if (r_ptr->r_tkills)
	{
		...
		if (r_ptr->flags3 & (RF3_DOOM)) flags3 |= (RF3_DOOM);
	...
	if (r_ptr->r_tkills)
	{
		...
		/* Group some variables */
		{
			...
			if (r_ptr->mexp)
			{
				...
				i = ((long)r_ptr->mexp * r_ptr->level / p_ptr->max_level) * r_ptr->mexp / div;
	...
	if (flags4 & (RF4_BO_SHARD))		vp[vn++] = "fire shard bolts";
	if (flags4 & (RF4_BL_SHARD))		vp[vn++] = "fire shard blasts";
	if (flags5 & (RF5_CH_SHARD))		vp[vn++] = "fire chainshards";
	...
	if (flags6 & (RF6_BR_SKULL))		vp[vn++] = "spawn skulls";
	...
	if (flags6 & (RF6_S_DOOM))		vp[vn++] = "summon minions of Id";
	if (flags6 & (RF6_S_HI_DOOM))		vp[vn++] = "summon powerful minions of Id";
	...
	/* Collect thriving */
	vn = 0;
	if (flags2 & (RF2_HEAL_ACID)) vp[vn++] = "acid";
	if (flags2 & (RF2_HEAL_ELEC)) vp[vn++] = "electricity";
	if (flags2 & (RF2_HEAL_FIRE)) vp[vn++] = "fire";
	if (flags2 & (RF2_HEAL_COLD)) vp[vn++] = "cold";
	if (flags2 & (RF2_HEAL_POIS)) vp[vn++] = "poison";
	...
	if (flags3 & (RF3_RES_SOUN)) vp[vn++] = "sound";
