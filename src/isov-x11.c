...
{       
    ...
    if(Metadpy->depth < 15) {
	printf("BIG PROBLEM: no TrueColor/HiColor visual!\n");
	printf("can't initialize graphics, aborting program!\n");
	exit(11);
    }
	...
	// RGB 555
	if(redMask == 31 && greenMask == 31) { 
	    R = (i & 0x7C00) >> 10;
	    G = (i & 0x03E0) >> 5;
	    B = (i & 0x001F) >> 0;
	} else if(redMask == 31 && greenMask == 63) { 
	    R = (i & 0x7C00) >> 10;
	    G = (i & 0x03E0) >> 4;
	    B = (i & 0x001F) >> 0;
	} else if(redMask == 255) {
	    ...
	    printf("BIG PROBLEM: unknown visual!\n");
	    printf("can't initialize graphics, aborting program!\n");
	    exit(11);
	}
...
void dr_textur(int xp, int yp, int w, int h)
{
    ...
    if(Metadpy->depth <= 16) {
	register const unsigned short * src = data8;
	register unsigned long * dest = (unsigned long *)texturimg->data;
...
/* generic transormatio code (slow!)   
