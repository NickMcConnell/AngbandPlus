...
static void load_prefs_aux(term_data *td, cptr sec_name)
{
	...
	g_g.m_bMacroBarShown = GetPrivateProfileInt_Reg(sec_name, "MacroBar", 0, ini_file);
	...
	case MACRO_BUTTON + 18:
		{
			...
			else Term_keypress(cecommandnum[offset-1]);
...
#ifdef _WIN32_WCE
		{
			...
			else
			{
				...
				ofn.lStructSize = sizeof(ofn);
				ofn.hwndOwner = data[0].w;
				ofn.lpstrFilter = _T("Save Files (*.faa)\0*.faa\0");
				ofn.nFilterIndex = 1;
...
#endif /* __MWERKS__ */
{
	...
	/* Acquire proper "term_data" info */
//#ifdef _WIN32_WCE /* this did not work in AngbandCE according to FWG */
//#else
	td = (term_data *)GetWindowLong(hWnd, 0);
//#endif
	...
	/* Hack out for now		if ((term_initialised) && (td->tile_wid) && (td->tile_hgt))  */
			if (0)
			{
				/* Get the text grid */
				xPos = GET_X_LPARAM(lParam);
...
BOOL InitInstance(HINSTANCE hInst, int nCmdShow)
{
	...
	wsprintf(szWindowClass, L"%s", L"FAANGBAND");
	wsprintf(szTitle, L"%s", L"faangband");
...

	{
		...
		wc.hInstance	= hInstance;
		wc.hIcon         = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_FAANGBAND));
		wc.hCursor       = 0;
...
#endif
{
	...
	create one through the zip files. */
	{
		...
		_tcscpy(saveDir, _T("\\My Documents\\FA"));
	...
	if (strcmp(lpCmdLine, ""))
	{
		...
		strcpy(lpCmdLine, "\\My Documents\\FA\\");
