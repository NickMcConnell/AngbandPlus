...
void do_cmd_walk_jump(int pickup)
{
	...
	if (get_rep_dir(&dir))
	{
		...
		move_player(dir, pickup);
                ...
                if (p_ptr->pclass == CLASS_DARK_LORD || ability(57)) dark_lord_aura(rand_int(5) * p_ptr->lev * 2, p_ptr->lev /8 + 3);
                /* Valkyries generate Thunder Aura(but the truth is that is use GF_LITE, not GF_ELEC!) */
                /* which can hurt monsters! */
                if (p_ptr->pclass == CLASS_VALKYRIE || ability(77)) valkyrie_aura(rand_int(5) * p_ptr->lev, 2);
        ...
        switch(p_ptr->prace)
	{
		...
		case RACE_DARK_ELF:
                        ...
                        if (amber_power == 2)
			{
                        ...
                        if (amber_power == 3)
			{
                                if (special_flag || p_ptr->pclass == CLASS_DARK_LORD)
                                {
		...
		case RACE_SPECTRE:
			...
			while (TRUE)
			{
                                if (!get_com("Use which power? [F]ireBall, [D]arkness Ball ", &ch))
				{
					amber_power = 0;
					break;
				}
                        ...
                        if (amber_power == 2)
			{
                                ...
                                {
                                        if (!get_aim_dir(&dir)) break;
                                        msg_print("You cast a Darkness Ball.");
					if (get_aim_dir(&dir))
                                                fire_ball(GF_DARK, dir, p_ptr->lev * 3, 1 + (p_ptr->lev/18));
                                        energy_use = 100;
                                }
                                else
                                {
...
void do_cmd_racial_power(void)
{
	...
	switch (p_ptr->prace)
	{
		...
		case RACE_VAMPIRE:
			...
			has_racial = TRUE;
                ...
                case RACE_SKATT:
                        racial_power = "High jump (Jump where you want in a radius of 10. Space must be seen.)";
			has_racial = TRUE;
                        break;
	...
	if (has_racial)
	{
        ...
        if (p_ptr->pclass == CLASS_NECRO || p_ptr->pclass == CLASS_LICH)
	{
                powers[num] = -5;
	...
	{
                powers[num] = -6;
                strcpy(power_desc[num], "Bless Item (Item become magic (and make weapons indestructible).)");
		num++;
	}

        if (p_ptr->pclass == CLASS_LEADER || p_ptr->pclass == CLASS_COMMANDER || ability(62))
	{
                powers[num] = -7;
                strcpy(power_desc[num], "Recruit Monster");
		num++;
	}

        if (p_ptr->pclass == CLASS_JUSTICE_WARRIOR || ability(60))
	{
                powers[num] = -8;
                strcpy(power_desc[num], "Divine Bolt (cost: 20 mana  dam 15 * level * 3)");
		num++;
	}
	...
	{
                powers[num] = -9;
                strcpy(power_desc[num], "Sword Of Devastation");
		num++;
	}

        if ((p_ptr->pclass == CLASS_HELLQUEEN && p_ptr->lev >= 10) || ability(12))
	{
                powers[num] = -10;
                strcpy(power_desc[num], "Blink Away(radius 30)");
		num++;
	}

        if (p_ptr->pclass == CLASS_HELLQUEEN && p_ptr->lev >= 25)
	{
                powers[num] = -11;
                strcpy(power_desc[num], "Accurate Teleport");
		num++;
	}

        if (p_ptr->pclass == CLASS_HELLQUEEN && p_ptr->lev >= 18)
	{
                powers[num] = -12;
                strcpy(power_desc[num], "Instant Recall");
		num++;
	}
        if (p_ptr->pclass == CLASS_ZELAR || ability(65))
	{
                powers[num] = -13;
                strcpy(power_desc[num], "Wave kick (10 times normal damages, hp reduced by 50%, ranged attack)");
		num++;
	}
        if (p_ptr->pclass == CLASS_ZELAR || ability(66))
	{
                powers[num] = -14;
                strcpy(power_desc[num], "Energy Spin (Fixed radius 3, half of normal damages)");
		num++;
	}
        if (p_ptr->pclass == CLASS_SKATTER || ability(71))
	{
                powers[num] = -15;
                strcpy(power_desc[num], "Enchant Bow (Enchant a bow to become magic)");
		num++;
	}
        if (p_ptr->pclass == CLASS_SKATTER || ability(69))
	{
                powers[num] = -16;
                strcpy(power_desc[num], "Create Arrows (Create Skatter Arrows. Damages increase with level.)");
		num++;
	}
        if (p_ptr->pclass == CLASS_FIRE_LORD || ability(73))
	{
                powers[num] = -17;
                strcpy(power_desc[num], "Fire Ball (Fixed radius 3, dam = current hp, hp - 33%)");
		num++;
	}
        if ((p_ptr->pclass == CLASS_FIRE_LORD && p_ptr->lev >= 12) || ability(74))
	{
                powers[num] = -18;
                strcpy(power_desc[num], "Fire Circle (Fixed radius 3, dam = curent hp * 2, hp - 33%, reduce constitution)");
		num++;
	}
        if ((p_ptr->pclass == CLASS_FIRE_LORD && p_ptr->lev >= 24) || ability(75))
	{
                powers[num] = -19;
                strcpy(power_desc[num], "Plasma Storm (Radius = lev/10 + 3, dam = current hp * 3, hp - 33%)");
		num++;
	}
        if (p_ptr->pclass == CLASS_BERSERKER && p_ptr->lev >= 10)
	{
                powers[num] = -20;
                strcpy(power_desc[num], "Identify Item");
		num++;
	}
        if ((p_ptr->pclass == CLASS_BERSERKER && p_ptr->lev >= 20) || (p_ptr->pclass == CLASS_DARK_LORD))
	{
                powers[num] = -21;
                strcpy(power_desc[num], "Full Identify");
		num++;
	}
        if ((p_ptr->pclass == CLASS_SKATTER && p_ptr->lev >= 10) || ability(70))
	{
                powers[num] = -22;
                strcpy(power_desc[num], "Exploding arrow (Fixed radius 2, dam = mana * lev / 5)");
		num++;
	}
        if ((p_ptr->pclass == CLASS_ZELAR && p_ptr->lev >= 20) || ability(67))
	{
                powers[num] = -23;
                strcpy(power_desc[num], "Circle Of Force (Fixed radius 3, 5 times normal damages, hp - 50%)");
		num++;
	}
        if (p_ptr->pclass == CLASS_LEADER || p_ptr->pclass == CLASS_COMMANDER || ability(63))
	{
                powers[num] = -24;
                strcpy(power_desc[num], "Put pet into a crystal (hypnotise ANY pet)");
		num++;
	}
        if (p_ptr->pclass == CLASS_HELLQUEEN)
	{
                powers[num] = -25;
                strcpy(power_desc[num], "Mana Blast (Beam, dam = used mana)");
		num++;
	}
        if (p_ptr->pclass == CLASS_VALKYRIE || ability(76))
        {
                powers[num] = -26;
                strcpy(power_desc[num], "Create Valkyrie Spear");
		num++;
        }
        if ((p_ptr->pclass == CLASS_DARK_LORD && p_ptr->lev >= 45) || ability(58))
        {
                powers[num] = -27;
                strcpy(power_desc[num], "Devastation Beam (cost 180 mana, damage = 10000)");
		num++;
        }
        if (ability(22))
        {
                powers[num] = -28;
                strcpy(power_desc[num], "Ball Of Light (Damages = level * 5)");
                num++;
        }
        if (ability(29))
        {
                powers[num] = -29;
                strcpy(power_desc[num], "Invoke Spirit (cost 25 mana)");
                num++;
        }
        if (ability(32))
        {
                powers[num] = -30;
                strcpy(power_desc[num], "Create Mage Staff");
                num++;
        }
        if (ability(47))
        {
                powers[num] = -31;
                strcpy(power_desc[num], "Illusion");
                num++;
        }
        if (p_ptr->pclass == CLASS_JUSTICE_WARRIOR || ability(61))
        {
                powers[num] = -32;
                strcpy(power_desc[num], "Invoke Angel (Cost 40 mana)");
                num++;
        }
        if (p_ptr->pclass == CLASS_VALKYRIE)
        {
                powers[num] = -33;
                strcpy(power_desc[num], "Invoke Valhala Maiden (Cost 5 mana)");
		num++;
        }
        if (p_ptr->pclass == CLASS_VALKYRIE || ability(81))
        {
                powers[num] = -34;
                strcpy(power_desc[num], "Flash (Cost X mana, dam = X * 10)");
		num++;
        }
        if (ability(150))
        {
                powers[num] = -35;
                strcpy(power_desc[num], "Summon Woman");
		num++;
        }
        if (p_ptr->pclass == CLASS_DARK_LORD)
        {
                powers[num] = -36;
                strcpy(power_desc[num], "Invoke Darkness (Need 50 mana(but consume ALL mana), summon darkness creatures)");
		num++;
        }
        if (p_ptr->pclass == CLASS_DARK_LORD)
        {
                powers[num] = -37;
                strcpy(power_desc[num], "Dark Flash (Cost X mana, dam = X * 10)");
		num++;
        }
        if (p_ptr->pclass == CLASS_BERSERKER)
        {
                powers[num] = -38;
                strcpy(power_desc[num], "Recall to dungeon (Return to the dungeon)");
		num++;
        }       
	...
	if (powers[i]<0)
	{
		if (powers[i] == -1) cmd_racial_power_aux();
                ...
                else if (powers[i] == -4)
                {
                        ...
                        s = "You have no monster to awaken. Make sure the monster is on the floor!";
                ...
                {
                        make_sword_devastation();
                }

                else if (powers[i] == -10)
                {
                        msg_print("You blink far away...");
                        teleport_player(30);
                        energy_use = 100;
                }
                ...
                else if (powers[i] == -11)
                {
                        ...
                        newx = get_quantity("Enter the X coordinate :", MAX_WID);
                        newy = get_quantity("Enter the Y coordinate :", MAX_HGT);
                        teleport_player_to(newx, newy);
                        energy_use = 100;
                }
                else if (powers[i] == -12)
                {
                        msg_print("You instantly go back to the town...");
                        p_ptr->energy -= 100;
                        p_ptr->word_recall=1;
                }
                else if (powers[i] == -13)
                ...
                {
                        if (!get_aim_dir(&dir)) return;
                        fire_ball(GF_LITE, dir, p_ptr->lev * 5, 3);
                        energy_use = 100;
                }
                else if (powers[i] == -29)
                {
                        if (p_ptr->csp >= 25)
                        {
                                msg_print("You invoke a spirit...");
                                summon_specific_friendly(py, px, p_ptr->lev, SUMMON_MIND_SPIRIT, FALSE);
                                p_ptr->csp -= 25;
                                energy_use = 100;
                        }
                ...
                {
                        msg_print("You create the illusion of a monster!");
                ...
                else if (powers[i] == -32)
                {
                        if (p_ptr->csp >= 40)
                        {
                                msg_print("You invoke an Angel...");
                                summon_specific_friendly(py, px, 80, SUMMON_ANGEL, FALSE);
                                p_ptr->csp -= 40;
                                energy_use = 100;
                        }
                        else msg_print("You don't have enough mana!");
                ...
                else if (powers[i] == -33)
                {
                        if (p_ptr->csp >= 5)
                        {
                                msg_print("You invoke a Valhala Maiden...");
                                summon_specific_friendly(py, px, 80, SUMMON_VALHALA_MAIDEN, FALSE);
                                p_ptr->csp -= 5;
                                update_and_handle();
                                energy_use = 100;
                        }
                ...
                else if (powers[i] == -34)
                {
                        int manaquantity;
                        char tmpstring[20];
                        sprintf(tmpstring, "How much mana? Max: %d", p_ptr->csp);
                        manaquantity = get_quantity(tmpstring, p_ptr->csp);
                        valkyrie_aura(manaquantity * 10, 3);
                        p_ptr->csp -= manaquantity;
                        update_and_handle();
                        energy_use = 100;
                }
                else if (powers[i] == -35)
                {
                        msg_print("You invoke a female warrior!");
                        summon_specific_friendly(py, px, 40 + p_ptr->lev, SUMMON_WOMAN, FALSE);
                        energy_use = 100;
                }
                else if (powers[i] == -36)
                {
                        if (p_ptr->csp >= 50)
                        {
                                msg_print("You invoke the Darkness!");
                                summon_specific_friendly(py, px, dun_level + randint(p_ptr->lev), SUMMON_DARKNESS, TRUE);
                                p_ptr->csp = 0;
                                update_and_handle();
                                energy_use = 100;
                        }
                        else msg_print("Not enough mana!");
                }
                else if (powers[i] == -37)
                {
                        int manaquantity;
                        ...
                        sprintf(tmpstring, "How much mana? Max: %d", p_ptr->csp);
                        manaquantity = get_quantity(tmpstring, p_ptr->csp);
                        dark_lord_aura(manaquantity * 10, 3);
                        p_ptr->csp -= manaquantity;
                        update_and_handle();
                        energy_use = 100;
                }
                ...
                {
                        if (dun_level == 0) p_ptr->word_recall = 1;
                        energy_use = 100;
                }
...
void do_cmd_skatt_jump(void)
{
          int     ii = 0, ij = 0;
          msg_print("You prepare to jump very high!");
          if (!tgt_pt(&ii,&ij)) return;
          if (!cave_empty_bold(ij,ii) || (cave[ij][ii].info & CAVE_ICKY) || !(cave[ij][ii].info & CAVE_MARK) || (distance(ij,ii,py,px) > 10))
          {
                msg_print("You can't jump here. Something is in the way...");
          }
          else teleport_player_to(ij,ii);
          energy_use = 100;
}
...
void change_age(void)
{
        ...
        if (ch == 'Y' || ch == 'y')
        {
                ...
                /* Let's be somewhat realistic(10 is young but...) */
                if (p_ptr->age < 10) p_ptr->age = 10;
        }
}
