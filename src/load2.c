...
static errr rd_item(object_type *o_ptr)
{
	...
	/* Hack - some special conversions */
	if (older_than(1, 3, 0))
	{
		switch(o_ptr->k_idx)
		{
			case 425: o_ptr->k_idx = lookup_kind(TV_AMULET, SV_AMULET_SENSING); break;
			case 513: o_ptr->k_idx = lookup_kind(TV_POTION, SV_POTION_ADRENALINE); break;
			case 221: o_ptr->k_idx = lookup_kind(TV_SCROLL, SV_SCROLL_STAR_DESTRUCTION); break;
		}
	}
...
static errr rd_extra(void)
{
	...
	rd_s16b(&p_ptr->awareness);
	rd_s16b(&p_ptr->adrenaline);
	rd_s16b(&p_ptr->biofeedback);
	rd_s16b(&p_ptr->shadow_form);
	rd_s16b(&p_ptr->inertial_barrier);
	rd_s16b(&p_ptr->prob_travel);
	rd_s16b(&p_ptr->precognition);
	rd_s16b(&p_ptr->mental_barrier);
	rd_s16b(&p_ptr->ts_anchor);
	rd_s16b(&p_ptr->intensify);
	rd_s16b(&no_fw);

	if ((no_fw < 0) || (no_fw > MAX_FORCE_WALLS))
	{
		note("Bad number of force walls");
		return(1);
	}
...
static errr rd_savefile_new_aux(void)
{
	...
	/* Hack -- Fill newly added stores */
	if (tmp16u < MAX_STORES)
	{
		for ( ; tmp16u < MAX_STORES ; tmp16u++)
		for (i = 1; i < 10; i++)
		{
			store_maint(tmp16u);	
		}
	...
	if (!p_ptr->is_dead)
	{
        ...
        /* Give a message about town change */
        if (older_than(1,2,1))
        {
        	if (p_ptr->depth)
        	{
        		note("The town has changed since your last visit!");
        	}
        	else
        	{
        		note("The town changes!");
        		generate_cave();
        	}
        }
