...
static errr rd_savefile_new_aux(void)
{
	...
	/* Attempt to read saved Python information */
	if (perform_event(EVENT_LOAD, Py_BuildValue("(O)",
			PyFile_FromFile(fff, "savefile", "r", NULL))))
	{
		/* Error in reading */
		note("Error reading script data");
		return (35);
	}
