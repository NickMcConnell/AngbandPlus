...
static errr rd_extra(void)
{
	...
	if (!older_than(2, 9, 1))
	{
		...
		rd_s16b(&p_ptr->temp_invis);
		rd_byte(&p_ptr->nasty_pois);

		for (i=0; i<34; i++) rd_byte(&p_ptr->drang_reserved[i]);
	}
    ...
    if (older_than(2,9,7))
    {
        rd_byte(&tmp8u);  /* previous Ethereal lock was a byte */
    }
    else
    {
        rd_s16b(&p_ptr->ethereal_lock);
    }
    rd_byte(&tmp8u);    /* oops */
	rd_byte(&tmp8u);	/* oops */
	rd_byte(&p_ptr->searching);
	rd_byte(&p_ptr->maximise);
