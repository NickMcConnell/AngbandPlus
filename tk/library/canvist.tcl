...
proc NSCanvist::Insert {oop index args} {
	...
	if {$index eq "end"} {set index $count}
...
proc NSCanvist::Delete {oop index1 index2} {
	...
	if {$index2 eq "end"} {set index2 $count}
...
proc NSCanvist::UpdateSelection {oop selected deselected} {
	...
	if {([llength $selected] == 1) && ($selected eq "all")} {
		set selected {}
...
proc NSCanvist::AutoScan {oop} {
	...
	if {[winfo containing $pointerx $pointery] eq $canvas} return
...
proc NSCanvist::itemSelect {oop} {
	...
	foreach index $list {
		...
		if {[lsearch -integer $doneRows $row] != -1} continue
