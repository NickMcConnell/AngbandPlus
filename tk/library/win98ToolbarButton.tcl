...
if 0 {
	...
	bind NSWin98ToolbarButtonBindTag <Enter> \
		"NSWin98ToolbarButton::ButtonEnter %W %x %y"
...
if 1 {
	...
	# The label
	if {1 || [Info $oop font] eq ""} {
		tk::button $path.b
		Info $oop font [$path.b cget -font]
		destroy $path.b
	}
	$canvasCmd create text 0 0 -text "" -font [Info $oop font] -tags labelbg -anchor s
	$canvasCmd create text 0 0 -text "" -font [Info $oop font]  -tags label -anchor s
...
proc NSWin98ToolbarButton::~NSWin98ToolbarButton {oop} {
	...
	variable Priv
		...
		if {[string length $labelVar]} {
			trace remove variable $labelVar write \
...
#		if {([Info $oop image] eq "") && ([Info $oop label] eq "")} \{
...
#		\}
		...
		set labelVar [Info $oop labelVar]
		if {$oldLabelVar ne $labelVar} {
			if {[string length $oldLabelVar]} {
				trace remove variable $oldLabelVar write \
					"NSWin98ToolbarButton::LabelVarChanged $oop"
			}
#			global $labelVar
			trace add variable $labelVar write \
				"NSWin98ToolbarButton::LabelVarChanged $oop"
...
if {$height < $minHeight} {
	...
	if {$hasMenu && !$onlyMenu} {
		...
		if {[Info $oop reliefMenu] != [Info $oop relief]} {
			if {[Info $oop reliefMenu] eq "raised"} {
				set fillTopLeft $::SystemButtonHighlight
				set fillBottomRight $::SystemButtonShadow
			} elseif {[Info $oop reliefMenu] eq "sunken"} {
				set fillTopLeft $::SystemButtonShadow
				set fillBottomRight $::SystemButtonHighlight
				set bump 1
			} else {
...
#	focus $canvas
	set menu [uplevel #0 $command [Info $oop command]]

	# This is non-empty for -hovermenu
	Info $oop postedMenu $menu
...
proc NSWin98ToolbarButton::HideMenu {oop} {
	...
	if {[Info $oop menuHideCmd] ne ""} {
		uplevel #0 [Info $oop menuHideCmd]
	}
...
#	catch {focus $Priv(focus)}
	...
	if {[winfo containing $pointerx $pointery] ne $canvas} {
		ButtonLeave $canvas
	} else {
...
proc NSWin98ToolbarButton::ButtonEnter {w x y} {

	variable Priv

	set oop $Priv(oop,$w)
	if {[Info $oop state] ne "disabled"} {
		if {![Info $oop menuVisible]} {
			if {[Info $oop hoverMenu]} {
				ButtonPress $w $x $y
			} else {
				Configure $oop -state active -relief raised -reliefmenu raised
			}
...
proc NSWin98ToolbarButton::ButtonLeave {w} {
	...
	if {[Info $oop state] ne "disabled"} {
		if {[Info $oop menuVisible]} {
			...
			# If the mouse doesn't enter the posted menu when -hovermenu=yes
			# then hide the menu.
			set postedMenu [Info $oop postedMenu]
			if {[Info $oop hoverMenu] && ($postedMenu ne "")} {
				set pointerxy [winfo pointerxy $w]
				set win [winfo containing {*}$pointerxy]
				if {$win eq "" ||
					[winfo toplevel $win] ne [winfo toplevel $postedMenu]} {
					HideMenu $oop
				}
			}
		} else {
...
proc NSWin98ToolbarButton::ButtonUp {w x y} {
	...
	set oop $Priv(oop,$w)
	if {([Info $oop state] ne "disabled") && ![Info $oop menuVisible]} {
		Configure $oop -state active -relief raised -reliefmenu raised
		set canvas [Info $oop canvas]
		set pointerx [winfo pointerx $canvas]
		set pointery [winfo pointery $canvas]
