...
if (py.misc.prace == 8)
  p_ptr->regenerate = TRUE; /*Those Amberites heal fast...*/
else
...
char command;
{
  ...
  int tmp, tmp2, selecting, from, to, xploss, equip_flag;
  ...
  do
      ...
      while (selecting && free_turn_flag)
	{
	  ...
	  if (from > to)
	      ...
	      if (!get_com(prt1, &which))
		{
		 ...
		 /* Code based upon Aaron Mandelbaum's suggestion to allow
		    players to use digits to to refer to a correspondingly
		    inscribed item.
		 */
		 if (which >= '0' && which <= '9') {
		     if (equip_flag) {
			 /* This is annoying because gaps in the equipment slots
			    need to be handled.
			 */
			 inven_type *iptr = &inventory[INVEN_WIELD];
			 int equip_ct = 0;

			 for (; iptr <= &inventory[INVEN_AUX]; ++iptr)
			     if (iptr->tval != TV_NOTHING) {
				 if (iptr->inscrip[0] == which &&
				     iptr->inscrip[1] == '\0') {
					 which = equip_ct + 'a';
					 break;
				 }
			 ...
			 for(item = from; item <= to; item++) {
			     if (inventory[item].inscrip[0] == which &&
				 inventory[item].inscrip[1] == '\0') {
				     which = item + 'a';
				     break;
			     }
			 }
       ...
       if (which == ' ' || which == '*')
	{
	 ...
	 if (py.misc.pclass != 0 && inventory[item].name2 == SN_GORLIM)
	    {   msg_print ("The black mask drains your life force away!");
	       xploss = randint(py.misc.max_exp/2);
	       lose_exp(xploss);
		py.misc.max_exp -= xploss;
	       while (randint(2)==1) {
	       xploss = randint(py.misc.max_exp/2);
	       lose_exp(xploss);
		py.misc.max_exp -= xploss;
		  ...
		  prt_experience();
		  prt_level();
		inventory[item].flags |= TR_CURSED; /* Just in case it has
						       been uncursed... TY */
	    }
  ...
  if (scr_state != BLANK_SCR)
    restore_screen();
  calc_bonuses();
  if (!in_store_flag)    /* If we ain't in a store, do the equippy chars -DGK*/
   prt_equippy_chars();
}


/* The CWS version for id'ing things... */
    ...
    *com_val = 0;
		 ...
		 */
		 if (which >= '0' && which <= '9') {
			 ...
			 for(k = 0; k < INVEN_WIELD; ++k)
			     if (inventory[k].inscrip[0] == which &&
				     inventory[k].inscrip[1] == '\0') {
				 which = k + 'a';
				 break;
			     }
		     }
	...
	switch (which) {
		  ...
		  case '*':
		    ...
		    break;
	case '-': case '>': case'.': /* A few more keys for people like
	    myself who think Nethack... TY */
	    ...
	    if (
		((inventory[(int) (which - 'a')].inscrip[0] == 'X' ||
		 inventory[(int) (which - 'a')].inscrip[0] == 'x')
		 && (inventory[(int) (which - 'a')].inscrip[1] == '\0'))
		 && !verify("Really use", *com_val)) {
			    test_flag = TRUE;
			    free_turn_flag = TRUE;
			    i_scr = (-1);
			    break;
...
#endif
  if (!find_flag || find_prself)
    print((no_graphics_flag?'@':'à'), y2, x2);
}
...
char *hit_from;
{
  ...
  if (py.misc.chp < 0) {
	char destination[80];
	if(getLine("death.txt",0,destination))
...
register int y, x;
{
  ...
  if (t_ptr->tval == TV_INVIS_TRAP)
  ...
  else if (t_ptr->tval == TV_SECRET_DOOR)
    {
      ...
      t_ptr->gchar = object_list[OBJ_CLOSED_DOOR].gchar;
...
int32u typ_dam;
{
  ...
  if (i > 0)
	...
	default: /* unknown damage type... */
      ...
      if ((do_damage == FALSE) ||
	(
	((py.flags.resist_acid) && (py.flags.acid_im || py.flags.acid_resist))
	&& (randint(6)>2)))
	{
...
char *kb_str;
{
  ...
  if (!(py.flags.resist_acid) || !(py.flags.acid_resist || py.flags.acid_im))
	inven_damage(set_corrodes, 5);

  /* acid attacks may now reduce your charisma -CWS */
        if (!(py.flags.acid_resist || py.flags.resist_acid || py.flags.acid_im)
             && randint(14) == 1) {
         if(!py.flags.sustain_chr)
           { msg_print("Your features are twisted!");
            dec_stat(A_CHR);}
         else msg_print("You keep your good looks!");
        }
}
...
char *kb_str;
{
  ...
  if (!(py.flags.poison_resist || py.flags.resist_poison
    ...
    py.flags.poisoned += 12 + randint(dam);
         if (!(py.flags.poison_resist || py.flags.resist_poison || py.flags.poison_im)
             && randint(14) == 1)  {
         if(!py.flags.sustain_con)
           { msg_print("Your health is damaged!");
            dec_stat(A_CON);}
         else msg_print("You keep fit!");
        }
}
...
char *kb_str;
{
  ...
  if (py.flags.fire_im)
    dam = 1;
  take_hit(dam, kb_str);
  if (!(py.flags.resist_heat) || !(py.flags.fire_resist || py.flags.fire_im))
	inven_damage(set_flammable, 3);
  if (!(py.flags.fire_resist || py.flags.resist_heat || py.flags.fire_im)
             && randint(14) == 1)  {
         if(!py.flags.sustain_str)
           { msg_print("You feel weaker!");
            dec_stat(A_STR);}
         else msg_print("You keep your strength.");
        }
}
...
char *kb_str;
{
  ...
  if (py.flags.cold_im)
    dam = 1;
  take_hit(dam, kb_str);
  if (!(py.flags.resist_cold) || !(py.flags.cold_resist || py.flags.cold_im))
	    inven_damage(set_frost_destroy, 5);
  if (!(py.flags.cold_resist || py.flags.resist_cold || py.flags.cold_im)
             && randint(14) == 1)  {
         if(!py.flags.sustain_str)
           { msg_print("You feel weaker!");
            dec_stat(A_STR);}
...
char *kb_str;
{
  ...
  if (py.flags.light_im)
    dam = 1;
  take_hit(dam, kb_str);
  if (!(py.flags.resist_light) || !(py.flags.lght_resist || py.flags.light_im))
	inven_damage(set_lightning_destroy, 3);
  if (!(py.flags.lght_resist || py.flags.resist_light || py.flags.light_im)
             && randint(14) == 1)  {
         if(!py.flags.sustain_dex)
           { msg_print("Your nervous system is damaged!");
            dec_stat(A_DEX);}
...
int getLine(char *file, int line, char *string)
    { FILE *input;
	    char dummy[81];
	    int current_line;
	    int max_lines=0;
	...
	if (!(input=fopen(file,"rt"))) {
			msg_print("ERROR - Can't open file!");
			return 0;
		    };
	fscanf(input,"%d",&max_lines);
	line=randint(max_lines)+1;
	while (!feof(input)) {
	...
	if (line>current_line) {
		fclose(input);
		msg_print("ERROR - Line does not exist!");
		return 0;
	    };
	dummy[80] = '\0';
	strcpy(string,dummy);
