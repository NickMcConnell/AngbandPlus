...
●自動拾い
  ...
  まず、UNIXなら~/.angband/Hengband/、それ以外のOSなら変愚蛮怒の中の lib/user/
  のディレクトリに picktype.prf 又は picktype-(名前).prf というファイルを
  作って下さい。picktype.prf はどんな名前のキャラでプレイするときでも読まれ、
  ...
  1行に1つのアイテムを書きます。
  (picktype.prf, picktype-(名前).prf 合わせて)最大512個まで登録できます。
  頭が#の行や空行は無視されます。
...
---------
  ...
  例外として、特定のキーワードで始まるものは特別扱いになります。
  代表的なキーワードは以下のようなものがあります。
    ・すべての〜 : すべてのアイテムが対象になります。
    ・未鑑定の〜 : 鑑定されていないアイテムが対象になります。
    ・未判明の〜 : 効果の分からないアイテムが対象になります。
    ・無価値の〜 : 価値のない(売却できない)アイテムが対象になります。
    〜の部分では以下のような文字列が特別扱いになります
    (ただし「賞金首の〜」では特別扱いになりません)。
    ・武器 : 武器が対象になります。
    ・防具 : 防具が対象になります。
    ・矢 : 矢、クロスボウの矢、石、弾が対象になります。
    (この3つは「未判明の〜」では特別扱いになりません)
    ・魔法アイテム : 巻物・杖・魔法棒・ロッドが対象になります。
    (これは「未鑑定の〜」では特別扱いになりません)
    ・がらくた : 折れた棒や像等の役に立たないものが対象になります。
    ・アイテム : すべてのアイテムが対象になります。
    〜の部分がこれ以外の文字列になっている場合は、その文字列が
    名前の一部に含まれているアイテムが対象になります。

    さらに、死体と骨に関しては次の2つが用意されています。
    ・賞金首の〜 : 賞金のかかっているモンスターの死体または骨が対象になります。
    ・ユニーク・モンスターの〜 : ユニーク・モンスターの死体または骨が対象になります。
    〜の部分には「死体」または「骨」が入ります。

   例)
# ショート・テレポートの巻物、テレポートの巻物、テレポートの杖など、
#「テレポート」と名のつくものを自動的に拾う
# ただし「テレポートの指輪{呪われている}」は自動的に破壊する
!テレポートの指輪{呪われている}
テレポート
...
すべてのアイテム
  ...
  例のように、先に書かれた規則のほうが優先されるので、一般的なルールを
  後のほうに、例外的な規則を先のほうに書くと便利です。

  現在の階のマップを確認するコマンド'M'を押すと、分かったいる範囲内のアイテムで
  自動拾いにヒットするものだけを表示します。

  また、銘のところでも書きましたが、銘に'=g'と書かれたアイテムは
  自動的に拾います。
  撃った矢を自動的に拾いたい場合はこちらが便利です。

  種族や職業、レベルによって別々に自動拾いの指定をすることもできます。
