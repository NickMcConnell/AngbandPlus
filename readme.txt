...
●Windows
  ...
  後は展開したフォルダに移動し、questbandをクリックすれば遊べます。
...
●UNIX
  ...
  ソースをダウンロードします。そして、
  tar -jxvf questband-x.x.x.tar.bz2
  cd questband-x.x.x
  ./configure
  make install
  ...
  ./questband -- -n＜ウィンドウの数＞
  タイルグラフィックス画面なら、
  ./questband -g -- -n＜ウィンドウの数＞      ## 通常の8x8タイル
  ./questband -g -- -b -n＜ウィンドウの数＞   ## 2倍幅サイズのタイル
  ./questband -g -- -a -n＜ウィンドウの数＞   ## 16x16タイル(別配布のタイルが必要)
  等で遊べます。
...
●プレイ記録
  ...
  UNIXでは ~/.angband/questband/ 、それ以外のOSでは変愚蛮怒の中の lib/user/ の
