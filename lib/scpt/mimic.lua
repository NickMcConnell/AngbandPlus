...
add_mimic_shape
{
	...
	["desc"] = 	"Eagles are master of the air, good hunters.",
	["realm"] =     "nature",
	["level"] =     10,
	["rarity"] =    30,
	["duration"] =  {10, 50},
...
add_mimic_shape
{
	...
	["calc"] =      function ()
			...
			if player.lev >= 10 then player.free_act = TRUE end
			if player.lev >= 20 then player.regenerate = TRUE end
			if player.lev >= 30 then player.resist_conf = TRUE end
			if player.lev >= 35 then player.resist_nexus = TRUE end
