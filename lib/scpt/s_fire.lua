...
GLOBELIGHT = add_spell
{
	["name"] = 	"Globe of Light",
		...
		local ret, dir
                ...
                if get_level(GLOBELIGHT, 50) > 3 then lite_area(10, 4)
                else lite_room(py, px) end
                if get_level(GLOBELIGHT, 50) > 15 then
		        fire_ball(GF_LITE, 0, 10 + get_level(GLOBELIGHT, 100), 5 + get_level(GLOBELIGHT, 6))
		end
	end,
	["info"] = 	function()
        	if get_level(GLOBELIGHT, 50) > 15 then
...
FIREFLASH = add_spell
{
	["name"] = 	"Fireflash",
		...
		local ret, dir, type
	        ...
	        fire_ball(type, dir, 20 + get_level(FIREFLASH, 500), 2 + get_level(FIREFLASH, 5))
	end,
	["info"] = 	function()
...
FIREWALL = add_spell
{
	["name"] = 	"Firewall",
        ...
        ["spell"] = 	function()
		local ret, dir, type
        	if (get_level(FIREWALL, 50) > 5) then
	        	type = GF_HOLY_FIRE
        	else
        		type = GF_FIRE
	        end
        	ret, dir = get_aim_dir()
                if ret == FALSE then return FALSE end
	        fire_wall(type, dir, 20 + get_level(FIREWALL, 150), 6 + get_level(FIREWALL, 14))
	end,
	["info"] = 	function()
		return "dam "..(20 + get_level(FIREWALL, 150)).." dur "..(6 + get_level(FIREWALL, 14))
	end,
        ["desc"] =	{
