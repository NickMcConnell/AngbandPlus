...
static bool borg_think_home_buy_aux(void)
{
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        if (slot != -1 && auto_items[slot].cursed) continue;
...
#define TNB_STORE_PROMPTS
#ifdef TNB_STORE_PROMPTS
        if (item->iqty > 1) borg_keypress('\n'); /* enter quantity */
        /* With auto_haggle the game asks 'Sell X for Y gold? [y/n]' */
        borg_keypress('y');
...
#ifdef TNB_STORE_PROMPTS
        if (item->iqty > 1) borg_keypress('\n'); /* enter quantity */
        /* With auto_haggle the game asks 'Buy X for Y gold? [y/n]' */
        borg_keypress('y');
#else /* TNB_STORE_PROMPTS */
        borg_keypress('\n');
        borg_keypress('\n');
...
bool borg_think_store(void)
{
    ...
    if (auto_items[INVEN_WIELD].tval == TV_DIGGING && borg_wear_stuff()) return (TRUE);
