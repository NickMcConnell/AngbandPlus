...
static bool borg_think(void)
{
    ...
    if (borg_save && borg_save_game())
    {
        ...
        /* Create a scum file */
        if (borg_skill[BI_CLEVEL] >= auto_dump_level ||
    ...
    if (storedata.shopping)
    {
        ...
        if (auto_do_browse_more != auto_shops[shop_num].more)
        {
            ...
            /* Save the new one */
            auto_do_browse = auto_shops[shop_num].more;
        }
#endif
        ...
        for (i = 0; i < 24; i++)
        {
            ...
            if (n < storedata.st_ptr->stock_num)
            {
                ...
                object_desc_store(desc, o_ptr, TRUE, 3);
    ...
    /* Track best level */
    if (borg_skill[BI_CLEVEL] > auto_max_level) auto_max_level = borg_skill[BI_CLEVEL];
...
static void get_stats_borg(void)
{
    ...
    borg_note(format("# Minimal stats rolled in %d turns.",auto_round));
...
#if 1 /* TNB */
		/* If the game exits, we should close this file cleanly */
		if (auto_fff) fflush(auto_fff);
#endif

#ifdef BORG_TK
        angtk_eval("angband_borg", "deactivate", NULL);

        /* Redraw, in case the interface was detached */
        do_cmd_redraw();
#endif /* BORG_TK */
...
/* Try to catch "Illegal object/spell choice" during resurrection */
if (borg_prompt &&
	(inkey_flags == INKEY_ITEM || inkey_flags == INKEY_SPELL) &&
	!borg_inkey(FALSE))
{
	borg_note("# Breaking out of illegal choice");
	return ESCAPE;
}
...
static char borg_inkey_hack(int flush_first)
{
	char ch = borg_inkey_hack_XXX(flush_first);
	borg_note(format("& Key <%c>\n", ch));
	return ch;
}
#endif
...
/* AJG needed to make this wider so I could read long formulas */
    while (0 == my_fgets(fp, buf, sizeof(buf)-1))
    {
...
#ifndef BORG_TK /* Because 'setting' file saves them */
        ...
        if (prefix(buf, "auto_stop_king ="))
        {
            if (buf[strlen("auto_stop_king =")+1] == 'T' ||
                buf[strlen("auto_stop_king =")+1] == '1' ||
                buf[strlen("auto_stop_king =")+1] == 't') auto_stop_king=TRUE;
            else auto_stop_king = FALSE;
            continue;
        }
        ...
        if (prefix(buf, "auto_respawn_winners ="))
        {
            if (buf[strlen("auto_respawn_winners =")+1] == 'T' ||
                buf[strlen("auto_respawn_winners =")+1] == '1' ||
                buf[strlen("auto_respawn_winners =")+1] == 't') auto_respawn_winners = TRUE;
            else auto_respawn_winners = FALSE;
            continue;
        }
        ...
        if (prefix(buf, "auto_respawn_race ="))
        {
            sscanf(buf+strlen("auto_respawn_race =")+1, "%d", &auto_respawn_race);
            continue;
        }
        if (prefix(buf, "auto_respawn_class ="))
        {
            sscanf(buf+strlen("auto_respawn_class =")+1, "%d", &auto_respawn_class);
            continue;
        }
        if (prefix(buf, "auto_dump_level ="))
        {
            sscanf(buf+strlen("auto_dump_level =")+1, "%d", &auto_dump_level);
            continue;
        }

        if (prefix(buf, "auto_stop_clevel ="))
        {
            sscanf(buf+strlen("auto_stop_clevel =")+1, "%d", &auto_stop_clevel);
            continue;
        }
        if (prefix(buf, "auto_stop_dlevel ="))
        {
            sscanf(buf+strlen("auto_stop_dlevel =")+1, "%d", &auto_stop_dlevel);
            continue;
        }
    ...
    if (auto_respawn_winners) auto_stop_king = FALSE;
...
void borg_status(void)
{
 ...
 for (j = 0; j < 8; j++)
 {
  ...
  if (op_ptr->window_flag[j] & (PW_BORG_2))
  {
   ...
   if (do_weak) attr = TERM_BLUE;
   else attr = TERM_SLATE;
   Term_putstr(36, 11, -1, attr, "Weak");

   if (do_poisoned) attr = TERM_BLUE;
   else attr = TERM_SLATE;
   Term_putstr(43, 11, -1, attr, "Poison");

   if (do_cut) attr = TERM_BLUE;
   else attr = TERM_SLATE;
   Term_putstr(29, 12, -1, attr, "Cut");

   if (do_stun) attr = TERM_BLUE;
   else attr = TERM_SLATE;
   Term_putstr(36, 12, -1, attr, "Stun");

   if (do_confused) attr = TERM_BLUE;
   else attr = TERM_SLATE;
   Term_putstr(43, 12, -1, attr, "Confused");

   if (c_t >= 12000) attr = TERM_BLUE;
   else attr = TERM_SLATE;
   ...
   Term_putstr(65, 11, -1, TERM_WHITE, format("%d",c_t - auto_began));

   Term_putstr(54, 12, -1, TERM_SLATE, "Since Town         ");
   Term_putstr(65, 12, -1, TERM_WHITE, format("%d",auto_time_town + (c_t - auto_began)));
   ...
   if (auto_kills_summoner != -1) attr = TERM_WHITE;
   else attr = TERM_SLATE;
   ...
   attr = TERM_WHITE;
   Term_putstr(64, 6, -1, attr,format("%s                              ", borg_prepared(auto_max_depth+1)));
   ...
   attr = TERM_WHITE;
   Term_putstr(56, 8, -1, attr,format("%d    ", auto_max_depth));
...
void do_cmd_borg(void)
{
    ...
    /* Command: check previous steps */
        ...
        case '_':
        {
            msg_format("This Function is no longer valid");
            break;
        }
       ...
       case 'q':
        ...
        case '2':
        {
          int i=0;
            ...
            msg_format("Max Level: %d  Prep'd For: %d  Reason: %s", auto_max_depth, i-1, borg_prepared(i) );
        ...
        case 'H':
        {
            ...
            for (;item < to; item++)
            {
                ...
                switch (cmd)
                {
                    case 'i':
                    case 'I':
                        msg_format("Item%03d value= %d", item, borg_has[item]);
                        break;
                    case 'w':
                    case 'W':
                        msg_format("WItem%03d value= %d", item-z_info->k_max, borg_has[item]);
                        break;
                    case 'a':
                    case 'A':
                        msg_format("Artifact%03d value= %d", item-z_info->k_max-z_info->k_max, borg_has[item]);
                        break;
                    default:
                        msg_format("skill (%s) value= %d",
                            prefix_pref[item -
                                        z_info->k_max -
                                        z_info->k_max -
...
struct
{
    ...
    {"auto_borg_message",
        "Memorize and log Borg messages",
        NULL, (void *) &auto_borg_message},
    {"auto_flag_save",
        "Save the game when the dungeon level changes",
        NULL, (void *) &auto_flag_save},
    {"auto_respawn_winners",
        "Respawn when the borg wins",
        NULL, (void *) &auto_respawn_winners},
    {"auto_stop_king",
        "Stop the borg on win",
        NULL, (void *) &auto_stop_king},
    {"borg_worships_damage",
        "Borg worships damage",
        NULL, (void *) &borg_worships_damage},
    {"borg_worships_speed",
        "Borg worships speed",
        NULL, (void *) &borg_worships_speed},
    {"borg_worships_hp",
        "Borg worshops hitpoints",
        NULL, (void *) &borg_worships_hp},
    {"borg_worships_mana",
        "Borg worships mana",
        NULL, (void *) &borg_worships_mana},
    {"borg_worships_ac",
        "Borg worships armor class",
        NULL, (void *) &borg_worships_ac},
    {"borg_worships_gold",
        "Borg worships gold",
        NULL, (void *) &borg_worships_gold},
    {"borg_plays_risky",
        "Borg plays risky",
        NULL, (void *) &borg_plays_risky},
    {"borg_scums_uniques",
        "Borg scums uniques",
        NULL, (void *) &borg_scums_uniques},
    {"borg_slow_optimizehome",
        "Slow home optimize",
        NULL, (void *) &borg_slow_optimizehome},
    {"borg_uses_calcs",
        "Borg uses dynamic calculations (slow)",
        NULL, (void *) &borg_uses_calcs},
    {"borg_uses_swaps",
        "Borg uses Swap Items",
        NULL, (void *) &borg_uses_swaps},
    {"delay_factor",
        "xyz",
        NULL, (void *) &borg_delay_factor},
    {NULL, NULL, NULL, 0L},
};

DLLEXPORT void *Borg_GetSettings(void);
...
DLLEXPORT int Borg_Init(Tcl_Interp *interp, char *path);
int Borg_Init(Tcl_Interp *interp, char *path)
{
    ...
    /*
     * A note about settings: We save some settings in borg-specific
     * files called 'setting'. APW Borg saves these in borg.prf as
     * well, but we ignore them there.
     */
    borg_worships_damage = FALSE;
    borg_worships_speed = FALSE;
    borg_worships_hp = FALSE;
