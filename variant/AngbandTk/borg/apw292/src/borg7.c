...
bool borg_check_lite(void)
{
    ...
    if (!when_detect_evil || (c_t - when_detect_evil  >= 183 - (20 - auto_max_level))) do_evil = TRUE;
...
/*    if (auto_items[INVEN_LEFT].iqty) return (FALSE);
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        /* apw skip artifact rings not star id'd  */
...
#if 0
            /* dump list and power...  for debugging */
            borg_note(format("Trying Item  (best power %ld)", b_p));
            borg_note(format("With Item    (borg_power %ld)", p));
            borg_note(format("Item inv %d %s.",  i, auto_items[i].desc));
#endif
