...
static void display_player_xtra_info(void)
{
	...
	Term_putstr(col + 9, 3, -1, TERM_L_BLUE, format("%4d",
			(int) p_ptr->age));

	/* Height */
	Term_putstr(col, 4, -1, TERM_WHITE, "Height");
	Term_putstr(col + 9, 4, -1, TERM_L_BLUE, format("%4d",
			(int) p_ptr->ht));

	/* Weight */
	Term_putstr(col, 5, -1, TERM_WHITE, "Weight");
	Term_putstr(col + 9, 5, -1, TERM_L_BLUE, format("%4d",
			(int) p_ptr->wt));

	/* Status */
	Term_putstr(col, 6, -1, TERM_WHITE, "Status");
	Term_putstr(col + 9, 6, -1, TERM_L_BLUE, format("%4d",
			(int) p_ptr->sc));
	...
	{
		Term_putstr(col + 8, 10, -1, TERM_L_GREEN, format("%10d",
				p_ptr->lev));
	}
	else
	{
		Term_putstr(col + 8, 10, -1, TERM_YELLOW, format("%10d",
				p_ptr->lev));
	}
	...
	{
		Term_putstr(col + 8, 11, -1, TERM_L_GREEN, format("%10ld",
				p_ptr->exp));
	}
	else
	{
		Term_putstr(col + 8, 11, -1, TERM_YELLOW, format("%10ld",
				p_ptr->exp));
	}
	...
	Term_putstr(col + 8, 12, -1, TERM_L_GREEN, format("%10ld",
			p_ptr->max_exp));
	...
	if (p_ptr->lev < PY_MAX_LEVEL)
	{
		...
		Term_putstr(col + 8, 13, -1, TERM_L_GREEN, format("%10ld",
				advance));
	}
	else
	{
		Term_putstr(col + 8, 13, -1, TERM_L_GREEN, format("%10s",
				"********"));
	}
	...
	/* Burden */
	sprintf(buf, "%ld.%ld lbs", p_ptr->total_weight / 10,
...
errr file_character(cptr name, bool full)
{
	...
	if (st_ptr->stock_num)
	{
		...
		for (i = 0; i < st_ptr->stock_num; i++)
		{
			...
			/* Describe random object attributes */
			info_length =
				identify_random_gen(&st_ptr->stock[i], info, 128);
...
bool show_file(cptr name, cptr what, int line, int mode)
{
	...
	while (TRUE)
	{
		...
		prt(format("[%s %s, %s, Line %d/%d]", VERSION_NAME, VERSION_STRING,
		...
		if (menu && isdigit(ch) && hook[D2I(ch)][0])
		{
			...
			if (!show_file(hook[D2I(ch)], NULL, 0, mode))
				ch = ESCAPE;
		}
...
void do_cmd_suicide(void)
{
	...
	else
	{
		...
		prompt_print("Please verify QUITTING by typing the '@' sign: "); /* TNB */
		flush();
		ch = inkey();
		prt("", 0, 0);
		if (ch != '@')
 ...
 */
/* static TNB */ int highscore_write(const high_score * score)
{
 ...
 */
/* static TNB */ int highscore_where(const high_score * score)
{
	int i;
 ...
 * Return the location (0 is best) or -1 on "failure"
 */
/* static TNB */ int highscore_add(const high_score * score)
{
	int i, slot;
	bool done = FALSE;
...
static void close_game_aux(void)
{
	...
	while (1)
	{
		...
		else if (ch == 'f')
		{
			...
			if (get_string("File name: ", ftmp, 79))
			{
				if (ftmp[0] && (ftmp[0] != ' '))
				{
					errr err;
