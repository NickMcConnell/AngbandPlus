...
void do_cmd_inven(void)
{
	...
	prompt_print("(Inventory) Command:"); /* TNB */
	...
	/* Load screen */
	screen_load();
	angtk_eval("angband_display", "inven", "hide", NULL); /* TNB */
	prompt_erase();	/* TNB */
...
void do_cmd_equip(void)
{
	...
	/* Prompt for a command */
	prompt_print("(Equipment) Command:");
	...
	/* Load screen */
	screen_load();
	angtk_eval("angband_display", "equip", "hide", NULL);
	prompt_erase();
...
#ifdef ALLOW_EASY_RING /* TNB */

	if (easy_ring && (o_ptr->tval == TV_RING) &&
...
void do_cmd_inscribe(void)
{
	...
	if (get_string("Inscription: ", tmp, 79))
	{
	...
	while (1)
	{
		...
		if (center_player)
		{
			sprintf(out_val,
				...
				(y2 / PANEL_HGT), (y2 % PANEL_HGT), (x2 / PANEL_WID),
				(x2 % PANEL_WID), tmp_val);
		}
