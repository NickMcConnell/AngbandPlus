...
static void convert_options(u32b flag[8], u32b mask[8])
{
	...
	if (older_than(2, 9, 0))
	{
		...
		opt_map[54] = OPT_easy_floor;
		opt_map[57] = OPT_easy_sense;
		opt_map[70] = OPT_easy_ring;

		/* Was "auto_destroy", inverse logic */
		opt_map[71] = OPT_verify_destroy_junk;
		opt_tog[71] = TRUE;

		/* Set new options to their defaults */
		op_ptr->opt[OPT_auto_more] = option_norm[OPT_auto_more];
		op_ptr->opt[OPT_smart_monsters] = option_norm[OPT_smart_monsters];
		op_ptr->opt[OPT_smart_packs] = option_norm[OPT_smart_packs];
	}
	...
	for (i = 0; i < OPT_MAX; i++)
	{
		...
		if (flag[old_os] & (1L << old_ob) && !opt_tog[i])
...
static errr rd_savefile_new_aux(void)
{
	...
	if (older_than(VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH)) /* TNB */
	...
	if (!g_grid_xtra_init)
	{
		op_ptr->opt[OPT_easy_alter] = option_norm[OPT_easy_alter];
		op_ptr->opt[OPT_easy_floor] = option_norm[OPT_easy_floor];
		op_ptr->opt[OPT_easy_open] = option_norm[OPT_easy_open];
...
static errr rd_squelch(void)
{
	...
	for (i = 0; i < tmp16u; i++)
	{
		...
		/* Paranoia */
		if (tmp8u > SQUELCH_ALL) tmp8u = SQUELCH_NONE;
...
static errr rd_tnb(void)
{
	...
	/* First check that the file version is 2.8.3 or greater. */
	if (older_than(2, 8, 3))
		return (0);
