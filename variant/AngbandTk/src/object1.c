...
bool get_item(int *cp, cptr pmt, cptr str, int mode)
{
	...
	while (!done)
	{
		...
		switch (which)
		{
			...
			case ' ':
			{
				...
				if (p_ptr->command_see)
				{
					...
					angtk_eval("angband_display",
						(p_ptr->command_wrk == (USE_EQUIP)) ?
						"equip" : (p_ptr->command_wrk == (USE_INVEN)) ?
						"inven" : "floor", "hide", NULL); /* TNB */
				}
			...
			case '@':
			{
				...
				/* Get item number */
				buf[0] = '\0';
				if (!get_string("item: ", buf, 2))
					break;
				k = atoi(buf);

				/* Sanity */
				if ((k < 0) /* || (k == INVEN_PACK)  */ || (k >= INVEN_TOTAL))
				{
					bell("Illegal object choice (at-sign)!");
					break;
				}
	...
	if (p_ptr->command_see)
	{
		...
		angtk_eval("angband_display",
			(p_ptr->command_wrk ==
(USE_EQUIP)) ? "equip" : (p_ptr->command_wrk == (USE_INVEN)) ? "inven" : "floor",
			"hide", NULL); /* TNB */
