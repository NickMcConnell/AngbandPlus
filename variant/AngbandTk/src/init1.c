	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'X')
		{
			...
			if (4 != sscanf(buf + 2, "%d:%d:%d:%d", &typ, &rat, &hgt,
					&wid)) return (PARSE_ERROR_GENERIC);
...
errr init_e_info_txt(FILE * fp, char *buf, header * head)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'X')
		{
			...
			/* Scan for the values */
			if (3 != sscanf(buf + 2, "%d:%d:%d", &slot, &rating, &xtra))
				return (PARSE_ERROR_GENERIC);
...
static errr grab_one_spell_flag(monster_race * r_ptr, cptr what)
{
	...
	plog_fmt("Unknown spell flag '%s'.", what);
	/* spell, not monster -- BUG */
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'W')
		{
			...
			if (4 != sscanf(buf + 2, "%d:%d:%d:%ld", &lev, &rar, &pad,
					&exp)) return (PARSE_ERROR_GENERIC);
...
static errr grab_one_racial_flag(player_race * pr_ptr, cptr what)
{
	...
	plog_fmt("Unknown player flag '%s'.", what);
		...
		if (buf[0] == 'N')
		{
			...
			*t++ = '\0';
			...
			if (!ot_ptr->owner_name)
				ot_ptr->owner_name = ++head->name_size;
