...
bool get_item(int *cp, cptr pmt, cptr str, int mode)
{
	...
	int choose_what = -1;
	int allow = 0;
	int allow_equip = FALSE;
	int allow_inven = FALSE;
	...
	if (allow_inven) allow |= USE_INVEN;
	if (allow_equip) allow |= USE_EQUIP;
	...
	while (!done)
	{
		...
		if (choose_what != command_wrk)
		{
			...
			Bind_Choose(KEYWORD_CHOOSE_ITEM + 1,
				(command_wrk ? 1 : 0) | (allow << 8), TRUE);
