/* CVS: Last edit by $Author: sfuerst $ on $Date: 2000/07/19 13:50:54 $ */
/* File: spells1.c */
...
static bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	else if (do_poly && (randint(90) > r_ptr->level))
	{
		if (polymorph_monster(y, x))
		{
			...
			note_sound.group = SNDGRP_EVENT;
			note_sound.index = SND_MUTATE;
			note_sound.mode = 0;
#endif /* TNB */
	...
	if (who)
	{
		...
		else
		{
			...
			{
#ifdef ALLOW_SOUND /* TNB */
				if (note_sound.group != -1)
					sound(note_sound.group, note_sound.index, note_sound.mode);
#endif /* TNB */
