...
bool sensed_p(object_type * o_ptr)
{
	...
	if (o_ptr->feeling == FEEL_NONE)
		return TRUE;

	/* Unknown feeling, curse is unknown */
	if (o_ptr->feeling == FEEL_UNKNOWN)
		return FALSE;
 ...
 * - Ed Cogburn
 *
 *  The threshold levels (in sense_chance) are:
 *     high mage = 48
 *          mage = 46
 *        priest = 43
 *   mindcrafter = 41
...
static bool sense_mixed(int threshold_level)
{
	...
	if (p_ptr->lev <= (threshold_level - 10)) return FALSE;

	/* Above mixed sensing levels? */
	if (p_ptr->lev >= threshold_level) return TRUE;
...
static void recharged_notice(object_type * o_ptr)
{
	...
	while (s)
	{
		...
		if (s[1] == '!')
		{
			...
			/* Notify the player */
			if ((temp == 0) && (o_ptr->number > 1))
				msg_format("Your %s are recharged.", o_name);
			else if (temp == 0)
				msg_format("Your %s is recharged.", o_name);
			else
				msg_format("%d of your %s are recharged.",
					o_ptr->number - temp, o_name);
#else /* TNB */
			/* Describe (briefly) */
			object_desc(o_name, o_ptr, FALSE, 0);
...
static void process_world(void)
{
	...
	if (p_ptr->word_recall)
	{
		...
		if (!p_ptr->word_recall)
		{
			...
			/* Sound */
/*			sound(SOUND_TPLEVEL); ALLOW_SOUND */
		}
	}
}
...
static void dungeon(void)
{
	...
	g_daytime = !dun_level && !p_ptr->inside_quest &&
