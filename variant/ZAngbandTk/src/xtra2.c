...
/*		sound(SOUND_KILL); ALLOW_SOUND */
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				note = " explodes!";
				sound(SNDGRP_MONSTER_ATTACK, RBM_EXPLODE, 0); /* ALLOW_SOUND */
				break;
			}
		}
#endif /* TNB */
				...
				while (1)
				{
					...
					else
					{
						...
						sprintf(out_val, "%s%s%s%s (%s) [r,%s]", s1, s2,
							s3, m_name, look_mon_desc(desc, c_ptr->m_idx),
							info);
#else /* TNB */
						cptr attitude;
...
bool get_rep_dir(int *dp)
{
	...
	while (!dir)
	{
		...
		/* Get a command (or Cancel) */
		okay = get_com("Direction (Escape to cancel)? ", &ch);
