...
void update_mon(int m_idx, bool full)
{
...
#if 1 /* TNB */
	/* Got 2 invisible Gnome Mages in a vault. */
	if (cave[fy][fx].m_idx == 0)
	{
		msg_format("INVISIBLE MONSTER BUG: m_idx=%d cave[%d][%d].m_idx == 0!",
			m_idx, fy, fx);
		msg_print(NULL);
	...
	if (easy)
	{
		...
		if (!(m_ptr->mflag & (MFLAG_VIEW)))
		{
			...
			if (disturb_near && (command_rep || running || resting))
			{
