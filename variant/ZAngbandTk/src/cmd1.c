...
/*			sound(SOUND_SELL); ALLOW_SOUND */
		...
		else
		{
			...
			else
			{
				...
				{
					char out_val[160];
					inkey_options = "carry_query_flag"; /* TNB */
	...
	bool * fear, char *m_name)
{
	...
	{
#if 1 /* TNB */
		sound(SNDGRP_EVENT, SND_BASH, 0); /* ALLOW_SOUND */
		angtk_invert_spot(m_ptr->fy, m_ptr->fx);
#endif /* TNB */
...
static void monk_attack(monster_type * m_ptr, long *k, char *m_name)
{
	...
	*k = damroll(ma_ptr->dd, ma_ptr->ds);
	...
	sound(SNDGRP_MARTIAL_ART, ma_idx, 0); /* ALLOW_SOUND */
...
/*			sound(SOUND_HIT); ALLOW_SOUND */
				...
				(randint(90) > r_ptr->level))
			{
					...
					!(r_ptr->flags1 & RF1_QUESTOR))
				{
					...
					{
						sound(SNDGRP_EVENT, SND_MUTATE, 0);	/* ALLOW_SOUND */
...
#endif /* ALLOW_EASY_ALTER -- TNB */
{
	...
	else if (!cave_floor_bold(y, x) && !p_can_pass_walls)
	{
		...
		/* Sound */
/*		sound(SOUND_HITWALL); ALLOW_SOUND */
	}
...
#ifdef USE_SCRIPT
		if (player_enter_grid_callback(y, x))
			return;

		/* Player movement callback */
		if (player_move_callback(y, x))
			return;
#endif /* USE_SCRIPT */
