...
static void warrior_probe_desc(void)
{
	...
	cptr s =
		"Warriors learn to probe monsters at level 35.  "
		"This costs nothing except a full turn.  "
		"When you reach this level, type 'm', and "
		"target the monster you would like to learn more about.  "
		"This reveals the monster's race, approximate HPs, and basic resistances.  "
		"Be warned:  the information given is not always complete...";

	angtk_eval("angband_display", "displayinfo", "show",
		"warrior_probe_desc", s, NULL);
...
static void pseudo_probe(void)
{
	...
	else
	{
		...
		approx_hp =
			m_ptr->hp - rand_int(m_ptr->hp / 4) + rand_int(m_ptr->hp / 4);

		/* Approximate monster HPs */
		if (r_ptr->mana)
			approx_mana =
				m_ptr->mana - rand_int(m_ptr->mana / 4) +
...
void shapechange(s16b shape)
{
	...
	lite_spot(p_ptr->py, p_ptr->px); /* TNB */
...
static void choose_ele_attack(void)
{
	...
	bool okay, flag = FALSE;

	char out_val[160];

	/* Calculate the number of allowed brands */
	num = (p_ptr->lev - 20) / 7;
	if (num <= 0)
		num = 1;

	/* Build a prompt */
	(void) sprintf(out_val, "(a-%c) Choose a temporary elemental brand:",
		I2A(num - 1));

	Bind_Choose(KEYWORD_CHOOSE_ELE_ATTACK + 1, 0, TRUE);

	while (!flag)
	{
		inkey_flags = (INKEY_ELE_ATTACK);
		...
		/* Verify */
		if (choice < 'a' || choice >= 'a' + num)
		{
			bell("illegal ele_attack choice!");
			continue;
		}
	...
	Bind_Choose(KEYWORD_CHOOSE_ELE_ATTACK + 1, 0, FALSE);
	...
	if (num >= 2)
		c_prt(TERM_L_WHITE, "        b) Cold Brand", 3, 14);
	else
		prt("", 3, 14);

	if (num >= 3)
		c_prt(TERM_L_DARK, "        c) Acid Brand", 4, 14);
	else
		prt("", 4, 14);

	if (num >= 4)
		c_prt(TERM_BLUE, "        d) Elec Brand", 5, 14);
	else
		prt("", 5, 14);
...
static void rebalance_weapon(void)
{
	...
	else if (randint(5) == 1)
	{
		...
		/* Describe */
		msg_format("Oh no!  A dreadful black aura surrounds your %s!",
			o_name);
 ...
 */
#if 1 /* TNB */
static int get_spell(int *sn, cptr prompt, int k_idx, bool known)
#else
...
#endif
{
	...
	*sn = (-1);
	...
	Bind_Choose(KEYWORD_CHOOSE_SPELL + 1, k_idx, TRUE);	/* TNB */
	...
	while (!flag)
	{
		...
		/* Build a prompt (accept all spells) */
		strnfmt(out_val, 78, "%^ss: %c-%c,", p, I2A(0),
			I2A(after_last_spell - first_spell - 1));
		...
		/* Build the prompt */
		strfmt(tmp_val, "(%s) %^s which %s? ", out_val, prompt, p);

		inkey_flags = (INKEY_SPELL);
		inkey_book = k_idx;

		/* Get a command */
		okay = get_com(tmp_val, &choice);
	...
	while (!flag && get_com(out_val, &choice))
	{
		...
		if (ver)
		{
			...
			/* Prompt */
			strnfmt(tmp_val, 78, "%^s %s (%d mana, %d%% fail)? ", prompt,
				spell_names[s_ptr->index], s_ptr->smana,
				spell_chance(spell));
	...
	Bind_Choose(KEYWORD_CHOOSE_SPELL + 1, 0, FALSE); /* TNB */

	if (redraw)
		angtk_eval("angband_display", "book", "hide", NULL); /* TNB */

	sound_cancel();	/* ALLOW_SOUND */
...
#if 1 /* TNB */
		if (!get_spell(&spell, "browse", o_ptr->k_idx, TRUE))
#else
...
/*
 * Test an object for learnable spells.
 */
...
void do_cmd_study(void)
{
	...
	/* Mention the result */
	sound(SNDGRP_EVENT, SND_SPELL_LEARN, 0); /* ALLOW_SOUND */
...
/*
 * Test an object for castable spells.
 */
...
void do_cmd_cast_or_pray(void)
{
	...
	/* Ask for a spell */
#if 1
	if (!get_spell(&spell, t, o_ptr->k_idx, TRUE))
#else
	...
	{
		sound(SNDGRP_EVENT, SND_SPELL_CAST, 0);	/* ALLOW_SOUND */
		...
		switch (s_ptr->index)
		{
			...
			case 0: /* Magic Missile */
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt(GF_MANA, dir, damroll(2, 4 + plev / 10));
				break;
			}
			...
			case 3: /* Light Area */
			{
				(void) lite_area(damroll(2, 1 + (plev / 5)),
					(plev / 10) + 1);
				break;
			}
			...
			case 45: /* Earthquake */
			{
				sound(SNDGRP_EVENT, SND_SPELL_EARTHQUAKE, 0x02); /* ALLOW_SOUND */
			...
			case 52: /* Strengthen Defenses */
			{
				...
				{
					(void) set_extra_defences(p_ptr->magicdef +
						randint(20));
				}
			...
			case 60: /* Rogue Spell: Day of Misrule */
			{
				...
				msg_format
					("%s of Night rejoice!  It's the Day of Misrule!", p);
			...
			case 68: /* Call Light */
			{
				(void) lite_area(damroll(2, 1 + (plev / 3)),
					(plev / 10) + 1);
				break;
			}
			case 69: /* Find Traps */
			...
			{
				(void) set_oppose_fire(p_ptr->oppose_fire + randint(10) +
					plev / 2);
			...
			case 81: /* Orb of Draining */
			{
				if (!get_aim_dir(&dir))
					return;
				fire_sphere(GF_HOLY_ORB, dir, (damroll(3,
							6) + plev / 4 + (plev / ((p_ptr->pclass ==
									CLASS_PRIEST) ? 2 : 4))),
					((plev < 30) ? 1 : 2), 30);
				break;
			}
			...
			case 83: /* Protection from Evil */
			{
				if (!p_ptr->protevil)
				{
					(void) set_protevil(p_ptr->protevil + randint(24) +
						3 * plev / 2);
				}
				else
				{
			...
			{
#if 1 /* TNB */
				(void) dispel_undead(plev * 2 + randint(plev));
#else /* TNB */
			...
			{
#if 1 /* TNB */
				(void) dispel_evil(plev * 2 + randint(plev));
#else /* TNB */
			...
			{
#if 1 /* TNB */
				(void) dispel_evil(plev * 3 + randint(plev));
#else
			...
			case 113: /* Dispel Curse */
			{
				if (remove_all_curse())
					msg_print
						("A beneficent force surrounds you for a moment.");
				break;
			}
			...
			case 116: /* Enchant Weapon or Armour */
			{
				...
				/* Query */
				msg_print
					("Would you like to enchant a 'W'eapon or 'A'rmour (w/a)");
				...
				while (1)
				{
					...
					else if ((answer == 'A') || (answer == 'a'))
					{
						...
						msg_print
							("Please type 'w' to enhant a weapon, 'a' to enchant armour, or ESC to cancel");
				}
			...
			case 129: /* call light */
			{
				(void) lite_area(damroll(2, 1 + (plev / 4)),
					(plev / 10) + 1);
				break;
			}
			case 130: /* foraging */
			...
			case 155: /* wither foe */
			{
				if (!get_aim_dir(&dir))
					return;
				aim_dir_preserve(dir); /* TNB */
				fire_bolt(GF_MANA, dir, damroll(plev / 7, 8));
				aim_dir_maintain(); /* TNB */
				(void) confuse_monster(dir, plev + 10);
				aim_dir_maintain(); /* TNB */
				(void) slow_monster(dir, plev + 10);
				aim_dir_finish(); /* TNB */
			...
			{
				msg_print("Boom!");
				fire_sphere(GF_SOUND, 0, plev + randint(40 + plev * 2),
					plev / 8, 20);
				break;
			}
			...
			case 169: /* sight beyond sight */
			{
				...
				{
					(void) set_tim_invis(p_ptr->tim_invis + randint(12) +
						12);
				}
				break;
			}
			...
			case 173: /* volcanic eruption */
			{
				...
				fire_sphere(GF_FIRE, 0,
					3 * plev / 2 + randint(50 + plev * 3), 1 + plev / 15,
					20);
				sound(SNDGRP_EVENT, SND_SPELL_EARTHQUAKE, 0x02); /* ALLOW_SOUND */
			...
			{
				msg_print
					("Light bright beyond enduring dazzles your foes!");
				fire_sphere(GF_LITE, 0, 5 * plev / 2 + randint(plev * 3),
					plev / 10, 20);
				break;
			}
...
#if 1 /* TNB */
				(void) dispel_monsters(plev + randint(plev));
			...
			{
				msg_print
					("Amidst the gloom, you envoke light and beauty; your body regains its natural vitality.");
			...
			case 181: /* time blast */
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt_or_beam(plev * 2, GF_TIME, dir, damroll(plev / 6,
						8));
				break;
			}
			...
			case 219: /* mana bolt */
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt_or_beam(beam, GF_MANA, dir, damroll(1 + plev / 2,
						8));
				break;
			}
			case 220: /* genocide */
			...
			{
				(void) set_oppose_acid(p_ptr->oppose_pois + randint(20) +
					20);
				(void) set_oppose_cold(p_ptr->oppose_cold + randint(20) +
					20);
				break;
			...
			{
				(void) set_protevil(p_ptr->protevil + plev / 2 +
					randint(plev));
				break;
			}
			...
			case 235: /* strike at life */
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt(GF_NETHER, dir, damroll(3 * plev / 5, 11));
				break;
			}
			...
			case 251: /* Rogue and Assassin Spell - Slip into the Shadows */
			{
				...
				{
					(void) set_superstealth(p_ptr->superstealth +
						randint(20));
				}
				break;
			}
	...
	else
	{
		...
		if (rand_int(100) < 50)
		{
			...
			/* Message */
			sound(SNDGRP_EVENT, SND_STAT_DRAIN, 0);	/* ALLOW_SOUND */
