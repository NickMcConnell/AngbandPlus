...
s16b get_mon_num(int level)
{
	...
	while (TRUE)
	{
		...
		for (i = 0; i < alloc_race_size; i++)
		{
			...
			if ((p_ptr->depth != 0) && (table[i].level <= 0))
				continue;
...
void update_mon(int m_idx, bool full)
{
	...
	{
		target_vis = flag &&
			projectable(p_ptr->py, p_ptr->px, m_ptr->fy, m_ptr->fx, 0);
...
void pack_gold(object_type * o_ptr, s32b cost)
{
	...
	for (k_idx = first_gold_idx; k_idx < first_gold_idx + SV_GOLD_MAX;
		k_idx++)
	{
		/* Paranoia -- Skip objects without value. */
		if (!k_info[k_idx].cost)
			continue;

		/* Stop if we're going too deep. */
		if (k_info[k_idx].level > gold_depth)
			break;
 ...
 */
/* static -- TNB */ bool place_monster_one(int y, int x, int r_idx,
	bool slp)
{
	int i;
	...
	if ((cave_feat[y][x] == FEAT_WATER) && ((strchr("uU", r_ptr->d_char))
...
static void place_monster_escort(int y, int x, int leader_idx, bool slp)
{
	...
	if (r_ptr->flags1 & (RF1_ESCORTS))
		escort_size = 6 + rand_int(15);
	else
		escort_size = 2 + rand_int(7);
...
bool alloc_monster(int dis, bool slp, bool quick)
{
	...
	if (place_monster(y, x, slp, TRUE, quick))
		return (TRUE);
...
bool summon_specific(int y1, int x1, bool scattered, int lev, int type)
{
	...
	r_idx =
		get_mon_num((p_ptr->depth + lev) / 2 + 1 + (p_ptr->depth >=
		20 ? 4 : p_ptr->depth / 5));
