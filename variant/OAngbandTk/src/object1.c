...
static cptr amulet_adj[MAX_AMULETS] = {
	...
	"Shark's Tooth", "Silver", "Stained Glass", "Starstone",
		"Tortoise Shell"
};
...
static cptr potion_adj[MAX_COLORS] = {
	...
	"Carnation", "Chartreuse", "Chocolate-brown", "Clear Blue",
		"Clotted Red",
...
void flavor_init(void)
{
	...
	if (mp_ptr->spell_book)
	{
		...
		{
			object_kind *k_ptr =
				&k_info[lookup_kind(mp_ptr->spell_book, i)];
			if (mp_ptr->book_start_index[i] ==
				mp_ptr->book_start_index[i + 1]) k_ptr->d_attr =
					TERM_SLATE;
		}
	}
}
...
void object_desc(char *buf, object_type * o_ptr, int pref, int mode)
{
	...
	if (basenm[0] == '&')
	{
		...
		else if (artifact_p(o_ptr) && ((known) || ((easy_know) &&
					(aware))))
		{
	...
	if ((f1 & (TR1_PERFECT_BALANCE)) && (f1 & (TR1_THROWING)) && (known) &&
		(!o_ptr->name1))
	{
		...
		else if (known && (o_ptr->tval == TV_ROD))
		{
			...
			if (o_ptr->timeout)
			{
				...
				if (o_ptr->number > 1)
				{
					 ...
					 */
					power =
						(o_ptr->timeout + (k_ptr->pval - 1)) / k_ptr->pval;
...
void show_equip(void)
{
	...
	for (j = 0; j < k; j++)
	{
		...
		else if ((!o_ptr) || (object_known_p(o_ptr)) ||
...
bool get_item(int *cp, cptr pmt, cptr str, int mode)
{
	...
	else
	{
		...
		if ((p_ptr->command_cmd == 'f') && allow_equip)
		{
			p_ptr->command_wrk = (USE_EQUIP);
		}
	...
	while (!done)
	{
		...
		if (p_ptr->command_wrk == (USE_INVEN))
		{
			...
			if (can_squelch)
				strcat(out_val, " ! for all SQUELCHED,");
		}
		...
		else if (p_ptr->command_wrk == (USE_FLOOR))
		{
			...
			/* List choices. */
				...
				sprintf(tmp_val, " %c-%c, . for a,", I2A(f1 - f1),
					I2A(f2 - f1));
...
cptr object_adj(int tval, int sval)
{
	...
	{
		case TV_AMULET:
			return amulet_adj[sval];
		case TV_RING:
			return ring_adj[sval];
		case TV_STAFF:
			return staff_adj[sval];
		case TV_WAND:
			return wand_adj[sval];
		case TV_ROD:
			return rod_adj[sval];
		case TV_POTION:
			return potion_adj[sval];
		case TV_SCROLL:
...
#if 1 /* ALLOW_SOUND */
	if (count > 0)
		sound(SNDGRP_EVENT, SND_OBJECT_DESTROYED, 0);
#endif /* ALLOW_SOUND */
	if (count > 0)
		msg_format("You destroy %i SQUELCH-ed item%s.", count,
			((count > 1) ? "s" : ""));
	else
		msg_print("No SQUELCH-ed items to destroy.");
