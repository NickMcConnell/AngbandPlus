...
void teleport_away(int m_idx, int dis)
{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	sound(SNDGRP_EVENT, SND_MONSTER_TELEPORT, 0x02);
#else
	sound(SOUND_TPOTHER);
#endif
	...
	if (cave_m_idx[y][x] > 0)
	{
		...
		if (cave_feat[y][x] == FEAT_WATER)
		{
					...
					(!r_ptr->flags2 & (RF2_FLYING))))
			{
				...
				mon_take_hit(cave_m_idx[y][x], damroll(2,
						18 + m_ptr->maxhp / 12), FALSE, note_dies);
...
void teleport_player(int dis, bool safe)
{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	sound(SNDGRP_EVENT, SND_SPELL_TELEPORT, 0x02);
#else
	...
	if (!safe)
	{
		...
		else if ((cave_feat[y][x] == FEAT_RUBBLE) && (rand_int(2) == 0))
		{
			...
			if (rand_int(3) == 0)
				set_stun(p_ptr->stun + damroll(2, 14));
			if (rand_int(3) != 0)
				set_cut(p_ptr->cut + damroll(2, 14) * 2);
		}
...
void take_hit(int dam, cptr kb_str)
{
	...
	if (p_ptr->chp < 0)
	{
		/* Sound */
#ifdef ALLOW_SOUND
		sound(SNDGRP_EVENT, SND_CHARACTER_DEATH, 0);
#else
		sound(SOUND_DEATH);
#endif
	...
	if (p_ptr->chp < warning)
	{
		...
		{
#ifdef ALLOW_SOUND
			sound(SNDGRP_EVENT, SND_WARN_HITPOINT, 0);
#else
	...
	for (i = 0; i < INVEN_PACK; i++)
	{
		...
		if ((*typ) (o_ptr))
		{
			...
			if (amt)
			{
				...
				/* Message */
				sound(SNDGRP_EVENT, SND_PACK_DESTROY, 0); /* ALLOW_SOUND */
...
static int minus_ac(int dam)
{
	...
	/* Message */
	sound(SNDGRP_EVENT, SND_PACK_DAMAGE_ACID, 0); /* ALLOW_SOUND */
...
void elec_dam(int dam, cptr kb_str)
{
	...
	if (dam > 30)
	{
		if (randint(dam - 15) > dam / 2)
			set_stun(p_ptr->stun +=
				(int) rand_int(dam > 900 ? 50 : 5 + dam / 20));
	}
...
bool apply_disenchant(int dam)
{
	...
	if ((SCHANGE) && (randint(dam) > 20 + (dam / 3)) &&
	...
	/* Message */
	sound(SNDGRP_EVENT, SND_PACK_DISENCHANT, 0); /* ALLOW_SOUND */
...
static bool project_m(int who, int y, int x, int dam, int typ, int flg)
{
	...
	switch (typ)
	{
		...
		case GF_PLASMA:
		{
			...
			if (seen)
				obvious = TRUE;
			if (prefix(name, "Plasma") || (r_ptr->flags4 & (RF4_BRTH_PLAS))
	...
	else if (do_stun && !(r_ptr->flags4 & (RF4_BRTH_SOUND)) &&
...
#if 1 /* BUG -- TNB */
	/* Don't do this for dead monster */
	if (cave_m_idx[y][x] > 0)
	{
#endif /* TNB */
...
static bool project_p(int who, int d, int y, int x, int dam, int typ)
{
	...
	if (((rp_ptr->flags_special) & PS_SHADOW) && (typ == GF_LITE))
		dam += dam / 3;
	if (((rp_ptr->flags_special) & PS_WOODEN) && (typ == GF_FIRE))
		dam += dam / 3;
	...
	switch (cave_feat[y][x])
	{
		...
		case FEAT_TREE:
		{
			if ((randint(8) == 1) && ((p_ptr->pclass == CLASS_RANGER) ||
	...
	switch (typ)
	{
		...
		case GF_ROCK:
		{
				...
				(!p_ptr->paralyzed))
			{
				...
				dodging =
					2 * (adj_dex_ta[p_ptr->stat_ind[A_DEX]] - 124) +
			...
			if (dam)
			{
				...
				dam -=
					(dam * ((p_ptr->ac + p_ptr->to_a <
						 150) ? p_ptr->ac + p_ptr->to_a : 150) / 300);
		...
		case GF_SHOT:
		{
			...
			/* Reduce damage if missile did not get deflected. */
				...
				dam -=
					(dam * ((p_ptr->ac + p_ptr->to_a <
						 150) ? p_ptr->ac + p_ptr->to_a : 150) / 250);
			...
			if (dam)
			{
				...
				if (rand_int(4) == 0)
				{
					...
					/* Be careful not to knock out the player immediately. */
					(void) set_stun(p_ptr->stun + ((dam / 3 >
								30) ? 30 : dam / 3));
				}
		...
		case GF_ARROW:
		{
			...
			if ((p_ptr->ac + p_ptr->to_a <
					150 ? p_ptr->ac + p_ptr->to_a : 150) >
...
rand_int(MAX_SHIELD_BASE_AC * 4)))
		...
		case GF_COLD:
		{
			...
			if ((r_ptr->flags2 & (RF2_MORGUL_MAGIC)) &&
				(!p_ptr->immune_cold) && (((!p_ptr->resist_cold) &&
						(!p_ptr->oppose_cold)) || (((!p_ptr->resist_cold)
							|| (!p_ptr->oppose_cold)) &&
						(rand_int(3) == 0))))
			{
				k = randint(3);
				...
				{
					msg_print
						("A deadly chill drives daggers into your soul!");
					if (!p_ptr->free_act)
					{
					...
					{
						(void) set_afraid(p_ptr->afraid + rand_int(21) +
							10);
					}
					if (!p_ptr->hold_life)
					{
		...
		case GF_POIS:
		{
			...
			if (r_ptr->flags2 & (RF2_MORGUL_MAGIC))
			{
					...
					(rand_int(dam / 2 + 20) > p_ptr->skill_sav))
				{
					...
					(void) set_paralyzed(p_ptr->paralyzed + rand_int(3) +
						2);
				}
		...
		case GF_HELLFIRE:
		{
			...
			if (randint(5 * r_ptr->level / 4) > p_ptr->skill_sav)
			{
				...
				{
					(void) set_confused(p_ptr->confused + rand_int(31) +
						30);
				}
			}
		...
		case GF_LITE:
		{
			...
			{
				(void) set_blind(p_ptr->blind + randint(5) + dam >
					40 ? 2 : 0);
			}
			take_hit(dam, killer);
			break;
		...
		case GF_INERTIA:
		{
			...
			(void) set_slow(p_ptr->slow + rand_int(5) + dam >=
				100 ? 6 : 4);
			take_hit(dam, killer);
			break;
		}
		...
		case GF_GRAVITY:
		{
			(void) set_slow(p_ptr->slow + rand_int(3) + dam >=
				100 ? 4 : 2);
			if (!p_ptr->resist_sound)
			{
		...
		case GF_WATER:
		{
			...
			if (fuzzy)
				msg_print("You are hit by something!");
			if ((!p_ptr->resist_sound) && (rand_int(2) == 0))
			{
				(void) set_stun(p_ptr->stun + randint(5 + dam / 10));
			}
			if ((!p_ptr->resist_confu) && (rand_int(2) == 0))
			{
		...
		case GF_STORM:
		{
			...
			if ((rand_int(2) == 0) && (!p_ptr->resist_confu))
			{
				(void) set_confused(p_ptr->confused + 5 +
					randint(dam / 3));
			}
...
static bool project_t(int who, int y, int x, int dam, int typ, int flg)
{
...
#if 1 /* BUG -- TNB */
	/* Get the monster name *before* teleporting it */
	if (m_ptr != NULL)
		monster_desc(m_name, m_ptr, 0);
#endif /* TNB */
	...
	switch (typ)
	{
		...
		case GF_FORCE:
		{
			...
			if ((affect_monster) && (r_ptr->flags4 & (RF4_BRTH_FORCE)))
				break;
		...
		case GF_STORM:
		{
			if ((typ == GF_STORM) && (affect_player))
			{
				...
				{
					msg_print
						("The wind grabs you, and whirls you around!");
					teleport_player(6, FALSE);
				}
			}
			...
			for (i = 0; i < path_n; ++i)
			{
				...
				if (!blind && !(flg & (PROJECT_HIDE)))
				{
					...
					if (player_has_los_bold(y, x))
					{
						...
						/* Visual effects */
						(void) angtk_effect_spell(y, x, typ, t);
						if (fresh_before)
							Term_fresh();
					...
					if (panel_contains(y, x) && player_has_los_bold(y, x))
					{
						...
						if ((op_ptr->delay_factor) || (fresh_before))
							Term_fresh();
						Term_xtra(TERM_XTRA_DELAY, msec);
						lite_spot(y, x);
						if ((op_ptr->delay_factor) || (fresh_before))
