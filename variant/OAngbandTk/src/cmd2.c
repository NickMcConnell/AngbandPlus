...
void do_cmd_go_up(void)
{
...
#else /* TNB */
		msg_print
			("This level will never appear again.  Really leave? (y/n)");
		answer = inkey();
	 ...
	 */
	if ((is_quest(p_ptr->depth) == FALSE) && (p_ptr->depth != 0) &&
		(randint(3) == 1))
	{
#if 1 /* TNB */
...
#else /* TNB */
		msg_print
			("The stairs continue down.  Go down another level? (y/n)");
...
static void chest_trap(int y, int x, s16b o_idx)
{
	...
	if (trap & (CHEST_H_SUMMON))
	{
		...
		{
			msg_print
				("Demons materialize in clouds of fire and brimstone!");
...
void do_cmd_alter(void)
{
	...
	else if ((p_ptr->pclass == CLASS_ROGUE) && (cave_naked_bold(y, x)))
	{
		if (num_trap_on_level == 0)
			...
			msg_print
				("You must disarm an existing trap to free up your equipment.");
		did_nothing = FALSE;
	}
...
void do_cmd_spike(void)
{
	...
	else
	{
		...
		{
#if 1
			cave_set_feat(y, x, cave_feat[y][x] + 0x01);
#else
			cave_feat[y][x] += 0x01;
#endif
...
static bool do_cmd_walk_test(int y, int x)
{
	...
	if (!cave_passable_bold(y, x))
	{
		...
		{
			/* If easy_alter option is on, doors are legal. */
			if (easy_alter)
				return (TRUE);
