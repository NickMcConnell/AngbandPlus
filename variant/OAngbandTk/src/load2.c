...
static void rd_item(object_type * o_ptr)
{
		...
		(o_ptr->pval != k_ptr->pval * o_ptr->number))
	{
		plog_fmt("The pval of%s %d Rod%s of %s\n"
			...
			(o_ptr->number > 1) ? " a stack of" : "", o_ptr->number,
			(o_ptr->number > 1) ? "s" : "", k_name + k_ptr->name,
			o_ptr->pval, k_ptr->pval * o_ptr->number);
		o_ptr->pval = k_ptr->pval * o_ptr->number;
	}
#endif /* BUG -- TNB */
...
static void rd_monster(monster_type * m_ptr)
{
	...
	if (o_older_than(0, 5, 1))
		m_ptr->r_idx = mon_index_conv[tmp16u];
...
static void convert_options(u32b flag[8], u32b mask[8])
{
	...
	byte opt_map[OPT_MAX];

	/* By default each option is unchanged */
	for (i = 0; i < OPT_MAX; i++)
		opt_map[i] = i;

	if (o_older_than(0, 4, 0))
	{
		/* Map options to their new positions */
		opt_map[14] = OPT_ring_bell;
		opt_map[30] = OPT_easy_sense;
		opt_map[31] = OPT_easy_open;
#if 0
		opt_map[42] = OPT_flow_by_sound;
		opt_map[43] = OPT_flow_by_smell;
#endif
		opt_map[44] = OPT_easy_alter;
		opt_map[45] = OPT_remember_recall;
/*		opt_map[57] = OPT_empty_levels; */

		/* Set new options to their defaults */
		op_ptr->opt[OPT_metric] = option_norm[OPT_metric];
	}

	if (o_older_than(0, 4, 1))
	{
		/* Set new options to their defaults */
		op_ptr->opt[OPT_verify_destroy_junk] =
			option_norm[OPT_verify_destroy_junk];
		op_ptr->opt[OPT_center_player] = option_norm[OPT_center_player];
		op_ptr->opt[OPT_center_running] = option_norm[OPT_center_running];
	}
	...
	for (i = 0; i < OPT_MAX; i++)
	{
		...
		if (flag[old_os] & (1L << old_ob))
			flag_new[new_os] |= (1L << new_ob);

		if (mask[old_os] & (1L << old_ob))
			mask_new[new_os] |= (1L << new_ob);
	}

	for (i = 0; i < 8; i++)
	{
		dbwin("flag[%d] was %#lX is %#lX\n", i, flag[i], flag_new[i]);
		flag[i] = flag_new[i];
		mask[i] = mask_new[i];
	}
}
...
static errr rd_extra(void)
{
	...
	if (p_ptr->psex > MAX_SEXES - 1)
		p_ptr->psex = MAX_SEXES - 1;
	...
	rd_byte(&tmp8u);
	if (older_than(2, 8, 5))
		adult_maximize = tmp8u;
	rd_byte(&tmp8u);
	if (older_than(2, 8, 5))
		adult_preserve = tmp8u;
...
static errr rd_dungeon(void)
{
	...
	for (i = o_max; i > 0; i--)
	{
		...
		if (!o_ptr->held_m_idx)
			continue;
		if (m_list[o_ptr->held_m_idx].hold_o_idx)
			continue;
...
static errr rd_savefile_new_aux(void)
{
	...
	if (o_older_than(O_VERSION_MAJOR, O_VERSION_MINOR, O_VERSION_PATCH))
	{
		...
		if ((o_sf_major == 0) && (o_sf_minor == 2))
		{
			angtk_eval("angband_load", "note",
				format("Loading an Angband %d.%d.%d savefile.", sf_major,
					sf_minor, sf_patch), NULL);
		}
		else
		{
			angtk_eval("angband_load", "note",
				format("Loading an OAngband %d.%d.%d savefile.",
					o_sf_major, o_sf_minor, o_sf_patch), NULL);
		}
#else /* TNB */
		if ((o_sf_major == 0) && (o_sf_minor == 2))
			msg_format("Loading an Angband %d.%d.%d savefile.", sf_major,
				sf_minor, sf_patch);
		else
	...
	if (o_older_than(0, 3, 0))
	{
		...
		if (arg_fiddle)
			note("Loaded artifacts, will create random artifacts.");
	}
	...
	if (!need_random_artifacts)
	{
		...
		{
			note(format
				("Number of stored random artifact names (%d) does not",
					num_of_random_arts));
			note(format
				("match the number of random artifacts in your copy of Oangband (%d).",
		...
		else
		{
			...
			if (err)
				note("Warning - random artifact naming failed!");
		}
	}
	...
	if (need_random_artifacts)
	{
		...
		if (arg_fiddle)
			note("Generated random artifacts");
	}
	...
	if (!g_grid_xtra_init)
	{
		op_ptr->opt[OPT_easy_sense] = option_norm[OPT_easy_sense];
		op_ptr->opt[OPT_remember_recall] =
			option_norm[OPT_remember_recall];
	}
...
static errr rd_squelch(void)
{
	...
	if (tmp16u > MAX_K_IDX)
	{
		note(format("Too many (%u) squelch entries!", tmp16u));
		return (1);
	}
...
static errr rd_tnb(void)
{
	...
	/* First check that the file version is 0.3.2 or greater. */
	if (o_older_than(0, 3, 2))
		return (0);
	...
	if (!older_than_tnb(1, 0, 1))
	{
		/* Read k_ptr->squelch */
		if (rd_squelch())
		{
			note("Error reading squelch array!");
			return (37);
		}
	}
