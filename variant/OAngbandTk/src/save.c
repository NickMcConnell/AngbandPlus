...
# endif
	...
	if (result)
		angtk_eval("angband_save", savefile, NULL);	/* TNB */
...
# endif
	...
	/* Get the name of the backup savefile: xyz.bkp */
	strcpy(dest, savefile);
	strcat(dest, ".bkp");

	/* Get the name of the temp savefile: xyz.bkp.new */
	strcpy(safe, dest);
	strcat(safe, ".new");
...
bool load_player(void)
{
			...
			(o_version_patch != o_sf_patch))
		{
			...
			if ((o_sf_major == 0) && (o_sf_minor == 2))
			{
				angtk_eval("angband_load", "note",
					format("Converted an Angband %d.%d.%d savefile.",
						sf_major, sf_minor, sf_patch), NULL);
			}
			else
			{
				angtk_eval("angband_load", "note",
					format("Converted an OAngband %d.%d.%d savefile.",
			...
			if ((o_sf_major == 0) && (o_sf_minor == 2))
				...
				msg_format("Converted an Oangband %d.%d.%d savefile.",
	...
	{
		angtk_eval("angband_load", "note",
			format("Error (%s) reading an Angband %d.%d.%d savefile.",
				what, sf_major, sf_minor, sf_patch), NULL);
	}
	else
	{
		angtk_eval("angband_load", "note",
			format("Error (%s) reading an OAngband %d.%d.%d savefile.",
				what, o_sf_major, o_sf_minor, o_sf_patch), NULL);
	}
#else /* TNB */
	if ((o_sf_major == 0) && (o_sf_minor == 2))
		...
		msg_format("Error (%s) reading an Oangband %d.%d.%d savefile.",
...
static void wr_tnb(void)
{
	 ...
	 * 1) The file version is 0.3.2 or greater.
