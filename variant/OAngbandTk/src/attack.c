...
static sint critical_shot(int chance, int sleeping_bonus,
	bool thrown_weapon, char o_name[], char m_name[], int visible)
{
	int i, k;
	int mult_a_crit = 10;
	...
	if (randint(i + 200) <= i)
	{
		...
		if (k < 125)
		{
			...
			{
				/* Visible monster */
				message_format(MSG_HIT, 0, "The %s strikes %s.", o_name,
					m_name);
			}
			mult_a_crit = 15;
		}
 ...
 */
static sint tot_dam_aux(object_type * o_ptr, monster_type * m_ptr,
	bool shooting)
{
	int mult_ego = 10;
	 ...
	 */
	if ((shooting) && (p_ptr->pclass == CLASS_PALADIN))
		allow_t_brand = FALSE;
	...
	switch (o_ptr->tval)
	{
		...
		case TV_DIGGING:
		{
			...
			if ((f1 & (TR1_SLAY_ANIMAL)) && (r_ptr->flags3 & (RF3_ANIMAL)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_ANIMAL) && (mult_ego < 20))
					mult_ego = 20;
				...
				(r_ptr->flags3 & (RF3_EVIL)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_EVIL) && (mult_ego < 17))
					mult_ego = 17;
			...
			if ((f1 & (TR1_SLAY_UNDEAD)) && (r_ptr->flags3 & (RF3_UNDEAD)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_UNDEAD) && (mult_ego < 25))
					mult_ego = 25;
			...
			if ((f1 & (TR1_SLAY_DEMON)) && (r_ptr->flags3 & (RF3_DEMON)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_DEMON) && (mult_ego < 25))
					mult_ego = 25;
			...
			if ((f1 & (TR1_SLAY_ORC)) && (r_ptr->flags3 & (RF3_ORC)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_ORC) && (mult_ego < 25))
					mult_ego = 25;
			...
			if ((f1 & (TR1_SLAY_TROLL)) && (r_ptr->flags3 & (RF3_TROLL)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_TROLL) && (mult_ego < 25))
					mult_ego = 25;
			...
			if ((f1 & (TR1_SLAY_GIANT)) && (r_ptr->flags3 & (RF3_GIANT)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_GIANT) && (mult_ego < 25))
					mult_ego = 25;
			...
			if ((f1 & (TR1_SLAY_DRAGON)) && (r_ptr->flags3 & (RF3_DRAGON)))
			{
				...
				if ((o_ptr->name2 == EGO_KILL_DRAGON) && (mult_ego < 25))
					mult_ego = 25;
					...
					(allow_t_brand)))
			{
				...
				{
					if ((o_ptr->name2 == EGO_BALROG) && (mult_ego < 30))
						mult_ego = 30;
					else if (mult_ego < 17)
						mult_ego = 17;
				}
...
static int get_druid_damage(int plev, char m_name[])
{
	...
	sound(SNDGRP_MARTIAL_ART, b_select - 1, 0);	/* ALLOW_SOUND */
...
void py_attack(int y, int x)
{
	...
	/* Bashing chance depends on melee Skill, Dex, and a class level bonus. */
		...
		bash_chance =
			...
			((p_ptr->pclass == CLASS_WARRIOR ||
				p_ptr->pclass == CLASS_PALADIN) ? p_ptr->lev : 0);
	...
	while (num++ < blows)
	{
				...
				r_ptr->ac + terrain_bonus, m_ptr->ml))
		{
			...
			/* Sound */
#if 1 /* ALLOW_SOUND */
			sound(SNDGRP_EVENT, SND_ATTACK_HIT, 0);
#else
			sound(SOUND_HIT);
#endif
			...
			if (hits == 1)
			{
				...
				if ((p_ptr->pclass == CLASS_ROGUE) ||
					(p_ptr->pclass == CLASS_ASSASSIN))
					add_wakeup_chance =
						p_ptr->base_wakeup_chance / 4 + 500;
			...
			else
			{
				...
				{
					k =
						1 + ((int) (adj_str_td[p_ptr->stat_ind[A_STR]]) -
						128);
			...
			if (mon_take_hit(cave_m_idx[y][x], k, &fear, NULL))
			{
				 ...
				 */
				if ((p_ptr->pclass == CLASS_WARRIOR) && (p_ptr->lev > 39)
					&& (num < p_ptr->num_blow) && (p_ptr->energy_use))
				{
					p_ptr->energy_use =
		...
		else
		{
			/* Sound */
#if 1 /* ALLOW_SOUND */
			sound(SNDGRP_EVENT, SND_ATTACK_MISS, 0);
#else
			sound(SOUND_MISS);
#endif
...
void do_cmd_fire(void)
{
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			if (test_hit_combat(chance2 + sleeping_bonus, armour,
					m_ptr->ml))
			{
				bool fear = FALSE;
				...
				tdam *=
					critical_shot(chance2, sleeping_bonus, FALSE, o_name,
					m_name, visible);
...
void do_cmd_throw(void)
{
	...
	if (f1 & (TR1_THROWING))
		...
		chance =
			((3 * p_ptr->skill_tht / 2) + (i_ptr->to_h * BTH_PLUS_ADJ));
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
					...
					r_ptr->ac + terrain_bonus, m_ptr->ml))
			{
				...
				if (f1 & (TR1_THROWING))
					tdam *=
						critical_shot(chance2, sleeping_bonus, TRUE,
				...
				{
					if (i_ptr->to_d > 0)
						tdam *= (100 + deadliness_conversion[i_ptr->to_d]);
				...
				tdam =
					damroll(tdam_whole,
					i_ptr->ds) + (tdam_remainder * damroll(1,
						i_ptr->ds) / 10000);
