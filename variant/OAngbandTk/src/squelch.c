...
static int do_cmd_squelch_aux(void)
{
	...
	else
	{
		...
		while (1)
		{
				...
				for (j = i; j < max_num; j++)
				{
					if ((k_info[choice[i]].tval > k_info[choice[j]].tval)
						|| ((k_info[choice[i]].tval ==
								k_info[choice[j]].tval) &&
							(k_info[choice[i]].cost >
								k_info[choice[j]].cost)))
					{
						temp = choice[i];
						choice[i] = choice[j];
						choice[j] = temp;
			...
			if (!max_num)
				c_put_str(TERM_RED, "No known objects of this type.", 3,
					0);
			...
			/* Choose! */
			if (!get_com(format
					("%s : Command? (^A: Squelch all   ^U: Unsquelch all)",
						tval_desc), &ch))
				return (1);
...
static int do_qual_squelch(void)
{
	...
	while (1) 
	{
		...
		for (num = 0; (num < 60) && tvals[num].tval; num++)
		{
			row = 2 + (num % 20);
			col = 30 * (num / 20);
			prt(format("(%c): %s", squelch_str[squelch_level[num]],
					tvals[num].desc), row, col);
		}
		...
		prt
			("Secondary Squelching Menu: Weapons/Armor (Squelch only after identify)",
			0, 0);
		...
		switch (ch)
		{
			...
			{
				squelch_level[index] =
					(squelch_level[index] + 1) % (SQUELCH_ALL + 1);
				break;
			}
			...
			{
				squelch_level[index] =
					(SQUELCH_ALL + squelch_level[index]) % (SQUELCH_ALL +
					1);
				break;
			}
...
int squelch_itemp(object_type *o_ptr, byte feeling, int fullid)
{
	...
	switch (squelch_level[num])
	{
		...
		case SQUELCH_CURSED:
		{
			result = (((feel == FEEL_BROKEN) || (feel == FEEL_TERRIBLE) ||
					(feel == FEEL_WORTHLESS) ||
