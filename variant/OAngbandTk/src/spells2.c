...
bool warding_glyph(void)
{
	...
	{
		msg_print
			("You cannot set any more glyphs until you desanctify your existing ones.");
		return (FALSE);
	}
	...
	if (num_glyph_on_level == 4)
		msg_print
			("You have now reached your glyph limit.  In order to set more, desanctify some.");
...
bool do_dec_stat(int stat)
{
	...
	if (sust)
	{
		/* Message */
		msg_format
			("You feel very %s for a moment, but the feeling passes.",
			desc_stat_neg[stat]);
...
bool do_res_stat(int stat)
{
	...
	{
		/* Message */
		sound(SNDGRP_EVENT, SND_STAT_RESTORE, 0); /* ALLOW_SOUND */
...
bool do_inc_stat(int stat)
{
	...
	{
		/* Message */
		sound(SNDGRP_EVENT, SND_STAT_GAIN, 0); /* ALLOW_SOUND */
...
bool restore_level(void)
{
	...
	{
		/* Message */
		sound(SNDGRP_EVENT, SND_EXPER_RESTORE, 0); /* ALLOW_SOUND */
...
/* TNB */
static int detection_aux(int range, bool show, cptr what)
{
	int y, x, dis, num = 0;
	...
	/* Visual feedback */
	if (show)
		angtk_detect_radius(py, px, range);
	...
	{
		/* Message */
		msg_format("You detect %d %s%s.", num, what, (num > 1) ? "s" : "");
	}
...
bool detect_traps(int range, bool show)
{
	...
	return detection_aux(range, show, "trap");
}
...
bool detect_doors(int range, bool show)
{
	...
	return detection_aux(range, show, "door");
}
...
bool detect_stairs(int range, bool show)
{
	...
	return detection_aux(range, show, "stairway");
}
...
static bool detect_hook_treasure(int y, int x)
{
	...
	{
		/* Expose the gold */
		cave_set_feat(y, x, cave_feat[y][x] + 0x02);
	}
...
bool detect_treasure(int range, bool show)
{
	...
	return detection_aux(range, show, "buried treasure");
}
...
bool detect_objects_gold(int range, bool show)
{
	...
	return detection_aux(range, show, "treasure");
}
...
bool detect_objects_normal(int range, bool show)
{
	...
	return detection_aux(range, show, "object");
}
...
bool detect_objects_magic(int range, bool show)
{
	...
	return detection_aux(range, show, "magic objects");
}
...
bool detect_monsters_normal(int range, bool show)
{
	...
	return detection_aux(range, show, "monster");
}
...
bool detect_monsters_invis(int range, bool show)
{
	...
	return detection_aux(range, show, "invisible creature");
}
...
bool detect_monsters_evil(int range, bool show)
{
	...
	return detection_aux(range, show, "evil creature");
}

/* TNB */
static bool detect_hook_monsters_living(int y, int x)
{
	int m_idx = cave_m_idx[y][x];
	monster_type *m_ptr = &m_list[m_idx];
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
...
{
	detection_hook = detect_hook_monsters_living;
	return detection_aux(range, show, "living creature");
}
...
bool detect_monsters_evil(int range, bool show)
{
	...
	if (num > 0)
	{
		...
		/* Print success message */
		if (num_off > 0)
			msg_format("You detect evil creatures (%i offscreen).",
				num_off);
		else
...
bool detect_monsters_living(int range, bool show)
{
	...
	if (num > 0)
	{
		...
		/* Print success message */
		if (num_off > 0)
			msg_format("You detect living creatures (%i offscreen).",
				num_off);
		else
...
bool detect_all(int range, bool show)
{
	...
	if (show)
#if 1 /* TNB */
		angtk_detect_radius(p_ptr->py, p_ptr->px, range);
#else
		animate_detect(range);
#endif
...
void set_ele_attack(u32b attack_type, int duration)
{
		...
		msg_format("For a while, the blows you deal will %s",
			((attack_type ==
					ATTACK_ACID) ? "melt with acid!" : ((attack_type ==
						ATTACK_ELEC) ? "shock your foes!" : ((attack_type
							== ATTACK_FIRE) ? "burn with fire!"
						: ((attack_type ==
								ATTACK_COLD) ? "chill to the bone!"
							: ((attack_type ==
									ATTACK_POIS) ? "poison your enemies!" :
								"do nothing special."))))));
		p_ptr->redraw |= PR_STATUS;
		note = TRUE; /* TNB */
	}
	...
	if (note)
		redraw_add(PR_ELE_ATTACK); /* ALLOW_STATUS_EXTRA */
...
bool ident_spell(void)
{
	...
	if (cursed_p(o_ptr) || broken_p(o_ptr))
		sound(SNDGRP_EVENT, SND_IDENT_WORTHLESS, 0);
	else if (artifact_p(o_ptr))
		sound(SNDGRP_EVENT, SND_IDENT_ARTIFACT, 0);
	else if (ego_item_p(o_ptr))
		sound(SNDGRP_EVENT, SND_IDENT_EGO, 0);
	...
	{
		msg_format("In your pack: %s (%c).  %s", o_name,
			index_to_label(item),
			((squelch == 1) ? "(Squelch)" : ((squelch ==
						-1) ? "(Squelch Failed)" : "")));
	}
	else
	{
...
#if 1 /* TNB */
		/* Call lite_spot so [angband cave track] can update the display */
...
#if 1 /* TNB */
	do_cmd_observe(o_ptr, item, FALSE);
#else /* TNB */
...
bool recharge(int power)
{
	...
	if (fail)
	{
		...
		else
		{
			...
			{
				sound(SNDGRP_EVENT,
					(item >= 0) ? SND_PACK_DESTROY : SND_OBJECT_DESTROYED, 0); /* ALLOW_SOUND */
				if (o_ptr->number > 1)
...
void do_starlight(int burst_number, int dam, bool strong)
{
	...
	for (i = 0; i < burst_number; i++)
	{
		...
		if (!(cave_info[y][x] & (CAVE_GLOW)))
			fire_meteor(-1, GF_CONFUSION, y, x, dam, strong ? 1 : 0,
				FALSE);
...
/*
 * A user-friendly probe, just like target_set(). -- TNB
 */
bool probing(void)
{
	int i, d, m;
	...
	while (!done)
	{
		...
		prompt_format("%^s has %d hit points. [q,+,-,<dir>]", m_name,
			m_ptr->hp);
		...
		switch (query)
		{
			...
			default:
			{
				...
				/* Oops */
				if (!d)
					bell("Illegal command for probing!");
...
void destroy_area(int y1, int x1, int r, bool full)
{
	...
	{
		/* Message */
		sound(SNDGRP_EVENT, SND_SPELL_DESTRUCTION, 0); /* ALLOW_SOUND */
		...
		for (dx = -r; dx <= r; dx++)
		{
			...
			if (cave_m_idx[yy][xx] > 0)
			{
					...
					!(r_ptr->flags2 & (RF2_PASS_WALL)))
				{
					...
					monster_disturb(cave_m_idx[yy][xx]); /* m_ptr->csleep = 0; -- TNB */
...
static void cave_temp_room_lite(void)
{
	...
	for (i = 0; i < temp_n; i++)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			{
				/* Wake up! */
				monster_disturb(cave_m_idx[y][x]); /* m_ptr->csleep = 0; -- TNB */
...
#if 1 /* TNB */
		if (g_feat_flag[cave_feat[y][x]] & FEAT_FLAG_BORING)
#else /* TNB */
		if (cave_feat[y][x] <= FEAT_INVIS)
#endif /* TNB */
		{
...
bool fire_ball(int typ, int dir, int dam, int rad, bool jump)
{
	...
	sound(SNDGRP_EVENT, SND_SPELL_BALL, 0x02); /* ALLOW_SOUND */
	...
	byte diameter_of_source)
{
	...
	/* Analyze the "dir" and the "target".  Hurt items on floor. */
	return (project(-1, rad, ty, tx, dam, typ, flg, 0,
			diameter_of_source));
}
...
bool fire_bolt(int typ, int dir, int dam)
{
	...
	sound(SNDGRP_EVENT, SND_SPELL_BOLT, 0x02); /* ALLOW_SOUND */
...
bool fire_beam(int typ, int dir, int dam)
{
	...
	sound(SNDGRP_EVENT, SND_SPELL_BEAM, 0x02); /* ALLOW_SOUND */
	return (project_hook(typ, dir, dam, flg));
}
