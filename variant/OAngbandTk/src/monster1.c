	...
	monster_type * m_ptr)
{
	...
	switch (ghost_class)
	{
		...
		case 1:
		{
			...
			if (r_ptr->flags4 & (RF4_SHOT))
				r_ptr->flags4 &= ~(RF4_SHOT);
			if (r_ptr->flags4 & (RF4_ARROW))
				r_ptr->flags4 &= ~(RF4_ARROW);
			if (r_ptr->flags4 & (RF4_BOLT))
				r_ptr->flags4 &= ~(RF4_BOLT);

			r_ptr->flags5 |= (RF5_BOLT_MANA);

			if (dun_level > 11)
				r_ptr->flags5 |= (RF5_BALL_POIS);
			if ((dun_level > 13) && (dun_level < 25))
				r_ptr->flags5 |= (RF5_BOLT_ELEC);
			if ((dun_level > 16) && (dun_level < 35))
				r_ptr->flags5 |= (RF5_BEAM_ICE);
			if (dun_level > 24)
				r_ptr->flags5 |= (RF5_BALL_ELEC);
			if (dun_level > 34)
				r_ptr->flags5 |= (RF5_BALL_COLD);
			if (dun_level > 44)
				r_ptr->flags5 |= (RF5_BALL_ACID);
			if (dun_level > 64)
				r_ptr->flags5 |= (RF5_BALL_MANA);

			if (dun_level > 19)
				r_ptr->flags6 |= (RF6_SLOW);
			if (dun_level > 39)
				r_ptr->flags6 |= (RF6_HOLD);
		...
		case 2:
		{
			...
			if (dun_level > 34)
				r_ptr->flags5 |= (RF5_BALL_LITE);
			if (dun_level > 64)
				r_ptr->flags5 |= (RF5_BEAM_NETHR);
			...
			if (dun_level > 24)
				r_ptr->flags6 |= (RF6_CURE);

			if (dun_level > 29)
				r_ptr->flags7 |= (RF7_S_MONSTER);
			if (dun_level > 49)
				r_ptr->flags7 |= (RF7_S_MONSTERS);
		...
		case 5:
		{
			...
			r_ptr->flags3 |=
				(RF3_IM_FIRE | RF3_IM_COLD | RF3_IM_ELEC | RF3_IM_ACID);
		...
		case 6:
		{
			...
			if (dun_level > 34)
				...
				r_ptr->flags5 |= (RF5_BOLT_COLD);

			if (dun_level > 39)
				r_ptr->flags5 |= (RF5_BALL_FIRE);
			...
			if (dun_level > 39)
				...
				r_ptr->flags5 |= (RF5_BOLT_ACID);

			if (dun_level > 44)
				r_ptr->flags5 |= (RF5_BALL_POIS);
			else if (dun_level > 24)
				r_ptr->flags5 |= (RF5_BOLT_POIS);

			if (dun_level > 29)
				r_ptr->flags5 |= (RF5_BOLT_PLAS);

			if (dun_level > 49)
				r_ptr->flags5 |= (RF5_BALL_STORM);
			else if (dun_level > 34)
				r_ptr->flags5 |= (RF5_BOLT_WATER);

			if (dun_level > 54)
				r_ptr->flags5 |= (RF5_BALL_SOUND);
			if (dun_level > 54)
				r_ptr->flags5 |= (RF5_BALL_SHARD);
			if (dun_level > 59)
				r_ptr->flags5 |= (RF5_ARC__FORCE);
			...
			if (dun_level < 40)
				r_ptr->flags7 |= (RF7_S_ANT);
			if (dun_level < 40)
				r_ptr->flags7 |= (RF7_S_SPIDER);
			if (dun_level > 20)
				r_ptr->flags7 |= (RF7_S_ANIMAL);
			if (dun_level > 20)
				r_ptr->flags7 |= (RF7_S_HOUND);
		...
		case 7:
		{
			...
			if (dun_level > 4)
				r_ptr->flags5 |= (RF5_BALL_POIS);
			if (dun_level > 14)
				r_ptr->flags5 |= (RF5_BALL_DARK);
			if (dun_level > 44)
				r_ptr->flags5 |= (RF5_BALL_NETHR);
			if (dun_level > 54)
				r_ptr->flags5 |= (RF5_BALL_MANA);
			if (dun_level > 54)
				r_ptr->flags5 |= (RF5_ARC__HFIRE);
			...
			if (dun_level > 49)
				r_ptr->flags6 |= (RF6_BRAIN_SMASH);
			else if (dun_level > 29)
				r_ptr->flags6 |= (RF6_MIND_BLAST);

			if (dun_level > 19)
				r_ptr->flags7 |= (RF7_S_DEMON);
			if (dun_level > 69)
				r_ptr->flags7 |= (RF7_S_HI_DEMON);
			if (dun_level > 29)
				r_ptr->flags7 |= (RF7_S_UNDEAD);
			if (dun_level > 79)
				r_ptr->flags7 |= (RF7_S_HI_UNDEAD);
		...
		case 8:
		{
			if ((dun_level > 9) && (r_ptr->freq_spell == 0))
				...
				r_ptr->freq_spell = 3 * r_ptr->freq_spell / 4;
...
bool prepare_ghost(int r_idx, monster_type * m_ptr, bool from_savefile)
{
	...
	err =
		(fscanf(fp, "%[^\n]\n%d\n%d\n%d\n%d:%[^\n]", ghost_name,
		  &ghost_sex, &ghost_race, &ghost_class, &ghost_string_type,
			ghost_string) != 6);
	...
	if (err)
	{
		...
		fp = my_fopen(path, "r");
		err =
			(fscanf(fp, "%[^\n]\n%d\n%d\n%d", ghost_name, &ghost_sex,
			  &ghost_race, &ghost_class) != 4);
	}
	...
	/* Sanity check. */
	if (ghost_race >= MAX_P_IDX)
		ghost_race = rand_int(MAX_P_IDX);
	...
	if (ghost_class >= MAX_CLASS)
		ghost_class = rand_int(MAX_CLASS);
