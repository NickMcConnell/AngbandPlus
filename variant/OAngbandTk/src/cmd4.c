...
void do_cmd_redraw(void)
{
	...
	/* Window stuff */
	p_ptr->window |=
		(PW_INVEN | PW_EQUIP | PW_SPELL | PW_PLAYER_0 | PW_PLAYER_1);
...
void do_cmd_message_one(void)
{
	...
	sound_message_play(0);
#endif /* ALLOW_SOUND -- TNB */

	/* Recall one message XXX XXX XXX */
#if 1 /* TNB */
	g_prompt_attr = message_color(0);
	prompt_format("> %s", message_str(0));
		...
		switch (ch)
		{
			case ESCAPE:
			{
				...
				for (i = OPT_cheat_start; i < OPT_cheat_end + 1; i++)
				{
					...
					{
						/* Set score option */
						op_ptr->opt[OPT_score_start + (i -
								OPT_cheat_start)] = TRUE;
					}
				}
	...
	while (1)
	{
		...
		switch (k)
		{
			...
			case 'S':
			{
				...
				/* Prompt */
				prt("Command: Append options to a file", 18, 0);
...
void do_cmd_version(void)
{
	/* Silly message */
	msg_format
		("You are playing Oangband %d.%d.%d.  Type '?' for more info.",
