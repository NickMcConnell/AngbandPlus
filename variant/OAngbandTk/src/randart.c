...
static bool get_quality(bool on_credit, int purchase, int pval, int a_idx)
{
	...
	switch (purchase)
	{
		...
		case ADD_WIS:
		{
			...
			{
				if (take_money(on_credit,
						(150 * pval) + (pval * pval * 8)))
				{
...
static void initialize_artifact(int a_idx)
{
	...
	artifact_rarity =
		(((potential + power_of_base_object) / 200) / base_object_rarity);
	...
	switch (k_ptr->tval)
	{
		...
		case TV_HAFTED:
		{
			...
			else if (selection < 1000)
			{
				...
				get_quality(TRUE, ADD_AC, randint(6) + potential / 800,
					a_idx);
		...
		case TV_BOW:
		{
			...
			else
			{
				if ((potential > 4000) && (randint(2) == 1))
					...
					get_quality(TRUE, MIGHT1, 0, a_idx);
		...
		case TV_DRAG_ARMOR:
		{
			...
			else if (selection < 62)
			{
				...
				get_quality(TRUE, ADD_AC, randint(3) + potential / 1600,
					a_idx);
		...
		case TV_SHIELD:
		{
			...
			else if (selection < 85)
			{
				...
				/* Increase magical ac. */
				get_quality(TRUE, ADD_AC, randint(6) + potential / 2000,
					a_idx);
		...
		case TV_CROWN:
		{
			...
			else if (selection < 57)
			{
				...
				if (randint(3) == 1)
					get_quality(FALSE, IMPROVE_BASE_AC, randint(2) + 2,
						a_idx);
			}
			/* ...strong in battle. */
			else if (selection < 68)
...
static void haggle_till_done(int a_idx)
{
	...
	{
		if ((a_ptr->to_a % 5 == 4) && (randint(2) == 1))
			a_ptr->to_a++;
		else if ((a_ptr->to_a % 5 == 1) && (randint(2) == 1))
			a_ptr->to_a--;
		else if ((a_ptr->to_a % 2 == 1) && (randint(4) != 1))
			a_ptr->to_a++;

		if ((a_ptr->to_h % 5 == 4) && (randint(2) == 1))
			a_ptr->to_h++;
		else if ((a_ptr->to_h % 5 == 1) && (randint(2) == 1))
			a_ptr->to_h--;
		else if ((a_ptr->to_h % 2 == 1) && (randint(4) != 1))
			a_ptr->to_h++;

		if ((a_ptr->to_d % 5 == 4) && (randint(2) == 1))
			a_ptr->to_d++;
		else if ((a_ptr->to_d % 5 == 1) && (randint(2) == 1))
			a_ptr->to_d--;
		else if ((a_ptr->to_d % 2 == 1) && (randint(4) != 1))
			a_ptr->to_d++;
	}
}
...
static void make_terrible(int a_idx)
{
	...
	if ((a_ptr->activation) && (randint(3) != 1))
		a_ptr->activation = 0;
	...
	for (i = 0; i < gauntlet_runs; i++)
	{
		...
		else if ((a_ptr->to_a > 0) && (randint(2) == 1))
			wheel_of_doom = 1;
		else
			wheel_of_doom = 2 + randint(2);
...
static char *find_word(int a_idx)
{
	...
	if (!art_name[0])
	{
		...
		{
			msg_print
				("Cannot find any files with names or naming resources!");
