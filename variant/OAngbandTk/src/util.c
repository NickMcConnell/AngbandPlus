...
#ifdef INKEY_SKIP_MORE
			/* Hack -- inkey_skip_more */
			else if (*str == 'm')
			{
				*s++ = 29;
			}
#endif /* INKEY_SKIP_MORE */
...
void macro_delete(int n)
{
	...
	/* Clear the macro__use flag.  It is updated below. */
	if (macro__pat[n])
	{
		p0 = (byte)(macro__pat[n][0]);
		macro__use[p0] = FALSE;
	}

	/* Free the pattern and action */
	string_free(macro__pat[n]);
	string_free(macro__act[n]);

	if (n < macro__num - 1)
	{
		macro__pat[n] = macro__pat[macro__num - 1];
		macro__act[n] = macro__act[macro__num - 1];
	}
	else
	{
		macro__pat[n] = macro__act[n] = NULL;
	}
	...
	{
		if (macro__pat[n] && (byte)(macro__pat[n][0]) == p0)
		{
			macro__use[p0] = TRUE;
			break;
		}
...
#ifdef INKEY_SKIP_MORE
/*
 * \m in a keymap or macro action means "skip all subsequent -more- prompts"
 */
static bool inkey_skip_more = FALSE;
#endif /* INKEY_SKIP_MORE */
...
char inkey(void)
{
	...
	if (inkey_skip_more && !inkey_xtra)
	{
		if (inkey_flags == INKEY_MORE)
		{
		...
		inkey_skip_more = FALSE;
	}

	/* Hack -- Use the "inkey_next" pointer */
	while (inkey_next && *inkey_next && !inkey_xtra)
	{
...
static void msg_print_aux(u16b type, cptr msg)
{
	...
	/* This won't hurt performance as long as updating the Messages Window
	 * is done at idle time. */
...
#if 1 /* TNB */
		Bind_Ask(KEYWORD_ASK_QUANTITY + 1, 0, max, TRUE);

		/* Ask for a quantity */
		ok = get_string(prompt, buf, 6);

		Bind_Ask(KEYWORD_ASK_QUANTITY + 1, 0, 0, FALSE);
 ...
 */
#if 1 /* TNB */
bool get_check_aux(cptr prompt, bool enter_is_yes)
{
	int i;

	char buf[160]; /* Allow the full prompt (for stores) */

	/* Paranoia XXX XXX XXX */
	msg_print(NULL);

	/* Hack -- Build a "useful" prompt */
	strnfmt(buf, 160, "%s[y/n] ", prompt);

	/* Prompt for it */
	prompt_print(buf);

	Bind_Ask(KEYWORD_ASK_YES_NO + 1, 0, 0, TRUE);
	...
	Bind_Ask(KEYWORD_ASK_YES_NO + 1, 0, 0, FALSE);
		...
		if (cmd == '0')
		{
			...
			while (1)
			{
				...
				if ((cmd == 0x7F) || (cmd == KTRL('H')))
				{
					...
					prompt_update(format("%d", p_ptr->command_arg)); /* TNB */
				}
			...
			if (p_ptr->command_arg == 0)
			{
				...
				/* Show current count */
				prt(format("Count: %d", p_ptr->command_arg), 0, 0);
//              angtk_eval("angband_prompt", "update", format("%d", p_ptr->command_arg), NULL); /* TNB */
			}
...
byte get_angle_to_grid[41][41] = {
	{67, 66, 66, 66, 66, 66, 66, 65, 63, 62, 61, 60, 58, 57, 55, 54, 52,
			50, 48, 46, 45, 44, 41, 40, 38, 36, 35, 33, 32, 30, 29, 28, 27,
			25, 24, 24, 23, 23, 23, 23, 22}
	,
	{68, 67, 66, 66, 66, 66, 66, 65, 63, 62, 61, 60, 58, 57, 55, 54, 52,
			50, 49, 47, 45, 43, 41, 40, 38, 36, 35, 33, 32, 30, 29, 28, 27,
			25, 24, 24, 23, 23, 23, 22, 21}
	,
	{68, 68, 67, 66, 66, 66, 66, 65, 63, 62, 61, 60, 58, 57, 55, 54, 52,
