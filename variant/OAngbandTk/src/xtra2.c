...
bool set_extra_defences(int v)
{
	...
	redraw_add(PR_MAGICDEF); /* ALLOW_STATUS_EXTRA */
...
bool set_tim_esp(int v)
{
	...
	redraw_add(PR_ESP);	/* ALLOW_STATUS_EXTRA */
...
bool set_superstealth(int v)
{
	...
	/* Redraw status */
	p_ptr->redraw |= PR_STATUS;
	redraw_add(PR_STEALTH);	/* ALLOW_STATUS_EXTRA */
...
(player_exp[p_ptr->lev - 1] * p_ptr->expfact / 100L)))
	{
		...
		/* Sound */
#ifdef ALLOW_SOUND
		sound(SNDGRP_EVENT, SND_EXPER_LEVEL_UP, 0);
#else
		sound(SOUND_LEVEL);
#endif
...
void monster_death(int m_idx)
{
	...
	if ((r_ptr->flags1 & (RF1_DROP_CHEST)) &&
		(r_ptr->flags1 & (RF1_DROP_90))) number = 1;
...
bool mon_take_hit(int m_idx, int dam, bool * fear, cptr note)
{
	...
	if (m_ptr->hp < 0)
	{
		...
		{
			message_format(MSG_KILL, p_ptr->lev, "You have killed %s.",
				m_name);
		}
		...
		{
			message_format(MSG_KILL, p_ptr->lev, "You have slain %s.",
				m_name);
		}
...
bool change_panel(int dy, int dx)
{
	...
	/* Bounds */
	if (y > DUNGEON_HGT - hgt)
		y = DUNGEON_HGT - hgt;
	if (y < 0)
		y = 0;
	if (x > DUNGEON_WID - wid)
		x = DUNGEON_WID - wid;
	if (x < 0)
		x = 0;
...
void panel_center(void)
{
	...
	if (new_panel_row + hgt > DUNGEON_HGT)
		new_panel_row = DUNGEON_HGT - hgt;
	if (new_panel_col + wid > DUNGEON_WID)
		new_panel_col = DUNGEON_WID - wid;
	...
	/* Check for "no change" */
	if ((new_panel_row == panel_row_min) &&
		(new_panel_col == panel_col_min)) return;
 ...
 */
#if 1 /* TNB */
char *look_mon_desc(char *buf, int m_idx)
{
	monster_type *m_ptr = &m_list[m_idx];
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
...
void target_set_location(int y, int x)
{
	...
	target_vis = p_ptr->target_set && (((y == p_ptr->py) &&
			(x == p_ptr->px)) || projectable(p_ptr->py, p_ptr->px, y, x, 0));
#endif /* TNB */
}
	...
	while (1)
	{
				...
				while (1)
				{
					...
					else
					{
						...
						sprintf(out_val, "%s%s%s%s (%s) [r,%s]", s1, s2,
							s3, m_name, look_mon_desc(desc, cave_m_idx[y][x]),
							info);
		...
		if (scan_floor(floor_list, &floor_num, y, x, 0x02))
		{
			...
			while (1)
			{
				...
				if ((floor_num > 1) &&
					((rogue_like_commands ? (query == 'x') : (query ==
								'l')) || (query == ' ') || (query == '*')
						|| (query == '?')))

				{
					...
					prompt_erase();
					angtk_eval("angband_display", "floor", "hide", NULL);
		...
		if (p_ptr->target_who == 0)
		{
			...
			if (!floor_target_los ||
				projectable(p_ptr->py, p_ptr->px,
					p_ptr->target_row, p_ptr->target_col, 0))
			{
				dir = 5;
			}
		}
