...
s16b get_obj_num(int level)
{
	...
	if (level > 0)
	{
		...
		if (rand_int(GREAT_OBJ) == 0)
			level += randint(20) + randint(level / 2);
	}
		...
		case TV_SWORD:
		{
			if (o_ptr->ds > k_ptr->ds)
			{
				aware_cost +=
					...
					k_ptr->ds) * o_ptr->dd * o_ptr->dd * 16L;
			}
			break;
		}
	}
...
static s32b object_value_real(object_type * o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_RING:
		{
			 ...
			 */
			if (f1 & (TR1_STR))
				value += (((o_ptr->pval + 1) * (o_ptr->pval + 1)) * 40L);
			if (f1 & (TR1_INT))
				value += (((o_ptr->pval + 1) * (o_ptr->pval + 1)) * 40L);
			if (f1 & (TR1_WIS))
				value += (((o_ptr->pval + 1) * (o_ptr->pval + 1)) * 40L);
			 ...
			 */
			if (f1 & (TR1_SPEED))
				value += (o_ptr->pval * o_ptr->pval * 5000L);
	...
	switch (o_ptr->tval)
	{
		...
		case TV_POLEARM:
		{
			...
			else
			{
				 ...
				 */
				value +=
					(((o_ptr->to_h + 10) * (o_ptr->to_d + 10) -
				   100) * (k_ptr->cost + 1000L) / 250) +
					(o_ptr->to_a * 100);
			}
		...
		case TV_SHOT:
		{
			...
			if (object_known_p(o_ptr) != object_known_p(j_ptr))
			{
					...
					|| (o_ptr->tval == TV_SHOT))
				{
					...
					if (((sensed_p(o_ptr) && object_known_p(j_ptr)) ||
						(sensed_p(j_ptr) && object_known_p(o_ptr))) &&
					...
					if (((o_ptr->ident & (IDENT_SENSE) &&
							...
							(j_ptr->ident & (IDENT_SENSE) &&
								object_known_p(o_ptr))) && o_ptr->to_h == 0
						&& o_ptr->to_d == 0 && j_ptr->to_h == 0 &&
						j_ptr->to_d == 0)
	...
	bool great)
{
	...
	if (good || (rand_int(100) < good_percent))
	{
		...
		/* Roll for "great" */
		if (great || (rand_int(100) < great_percent))
			power = 2;
	}
	...
	if (o_ptr->name2)
	{
		 ...
		 */
		{
			if ((o_ptr->to_h % 2 == 1) && (randint(4) != 1))
				o_ptr->to_h++;
			if ((o_ptr->to_d % 2 == 1) && (randint(4) != 1))
				o_ptr->to_d++;
		}
	...
	switch (k_ptr->tval)
	{
		...
		case TV_AMULET:
		{
			if (k_ptr->sval == SV_AMULET_THE_MAGI)
				return (TRUE);
			if (k_ptr->sval == SV_AMULET_ESCAPING)
				return (TRUE);
			if (k_ptr->sval == SV_AMULET_LION)
				return (TRUE);
  ...
  try_again:
	...
	if ((rand_int(prob) != 0) || !make_artifact_special(j_ptr))
	{
		 ...
		 */
			...
			(k_ptr->cost < object_level - 20) && (randint(3) == 1))
			goto try_again;
...
bool make_gold(object_type * j_ptr)
{
	...
	j_ptr->pval =
		k_info[treasure].cost / 2 + randint((k_info[treasure].cost +
			1) / 2);
...
int process_quiver(int num_new, object_type * o_ptr)
{
	...
	for (i = INVEN_Q0; i <= INVEN_Q9; i++)
	{
		...
		if ((get_tag_num(i, &tag_num)) && (tag_num != i - INVEN_Q0))
		{
			...
			if (slot == i)
				slot = INVEN_Q0 + tag_num;
			else if (slot == INVEN_Q0 + tag_num)
				slot = i;
