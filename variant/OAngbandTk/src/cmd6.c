...
void do_cmd_eat_food(void)
{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	sound(SNDGRP_EVENT, SND_USE_FOOD, 0);
#else
	sound(SOUND_EAT);
#endif
	...
	switch (o_ptr->sval)
	{
		case SV_FOOD_POISON:
		{
			if (!p_ptr->resist_pois || !p_ptr->oppose_pois)
			{
				...
				{
					if (set_poisoned(p_ptr->poisoned + rand_int(10) + 5))
						ident = TRUE;
				}
			}
			break;
...
void do_cmd_quaff_potion(void)
{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	sound(SNDGRP_EVENT, SND_USE_POTION, 0);
#else
	sound(SOUND_QUAFF);
#endif
	...
	switch (o_ptr->sval)
	{
		...
		case SV_POTION_POISON:
		{
			if (!p_ptr->resist_pois || !p_ptr->oppose_pois)
			{
				if (!p_ptr->resist_pois && !p_ptr->oppose_pois)
				{
					if (set_poisoned(p_ptr->poisoned + rand_int(25) + 10))
						ident = TRUE;
				}
				else
				{
		...
		case SV_POTION_CURE_CRITICAL:
		{
			...
			if (set_confused(0))
				ident = TRUE;
			if (set_poisoned((p_ptr->poisoned / 2) - 10))
				ident = TRUE;
			if (set_stun(0))
				ident = TRUE;
			if (set_cut(p_ptr->cut - 50))
				ident = TRUE;
			break;
		}
		...
		case SV_POTION_HEALING:
		{
			...
			if (set_confused(0))
				ident = TRUE;
			if (set_poisoned(p_ptr->poisoned - 200))
				ident = TRUE;
			if (set_stun(0))
				ident = TRUE;
			if (set_cut(0))
...
void do_cmd_read_scroll(void)
{
	...
	sound(SNDGRP_EVENT, SND_USE_SCROLL, 0);	/* ALLOW_SOUND */
...
void do_cmd_use_staff(void)
{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	sound(SNDGRP_EVENT, SND_USE_STAFF, 0);
#else
	sound(SOUND_ZAP);
#endif
	...
	switch (o_ptr->sval)
	{
		...
		case SV_STAFF_MSTORM:
		{
			...
			fire_sphere(GF_MANA, 0, randint(75) + 125, 5, 20);
		...
		case SV_STAFF_STARBURST:
		{
			...
			fire_sphere(GF_LITE, 0, randint(67) + 100, 5, 20);
			ident = TRUE;
			break;
		}
		...
		(rand_int((p_ptr->pclass == CLASS_MAGE) ? 25 : 35) == 0))
	{
		...
		if (strlen(obj_special_info[3][o_ptr->sval]))
		{
			if (artifact_p(o_ptr))
				...
				msg_format("You feel you know more about staffs of %s.",
					k_name + k_ptr->name);
		}
	}
...
void do_cmd_aim_wand(void)
{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	/* Wands play the bolt or beam sound SND_USE_WAND is therefore UNUSED */
/*	sound(SNDGRP_EVENT, SND_USE_WAND, 0); */
#else
	sound(SOUND_ZAP);
#endif
	...
	switch (sval)
	{
		...
		{
			fire_bolt_or_beam(plev, GF_ACID, dir, damroll(5 + plev / 10,
					8));
			ident = TRUE;
			break;
		}
		...
		{
			fire_bolt_or_beam(plev, GF_ELEC, dir, damroll(3 + plev / 14,
					8));
			ident = TRUE;
			break;
		}
		...
		{
			fire_bolt_or_beam(plev, GF_FIRE, dir, damroll(6 + plev / 8,
					8));
			ident = TRUE;
			break;
		}
		...
		{
			fire_bolt_or_beam(plev, GF_COLD, dir, damroll(4 + plev / 12,
					8));
			ident = TRUE;
			break;
		}
		...
		{
			if (drain_life(dir, 100 + randint(plev * 4)))
				ident = TRUE;
			break;
		}
		...
		(rand_int((p_ptr->pclass == CLASS_MAGE) ? 30 : 40) == 0))
	{
		...
		if (strlen(obj_special_info[4][o_ptr->sval]))
		{
			if (artifact_p(o_ptr))
				...
				msg_format("You feel you know more about wands of %s.",
					k_name + k_ptr->name);
		}
	}
}
...
void do_cmd_zap_rod(void)
{
	...
	if (((o_ptr->sval >= SV_ROD_MIN_DIRECTION) &&
		...
		(o_ptr->sval == SV_ROD_DELVING) || !object_aware_p(o_ptr))
	{
	...
	/* Sound */
#ifdef ALLOW_SOUND
	sound(SNDGRP_EVENT, SND_USE_ROD, 0);
#else
	sound(SOUND_ZAP);
#endif
		...
		case SV_ROD_SHADOW:
		{
			if (p_ptr->pclass == CLASS_ROGUE)
			{
				if (p_ptr->superstealth)
					...
					(void) set_superstealth(p_ptr->superstealth + 75);
			}
			else
			{
				if (p_ptr->superstealth)
					...
					(void) set_superstealth(p_ptr->superstealth + 50);
			}
			ident = TRUE;
			break;
		}
		...
		(rand_int((p_ptr->pclass == CLASS_MAGE) ? 45 : 65) == 0))
	{
		...
		if (strlen(obj_special_info[5][o_ptr->sval]))
		{
			if (artifact_p(o_ptr))
				...
				msg_format("You feel you know more about rods of %s.",
					k_name + k_ptr->name);
		}
	}
}
...
void do_cmd_activate(void)
{
	...
	sound(SNDGRP_EVENT, SND_ACTIVATE, 0); /* ALLOW_SOUND */
	...
	switch (o_ptr->xtra2)
	{
		...
		{
			msg_print
				("You understand the structure of the dungeon around you.");
			map_area(0, 0, FALSE);
		...
		{
			msg_print
				("The bolt you have ready to hand gleams with deadly power.");
		...
		{
			msg_print
				("You feel a sphere of fire form between your hands.");
			if (!get_aim_dir(&dir))
				return;
		...
		{
			msg_print
				("Spells to dispel an undead antagonist surround you...");
			if (!get_aim_dir(&dir))
				return;
		...
		{
			msg_print
				("Spells to dispel a demonic adversary surround you...");
			if (!get_aim_dir(&dir))
				return;
		...
		{
			msg_print
				("You reveal yourself in wrath; your enemies tremble!");
		...
		{
			msg_print
				("A thrilling battle song awakes the warrior within you!");
			(void) hp_player(10);
			(void) set_afraid(0);
		...
		{
			msg_print
				("Quadricolored magics swirl around you protectingly.");
			(void) set_oppose_acid(p_ptr->oppose_acid + randint(20) + 20);
			(void) set_oppose_elec(p_ptr->oppose_elec + randint(20) + 20);
			(void) set_oppose_fire(p_ptr->oppose_fire + randint(20) + 20);
			(void) set_oppose_cold(p_ptr->oppose_cold + randint(20) + 20);
		...
		{
			msg_print
				("Penticolored magics swirl around you protectingly.");
			(void) set_oppose_acid(p_ptr->oppose_acid + randint(20) + 20);
			(void) set_oppose_elec(p_ptr->oppose_elec + randint(20) + 20);
			(void) set_oppose_fire(p_ptr->oppose_fire + randint(20) + 20);
			(void) set_oppose_cold(p_ptr->oppose_cold + randint(20) + 20);
		...
		{
			msg_print
				("Surrounded by darkness, you envoke light and beauty.");
		...
		{
			msg_print
				("A fine dust appears in your hand, and you throw it...");
			if (!get_aim_dir(&dir))
				return;
				...
				msg_format("You breathe %s.",
					((chance == 1) ? "lightning" : ((chance ==
								2) ? "frost" : ((chance ==
									3) ? "acid" : ((chance ==
										4) ? "poison gas" : "fire")))));
				fire_arc(((chance == 1) ? GF_ELEC : ((chance ==
								2) ? GF_COLD : ((chance ==
									3) ? GF_ACID : ((chance ==
		...
		case ACT_DRAGON_SHINING:
		{
			...
			else
			{
				...
				fire_arc((chance == 0 ? GF_LITE : GF_DARK), dir, 160, 10,
					40);
			}
			o_ptr->timeout = rand_int(300) + 300;
			break;
		...
		case ACT_DRAGON_LAW:
		{
			...
			else
			{
				...
				fire_arc((chance == 1 ? GF_SOUND : GF_SHARD), dir, 190, 10,
					40);
			}
			o_ptr->timeout = rand_int(300) + 300;
			break;
		...
		case ACT_DRAGON_GOLD:
		{
			...
			{
				msg_print
					("You become a small dragon with a deafening roar.");
				shapechange(SHAPE_WYRM);
			}
			else
			{
				...
				msg_format("You breathe %s.",
					((chance == 1) ? "chaos" : ((chance ==
								2) ? "disenchantment" : ((chance ==
									3) ? "sound" : "shards"))));
				fire_arc(((chance == 1) ? GF_CHAOS : ((chance ==
								2) ? GF_DISENCHANT : ((chance ==
									3) ? GF_SOUND : GF_SHARD))), dir, 210,
					10, 40);
			}
			o_ptr->timeout = rand_int(300) + 300;
			break;
