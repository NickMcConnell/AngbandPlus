...
#else /* BUG -- TNB */
			p_ptr->total_weight +=
				o_ptr->weight * (o_ptr->number - old_num);
#endif /* BUG -- TNB */
...
byte py_pickup(int pickup)
{
	...
	if (!pickup)
	{
		...
		if (floor_num == 1)
		{
			...
			/* Message */
			msg_format("You %s %s.", (p_ptr->blind ? "feel" : "see"),
				o_name);
		}
	...
	switch (cave_feat[y][x])
	{
		...
		case FEAT_TRAP_HEAD + 0x01:
		{
			...
			if ((nastyness > 80) && (randint(3) != 3))
			{
				...
				if (p_ptr->ffall)
				{
					...
					msg_print
						("You carefully avoid setting off the daggers.");
				}
		...
		case FEAT_TRAP_HEAD + 0x03:
		{
			...
			if (selection == 4)
			{
				if ((nastyness >= 50) && (randint(2) == 1))
				{
					...
					msg_print
						("A cauldron of acid is tipped over your head!");
					dam = damroll(6, 30);
		...
		case FEAT_TRAP_HEAD + 0x04:
		{
			...
			if (selection == 4)
			{
				...
				if (!p_ptr->free_act)
				{
					(void) set_paralyzed(p_ptr->paralyzed + rand_int(10) +
						5);
				}
				else
					msg_print("You are unaffected."); /* TNB */
		...
		case FEAT_TRAP_HEAD + 0x07:
		{
			...
			else if (nastyness < 75)
			{
				...
				msg_print
					("You feel yourself being twisted by wild magic!");
			...
			else if (nastyness < 90)
			{
				...
				fire_meteor(0, GF_CONFUSION, p_ptr->py, p_ptr->px, 75, 0,
					TRUE);
			}
...
#endif /* ALLOW_EASY_ALTER -- TNB */
{
	...
	else
	{
		...
		if (easy_alter && !jumping && (cave_info[y][x] & (CAVE_MARK)) &&
		...
		else
		{
			...
			switch (cave_feat[y][x])
			{
				...
				case FEAT_WATER:
				{
					...
					if (can_move == FALSE)
						msg_print
							("You dare not cross carrying so much weight.");
			...
			if (can_move)
			{
				 ...
				 */
#ifdef ALLOW_EASY_ALTER	/* TNB */
				(void) py_pickup(jumping != always_pickup);
#else /* ALLOW_EASY_ALTER -- TNB */
				(void) py_pickup(do_pickup);
#endif /* ALLOW_EASY_ALTER -- TNB */
...
static bool run_test(void)
{
	...
	if (disturb_trap_detect && p_ptr->dtrap_x && p_ptr->dtrap_y &&
		p_ptr->dtrap_rad)
	{
		if (distance(p_ptr->py, p_ptr->px, p_ptr->dtrap_y,
				p_ptr->dtrap_x) >= p_ptr->dtrap_rad)
		{
			p_ptr->dtrap_x = 0;
			p_ptr->dtrap_y = 0;
			p_ptr->dtrap_rad = 0;
