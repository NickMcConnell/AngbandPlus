...
errr process_pref_file_aux(char *buf)
{
	...
	else if (buf[0] == 'X')
	{
		for (i = 0; i < OPT_adult_start; i++)
		{
			...
			{
				op_ptr->opt[i] = FALSE;
				Bind_Option(option_text[i], 0); /* TNB */
				return (0);
			}
		}
	}
	...
	else if (buf[0] == 'Y')
	{
		for (i = 0; i < OPT_adult_start; i++)
		{
			...
			{
				op_ptr->opt[i] = TRUE;
				Bind_Option(option_text[i], 1); /* TNB */
				return (0);
			}
		}
	}
 ...
 */
/* static TNB */ cptr likert(int x, int y)
{
	/* Paranoia */
	if (y <= 0)
		y = 1;
...
static void display_player_various(void)
{
	...
	if (use_metric)
		put_str(format("%d meters", p_ptr->see_infra * 3), 20, 67);
	else
		put_str(format("%d feet", p_ptr->see_infra * 10), 20, 67);
}
...
void display_player(int mode)
{
	...
	if ((mode == 0) || (mode == 1))
	{
		...
		{
			prt_num("Height           ", (int) p_ptr->ht * 254 / 100, 3,
				27, TERM_L_BLUE);
			prt_num("Weight           ", (int) p_ptr->wt * 10 / 22, 4, 27,
				TERM_L_BLUE);
		}
		else
		{
			prt_num("Height           ", (int) p_ptr->ht, 3, 27,
				TERM_L_BLUE);
			prt_num("Weight           ", (int) p_ptr->wt, 4, 27,
				TERM_L_BLUE);
		}
...
bool show_file(cptr name, cptr what, int line, int mode)
{
	...
	while (TRUE)
	{
		...
		prt(format("[Oangband %d.%d.%d, %s, Line %d/%d]", O_VERSION_MAJOR,
				O_VERSION_MINOR, O_VERSION_PATCH, caption, line, size), 0,
			0);
		...
		else if (size <= hgt - 4)
		{
			/* Wait for it */
			prt
				("[Press ESC to return to the previous file, or ? to exit.]",
				hgt - 1, 0);
		}
		...
		else
		{
			/* Wait for it */
			prt
				("[Press Space to advance, ESC to return to the previous file, or ? to exit.]",
				hgt - 1, 0);
		}
		...
		if (menu && isdigit(k) && hook[D2I(k)][0])
		{
			...
			if (!show_file(hook[D2I(k)], NULL, 0, mode))
				k = '?';
		}
...
void do_cmd_suicide(void)
{
	...
	{
		/* Verify */
		if (!get_check("Do you really want to suicide? "))
			return;
...
void do_cmd_backup_game(void)
{
	...
	/* Save the player */
	if (!save_player_backup())
	{
		prompt_print("Saving game... failed!");
	}
...
static char *get_personalized_string(byte choice)
{
	...
	if (choice == 1)
		prt
			("Enter a message for your character's ghost above, or hit ESCAPE.",
			15, 8);
	else if (choice == 2)
		prt
			("Enter an addition to your character ghost's description, or hit ESCAPE.",
			15, 4);
	else
		return NULL;

	prt("(79 characters maximum.  Entry will be used as (a) sentence(s).)",
		16, 8);
	...
	if (!(p_ptr->noscore & 0x00FF))
	{
		...
		if (p_ptr->depth)
		{
			...
			while (1)
			{
				 ...
				 */
				prt
					("Information about your character has been saved in a bones file.",
					15, 7);
				...
				prt("ghost a special message or description? (y/n)", 16,
					30);
				...
				{
					prt
						("Will you add something for your ghost to say, or add",
						15, 13);
					prt
						("to the monster description? ((M)essage/(D)escription)",
						16, 13);
					...
					while (1)
					{
						...
						{
							prt
								("Please press 'm' to include a message, 'd' to include",
								15, 13);
							prt
								("a description, or the Escape key to cancel.",
								16, 18);
						}
					}
				}
...
static errr top_twenty(void)
{
	...
	sprintf(the_score.day, "%-.6s %-.2s", ctime(&death_time) + 4,
		ctime(&death_time) + 22);
#else
...
static void kingly(void)
{
	...
	sound(SNDGRP_EVENT, SND_CHARACTER_RETIRE, 0x02); /* ALLOW_SOUND */
	...
	for (k = from, place = k + 1; k < i; k += per_screen)
	{
		...
		for (j = k, n = 0; j < i && n < per_screen; place++, j++, n++)
		{
			...
			sprintf(out_val, "%3d.%9s  %s the %s %s, Level %d", place,
				the_score.pts, the_score.who, p_name + p_info[pr].name,
		...
		/* Wait for response */
		prt_center("[Press ESC to quit, any other key to continue.]",
			hgt - 1);
		j = inkey();
		prt("", hgt - 1, 0);
...
static errr enter_score(void)
{
	...
	/* Save the version */
	sprintf(the_score.what, "%u.%u.%u", O_VERSION_MAJOR, O_VERSION_MINOR,
		O_VERSION_PATCH);
...
static void close_game_aux(void)
{
	...
	/* Prompt */
	p =
		"['i' for info, 'f' to file, 't' for scores, 'x' to examine, or ESC]";
