...
static s32b price_item(object_type * o_ptr, int greed, bool flip)
{
	...
	racial_factor =
		g_info[(ot_ptr->owner_race * MAX_P_IDX) + p_ptr->prace];
...
static bool sell_haggle(object_type * o_ptr, s32b * price)
{
	...
	*price = 0;
	...
	if (o_ptr->discount == 80)
		msg_print
			("This item looks homemade.  I cannot offer much for it.");
...
static void store_inspect(void)
{
	...
	/* Examine the item. */
#if 1 /* TNB */
	do_cmd_observe(o_ptr, item, TRUE);
#else /* TNB */
	...
	if (artifact_p(o_ptr))
	{
		...
		if (!object_known_p(o_ptr))
		{
			if (cursed_p(o_ptr) || broken_p(o_ptr))
				o_ptr->feel = FEEL_TERRIBLE;
			else
				o_ptr->feel = FEEL_SPECIAL;
...
void do_cmd_store(void)
{
	...
	sound(SNDGRP_EVENT,
		(store_num == 7) ? SND_HOME_ENTER : SND_STORE_ENTER, 0x02);	/* ALLOW_SOUND */
	...
	sound(SNDGRP_EVENT, (store_num == STORE_HOME) ?
		SND_HOME_LEAVE : SND_STORE_LEAVE, 0x02);
