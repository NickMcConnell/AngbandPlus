...
void do_cmd_wield(void)
{
	...
	if ((slot < INVEN_Q0) && cursed_p(&inventory[slot]))
	{
		...
		sound(SNDGRP_EVENT, SND_IDENT_WORTHLESS, 0); /* ALLOW_SOUND */
	...
	if ((slot == INVEN_WIELD) && (inventory[INVEN_ARM].k_idx))
	{
		if (f3 & (TR3_TWO_HANDED_REQ) || (f3 & (TR3_TWO_HANDED_DES) &&
				(p_ptr->stat_ind[A_STR] < 25 + (o_ptr->weight / 50 >
						11 ? 11 : o_ptr->weight / 50))))
		{
	...
	if ((slot == INVEN_ARM) && (inventory[INVEN_WIELD].k_idx))
	{
		...
		if (f3 & (TR3_TWO_HANDED_REQ) || (f3 & (TR3_TWO_HANDED_DES) &&
				(p_ptr->stat_ind[A_STR] < 29 + (i_ptr->weight / 50 >
						8 ? 8 : i_ptr->weight / 50))))
		{
			p_ptr->shield_on_back = TRUE;
		}
	...
	/* Message */
	sound(SNDGRP_EVENT, SND_PACK_WEAR, 0); /* ALLOW_SOUND */
...
void do_cmd_destroy(void)
{
	...
	inkey_options = "verify_destroy verify_destroy_junk"; /* TNB */
	...
	/* Message */
	sound(SNDGRP_EVENT, (item >= 0) ? SND_PACK_DESTROY : SND_OBJECT_DESTROYED, 0); /* ALLOW_SOUND */
...
/*
 * If o_ptr is not NULL, then item is:
 *	< 0 floor
 *	>= 0 equipment or inventory (in_store 0)
 *	>= 0 store (in_store 1)
 */
void do_cmd_observe(object_type * o_ptr, int item, bool in_store)
{
	cptr q, s;
	...
	angtk_eval("angband_display", "displayinfo", "show", "object", q,
		format("%d", item), NULL);
...
void do_cmd_observe(object_type * o_ptr, bool in_store)
{
	 ...
	 */
	if ((in_store) && ((!k_ptr->known_effect) && ((o_ptr->tval < TV_SHOT)
...
static void do_cmd_refill_lamp(void)
{
	...
	/* Message */
	sound(SNDGRP_EVENT, SND_USE_FUEL, 0); /* ALLOW_SOUND */
...
void do_cmd_locate(void)
{
	...
	while (1)
	{
		...
		sprintf(out_val,
			...
			y2 / (hgt / 2), y2 % (hgt / 2), x2 / (wid / 2), x2 % (wid / 2),
			tmp_val);
...
void py_steal(int y, int x)
{
	...
	if (randint(theft_protection) < filching_power)
		success = TRUE;
...
void py_set_trap(int y, int x)
{
	...
	/* Notify the player. */
	sound(SNDGRP_EVENT, SND_MTRAP_SET, 0); /* ALLOW_SOUND */
