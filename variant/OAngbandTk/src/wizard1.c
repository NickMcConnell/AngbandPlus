	...
	s32b * val, int k)
{
	...
	/* Weight */
	if (use_metric)
		sprintf(wgt, "%3d.%d", make_metric(i_ptr->weight) / 10,
...
typedef struct
{
	...
	cptr misc_magic[N_ELEMENTS(misc_flags3_desc) +
		1 /* Permanent Light */  + 1 /* type of curse */  + 1];	/* sentinel NULL */
...
static cptr *spoiler_flag_aux(const u32b art_flags,
	const flag_desc * flag_x_ptr, cptr * desc_x_ptr, const int n_elmnts)
{
	int i;
...
static void analyze_pval(object_type * o_ptr, pval_info_type * pval_x_ptr)
{
	...
	/* Create the "+N" string */
	sprintf(pval_x_ptr->pval_desc, "%s%d", POSITIZE(o_ptr->pval),
		o_ptr->pval);
...
static void analyze_launcher(object_type * o_ptr, cptr * launcher_list)
{
	...
	launcher_list =
		spoiler_flag_aux(f1, launcher_flags_desc, launcher_list,
...
static void analyze_misc(object_type * o_ptr, char *misc_desc)
{
	...
	if (use_metric)
		sprintf(misc_desc, "Level %u, Rarity %u, %d.%d kgs, " "%ld Gold",
			a_ptr->level, a_ptr->rarity, make_metric(a_ptr->weight) / 10,
...
static void spoil_obj_gen(cptr fname)
{
	...
	fprintf(fff,
		"Object Generation Spoiler for Oangband Version %d.%d.%d\n",
		O_VERSION_MAJOR, O_VERSION_MINOR, O_VERSION_PATCH);
...
static void spoil_mon_gen(cptr fname)
{
	...
	fprintf(fff,
		"Monster Generation Spoiler for Oangband Version %d.%d.%d\n",
		O_VERSION_MAJOR, O_VERSION_MINOR, O_VERSION_PATCH);
	...
	prt
		("Monster generation uses a three-level table:  The base probability, ",
		5, 5);
	prt
		("gotten from r_info.txt, the modified probability as adjusted by , ",
		6, 5);
	prt
		("special monster restrictions (like only dragons), and the final , ",
		7, 5);
	prt
		("probability, which depends on values in the first two levels plus , ",
		8, 5);
	prt("the current generation depth.  , ", 9, 5);

	prt
		("The quick monster generator builds the third probability level only ,",
		11, 5);
	prt
		("once, and then draws monsters from it.  The standard method is to , ",
		12, 5);
