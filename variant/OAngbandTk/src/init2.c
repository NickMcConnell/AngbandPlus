...
static errr init_h_info(void)
{
	...
	/* Parse it */
	if (!fp)
		quit("Cannot open 'h_info.txt' file.");
	...
	if (err)
	{
		...
		/* Oops */
		msg_format("Error %d at line %d of 'h_info.txt'.", err,
			error_line);
		msg_format("Record %d contains a '%s' error.", error_idx, oops);
		msg_format("Parsing '%s'.", buf);
		msg_print(NULL);
	...
	/* Process existing "raw" file */
	if (fd < 0)
		quit("Cannot load 'h_info.raw' file.");
	...
	/* Error */
	if (err)
		quit("Cannot parse 'h_info.raw' file.");
...
static errr init_b_info(void)
{
	...
	/* Parse it */
	if (!fp)
		quit("Cannot open 'b_info.txt' file.");
	...
	if (err)
	{
		...
		/* Oops */
		msg_format("Error %d at line %d of 'b_info.txt'.", err,
			error_line);
		msg_format("Record %d contains a '%s' error.", error_idx, oops);
		msg_format("Parsing '%s'.", buf);
		msg_print(NULL);
	...
	/* Process existing "raw" file */
	if (fd < 0)
		quit("Cannot load 'b_info.raw' file.");
	...
	/* Error */
	if (err)
		quit("Cannot parse 'b_info.raw' file.");
...
static errr init_g_info(void)
{
	...
	/* Parse it */
	if (!fp)
		quit("Cannot open 'g_info.txt' file.");
	...
	if (err)
	{
		...
		/* Oops */
		msg_format("Error %d at line %d of 'g_info.txt'.", err,
			error_line);
		msg_format("Record %d contains a '%s' error.", error_idx, oops);
		msg_format("Parsing '%s'.", buf);
		msg_print(NULL);
	...
	/* Process existing "raw" file */
	if (fd < 0)
		quit("Cannot load 'g_info.raw' file.");
	...
	/* Error */
	if (err)
		quit("Cannot parse 'g_info.raw' file.");
...
void init_artifacts(void)
{
	...
	for (loop = 0; loop <= 1; loop++)
	{
		...
		for (a_idx = 1; a_idx < MAX_A_IDX; a_idx++)
		{
			...
			if ((a_idx < ART_MIN_NORMAL) || (a_ptr->tval == TV_ROD) ||
	...
	dbwin
		("init_artifacts: artifact_normal_cnt is %d, artifact_special_cnt is %d\n",
		artifact_normal_cnt, artifact_special_cnt);
	if (1)
	{
		int i;
		for (i = 0; i < artifact_normal_cnt; i++)
		{
			...
			make_fake_artifact(&ot, artifact_normal[i]);
			object_aware(&ot);
			object_known(&ot);
			object_desc(o_name, &ot, TRUE, 3);
			dbwin("NORMAL: %s\n", o_name);
		}
		...
		for (i = 0; i < artifact_special_cnt; i++)
		{
			...
			make_fake_artifact(&ot, artifact_special[i]);
			object_aware(&ot);
			object_known(&ot);
			object_desc(o_name, &ot, TRUE, 3);
			dbwin("SPECIAL: %s\n", o_name);
...
static errr init_p_info(void)
{
	...
	/* Parse it */
	if (!fp)
		quit("Cannot open 'p_info.txt' file.");
	...
	if (err)
	{
		...
		/* Oops */
		msg_format("Error %d at line %d of 'p_info.txt'.", err,
			error_line);
		msg_format("Record %d contains a '%s' error.", error_idx, oops);
		msg_format("Parsing '%s'.", buf);
		msg_print(NULL);
	...
	/* Process existing "raw" file */
	if (fd < 0)
		quit("Cannot open 'p_info.raw' file.");
	...
	/* Error */
	if (err)
		quit("Cannot parse 'p_info.raw' file.");
...
errr init_t_info(byte chosen_level)
{
	...
	if (err)
	{
		...
		msg_format("Error %d at line %d of 't_info.txt'.", err,
			error_line);
		msg_format("Record %d contains a '%s' error.", error_idx, oops);
		msg_format("Parsing '%s'.", buf);
		msg_print(NULL);
	...
	for (i = 1; i < MAX_K_IDX; i++)
	{
		...
		for (j = 0; j < 4; j++)
		{
			...
			if (k_ptr->chance[j])
			{
				...
				if (k_ptr->flags3 & TR3_INSTA_ART) /* BUG -- TNB */
					quit_fmt("k_info %d INSTA_ART should not have A: line",
						i);
			}
		}
	}
