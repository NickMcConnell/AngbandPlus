...
static void prt_shape(void)
{
	...
	Bind_Status(KEYWORD_STATUS_SHAPE + 1);
...
static void prt_depth(void)
{
	...
	{
		if (use_metric)
			sprintf(depths, "%d m", p_ptr->depth * 15);
		else
	...
	c_prt(TERM_L_BLUE, format("%7s", depths), Term->hgt - 1,
		Term->wid - 10);
...
#if 1 /* TNB */
		/*
		 * This "fix" is needed because update_view() does not recognize
		 * changes in light radius. It isn't a problem when the grids
		 * surrounding the character are not marked.
		 */
		int y, x, r = MAX(p_ptr->old_lite, p_ptr->cur_lite);
...
sint add_special_melee_skill(byte pclass, s16b weight, object_type * o_ptr)
{
	...
	{
		if ((rp_ptr->flags_special) & PS_SWORD_SKILL)
			add_skill += 3 + p_ptr->lev / 7;
		else if ((rp_ptr->flags_special) & PS_SWORD_UNSKILL)
			add_skill -= 3 + p_ptr->lev / 7;
	}

	else if (o_ptr->tval == TV_POLEARM)
	{
		if ((rp_ptr->flags_special) & PS_POLEARM_SKILL)
			add_skill += 3 + p_ptr->lev / 7;
		else if ((rp_ptr->flags_special) & PS_POLEARM_UNSKILL)
			add_skill -= 3 + p_ptr->lev / 7;
	}

	else if (o_ptr->tval == TV_HAFTED)
	{
		if ((rp_ptr->flags_special) & PS_HAFTED_SKILL)
			add_skill += 3 + p_ptr->lev / 7;
		else if ((rp_ptr->flags_special) & PS_HAFTED_UNSKILL)
			add_skill -= 3 + p_ptr->lev / 7;
	}
...
sint add_special_missile_skill(byte pclass, s16b weight,
	object_type * o_ptr)
{
	int add_skill = 0;
	...
	if (p_ptr->ammo_tval == TV_BOLT)
	{
		if ((rp_ptr->flags_special) & PS_XBOW_SKILL)
			add_skill += 3 + p_ptr->lev / 7;
		else if ((rp_ptr->flags_special) & PS_XBOW_UNSKILL)
			add_skill -= 3 + p_ptr->lev / 7;
	}
	else if (p_ptr->ammo_tval == TV_ARROW)
	{
		if ((rp_ptr->flags_special) & PS_BOW_SKILL)
			add_skill += 3 + p_ptr->lev / 7;
		else if ((rp_ptr->flags_special) & PS_BOW_UNSKILL)
			add_skill -= 3 + p_ptr->lev / 7;
	}
	else if (p_ptr->ammo_tval == TV_SHOT)
	{
		if ((rp_ptr->flags_special) & PS_SLING_SKILL)
			add_skill += 3 + p_ptr->lev / 7;
		else if ((rp_ptr->flags_special) & PS_SLING_UNSKILL)
			add_skill -= 3 + p_ptr->lev / 7;
	}
	return (add_skill);
}
...
static void calc_bonuses(void)
{
  ...
  calc_again:
	 ...
	 */
	float_simulator = 100;
	if (p_ptr->skill_stl % 3 == 1)
		float_simulator = 126;
	if (p_ptr->skill_stl % 3 == 2)
		float_simulator = 159;

	p_ptr->base_wakeup_chance =
		85000L * extract_energy[p_ptr->pspeed] /
		((1L << (p_ptr->skill_stl / 3)) * float_simulator);
	...
	if (o_ptr->k_idx && !p_ptr->heavy_wield)
	{
		...
		str_index =
			mul * adj_str_blow[p_ptr->stat_ind[A_STR]] / effective_weight;
...
void redraw_stuff(void)
{
	...
	if (p_ptr->redraw & (PR_TARGET))
	{
		...
		if (change || (((target_state & TARGET_STATE_SET) != 0)
			!= (p_ptr->target_set != 0)))
		{
			Bind_Generic(EVENT_TARGET, (p_ptr->target_set
		...
		if (p_ptr->target_set != 0)
			target_state |= TARGET_STATE_SET;
		if (target_vis != 0)
			target_state |= TARGET_STATE_VIS;
	}
