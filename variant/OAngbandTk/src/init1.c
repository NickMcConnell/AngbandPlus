...
int r_info_flags_max = 7;
#endif /* TNB */

/*
 * Special Player Race Flags
 */
/* static TNB */ cptr p_info_flags_sp[] = {
	"SWORD_SKILL",
	"POLEARM_SKILL",
	"HAFTED_SKILL",
	"SLING_SKILL",
...
errr init_v_info_txt(FILE * fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'X')
		{
			...
			/* Scan for the values */
			if (6 != sscanf(buf + 2, "%d:%d:%d:%d:%d:%d", &typ, &rat, &hgt,
					&wid, &min_lev, &max_lev))
				return (1);
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'N')
		{
			...
			*s++ = '\0';
			...
			/* Hack -- Verify space */
			if (t_head->name_size + strlen(s) + 8 > FAKE_NAME_SIZE)
				return (7);

			/* Advance and Save the name index */
			if (!t_ptr->name)
				t_ptr->name = ++t_head->name_size;
		...
		if (buf[0] == 'D')
		{
			...
			/* Hack -- Verify space */
			if (t_head->text_size + strlen(s) + 8 > FAKE_TEXT_SIZE)
				return (7);

			/* Advance and Save the text index */
			if (!t_ptr->text)
				t_ptr->text = ++t_head->text_size;
...
errr init_a_info_txt(FILE * fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'P')
		{
			...
			/* Scan for the values */
			if (7 != sscanf(buf + 2, "%d:%dd%d:%d:%d:%d:%d", &ac, &hd1,
					&hd2, &th, &td, &ta, &act))
				return (1);
		...
		if (buf[0] == 'D')
		{
			...
			else
			{
				...
				if ((t[-1] != ' ') && (t[-1] != '-'))
				{
					t[0] = ' ';
					r_head->text_size++;
/* dbwin("monster %d description is screwed\n", error_idx); */
				}
			}
#endif /* TNB */
		...
		if (buf[0] == 'W')
		{
			...
			/* Scan for the values */
			if (4 != sscanf(buf + 2, "%d:%d:%d:%ld", &lev, &rar, &mana,
					&exp)) return (1);
...
errr init_p_info_txt(FILE * fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'V')
		{
			...
			/* Scan for the values */
			if ((3 != sscanf(buf + 2, "%d.%d.%d", &v1, &v2, &v3)) ||
				(v1 != p_head->v_major) || (v2 != p_head->v_minor) ||
				(v3 != p_head->v_patch))
			{
		...
		if (buf[0] == 'R')
		{
			...
			/* Scan for the values */
			if (8 != sscanf(buf + 2, "%d:%d:%d:%d:%d:%d:%d:%d", &dis, &dev,
					&sav, &stl, &srh, &fos, &thn, &thb))
				return (PARSE_ERROR_GENERIC);
		...
		if (buf[0] == 'M')
		{
			...
			/* Scan for the values */
			if (8 != sscanf(buf + 2, "%d:%d:%d:%d:%d:%d:%d:%d", &xdis,
					&xdev, &xsav, &xstl, &xsrh, &xfos, &xthn, &xthb))
				return (PARSE_ERROR_GENERIC);
		...
		if (buf[0] == 'X')
		{
			...
			/* Scan for the values */
			if (3 != sscanf(buf + 2, "%d:%d:%d", &mhp, &exp, &infra))
				return (PARSE_ERROR_GENERIC);
		...
		if (buf[0] == 'H')
		{
			...
			/* Scan for the values */
			if (4 != sscanf(buf + 2, "%d:%d:%d:%d", &m_b_ht, &m_m_ht,
					&f_b_ht, &f_m_ht))
				return (PARSE_ERROR_GENERIC);
		...
		if (buf[0] == 'W')
		{
			...
			/* Scan for the values */
			if (4 != sscanf(buf + 2, "%d:%d:%d:%d", &m_b_wt, &m_m_wt,
					&f_b_wt, &f_m_wt))
				return (PARSE_ERROR_GENERIC);
...
errr init_h_info_txt(FILE * fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'V')
		{
			...
			/* Scan for the values */
			if ((3 != sscanf(buf, "V:%d.%d.%d", &v1, &v2, &v3)) ||
				(v1 != h_head->v_major) || (v2 != h_head->v_minor) ||
				(v3 != h_head->v_patch))
			{
				return (PARSE_ERROR_OBSOLETE_FILE);
			}
...
errr init_b_info_txt(FILE * fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'V')
		{
			...
			/* Scan for the values */
			if ((3 != sscanf(buf, "V:%d.%d.%d", &v1, &v2, &v3)) ||
				(v1 != b_head->v_major) || (v2 != b_head->v_minor) ||
				(v3 != b_head->v_patch))
			{
				return (PARSE_ERROR_OBSOLETE_FILE);
			}
		...
		if (buf[0] == 'N')
		{
			...
			*t++ = '\0';
			...
			if (!ot_ptr->owner_name)
				ot_ptr->owner_name = ++b_head->name_size;
		...
		if (buf[0] == 'I')
		{
			...
			/* Scan for the values */
			if (6 != sscanf(buf + 2, "%d:%d:%d:%d:%d:%d", &idx, &gld, &max,
					&min, &hgl, &tol))
				return (PARSE_ERROR_GENERIC);
...
errr init_g_info_txt(FILE * fp, char *buf)
{
	...
	while (0 == my_fgets(fp, buf, 1024))
	{
		...
		if (buf[0] == 'V')
		{
			...
			/* Scan for the values */
			if ((3 != sscanf(buf, "V:%d.%d.%d", &v1, &v2, &v3)) ||
				(v1 != g_head->v_major) || (v2 != g_head->v_minor) ||
				(v3 != g_head->v_patch))
			{
				return (PARSE_ERROR_OBSOLETE_FILE);
			}
