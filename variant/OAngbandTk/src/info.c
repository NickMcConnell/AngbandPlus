...
cptr obj_class_info[101] = {
	...
	"Experienced adventurers wrap a cloak around their body.  Some rare items of this type may allow you to slip silently around less alert enemies.",
		...
		"Some kind of body protection will become a necessity as you face ever more dangerous opponents; rare items of this type may hold many and varied protective magics.",
		"Armour made of dragon scales is rare indeed, and powerful dragon magics allow you to sometimes breathe even as great wyrms do.",
...
cptr item_activation(object_type * o_ptr)
{
	...
	switch (o_ptr->xtra2)
	{
		...
		{
			return
				"Assume Dragonform; Activation in Dragonform: star ball (150) every 1000 turns";
		}
		case ACT_BLADETURNER:
		{
			return
				"Assume Dragonform; Activation in Dragonform: heroism, bless, and resistance every 400 turns";
		}
		case ACT_SOULKEEPER:
		{
		...
		{
			return
				"assume dragon form; Activation in dragon form: breathe lightning (130) every 350+d350 turns";
		}
		case ACT_DRAGON_WHITE:
		{
			return
				"assume dragon form; Activation in dragon form: breathe frost (140) every 350+d350 turns";
		}
		case ACT_DRAGON_BLACK:
		{
			return
				"assume dragon form; Activation in dragon form: breathe acid (150) every 350+d350 turns";
		}
		case ACT_DRAGON_GREEN:
		{
			return
				"assume dragon form; Activation in dragon form: breathe poison gas (150) every 350+d350 turns";
		}
		case ACT_DRAGON_RED:
		{
			return
				"assume dragon form; Activation in dragon form: breathe fire (160) every 350+d350 turns";
		}
		case ACT_DRAGON_MULTIHUED:
		{
			return
				"assume dragon form; Activation in dragon form: breathe an element or poison (190) every 350+d350 turns";
		}
		case ACT_DRAGON_BRONZE:
		{
			return
				"assume dragon form; Activation in dragon form: breathe confusion (130) every 300+d300 turns";
		}
		case ACT_DRAGON_GOLD:
		{
			return
				"assume dragon form; Activation in dragon form: breathe sound (130) every 300+d300 turns";
		}
		case ACT_DRAGON_CHAOS:
		{
			return
				"assume dragon form; Activation in dragon form: breathe chaos/disenchant (180) every 300+d300 turns";
		}
		case ACT_DRAGON_LAW:
		{
			return
				"assume dragon form; Activation in dragon form: breathe sound/shards (190) every 300+d300 turns";
		}
		case ACT_DRAGON_BALANCE:
		{
			return
				"assume dragon form; Activation in dragon form: breathe balance (210) every 300+d300 turns";
		}
		case ACT_DRAGON_SHINING:
		{
			return
				"assume dragon form; Activation in dragon form: breathe light/darkness (160) every 300+d300 turns";
		}
		case ACT_DRAGON_POWER:
		{
			return
				"assume dragon form; Activation in dragon form: breathe the elements (240) every 300+d300 turns";
		}

		case ACT_RING_ACID:
		{
			return
				"cast a acid ball(80) and oppose acid every 50+d100 turns";
		}
		case ACT_RING_COLD:
		{
			return
				"cast a cold ball(80) and oppose cold every 50+d100 turns";
		}
		case ACT_RING_FIRE:
		{
			return
				"cast a fire ball(80) and oppose fire every 50+d100 turns";
		}
		case ACT_RING_ELEC:
		{
			return
				"cast a electricity ball(80) and oppose electricity every 50+d100 turns";
		}
		...
		{
			return
				"lash out at at range 2 for base weapon damage (unless target resists fire)";
		}
		default:
		{
...
void identify_fully_aux(object_type * o_ptr)
{
	...
	if (o_ptr->tval == TV_LITE)
	{
		if (artifact_p(o_ptr))
		{
			if (o_ptr->name1 != ART_STONE_LORE)	/* TNB */
	...
	if (f1 & (TR1_THROWING))
	{
		if (f1 & (TR1_PERFECT_BALANCE))
		{
			...
			info[i++] = "It can be thrown effectively.";
	}
...
void identify_fully_aux(object_type * o_ptr)
{
			...
			(f1 & (TR1_CHR))))
	{
		...
		{
			if (o_ptr->pval > 0)
				roff("It increases all your stats", 3, 77);
			else
				roff("It decreases all your stats", 3, 77);
		}
		else
		{
			if (o_ptr->pval > 0)
				roff("It increases your", 3, 77);
			else
				roff("It decreases your", 3, 77);
			...
			for (j = 0; j < 6; j++)
			{
				...
				if (j == 0)
					roff(" strength", 3, 77);
				if (j == 1)
					roff(" intelligence", 3, 77);
				if (j == 2)
					roff(" wisdom", 3, 77);
				if (j == 3)
					roff(" dexterity", 3, 77);
				if (j == 4)
					roff(" constitution", 3, 77);
				if (j == 5)
					roff(" charisma", 3, 77);
			}
		}
		...
		(f2 & (TR2_SUST_CON)) || (f2 & (TR2_SUST_CHR)))
	{
		...
		else
		{
			...
			for (j = 0; j < 6; j++)
			{
				...
				if ((j == 0) && (f2 & (TR2_SUST_STR)))
					list_ok = TRUE;
				if ((j == 1) && (f2 & (TR2_SUST_INT)))
					list_ok = TRUE;
				if ((j == 2) && (f2 & (TR2_SUST_WIS)))
					list_ok = TRUE;
				if ((j == 3) && (f2 & (TR2_SUST_DEX)))
					list_ok = TRUE;
				if ((j == 4) && (f2 & (TR2_SUST_CON)))
					list_ok = TRUE;
				if ((j == 5) && (f2 & (TR2_SUST_CHR)))
					list_ok = TRUE;
	...
	if ((o_ptr->pval) && ((f1 & (TR1_MAGIC_MASTERY)) ||
			(f1 & (TR1_STEALTH)) || (f1 & (TR1_SEARCH)) ||
			(f1 & (TR1_INFRA)) || (f1 & (TR1_TUNNEL)) ||
			(f1 & (TR1_SPEED))))
	{
		/* Clear number of items to list, and items listed. */
		attr_num = 0;
		...
		for (j = 0; j < 6; j++)
		{
			...
			if ((j == 0) && (f1 & (TR1_MAGIC_MASTERY)))
				list_ok = TRUE;
			if ((j == 1) && (f1 & (TR1_STEALTH)))
				list_ok = TRUE;
			if ((j == 2) && (f1 & (TR1_SEARCH)))
				list_ok = TRUE;
			if ((j == 3) && (f1 & (TR1_INFRA)))
				list_ok = TRUE;
			if ((j == 4) && (f1 & (TR1_TUNNEL)))
				list_ok = TRUE;
			if ((j == 5) && (f1 & (TR1_SPEED)))
				list_ok = TRUE;
			...
			/* List the attribute description, in its proper place. */
			if (j == 0)
				roff(" magical device skill", 3, 77);
			if (j == 1)
				roff(" stealth", 3, 77);
			if (j == 2)
				roff(" searching", 3, 77);
			if (j == 3)
				roff(" infravision", 3, 77);
			if (j == 4)
				roff(" tunnelling ability", 3, 77);
			if (j == 5)
				roff(" speed", 3, 77);
		}
	...
	if (o_ptr->tval == TV_BOW)
	{
		...
		{
			roff
				("It affects your shooting speed, and greatly affects your shooting power. \n",
				3, 77);
		}
		...
		(f1 & (TR1_SLAY_GIANT)) || (f1 & (TR1_SLAY_DRAGON)))
	{
		...
		for (j = 0; j < 8; j++)
		{
			...
			if ((j == 0) && (f1 & (TR1_SLAY_ANIMAL)))
				list_ok = TRUE;
			if ((j == 1) && (f1 & (TR1_SLAY_EVIL)))
				list_ok = TRUE;
			if ((j == 2) && (f1 & (TR1_SLAY_UNDEAD)))
				list_ok = TRUE;
			if ((j == 3) && (f1 & (TR1_SLAY_DEMON)))
				list_ok = TRUE;
			if ((j == 4) && (f1 & (TR1_SLAY_ORC)))
				list_ok = TRUE;
			if ((j == 5) && (f1 & (TR1_SLAY_TROLL)))
				list_ok = TRUE;
			if ((j == 6) && (f1 & (TR1_SLAY_GIANT)))
				list_ok = TRUE;
			if ((j == 7) && (f1 & (TR1_SLAY_DRAGON)))
				list_ok = TRUE;
		...
		(f1 & (TR1_BRAND_POIS)))
	{
		...
		for (j = 0; j < 5; j++)
		{
			...
			if ((j == 0) && (f1 & (TR1_BRAND_ACID)))
				list_ok = TRUE;
			if ((j == 1) && (f1 & (TR1_BRAND_ELEC)))
				list_ok = TRUE;
			if ((j == 2) && (f1 & (TR1_BRAND_FIRE)))
				list_ok = TRUE;
			if ((j == 3) && (f1 & (TR1_BRAND_COLD)))
				list_ok = TRUE;
			if ((j == 4) && (f1 & (TR1_BRAND_POIS)))
				list_ok = TRUE;
		...
		(f2 & (TR2_IM_FIRE)) || (f2 & (TR2_IM_COLD)))
	{
		...
		for (j = 0; j < 4; j++)
		{
			...
			if ((j == 0) && (f2 & (TR2_IM_ACID)))
				list_ok = TRUE;
			if ((j == 1) && (f2 & (TR2_IM_ELEC)))
				list_ok = TRUE;
			if ((j == 2) && (f2 & (TR2_IM_FIRE)))
				list_ok = TRUE;
			if ((j == 3) && (f2 & (TR2_IM_COLD)))
				list_ok = TRUE;
		...
		(f2 & (TR2_RES_DISEN)))
	{
		...
		for (j = 0; j < 13; j++)
		{
			...
			if ((j == 0) && (f2 & (TR2_RES_ACID)))
				list_ok = TRUE;
			if ((j == 1) && (f2 & (TR2_RES_ELEC)))
				list_ok = TRUE;
			if ((j == 2) && (f2 & (TR2_RES_FIRE)))
				list_ok = TRUE;
			if ((j == 3) && (f2 & (TR2_RES_COLD)))
				list_ok = TRUE;
			if ((j == 4) && (f2 & (TR2_RES_POIS)))
				list_ok = TRUE;
			if ((j == 5) && (f2 & (TR2_RES_LITE)))
				list_ok = TRUE;
			if ((j == 6) && (f2 & (TR2_RES_DARK)))
				list_ok = TRUE;
			if ((j == 7) && (f2 & (TR2_RES_SOUND)))
				list_ok = TRUE;
			if ((j == 8) && (f2 & (TR2_RES_SHARD)))
				list_ok = TRUE;
			if ((j == 9) && (f2 & (TR2_RES_NEXUS)))
				list_ok = TRUE;
			if ((j == 10) && (f2 & (TR2_RES_NETHR)))
				list_ok = TRUE;
			if ((j == 11) && (f2 & (TR2_RES_CHAOS)))
				list_ok = TRUE;
			if ((j == 12) && (f2 & (TR2_RES_DISEN)))
				list_ok = TRUE;
	...
	if (f2 & (TR2_RES_BLIND))
	{
		if ((attr_num > 1) && (f2 & (TR2_RES_FEAR)))
			...
			roff("It provides resistance to blindness", 3, 77);
	...
	if (f2 & (TR2_RES_CONFU))
	{
		if ((attr_num > 1) && (!(f2 & (TR2_RES_BLIND))))
			...
			roff("It provides resistance to confusion.\n", 3, 77);
	}


	/* Miscellanious abilities. */
	if ((f3 & (TR3_SLOW_DIGEST)) || (f3 & (TR3_FEATHER)) ||
		(f3 & (TR3_LITE)) || (f3 & (TR3_REGEN)) || (f3 & (TR3_TELEPATHY))
		|| (f3 & (TR3_SEE_INVIS)) || (f3 & (TR3_FREE_ACT)) ||
		(f3 & (TR3_HOLD_LIFE)) || (f3 & (TR3_IMPACT)) ||
		(f3 & (TR3_BLESSED)))
	{
		/* Clear number of items to list, and items listed. */
		attr_num = 0;
		...
		for (j = 0; j < 10; j++)
		{
			...
			if ((j == 0) && (f3 & (TR3_SLOW_DIGEST)))
				list_ok = TRUE;
			if ((j == 1) && (f3 & (TR3_FEATHER)))
				list_ok = TRUE;
			if ((j == 2) && (f3 & (TR3_LITE)))
				list_ok = TRUE;
			if ((j == 3) && (f3 & (TR3_REGEN)))
				list_ok = TRUE;
			if ((j == 4) && (f3 & (TR3_TELEPATHY)))
				list_ok = TRUE;
			if ((j == 5) && (f3 & (TR3_SEE_INVIS)))
				list_ok = TRUE;
			if ((j == 6) && (f3 & (TR3_FREE_ACT)))
				list_ok = TRUE;
			if ((j == 7) && (f3 & (TR3_HOLD_LIFE)))
				list_ok = TRUE;
			if ((j == 8) && (f3 & (TR3_IMPACT)))
				list_ok = TRUE;
			if ((j == 9) && (f3 & (TR3_BLESSED)))
				list_ok = TRUE;
			...
			/* List the attribute description, in its proper place. */
			if (j == 0)
				roff(" slows your metabolism", 3, 77);
			if (j == 1)
				roff(" induces feather falling", 3, 77);
			if (j == 2)
				roff(" provides permanent light", 3, 77);
			if (j == 3)
				roff(" speeds your regenerative powers", 3, 77);
			if (j == 4)
				roff(" gives telepathic powers", 3, 77);
			if (j == 5)
				roff(" allows you to see invisible monsters", 3, 77);
			if (j == 6)
				roff(" provides immunity to paralysis", 3, 77);
			if (j == 7)
				roff(" provides resistance to life draining", 3, 77);
			if (j == 8)
				roff(" induces earthquakes", 3, 77);
			if (j == 9)
				roff(" has been blessed by the gods", 3, 77);
		}
		...
		(f3 & (TR3_DRAIN_EXP)) || (cursed_p(o_ptr)))
	{
		...
		for (j = 0; j < 6; j++)
		{
			...
			if ((j == 0) && (f3 & (TR3_TELEPORT)))
				list_ok = TRUE;
			if ((j == 1) && (f3 & (TR3_AGGRAVATE)))
				list_ok = TRUE;
			if ((j == 2) && (f3 & (TR3_DRAIN_EXP)))
				list_ok = TRUE;
			if (cursed_p(o_ptr))
			{
				if ((j == 3) && (f3 & (TR3_PERMA_CURSE)))
					list_ok = TRUE;
				if ((j == 4) && (f3 & (TR3_HEAVY_CURSE)))
					list_ok = TRUE;
			...
			/* List the attribute description, in its proper place. */
			if (j == 0)
				roff(" induces random teleportation", 3, 77);
			if (j == 1)
				roff(" aggravates nearby creatures", 3, 77);
			if (j == 2)
				roff(" drains experience", 3, 77);
			if (j == 3)
			{
		...
		(f3 & (TR3_IGNORE_FIRE)) || (f3 & (TR3_IGNORE_COLD)))
	{
		...
		for (j = 0; j < 4; j++)
		{
			...
			if ((j == 0) && (f3 & (TR3_IGNORE_ACID)))
				list_ok = TRUE;
			if ((j == 1) && (f3 & (TR3_IGNORE_ELEC)))
				list_ok = TRUE;
			if ((j == 2) && (f3 & (TR3_IGNORE_FIRE)))
				list_ok = TRUE;
			if ((j == 3) && (f3 & (TR3_IGNORE_COLD)))
				list_ok = TRUE;
...
void self_knowledge(void)
{
	...
	{
		info[i++] =
			"You have enhanced protection from the spells of others.";
	}
	if (p_ptr->searching)
	{
...
bool spell_okay(int spell, bool known)
{
	...
	/* Spell is forgotten */
	if ((spell <
			32) ? (p_ptr->spell_forgotten1 & (1L << spell)) : (p_ptr->
	...
	/* Spell is learned */
	if ((spell <
			32) ? (p_ptr->spell_learned1 & (1L << spell)) : (p_ptr->
			spell_learned2 & (1L << (spell - 32))))
	{
#if 1/* TNB */
		/* Not enough mana? */
		if (known && (s_ptr->smana > p_ptr->csp))
			return (verify_mana);
#endif /* TNB */
...
void spell_info(char *p, int spell_index)
{
	...
	int beam = (((p_ptr->pclass == CLASS_MAGE) ||
	...
	switch (spell_index)
	{
		...
		case 0:
			sprintf(p, " dam 2d%d", 4 + plev / 10);
			break;
		case 2:
			strcpy(p, " range 10");
			break;
		case 3:
			sprintf(p, " dam 2d%d, rad %d", 1 + (plev / 5),
				(plev / 10) + 1); break;
		case 5:
		...
		case 8:
			...
			break;
		case 10:
			sprintf(p, " dam %dd8, beam %d%%", (2 + ((plev - 5) / 5)),
				beam);
			break;
		case 14:
		...
		case 15:
			...
			break;
		case 17:
			sprintf(p, " dam %d, range %d", 20 + plev, 3 + plev / 10);
			break;
		case 23:
			sprintf(p, " dam %dd8, beam %d%%", (7 + ((plev - 5) / 5)),
				beam);
			break;
		case 25:
			strcpy(p, " dam 20+d30");
		...
		case 52:
			...
			break;
		case 53:
			sprintf(p, " dam %dd8, beam %d%%", (5 + ((plev - 5) / 5)),
				beam);
			break;
		case 54:
			sprintf(p, " dam %d, rad 3", 10 + plev);
			break;
		case 55:
			sprintf(p, " dam %d, rad 2", 2 * plev);
			break;
		case 56:
		...
		case 57:
			...
			break;
		case 58:
			sprintf(p, " dam %d, rad 4", 120 + plev * 2);
			break;
		...
		case 81:
			sprintf(p, " dam %d+3d6, rad %d",
				plev / 4 + (plev / ((p_ptr->pclass ==
							CLASS_PRIEST) ? 2 : 4)), (plev < 30) ? 1 : 2);
			break;
		case 82:
		...
		case 89:
#if 1 /* TNB */
			sprintf(p, " dam %d-%d", 2 * plev + 1, 3 * plev);
#else /* TNB */
		...
		case 94:
#if 1 /* TNB */
			sprintf(p, " dam %d-%d, heal 300", plev * 3 + 1, plev * 4);
#else /* TNB */
		...
		case 155:
			...
			break;
		case 159:
			sprintf(p, " dam %d+d%d, rad %d", plev, 60 + plev * 2,
				1 + plev / 15);
			break;
		case 160:
			sprintf(p, " dam %d+d%d, rad %d", plev, 40 + plev * 2,
				plev / 8); break;
		case 169:
			strcpy(p, " dur 24+d24");
			break;
		case 170:
			...
			break;
		case 171:
			sprintf(p, " dam %d+d%d, rad %d", plev, 50 + plev * 2,
				1 + plev / 12);
			break;
		case 172:
			sprintf(p, " dam %d+d%d, rad %d", 3 * plev / 2, 30 + plev * 2,
				plev / 7);
			break;
		case 173:
			...
			break;
		case 174:
			sprintf(p, " dam %d+d%d, rad 1", 3 * plev, 50 + plev * 5);
			break;
		case 175:
			sprintf(p, " dam %d+d%d, rad %d", 5 * plev / 2, plev * 3,
				plev / 10);
			break;
		case 177:
		...
		case 178:
#if 1 /* TNB */
			sprintf(p, " dam %d-%d", plev * 2, plev * 4);
#else /* TNB */
			sprintf(p, " dam d%d-%d", plev * 2, plev * 4);
#endif /* TNB */
			break;
		case 181:
		...
		case 233:
			...
			break;
		case 235:
			sprintf(p, " dam %dd11", 3 * plev / 5);
			break;
		case 236:
			sprintf(p, " dam %d, hurt 2d8", 15 + plev * 3);
			break;
		case 237:
			sprintf(p, " dam %d+d%d", 60, plev * 2);
			break;
		case 238:
...
void print_spells(int tval, int sval, int y, int x)
{
	...
	for (i = first_spell; i < after_last_spell; i++)
	{
		...
		c_put_str(attr_name, format("%-30s", spell_names[s_ptr->index]),
			y + j + 1, x + 5);
		put_str(format("%2d %4d %3d%%", s_ptr->slevel, s_ptr->smana,
