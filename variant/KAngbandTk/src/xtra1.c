...
static void calc_mana(void)
{
	...
	if ((mp_ptr->spell_book == TV_MAGIC_BOOK) | (mp_ptr->spell_book == TV_ILLUSION_BOOK)) /* -KMW- */
	{
		u32b f1, f2, f3;
		...
		if (o_ptr->k_idx && !(f3 & (TR3_FREE_ACT)) && !((f1 & (TR1_DEX)) &&
				(o_ptr->pval > 0)))
		{
			/* Encumbered */
...
extern void calc_bonuses(void)
{
	...
	if (f2 & (TR2_HURT_ACID))
		p_ptr->hurt_acid = TRUE;
	if (f2 & (TR2_HURT_ELEC))
		p_ptr->hurt_elec = TRUE;
	if (f2 & (TR2_HURT_FIRE))
		p_ptr->hurt_fire = TRUE;
	if (f2 & (TR2_HURT_COLD))
		p_ptr->hurt_cold = TRUE;
	...
	if (((p_ptr->pclass == 1) || (p_ptr->pclass == 6)) &&
		((o_ptr->sval != SV_DAGGER) && (o_ptr->sval != SV_MAIN_GAUCHE) &&
			(o_ptr->sval != SV_QUARTERSTAFF) && (o_ptr->tval)))
	{
		/* Reduce the real bonuses */
		p_ptr->to_h -= 5;
		p_ptr->to_d -= 5;
...
void redraw_stuff(void)
{
	...
	if (p_ptr->redraw & (PR_TARGET))
	{
		...
		/* Target set */
		if (change ||
			(((target_state & TARGET_STATE_SET) != 0) !=
	  (p_ptr->target_set != 0)))
		{
			Bind_Generic(EVENT_TARGET,
				(p_ptr->
target_set ? KEYWORD_TARGET_SET : KEYWORD_TARGET_UNSET) + 1);
		}

		/* Target visibility */
		if (change ||
			(((target_state & TARGET_STATE_VIS) != 0) != (target_vis !=
					0)))
		{
