...
{
	/* Handle quest exit */
	if (cave_feat[p_ptr->py][p_ptr->px] == FEAT_QUEST_EXIT)
	{
		do_cmd_exit_quest();
		return;
	}
	...
	{
		msg_print("The staircase leads nowhere.");
		return;
	}
	...
	/* Success */
	sound(SNDGRP_EVENT, SND_DEPTH_UP, 0); /* ALLOW_SOUND */
...
void do_cmd_go_down(void)
{
	...
	/* Success */
	sound(SNDGRP_EVENT, SND_DEPTH_DOWN, 0);	/* ALLOW_SOUND */
...
static bool do_cmd_open_chest(int y, int x, s16b o_idx)
{
	...
	if (o_ptr->pval > 0)
	{
		...
		if (rand_int(100) < j)
		{
			sound(SNDGRP_EVENT, SND_PICK_LOCK, 0); /* ALLOW_SOUND */
	...
	if (flag)
	{
		sound(SNDGRP_EVENT, SND_OPEN, 0x02); /* ALLOW_SOUND */
...
static bool do_cmd_open_aux(int y, int x)
{
	...
	else
	{
		...
		/* Sound */
#if 1 /* ALLOW_SOUND */
		sound(SNDGRP_EVENT, SND_OPEN, 0x02);
#else
		sound(MSG_OPENDOOR);
#endif
	}
...
static bool do_cmd_close_aux(int y, int x)
{
	...
	else
	{
		...
		/* Sound */
#if 1 /* ALLOW_SOUND */
		sound(SNDGRP_EVENT, SND_CLOSE, 0x02);
#else
		sound(MSG_SHUTDOOR);
#endif
	}
...
#ifdef ALLOW_TUNNEL_PATCH /* TNB */

int tunnel_y = -1, tunnel_x = -1; /* Cave location of tunnel activity */
int tunnel_points = 0; /* Wall hitpoint counter */

/*
 * Called before do_cmd_tunnel_aux(). If this is the start of a
 * new attempt at tunnelling, then determine the number of
 * wall "hit points". Tunnelling at the same location does not
 * count as a new attempt.
 */
static void tunnel_setup(int y, int x)
{
	if (!command_repeating && ((y != tunnel_y) || (x != tunnel_x)))
	{
		/* Trees */
...
static bool do_cmd_tunnel_aux(int y, int x)
{
	...
	damage = (p_ptr->skill_dig * 2 / 3) + randint(p_ptr->skill_dig / 3);
	tunnel_points -= damage;
	...
	{
		sound(SNDGRP_EVENT, SND_TUNNEL_PERMANENT, 0); /* ALLOW_SOUND */
	...
	{
		sound(SNDGRP_EVENT, SND_TUNNEL_GRANITE, 0x01); /* ALLOW_SOUND */
	...
	else if (cave_feat[y][x] >= FEAT_MAGMA)
	{
		...
		sound(SNDGRP_EVENT, hard ? SND_TUNNEL_QUARTZ : SND_TUNNEL_MAGMA, 0x01);	/* ALLOW_SOUND */
		...
		if ((tunnel_points < 0) && twall(y, x))
		{
			...
			if (gold)
			{
				...
				/* Message */
				sound(SNDGRP_EVENT, SND_DISCOVER, 0x01); /* ALLOW_SOUND */
	...
	{
		sound(SNDGRP_EVENT, SND_TUNNEL_RUBBLE, 0x01); /* ALLOW_SOUND */
	...
	{
		sound(SNDGRP_EVENT, SND_TUNNEL_DOOR, 0x01);	/* ALLOW_SOUND */
...
void do_cmd_spike(bool spell)
{
	...
	/* Verify legality */
	if (!do_cmd_spike_test(y, x, spell))
		return;
	...
	else
	{
		...
		if (spell)
		{
			sound(SNDGRP_EVENT, SND_USE_SPIKE, 0x01); /* ALLOW_SOUND */
		...
		{
#if 1 /* TNB */
			cave_set_feat(y, x, cave_feat[y][x] + 0x08);
#else
			cave_feat[y][x] += 0x08;
#endif
		}
		...
		if (spell)
		{
			/* Add jam to the door */
#if 1 /* TNB */
			cave_set_feat(y, x, FEAT_DOOR_TAIL - 1);
#else
...
{
	return player_test_feature(y, x, 0x01);
}
...
static void do_cmd_hold_or_stay(int pickup)
{
	...
	if (!command_repeating)
		Bind_Track(KEYWORD_TRACK_GRID + 1, 0, p_ptr->py, p_ptr->px);
#endif /* TNB */
...
void do_cmd_rest(void)
{
	...
	sound(SNDGRP_EVENT, SND_REST, 0x02); /* ALLOW_SOUND */
...
void do_cmd_fire(void)
{
	...
	/* Sound */
#if 1 /* ALLOW_SOUND */
	sound(SNDGRP_EVENT, SND_ATTACK_BOW, 0);
#else
	sound(MSG_SHOOT);
#endif
	...
	for (i = 0; i < path_n; ++i)
	{
		...
		if (cave_m_idx[y][x] > 0)
		{
			...
			if (test_hit_fire(chance2, r_ptr->ac, m_ptr->ml))
			{
				...
				sound(SNDGRP_EVENT, SND_MISSILE_HIT, 0); /* ALLOW_SOUND */
			...
			{
				sound(SNDGRP_EVENT, SND_MISSILE_MISS, 0x02);
			}
...
void do_cmd_throw(void)
{
	...
	sound(SNDGRP_EVENT, SND_ATTACK_THROW, 0x02); /* ALLOW_SOUND */
