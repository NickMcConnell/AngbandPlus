...
void init_file_paths(char *path)
{
	...
	path_parse(buf2, 1024, PRIVATE_USER_PATH);

	/* Build the path to the user specific directory */
	path_build(buf, 1024, buf2, VERSION_NAME);
...
static void display_parse_error(cptr filename, errr err, cptr buf)
{
	...
	/* Oops */
#if 1
	quit_fmt("Error %d at line %d of '%s'.\n"
		"Record %d contains a '%s' error.\n"
		"Parsing '%s'.",
		err, error_line, filename, error_idx, oops, buf);
#else
 ...
 */
static errr init_info(cptr filename, header * head, void **info,
	void **name, void **text)
{
	int fd;
	...
	if (err)
	{
		...
		/* Build the filename */
		path_build(buf, 1024, ANGBAND_DIR_EDIT, format("%s.txt",
				filename));
		...
		/* Errors */
		if (err)
			display_parse_error(format("%s.txt", filename), err, buf);
		...
		/* Build the filename */
		path_build(buf, 1024, ANGBAND_DIR_DATA, format("%s.raw",
				filename));
...
static errr init_z_info(void)
{
	...
	return init_info("z_info", &z_head, (void *) &z_info, NULL, NULL);
}
...
static errr init_f_info(void)
{
	...
	return init_info("f_info", &f_head, (void *) &f_info, (void *) &f_name,
		(void *) &f_text);
}
...
static errr init_k_info(void)
{
	...
	return init_info("k_info", &k_head, (void *) &k_info, (void *) &k_name,
		(void *) &k_text);
}
...
static errr init_a_info(void)
{
	...
	return init_info("a_info", &a_head, (void *) &a_info, (void *) &a_name,
		(void *) &a_text);
}
...
static errr init_e_info(void)
{
	...
	return init_info("e_info", &e_head, (void *) &e_info, (void *) &e_name,
		(void *) &e_text);
}
...
static errr init_r_info(void)
{
	...
	return init_info("r_info", &r_head, (void *) &r_info, (void *) &r_name,
		(void *) &r_text);
}
...
static errr init_v_info(void)
{
	...
	return init_info("v_info", &v_head, (void *) &v_info, (void *) &v_name,
		(void *) &v_text);
}
...
static errr init_p_info(void)
{
	...
	return init_info("p_info", &p_head, (void *) &p_info, (void *) &p_name,
		(void *) &p_text);
}
...
static errr init_h_info(void)
{
	...
	return init_info("h_info", &h_head, (void *) &h_info, NULL,
		(void *) &h_text);
}
...
static errr init_b_info(void)
{
	...
	return init_info("b_info", &b_head, (void *) &b_info, (void *) &b_name,
		(void *) &b_text);
}
...
static errr init_g_info(void)
{
	...
	return init_info("g_info", &g_head, (void *) &g_info, (void *) &g_name,
		(void *) &g_text);
}
...
/*
 * Initialize the plot names
 */
errr init_plot_info(void)
{
	init_flags = INIT_SHOW_TEXT | INIT_ASSIGN;
	process_dungeon_file("t_info.txt", 0, 0, 0, 0);
 ...
 */
static const byte store_table[MAX_STORES][STORE_CHOICES][2] = {
	{
			/* General Store */
...

	{
			...
			{TV_POTION, SV_SCROLL_WORD_OF_RECALL},
			{TV_POTION, SV_SCROLL_WORD_OF_RECALL},
 ...
 */
static errr init_plots(void)
{
	/*** Prepare the plots ***/

	/* Allocate the plots */
	C_MAKE(plots, z_info->q_max, plot_type);
...
static errr init_other(void)
{
	...
	/* Special array */
	C_MAKE(cave_special, DUNGEON_HGT, s16b_wid);
...
#if 1 /* TNB */
	/* Initialize the hitpoint_warn */
	op_ptr->hitpoint_warn = 3;

	/* Initialize the delay_factor */
	op_ptr->delay_factor = 4;
#endif /* TNB */
...
void init_angband(void)
{
	...
	dbwin("Before Term_fresh()-\n");

	/* Flush it */
	Term_fresh();

	dbwin("Before -high score file stuff-\n");
	...
	dbwin("Before -init some arrays-()\n");

	/*** Initialize some arrays ***/

	/* Initialize size info */
	note("[Initializing array sizes...]");
	angtk_eval("angband_startup", "init_z_info", NULL);	/* TNB */
	if (init_z_info())
	...
	angtk_eval("angband_startup", "init_v_info", NULL);	/* TNB */
	if (init_v_info())
		quit("Cannot initialize vaults");

	/* Initialize history info */
	note("[Initializing arrays... (histories)]");
	angtk_eval("angband_startup", "init_h_info", NULL);	/* TNB */
	if (init_h_info())
		quit("Cannot initialize histories");

	/* Initialize race info */
	note("[Initializing arrays... (races)]");
	angtk_eval("angband_startup", "init_p_info", NULL);	/* TNB */
	if (init_p_info())
		quit("Cannot initialize races");

	/* Initialize owner info */
	note("[Initializing arrays... (owners)]");
	angtk_eval("angband_startup", "init_b_info", NULL);	/* TNB */
	if (init_b_info())
		quit("Cannot initialize owners");

	/* Initialize price info */
	note("[Initializing arrays... (prices)]");
	angtk_eval("angband_startup", "init_g_info", NULL);	/* TNB */
	if (init_g_info())
		quit("Cannot initialize prices");

	/* Initialize plot info -KMW- */
	note("[Initializing plots... (other)]");
	angtk_eval("angband_startup", "init_plots", NULL); /* TNB */
	if (init_plots())
		quit("Cannot initialize plots");
