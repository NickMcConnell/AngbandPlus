...
void display_map(int *cy, int *cx)
{
	...
	if ((cy != NULL) && (map_hgt > SCREEN_HGT))
		map_hgt = SCREEN_HGT;
	if ((cx != NULL) && (map_wid > SCREEN_WID))
		map_wid = SCREEN_WID;
...
/* #define FRESH_LITE -- TNB */
	...
	if (p_ptr->target_set && (p_ptr->target_who == 0))
	{
		...
		target_vis = ((y == p_ptr->py) && (x == p_ptr->px)) ||
			projectable(p_ptr->py, p_ptr->px, y, x);
...
#define MAP_RANGE 40
	...
	/* Pick an area to map */
	y1 = p_ptr->py - MAP_RANGE;
	y2 = p_ptr->py + MAP_RANGE;
	x1 = p_ptr->px - MAP_RANGE;
	x2 = p_ptr->px + MAP_RANGE;
