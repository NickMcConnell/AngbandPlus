...
static void choose_item(int a_idx)
{
	...
	switch (a_ptr->tval)
	{
		...
		case TV_POLEARM:
			a_ptr->to_h +=
				(s16b) (a_ptr->level / 10 + rand_int(4) + rand_int(4));
		...
		case TV_HARD_ARMOR:
			a_ptr->to_a +=
				(s16b) (a_ptr->level / 10 + a_ptr->ac / 3 + rand_int(8));
...
static void scramble_artifact(int a_idx)
{
	...
	if (a_idx >= ART_MIN_NORMAL)
	{
		...
		while ((count < MAX_TRIES) && ((ap2 > (power * 8) / 10 + 1) ||
				(ap2 < (power / 10))));
	}
	else
	{
...
static int artifacts_acceptable(void)
{
	...
	if (swords > 0 || polearms > 0 || blunts > 0 || bows > 0 || bodies > 0
		|| shields > 0 || cloaks > 0 || hats > 0 || gloves > 0 ||
		boots > 0)
	{
		if (randart_verbose)
		{
			...
			sprintf(types, "%s%s%s%s%s%s%s%s%s%s",
				swords > 0 ? " swords" : "",
				polearms > 0 ? " polearms" : "",
				blunts > 0 ? " blunts" : "", bows > 0 ? " bows" : "",
				bodies > 0 ? " body-armors" : "",
				shields > 0 ? " shields" : "", cloaks > 0 ? " cloaks" : "",
				hats > 0 ? " hats" : "", gloves > 0 ? " gloves" : "",
				boots > 0 ? " boots" : "");
