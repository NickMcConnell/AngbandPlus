...
bool set_blind(int v)
{
	...
	if (v)
	{
		if (!p_ptr->blind)
		{
			sound(SNDGRP_EVENT, SND_STATE_BLIND, 0); /* ALLOW_SOUND */
	...
	else
	{
		if (p_ptr->blind)
		{
			sound(SNDGRP_EVENT, SND_STATE_RECOVER, 0); /* ALLOW_SOUND */
...
bool set_confused(int v)
{
	...
	if (v)
	{
		if (!p_ptr->confused)
		{
			sound(SNDGRP_EVENT, SND_STATE_CONFUSED, 0);	/* ALLOW_SOUND */
...
bool set_poisoned(int v)
{
	...
	if (v)
	{
		if (!p_ptr->poisoned)
		{
			sound(SNDGRP_EVENT, SND_STATE_POISONED, 0x01);	/* ALLOW_SOUND */
...
bool set_afraid(int v)
{
	...
	if (v)
	{
		if (!p_ptr->afraid)
		{
			sound(SNDGRP_EVENT, SND_STATE_AFRAID, 0); /* ALLOW_SOUND */
...
bool set_paralyzed(int v)
{
	...
	if (v)
	{
		if (!p_ptr->paralyzed)
		{
			sound(SNDGRP_EVENT, SND_STATE_PARALYZED, 0); /* ALLOW_SOUND */
...
bool set_image(int v)
{
	...
	if (v)
	{
		if (!p_ptr->image)
		{
			sound(SNDGRP_EVENT, SND_STATE_HALLUCINATE, 0); /* ALLOW_SOUND */
...
bool set_fast(int v)
{
	...
	if (v)
	{
		if (!p_ptr->fast)
		{
			sound(SNDGRP_EVENT, SND_STATE_FAST, 0);	/* ALLOW_SOUND */
	...
	else
	{
		if (p_ptr->fast)
		{
			sound(SNDGRP_EVENT, SND_STATE_CHANGE, 0); /* ALLOW_SOUND */
...
bool set_slow(int v)
{
	...
	if (v)
	{
		if (!p_ptr->slow)
		{
			sound(SNDGRP_EVENT, SND_STATE_SLOW, 0);	/* ALLOW_SOUND */
...
bool set_shield(int v)
{
	...
	if (v)
	{
		if (!p_ptr->shield)
		{
			sound(SNDGRP_EVENT, SND_STATE_SHIELD, 0); /* ALLOW_SOUND */
...
bool set_blessed(int v)
{
	...
	if (v)
	{
		if (!p_ptr->blessed)
		{
			sound(SNDGRP_EVENT, SND_STATE_BLESS, 0); /* ALLOW_SOUND */
...
bool set_hero(int v)
{
	...
	if (v)
	{
		if (!p_ptr->hero)
		{
			sound(SNDGRP_EVENT, SND_STATE_HERO, 0);	/* ALLOW_SOUND */
...
bool set_shero(int v)
{
	...
	if (v)
	{
		if (!p_ptr->shero)
		{
			sound(SNDGRP_EVENT, SND_STATE_BERSERK, 0); /* ALLOW_SOUND */
...
bool set_protevil(int v)
{
	...
	if (v)
	{
		if (!p_ptr->protevil)
		{
			sound(SNDGRP_EVENT, SND_STATE_PROT_EVIL, 0); /* ALLOW_SOUND */
...
bool set_invuln(int v)
{
	...
	if (v)
	{
		if (!p_ptr->invuln)
		{
			sound(SNDGRP_EVENT, SND_STATE_INVULN, 0); /* ALLOW_SOUND */
...
bool set_tim_invis(int v)
{
	...
	if (v)
	{
		if (!p_ptr->tim_invis)
		{
			sound(SNDGRP_EVENT, SND_STATE_SEE_INVIS, 0); /* ALLOW_SOUND */
...
bool set_tim_infra(int v)
{
	...
	if (v)
	{
		if (!p_ptr->tim_infra)
		{
			sound(SNDGRP_EVENT, SND_STATE_INFRA, 0); /* ALLOW_SOUND */
...
bool set_oppose_acid(int v)
{
	...
	if (v)
	{
		...
		{
			sound(SNDGRP_EVENT, SND_STATE_RESIST_ACID, 0); /* ALLOW_SOUND */
...
bool set_oppose_elec(int v)
{
	...
	if (v)
	{
		...
		{
			sound(SNDGRP_EVENT, SND_STATE_RESIST_LIGHTNING, 0);	/* ALLOW_SOUND */
...
bool set_oppose_fire(int v)
{
	...
	if (v)
	{
		...
		{
			sound(SNDGRP_EVENT, SND_STATE_RESIST_FIRE, 0); /* ALLOW_SOUND */
...
bool set_oppose_cold(int v)
{
	...
	if (v)
	{
		...
		{
			sound(SNDGRP_EVENT, SND_STATE_RESIST_COLD, 0); /* ALLOW_SOUND */
...
bool set_oppose_pois(int v)
{
	...
	if (v)
	{
		...
		{
			sound(SNDGRP_EVENT, SND_STATE_RESIST_POISON, 0); /* ALLOW_SOUND */
...
bool set_tim_ts_anchor(int v)
{
	...
	redraw_add(PR_ANCHOR); /* ALLOW_STATUS_EXTRA */
...
bool set_tim_pl_invis(int v)
{
	...
	redraw_add(PR_INVIS); /* ALLOW_STATUS_EXTRA */
...
bool set_tim_ghostly(int v)
{
	...
	redraw_add(PR_GHOST); /* ALLOW_STATUS_EXTRA */
...
bool set_tim_levitate(int v)
{
	...
	redraw_add(PR_LEVITATE);	/* ALLOW_STATUS_EXTRA */
...
bool set_tim_sus_str(int v)
{
	...
	redraw_add(PR_SUS_STR);	/* ALLOW_STATUS_EXTRA */
...
bool set_tim_sus_int(int v)
{
	...
	redraw_add(PR_SUS_INT);	/* ALLOW_STATUS_EXTRA */
...
bool set_tim_sus_wis(int v)
{
	...
	redraw_add(PR_SUS_WIS);	/* ALLOW_STATUS_EXTRA */
...
bool set_tim_sus_dex(int v)
{
	...
	redraw_add(PR_SUS_DEX);	/* ALLOW_STATUS_EXTRA */
...
bool set_tim_sus_con(int v)
{
	...
	redraw_add(PR_SUS_CON);	/* ALLOW_STATUS_EXTRA */
...
bool set_tim_sus_chr(int v)
{
	...
	redraw_add(PR_SUS_CHR);	/* ALLOW_STATUS_EXTRA */
...
bool set_oppose_ld(int v)
{
	...
	redraw_add(PR_OPPOSE_LD); /* ALLOW_STATUS_EXTRA */
...
bool set_oppose_cc(int v)
{
	...
	redraw_add(PR_OPPOSE_CC); /* ALLOW_STATUS_EXTRA */
...
bool set_oppose_ss(int v)
{
	...
	redraw_add(PR_OPPOSE_SS); /* ALLOW_STATUS_EXTRA */
...
bool set_oppose_nexus(int v)
{
	...
	redraw_add(PR_OPPOSE_NEX); /* ALLOW_STATUS_EXTRA */
...
bool set_oppose_nethr(int v)
{
	...
	redraw_add(PR_OPPOSE_NETH);	/* ALLOW_STATUS_EXTRA */
...
bool set_stun(int v)
{
	...
	if (new_aux > old_aux)
	{
		sound(SNDGRP_EVENT, SND_STATE_STUN, 0);	/* ALLOW_SOUND */
...
bool set_cut(int v)
{
	...
	if (new_aux > old_aux)
	{
		sound(SNDGRP_EVENT, SND_STATE_CUT, 0); /* ALLOW_SOUND */
...
bool set_food(int v)
{
	...
	if (new_aux > old_aux)
	{
		...
		switch (new_aux)
		{
			...
			case 1:
			{
				sound(SNDGRP_EVENT, SND_STATE_HUNGRY, 0); /* ALLOW_SOUND */
			...
			case 3:
			{
				sound(SNDGRP_EVENT, SND_STATE_HUNGRY_END, 0); /* ALLOW_SOUND */
			...
			case 4:
			{
				sound(SNDGRP_EVENT, SND_STATE_FULL, 0); /* ALLOW_SOUND */
			...
			case 5:
			{
				sound(SNDGRP_EVENT, SND_STATE_GORGED, 0); /* ALLOW_SOUND */
	...
	else if (new_aux < old_aux)
	{
		...
		switch (new_aux)
		{
			...
			case 3:
			{
				sound(SNDGRP_EVENT, SND_STATE_FULL_END, 0); /* ALLOW_SOUND */
			...
			case 4:
			{
				sound(SNDGRP_EVENT, SND_STATE_GORGED_END, 0); /* ALLOW_SOUND */
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_FOOD + 1); /* TNB */
...
(player_exp[p_ptr->lev - 2] * p_ptr->expfact / 100L)))
	{
		...
		sound(SNDGRP_EVENT, SND_EXPER_LEVEL_DOWN, 0x02); /* ALLOW_SOUND */
...
(player_exp[p_ptr->lev - 1] * p_ptr->expfact / 100L)))
	{
		...
		sound(SNDGRP_EVENT, SND_EXPER_LEVEL_UP, 0);	/* ALLOW_SOUND */
	...
	while ((p_ptr->max_lev < PY_MAX_LEVEL) &&
		(p_ptr->max_exp >=
(player_exp[p_ptr->max_lev - 1] * p_ptr->expfact / 100L)))
	{
		/* Gain max level */
		p_ptr->max_lev++;
...
void monster_death(int m_idx)
{
		...
		switch (quest[i].type)
		{
			case QUEST_TYPE_KILL_NUMBER:
			{
				...
				if (quest[i].cur_num >= quest[i].num_mon)
				{
					...
					{
						sound(SNDGRP_EVENT, SND_QUEST_DONE, 0);	/* ALLOW_SOUND */
	...
	if (strstr((r_name + r_ptr->name), "Morgoth, Lord of Darkness"))
	{
		...
		/* Congratulations */
		sound(SNDGRP_EVENT, SND_CHARACTER_WIN, 0); /* ALLOW_SOUND */
...
bool mon_take_hit(int m_idx, int dam, bool * fear, cptr note, bool bypet)
{
	...
	if (m_ptr->hp < 0)
	{
...
#ifdef ALLOW_SOUND /* TNB */
		sound_stop(SNDGRP_MONSTER, m_ptr->r_idx);
		sound(SNDGRP_EVENT, SND_MONSTER_DEATH, 0);
		if (r_ptr->flags1 & (RF1_UNIQUE))
			sound(SNDGRP_EVENT, SND_MONSTER_DEATH_UNIQUE, 0);
		...
		{
			message_format(MSG_KILL, m_ptr->r_idx,
				"You have destroyed %s.", m_name);
		}
		...
		if (m_ptr->ml || (r_ptr->flags1 & (RF1_UNIQUE)))
		{
			...
			if (l_ptr->r_pkills < MAX_SHORT)
				l_ptr->r_pkills++;

			/* Count kills in all lives */
			if (l_ptr->r_tkills < MAX_SHORT)
				l_ptr->r_tkills++;
 ...
 */
/* static -- TNB */ void look_mon_desc(char *buf, int m_idx)
{
	monster_type *m_ptr = &m_list[m_idx];
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
	...
	if (m_ptr->hold_o_idx)
	{
		...
		sprintf(buf + strlen(buf), ", %d object%s",
			count, (count > 1) ? "s" : "");
	}

	if (m_ptr->mspeed > 110)
		sprintf(buf + strlen(buf), ", speed +%d", m_ptr->mspeed - 110);
	else if (m_ptr->mspeed < 110)
		sprintf(buf + strlen(buf), ", speed -%d", 110 - m_ptr->mspeed);
#endif /* TNB */
}
...
void target_set_location(int y, int x)
{
	...
	p_ptr->redraw |= (PR_TARGET);
	target_state |= TARGET_STATE_CHANGE;
	target_vis = p_ptr->target_set && (((y == p_ptr->py) &&
			(x == p_ptr->px)) || projectable(p_ptr->py, p_ptr->px, y, x));
#endif /* TNB */
}
 ...
 */
/* static -- TNB */ bool ang_sort_comp_distance(vptr u, vptr v, int a, int b)
{
	int py = p_ptr->py;
	int px = p_ptr->px;
 ...
 */
/* static -- TNB */ void ang_sort_swap_distance(vptr u, vptr v, int a, int b)
{
	byte *x = (byte *) (u);
	byte *y = (byte *) (v);
...
static bool target_set_interactive_accept(int y, int x)
{
	...
	if (cave_info[y][x] & (CAVE_MARK))
	{
		...
		if (cave_feat[y][x] == FEAT_SHAL_WATER)
			return (TRUE);
		if (cave_feat[y][x] == FEAT_DEEP_LAVA)
			return (TRUE);
		...
		if (cave_feat[y][x] == FEAT_CHASM)
			return (TRUE);
		if (cave_feat[y][x] == FEAT_TREES)
			return (TRUE);
		if (cave_feat[y][x] == FEAT_MOUNTAIN)
			return (TRUE);

#if 1 /* BUG -- TNB */
		/* Notice quest entrance/exit */
		if (cave_feat[y][x] == FEAT_QUEST_ENTER)
			return (TRUE);
		if (cave_feat[y][x] == FEAT_QUEST_EXIT)
			return (TRUE);
		if (cave_feat[y][x] == FEAT_QUEST_UP)
			return (TRUE);
		if (cave_feat[y][x] == FEAT_QUEST_DOWN)
			return (TRUE);
#endif /* BUG -- TNB */
	}
	...
	while (1)
	{
			...
			if (m_ptr->ml)
			{
				...
				while (1)
				{
					...
					else
					{
						...
						{
							sprintf(out_val,
								"%s%s%s%s (%s) [r,%s] (%d:%d)", s1, s2, s3,
								m_name, buf, info, y, x);
						}
						else
						{
							sprintf(out_val, "%s%s%s%s (%s) [r,%s]", s1,
								s2, s3, m_name, buf, info);
						}
					...
					this_o_idx = next_o_idx)
				{
					...
					/* Describe the object */
					if (p_ptr->wizard)
						sprintf(out_val, "%s%s%s%s [%s] (%d:%d)", s1, s2,
							s3, o_name, info, y, x);
					else
			...
			if (floor_num > 1)
			{
				...
				while (1)
				{
					...
					if (p_ptr->wizard)
						...
						sprintf(out_val,
							"%s%s%sa pile of %d objects [r,%s]", s1, s2,
							s3, floor_num, info);
					prt(out_val, 0, 0);
					move_cursor_relative(y, x);
					query = inkey();
			...
			this_o_idx = next_o_idx)
		{
			...
			if (o_ptr->marked)
			{
				...
				/* Describe the object */
				if (p_ptr->wizard)
					sprintf(out_val, "%s%s%s%s [%s] (%d:%d)", s1, s2, s3,
						o_name, info, y, x);
				else
...
bool target_set_interactive(int mode)
{
	...
	int target_set = FALSE, target_who = 0;	/* TNB */
	...
	while (!done)
	{
		...
		if (flag && temp_n)
		{
			...
			switch (query)
			{
				...
				case '@':
				{
					...
					if (!get_string("y: ", ybuf, 3))
						break;
					if (!get_string("x: ", xbuf, 3))
						break;
					ty = atoi(ybuf);
					tx = atoi(xbuf);
		...
		else
		{
			...
			query =
				target_set_interactive_aux(y, x, mode | TARGET_LOOK, info);
			...
			if (d)
			{
				...
				{
					/* Slide into legality */
					if (x >= p_ptr->wx + SCREEN_WID)
						x--;
					else if (x < p_ptr->wx)
						x++;

					/* Slide into legality */
					if (y >= p_ptr->wy + SCREEN_HGT)
						y--;
					else if (y < p_ptr->wy)
						y++;
				}
...
bool get_aim_dir(int *dp)
{
	...
	if (use_old_target && target_okay())
	{
		if (p_ptr->target_who == 0)
		{
			bool floor_target_los = TRUE; /* TODO: make this an option */
			/* NOTE: The player grid is not projectable */
			if (!floor_target_los ||
				projectable(p_ptr->py, p_ptr->px,
		...
		switch (ch)
		{
			...
			case '*':
			{
				if (target_set_interactive(TARGET_KILL))
					dir = 5;
				if (dir == 5)
					sound(SNDGRP_EVENT, SND_TARGET_SET, 0);	/* ALLOW_SOUND */
				break;
			}
	...
	/* Save the non-confused direction the user entered */
