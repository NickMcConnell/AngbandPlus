...
void teleport_away(int m_idx, int dis)
{
	...
	ny = oy, nx = ox; /* Compiler warning -- TNB */
...
/* XXX Mega-Hack -- See Dimension Door in do_cmd_cast() */
				if (distance(y1, x1, p_ptr->py,
						p_ptr->px) > p_ptr->lev + 2)
				{
					x1 -= ddx[d];
					y1 -= ddy[d];
				}
...
void take_hit(int dam, cptr kb_str)
{
	...
	if (p_ptr->chp < warning)
	{
		...
		{
/*			bell("Low hitpoint warning!"); ALLOW_SOUND */
			sound(SNDGRP_EVENT, SND_WARN_HITPOINT, 0); /* ALLOW_SOUND */
		}
...
void acid_dam(int dam, cptr kb_str)
{
	...
	if (p_ptr->resist_acid)
		dam = (dam + 2) / 3;
	if (p_ptr->oppose_acid && !p_ptr->hurt_acid)
		dam = (dam + 2) / 3;
...
void elec_dam(int dam, cptr kb_str)
{
	...
	if (p_ptr->resist_elec)
		dam = (dam + 2) / 3;
	if (p_ptr->oppose_elec && !p_ptr->hurt_elec)
		dam = (dam + 2) / 3;
...
void fire_dam(int dam, cptr kb_str)
{
	...
	if (p_ptr->resist_fire)
		dam = (dam + 2) / 3;
	if (p_ptr->oppose_fire && !p_ptr->hurt_fire)
		dam = (dam + 2) / 3;
...
void cold_dam(int dam, cptr kb_str)
{
	...
	if (p_ptr->resist_cold)
		dam = (dam + 2) / 3;
	if (p_ptr->oppose_cold && !p_ptr->hurt_cold)
		dam = (dam + 2) / 3;
...
static bool project_p(int who, int r, int y, int x, int dam, int typ)
{
	...
	switch (typ)
	{
		...
		case GF_LITE:
		{
			...
			if (!p_ptr->resist_lite && !p_ptr->oppose_ld && !blind &&
				!p_ptr->resist_blind)
			{
		...
		case GF_DARK:
		{
			...
			if (!p_ptr->resist_dark && !p_ptr->oppose_ld && !blind &&
				!p_ptr->resist_blind)
			{
				(void) set_blind(p_ptr->blind + randint(5) + 2);
			}
