...
#include "tnb.h" /* TNB */
#define TNB_STORE_PROMPTS /* minimize store prompts with auto_haggle -- TNB */
...
static void purchase_analyze(s32b price, s32b value, s32b guess)
{
	...
	{
		/* Comment */
		sound(SNDGRP_EVENT, SND_STORE_SELL_BAD, 0);	/* ALLOW_SOUND */
	...
	{
		/* Comment */
		sound(SNDGRP_EVENT, SND_STORE_SELL_GOOD, 0); /* ALLOW_SOUND */
 ...
 */
/* static TNB */ bool store_will_buy(const object_type * o_ptr)
{
		...
		case STORE_TEMPLE:
		{
			...
			switch (o_ptr->tval)
			{
				...
				case TV_SWORD:
				{
					...
					if (is_blessed(o_ptr) && object_known_p(o_ptr))
						break;
				}
				default:
					return (FALSE);
...
static int get_haggle(cptr pmt, s32b * poffer, s32b price, int final)
{
	...
	if (final)
	{
#ifdef TNB_STORE_PROMPTS
		if (auto_haggle)
		{
extern bool get_check_aux(cptr prompt, bool enter_is_yes);
			if (get_check_aux(pmt, TRUE))
			{
				*poffer = price;
				last_inc = 0L;
				return TRUE;
...
#if 1 /* TNB */
		Bind_Ask(KEYWORD_ASK_HAGGLE + 1, price, 0, TRUE);

		/* Ask for a quantity */
		ok = get_string(buf, out_val, 79);

		Bind_Ask(KEYWORD_ASK_HAGGLE + 1, 0, 0, FALSE);

		if (!ok)
			return (FALSE);
#else /* TNB */
		/* Ask the user for a response */
		if (!get_string(buf, out_val, 79))
			return (FALSE);
#endif /* TNB */
	...
	*price = 0;
	...
	for (flag = FALSE; !flag;)
	{
		...
		while (!flag && loop_flag)
		{
			...
			angtk_eval("angband_store", "price_owner", out_val, NULL);
#else /* TNB */
			sprintf(out_val, "%s :  %ld", pmt, (long) cur_ask);
			put_str(out_val, 1, 0);
#endif /* TNB */
			...
			if (auto_haggle)
			{
				char o_name[80];
				object_desc_store(o_name, o_ptr, TRUE, 3);
				sprintf(out_val, "Buy %s for %ld gold? ", o_name, cur_ask);
			}
			else
				strcpy(out_val, "What do you offer? ");
			cancel = receive_offer(out_val, &offer, last_offer, 1,
				cur_ask, final);
#else /* !TNB_STORE_PROMPTS */
			cancel =
	...
	*price = 0;
	...
	for (flag = FALSE; !flag;)
	{
		while (1)
		{
			...
			{
				char o_name[80];
				bool flavors = show_flavors;
				show_flavors = FALSE;
				object_desc(o_name, o_ptr, TRUE, 3);
				show_flavors = flavors;
				sprintf(out_val, "Sell %s for %ld gold? ", o_name, cur_ask);
			}
			else
				strcpy(out_val, "What do you offer? ");
			cancel = receive_offer(out_val, &offer,
		...
		if (!flag)
		{
			...
			if (!flag)
			{
				...
				angtk_eval("angband_store", "price_owner", "", NULL); /* TNB */
#if 1 /* TNB */
				sprintf(out_val, "Your last bid: %ld", (long) last_offer);
#else /* TNB */
				sprintf(out_val, "Your last bid %ld", (long) last_offer);
				put_str(out_val, 1, 39);
#endif /* TNB */
...
static void store_purchase(void)
{
	...
	if (store_num != STORE_HOME)
	{
		...
		if (choice == 0)
		{
			...
			if (p_ptr->au >= price)
			{
				...
				/* Message */
				sound(SNDGRP_EVENT, SND_STORE_BUY, 0); /* ALLOW_SOUND */
#ifdef TNB_STORE_PROMPTS
				if (!auto_haggle)
#endif /* TNB_STORE_PROMPTS */
	...
	else
	{
		...
		/* Message */
		msg_format("You pick up %s (%c).", o_name, store_to_label(item));
...
static void store_sell(void)
{
	...
	if (store_num != STORE_HOME)
	{
		...
		if (choice == 0)
		{
			...
			/* Window stuff */
			p_ptr->window |=
				(PW_INVEN | PW_EQUIP | PW_PLAYER_0 | PW_PLAYER_1);
			...
			sound(SNDGRP_EVENT, SND_STORE_SELL, 0);	/* ALLOW_SOUND */
...
static void home_destroy(void)
{
	...
	/* Get the object number to be bought */
	if (!get_stock(&item, "Destroy which item? "))
		return;
...
void do_cmd_store(void)
{
	...
	sound(SNDGRP_EVENT, (store_num == STORE_HOME)
		? SND_HOME_ENTER
		: SND_STORE_ENTER, 0x02); /* ALLOW_SOUND */
	...
	sound(SNDGRP_EVENT, (store_num == STORE_HOME)
		? SND_HOME_LEAVE
		: SND_STORE_LEAVE, 0x02); /* ALLOW_SOUND */
