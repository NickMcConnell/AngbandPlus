	...
	while (1)
	{
		...
		else if ((ch == 'D') || (ch == 'd'))
		{
			...
			while (1)
			{
				...
				prt("New base delay factor (0-9 or ESC to accept): ", 21,
					0);
		...
		else if ((ch == 'H') || (ch == 'h'))
		{
			...
			while (1)
			{
				...
				prt("New hitpoint warning (0-9 or ESC to accept): ", 21,
					0);
...
void do_cmd_objects(void)
{
	...
	while (TRUE)
	{
		...
		/* Choose! */
		if (!get_com
			("Interact with which type of object? (Press ESC to exit)",
				&ch)) break;
				...
				(quest[i].level < rand_level))
			{
				...
				if (quest[i].max_num > 1)
				{
					...
					sprintf(rand_tmp_str,
						...
						quest[i].name, quest[i].level, quest[i].max_num,
						name, quest[i].cur_num);
				}
				else
				{
					sprintf(rand_tmp_str,
