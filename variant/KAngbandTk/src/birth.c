...
static bool player_birth_aux_1(void)
{
	...
	while (1)
	{
		sprintf(buf, "Choose a sex (%c-%c, or * for random): ", I2A(0),
			I2A(n - 1));
		put_str(buf, 20, 2);
		ch = inkey();
		if (ch == 'Q')
	...
	while (1)
	{
		sprintf(buf, "Choose a race (%c-%c, or * for random): ", I2A(0),
			I2A(n - 1));
		put_str(buf, 20, 2);
		ch = inkey();
		if (ch == 'Q')
	...
	while (1)
	{
		sprintf(buf, "Choose a class (%c-%c, or * for random): ", I2A(0),
			I2A(n - 1));
		put_str(buf, 20, 2);
		ch = inkey();
		if (ch == 'Q')
	...
	while (1)
	{
		sprintf(buf, "Choose a plot (%c-%c, or * for random): ", I2A(0),
			I2A(n - 1));
		put_str(buf, 20, 2);
		ch = inkey();
		if (ch == 'Q')
