	...
	else
	{
		...
		if (get_string(tmp_str, out_val, 32))
		{
			...
			do
			{
				switch (cmd)
				{
					case BACT_IN_BETWEEN: /* Game of In-Between */
						...
						angtk_eval("angband_building", "inbetween",
							format("%d %d %d", roll1, choice, roll2), NULL);
#else /* TNB */
					...
					case BACT_CRAPS: /* Game of Craps */
						...
						else
						{
							do
							{
								...
								roll3 = roll1 + roll2;
								angtk_eval("angband_building", "craps",
									format("%d %d %d", roll1, roll2, roll3),
									NULL);
								if (roll3 == choice)
									win = TRUE;
								else if (roll3 == 7)
					...
					case BACT_SPIN_WHEEL: /* Spin the Wheel Game */
						...
						odds = 10;
						strcpy(out_val, "");
						get_string("Pick a number (0-9): ", out_val, 32); /* BUG -- TNB */
						for (p = out_val; *p == ' '; p++);
						choice = atol(p);
						if (choice < 0)
						{
						...
						msg_print(NULL);
						roll1 = randint(10) - 1;
						angtk_eval("angband_building", "wheel", format("%d %d",
								choice, roll1), NULL);
						if (roll1 == choice)
							win = TRUE;
#else /* TNB */
					...
					case BACT_DICE_SLOTS: /* The Dice Slots */
						...
						angtk_eval("angband_building", "diceslots",
							format("%d %d %d", roll1, roll2, choice), NULL);
				...
				{
					prt("YOU WON", 16, 37);
					angtk_eval("angband_building", "winner", format("%ld %d",
							odds * wager, odds), NULL);	/* TNB */
					p_ptr->au += odds * wager;
...
static void get_questinfo(int questnum)
{
		...
		(quest[questnum].status == QUEST_STATUS_TAKEN))
	{
		...
		if (r_ptr->flags1 & RF1_UNIQUE)
		{
			sprintf(tmp_str, "Kill %s on dungeon level %d.", name,
				quest[questnum].level);
		}
		else
		{
		...
		angtk_eval("angband_building", "quest", "info", quest[questnum].name,
			format("%d", quest[questnum].level), tmp_str, NULL);
		any_more(NULL);
#endif /* TNB */
	}
	else
...
#if 1 /* TNB */
		buf[0] = '\0';
		(void) strcat(buf, "You have not completed your current quest yet!\n");
		(void) strcat(buf, "Use CTRL-Q to check the status of your quest.\n\n");
...
static void show_highclass(int building)
{
	...
	while ((m < 9) || (j < MAX_HISCORES))
	{
			...
			((building == 2) && (al > MAX_ARENA_MONS)))
		{
			sprintf(out_val, "%3d) %s the %s (Level %2d)", (m + 1),
				the_score.who, p_name + rp_ptr->name, clev);
			prt(out_val, (m + 7), 0);
			m++;
		}
		j++;
	...
	{
		sprintf(out_val, "You) %s the %s (Level %2d)", op_ptr->full_name,
			p_name + rp_ptr->name, p_ptr->lev);
		prt(out_val, (m + 8), 0);
	}
...
static void race_score(int race_num)
{
	...
	{
		sprintf(out_val, "You) %s the %s (Level %3d)", op_ptr->full_name,
			p_name + rp_ptr->name, p_ptr->lev);
		prt(out_val, (m + 8), 0);
	}
...
void compare_weapon_aux2(object_type * o_ptr, int mult, char *flag)
{
	angtk_eval("angband_building", "compare_weapons", "flag", flag,
		format("%d", mult), NULL);
}
 ...
 */
void compare_weapon_aux1(object_type * o_ptr, int col, int row)
{
	u32b f1, f2, f3;
	...
	if (f1 & (TR1_SLAY_ANIMAL))
		compare_weapon_aux2(o_ptr, 2, "SLAY_ANIMAL");
	if (f1 & (TR1_SLAY_EVIL))
		compare_weapon_aux2(o_ptr, 2, "SLAY_EVIL");
	if (f1 & (TR1_SLAY_UNDEAD))
		compare_weapon_aux2(o_ptr, 3, "SLAY_UNDEAD");
	if (f1 & (TR1_SLAY_DEMON))
		compare_weapon_aux2(o_ptr, 3, "SLAY_DEMON");
	if (f1 & (TR1_SLAY_ORC))
		compare_weapon_aux2(o_ptr, 3, "SLAY_ORC");
	if (f1 & (TR1_SLAY_TROLL))
		compare_weapon_aux2(o_ptr, 3, "SLAY_TROLL");
	if (f1 & (TR1_SLAY_GIANT))
		compare_weapon_aux2(o_ptr, 3, "SLAY_GIANT");
	if (f1 & (TR1_SLAY_DRAGON))
		compare_weapon_aux2(o_ptr, 3, "SLAY_DRAGON");
	if (f1 & (TR1_KILL_DRAGON))
		compare_weapon_aux2(o_ptr, 5, "KILL_DRAGON");
	if (f1 & (TR1_BRAND_ACID))
		compare_weapon_aux2(o_ptr, 3, "BRAND_ACID");
	if (f1 & (TR1_BRAND_ELEC))
		compare_weapon_aux2(o_ptr, 3, "BRAND_ELEC");
	if (f1 & (TR1_BRAND_FIRE))
		compare_weapon_aux2(o_ptr, 3, "BRAND_FIRE");
	if (f1 & (TR1_BRAND_COLD))
		compare_weapon_aux2(o_ptr, 3, "BRAND_COLD");
	if (f1 & (TR1_BRAND_POIS))
		compare_weapon_aux2(o_ptr, 3, "BRAND_POIS");
	if (f1 & (TR1_FORCE))
		compare_weapon_aux2(o_ptr, 3, "FORCE");
}

/*
 * list_weapon -KMW-
 */
void list_weapon(object_type * o_ptr, int row, int col)
{
	...
	angtk_eval("angband_building", "compare_weapons", "list_weapon",
		o_name, format("%d %d %d %d %d %d %d", o_ptr->to_h, o_ptr->to_d,
			o_ptr->dd, o_ptr->ds, p_ptr->num_blow, p_ptr->to_h,
			p_ptr->to_d), NULL);
}
...
static void compare_weapon_aux2(object_type * o_ptr, int numblows, int r,
	int c, int mult, char attr[80], u32b f1, u32b f2, u32b f3, byte color)
{
	char tmp_str[80];
...
static void compare_weapon_aux1(object_type * o_ptr, int col, int r)
{
	...
	if (f1 & (TR1_FORCE))
		compare_weapon_aux2(o_ptr, p_ptr->num_blow, r++, col, 3, "Force:",
			f1, f2, f3, TERM_RED);
}
...
static void list_weapon(object_type * o_ptr, int row, int col)
{
	...
	sprintf(tmp_str, "To Hit: %d   To Damage: %d", o_ptr->to_h,
		o_ptr->to_d);
 ...
 */
/* From ZAngband - KAngband version is fried */
...
static bool enchant_item(int cost, int to_hit, int to_dam, int to_ac)
{
	...
	angtk_eval("angband_building", "enchant_item", "init2",
		format("The price for the service is %d gold per item.", cost), NULL);
#else /* TNB */
	clear_bldg(5, 18);
...
static void bldg_process_command(building_type * bldg, int i)
{
...

	{
		switch (bact)
		{
			...
			case BACT_UNUSED_4:
				msg_format
					("This function (%d) has not been implemented yet.",
					bact);
				message_flush();
				break;
		}
...
void do_cmd_bldg(void)
{
	...
	g_buildingdata.building_loc = building_loc;
