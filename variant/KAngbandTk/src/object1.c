...
static cptr ring_adj[MAX_ROCKS] = {
	...
	"Copper", "Tanzanite", "Iron", "Steel",
	"Wood", "Star Sapphire", "Diamond",
...
static byte ring_col[MAX_ROCKS] = {
	...
	TERM_UMBER, TERM_VIOLET, TERM_SLATE, TERM_WHITE,
	TERM_UMBER, TERM_BLUE, TERM_BLUE,
 ...
 */
static void object_flags_aux(int mode, const object_type * o_ptr,
	u32b * f1, u32b * f2, u32b * f3)
{
	...
	if (mode != OBJECT_FLAGS_FULL)
	{
		...
		if (artifact_p(o_ptr) || cheat_iden)
			spoil = TRUE;
#endif /* SPOIL_ARTIFACTS */

#ifdef SPOIL_EGO_ITEMS
		/* Full knowledge for some ego-items */
		if (ego_item_p(o_ptr) || cheat_iden)
			spoil = TRUE;
#endif /* SPOIL_ARTIFACTS */

		/* Need full knowledge or spoilers */
		if (!spoil && !(o_ptr->ident & IDENT_MENTAL) && !cheat_iden)
			return;
		...
		if (!(o_ptr->ident & IDENT_MENTAL) && !cheat_iden)
			return;
	}
 ...
 * Obtain the "flags" for an item
 */
void object_flags(const object_type * o_ptr, u32b * f1, u32b * f2,
	u32b * f3)
{
...
cptr item_activation(const object_type * o_ptr)
{
	...
	if (o_ptr->name1)
	{
		...
		if (a_ptr->time && a_ptr->randtime)
			sprintf(turns, " every %d+d%d turns", a_ptr->time,
				a_ptr->randtime);
		else if (a_ptr->time)
	...
	cptr * info, int len)
{
	...
	if (f1 & (TR1_FORCE)) /* -KMW- */
	{
		info[i++] = "It ripples outward with force.";
	}

	if (f3 & (TR3_CHAOTIC))	/* -KMW- */
	{
		info[i++] = "It spreads chaos.";
	}

	if (f1 & (TR1_VORPAL)) /* -KMW- */
	{
		info[i++] = "It is magically sharp.";
	}

	if (f3 & (TR3_VAMPIRIC)) /* -KMW- */
	{
		info[i++] = "It drains life from others into you.";
	}

	if (f3 & (TR3_LEVITATION)) /* -KMW- */
	{
	...
	if (f3 & (TR3_GHOSTLY))
	{
		info[i++] = "It allows you to pass through walls.";
	}
	if (f3 & (TR3_INVISIBLE))
	{
...
bool get_item(int *cp, cptr pmt, cptr str, int mode)
{
	...
	int choose_what = -1;
	int allow = 0;
	cptr saved_options = inkey_options;
#endif
	...
	/* Get the item index */
	if (repeat_pull(cp) && repeat_pull(&repeat_k_idx))
	{
		/* Verify the item */
		if (get_item_okay(*cp))
		{
...
#if 1 /* TNB */
	if (i1 <= i2) allow |= USE_INVEN;
	if (e1 <= e2) allow |= USE_EQUIP;
	if (allow_floor) allow |= USE_FLOOR;
#endif /* TNB */
	...
	else
	{
		...
		else if (easy_floor && allow_floor)
		{
	...
	while (!done)
	{
		...
		if (choose_what != p_ptr->command_wrk)
		{
			inkey_options = saved_options;

			switch (p_ptr->command_wrk)
			{
				case USE_INVEN:
					Bind_Choose(KEYWORD_CHOOSE_ITEM + 1, 0 | (allow << 8), TRUE);
					break;

				case USE_EQUIP:
					Bind_Choose(KEYWORD_CHOOSE_ITEM + 1, 1 | (allow << 8), TRUE);
					break;

				case USE_FLOOR:
					Bind_Choose(KEYWORD_CHOOSE_ITEM + 1, 2 | (allow << 8), TRUE);
					break;
			}

			choose_what = p_ptr->command_wrk;
			inkey_options = NULL;
		}
		...
		switch (which)
		{
			...
			case '-':
			{
				...
				for (i = 0; i < floor_num; ++i)
				{
					...
					if (floor_query_flag && !verify_item("Try", k))
						continue;
	...
	if (p_ptr->command_see)
	{
		...
		angtk_eval("angband_display",
			(p_ptr->command_wrk == (USE_EQUIP))
				? "equip"
				: (p_ptr->command_wrk == (USE_INVEN)) ? "inven" : "floor",
			"hide", NULL); /* TNB */
