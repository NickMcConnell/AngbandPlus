...
void lore_treasure(int m_idx, int num_item, int num_gold)
{
	...
	if (num_item > l_ptr->r_drop_item)
		l_ptr->r_drop_item = num_item;
	if (num_gold > l_ptr->r_drop_gold)
		l_ptr->r_drop_gold = num_gold;

	/* Hack -- memorize the good/great flags */
	if (r_ptr->flags1 & (RF1_DROP_GOOD))
		l_ptr->r_flags1 |= (RF1_DROP_GOOD);
	if (r_ptr->flags1 & (RF1_DROP_GREAT))
		l_ptr->r_flags1 |= (RF1_DROP_GREAT);
...
void update_mon(int m_idx, bool full)
{
	...
	if (p_ptr->wizard && wiz_see_monsters)
		flag = TRUE; /* TNB */
			...
			else if (r_ptr->flags2 & (RF2_WEIRD_MIND))
			{
				...
				if ((m_idx % 10) == 5)
				{
					...
					if (r_ptr->flags2 & (RF2_SMART))
						l_ptr->r_flags2 |= (RF2_SMART);
					if (r_ptr->flags2 & (RF2_STUPID))
						l_ptr->r_flags2 |= (RF2_STUPID);
				}
			}
	...
	if (easy)
	{
		...
		if (!(m_ptr->mflag & (MFLAG_VIEW)))
		{
			...
			if (disturb_near && (p_ptr->command_rep || p_ptr->running ||
				p_ptr->resting))
			{
				sound(SNDGRP_EVENT, (r_ptr->flags1 & RF1_UNIQUE)
					? SND_DISTURB_UNIQUE : SND_DISTURB, 0x02);
			}
	...
	else
	{
		...
		if (m_ptr->mflag & (MFLAG_VIEW))
		{
			...
			/* Disturb on disappearance */
			if (disturb_near && !m_ptr->is_friendly)
				disturb(1, 0);
		}
	}
	...
	if (p_ptr->target_set && (p_ptr->target_who == m_idx))
	{
		target_vis = flag &&
			projectable(p_ptr->py, p_ptr->px, m_ptr->fy, m_ptr->fx);
...
s16b monster_carry(int m_idx, object_type * j_ptr)
{
		...
		this_o_idx = next_o_idx)
	{
		...
		{
			/* Uh-oh: Restrict the total value to 16-bit */
			if ((u32b) o_ptr->pval + j_ptr->pval > SHRT_MAX)
				continue;
 ...
 */
#if 1 /* TNB */
bool place_monster_one(int y, int x, int r_idx, bool slp, bool pet)
#else /* TNB */
	...
	bool pet)
{
	...
	if (!place_monster_one(y, x, r_idx, slp, pet))
		return (FALSE);
...
bool place_monster(int y, int x, bool slp, bool grp)
{
	...
	if (place_monster_aux(y, x, r_idx, slp, grp, 0))
		return (TRUE);
...
bool alloc_aquatic_monster(int dis, bool slp)
{
	...
	/* Attempt to place the monster, allow groups */
	if (place_aquatic_monster(y, x, slp, TRUE))
		return (TRUE);
...
bool summon_specific(int y1, int x1, int lev, int type, bool pet)
{
	...
	if (!place_monster_aux(y, x, r_idx, FALSE, TRUE, pet))
		return (FALSE);
...
bool multiply_monster(int m_idx)
{
	...
	for (i = 0; i < 18; i++)
	{
		...
		result =
			place_monster_aux(y, x, m_ptr->r_idx, FALSE, FALSE, make_pets);
