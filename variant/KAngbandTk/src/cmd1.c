...
bool test_hit_fire(int chance, int ac, int vis)
{
	...
	if ((chance > 0) && (rand_int(chance) >= (ac * 3 / 4)))
		return (TRUE);
...
sint critical_shot(int weight, int plus, int dam)
{
	...
	if (randint(5000) <= i)
	{
		...
		if (k < 500)
		{
			sound(SNDGRP_EVENT, SND_ATTACK_GOOD, 0); /* ALLOW_SOUND */
			msg_print("It was a good hit!");
			dam = 2 * dam + 5;
		}
		else if (k < 1000)
		{
			sound(SNDGRP_EVENT, SND_ATTACK_GREAT, 0); /* ALLOW_SOUND */
			msg_print("It was a great hit!");
			dam = 2 * dam + 10;
		}
		else
		{
			sound(SNDGRP_EVENT, SND_ATTACK_SUPERB, 0); /* ALLOW_SOUND */
...
sint critical_norm(int weight, int plus, int dam)
{
	...
	if (randint(5000) <= i)
	{
		...
		{
			sound(SNDGRP_EVENT, SND_ATTACK_GREAT2, 0); /* ALLOW_SOUND */
			msg_print("It was a *GREAT* hit!");
			dam = 3 * dam + 20;
		}
		else
		{
			sound(SNDGRP_EVENT, SND_ATTACK_SUPERB2, 0);	/* ALLOW_SOUND */
			msg_print("It was a *SUPERB* hit!");
			dam = ((7 * dam) / 2) + 25;
		}
	}
 ...
 */
sint tot_dam_aux(const object_type * o_ptr, int tdam,
		...
		for (x = (px - 1); x <= (px + 1); x++)
		{
			...
			if (rand_int(100) < chance)
			{
				...
				if (cave_feat[y][x] == FEAT_INVIS)
				{
					...
					/* Message */
					sound(SNDGRP_EVENT, SND_DISCOVER, 0); /* ALLOW_SOUND */
...
bool auto_pickup_okay(object_type *o_ptr)
{
	...
	for (j = 0; j < INVEN_PACK; j++)
	{
		...
		/* Option: Auto-pickup similar objects */
		if (pickup_similar)
			return (TRUE);
...
static bool auto_pickup_okay(const object_type * o_ptr)
{
	...
	if (k_info[o_ptr->k_idx].auto_pick)
		return (TRUE);
...
static void py_pickup_aux(int o_idx)
{
	...
	/* Message */
	sound(SNDGRP_EVENT, SND_PACK_PICKUP, 0); /* ALLOW_SOUND */
...
static void auto_destroy_items(int py, int px)
{
		...
		this_o_idx = next_o_idx)
	{
...
#endif /* TNB */
		{
			...
			/* Print a message */
			sound(SNDGRP_EVENT, SND_OBJECT_AUTO_DESTROYED, 0x01); /* ALLOW_SOUND */
...
void py_pickup(int pickup)
{
		...
		this_o_idx = next_o_idx)
	{
		...
		if (o_ptr->tval == TV_GOLD)
		{
			...
			if (o_ptr->pval > 600)
				sound(SNDGRP_EVENT, SND_MONEY_600, 0);
			else if (o_ptr->pval > 200)
				sound(SNDGRP_EVENT, SND_MONEY_200, 0);
			else
				sound(SNDGRP_EVENT, SND_MONEY, 0);
		...
		if (!inven_carry_okay(o_ptr))
		{
			...
			/* Play a sound if it isn't annoying */
			if (!always_pickup)
			{
				sound(SNDGRP_EVENT, SND_ACTION_FAILED, 0);
			}
...
static int check_hit(int power)
{
	...
	/* Power competes against Armor */
	if ((power > 0) && (randint(power) >= (ac * 3 / 4)))
		return (TRUE);
...
void hit_trap(int y, int x)
{
	...
	switch (cave_feat[y][x])
	{
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_DOOR, 0); /* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_PIT_OPEN, 0); /* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_PIT_SPIKED, 0); /* ALLOW_SOUND */
		...
		case FEAT_TRAP_HEAD + 0x03:
		{
			...
			else
			{
				...
				if (rand_int(100) < 50)
				{
					sound(SNDGRP_EVENT, SND_TRAP_PIT_POISON, 0); /* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_RUNE_SUMMON, 0); /* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_RUNE_TELEPORT, 0);	/* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_SPOT_FIRE, 0);	/* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_SPOT_ACID, 0);	/* ALLOW_SOUND */
		...
		case FEAT_TRAP_HEAD + 0x08:
		{
			if (check_hit(125))
			{
				sound(SNDGRP_EVENT, SND_TRAP_DART_SLOW, 0);	/* ALLOW_SOUND */
			...
			{
				sound(SNDGRP_EVENT, SND_TRAP_MISS, 0); /* ALLOW_SOUND */
		...
		case FEAT_TRAP_HEAD + 0x09:
		{
			if (check_hit(125))
			{
				sound(SNDGRP_EVENT, SND_TRAP_DART_STRENGTH, 0);	/* ALLOW_SOUND */
				msg_print("A small dart hits you!");
				dam = damroll(1, 4);
				take_hit(dam, name);
		...
		case FEAT_TRAP_HEAD + 0x0A:
		{
			if (check_hit(125))
			{
				sound(SNDGRP_EVENT, SND_TRAP_DART_DEXTERITY, 0); /* ALLOW_SOUND */
				msg_print("A small dart hits you!");
				dam = damroll(1, 4);
				take_hit(dam, name);
		...
		case FEAT_TRAP_HEAD + 0x0B:
		{
			if (check_hit(125))
			{
				sound(SNDGRP_EVENT, SND_TRAP_DART_CONSTITUTION, 0);	/* ALLOW_SOUND */
				msg_print("A small dart hits you!");
				dam = damroll(1, 4);
				take_hit(dam, name);
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_GAS_BLIND, 0);	/* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_GAS_CONFUSE, 0); /* ALLOW_SOUND */
			msg_print
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_GAS_POISON, 0); /* ALLOW_SOUND */
		...
		{
			sound(SNDGRP_EVENT, SND_TRAP_GAS_SLEEP, 0);	/* ALLOW_SOUND */
...
void py_attack(int y, int x)
{
	...
	/* Disturb the monster */
#if 1 /* TNB */
	monster_disturb(cave_m_idx[y][x]);
#else
	m_ptr->csleep = 0;
#endif
	...
	while (num++ < p_ptr->num_blow)
	{
		...
		{
#if 1 /* TNB */
			sound(SNDGRP_EVENT, SND_ATTACK_HIT, 0);	/* ALLOW_SOUND */
			...
			if (mon_take_hit(cave_m_idx[y][x], k, &fear, NULL, FALSE))
				break;
		...
		{
			sound(SNDGRP_EVENT, SND_ATTACK_MISS, 0); /* ALLOW_SOUND */
...
void move_player(int dir, int jumping)
{
		...
		(!cave_perma_bold(y, x)))
	{
		...
		else
		{
			...
			{
				message(MSG_HITWALL, 0,
					"There is a damn wall blocking your way.");
			}
		}
	}
...
#if 1 /* BUG -- TNB */
	if (cave_feat[y][x] == FEAT_CHASM && !p_ptr->levitate)
		return (TRUE);
#endif /* BUG -- TNB */

	/* Don't treat grass and dirt as walls when running */
	if ((cave_feat[y][x] >= FEAT_GRASS) && (cave_feat[y][x] <= FEAT_DIRT))
		return (FALSE);
 ...
 */
static const byte chome[] = { 0, 8, 9, 10, 7, 0, 11, 6, 5, 4 };
