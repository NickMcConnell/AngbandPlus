...
static bool make_ego_item(object_type * o_ptr, bool cursed)
{
	...
	for (i = 0; i < alloc_ego_size; i++)
	{
		...
		if (!cursed && (e_ptr->flags3 & TR3_LIGHT_CURSE))
			continue;
		if (cursed && !(e_ptr->flags3 & TR3_LIGHT_CURSE))
			continue;
	...
	bool great)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_BOLT:
		{
			...
			if (((power > 1) ? TRUE : FALSE) ||
				((power < -1) ? TRUE : FALSE))
				(void) make_ego_item(o_ptr,
					(bool) ((power < 0) ? TRUE : FALSE));
			break;
		}
	...
	if (o_ptr->name2)
	{
		...
		if (e_ptr->xtra)
		{
			...
			switch (o_ptr->xtra1)
			{
				...
				{
					o_ptr->xtra2 =
						(byte) rand_int(OBJECT_XTRA_SIZE_SUSTAIN);
					break;
				}
	...
	switch (k_ptr->tval)
	{
		...
		case TV_RING:
		{
			if (k_ptr->sval == SV_RING_SPEED)
				return (TRUE);
			if (k_ptr->sval == SV_RING_LORDLY)
				return (TRUE);
			if (k_ptr->sval == SV_RING_ATTACKS)
				return (TRUE);
			return (FALSE);
		}
...
#if 1 /* Icons may change based on number in a group! -- TNB */
			/* Notice */
			note_spot(y, x);
...
s16b inven_carry(object_type * o_ptr)
{
	...
	if (i < INVEN_PACK)
	{
		...
		for (j = 0; j < INVEN_PACK; j++)
		{
			...
			if (!object_aware_p(o_ptr) && !cheat_iden)
				continue;
			if (!object_aware_p(j_ptr) && !cheat_iden)
				break;
			...
			if (!object_known_p(o_ptr) && !cheat_iden)
				continue;
			if (!object_known_p(j_ptr) && !cheat_iden)
				break;
...
s16b inven_takeoff(int item, int amt)
{
	...
	/* Message */
	sound(SNDGRP_EVENT, SND_PACK_REMOVE, 0); /* ALLOW_SOUND */
...
void print_spells(const byte * spells, int num, int y, int x)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if ((spell <
				32) ? ((p_ptr->
			   spell_forgotten1 & (1L << spell))) : ((p_ptr->
			   spell_forgotten2 & (1L << (spell - 32)))))
		{
			comment = " forgotten";
		...
		sprintf(out_val, "  %c) %-30s%2d %4d %3d%%%s", I2A(i),
			spell_names[mp_ptr->spell_type][spell], s_ptr->slevel,
			s_ptr->smana, spell_chance(spell), comment);
...
#ifdef TNB_SQUELCH
bool object_squelch_p(object_type *o_ptr)
{
	...
	/* See if the object is "sensed" */
	bool sensed = (sensed_p(o_ptr) ? TRUE : FALSE);

	byte feeling = INSCRIP_NULL;
	...
	switch (o_ptr->tval)
	{
		...
		case TV_DRAG_ARMOR:
			if (known)
				...
				feeling = o_ptr->discount;
			break;
			
		default:
			if (known)
				feeling = INSCRIP_AVERAGE;
			break;
	}
	...
	switch (feeling)
	{
		...
		case INSCRIP_CURSED:
			if (k_info[o_ptr->k_idx].squelch >= SQUELCH_CURSED)
				return TRUE;
			break;
		case INSCRIP_UNCURSED:
		case INSCRIP_AVERAGE:
			if (k_info[o_ptr->k_idx].squelch >= SQUELCH_AVERAGE)
				return TRUE;
			break;
		case INSCRIP_GOOD:
			if (k_info[o_ptr->k_idx].squelch >= SQUELCH_GOOD)
				return TRUE;
			break;
		case INSCRIP_EXCELLENT:
			if (k_info[o_ptr->k_idx].squelch >= SQUELCH_EXCELLENT)
				return TRUE;
			break;
		default:
			/* Squelch "all" will squelch even unidentified stuff.
			 * This is good for Amulets of Charisma or digging tools. */
			if (k_info[o_ptr->k_idx].squelch == SQUELCH_ALL)
				return TRUE;
			break;
	}
