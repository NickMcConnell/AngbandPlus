...
static errr rd_savefile_new_aux(void)
{
	...
	if (older_than_extra(VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH,
		VERSION_EXTRA))	/* TNB */
	...
	{
		note
			("Kangband no longer supports savefile older than version 2.8.3.");
		return (-1);
	}
	...
	if (!g_grid_xtra_init)
	{
		...
		op_ptr->opt[OPT_easy_ring] = option_norm[OPT_easy_ring];
		op_ptr->opt[OPT_verify_destroy_junk] =
			option_norm[OPT_verify_destroy_junk];
		op_ptr->opt[OPT_verify_mana] =
			option_norm[OPT_verify_mana];
	}
...
static errr rd_tnb(void)
{
	...
	/* First check that the file version is 2.8.3.9 or greater. */
	if (older_than_extra(2, 8, 3, 9))
		return (0);
