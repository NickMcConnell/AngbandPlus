...
void do_cmd_zap_rod(void)
{
	...
	if (((o_ptr->sval >= SV_ROD_MIN_DIRECTION) &&
			(o_ptr->sval < SV_ROD_MIN_ARTIFACT)) || !object_aware_p(o_ptr))
	{
...
void do_cmd_activate(void)
{
	...
	if (o_ptr->name1)
	{
		...
		switch (a_ptr->activation)
		{
			...
			case ACT_DISP_EVIL:
			{
				msg_format("The %s floods the area with goodness...",
					o_name);
			...
			case ACT_STAR_BALL:
			{
				msg_format("Your %s is surrounded by lightning...",
					o_name);
				for (i = 0; i < 8; i++)
			...
			{
				msg_format("Your %s lets out a long, shrill note...",
					o_name);
				(void) mass_genocide();
				break;
			}
			...
			case ACT_CONFUSE:
			{
				msg_format("Your %s glows in scintillating colours...",
					o_name);
				if (!get_aim_dir(&dir))
					return;
			...
			case ACT_FIRESTORM:
			{
				msg_format("Your %s erupts in fire...", o_name);
				if (!get_aim_dir(&dir))
					return;
	...
	if (o_ptr->tval == TV_DRAG_ARMOR)
	{
		...
		switch (o_ptr->sval)
		{
			...
			case SV_DRAGON_LAW:
			{
				...
				fire_ball((chance == 1 ? GF_SOUND : GF_SHARD), dir, 230,
					2);
				o_ptr->timeout = rand_int(300) + 300;
				break;
			}
