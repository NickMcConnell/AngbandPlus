...
static int detection_aux(cptr what)
{
	...
	int range = 30; /* was MAX_RANGE, now agrees with OAngband && Z+Angband */
...
bool recharge(int num)
{
	...
	else
	{
		...
		if ((i <= 1) || (rand_int(i) == 0))
		{
			...
			sound(SNDGRP_EVENT, (item >= 0)
				? SND_PACK_DESTROY
				: SND_OBJECT_DESTROYED, 0); /* ALLOW_SOUND */
...
void destroy_area(int y1, int x1, int r, bool full)
{
	...
	if (flag)
	{
		...
		angtk_destroy_area(2); /* TNB */
		msg_print(NULL); /* TNB */
		angtk_widget_lock(FALSE); /* TNB */
		angtk_destroy_area(3); /* TNB */
...
#if 1 /* TNB */
					monster_disturb(cave_m_idx[yy][xx]);
#else
					m_ptr->csleep = 0;
#endif
...
bool fear_monsters_touch(void)
{
	...
	return (project(0, 1, p_ptr->py, p_ptr->px, p_ptr->lev, GF_TURN_ALL,
			flg));
}
