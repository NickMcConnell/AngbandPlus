...
#endif /* TNB */
{
	...
	if ((mp_ptr->spell_book == TV_ILLUSION_BOOK) ||
		(mp_ptr->spell_book == TV_MAGIC_BOOK))
	{
		p = "spell";
	}
#else /* BUG -- TNB */
	...
	if (repeat_pull(sn))
	{
		...
		if ((spell < 32)
			...
			: (spell_flags[mp_ptr->spell_type][sval][1] & (1L << (spell -
				32))))
		{
	...
	for (spell = 0; spell < PY_MAX_SPELLS; spell++)
	{
		...
		if ((spell <
				32) ? (spell_flags[mp_ptr->
			   spell_type][sval][0] & (1L << spell))
			: (spell_flags[mp_ptr->spell_type][sval][1] & (1L << (spell -
						32))))
		{
			/* Collect this spell */
	...
	while (!flag && get_com(out_val, &choice))
	{
		...
		if (verify)
		{
			...
			strnfmt(tmp_val, 78, "%^s %s (%d mana, %d%% fail)? ", prompt,
				spell_names[mp_ptr->spell_type][spell], s_ptr->smana,
				spell_chance(spell));
...
void do_cmd_browse(void)
{
	...
	prompt_print("(Browsing) Command:"); /* TNB */
	angtk_eval("angband_display", "book", "show", format("%d", inkey_book), NULL); /* TNB */
 ...
 */
static bool item_tester_hook_book(const object_type * o_ptr, bool known)
{
	int spell;
	...
	for (spell = 0; spell < 64; spell++)
	{
		/* Check for this spell */
		if ((spell <
				32) ? (spell_flags[mp_ptr->spell_type][o_ptr->
			   sval][0] & (1L << spell)) : (spell_flags[mp_ptr->
					spell_type][o_ptr->sval][1] & (1L << (spell - 32))))
		{
			/* Look for "okay" spells */
			if (spell_okay(spell, known))
				return (TRUE);
 ...
 */
bool item_tester_hook_cast(const object_type * o_ptr)
{
 ...
 */
bool item_tester_hook_study(const object_type * o_ptr)
{
...
void do_cmd_study(void)
{
	...
	if ((mp_ptr->spell_book == TV_MAGIC_BOOK) || (mp_ptr->spell_book == TV_ILLUSION_BOOK)) /* added -KMW- */
	{
...
static void do_cmd_cast_spell(void)
{
	...
	/* Ask for a spell */
#if 1 /* TNB */
	if (!get_spell(&spell, "cast", o_ptr->k_idx, TRUE))
#else /* TNB */
	...
	else
	{
		...
		switch (spell)
		{
			...
			case SPELL_SPEAR_OF_LIGHT:
			{
				...
				fire_beam(GF_LITE, dir, damroll(2 + ((plev - 5) / 4), 6));
				lite_line(dir);
				break;
			}
			...
			case SPELL_TELEPORT_OTHER:
			{
				...
				{
					msg_print
						("Your time/space anchor prevents it from leaving.");
					break;
				}
				if (!get_aim_dir(&dir))
					return;
			...
			case SPELL_TELEPORT_LEVEL:
			{
				...
				{
					msg_print
						("Your time/space anchor prevents you from leaving.");
					break;
				}
			...
			case SPELL_PLASMA_BOLT:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt(GF_PLASMA, dir, damroll(10 + ((plev - 5) / 4),
						8));
				break;
			}
			...
			{
				(void) set_tim_ts_anchor(p_ptr->ts_anchor + randint(100) +
					p_ptr->lev);
				break;
			}
...
static void do_cmd_pray_priest(void)
{
	...
	/* Choose a spell */
#if 1 /* TNB */
	if (!get_spell(&spell, "recite", o_ptr->k_idx, TRUE))
#else /* TNB */
	...
	else
	{
		...
		switch (spell)
		{
			...
			{
				(void) set_tim_levitate(p_ptr->tim_levitate + randint(50) +
					50);
				break;
			}
			...
			case PRAYER_ORB_OF_DRAINING:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_HOLY_ORB, dir, (damroll(3,
							6) + plev + (plev / ((p_ptr->pclass ==
									2) ? 2 : 4))), ((plev < 30) ? 2 : 3));
				break;
			}
			...
			{
				(void) set_oppose_nethr(p_ptr->oppose_nethr + randint(20) +
					20);
				break;
			}
			...
			case PRAYER_BALEFIRE:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_beam(GF_BALEFIRE, dir, damroll(15 + ((plev - 5) / 3),
						10));
				break;
			}
			...
			case PRAYER_IMMOLATION:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_FIRE, dir, 100 + (plev * 2), 3);
				break;
			}
		}
...
static void do_cmd_cast_illusion(void)
{
	...
	else
	{
		...
		/* Hack -- chance of "beam" instead of "bolt" */
		beam = ((p_ptr->pclass == CLASS_ILLUSIONIST) ? plev : (plev / 2));
		...
		switch (spell)
		{
			...
			case ILLUS_FOG_CLOUD:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_FOG, dir, 10 + (plev / 2), 3);
				break;
			}
			...
			case ILLUS_SHADOW_MONSTER:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt(GF_FORCE, dir, damroll(5 + ((plev - 6) / 4), 8));
				break;
			}
			...
			case ILLUS_CHAOS:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_CHAOS, dir, 25 + plev, 2);
				break;
			}

			case ILLUS_MENTAL_BARRIER:
			{
				(void) set_tim_sus_int(p_ptr->tim_sus_int + randint(50) +
					50);
				(void) set_tim_sus_wis(p_ptr->tim_sus_wis + randint(50) +
					50);
				break;
			}
			...
			{
				(void) set_tim_ghostly(p_ptr->tim_ghostly + plev +
					randint(24));
				(void) set_tim_invis(p_ptr->tim_invis + plev +
					randint(24));
				break;
			}
			...
			case ILLUS_BEDLAM:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_CHAOS, dir, 50 + plev, 10);
				break;
			}
			...
			case ILLUS_SUNFIRE:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_LITE, dir, 50 + plev, 10);
				break;
			}
			...
			case ILLUS_CLENCHED_FIST:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_beam(GF_FORCE, dir, damroll(10 + ((plev - 5) / 4),
						8));
				break;
			}
			...
			case ILLUS_CRUSHING_HAND:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_bolt(GF_GRAVITY, dir, damroll(12 + ((plev - 5) / 4),
						8));
				break;
			}
			...
			case ILLUS_FORCE_BLAST:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_FORCE, 5, 300 + (plev * 2), 3);
				break;
			}
			...
			case ILLUS_SPHERE_OF_LIGHT:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_LITE, dir, 30 + (plev), 2);
				break;
			}
			...
			case ILLUS_SPHERE_OF_CONFUSION:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_CONFUSION, dir, 40 + (plev), 2);
				break;
			}
			...
			case ILLUS_SPHERE_OF_CHAOS:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_CHAOS, dir, 45 + (plev), 2);
				break;
			}
			...
			case ILLUS_SPHERE_OF_SOUND:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_SOUND, dir, 50 + (plev), 2);
				break;
			}
			...
			case ILLUS_EXPLOSION:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_SHARD, dir, 80 + (plev), 2);
				break;
			}
			...
			{
				(void) set_oppose_nexus(p_ptr->oppose_nexus + randint(20) +
					20);
				break;
			}
			...
			case ILLUS_SHADOW_MONSTERS:
			{
				...
				fire_beam(GF_FORCE, dir, damroll(6 + ((plev - 5) / 4), 8));
				break;
			}
			...
			case ILLUS_SHADOW_BALL_II:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_DARK, dir, 40 + (plev), 2);
				break;
			}
...
static void do_cmd_pray_druid(void)
{
	...
	else
	{
		...
		/* Hack -- chance of "beam" instead of "bolt" */
		beam = ((p_ptr->pclass == CLASS_DRUID) ? plev : (plev / 2));
		...
		switch (spell)
		{
			...
			{
				fire_ball(GF_FIRE, 5, 35 + (plev * 2), 10);
				break;
			}
			...
			case DRUID_NEXUS_BALL:
			{
				if (!get_aim_dir(&dir))
					return;
				fire_ball(GF_NEXUS, dir, 70 + (plev), 2);
				break;
			}
			...
			{
				(void) set_tim_sus_str(p_ptr->tim_sus_str + randint(50) +
					50);
				(void) set_tim_sus_int(p_ptr->tim_sus_int + randint(50) +
					50);
				(void) set_tim_sus_wis(p_ptr->tim_sus_wis + randint(50) +
					50);
				(void) set_tim_sus_dex(p_ptr->tim_sus_dex + randint(50) +
					50);
				(void) set_tim_sus_con(p_ptr->tim_sus_con + randint(50) +
					50);
				(void) set_tim_sus_chr(p_ptr->tim_sus_chr + randint(50) +
					50);
				break;
			}
...
void do_cmd_cast(void)
{
	...
	else if (mp_ptr->spell_book == TV_ILLUSION_BOOK)
	{
...
void do_cmd_pets(void)
{
	...
	if (p_ptr->pet_follow_dist == PET_CLOSE_DIST)
		mode = num;
	powers[num++] = PET_STAY_CLOSE;

	power_desc[num] = "follow me";
	if (p_ptr->pet_follow_dist == PET_FOLLOW_DIST)
		mode = num;
	powers[num++] = PET_FOLLOW_ME;

	power_desc[num] = "seek and destroy";
	if (p_ptr->pet_follow_dist == PET_DESTROY_DIST)
		mode = num;
	powers[num++] = PET_SEEK_AND_DESTROY;

	power_desc[num] = "give me space";
	if (p_ptr->pet_follow_dist == PET_SPACE_DIST)
		mode = num;
	powers[num++] = PET_ALLOW_SPACE;

	power_desc[num] = "stay away";
	if (p_ptr->pet_follow_dist == PET_AWAY_DIST)
		mode = num;
