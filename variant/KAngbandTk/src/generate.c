...
static void cave_gen(void)
{
	...
	if ((p_ptr->depth <= 2) && (randint(20) > 15))
	{
		...
		{
			if (cheat_room)
				msg_print("Terrain Streamer: Trees");
	...
	else if ((p_ptr->depth <= 19) && (randint(20) > 15))
	{
		...
		{
			if (cheat_room)
				msg_print("Terrain Streamer: Shallow water");
		...
		if (randint(20) > 15)
		{
			...
			{
				if (cheat_room)
					msg_print("Terrain Streamer: Deep water");
	...
	else if ((p_ptr->depth > 19) && (randint(20) > 15))
	{
		...
		{
			if (cheat_room)
				msg_print("Terrain Streamer: Shallow lava");
		...
		if (randint(20) > 15)
		{
			...
			{
				if (cheat_room)
					msg_print("Terrain Streamer: Deep lava");
...
void generate_cave(void)
{
	...
	/* A user sent me a savefile (Raille), and oldpy/oldpx were zero! */
	if (!in_bounds_fully(p_ptr->py, p_ptr->px))
	{
		plog("Illegal player position corrected!");
		new_player_spot();
	}
