...
void do_curse (artifact_type *a_ptr)
{
    ...
    /* Occationally make something really nasty */
    if (rand_int (8) == 0) a_ptr->flags3 |= TR3_TM_CURSE;
}
...
s32b artifact_power (artifact_type *a_ptr)
{
    ...
    if (a_ptr->flags3 & TR3_INVIS) p += 15;
    ...
    if (r < 5)      /* Pick something dependent on item type. */
    {
        ...
        switch (a_ptr->tval)
        {
            ...
            case TV_SWORD:
            {
                ...
                {
                    /* Was TR1_BLOWS */
                    /* Add well balanced if basic type
                     * is a throwing weapon */
                    if (a_ptr->flags3 & TR3_THROWING)
                    {
                        a_ptr->flags3 |= TR3_BALANCED;
                    }
                }
                else if (r < 89)
                {
            ...
            case TV_CLOAK:
            {
                ...
                else if (r < 80) a_ptr->flags3 |= TR3_INVIS;
                else a_ptr->to_a += 3 + rand_int (3);
                break;
            }
            case TV_SOFT_ARMOR:
