...
void resurrect_borg(void)
{
       ...
       auto_respawn_race == -2)
   {
    ...
    {
       /* random dragon race */
       if (auto_respawn_race == -2)
       {
           p_ptr->prace = rand_range(RACE_MIN_DRAGON,MAX_RACES);
       }
       else /* any race */
       {
...
void borg_log_death(void)
{
   ...
   fprintf(borg_log_file, "DBorg Compile Date: %s\n", borg_engine_date);
...
#ifdef BORG_TK
    path_build(buf, 1024, BORG_DIR_DATA, "dborg.dat");
#else /* BORG_TK */
   path_build(buf, 1024, ANGBAND_DIR_USER, "dborg.dat");
#endif /* BORG_TK */
                           ...
                           int power)
{
    if ((class_num >= MAX_CLASS &&
        ...
        range_to < range_from)
    {
        borg_note("Malformed item power in dborg.txt: values out of range");
        return FALSE;
    }
...
static bool borg_load_power(char * string)
{
    ...
    {
        borg_note("Malformed item power in dborg.txt: missing _CLASS");
        return FALSE;
    }
    if (-1000 == (depth_num = borg_getval(&string, "_DEPTH")))
    {
        borg_note("Malformed item power in dborg.txt: missing _DEPTH");
        return FALSE;
    }
    ...
    {
        borg_note("Malformed item power in dborg.txt: missing _RANGE");
        return FALSE;
    }
    if (-1000 == (range_to = borg_getval(&string, "TO")))
    {
        borg_note("Malformed item power in dborg.txt: messed up _RANGE");
        return FALSE;
    }
    ...
    if (-1000 != (item_num = borg_getval(&string, "_FORMULA")))
    {
        ...
        {
            borg_note("Malformed item power in dborg.txt: range must be 0-999 formulas");
            return FALSE;
        }
    ...
    borg_note("Malformed item power in dborg.txt");
    return FALSE;
}
static bool add_required_item(int class_num, int depth_num, int item_num, int number_items)
{
    if ((class_num >= MAX_CLASS &&
        ...
        item_num >= (MAX_K_IDX + MAX_K_IDX + MAX_A_IDX + BI_MAX))
    {
        borg_note("Malformed item requirment in dborg.txt: value out of range");
        return FALSE;
    }
    /* The class 999 is for all classes */
    if (class_num == 999)
...
static bool borg_load_requirement(char * string)
{
    ...
    if (-1000 == (class_num = borg_getval(&string, "_CLASS")))
    {
        borg_note("Malformed item requirment in dborg.txt");
        return FALSE;
    }
    if (-1000 == (depth_num = borg_getval(&string, "_DEPTH")))
    {
...
void init_borg_txt_file(void)
{
...
#else /* BORG_TK */
    path_build(buf, 1024, ANGBAND_DIR_USER, "dborg.txt");
#endif /* BORG_TK */
    ...
    {
        /* Complain */
        msg_print("*****WARNING***** You do not have a proper dborg.txt file!");
        msg_print("Make sure dborg.txt is located in the 'user' subdirectory!");
        msg_print(NULL);
...
void borg_init_9(void)
{
    ...
    /* Message */
    prt("Initializing the Borg... (dborg.txt)", 0, 0);
    init_borg_txt_file();
...
void borg_write_map(bool ask)
{
   ...
   fprintf(borg_map_file, "DBorg Compile Date: %s\n", borg_engine_date);
...
void do_cmd_borg(void)
{
    ...
    switch (cmd)
    {
        ...
        case '$':
        {
            ...
            /* Message */
            borg_note("Reloading the Borg rules... (dborg.txt)");
            for (i = 0; i < MAX_CLASS; i++)
            {
        ...
        case 'H':
        {
            ...
            for (;item < to; item++)
            {
                ...
                switch (cmd)
                {
                    ...
                    case 'w':
                    case 'W':
                        msg_format("WItem%03d value= %d", item-MAX_K_IDX, borg_has[item]);
                        break;
                    case 'a':
                    case 'A':
                        msg_format("Artifact%03d value= %d", item-MAX_K_IDX-MAX_K_IDX, borg_has[item]);
                        break;
                    default:
...
struct
{
    ...
    {"auto_borg_message",
        ...
        NULL, (long) &auto_borg_message},
    {"auto_flag_save",
        "Save the game when the dungeon level changes",
        NULL, (long) &auto_flag_save},
    {"auto_stop_king",
        "Stop the borg on win",
        NULL, (long) &auto_stop_king},
    {"borg_worships_damage",
