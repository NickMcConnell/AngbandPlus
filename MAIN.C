...
extern int rating;
short show_rating = FALSE;  // A rather blatant hack for the check
                            // to show  a newly created
                            // character his/her life rating...
                            // I think he really deserves to know it - TY
...
/* erase the "(saved)" entry from the highscore list, otherwise it will
   ...
   successful character, which would be bad... -CFT */
	...
	u_list[i].exist=0, u_list[i].dead=0;
      create_character();
       /* Character's initial speed is now 9. (formerly was 0.) */
       py.flags.speed = -9;
      show_rating = TRUE;
      char_inven_init();
      py.flags.food = 7500;
