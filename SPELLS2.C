...
/*
 * Range of all detection spells. Perhaps this should be variable.
 * Doesn't apply to telepathy.
 */

#define DETECT_RADIUS   30
...
void self_knowledge(void)
{
    ...
    {
        info[i++] = "The heavens around you are sealed.";
    }
    if (p_ptr->confusing)
    {
    ...
    if (p_ptr->invis)
    {
        info[i++] = "You are currently invisible.";
    }
    ...
    {
        info[i++] = format("You are resistant to disenchantment. %d", p_ptr->resist_disen);
    }
 ...
 * Fail% failure to detect a trap
 */
bool detect_traps(int fail)
{
    int y, x;
    int r = DETECT_RADIUS;
    ...
    for (y = (y1 - r); y <= (y1 + r); y++)
    {
        for (x = (x1 - r); x <= (x1 + r); x++)
        {
            ...
            /* Ignore things outside this */
            if (k > r) continue;

            /* Miss rate */
            if (randint(100) < fail) continue;
...
bool detect_objects_gold(void)
{
    ...
    for (i = 1; i < o_max; i++)
    {
        ...
        if (distance(y, x, p_ptr->py, p_ptr->px) > r) continue;
 ...
 * Fail% monsters undetected.
 */
bool detect_monsters_normal(int fail)
{
    int y, x;
    int r = DETECT_RADIUS;
    ...
    for (y = (y1 - r); y <= (y1 + r); y++)
    {
        for (x = (x1 - r); x <= (x1 + r); x++)
        {
            ...
            /* Ignore player and empty grids */
            if (m_idx < 1) continue;

            /* Otherwise get pointer to monster */
            m_ptr = &m_list[m_idx];
...
/*
 * Dispel monsters, teleport them around,
 * earthquake and freeze them briefly.
 */
void great_storm(int dam)
{
    /* Damage them */
    dispel_monsters(dam);

    /* Phase door them */
    project_hack(GF_AWAY_ALL, 10);

    /* Earthquake */
    earthquake(p_ptr->py, p_ptr->px, 10);

    /* Finally freeze them */
    project_hack(GF_PARALYSE, 2);
}


/*
 * Confuse all monsters in LOS
 */
...
/*
 * Teleport all nearby monsters
 */
bool scattering(void)
{
    int i;
    ...
    for (i = 1; i < m_max; i++)
    {
        ...
        /* Hack -- Smaller teleport on uniques */
...
bool probing(void)
{
    ...
    for (i = 1; i < m_max; i++)
    {
        ...
        if (m_ptr->ml)
        {
            ...
            msg_format("%^s has %d (of %d) mana points.", m_name, m_ptr->mana, m_ptr->maxmana);
    ...
    for (y = (y1 - r); y <= (y1 + r); y++)
    {
        for (x = (x1 - r); x <= (x1 + r); x++)
        {
            ...
            if (cave_valid_bold(y, x))
            {
                ...
                /* If the tile is destroyed, remove
                 * any monsters */
                if (feat != FEAT_FLOOR)
                {
                    delete_monster(y, x);
                }
                /* If a monster survived */
                if (cave_m_idx[y][x] > 0)
                {
                    ...
                    /* severely hurt the monster */
                    /* Apply damage directly */
                    m_ptr->hp -= damroll(30, 30);
                ...
                if (!(r_ptr->flags2 & (RF2_KILL_WALL)) &&
                    ...
                    /* Teleport it a little if it is still there. */
                    if (cave_m_idx[y][x] > 0) teleport_away(cave_m_idx[y][x], 15);
...
bool fire_ball(int typ, int dir, int dam, int rad)
{
    ...
    /* --Hack-- Meteor hurts the player
     * Perhaps all ball spells should? */
    flg |= PROJECT_PLAY;
...
bool jam_doors(void)
{
    ...
    return (project(-1, 1, py, px, 0, GF_JAM_DOOR, flg, 0, 0));
}
...
/* Note: Range was MAX_SIGHT*5. MAX_SIGHT = 20 */
bool teleport_monster(int dir)
{
    int flg = PROJECT_BEAM | PROJECT_KILL;
    return (project_hook(GF_AWAY_ALL, dir, MAX_SIGHT + p_ptr->lev, flg));
}
