...
void do_cmd_change_name(void)
{
	...
	while (1)
	{
		...
		/* Prompt */
		Term_putstr(2, screen_y-2, -1, TERM_WHITE, p);
...
void do_cmd_messages(void)
{
	...
	while (1)
	{
		...
		for (j = 0; (j < screen_y-5) && (i + j < n); j++)
		{
			...
			Term_putstr(0, screen_y-4-j, -1, TERM_WHITE, msg);
			...
			if (shower[0])
			{
				...
				while ((str = strstr(str, shower)) != NULL)
				{
					...
					/* Display the match */
					Term_putstr(str-msg, screen_y-4-j, len, TERM_YELLOW, shower);
		...
		prt("[Press 'p' for older, 'n' for newer, ..., or ESCAPE]", screen_y-2, 0);
...
void do_cmd_note(void)
{
	...
	/* 
	 * Make "@info" expand to turns, experience, level,
	 * depth, etc.
	 */
	if (!stricmp(tmp, "@info"))
	{
		/* Get depth in preferred format */
		if (depth_in_feet)
		{
			sprintf(buf, "%d ft", p_ptr->depth * 50);
		}
		else
		{
		...
		sprintf(tmp, "Turn: %lu, Exp: %lu, Max Exp: %lu, Lvl: %d, Depth %s, Gold: %lu",
			(long)turn, (long)p_ptr->exp, (long)p_ptr->max_exp,
			p_ptr->lev, buf, (long)p_ptr->au);
	}

	/* Add the note to the message recall */
	msg_format("Note: %s", tmp);

	/* -TM- Append notes to file */
	sprintf(buf, "%s.txt", op_ptr->base_name);

	if (get_string("File name: ", buf, 80))
	{
		if (buf[0] && (buf[0] != ' '))
		{
			/* Drop priv's */
			...
			/* Build the filename */
			path_build(filename, 1024, ANGBAND_DIR_USER, buf);

			/* Does it already exist? */
			if (!(fff = my_fopen(filename, "r")))
			{
				/* Okay, close it */
				my_fclose(fff);
				...
				/* With first dumps we give extra info */
				sprintf(buf, "Character Log: %s the %s %s\n",
					op_ptr->full_name,
			...
			fff = my_fopen(filename, "a");

			/* Grab priv's */
			safe_setuid_grab();
			
			/* Jump to end of file */
			x = fseek(fff, 0, SEEK_END);

			/* With first dump we give extra info */
			if (x==1)
			{
				sprintf(buf, "Character Log: %s the %s %s\n",
					op_ptr->full_name,
