...
void do_cmd_eat_food(void)
{
    ...
    switch (o_ptr->sval)
    {
        case SV_FOOD_POISON:
        {
            ...
            if (set_poisoned(p_ptr->poisoned + ((rand_int(20) + 20)*x)/100))
            {
                    ident = TRUE;
            }
            break;
...
void do_cmd_quaff_potion(void)
{
    ...
    switch (o_ptr->sval)
    {
        ...
        case SV_POTION_STAR_HEALING:
        {
            ...
            if (set_blind(0)) ident = TRUE;
            if (set_nasty_poisoned(FALSE)) ident = TRUE;
            if (set_confused(0)) ident = TRUE;
            if (set_poisoned(0)) ident = TRUE;
            if (set_stun(0)) ident = TRUE;
            if (set_cut(0)) ident = TRUE;
...
void do_cmd_read_scroll(void)
{
    ...
    switch (o_ptr->sval)
    {
        ...
        {
            if (detect_traps(0)) ident = TRUE;
            break;
        }

        case SV_SCROLL_DETECT_DOOR:
        {
            if (detect_doors(0)) ident = TRUE;
            if (detect_stairs(0)) ident = TRUE;
            break;
        }
        ...
        {
            acquirement(py, px, randint(2) + 1, TRUE, TRUE);
            ident = TRUE;
            break;
        }
    }
...
void do_cmd_aim_wand(void)
{
    ...
    switch (sval)
    {
        ...
        {
            fire_bolt(GF_FORCE, dir, damroll(6, 9));
            ident = TRUE;
            break;
        }
        ...
        case SV_WAND_CREATE_WALL:
        {
            if (!get_aim_dir(&dir)) return;
            if (fire_arc(GF_MAKE_WALL, dir, 0, 1, 45)) ident = TRUE;
            break;
        }
    }
...
void do_cmd_activate(void)
{
    ...
    /* -TM- Hack -- Elemental rings may be activated as well */
    if (o_ptr->tval == TV_RING)
    {
        /* Branch on the sub_type */
        switch (o_ptr->sval)
        {
            case SV_RING_FLAMES:
            {
                msg_print("Your ring is covered in fire!");
            ...
            case SV_RING_ICE:
            {
                msg_print("Your ring is covered in frost!");
                set_oppose_cold(p_ptr->oppose_fire + randint(20) + 20);
                o_ptr->timeout = rand_int(10) + 10;
                break;
            }
            ...
            {
                msg_print("Your ring crackles with energy!");
                set_oppose_elec(p_ptr->oppose_fire + randint(20) + 20);
                o_ptr->timeout = rand_int(10) + 10;
                break;
            }
        }
