...
static void wr_extra(void)
{
	...
	wr_s16b(p_ptr->temp_invis);
	wr_byte(p_ptr->nasty_pois);
    for (i=0; i<34; i++) wr_byte(p_ptr->drang_reserved[i]);

    wr_byte(p_ptr->confusing);
    wr_s16b(p_ptr->ethereal_lock); /* oops */
	wr_byte(0);	/* oops */
	wr_byte(0);	/* oops */
	wr_byte(p_ptr->searching);
	wr_byte(p_ptr->maximise);
