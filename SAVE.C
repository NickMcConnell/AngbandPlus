...
static int sv_write()
{
  ...
  if (no_haggle_flag)
    l |= 0x400L;
#ifdef TC_COLOR
  if ((no_color_flag) && !(invulner_flag))
    l |= 0x800L;
#endif
#endif    
  if (!carry_query_flag)
...
*/
  ...
  wr_byte((int8u)c_list[MAX_CREATURES - 1].gchar); /*Let's save the
                graphic character, too! -TY*/
...
#ifndef MSDOS
    no_graphics_flag = TRUE;  /* I don't think this will work on other systems
                                   -TY */
#endif
   if (l & 0x20000L)
    equippy_chars = TRUE;
...
/*         t_ptr->gchar = wallsym;*/ }
      ...
      rd_byte((int8u *)&(c_list[MAX_CREATURES - 1].gchar));
      /* Get the graphic character - TY */
...
register monster_type *mon;
{
  ...
  rd_byte(&mon->color);
#endif
#if 0 /* This should be no longer necessary as I'm saving the maxhp now - TY */
