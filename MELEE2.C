...
static void bolt(int m_idx, int typ, int dam_hp)
{
	...
	if (!(r_ptr->flags2 & (RF2_SEE_INVIS)) && (p_ptr->invis))
    {
        ...
        miss_x = ((rand_int(p_ptr->invis)) * sin_x);
        miss_y = ((rand_int(p_ptr->invis)) * sin_y);
...
static void breath(int m_idx, int typ, int dam_hp, int degrees_of_arc)
{
    ...
    if (!(r_ptr->flags2 & (RF2_SEE_INVIS)) && (p_ptr->invis))
    {
        ...
        miss_x = ((rand_int(p_ptr->invis)) * sin_x);
        miss_y = ((rand_int(p_ptr->invis)) * sin_x);
	...
	switch (thrown_spell)
	{
		...
		case 96+4:
		{
			...
			if (!rand_int(2))
			{
				...
				apply_magic(&object_temp, p_ptr->depth, FALSE, FALSE, FALSE);
