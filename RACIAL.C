			...
			if (command == -2)
			{
				if (racial_aux(25, ((3*plev/2)>4 ? (3*plev/2) : 4), A_CON, 6))
				{
					...
					msg_format("You breathe shards!");
                  			fire_arc(GF_SHARD, dir, p_ptr->chp/3, (plev / 10) + 5, 30);
				}
			}
            else if (command == -1)
			{
			...
			if (command == -2)
			{
				if (racial_aux(25, ((3*plev/2)>4 ? (3*plev/2) : 4), A_CON, 6))
				{
					...
					msg_format("You breathe disenchantment!");
                  			fire_arc(GF_DISENCHANT, dir, p_ptr->chp/3, (plev / 10) + 5, 30);
				}
			}
            else if (command == -1)
			{
			...
			if (command == -2)
			{
				if (racial_aux(25, ((3*plev/2)>4 ? (3*plev/2) : 4), A_CON, 6))
				{
					...
					msg_format("You breathe confusion!");
                  			fire_arc(GF_CONFUSION, dir, p_ptr->chp/3, (plev / 10) + 5,30);
				}
			}
            else if (command == -1)
			{
			...
			if (command == -2)
			{
				if (racial_aux(25, ((3*plev/2)>4 ? (3*plev/2) : 4), A_CON, 6))
				{
					...
					msg_format("You breathe sound!");
                  			fire_arc(GF_SOUND, dir, p_ptr->chp/3, (plev / 10) + 5,30);
				}
			}
                        else if (command == -1)
			{
			...
			if (command == -5)
			{
				if (racial_aux(45, ((plev*3/2)>4 ? (plev*3/2) : 4), A_CON, 30))
				{
					...
					msg_format("You breathe poison!");
                  			fire_arc(GF_POIS, dir, (p_ptr->chp*10)/36, (plev / 10) + 5,30);
				}
			}
			...
			if (command == -4)
			{
				if (racial_aux(35, (plev>4 ? plev : 4), A_CON, 20))
				{
					...
					msg_format("You breathe fire!");
                  			fire_arc(GF_FIRE, dir, (p_ptr->chp*10)/30, (plev / 10) + 5,30);
				}
			}
			...
			else if (command == -3)
			{
				if (racial_aux(25, ((plev*2/3)>4 ? (plev*2/3) : 4), A_CON, 12))
				{
					...
					msg_format("You breathe ice!");
                  			fire_arc(GF_ICE, dir, (p_ptr->chp*10)/35, (plev / 10) + 5,30);
				}
			}
			...
			else if (command == -2)
			{
				if (racial_aux(15, ((plev/2)>4 ? (plev/2) : 4), A_CON, 6))
				{
					...
					msg_format("You breathe electricity!");
                  			fire_arc(GF_ELEC, dir, (p_ptr->chp*10)/40, (plev / 10) + 5,30);
				}
			}
			...
			else if (command == -1)
			{
				if (racial_aux(5, ((plev/3)>4 ? (plev/3) : 4), A_CON, 3))
				{
					...
					msg_format("You breathe acid!");
                  			fire_arc(GF_ACID, dir, (p_ptr->chp*10)/45, (plev / 10) + 5,30);
				}
			}
			break;
		}
...
void do_cmd_racial_power(void)
{
	...
	switch (p_ptr->prace)
	{
		...
		case RACE_PIXIE:
            ...
            sprintf(power_desc[1].name, "phase door(5)");
