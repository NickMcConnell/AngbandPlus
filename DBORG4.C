...
static void borg_notice_aux1(void)
{
    ...
    borg_skill[BI_THN] = rb_ptr->r_thn[wield_skill] + cb_ptr->c_thn[wield_skill];
    ...
    if (f3 & (TR3_INVIS)) borg_skill[BI_INVIS] = TRUE;
    ...
    /* Resistance flags */
    if (f2 & (TR2_RES_ACID)) borg_skill[BI_RACID] = 1;
    if (f2 & (TR2_RES_ELEC)) borg_skill[BI_RELEC] = 1;
    if (f2 & (TR2_RES_FIRE)) borg_skill[BI_RFIRE] = 1;
    if (f2 & (TR2_RES_COLD)) borg_skill[BI_RCOLD] = 1;
    if (f2 & (TR2_RES_POIS)) borg_skill[BI_RPOIS] = 1;
    if (f2 & (TR2_RES_FEAR)) borg_skill[BI_RFEAR] = 1;
    if (f2 & (TR2_RES_LITE)) borg_skill[BI_RLITE] = 1;
    if (f2 & (TR2_RES_DARK)) borg_skill[BI_RDARK] = 1;
    if (f2 & (TR2_RES_BLIND)) borg_skill[BI_RBLIND] = 1;
    if (f2 & (TR2_RES_CONFU)) borg_skill[BI_RCONF] = 1;
    if (f2 & (TR2_RES_SOUND)) borg_skill[BI_RSND] = 1;
    if (f2 & (TR2_RES_SHARD)) borg_skill[BI_RSHRD] = 1;
    if (f2 & (TR2_RES_NEXUS)) borg_skill[BI_RNXUS] = 1;
    if (f2 & (TR2_RES_NETHR)) borg_skill[BI_RNTHR] = 1;
    if (f2 & (TR2_RES_CHAOS)) borg_skill[BI_RKAOS] = 1;
    if (f2 & (TR2_RES_DISEN)) borg_skill[BI_RDIS] = 1;
    ...
    for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
    {
        ...
        /* Two handed weapons are not good borg weapons */
        if (item->flags3 & TR3_2HANDS) continue;
        ...
        if (item->flags3 & TR3_INVIS) borg_skill[BI_INVIS] = TRUE;
        ...
        if (item->flags2 & TR2_IM_FIRE)
        {
            ...
            borg_skill[BI_RFIRE] += 10;
        }
        if (item->flags2 & TR2_IM_ACID)
        {
            borg_skill[BI_IACID] = TRUE;
            borg_skill[BI_RACID] += 10;
        }
        if (item->flags2 & TR2_IM_COLD)
        {
            borg_skill[BI_ICOLD] = TRUE;
            borg_skill[BI_RCOLD] += 10;
        }
        if (item->flags2 & TR2_IM_ELEC)
        {
            borg_skill[BI_IELEC] = TRUE;
            borg_skill[BI_RELEC] += 10;
        }

        /* Resistance flags */
        if (item->flags2 & TR2_RES_ACID) borg_skill[BI_RACID] ++;
        if (item->flags2 & TR2_RES_ELEC) borg_skill[BI_RELEC] ++;
        if (item->flags2 & TR2_RES_FIRE) borg_skill[BI_RFIRE] ++;
        if (item->flags2 & TR2_RES_COLD) borg_skill[BI_RCOLD] ++;
        if (item->flags2 & TR2_RES_POIS) borg_skill[BI_RPOIS] ++;
        if (item->flags2 & TR2_RES_CONFU) borg_skill[BI_RCONF] ++;
        ...
        if (item->flags2 & TR2_RES_DISEN) borg_skill[BI_RDIS] ++;
        if (item->flags2 & TR2_RES_SHARD) borg_skill[BI_RSHRD] ++;
        if (item->flags2 & TR2_RES_NEXUS) borg_skill[BI_RNXUS] ++;
        if (item->flags2 & TR2_RES_BLIND) borg_skill[BI_RBLIND] ++;
        if (item->flags2 & TR2_RES_NETHR) borg_skill[BI_RNTHR] ++;
    ...
    /* Temporary Resists */
    if (my_oppose_acid) borg_skill[BI_RACID] ++;
    if (my_oppose_elec) borg_skill[BI_RELEC] ++;
    if (my_oppose_cold) borg_skill[BI_RCOLD] ++;
    if (my_oppose_fire) borg_skill[BI_RFIRE] ++;
    if (my_oppose_pois) borg_skill[BI_RPOIS] ++;
    if (borg_prot_from_evil) borg_skill[BI_RNTHR] ++;
    ...
    borg_skill[BI_THN] +=(cb_ptr->x_thn[wield_skill] * auto_max_level / 10);
...
static void borg_notice_aux2(void)
{
    ...
    /* Handle Diggers */
    if (borg_spell_legal_fail(7, 3, 40))
    {
        amt_digger += 1;
    }
...
#if 0 /* no teleport level in Dr yet */
    ...
    if ((borg_prayer_okay_fail(1, 1, 1) || borg_prayer_okay_fail(4, 1, 1) ||
          borg_spell_okay_fail(1, 4, 1) || borg_spell_okay_fail(2, 6, 1)) &&
    ...
    if (borg_spell_legal_fail(7, 5, 4) && borg_skill[BI_RBLIND] &&
...
void borg_notice_weapon_swap(void)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        /* Two handed weapons are not good swaps */
        if (item->flags3 & TR3_2HANDS) continue;
       ...
       /* Clear all the swap weapon flags as I look at each one. */
        ...
        switch (item->tval)
        {
            ...
            if (item->flags1 & TR1_TUNNEL)
            {
                ...
                if (item->value <= 0) break;
                if (item->cursed) break;
                if (!borg_spell_okay_fail(7, 3, 40) &&
...
void borg_notice_armour_swap(void)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        /* Shields are not good swaps if sword is two handed*/
...
static s32b borg_power_aux2(void)
{
    ...
    /* Hack - borg does not yet handle 2handed weapons */
    if (auto_items[INVEN_WIELD].flags3 & TR3_2HANDS) value -= 9999999;
    ...
    if ((do_hungry || do_weak) && borg_skill[BI_FOOD]) value += 900000;
...
static s32b borg_power_aux3(void)
{
    ...
    value += (borg_skill[BI_CUR_LITE] * 100000L);
        ...
        (my_stat_ind[A_INT] <= 37 ))
    {
        ...
        if (auto_class == CLASS_MAGE)
        {
            ...
            if (borg_spell_legal(7,5)) value += my_stat_ind[A_INT] * 35000L;
        ...
        (my_stat_ind[A_WIS] <= 37 ))
    {
        ...
        if (auto_class == CLASS_PRIEST)
        {
            ...
            if (borg_prayer_legal(3, 7)) value += my_stat_ind[A_WIS] * 35000L;
    ...
    if (borg_skill[BI_INVIS] && auto_depth <= 50) value += 50000L;
    if (borg_skill[BI_INVIS] && auto_depth >= 51) value += 10000L;
     ...
     */
     if (borg_skill[BI_SPEED] >= 140)
     {
        ...
        if (borg_skill[BI_RACID] >= 2) value  += 12000L * borg_skill[BI_RACID]-1;
        if (borg_skill[BI_RELEC] >= 2) value  += 15000L * borg_skill[BI_RELEC]-1;
        if (borg_skill[BI_RCOLD] >= 2) value  += 17000L * borg_skill[BI_RCOLD]-1;
        if (borg_skill[BI_RFIRE] >= 2) value  += 20000L * borg_skill[BI_RFIRE]-1;
     }
    ...
    if (borg_worships_ac)
    {
        ...
        if (borg_skill[BI_ARMOR] >= 15 && borg_skill[BI_ARMOR] < 75) value += ((borg_skill[BI_ARMOR]) * 2000L);
        if (borg_skill[BI_ARMOR] >= 75) value += ((borg_skill[BI_ARMOR]) * 1500L);
    }
    else
    {
        if (borg_skill[BI_ARMOR] < 15)   value += ((borg_skill[BI_ARMOR]) * 2000L);
        if (borg_skill[BI_ARMOR] >= 15 &&
            borg_skill[BI_ARMOR] < 75)   value += ((borg_skill[BI_ARMOR]) * 1500L);
        if (borg_skill[BI_ARMOR] >= 75)  value += ((borg_skill[BI_ARMOR]) * 500L);
    }
...
static s32b borg_power_aux4(void)
{
    ...
    /* Reward food */
    k = 0;
    for (; k < 8 && k < borg_skill[BI_FOOD]; k++) value += 50000L;
    for (; k < 15 && k < borg_skill[BI_FOOD]; k++) value += 200L;
...
static s32b borg_power_home_aux2(void)
{
    ...
    /* Collect food */
    for (k = 0; k < 99 && k < num_food; k++) value += 8000L - k*10L;
...
static int borg_danger_aux1(int i)
{
    ...
    for (k = 0; k < 4; k++)
    {
        ...
        switch (b_ptr->effect)
        {
            ...
            z = (b_ptr->d_dice * b_ptr->d_side);
            if (borg_skill[BI_RACID]) z /= (2 * borg_skill[BI_RACID]);
            /* if invulnurable, no damage */
            if ((borg_goi) && !borg_attacking)
                z = z * 2/3;;
            if (!borg_full_damage)
            ...
            if ((borg_goi) && !borg_attacking)
                z = z * 2/3;;
            if (borg_skill[BI_RELEC]) z /= (2 * borg_skill[BI_RELEC]);
            if (!borg_full_damage)
                z += 10;
            if ((pfe) && !borg_attacking)
                z /= 2;
            ...
            if ((borg_goi) && !borg_attacking)
                z = z * 2/3;;
            if (borg_skill[BI_RFIRE]) z /= (2 * borg_skill[BI_RFIRE]);
            if (!borg_full_damage)
                z += 20;
            if ((pfe) && !borg_attacking)
                z /= 2;
            ...
            if ((borg_goi) && !borg_attacking)
                z = z * 2/3;;
            if (borg_skill[BI_RCOLD]) z /= (2 * borg_skill[BI_RCOLD]);
            if (!borg_full_damage)
                z += 15;
            if ((pfe) && !borg_attacking)
                z /= 2;
    ...
    /* if invulnurable, very little damage 25% */
    if (borg_goi) n = (n*25/100);
    ...
    for (q = 0; q < num; q++)
    {
        ...
        switch (spell[q])
        {
            ...
            if (z > 800)
                z = 800;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RPOIS]) z /= (2 * borg_skill[BI_RPOIS]);
            if (my_oppose_pois) break;
            if (borg_skill[BI_RPOIS]) break;
            /* if looking at full damage, things that are just annoying */
            /* do not count. */
            ...
            if (z > 400)
                z = 400;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RDARK]) z /= (2 * borg_skill[BI_RDARK]);
            ...
            if (z > 400)
                z = 400;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RCONF]) z /= (2 * borg_skill[BI_RCONF]);
            if (borg_skill[BI_RCONF]) break;
            /* if looking at full damage, things that are just annoying */
            /* do not count. */
            if (!borg_full_damage)
            ...
            if (z > 400)
                z = 400;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RSND]) z /= (2 * borg_skill[BI_RSND]);
            ...
            if (z > 600)
                z = 600;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RKAOS]) z /= (2 * borg_skill[BI_RKAOS]);
            if (!borg_full_damage)
                p += 100;
            ...
            if (z > 500)
                z = 500;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RDIS]) z /= (2 * borg_skill[BI_RDIS]);
            if (borg_skill[BI_RDIS]) break;
            p += 500;
            break;
            ...
            if (z > 250)
                z = 250;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RNXUS]) z /= (2 * borg_skill[BI_RNXUS]);
            ...
            if (z > 400)
                z = 400;
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RSHRD]) z /= (2 * borg_skill[BI_RSHRD]);
            ...
            z = (50 +50 + (10 * 10) + lev);
            if (borg_goi) z /=3;;
            if (borg_skill[BI_RNTHR]) z /= (2 * borg_skill[BI_RNTHR]);
...
int borg_danger_aux(int y, int x, int c, int i)
{
    ...
    if (!(r_ptr->flags2 & RF2_SEE_INVIS) &&
        (my_inviso || borg_skill[BI_INVIS]))
    {
        v1 = v1*8/10;
    }
...
cptr borg_prepared(int depth)
{
      ...
      /* unless all the uniques are dead. */
    {
          ...
          if (auto_max_depth == 99 && depth <= 98 &&
              (rand_int(100) < 3 ||
               borg_prayer_okay_fail(4, 6, 20) ||
               borg_spell_okay_fail(5, 6, 20)))
          {
              if (borg_scums_uniques && auto_max_level >= 35) auto_scum = TRUE;
              return ((cptr)NULL);
          }
